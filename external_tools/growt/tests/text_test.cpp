...
int push_file(Hash& hash, std::ifstream& in_stream, [[maybe_unused]] size_t id)
{
    ...
    while (!in_stream.eof())
    {
        ...
        // auto lout =
        // otm::locally_buffered_output<otm::output_type>(otm::out());
        if (start > 0)
        {
...
struct test_in_stages
{
    static int execute(ThreadType t, size_t cap, size_t it, std::string file)
    {
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            {
                auto duration =
                    t.synchronized(push_file<handle_type>, hash, in_file, t.id);
...
int main(int argn, char** argc)
{
    ...
    otm::out() << otm::width(5) << "#i" << otm::width(5) << "p"
               << otm::width(11) << "cap" << otm::width(12) << "time"
               << otm::width(9) << "words" << otm::width(9) << "uniques"
               << "    " << text_config::name() << std::endl;
