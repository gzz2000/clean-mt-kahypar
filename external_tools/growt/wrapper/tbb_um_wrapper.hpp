...
template <class Key, class Data, class Hasher, class Alloc> class tbb_um_wrapper
{
  ...
  public:
    ...
    template <class... Args> inline insert_return_type emplace(Args&&... args);
    ...
    template <class F, class... Types>
    inline insert_return_type
    emplace_or_update(key_type&& k, mapped_type&& d, F, Types&&... args);
...
class tbb_um_config
{
  public:
    ...
    static_assert(is_viable,
                  "tbb um wrapper does not support the chosen flags");

    using table_type =
        tbb_um_wrapper<key_type, mapped_type, hash_fct_type, allocator_type>;
...
tbb_um_wrapper<K, D, HF, AL>::insert(const key_type& k, const mapped_type& d)
{
    ...
    // return (ret) ? ReturnCode::SUCCESS_IN :
...
template <class F, class... Types>
typename tbb_um_wrapper<K, D, HF, AL>::insert_return_type
tbb_um_wrapper<K, D, HF, AL>::insert_or_update(const key_type&    k,
...
template <class F, class... Types>
typename tbb_um_wrapper<K, D, HF, AL>::insert_return_type
tbb_um_wrapper<K, D, HF, AL>::update_unsafe(const key_type& k, F f,
                                            Types&&... args)
{
...
template <class F, class... Types>
typename tbb_um_wrapper<K, D, HF, AL>::insert_return_type
tbb_um_wrapper<K, D, HF, AL>::insert_or_update_unsafe(const key_type&    k,
                                                      const mapped_type& d, F f,
                                                      Types&&... args)
{
...
template <class K, class D, class HF, class AL>
size_t tbb_um_wrapper<K, D, HF, AL>::erase(const key_type&)
{
