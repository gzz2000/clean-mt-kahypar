...
template <class Key, class Hasher> class hash_comparison
{
  private:
    using key_type = Key;
    Hasher fct;
  ...
  public:
    ...
    inline size_t hash(const key_type& k) const { return fct(k); }
};
...
class tbb_hm_config
{
  public:
    ...
    static_assert(is_viable,
                  "tbb hm wrapper does not support the chosen flags");

    using table_type =
        tbb_hm_wrapper<key_type, mapped_type, hash_fct_type, allocator_type>;
...
template <class F, class... Types>
typename tbb_hm_wrapper<K, D, HF, AL>::insert_return_type
tbb_hm_wrapper<K, D, HF, AL>::insert_or_update(const key_type&    k,
...
template <class F, class... Types>
typename tbb_hm_wrapper<K, D, HF, AL>::insert_return_type
tbb_hm_wrapper<K, D, HF, AL>::update_unsafe(const key_type& k, F f,
                                            Types&&... args)
{
...
template <class F, class... Types>
typename tbb_hm_wrapper<K, D, HF, AL>::insert_return_type
tbb_hm_wrapper<K, D, HF, AL>::insert_or_update_unsafe(const key_type&    k,
                                                      const mapped_type& d, F f,
                                                      Types&&... args)
{
...
template <class K, class D, class HF, class AL>
size_t tbb_hm_wrapper<K, D, HF, AL>::erase(const key_type& k)
{
    return hash.erase(k);
}
