...
void _concurrency_barrier(int threads = tbb_kahypar::task_arena::automatic) {
    if(threads == tbb_kahypar::task_arena::automatic)
        threads = tbb_kahypar::this_task_arena::max_concurrency();
    if(threads < 2)
        return;
