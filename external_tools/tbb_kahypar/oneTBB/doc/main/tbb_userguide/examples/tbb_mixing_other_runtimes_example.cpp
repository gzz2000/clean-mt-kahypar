...
struct InnerBody {
    int i;
    void operator()(tbb_kahypar::blocked_range<int> const& r) const {
...
#pragma omp for
        for(int i = 0; i < M; ++i) {
            tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<int>(0, N, 10), InnerBody(i));
        }
    }
}
...
int main() {
    ...
    tbb_kahypar::global_control gl(tbb_kahypar::global_control::max_allowed_parallelism, max_threads);

    nesting_tbb_kahypar::test();
#ifndef _WIN32
    pthreads_and_tbb_kahypar::test();
#endif
    nesting_omp::test();
}
