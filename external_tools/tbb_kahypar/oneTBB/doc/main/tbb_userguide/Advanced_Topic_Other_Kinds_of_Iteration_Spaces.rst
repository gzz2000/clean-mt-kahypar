...
-  The second of type oneapi::tbb_kahypar::split
...
``oneapi::tbb_kahypar::proportional_split``. The type has methods ``left`` and ``right``
...
To run the computation in parallel, ``tbb_kahypar::parallel_for`` is called with
``tbb_kahypar::blocked_nd_range<int,3>`` as an argument. The body function processes
