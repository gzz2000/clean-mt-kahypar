...
- The ``oneapi::tbb_kahypar::info`` namespace interfaces might unexpectedly change the process affinity mask on Windows* OS systems (see https://github.com/open-mpi/hwloc/issues/366 for details) when using hwloc version lower than 2.5.
...
## :hammer: Issues Fixed
- Fixed deadlock when using `tbb_kahypar::concurrent_vector::grow_by()` (https://github.com/uxlfoundation/oneTBB/issues/1531).
...
- Improved support of ``tbb_kahypar::task_arena::constraints`` functionality on Windows* systems with multiple processor groups.
