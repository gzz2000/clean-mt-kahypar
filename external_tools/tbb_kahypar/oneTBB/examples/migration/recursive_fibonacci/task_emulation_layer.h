...
struct task_group_pool {
    task_group_pool() : pool_size(tbb_kahypar::this_task_arena::max_concurrency()), task_submitters(new tbb_kahypar::task_group[pool_size]) {}
    ...
    tbb_kahypar::task_group& operator[] (std::size_t idx) { return task_submitters[idx]; }

    const std::size_t pool_size;
    tbb_kahypar::task_group* task_submitters;
};
...
private:
    template <typename F, typename... Args>
    friend F create_root_task(tbb_kahypar::task_group& tg, Args&&... args);

    template <typename F, typename... Args>
    friend F* allocate_root_task(tbb_kahypar::task_group& tg, Args&&... args);
...
public:
    root_task(tbb_kahypar::task_group& tg) : m_tg(tg), m_callback(m_tg.defer([] { /* Create empty callback to preserve reference for wait. */})) {
        add_child_reference();
...
private:
    ...
    tbb_kahypar::task_handle m_callback;
};

template <typename F, typename... Args>
F create_root_task(tbb_kahypar::task_group& tg, Args&&... args) {
    F obj{std::forward<Args>(args)...};
...
template <typename F, typename... Args>
F* allocate_root_task(tbb_kahypar::task_group& tg, Args&&... args) {
    F* obj = new F{std::forward<Args>(args)...};
...
template <typename F>
void run_task(F&& f) {
    tg_pool[tbb_kahypar::this_task_arena::current_thread_index()].run(std::forward<F>(f));
}

template <typename F>
void run_task(F* f) {
    tg_pool[tbb_kahypar::this_task_arena::current_thread_index()].run(std::ref(*f));
}

template <typename F>
void run_and_wait(tbb_kahypar::task_group& tg, F* f) {
