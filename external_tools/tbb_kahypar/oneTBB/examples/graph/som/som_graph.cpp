...
typedef oneapi::tbb_kahypar::flow::function_node<SOM_element, search_result_type> search_node;
typedef oneapi::tbb_kahypar::flow::broadcast_node<SOM_element> b_node;
typedef std::vector<search_node *> search_node_vector_type;
typedef std::vector<search_node_vector_type> search_node_array_type;
typedef std::vector<oneapi::tbb_kahypar::flow::graph *> graph_vector_type;
typedef std::vector<graph_vector_type> graph_array_type;

#define SPECULATION_CNT 2

oneapi::tbb_kahypar::flow::graph *g[SPECULATION_CNT]; // main graph; there should only be one per epoch
b_node *send_to[SPECULATION_CNT]; // broadcast node to send exemplar to all function_nodes
oneapi::tbb_kahypar::flow::queue_node<search_result_type>
...
oneapi::tbb_kahypar::flow::graph *construct_graph(oneapi::tbb_kahypar::task_arena &ta) {
    oneapi::tbb_kahypar::flow::graph *result;
    ta.execute([&result] {
        result = new oneapi::tbb_kahypar::flow::graph();
    });
    return result;
}
...
void build_BMU_graph(SOMap &map1, oneapi::tbb_kahypar::task_arena &ta) {
    // build current graph
    ...
    for (int scnt = 0; scnt < SPECULATION_CNT; ++scnt) {
        ...
        q[scnt] = new oneapi::tbb_kahypar::flow::queue_node<search_result_type>(*(g[scnt])); // output queue
        ...
        for (int i = 0; i < (int)map1.size(); i += xsize) {
            ...
            for (int j = 0; j < (int)map1[0].size(); j += ysize) {
                ...
                oneapi::tbb_kahypar::flow::graph *g_local = construct_graph(ta);
                search_node *s =
                    new search_node(*g_local, oneapi::tbb_kahypar::flow::serial, bb); // copies Body
                (*g_array[scnt])[xindex].push_back(g_local);
                (*s_array[scnt])[xindex].push_back(s);
                oneapi::tbb_kahypar::flow::make_edge(*(send_to[scnt]),
                                             *s); // broadcast_node -> function_node
                oneapi::tbb_kahypar::flow::make_edge(*s, *(q[scnt])); // function_node -> queue_node
            }
        }
    }
}
...
void graph_teach(SOMap &map1, teaching_vector_type &in, oneapi::tbb_kahypar::task_arena &ta) {
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        oneapi::tbb_kahypar::task_arena ta(1);
    ...
    for (int p = std::max(threads.first, 2); p <= std::max(threads.last, 2); ++p) {
        ...
        oneapi::tbb_kahypar::task_arena ta(p);
        if (extra_debug)
