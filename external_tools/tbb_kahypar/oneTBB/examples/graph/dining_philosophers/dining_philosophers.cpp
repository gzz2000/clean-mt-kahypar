...
oneapi::tbb_kahypar::tick_count t0;
bool verbose = false;
...
oneapi::tbb_kahypar::spin_mutex my_mutex;

class chopstick {};

typedef std::tuple<oneapi::tbb_kahypar::flow::continue_msg, chopstick, chopstick> join_output;
typedef oneapi::tbb_kahypar::flow::join_node<join_output, oneapi::tbb_kahypar::flow::reserving> join_node_type;

typedef oneapi::tbb_kahypar::flow::function_node<oneapi::tbb_kahypar::flow::continue_msg,
                                         oneapi::tbb_kahypar::flow::continue_msg>
    think_node_type;
typedef oneapi::tbb_kahypar::flow::function_node<join_output, oneapi::tbb_kahypar::flow::continue_msg>
    eat_node_type;
typedef oneapi::tbb_kahypar::flow::multifunction_node<oneapi::tbb_kahypar::flow::continue_msg, join_output>
    forward_node_type;
...
private:
    ...
    void think();
    void eat();
    void forward(const oneapi::tbb_kahypar::flow::continue_msg &in,
...
public:
    ...
    oneapi::tbb_kahypar::flow::continue_msg operator()(oneapi::tbb_kahypar::flow::continue_msg /*m*/) {
...
public:
    ...
    oneapi::tbb_kahypar::flow::continue_msg operator()(const join_output &in) {
        my_philosopher.eat();
        return oneapi::tbb_kahypar::flow::continue_msg();
    }
};
...
public:
    ...
    void operator()(const oneapi::tbb_kahypar::flow::continue_msg &in,
...
void philosopher::forward(const oneapi::tbb_kahypar::flow::continue_msg & /*in*/,
                          forward_node_type::output_ports_type &out_ports) {
    if (my_count < 0)
        abort();
    --my_count;
    ...
    if (my_count > 0) {
        (void)std::get<0>(out_ports).try_put(
            oneapi::tbb_kahypar::flow::continue_msg()); //start thinking again
    }
    else {
        if (verbose) {
            oneapi::tbb_kahypar::spin_mutex::scoped_lock lock(my_mutex);
...
typedef oneapi::tbb_kahypar::flow::queue_node<oneapi::tbb_kahypar::flow::continue_msg> thinking_done_type;
...
int main(int argc, char *argv[]) {
    ...
    oneapi::tbb_kahypar::tick_count main_time = oneapi::tbb_kahypar::tick_count::now();
    int num_threads;
    ...
    for (num_threads = options.threads.first; num_threads <= options.threads.last;
        ...
        oneapi::tbb_kahypar::global_control c(oneapi::tbb_kahypar::global_control::max_allowed_parallelism,
                                      num_threads);
        ...
        std::vector<oneapi::tbb_kahypar::flow::queue_node<chopstick>> places(
            num_philosophers, oneapi::tbb_kahypar::flow::queue_node<chopstick>(g));
        ...
        for (int i = 0; i < num_philosophers; ++i) {
            ...
            think_nodes.push_back(new think_node_type(
                g, oneapi::tbb_kahypar::flow::unlimited, think_node_body(philosophers[i])));
            eat_nodes.push_back(
                new eat_node_type(g, oneapi::tbb_kahypar::flow::unlimited, eat_node_body(philosophers[i])));
            forward_nodes.push_back(new forward_node_type(
                g, oneapi::tbb_kahypar::flow::unlimited, forward_node_body(philosophers[i])));
        }
        ...
        for (int i = 0; i < num_philosophers; ++i)
            think_nodes[i]->try_put(oneapi::tbb_kahypar::flow::continue_msg());
    ...
    utility::report_elapsed_time((oneapi::tbb_kahypar::tick_count::now() - main_time).seconds());
    return 0;
}
