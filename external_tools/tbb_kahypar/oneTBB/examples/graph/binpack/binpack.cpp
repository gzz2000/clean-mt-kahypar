...
typedef oneapi::tbb_kahypar::flow::
    multifunction_node<value_type, std::tuple<value_type, bin>, oneapi::tbb_kahypar::flow::rejecting>
        bin_packer;
// Items are placed into a pool that all bin packers grab from, represent by a queue_node:
typedef oneapi::tbb_kahypar::flow::queue_node<value_type> value_pool;
// Packed bins are placed in this buffer waiting to be serially printed and/or accounted for:
typedef oneapi::tbb_kahypar::flow::buffer_node<bin> bin_buffer;
// Packed bins are taken from the_bin_buffer and processed by the_writer:
typedef oneapi::tbb_kahypar::flow::
    function_node<bin, oneapi::tbb_kahypar::flow::continue_msg, oneapi::tbb_kahypar::flow::rejecting>
        bin_writer;
// Items are injected into the graph when this node sends them to the_value_pool:
typedef oneapi::tbb_kahypar::flow::input_node<value_type> value_source;
        ...
        if (done)
            ...
            if (remaining == 1 &&
                ...
                if (my_used < desired_bin_capacity / (1 + optimality * .1)) {
                    ...
                    oneapi::tbb_kahypar::flow::remove_edge(*the_value_pool, *my_bin_packer); // deactivate
                    done = true;
                    std::get<0>(p).try_put(desired_bin_capacity + 1); // send out signal
                }
                else { // this bin is well-utilized; send out bin and deactivate
                    oneapi::tbb_kahypar::flow::remove_edge(*the_value_pool,
...
public:
    ...
    value_type operator()(oneapi::tbb_kahypar::flow_control& fc) {
...
public:
    bin_printer()
            : running_count(0),
              ...
              my_max(0),
              avg(0) {}
    oneapi::tbb_kahypar::flow::continue_msg operator()(bin b) {
        value_type sum = 0;
        ++bins_num;
        if (verbose)
            std::cout << "[ ";
        ...
        return oneapi::tbb_kahypar::flow::continue_msg(); // need to return something
    }
};
...
int main(int argc, char* argv[]) {
    ...
    oneapi::tbb_kahypar::tick_count start = oneapi::tbb_kahypar::tick_count::now();
    for (int p = threads.first; p <= threads.last; p = threads.step(p)) {
        oneapi::tbb_kahypar::global_control c(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, p);
        packed_sum = 0;
        packed_items = 0;
        bins_num = 0;
        ...
        oneapi::tbb_kahypar::flow::make_edge(the_source, the_value_pool);
        ...
        for (int i = 0; i < num_bin_packers; ++i) {
            ...
            oneapi::tbb_kahypar::flow::make_edge(the_value_pool, *(bins[i]));
            oneapi::tbb_kahypar::flow::make_edge(oneapi::tbb_kahypar::flow::output_port<0>(*(bins[i])),
                                         the_value_pool);
            oneapi::tbb_kahypar::flow::make_edge(oneapi::tbb_kahypar::flow::output_port<1>(*(bins[i])),
                                         the_bin_buffer);
        }
    ...
    utility::report_elapsed_time((oneapi::tbb_kahypar::tick_count::now() - start).seconds());
    return 0;
}
