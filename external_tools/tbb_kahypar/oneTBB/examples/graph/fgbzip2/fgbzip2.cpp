...
typedef oneapi::tbb_kahypar::flow::async_node<oneapi::tbb_kahypar::flow::continue_msg, BufferMsg>
    async_file_reader_node;
typedef oneapi::tbb_kahypar::flow::async_node<BufferMsg, oneapi::tbb_kahypar::flow::continue_msg>
...
private:
    ...
    oneapi::tbb_kahypar::concurrent_bounded_queue<BufferMsg> m_writeQueue;
...
void fgCompressionAsyncNode(IOOperations& io, int blockSizeIn100KB) {
    ...
    async_file_reader_node file_reader(
        g,
        oneapi::tbb_kahypar::flow::unlimited,
        [&asyncNodeActivity](const oneapi::tbb_kahypar::flow::continue_msg& msg,
                             async_file_reader_node::gateway_type& gateway) {
            asyncNodeActivity.submitRead(gateway);
        });

    oneapi::tbb_kahypar::flow::function_node<BufferMsg, BufferMsg> compressor(
        g, oneapi::tbb_kahypar::flow::unlimited, BufferCompressor(blockSizeIn100KB));

    oneapi::tbb_kahypar::flow::sequencer_node<BufferMsg> ordering(g,
    ...
    async_file_writer_node output_writer(
        g,
        oneapi::tbb_kahypar::flow::serial,
    ...
    file_reader.try_put(oneapi::tbb_kahypar::flow::continue_msg());
...
void fgCompression(IOOperations& io, int blockSizeIn100KB) {
    ...
    oneapi::tbb_kahypar::flow::input_node<BufferMsg> file_reader(
        g, [&io](oneapi::tbb_kahypar::flow_control& fc) -> BufferMsg {
            if (io.hasDataToRead()) {
    ...
    oneapi::tbb_kahypar::flow::sequencer_node<BufferMsg> ordering(g, [](const BufferMsg& buffer) -> size_t {
        return buffer.seqId;
    });

    oneapi::tbb_kahypar::flow::function_node<BufferMsg> output_writer(
        g, oneapi::tbb_kahypar::flow::serial, [&io](const BufferMsg& bufferMsg) {
