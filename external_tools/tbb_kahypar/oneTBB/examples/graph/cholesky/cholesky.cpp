...
typedef oneapi::tbb_kahypar::flow::multifunction_node<tagged_tile_t, t1_t> dpotf2_node_t;
typedef oneapi::tbb_kahypar::flow::multifunction_node<t2_t, t2_t> dtrsm_node_t;
typedef oneapi::tbb_kahypar::flow::multifunction_node<t3_t, t3_t> dsyr2k_node_t;

typedef oneapi::tbb_kahypar::flow::join_node<t2_t, oneapi::tbb_kahypar::flow::tag_matching> dtrsm_join_t;
typedef oneapi::tbb_kahypar::flow::join_node<t3_t, oneapi::tbb_kahypar::flow::tag_matching> dsyr2k_join_t;
...
protected:
    virtual void func(void *ptr, int n, int b) {
        using oneapi::tbb_kahypar::flow::unlimited;
...
typedef oneapi::tbb_kahypar::flow::continue_node<oneapi::tbb_kahypar::flow::continue_msg> continue_type;
...
protected:
    virtual void func(void *ptr, int n, int b) {
        ...
        oneapi::tbb_kahypar::flow::graph g;
        for (int k = p - 1; k >= 0; --k) {
            c[k] = new continue_type(g, [=](const oneapi::tbb_kahypar::flow::continue_msg &) {
                call_dpotf2(tile, b, k);
            });
            ...
            for (int j = k + 1; j < p; ++j) {
                t[k][j] = new continue_type(g, [=](const oneapi::tbb_kahypar::flow::continue_msg &) {
                    call_dtrsm(tile, b, k, j);
                });
                make_edge(*c[k], *t[k][j]);
                ...
                for (int i = k + 1; i <= j; ++i) {
                    u[k][j][i] = new continue_type(g, [=](const oneapi::tbb_kahypar::flow::continue_msg &) {
                        call_dsyr2k(tile, b, k, j, i);
                    });
        ...
        c[0]->try_put(oneapi::tbb_kahypar::flow::continue_msg());
        g.wait_for_all();
