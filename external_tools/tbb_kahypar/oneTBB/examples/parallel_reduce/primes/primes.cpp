...
public:
    ...
    Multiples(const Multiples& f, oneapi::tbb_kahypar::split)
            : n_factor(f.n_factor),
              m(f.m),
...
public:
    ...
    SieveRange(SieveRange& r, oneapi::tbb_kahypar::split)
...
public:
    ...
    Sieve(Sieve& other, oneapi::tbb_kahypar::split)
            : multiples(other.multiples, oneapi::tbb_kahypar::split()),
              count(0) {}
    //------------------------------------------------------------------------
    // End of signatures required by parallel_reduce
    //------------------------------------------------------------------------
...
NumberType ParallelCountPrimes(NumberType n, int number_of_threads, NumberType grain_size) {
    ...
    if (n >= 3) {
        ...
        oneapi::tbb_kahypar::parallel_reduce(SieveRange(s.multiples.m, n, s.multiples.m, grain_size),
                                     s,
