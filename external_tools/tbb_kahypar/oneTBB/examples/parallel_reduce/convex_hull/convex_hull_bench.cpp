...
typedef oneapi::tbb_kahypar::blocked_range<std::size_t> range_t;

#if USECONCVEC
#include "oneapi_kahypar/tbb_kahypar/concurrent_vector.h"

typedef oneapi::tbb_kahypar::concurrent_vector<point_t> pointVec_t;
...
typedef oneapi::tbb_kahypar::spin_mutex mutex_t;
typedef std::vector<point_t> pointVec_t;
...
void initialize(pointVec_t &points) {
    ...
    //      - oneapi::tbb_kahypar::simple_partitioner produce the same set of ranges on every call to
    ...
    oneapi::tbb_kahypar::parallel_for(range_t(0, cfg::numberOfPoints, BodyType::grainSize),
                              BodyType(points),
...
public:
    ...
    FindXExtremum(const FindXExtremum &fxex, oneapi::tbb_kahypar::split)
            : points(fxex.points),
              exType(fxex.exType),
...
point_t extremum(const pointVec_t &P) {
    ...
    oneapi::tbb_kahypar::parallel_reduce(range_t(0, P.size(), FindXExtremum::grainSize), fxBody);
...
public:
    ...
    SplitByCP(SplitByCP &sbcp, oneapi::tbb_kahypar::split)
            : p1(sbcp.p1),
              p2(sbcp.p2),
...
public:
    ...
    SplitByCP_buf(SplitByCP_buf &sbcp, oneapi::tbb_kahypar::split)
            : p1(sbcp.p1),
              p2(sbcp.p2),
              initialSet(sbcp.initialSet),
              reducedSet(sbcp.reducedSet),
...
point_t divide(const pointVec_t &P, pointVec_t &P_reduced, const point_t &p1, const point_t &p2) {
    ...
    oneapi::tbb_kahypar::parallel_reduce(
        range_t(0, P.size(), BodyType::grainSize), body, oneapi::tbb_kahypar::simple_partitioner());
