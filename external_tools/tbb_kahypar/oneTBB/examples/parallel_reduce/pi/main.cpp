...
int main(int argc, char* argv[]) {
    try {
        tbb_kahypar::tick_count main_start_time = tbb_kahypar::tick_count::now();
        ...
        for (int p = threads.first; p <= threads.last; p = threads.step(p)) {
            ...
            if (p == 0) {
                ...
                tbb_kahypar::tick_count compute_start_time = tbb_kahypar::tick_count::now();
                pi = compute_pi_serial();
                compute_time = (tbb_kahypar::tick_count::now() - compute_start_time).seconds();
            }
            else {
        ...
        utility::report_elapsed_time((tbb_kahypar::tick_count::now() - main_start_time).seconds());
        return 0;
    }
