...
int main(int argc, char* argv[]) {
    ...
    oneapi::tbb_kahypar::tick_count main_start = oneapi::tbb_kahypar::tick_count::now();
    ...
    for (int p = threads.first; p <= threads.last; p = threads.step(p)) {
        ...
        oneapi::tbb_kahypar::tick_count::interval_t interval = oneapi::tbb_kahypar::tick_count::now() - t0;
        if (!SilentFlag) {
            std::cout << interval.seconds() << " seconds using " << p << " threads ("
                      << root_set_size << " nodes in root_set)\n";
        }
    }
    utility::report_elapsed_time((oneapi::tbb_kahypar::tick_count::now() - main_start).seconds());
