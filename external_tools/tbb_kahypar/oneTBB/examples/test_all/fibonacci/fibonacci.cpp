...
value SerialQueueFib(int n) {
    oneapi::tbb_kahypar::concurrent_queue<Matrix2x2> Q;
    for (int i = 1; i < n; i++)
        Q.push(Matrix1110);
    Matrix2x2 A, B;
    while (true) {
...
value SerialVectorFib(int n) {
    oneapi::tbb_kahypar::concurrent_vector<value> A;
    A.grow_by(2);
    A[0] = 0;
    A[1] = 1;
    for (int i = 2; i <= n; i++) {
...
void SharedSerialFibBody<std::mutex>::operator()(
    const oneapi::tbb_kahypar::blocked_range<int> &range) const {
    for (;;) {
...
value SharedSerialFib(int n) {
    ...
    SharedN = n;
    M mutex;
    parallel_for(oneapi::tbb_kahypar::blocked_range<int>(0, 4, 1), SharedSerialFibBody<M>(mutex));
    return SharedB;
}
...
typedef oneapi::tbb_kahypar::concurrent_hash_map<int, value, IntHashCompare> NumbersTable;
...
typedef oneapi::tbb_kahypar::concurrent_queue<Matrix2x2> queue_t;
...
struct InputFunc {
    InputFunc() {}
    queue_t *operator()(oneapi::tbb_kahypar::flow_control &fc) const {
...
value ParallelPipeFib(int n) {
    ...
    oneapi::tbb_kahypar::parallel_pipeline(
        n,
        oneapi::tbb_kahypar::make_filter<void, queue_t *>(oneapi::tbb_kahypar::filter_mode::parallel, InputFunc()) &
            oneapi::tbb_kahypar::make_filter<queue_t *, void>(oneapi::tbb_kahypar::filter_mode::parallel,
                                                      MultiplyFunc()));
...
struct parallel_reduceFibBody {
    ...
    parallel_reduceFibBody(parallel_reduceFibBody &other, oneapi::tbb_kahypar::split)
            : sum(Matrix1110),
...
value parallel_reduceFib(int n) {
    ...
    oneapi::tbb_kahypar::parallel_reduce(oneapi::tbb_kahypar::blocked_range<int>(2, n, 3),
...
struct parallel_scanFibBody {
    ...
    //! Splitting constructor
    parallel_scanFibBody(parallel_scanFibBody &b, oneapi::tbb_kahypar::split)
    ...
    void operator()(const oneapi::tbb_kahypar::blocked_range<int> &r, Tag tag) {
...
value parallel_scanFib(int n) {
    ...
    oneapi::tbb_kahypar::parallel_scan(oneapi::tbb_kahypar::blocked_range<int>(0, n, 3), b);
...
static oneapi::tbb_kahypar::tick_count t0;
...
value Measure(const char *name, MeasureFunc func, int n) {
    ...
    for (int number = 2; number <= n; number++)
        result = func(number);
    if (Verbose)
        printf("\t- in %f msec\n", (oneapi::tbb_kahypar::tick_count::now() - t0).seconds() * 1000);
    return result;
}
    ...
    for (unsigned long i = 0; i < ntrial; ++i) {
        for (int threads = NThread.low; threads <= NThread.high; threads *= 2) {
            ...
            sum = Measure("Shared serial (spin_mutex)",
                          SharedSerialFib<oneapi::tbb_kahypar::spin_mutex>,
                          NumbersCount);
            assert(result == sum);
            sum = Measure("Shared serial (queuing_mutex)",
                          SharedSerialFib<oneapi::tbb_kahypar::queuing_mutex>,
                          NumbersCount);
            assert(result == sum);
