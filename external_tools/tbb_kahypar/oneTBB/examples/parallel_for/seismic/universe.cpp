...
void Universe::ParallelUpdateStress(oneapi::tbb_kahypar::affinity_partitioner& affinity) {
    oneapi::tbb_kahypar::parallel_for(
        oneapi::tbb_kahypar::blocked_range<int>(0, UniverseHeight - 1), // Index space for loop
...
struct UpdateVelocityBody {
    ...
    void operator()(const oneapi::tbb_kahypar::blocked_range<int>& y_range) const {
        u_.UpdateVelocity(
            Universe::Rectangle(1, y_range.begin(), u_.UniverseWidth - 1, y_range.size()));
    }
};

void Universe::ParallelUpdateVelocity(oneapi::tbb_kahypar::affinity_partitioner& affinity) {
    oneapi::tbb_kahypar::parallel_for(
        oneapi::tbb_kahypar::blocked_range<int>(1, UniverseHeight), // Index space for loop
