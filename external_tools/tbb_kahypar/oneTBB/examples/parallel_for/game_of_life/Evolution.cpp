...
void SequentialEvolution::Run(double execution_time, int nthread) {
    ...
    while (!m_done) {
        if (!is_paused) {
            oneapi::tbb_kahypar::tick_count t = oneapi::tbb_kahypar::tick_count::now();
            Step();
...
void ParallelEvolution::Run(double execution_time, int nthread) {
    ...
    oneapi::tbb_kahypar::global_control* pGlobControl = new oneapi::tbb_kahypar::global_control(
        oneapi::tbb_kahypar::global_control::max_allowed_parallelism, nthread);
...
public:
    ...
    void operator()(const oneapi::tbb_kahypar::blocked_range<std::size_t>& r) const {
...
void ParallelEvolution::Step() {
    ...
    //! do calculation loop
    parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(begin, end, GRAIN_SIZE),
                 tbb_parallel_task());
    UpdateMatrix();
}
