...
void OverlayOnePolygonWithMap(Polygon_map_t *resultMap,
                              RPolygon *myPoly,
                              Polygon_map_t *map2,
                              oneapi::tbb_kahypar::spin_mutex *rMutex) {
    ...
    for (unsigned int j = 1; (j < map2->size()) && (p1Area > 0); j++) {
        ...
        if (PolygonsOverlap(myPoly, p2, newxMin, newyMin, newxMax, newyMax)) {
            ...
            if (rMutex) {
                oneapi::tbb_kahypar::spin_mutex::scoped_lock lock(*rMutex);
...
class ApplyOverlay {
    ...
    oneapi::tbb_kahypar::spin_mutex *m_rMutex;
...
void NaiveParallelOverlay(Polygon_map_t *&result_map,
    ...
    // push the map size as the first polygon,
    oneapi::tbb_kahypar::spin_mutex *resultMutex = new oneapi::tbb_kahypar::spin_mutex();
    ...
    for (int nthreads = gThreadsLow; nthreads <= gThreadsHigh; nthreads++) {
        ...
        oneapi::tbb_kahypar::parallel_for(
            oneapi::tbb_kahypar::blocked_range<int>(1, (int)(polymap1.size()), grain_size),
...
class blocked_range_with_maps {
    typedef oneapi::tbb_kahypar::blocked_range<T> my_range_type;
...
public:
    ...
    blocked_range_with_maps(blocked_range_with_maps &lhs_r, oneapi::tbb_kahypar::split)
            : my_range(my_range_type(lhs_r.my_range, oneapi::tbb_kahypar::split())) {
...
public:
    ...
    */
    void operator()(/*const*/ blocked_range_with_maps<int> &r) const {
        ...
        // confusing things.
        oneapi::tbb_kahypar::spin_mutex::scoped_lock lock(*m_rMutex);
...
void SplitParallelOverlay(Polygon_map_t **result_map,
    ...
    oneapi::tbb_kahypar::tick_count t0, t1;
    oneapi::tbb_kahypar::spin_mutex *resultMutex;
    if (gThreadsLow == THREADS_UNSET || gThreadsLow == utility::get_default_num_threads()) {
        gThreadsLow = gThreadsHigh = utility::get_default_num_threads();
        automatic_threadcount = true;
    }
    *result_map = new Polygon_map_t;
    ...
    p0->get(&ignore1, &ignore2, &mapxSize, &mapySize);
    (*result_map)->reserve(mapxSize * mapySize); // can't be any bigger than this
    resultMutex = new oneapi::tbb_kahypar::spin_mutex();
    ...
    for (nthreads = gThreadsLow; nthreads <= gThreadsHigh; nthreads++) {
        ...
        t1 = oneapi::tbb_kahypar::tick_count::now();
...
void SplitParallelOverlayETS(ETS_Polygon_map_t **result_map,
    ...
    *result_map = new ETS_Polygon_map_t;
    ...
    for (nthreads = gThreadsLow; nthreads <= gThreadsHigh; nthreads++) {
        ...
        {
            Polygon_map_t s_result_map;
            oneapi::tbb_kahypar::flattened2d<ETS_Polygon_map_t> psv = flatten2d(**result_map);
            s_result_map.push_back(RPolygon(0, 0, mapxSize, mapySize));
            for (oneapi::tbb_kahypar::flattened2d<ETS_Polygon_map_t>::const_iterator ci = psv.begin();
                 ci != psv.end();
                 ++ci) {
