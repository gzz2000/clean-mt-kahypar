...
class AsyncActivity {
    ...
    tbb_kahypar::task::suspend_point suspend_point;
...
public:
    AsyncActivity()
            : offload_ratio(0),
              ...
              service_thread([this] {
                  ...
                  // passed into tbb_kahypar::task_group
                  ...
                  tbb_kahypar::task::resume(suspend_point);
              }) {}
    ...
    void submit(float ratio, tbb_kahypar::task::suspend_point sus_point) {
        offload_ratio = ratio;
...
int main() {
    ...
    tbb_kahypar::global_control gc(tbb_kahypar::global_control::max_allowed_parallelism,
    ...
    tg.run([&] {
        ...
        sstream << "start index for CPU = " << i_start << "; end index for CPU = " << i_end
                << std::endl;
        std::cout << sstream.str();
        tbb_kahypar::parallel_for(i_start, i_end, [](std::size_t index) {
            c_array[index] = a_array[index] + alpha * b_array[index];
        });
    });

    // Run GPU part
    tbb_kahypar::task::suspend([&](tbb_kahypar::task::suspend_point suspend_point) {
