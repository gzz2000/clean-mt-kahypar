...
using async_node_type = tbb_kahypar::flow::async_node<float, done_tag>;
...
int main() {
    ...
    auto mp = tbb_kahypar::global_control::max_allowed_parallelism;
    tbb_kahypar::global_control gc(mp, nth + 1); // One more thread, but sleeping
    tbb_kahypar::flow::graph g;

    // Input node:
    tbb_kahypar::flow::input_node<float> in_node{ g, [&](tbb_kahypar::flow_control& fc) -> float {
    ...
    // CPU node
    tbb_kahypar::flow::function_node<float, done_tag> cpu_node{
        g,
        tbb_kahypar::flow::unlimited,
        [&](float offload_ratio) {
            ...
            tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<size_t>{ i_start, i_end },
                              [&](const tbb_kahypar::blocked_range<size_t>& r) {
    ...
    // join node
    using join_t = tbb_kahypar::flow::join_node<std::tuple<done_tag, done_tag>, tbb_kahypar::flow::queueing>;
    join_t node_join{ g };

    // out node
    tbb_kahypar::flow::function_node<join_t::output_type> out_node{
        g,
        tbb_kahypar::flow::unlimited,
    ...
    // construct graph
    tbb_kahypar::flow::make_edge(in_node, a_node);
    tbb_kahypar::flow::make_edge(in_node, cpu_node);
    tbb_kahypar::flow::make_edge(a_node, tbb_kahypar::flow::input_port<0>(node_join));
    tbb_kahypar::flow::make_edge(cpu_node, tbb_kahypar::flow::input_port<1>(node_join));
    tbb_kahypar::flow::make_edge(node_join, out_node);
