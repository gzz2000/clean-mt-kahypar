...
oneapi::tbb_kahypar::spin_mutex* locks; // a lock for each vertex
oneapi::tbb_kahypar::task_group* sp_group; // task group for tasks executing sub-problems
...
oneapi::tbb_kahypar::concurrent_priority_queue<vertex_rec, compare_f> open_set; // tentative vertices

void shortpath_helper();

void shortpath() {
    sp_group = new oneapi::tbb_kahypar::task_group;
...
void shortpath_helper() {
    ...
    while (open_set.try_pop(u_rec)) {
        ...
        double old_g_u = 0.0;
        {
            oneapi::tbb_kahypar::spin_mutex::scoped_lock l(locks[u]);
            if (f > f_distance[u])
        ...
        for (std::size_t i = 0; i < edges[u].size(); ++i) {
            ...
            bool push = false;
            {
                oneapi::tbb_kahypar::spin_mutex::scoped_lock l(locks[v]);
...
void InitializeGraph() {
    ...
    g_distance.resize(N);
    f_distance.resize(N);
    locks = new oneapi::tbb_kahypar::spin_mutex[N];

    if (verbose)
        printf("Generating vertices...\n");
    oneapi::tbb_kahypar::parallel_for(
        oneapi::tbb_kahypar::blocked_range<std::size_t>(0, N, 64),
        [&](oneapi::tbb_kahypar::blocked_range<std::size_t>& r) {
...
void ResetGraph() {
    oneapi::tbb_kahypar::global_control c(oneapi::tbb_kahypar::global_control::max_allowed_parallelism,
                                  utility::get_default_num_threads());
    oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(0, N),
                              [&](oneapi::tbb_kahypar::blocked_range<std::size_t>& r) {
                                  for (std::size_t i = r.begin(); i != r.end(); ++i) {
...
int main(int argc, char* argv[]) {
    ...
    for (int n_thr = threads.first; n_thr <= threads.last; n_thr = threads.step(n_thr)) {
        ResetGraph();
        oneapi::tbb_kahypar::global_control c(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, n_thr);
