...
public:
    void operator()(oneapi::tbb_kahypar::blocked_range2d<int> &r) const {
        if (v->next_frame())
...
void fractal::render(oneapi::tbb_kahypar::task_group_context &context) {
    ...
    if (schedule_auto)
        oneapi::tbb_kahypar::parallel_for(
            oneapi::tbb_kahypar::blocked_range2d<int>(0, size_y, grain_size, 0, size_x, grain_size),
            body,
...
void fractal::run(oneapi::tbb_kahypar::task_group_context &context) {
    clear();
    context.reset();
    render(context);
}
