...
static const std::size_t cache_line_size = tbb_kahypar::detail::max_nfs_size;
...
private:
    ...
    void remove_server_ref() {
        if( --my_ref_count==0 ) {
            ...
            this->~private_server();
            tbb_kahypar::cache_aligned_allocator<private_server>().deallocate( this, 1 );
        }
    }
...
#endif /* TBB_USE_ASSERT */
{
    my_thread_array = tbb_kahypar::cache_aligned_allocator<padded_private_worker>().allocate( my_n_thread );
    for( std::size_t i=0; i<my_n_thread; ++i ) {
...
private_server::~private_server() {
    ...
    tbb_kahypar::cache_aligned_allocator<padded_private_worker>().deallocate( my_thread_array, my_n_thread );
    tbb_kahypar::detail::poison_pointer( my_thread_array );
}
...
tbb_server* make_private_server( tbb_client& client ) {
    return new( tbb_kahypar::cache_aligned_allocator<private_server>().allocate(1) ) private_server(client);
}
