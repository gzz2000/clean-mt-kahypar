...
class MallocMutex : tbb_kahypar::detail::no_copy {
    std::atomic_flag m_flag = ATOMIC_FLAG_INIT;

    void lock() {
        tbb_kahypar::detail::atomic_backoff backoff;
...
public:
    class scoped_lock : tbb_kahypar::detail::no_copy {
        MallocMutex& m_mutex;
        bool m_taken;
...
inline void SpinWaitWhileEq(const std::atomic<intptr_t>& location, const intptr_t value) {
    tbb_kahypar::detail::spin_wait_while_eq(location, value);
}
...
inline void SpinWaitUntilEq(const std::atomic<intptr_t>& location, const intptr_t value) {
    tbb_kahypar::detail::spin_wait_until_eq(location, value);
}
