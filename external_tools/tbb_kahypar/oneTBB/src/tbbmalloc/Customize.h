...
#include "oneapi_kahypar/tbb_kahypar/detail/_assert.h"
...
inline intptr_t BitScanRev(uintptr_t x) {
    return x == 0 ? -1 : static_cast<intptr_t>(tbb_kahypar::detail::log2(x));
}

template<typename T>
static inline bool isAligned(T* arg, uintptr_t alignment) {
    return tbb_kahypar::detail::is_aligned(arg,alignment);
}

static inline bool isPowerOfTwo(uintptr_t arg) {
    return tbb_kahypar::detail::is_power_of_two(arg);
}
static inline bool isPowerOfTwoAtLeast(uintptr_t arg, uintptr_t power2) {
    return arg && tbb_kahypar::detail::is_power_of_two_at_least(arg,power2);
}

inline void do_yield() {
    tbb_kahypar::detail::yield();
}
...
#include "oneapi_kahypar/tbb_kahypar/detail/_aggregator.h"

template <typename OperationType>
struct MallocAggregator {
    typedef tbb_kahypar::detail::d1::aggregator_generic<OperationType> type;
};

//! aggregated_operation base class
template <typename Derived>
struct MallocAggregatedOperation {
    typedef tbb_kahypar::detail::d1::aggregated_operation<Derived> type;
};
