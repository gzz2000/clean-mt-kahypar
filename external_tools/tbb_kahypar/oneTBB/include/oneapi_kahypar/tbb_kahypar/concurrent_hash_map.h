...
public:
    ...
    using allocator_traits_type = tbb_kahypar::detail::allocator_traits<allocator_type>;
    using bucket_allocator_type = typename allocator_traits_type::template rebind_alloc<bucket>;
    using bucket_allocator_traits = tbb_kahypar::detail::allocator_traits<bucket_allocator_type>;
    ...
    static segment_index_type segment_index_of( size_type index ) {
        return segment_index_type(tbb_kahypar::detail::log2( index|1 ));
    }
    ...
    segment_index_type insert_new_node( bucket *b, node_base *n, hashcode_type mask ) {
        ...
        if( sz >= mask ) { // TODO: add custom load_factor
            segment_index_type new_seg = tbb_kahypar::detail::log2( mask+1 ); //optimized segment_index_of
...
private:
    ...
    template <typename Key, typename T, typename HashCompare, typename A
            ...
            , typename M
             >
        __TBB_requires(tbb_kahypar::detail::hash_compare<HashCompare, Key> &&
...
public:
    ...
    using allocator_type = tbb_kahypar::detail::type_identity_t<Allocator>;
    using hash_compare_type = tbb_kahypar::detail::type_identity_t<HashCompare>;
...
protected:
    ...
    using node_allocator_traits = tbb_kahypar::detail::allocator_traits<node_allocator_type>;
    ...
    void rehash_bucket( bucket *b_new, const hashcode_type hash ) {
        ...
        hashcode_type mask = (hashcode_type(1) << tbb_kahypar::detail::log2(hash)) - 1; // get parent mask from the topmost bit
...
public:
    ...
    concurrent_hash_map& operator=( concurrent_hash_map &&table ) {
        if( this != &table ) {
            ...
            internal_move_assign(std::move(table), tbb_kahypar::detail::disjunction<is_equal_type, pocma_type>());
        }
        return *this;
    }
        ...
        for(; b <= mask; b++, bp++ ) {
            ...
            if (rehash_required(n)) { // rehash bucket, conditional because rehashing of a previous bucket may affect this one
                ...
                do {
                    ...
                    hashcode_type m = ( hashcode_type(1) << tbb_kahypar::detail::log2( h ) ) - 1; // get parent mask from the topmost bit
    ...
    void swap(concurrent_hash_map& table) {
        ...
        internal_swap(table, tbb_kahypar::detail::disjunction<pocs_type, is_equal_type>());
    }
...
protected:
    ...
    bool lookup( const K &key, const T *t, const_accessor *result, bool write, AllocateNodeType allocate_node, node *tmp_n  = nullptr)
    {
        ...
        exists:
            ...
            if( !result->try_acquire( n->mutex, write ) ) {
                for( tbb_kahypar::detail::atomic_backoff backoff(true);; ) {
    ...
    std::pair<I, I> internal_equal_range( const K& key, I end_ ) const {
        ...
        while (rehash_required(b->node_list.load(std::memory_order_relaxed))) {
            m = ( hashcode_type(1) << tbb_kahypar::detail::log2( h ) ) - 1; // get parent mask from the topmost bit
