...
concept parallel_scan_body = splittable<Body> &&
                             requires( Body& body, const Range& range, Body& other ) {
                                 body(range, tbb_kahypar::detail::d1::pre_scan_tag{});
                                 body(range, tbb_kahypar::detail::d1::final_scan_tag{});
...
public:
    ...
    template<typename Tag>
    void operator()( const Range& r, Tag tag ) {
        m_sum_slot = tbb_kahypar::detail::invoke(m_scan, r, m_sum_slot, tag);
    }

    void reverse_join( lambda_scan_body& a ) {
        m_sum_slot = tbb_kahypar::detail::invoke(m_reverse_join, a.m_sum_slot, m_sum_slot);
    }
