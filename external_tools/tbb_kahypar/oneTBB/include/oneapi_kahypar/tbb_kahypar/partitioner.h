...
class range_vector {
    ...
    tbb_kahypar::detail::aligned_space<T, MaxCapacity> my_pool;
...
struct proportional_mode : adaptive_mode<Partition> {
    ...
    proportional_split get_split() {
        ...
        //   otherwise, the tbb_kahypar::proportional_split object will be implicitly (for Range implementer)
        //   casted to tbb_kahypar::split
...
static std::size_t get_initial_partition_head() {
    ...
    if (current_index == tbb_kahypar::task_arena::not_initialized)
        current_index = 0;
