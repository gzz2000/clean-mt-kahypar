...
private:
    using allocator_traits = tbb_kahypar::detail::allocator_traits<container_allocator_type>;
...
template <typename Traits>
tbb_kahypar::detail::synthesized_three_way_result<typename Traits::value_type>
operator<=>( const concurrent_skip_list<Traits>& lhs, const concurrent_skip_list<Traits>& rhs ) {
    return std::lexicographical_compare_three_way(lhs.begin(), lhs.end(),
                                                  rhs.begin(), rhs.end(),
                                                  tbb_kahypar::detail::synthesized_three_way_comparator{});
}
#else
template <typename Traits>
bool operator<( const concurrent_skip_list<Traits>& lhs, const concurrent_skip_list<Traits>& rhs ) {
...
public:
    ...
    std::size_t operator()() {
        ...
        std::size_t result = max_level - std::size_t(tbb_kahypar::detail::log2(engines.local()() + 1)) - 1;
        __TBB_ASSERT(result <= max_level, nullptr);
        return result;
    }

private:
    tbb_kahypar::enumerable_thread_specific<std::minstd_rand> engines;
};
