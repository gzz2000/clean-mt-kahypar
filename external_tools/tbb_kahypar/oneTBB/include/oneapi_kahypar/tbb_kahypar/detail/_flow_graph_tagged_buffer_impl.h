...
// included in namespace tbb_kahypar::flow::interfaceX::internal
...
template
    <
     ...
     typename Allocator=tbb_kahypar::cache_aligned_allocator<ElementType>
    >
...
private:
    ...
    void internal_insert_with_key( element_type **p_pointer_array, size_t p_sz, list_array_type &p_free_list,
        ...
        size_t h = this->hash(tbb_kahypar::detail::invoke(*my_key, get_value_from_pack(args...))) & l_mask;
...
public:
    ...
    bool insert_with_key(const value_type &v, Args&&... args) {
        ...
        __TBB_ASSERT(my_key, "Error: value-to-key functor not provided");
        if(find_element_ref_with_key(tbb_kahypar::detail::invoke(*my_key, v), p)) {
            p->destroy_element();
    ...
    bool find_element_ref_with_key(const Knoref& k, element_type*& v) {
        ...
        for(element_type* p = pointer_array[i]; p; p = (element_type *)(p->get_next())) {
            __TBB_ASSERT(my_key, "Error: value-to-key functor not provided");
            if(this->equal(tbb_kahypar::detail::invoke(*my_key, *p->get_value_ptr()), k)) {
                v = p;
                return true;
            }
        }
    ...
    void delete_with_key(const Knoref& k) {
        ...
        for(element_type* p = pointer_array[h]; p; prev = p, p = (element_type *)(p->get_next())) {
            ...
            __TBB_ASSERT(my_key, "Error: value-to-key functor not provided");
            if(this->equal(tbb_kahypar::detail::invoke(*my_key, *vp), k)) {
                p->destroy_element();
...
template
    <
     ...
     typename ValueToKey,  // abstract method that returns "const Key" or "const Key&" given ValueType
     typename HashCompare, // has hash and equal
     typename Allocator=tbb_kahypar::cache_aligned_allocator<hash_buffer_element<Key, ValueType>>
    >
...
template
    <
     ...
     typename ValueToKey,  // abstract method that returns "const Key" or "const Key&" given ValueType
     typename HashCompare, // has hash and equal
     typename Allocator=tbb_kahypar::cache_aligned_allocator<metainfo_hash_buffer_element<Key, ValueType>>
    >
