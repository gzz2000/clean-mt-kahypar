...
private:
    ...
    using value_node_allocator_traits = tbb_kahypar::detail::allocator_traits<value_node_allocator_type>;

    static constexpr size_type round_up_to_power_of_two( size_type bucket_count ) {
        return size_type(1) << size_type(tbb_kahypar::detail::log2(uintptr_t(bucket_count == 0 ? 1 : bucket_count) * 2 - 1));
    }
...
public:
    ...
    concurrent_unordered_base& operator=( concurrent_unordered_base&& other ) noexcept(unordered_segment_table::is_noexcept_assignment) {
        if (this != &other) {
            ...
            using is_always_equal = typename allocator_traits_type::is_always_equal;
            internal_move_assign(std::move(other), tbb_kahypar::detail::disjunction<pocma_type, is_always_equal>());
        }
        return *this;
    }
    ...
    void swap( concurrent_unordered_base& other ) noexcept(unordered_segment_table::is_noexcept_swap) {
        if (this != &other) {
            ...
            using is_always_equal = typename allocator_traits_type::is_always_equal;
            internal_swap(other, tbb_kahypar::detail::disjunction<pocs_type, is_always_equal>());
        }
    }
    ...
    void max_load_factor( float mlf ) {
        if (mlf != mlf || mlf < 0) {
            tbb_kahypar::detail::throw_exception(exception_id::invalid_load_factor);
        }
...
private:
        ...
        : public d1::segment_table<std::atomic<node_ptr>, allocator_type, unordered_segment_table, pointers_per_embedded_table>
    {
        ...
        using segment_allocator_traits = tbb_kahypar::detail::allocator_traits<segment_allocator_type>;
    public:
        ...
        static constexpr bool is_noexcept_swap = tbb_kahypar::detail::is_nothrow_swappable<hasher>::value &&
                                                 tbb_kahypar::detail::is_nothrow_swappable<key_equal>::value &&
                                                 segment_allocator_traits::is_always_equal::value;
...
private:
    ...
    size_type get_parent( size_type bucket ) const {
        ...
        size_type msb = tbb_kahypar::detail::log2(bucket);
        return bucket & ~(size_type(1) << msb);
    }

    size_type get_next_bucket_index( size_type bucket ) const {
        size_type bits = tbb_kahypar::detail::log2(my_bucket_count.load(std::memory_order_relaxed));
