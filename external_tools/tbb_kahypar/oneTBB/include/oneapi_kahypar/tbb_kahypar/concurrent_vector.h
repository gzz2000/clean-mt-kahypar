    ...
    : private segment_table<T, Allocator, concurrent_vector<T, Allocator>, embedded_table_num_segments>
{
    ...
    class generic_range_type : public tbb_kahypar::blocked_range<Iterator> {
        using base_type = tbb_kahypar::blocked_range<Iterator>;
    public:
        using value_type = T;
        using reference = T&;
...
public:
    ...
    void reserve( size_type n ) {
        ...
        if (n > max_size()) {
            tbb_kahypar::detail::throw_exception(exception_id::reservation_length_error);
        }
...
private:
    ...
    using segment_element_allocator_traits = tbb_kahypar::detail::allocator_traits<segment_element_allocator_type>;
    ...
    reference internal_subscript_with_exceptions( size_type index ) {
        if (index >= this->my_size.load(std::memory_order_acquire)) {
            tbb_kahypar::detail::throw_exception(exception_id::out_of_range);
        }
...
template <typename T, typename Allocator>
tbb_kahypar::detail::synthesized_three_way_result<typename concurrent_vector<T, Allocator>::value_type>
