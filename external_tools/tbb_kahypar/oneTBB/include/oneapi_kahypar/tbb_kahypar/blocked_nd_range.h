...
/*
    ...
    exactly N arguments of the type tbb_kahypar::blocked_range<Value>. Such ctor provides an opportunity
...
public:
    ...
    using dim_range_type = tbb_kahypar::blocked_range<value_type>;
    ...
    //! Constructs N-dimensional range over N half-open intervals each represented as tbb_kahypar::blocked_range<Value>.
...
// blocked_nd_range(blocked_nd_range<Value, N>&, oneapi::tbb_kahypar::split)
template <typename Value, unsigned int N>
blocked_nd_range(blocked_nd_range<Value, N>, oneapi::tbb_kahypar::split)
-> blocked_nd_range<Value, N>;

// blocked_nd_range(blocked_nd_range<Value, N>&, oneapi::tbb_kahypar::proportional_split)
template <typename Value, unsigned int N>
blocked_nd_range(blocked_nd_range<Value, N>, oneapi::tbb_kahypar::proportional_split)
-> blocked_nd_range<Value, N>;
