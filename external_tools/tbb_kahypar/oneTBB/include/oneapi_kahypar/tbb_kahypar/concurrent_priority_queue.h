...
private:
    ...
    void push_back_helper_impl( const T&, /*is_copy_constructible = */std::false_type ) {
        __TBB_ASSERT(false, "error: calling tbb_kahypar::concurrent_priority_queue.push(const value_type&) for move-only type");
    }
...
template <typename It,
          ...
          typename Alloc = tbb_kahypar::cache_aligned_allocator<iterator_value_t<It>>,
...
template <typename T,
          ...
          typename Alloc = tbb_kahypar::cache_aligned_allocator<T>,
          typename = std::enable_if_t<is_allocator_v<Alloc>>,
          typename = std::enable_if_t<!is_allocator_v<Comp>>>
