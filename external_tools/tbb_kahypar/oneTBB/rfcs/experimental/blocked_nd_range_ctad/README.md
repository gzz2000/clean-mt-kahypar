...
`oneapi::tbb_kahypar::blocked_nd_range` class was introduced as a representation for recursively divisible N-dimensional range for oneTBB parallel algorithms.
...
```cpp
oneapi::tbb_kahypar::blocked_range<int> range1(0, 100);
oneapi::tbb_kahypar::blocked_range<int> range2(0, 200);
oneapi::tbb_kahypar::blocked_range<int> range3(0, 300);

// Since 3 ranges of type int are provided, the type of nd_range
// can be deduced as oneapi::tbb_kahypar::blocked_nd_range<int, 3>
oneapi::tbb_kahypar::blocked_nd_range nd_range(range1, range2, range3);
```
...
The `oneapi::tbb_kahypar::blocked_nd_range` supports the following set of constructors:
...
tbb_kahypar::blocked_range<int> dim_range(0, 100);

tbb_kahypar::blocked_nd_range<int, 2> nd_range_1(dim_range, tbb_kahypar::blocked_range<int>(0, 200));

// Constructing in-place from braced-init-lists
tbb_kahypar::blocked_nd_range<int, 2> nd_range_2({0, 100}, {0, 200, 5});

// Combined approach
tbb_kahypar::blocked_nd_range<int, 2> nd_range_3(dim_range, {0, 200, 5});
```
...
tbb_kahypar::blocked_nd_range<int, 3> nd_range_1(sizes);

// Using the grainsize - each dim range will have grainsize 5
tbb_kahypar::blocked_nd_range<int, 3> nd_range_2(sizes, 5);

// Passing the braced-init-list
tbb_kahypar::blocked_nd_range<int, 3> nd_range_3({100, 200, 300});
```
...
```cpp
int array[2] = {100, 200};
tbb_kahypar::blocked_nd_range range(array);
```
...
* Support the use-case above but do not support CTAD for braced-init-lists at all. The major downside is that the user would 
  need to always specify the exact type `tbb_kahypar::blocked_range` while using the braced-init-list construction.

### Using the constructor `[1]` with "mixed" arguments

There is a limitation of the deduction guides proposed if the constructor `[1]` is used with both arguments of exact `tbb_kahypar::blocked_range` type
and the braced-init-lists:

```cpp
tbb_kahypar::blocked_range<int> dim_range(0, 100);
tbb_kahypar::blocked_nd_range nd_range(dim_range, {0, 200}, {0, 300}, dim_range);
```
...
the set of braced-init-lists or the set of `tbb_kahypar::blocked_range` objects.
