...
not trees. This proposal expands `tbb_kahypar::task_group` to support additional use cases.
...
`tbb_kahypar::task_group` is shown below. Note the existing `defer` function, since this
...
It allows the `Func` to be an instance of C++ `FunctionObject` returning `tbb_kahypar::task_handle` representing a hint for the next task that should be executed:
    ...
    auto split_task = [&tg] -> tbb_kahypar::task_handle {
...
`task_group::defer` returns a `tbb_kahypar::task_handle` that represents a created
...
An example of this is the implementation of `tbb_kahypar::parallel_for` that 
performs a parallel recursive decomposition of a range. Currently,
the oneTBB algorithms, such as `tbb_kahypar::parallel_for`, are implemented using the non-public,
low-level tasking API, rather than `tbb_kahypar::task_group`. This low-level tasking API
...
to complete.
    ...
    tbb_kahypar::task_handle first_task = tg.defer([&] { /* task body */ });
    tbb_kahypar::task_handle second_task = tg.defer([&] { /* task body */ });
    tbb_kahypar::task_handle final_task = tg.defer([&] { /* task body */ });
...
logic, the tasks that must complete before the new work can start.
    ...
    tbb_kahypar::task_handle add_another_task(tbb_kahypar::task_group& tg, int work_id) {
        tbb_kahypar::task_handle new_task = tg.defer([=] { do_work(work_id); });

        for (tbb_kahypar::task_handle& p : users::find_predecessors(work_id)) {
...
`users::create_right_range`, and `users::do_merge`.
    ...
    template<typename T>
    void merge_sort(tbb_kahypar::task_group& tg, T b, T e) {
        if (users::range_is_too_small(b, e)) {
            ...
            tbb_kahypar::task_handle sortleft =
                tg.defer([lb, le, &tg] {
                    merge_sort(tg, lb, le);
                });
            tbb_kahypar::task_handle sortright =
                tg.defer([rb, re, &tg] {
                    merge_sort(tg, rb, re);
                });
            tbb_kahypar::task_handle merge =
            ...
            tbb_kahypar::task_group::transfer_successors_to(merge);
