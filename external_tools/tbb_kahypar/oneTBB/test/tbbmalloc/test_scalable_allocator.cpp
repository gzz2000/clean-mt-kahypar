...
typedef StaticCountingAllocator<tbb_kahypar::memory_pool_allocator<char>> cnt_alloc_t;
...
{
    tbb_kahypar::memory_pool<NullAllocator> pool;
...
#if TBB_USE_EXCEPTIONS
        try {
            tbb_kahypar::fixed_pool pool(buf, sz);
...
#if TBB_USE_EXCEPTIONS
    try {
        tbb_kahypar::fixed_pool pool(nullptr, 10*1024*1024);
...
TEST_CASE("Allocator concept") {
    ...
    // allocate/deallocate
    TestAllocator<tbb_kahypar::scalable_allocator<void>>(Concept);
    {
        tbb_kahypar::memory_pool<tbb_kahypar::scalable_allocator<int>> pool;
        TestAllocator(Concept, tbb_kahypar::memory_pool_allocator<void>(pool));
    }{
        // tbb_kahypar::memory_pool<MinimalAllocator> pool;
        // cnt_alloc_t alloc( tbb_kahypar::memory_pool_allocator<char>(pool) ); // double parentheses to avoid function declaration
        // TestAllocator(Concept, alloc);
    }{
        static char buf[1024*1024*4];
        tbb_kahypar::fixed_pool pool(buf, sizeof(buf));
        ...
        typedef tbb_kahypar::memory_pool<tbb_kahypar::memory_pool_allocator<char, tbb_kahypar::fixed_pool> > NestedPool;

        static char buffer[8*1024*1024];
        tbb_kahypar::fixed_pool fixedPool(buffer, sizeof(buffer));
        // Underlying fixed pool allocator
        tbb_kahypar::memory_pool_allocator<char, tbb_kahypar::fixed_pool> fixedPoolAllocator(fixedPool);
        // Memory pool that handles fixed pool allocator
        NestedPool nestedPool(fixedPoolAllocator);
        // Top level memory pool allocator
        tbb_kahypar::memory_pool_allocator<char, NestedPool> nestedAllocator(nestedPool);

        TestAllocator(Concept, nestedAllocator);
    }
    tbb_kahypar::memory_pool<tbb_kahypar::scalable_allocator<int>> mpool;

    tbb_kahypar::memory_pool_allocator<int> mpalloc(mpool);

    TestAllocator<tbb_kahypar::memory_pool_allocator<int>>(Concept, mpalloc);
    TestAllocator<tbb_kahypar::memory_pool_allocator<void>>(Concept, mpalloc);

    // operator==
    TestAllocator<tbb_kahypar::scalable_allocator<void>>(Comparison);
    TestAllocator<tbb_kahypar::memory_pool_allocator<void>>(Comparison, tbb_kahypar::memory_pool_allocator<void>(mpool));
    TestAllocator<tbb_kahypar::memory_pool_allocator<int>>(Comparison, mpalloc);
    TestAllocator<tbb_kahypar::memory_pool_allocator<void>>(Comparison, mpalloc);
}
...
TEST_CASE("Exceptions") {
    TestAllocator<tbb_kahypar::scalable_allocator<void>>(Exceptions);
}
...
TEST_CASE("Thread safety") {
    TestAllocator<tbb_kahypar::scalable_allocator<void>>(ThreadSafety);
}
...
TEST_CASE("Broken allocator concept") {
    TestAllocator<tbb_kahypar::scalable_allocator<void>>(Broken);
    
    tbb_kahypar::memory_pool<tbb_kahypar::scalable_allocator<int>> mpool; 
    TestAllocator<tbb_kahypar::memory_pool_allocator<void>>(Broken, tbb_kahypar::memory_pool_allocator<void>(mpool));
}
#endif
...
TEST_CASE("Integration with STL containers") {
    TestAllocatorWithSTL<tbb_kahypar::scalable_allocator<void> >();
    tbb_kahypar::memory_pool<tbb_kahypar::scalable_allocator<int> > mpool;
    TestAllocatorWithSTL(tbb_kahypar::memory_pool_allocator<void>(mpool) );
    static char buf[1024*1024*4];
    tbb_kahypar::fixed_pool fpool(buf, sizeof(buf));
    TestAllocatorWithSTL(tbb_kahypar::memory_pool_allocator<void>(fpool) );
}
...
TEST_CASE("polymorphic_allocator test") {
    REQUIRE_MESSAGE(!tbb_kahypar::scalable_memory_resource()->is_equal(*std::pmr::get_default_resource()),
            "Scalable resource shouldn't be equal to standard resource." );
    REQUIRE_MESSAGE(tbb_kahypar::scalable_memory_resource()->is_equal(*tbb_kahypar::scalable_memory_resource()),
    ...
    TestAllocatorWithSTL(pmr_alloc_t(tbb_kahypar::scalable_memory_resource()));
}
#endif
