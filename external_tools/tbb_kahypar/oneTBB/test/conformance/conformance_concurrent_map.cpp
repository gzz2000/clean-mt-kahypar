...
struct AllowMultimapping<oneapi::tbb_kahypar::concurrent_multimap<Args...>> : std::true_type {};

template <typename Key, typename Mapped>
using Allocator = LocalCountingAllocator<std::allocator<std::pair<const Key, Mapped>>>;

using map_type = oneapi::tbb_kahypar::concurrent_map<int, int, std::less<int>, Allocator<int, int>>;
using multimap_type = oneapi::tbb_kahypar::concurrent_multimap<int, int, std::less<int>, Allocator<int, int>>;
...
void test_member_types() {
    ...
    static_assert(std::is_same<typename default_container_type::allocator_type, oneapi::tbb_kahypar::tbb_allocator<std::pair<const int, int>>>::value,
...
struct COMapTraits : OrderedMoveTraitsBase {
    ...
    using container_type = oneapi::tbb_kahypar::concurrent_map<T, T, std::less<T>, Allocator>;
...
struct COMultimapTraits : OrderedMoveTraitsBase {
    template <typename T, typename Allocator>
    using container_type = oneapi::tbb_kahypar::concurrent_multimap<T, T, std::less<T>, Allocator>;
...
TEST_CASE("concurrent_map member types") {
    test_member_types<oneapi::tbb_kahypar::concurrent_map>();
}
...
TEST_CASE("heterogeneous overloads in concurrent_map") {
    test_heterogeneous_functions<oneapi::tbb_kahypar::concurrent_map>();
}

//! Testing insert overloads with generic pair in concurrent_map
//! \brief \ref interface \ref requirement
TEST_CASE("insertion by generic pair in concurrent_map") {
    test_insert_by_generic_pair<oneapi::tbb_kahypar::concurrent_map>();
}
...
TEST_CASE("CTAD support in concurrent_map") {
    test_deduction_guides<oneapi::tbb_kahypar::concurrent_map>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
...
TEST_CASE("test concurrent_map comparisons") {
    test_map_comparisons<oneapi::tbb_kahypar::concurrent_map>();
}

//! Testing concurrent_multimap member types
//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_multimap member types") {
    test_member_types<oneapi::tbb_kahypar::concurrent_multimap>();
}
...
TEST_CASE("heterogeneous overloads in concurrent_multimap") {
    test_heterogeneous_functions<oneapi::tbb_kahypar::concurrent_multimap>();
}
...
TEST_CASE("insertion by generic pair in concurrent_multimap") {
    test_insert_by_generic_pair<oneapi::tbb_kahypar::concurrent_multimap>();
}
...
TEST_CASE("CTAD support in concurrent_multimap") {
    test_deduction_guides<oneapi::tbb_kahypar::concurrent_multimap>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
...
TEST_CASE("test concurrent_multimap comparisons") {
    test_map_comparisons<oneapi::tbb_kahypar::concurrent_multimap>();
}
