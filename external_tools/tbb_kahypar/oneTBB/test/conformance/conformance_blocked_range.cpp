    ...
    for( int x=-10; x<10; ++x ) {
        for( int y=-10; y<10; ++y ) {
            ...
            for( std::size_t k=1; k<10; ++k ) {
                typedef oneapi::tbb_kahypar::blocked_range<AbstractValueType> range_type;
                range_type r( i, j, k );
                ...
                if( x<=y ) {
                    ...
                    if( r.is_divisible() ) {
                        oneapi::tbb_kahypar::blocked_range<AbstractValueType> r2(r,oneapi::tbb_kahypar::split());
...
struct Striker {
    void operator()( const oneapi::tbb_kahypar::blocked_range<int>& r ) const {
        for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator i=r.begin(); i!=r.end(); ++i )
            ++Array[i];
    }
};
...
void ParallelTest() {
    for (int i=0; i<N; i=i<3 ? i+1 : i*3) {
        const oneapi::tbb_kahypar::blocked_range<int> r( 0, i, 10 );
...
TEST_CASE("blocked_range proportional splitting") {
    oneapi::tbb_kahypar::blocked_range<int> original(0, 100);
    oneapi::tbb_kahypar::blocked_range<int> first(original);
    oneapi::tbb_kahypar::proportional_split ps(3, 1);
    oneapi::tbb_kahypar::blocked_range<int> second(first, ps);

    // Test proportional_split -> split conversion
    oneapi::tbb_kahypar::blocked_range<int> copy(original);
    oneapi::tbb_kahypar::split s = oneapi::tbb_kahypar::split(ps);
    oneapi::tbb_kahypar::blocked_range<int> splitted_copy(copy, s);
...
TEST_CASE("Deduction guides") {
    ...
    oneapi::tbb_kahypar::blocked_range r1(v.begin(), v.end());
    static_assert(std::is_same<decltype(r1), oneapi::tbb_kahypar::blocked_range<decltype(v)::iterator>>::value);

    // check blocked_range(blocked_range &)
    oneapi::tbb_kahypar::blocked_range r2(r1);
    static_assert(std::is_same<decltype(r2), decltype(r1)>::value);

    // check blocked_range(blocked_range &&)
    oneapi::tbb_kahypar::blocked_range r3(std::move(r1));
