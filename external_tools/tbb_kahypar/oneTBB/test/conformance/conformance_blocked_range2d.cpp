...
static void SerialTest() {
    ...
    typedef oneapi::tbb_kahypar::blocked_range2d<row_type,col_type> range_type;
...
struct Striker {
   // Note: we use <int> here instead of <long> in order to test for problems similar to Quad 407676
    void operator()( const oneapi::tbb_kahypar::blocked_range2d<int>& r ) const {
        for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator i=r.rows().begin(); i!=r.rows().end(); ++i )
            for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator j=r.cols().begin(); j!=r.cols().end(); ++j )
                ++Array[i][j];
    }
};
...
void ParallelTest() {
    for( int i=0; i<N; i=i<3 ? i+1 : i*3 ) {
        for( int j=0; j<N; j=j<3 ? j+1 : j*3 ) {
            const oneapi::tbb_kahypar::blocked_range2d<int> r( 0, i, 7, 0, j, 5 );
...
TEST_CASE("blocked_range2d proportional splitting") {
    oneapi::tbb_kahypar::blocked_range2d<int> original(0, 100, 0, 100);
    oneapi::tbb_kahypar::blocked_range2d<int> first(original);
    oneapi::tbb_kahypar::proportional_split ps(3, 1);
    oneapi::tbb_kahypar::blocked_range2d<int> second(first, ps);
...
TEST_CASE("Deduction guides") {
    ...
    oneapi::tbb_kahypar::blocked_range2d r1(v.begin(), v.end(), 2, v2.begin(), v2.end(), 2);
    static_assert(std::is_same<decltype(r1), oneapi::tbb_kahypar::blocked_range2d<decltype(v)::iterator, decltype(v2)::iterator>>::value);

    // check blocked_range2d(blocked_range2d &)
    oneapi::tbb_kahypar::blocked_range2d r2(r1);
    static_assert(std::is_same<decltype(r2), decltype(r1)>::value);

    // check blocked_range2d(blocked_range2d &&)
    oneapi::tbb_kahypar::blocked_range2d r3(std::move(r1));
