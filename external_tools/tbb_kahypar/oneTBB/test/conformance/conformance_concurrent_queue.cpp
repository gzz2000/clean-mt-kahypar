...
template <typename T>
using test_allocator = StaticSharedCountingAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<T>>;
...
template<typename T>
class ConcQWithCapacity : public oneapi::tbb_kahypar::concurrent_queue<T, test_allocator<T>> {
    using base_type = oneapi::tbb_kahypar::concurrent_queue<T, test_allocator<T>>;
public:
...
void TestEmptiness() {
    ...
    TestEmptyQueue<oneapi::tbb_kahypar::concurrent_bounded_queue<char, test_allocator<char>>, char>();
    TestEmptyQueue<oneapi::tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo,
...
void TestFullness() {
    ...
    TestFullQueue<oneapi::tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, test_allocator<move_support_tests::Foo>>, move_support_tests::Foo>();
}
...
void TestClearWorks() {
    ...
    TestClear<oneapi::tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, test_allocator<move_support_tests::Foo>>>();
}
...
void TestQueueIteratorWorks() {
    TestIterator<oneapi::tbb_kahypar::concurrent_queue<move_support_tests::Foo, test_allocator<move_support_tests::Foo>>>();
    TestIterator<oneapi::tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, test_allocator<move_support_tests::Foo>>>();
}

// Define wrapper classes to test oneapi::tbb_kahypar::concurrent_queue<T>
template<typename T, typename A = oneapi::tbb_kahypar::cache_aligned_allocator<T>>
class ConcQWithSizeWrapper : public oneapi::tbb_kahypar::concurrent_queue<T, A> {
public:
    ConcQWithSizeWrapper() {}
    ConcQWithSizeWrapper( const ConcQWithSizeWrapper& q ) : oneapi::tbb_kahypar::concurrent_queue<T, A>(q) {}
    ConcQWithSizeWrapper( const ConcQWithSizeWrapper& q, const A& a ) : oneapi::tbb_kahypar::concurrent_queue<T, A>(q, a) {}
    ConcQWithSizeWrapper( const A& a ) : oneapi::tbb_kahypar::concurrent_queue<T, A>( a ) {}

    ConcQWithSizeWrapper( ConcQWithSizeWrapper&& q ) : oneapi::tbb_kahypar::concurrent_queue<T>(std::move(q)) {}
    ConcQWithSizeWrapper( ConcQWithSizeWrapper&& q, const A& a )
        : oneapi::tbb_kahypar::concurrent_queue<T, A>(std::move(q), a) { }

    template<typename InputIterator>
    ConcQWithSizeWrapper( InputIterator begin, InputIterator end, const A& a = A() )
        : oneapi::tbb_kahypar::concurrent_queue<T, A>(begin, end, a) {}
    typename oneapi::tbb_kahypar::concurrent_queue<T, A>::size_type size() const { return this->unsafe_size(); }
};
...
void TestQueueConstructors() {
    ...
    TestConstructors<oneapi::tbb_kahypar::concurrent_bounded_queue<Bar>, Bar, BarIterator, oneapi::tbb_kahypar::concurrent_bounded_queue<BarEx>, BarEx>();
}

template<typename T>
struct TestNegativeQueueBody {
    oneapi::tbb_kahypar::concurrent_bounded_queue<T>& queue;
    const std::size_t nthread;
    TestNegativeQueueBody( oneapi::tbb_kahypar::concurrent_bounded_queue<T>& q, std::size_t n ) : queue(q), nthread(n) {}
...
template<typename T>
class ConcQPushPopWrapper : public oneapi::tbb_kahypar::concurrent_queue<T, test_allocator<T>> {
public:
    ...
    bool try_pop( T& dest ) { return this->oneapi::tbb_kahypar::concurrent_queue<T, test_allocator<T>>::try_pop(dest); }
    std::size_t my_capacity;
};
...
void TestConcurrentPushPop() {
    for (std::size_t nthread = MinThread; nthread <= MaxThread; ++nthread) {
        ...
        for (std::size_t prefill = 0; prefill < 64; prefill += (1 + prefill / 3) ) {
            TestPushPop<oneapi::tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, test_allocator<move_support_tests::Foo>>,
...
void TestExceptions() {
    using allocator_t = StaticSharedCountingAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<std::size_t>>;
    using allocator_char_t = StaticSharedCountingAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<char>>;
    TestExceptionBody<ConcQWithSizeWrapper, allocator_t, allocator_char_t, FooEx>();
    TestExceptionBody<oneapi::tbb_kahypar::concurrent_bounded_queue, allocator_t, allocator_char_t, FooEx>();
...
class SimplePushBody {
    oneapi::tbb_kahypar::concurrent_bounded_queue<int>* q;
    std::size_t max;
public:
    SimplePushBody(oneapi::tbb_kahypar::concurrent_bounded_queue<int>* _q, std::size_t hi_thr) : q(_q), max(hi_thr) {}
...
public:
    SimplePopBody(oneapi::tbb_kahypar::concurrent_bounded_queue<int>* _q, std::size_t hi_thr, std::size_t nitems)
...
void TestAbort() {
    for (std::size_t nthreads = MinThread; nthreads <= MaxThread; ++nthreads) {
        oneapi::tbb_kahypar::concurrent_bounded_queue<int> iq1;
        iq1.set_capacity(0);
        ...
        oneapi::tbb_kahypar::concurrent_bounded_queue<int> iq2;
        iq2.set_capacity(2);
        for (std::size_t i=0; i < 10; ++i) {
            num_pushed.store(0, std::memory_order_relaxed);
            num_popped.store(0, std::memory_order_relaxed);
        ...
        oneapi::tbb_kahypar::concurrent_bounded_queue<int> iq3;
        iq3.set_capacity(2);
        for (std::size_t i = 0; i < 10; ++i) {
            num_pushed.store(0, std::memory_order_relaxed);
            num_popped.store(0, std::memory_order_relaxed);
        ...
        oneapi::tbb_kahypar::concurrent_bounded_queue<int> iq4;
...
void test_member_types() {
    ...
    static_assert(std::is_same<typename container_type::allocator_type, oneapi::tbb_kahypar::cache_aligned_allocator<int>>::value,
...
void TestMoveSupportInPushPop() {
    TestMoveSupport<oneapi::tbb_kahypar::concurrent_queue<MoveOperationTracker>, push_op, try_pop_op>();
    TestMoveSupport<oneapi::tbb_kahypar::concurrent_bounded_queue<MoveOperationTracker>, push_op, pop_op>();
    TestMoveSupport<oneapi::tbb_kahypar::concurrent_bounded_queue<MoveOperationTracker>, try_push_op, try_pop_op>();
}

template<class T>
class allocator: public oneapi::tbb_kahypar::cache_aligned_allocator<T> {
public:
...
void bounded_queue_specific_test(oneapi::tbb_kahypar::concurrent_queue<T, A> &, const std::vector<T> &) { /* do nothing */ }

template <typename T, typename A>
void bounded_queue_specific_test(oneapi::tbb_kahypar::concurrent_bounded_queue<T, A> &q, const std::vector<T> &vec) {
    typedef typename oneapi::tbb_kahypar::concurrent_bounded_queue<T, A>::size_type size_type;

    FillTest<try_push_op>(q, vec);
    oneapi::tbb_kahypar::concurrent_bounded_queue<T, A> q2 = q;
...
void TestMoveConstruction() {
    ...
    TestMoveConstructors<oneapi::tbb_kahypar::concurrent_bounded_queue<Bar, allocator<Bar>>, Bar>();
}
...
void TestEmplace() {
    ...
    TestEmplaceInQueue<oneapi::tbb_kahypar::concurrent_bounded_queue<NonTrivialConstructorType>, emplace_op>();
    TestEmplaceInQueue<oneapi::tbb_kahypar::concurrent_bounded_queue<NonTrivialConstructorType>, try_emplace_op>();
}
...
TEST_CASE("testing concurrent_queues member types"){
    test_member_types<oneapi::tbb_kahypar::concurrent_queue>();
    test_member_types<oneapi::tbb_kahypar::concurrent_bounded_queue>();

    // Test size_type
    static_assert(std::is_unsigned<typename oneapi::tbb_kahypar::concurrent_queue<int>::size_type>::value,
                  "Incorrect oneapi::tbb_kahypar::concurrent_queue::size_type member type");
    static_assert(std::is_signed<typename oneapi::tbb_kahypar::concurrent_bounded_queue<int>::size_type>::value,
                  "Incorrect oneapi::tbb_kahypar::concurrent_bounded_queue::size_type member type");
}
...
TEST_CASE("testing deduction guides") {
    TestDeductionGuides<oneapi::tbb_kahypar::concurrent_queue>();
    TestDeductionGuides<oneapi::tbb_kahypar::concurrent_bounded_queue>();
}
#endif

//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_queue iterator comparisons") {
    TestQueueIteratorComparisons<oneapi::tbb_kahypar::concurrent_queue<int>>();
}

//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_bounded_queue iterator comparisons") {
    TestQueueIteratorComparisons<oneapi::tbb_kahypar::concurrent_bounded_queue<int>>();
}
...
TEST_CASE("Test with minimalistic object type") {
    test_with_minimalistic_objects<oneapi::tbb_kahypar::concurrent_queue>();
    test_with_minimalistic_objects<oneapi::tbb_kahypar::concurrent_bounded_queue>();
}
...
TEST_CASE("testing assignment and swapping") {
    test_queue_helper<tbb_kahypar::concurrent_queue<std::vector<int>>>();
    test_queue_helper<tbb_kahypar::concurrent_bounded_queue<std::vector<int>>>();
}
...
TEST_CASE("testing move assignment with equal counting allocators") {
    ...
    TestMoveQueue<tbb_kahypar::concurrent_queue<move_support_tests::Foo, allocator_type>>();
    TestMoveQueue<tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, allocator_type>>();
}
...
TEST_CASE("testing move assignment with unequal counting allocators") {
    ...
    TestMoveQueueUnequal<tbb_kahypar::concurrent_queue<move_support_tests::Foo, allocator_type>>();
    TestMoveQueueUnequal<tbb_kahypar::concurrent_bounded_queue<move_support_tests::Foo, allocator_type>>();
}
...
void test_move_assignment_test_equal() {
    ...
    tbb_kahypar::concurrent_queue<std::vector<int>> src({vect1, vect2, vect3});
    tbb_kahypar::concurrent_queue<std::vector<int>> dst(src.get_allocator());
    tbb_kahypar::concurrent_queue<std::vector<int>> cpy(src.get_allocator());
    REQUIRE_MESSAGE(src.get_allocator() == dst.get_allocator(), "Incorrect test setup: allocators should be equal");
    cpy = src;
    dst = std::move(src);

    tbb_kahypar::concurrent_bounded_queue<std::vector<int>> src_bnd({vect1, vect2, vect3});
    tbb_kahypar::concurrent_bounded_queue<std::vector<int>> dst_bnd(src_bnd.get_allocator());
    tbb_kahypar::concurrent_bounded_queue<std::vector<int>> cpy_bnd(src_bnd.get_allocator());
    ...
    test_check_move_allocator<tbb_kahypar::concurrent_queue<std::vector<int>>>(src, dst, cpy);
    REQUIRE_MESSAGE(cpy.unsafe_size() == dst.unsafe_size(), "Queues are not equal");

    test_check_move_allocator<tbb_kahypar::concurrent_bounded_queue<std::vector<int>>>(src_bnd, dst_bnd, cpy_bnd);
...
void test_move_assignment_test_unequal() {
    ...
    tbb_kahypar::concurrent_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> src(src_alloc);
    ...
    dst_alloc.state = 1;
    tbb_kahypar::concurrent_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> dst(dst_alloc);
    tbb_kahypar::concurrent_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> cpy(src_alloc);
    REQUIRE_MESSAGE(src.get_allocator() != dst.get_allocator(), "Incorrect test setup: allocators should be unequal");
    cpy = src;
    dst = std::move(src);

    tbb_kahypar::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> src_bnd(src_alloc);
    tbb_kahypar::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> dst_bnd(dst_alloc);
    tbb_kahypar::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> cpy_bnd(src_alloc);
    ...
    test_check_move_allocator<tbb_kahypar::concurrent_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>>>(src, dst, cpy);
    REQUIRE_MESSAGE(dst.unsafe_size() == cpy.unsafe_size(), "Queues are not equal");

    test_check_move_allocator<tbb_kahypar::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>>>(src_bnd, dst_bnd, cpy_bnd);
