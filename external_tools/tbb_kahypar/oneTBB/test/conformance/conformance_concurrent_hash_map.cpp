...
using test_table_type = oneapi::tbb_kahypar::concurrent_hash_map<MyKey, MyData, MyHashCompare, test_allocator_type>;
using other_test_table_type = oneapi::tbb_kahypar::concurrent_hash_map<MyKey, MyData2, MyHashCompare>;

template <template <typename...> class ContainerType>
void test_member_types() {
    using container_type = ContainerType<int, int>;
    static_assert(std::is_same<typename container_type::allocator_type, oneapi::tbb_kahypar::tbb_allocator<std::pair<const int, int>>>::value,
...
void TestAssignment() {
    ...
    oneapi::tbb_kahypar::concurrent_hash_map<int, int> test_map({{1, 2}, {2, 4}});
...
void TestIteratorsAndRanges() {
    ...
    using YourTable1 = oneapi::tbb_kahypar::concurrent_hash_map<MyKey,MyData2,YourHashCompare, test_allocator_type2>;
    using YourTable2 = oneapi::tbb_kahypar::concurrent_hash_map<MyKey,MyData2,YourHashCompare>;
    YourTable1 t1;
    FillTable( t1, 10 );
    CheckTable(t1, 10 );
...
struct ctor_test {
 template<typename container_type, typename element_type>
    static void test( std::initializer_list<element_type> il, container_type const& expected ) {
        container_type vd(il, tbb_kahypar::tbb_allocator<std::pair<element_type, element_type>>{});
        REQUIRE_MESSAGE( vd == expected, "inserting with an initializer list failed" );
    }
};
...
void TestInitList(){
    ...
    using ch_map_type = oneapi::tbb_kahypar::concurrent_hash_map<int,int>;
...
class only_node_counting_allocator : public StaticSharedCountingAllocator<base_alloc_type> {
    ...
    using base_traits = oneapi::tbb_kahypar::detail::allocator_traits<base_alloc_type>;
public:
    template<typename U>
    struct rebind {
...
void TestExceptions() {
    using allocator_type = only_node_counting_allocator<oneapi::tbb_kahypar::tbb_allocator<std::pair<const MyKey, MyData2>>>;
    using throwing_table = oneapi::tbb_kahypar::concurrent_hash_map<MyKey, MyData2, MyHashCompare, allocator_type>;
    enum methods {
        zero_method = 0,
...
struct hash_map_traits : default_container_traits {
    ...
    using container_type = oneapi::tbb_kahypar::concurrent_hash_map<T, T, hash_compare<T>, Allocator>;
    ...
    struct apply {
        using type = oneapi::tbb_kahypar::concurrent_hash_map<element_type, element_type, hash_compare<element_type>, allocator_type>;
    };
...
using DataStateTrackedTable = oneapi::tbb_kahypar::concurrent_hash_map<MyKey, move_support_tests::Foo, MyHashCompare>;
...
public:
    void operator()( const oneapi::tbb_kahypar::blocked_range<int>& range ) const {
...
void DoConcurrentOperations( TableType& table, int n, const char* what, std::size_t nthread ) {
    ...
    oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<int>(0, n ,100), TableOperation<Op, TableType>(table));
}
...
using YourTable = oneapi::tbb_kahypar::concurrent_hash_map<MyKey, MyData, YourHashCompare>;
...
void ParallelTraverseTable( test_table_type& table, size_t n, size_t expected_size ) {
    ...
    oneapi::tbb_kahypar::parallel_for( r, ParallelTraverseBody<test_table_type::range_type>( array, n ));
    Check( array, n, expected_size );

    const test_table_type& const_table = table;
    memset( static_cast<void*>(array), 0, n*sizeof(AtomicByte) );
    test_table_type::const_range_type cr = const_table.range(10);
    oneapi::tbb_kahypar::parallel_for( cr, ParallelTraverseBody<test_table_type::const_range_type>( array, n ));
    Check( array, n, expected_size );
...
void TestConcurrency( std::size_t nthread ) {
    ...
    test_allocator_type::init_counters();
    {
        ...
        EraseCount = 0;
        t0 = oneapi::tbb_kahypar::tick_count::now();
...
struct non_default_constructible_hash_compare : oneapi::tbb_kahypar::detail::d1::tbb_hash_compare<Key> {
...
void TestHashCompareConstructors() {
    using key_type = int;
    using map_type = oneapi::tbb_kahypar::concurrent_hash_map<key_type, key_type, non_default_constructible_hash_compare<key_type>>;
...
template <typename T>
struct debug_hash_compare : public oneapi::tbb_kahypar::detail::d1::tbb_hash_compare<T> {};
...
void TestDeductionGuides() {
    ...
    using DefaultCompare = oneapi::tbb_kahypar::detail::d1::tbb_hash_compare<Key>;
    using Compare = debug_hash_compare<Key>;
    using DefaultAllocator = oneapi::tbb_kahypar::tbb_allocator<ComplexTypeConst>;
...
void TestCHMapComparisons() {
    using integral_container = oneapi::tbb_kahypar::concurrent_hash_map<int, int>;
    using two_way_comparable_container = oneapi::tbb_kahypar::concurrent_hash_map<comparisons_testing::TwoWayComparable,
...
void TestCHMapIteratorComparisons() {
    using chmap_type = oneapi::tbb_kahypar::concurrent_hash_map<int, int>;
...
void test_heterogeneous_find() {
    ...
    using chmap_type = oneapi::tbb_kahypar::concurrent_hash_map<key_type, int, HeterogeneousHashCompare>;
...
void test_heterogeneous_insert() {
    ...
    using chmap_type = oneapi::tbb_kahypar::concurrent_hash_map<key_type, DefaultConstructibleValue, HeterogeneousHashCompare>;
...
TEST_CASE("test types"){
    test_member_types<oneapi::tbb_kahypar::concurrent_hash_map>();
}
...
TEST_CASE("testing deduction guides") {
    TestDeductionGuides<oneapi::tbb_kahypar::concurrent_hash_map>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
