...
struct AllowMultimapping<oneapi::tbb_kahypar::concurrent_multiset<Args...>> : std::true_type {};

template <typename Key>
using Allocator = LocalCountingAllocator<std::allocator<Key>>;

using set_type = oneapi::tbb_kahypar::concurrent_set<int, std::less<int>, Allocator<int>>;
using multiset_type = oneapi::tbb_kahypar::concurrent_multiset<int, std::less<int>, Allocator<int>>;
...
struct COSetTraits : OrderedMoveTraitsBase {
    ...
    using container_type = oneapi::tbb_kahypar::concurrent_set<T, std::less<T>, Allocator>;
...
struct COMultisetTraits : OrderedMoveTraitsBase {
    template <typename T, typename Allocator>
    using container_type = oneapi::tbb_kahypar::concurrent_multiset<T, std::less<T>, Allocator>;
...
TEST_CASE("concurrent_set member types") {
    test_member_types<oneapi::tbb_kahypar::concurrent_set>();
}
...
TEST_CASE("heterogeneous overloads in concurrent_set") {
    test_heterogeneous_functions<oneapi::tbb_kahypar::concurrent_set>();
}
...
TEST_CASE("CTAD support in concurrent_set") {
    test_deduction_guides<oneapi::tbb_kahypar::concurrent_set>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
...
TEST_CASE("test concurrent_set comparisons") {
    test_set_comparisons<oneapi::tbb_kahypar::concurrent_set>();
}

//! Testing concurrent_multiset member types
//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_multiset member types") {
    test_member_types<oneapi::tbb_kahypar::concurrent_multiset>();
}
...
TEST_CASE("heterogeneous overloads in concurrent_multiset") {
    test_heterogeneous_functions<oneapi::tbb_kahypar::concurrent_multiset>();
}
...
TEST_CASE("CTAD support in concurrent_multiset") {
    test_deduction_guides<oneapi::tbb_kahypar::concurrent_multiset>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
...
//! \brief \ref interface \ref requirement
TEST_CASE("test concurrent_set comparisons") {
    test_set_comparisons<oneapi::tbb_kahypar::concurrent_multiset>();
}
