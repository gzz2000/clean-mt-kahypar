...
void WorkProducingTest(Context&... context) {
    for ( auto concurrency_level : utils::concurrency_range() ) {
        ...
        element_counter = 0;
        oneapi::tbb_kahypar::parallel_for_each(v.begin(), v.end(), set_to<0>(), context...);
        ...
        element_counter = 0;
        oneapi::tbb_kahypar::parallel_for_each(v, set_to<1>(), context...);
        REQUIRE_MESSAGE((element_counter == v.size() && element_counter == elements),
            "not all elements were set");
        ...
        element_counter = 0;
        oneapi::tbb_kahypar::parallel_for_each(oneapi::tbb_kahypar::blocked_range<std::deque<size_t>::iterator>(v.begin(), v.end()), set_to<0>(), context...);
        REQUIRE_MESSAGE((element_counter == v.size() && element_counter == elements),
            "not all elements were set");
        REQUIRE_MESSAGE(range_based_for_accumulate(v, std::plus<size_t>(), init_sum) == init_sum,
...
public:
    ...
    void do_action_and_feed(oneapi::tbb_kahypar::feeder<ForEachInvokeItem>& feeder) const {
...
void test_pfor_each_invoke_basic() {
    ...
    // Test without feeder
    oneapi::tbb_kahypar::parallel_for_each(iterator_type(items_to_proceed.data()),
