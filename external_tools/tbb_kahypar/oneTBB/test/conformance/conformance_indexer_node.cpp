...
using my_indexer_type = oneapi::tbb_kahypar::flow::indexer_node<int, float, input_msg>;
...
TEST_CASE("indexer_node broadcasts"){
    ...
    for(std::size_t i = 0; i < 3; ++i) {
        ...
        oneapi::tbb_kahypar::flow::make_edge(testing_node, *receiver_nodes.back());
    }

    oneapi::tbb_kahypar::flow::input_port<0>(testing_node).try_put(6);
    oneapi::tbb_kahypar::flow::input_port<1>(testing_node).try_put(1.5);
    oneapi::tbb_kahypar::flow::input_port<2>(testing_node).try_put(input_msg(1));
    g.wait_for_all();
...
TEST_CASE("indexer_node superclasses"){
    CHECK_MESSAGE((std::is_base_of<oneapi::tbb_kahypar::flow::graph_node, my_indexer_type>::value), "indexer_node should be derived from graph_node");
}
...
TEST_CASE("indexer_node buffering") {
    ...
    oneapi::tbb_kahypar::flow::limiter_node<my_output_type> rejecter(g,0);
    oneapi::tbb_kahypar::flow::make_edge(testing_node, rejecter);
...
TEST_CASE("indexer_node behaviour") {
    oneapi::tbb_kahypar::flow::graph g;
    oneapi::tbb_kahypar::flow::function_node<int, int> f1( g, oneapi::tbb_kahypar::flow::unlimited,
                                [](const int &i) { return 2*i; } );
    oneapi::tbb_kahypar::flow::function_node<float, float> f2( g, oneapi::tbb_kahypar::flow::unlimited,
                                [](const float &f) { return f/2; } );
    oneapi::tbb_kahypar::flow::continue_node<input_msg> c1( g,
                                [](oneapi::tbb_kahypar::flow::continue_msg) { return input_msg(5); } );

    my_indexer_type testing_node(g);

    oneapi::tbb_kahypar::flow::function_node<my_output_type>
        f3( g, oneapi::tbb_kahypar::flow::unlimited,
            []( const my_output_type &v ) {
                if (v.tag() == 0) {
                    CHECK_MESSAGE((v.is_a<int>()), "Expected to int" );
                    CHECK_MESSAGE((oneapi::tbb_kahypar::flow::cast_to<int>(v) == 6), "Expected to receive 6" );
                } else if (v.tag() == 1) {
                    CHECK_MESSAGE((v.is_a<float>()), "Expected to float" );
                    CHECK_MESSAGE((oneapi::tbb_kahypar::flow::cast_to<float>(v) == 1.5), "Expected to receive 1.5" );
                } else {
                    CHECK_MESSAGE((v.is_a<input_msg>()), "Expected to float" );
                    CHECK_MESSAGE((oneapi::tbb_kahypar::flow::cast_to<input_msg>(v) == 5), "Expected to receive input_msg(5)" );
                }
            }
        );
    ...
    oneapi::tbb_kahypar::flow::make_edge(testing_node, f3);
...
TEST_CASE("indexer_node output_type") {
    CHECK_MESSAGE((conformance::check_output_type<my_output_type, oneapi::tbb_kahypar::flow::tagged_msg<size_t, int, float, input_msg>>()), "indexer_node output_type should returns a tagged_msg");
}
