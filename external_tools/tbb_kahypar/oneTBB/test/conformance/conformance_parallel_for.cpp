...
public:
    ...
    FooRange( FooRange& original, oneapi::tbb_kahypar::split ) : size(original.size/2) {
        original.size -= size;
...
struct Invoker<parallel_tag, empty_partitioner_tag, Range, Body> {
    void operator()( const Range& r, const Body& body, empty_partitioner_tag& ) {
        oneapi::tbb_kahypar::parallel_for( r, body );
    }
};
...
struct Invoker<parallel_tag, Partitioner, Range, Body> {
    void operator()( const Range& r, const Body& body, Partitioner& p ) {
        oneapi::tbb_kahypar::parallel_for( r, body, p );
    }
};
...
struct InvokerStep<parallel_tag, empty_partitioner_tag, T, Body> {
    void operator()( const T& first, const T& last, const Body& f, empty_partitioner_tag& ) {
        oneapi::tbb_kahypar::parallel_for( first, last, f );
    }
    void operator()( const T& first, const T& last, const T& step, const Body& f, empty_partitioner_tag& ) {
        oneapi::tbb_kahypar::parallel_for( first, last, step, f );
    }
};
...
struct InvokerStep<parallel_tag, Partitioner, T, Body> {
    void operator()( const T& first, const T& last, const Body& f, Partitioner& p ) {
        oneapi::tbb_kahypar::parallel_for( first, last, f, p );
    }
    void operator()( const T& first, const T& last, const T& step, const Body& f, Partitioner& p ) {
        oneapi::tbb_kahypar::parallel_for( first, last, step, f, p );
    }
};
    ...
    for ( int i=0; i<N; ++i ) {
        for ( int mode = 0; mode < 4; ++mode) {
            ...
            case 0: {
                ...
                break;
            case 1: {
                Invoker< Flavor, const oneapi::tbb_kahypar::simple_partitioner, FooRange<Pad>, FooBody<Pad> > invoke_for;
                invoke_for( rc, fc, oneapi::tbb_kahypar::simple_partitioner() );
            }
                break;
            case 2: {
                Invoker< Flavor, const oneapi::tbb_kahypar::auto_partitioner, FooRange<Pad>, FooBody<Pad> > invoke_for;
                invoke_for( rc, fc, oneapi::tbb_kahypar::auto_partitioner() );
            }
                break;
            case 3: {
                static oneapi::tbb_kahypar::affinity_partitioner affinity;
                Invoker< Flavor, oneapi::tbb_kahypar::affinity_partitioner, FooRange<Pad>, FooBody<Pad> > invoke_for;
...
void TestParallelForWithStepSupport() {
    static oneapi::tbb_kahypar::affinity_partitioner affinity_p;
    oneapi::tbb_kahypar::auto_partitioner auto_p;
    oneapi::tbb_kahypar::simple_partitioner simple_p;
    oneapi::tbb_kahypar::static_partitioner static_p;
    empty_partitioner_tag p;

    // Try out all partitioner combinations
    TestParallelForWithStepSupportHelper< Flavor,T,empty_partitioner_tag >(p);
    TestParallelForWithStepSupportHelper< Flavor,T,const oneapi::tbb_kahypar::auto_partitioner >(auto_p);
    TestParallelForWithStepSupportHelper< Flavor,T,const oneapi::tbb_kahypar::simple_partitioner >(simple_p);
    TestParallelForWithStepSupportHelper< Flavor,T,oneapi::tbb_kahypar::affinity_partitioner >(affinity_p);
    TestParallelForWithStepSupportHelper< Flavor,T,oneapi::tbb_kahypar::static_partitioner >(static_p);

    // Testing some corner cases
    oneapi::tbb_kahypar::parallel_for(static_cast<T>(2), static_cast<T>(1), static_cast<T>(1), TestFunctor<T>());
}
...
void test_pfor_body_invoke() {
    ...
    oneapi::tbb_kahypar::parallel_for(range, &range_type::increase);
    oneapi::tbb_kahypar::parallel_for(range, &range_type::increase, oneapi::tbb_kahypar::simple_partitioner());
    oneapi::tbb_kahypar::parallel_for(range, &range_type::increase, oneapi::tbb_kahypar::auto_partitioner());
    oneapi::tbb_kahypar::parallel_for(range, &range_type::increase, oneapi::tbb_kahypar::static_partitioner());
    oneapi::tbb_kahypar::affinity_partitioner aff;
    oneapi::tbb_kahypar::parallel_for(range, &range_type::increase, aff);
...
void test_pfor_func_invoke() {
    ...
    oneapi::tbb_kahypar::parallel_for(first, last, &SmartIndex::increase);
    oneapi::tbb_kahypar::parallel_for(first, last, &SmartIndex::increase, oneapi::tbb_kahypar::simple_partitioner());
    oneapi::tbb_kahypar::parallel_for(first, last, &SmartIndex::increase, oneapi::tbb_kahypar::auto_partitioner());
    oneapi::tbb_kahypar::parallel_for(first, last, &SmartIndex::increase, oneapi::tbb_kahypar::static_partitioner());
    oneapi::tbb_kahypar::affinity_partitioner aff;
    oneapi::tbb_kahypar::parallel_for(first, last, &SmartIndex::increase, aff);
    ...
    oneapi::tbb_kahypar::parallel_for(first, last, stride, &SmartIndex::increase);
    oneapi::tbb_kahypar::parallel_for(first, last, stride, &SmartIndex::increase, oneapi::tbb_kahypar::simple_partitioner());
    oneapi::tbb_kahypar::parallel_for(first, last, stride, &SmartIndex::increase, oneapi::tbb_kahypar::auto_partitioner());
    oneapi::tbb_kahypar::parallel_for(first, last, stride, &SmartIndex::increase, oneapi::tbb_kahypar::static_partitioner());
    oneapi::tbb_kahypar::parallel_for(first, last, stride, &SmartIndex::increase, aff);
...
TEST_CASE("Basic parallel_for") {
    ...
    oneapi::tbb_kahypar::parallel_for(std::size_t(0), iterations, [&](std::size_t) {
        counter++;
    });

    oneapi::tbb_kahypar::parallel_for(std::size_t(0), iterations, [&](std::size_t) {
        counter++;
    }, oneapi::tbb_kahypar::simple_partitioner());

    oneapi::tbb_kahypar::parallel_for(std::size_t(0), iterations, [&](std::size_t) {
        counter++;
    }, oneapi::tbb_kahypar::auto_partitioner());

    oneapi::tbb_kahypar::parallel_for(std::size_t(0), iterations, [&](std::size_t) {
        counter++;
    }, oneapi::tbb_kahypar::static_partitioner());
...
TEST_CASE("Testing parallel_for with partitioners") {
    ...
    parallel_for(Range1(false, true), b, oneapi::tbb_kahypar::simple_partitioner());
    parallel_for(Range6(false, true), b, oneapi::tbb_kahypar::simple_partitioner());

    parallel_for(Range1(false, true), b, oneapi::tbb_kahypar::auto_partitioner());
    parallel_for(Range6(false, true), b, oneapi::tbb_kahypar::auto_partitioner());

    parallel_for(Range1(true, false), b, oneapi::tbb_kahypar::static_partitioner());
    parallel_for(Range6(false, true), b, oneapi::tbb_kahypar::static_partitioner());
}
