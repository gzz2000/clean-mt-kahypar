...
TEST_CASE("NUMA aware arenas task execution test") {
    ...
    for(auto& numa_index: oneapi::tbb_kahypar::info::numa_nodes()) {
        oneapi::tbb_kahypar::task_arena arena(oneapi::tbb_kahypar::task_arena::constraints{numa_index});
...
TEST_CASE("Test NUMA topology traversal correctness") {
    ...
    std::vector<oneapi::tbb_kahypar::numa_node_id> numa_indexes = oneapi::tbb_kahypar::info::numa_nodes();
...
TEST_CASE("Test validity of NUMA interfaces when HWLOC is not present on the system") {
    ...
    // Do lvalue-to-rvalue conversion to not odr-use tbb_kahypar::task_arena::automatic
    REQUIRE_MESSAGE(numa_indexes[0] != static_cast<tbb_kahypar::numa_node_id>(tbb_kahypar::task_arena::automatic),
        "Index of NUMA node must NOT be pinned to tbb_kahypar::task_arena::automatic, since self-contained TBBBind is loaded.");
#else
    REQUIRE_MESSAGE(numa_indexes.size() == 1,
        "Number of NUMA nodes must be pinned to 1, if we have no HWLOC on the system.");
    REQUIRE_MESSAGE(numa_indexes[0] == static_cast<tbb_kahypar::numa_node_id>(tbb_kahypar::task_arena::automatic),
        "Index of NUMA node must be pinned to tbb_kahypar::task_arena::automatic, if we have no HWLOC on the system.");
    REQUIRE_MESSAGE(oneapi::tbb_kahypar::info::default_concurrency(numa_indexes[0]) == utils::get_platform_max_threads(),
