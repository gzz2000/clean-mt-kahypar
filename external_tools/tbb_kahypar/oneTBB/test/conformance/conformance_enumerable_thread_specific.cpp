...
#include "oneapi_kahypar/tbb_kahypar/detail/_utils.h"
...
template<size_t N = oneapi::tbb_kahypar::detail::max_nfs_size>
...
T& check_alignment(T& t, const char *aname) {
    if( !oneapi::tbb_kahypar::detail::is_aligned(&t, AlignMask)) {
...
class parallel_scalar_body: utils::NoAssign {
    typedef oneapi::tbb_kahypar::enumerable_thread_specific<T, Allocator<T> > ets_type;
    ets_type &sums;
...
public:
    ...
    void operator()( const oneapi::tbb_kahypar::blocked_range<int> &r ) const {
...
void run_parallel_scalar_tests_nocombine(const char* /* test_name */, const char *allocator_name) {
    ...
    gThrowValue = 0;
    {
        ...
        for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
            oneapi::tbb_kahypar::global_control gc(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, p);
            ...
            for (int t = -1; t < REPETITIONS; ++t) {
                ...
                REQUIRE( sums.empty());
                oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>( 0, N*p, RANGE_MIN ), parallel_scalar_body<T,Allocator>( sums, allocator_name ) );
                REQUIRE( !sums.empty());

                REQUIRE( finit_ets.empty());
                oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>( 0, N*p, RANGE_MIN ), parallel_scalar_body<T,Allocator>( finit_ets, allocator_name ) );
                REQUIRE( !finit_ets.empty());

                REQUIRE(static_sums.empty());
                oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>( 0, N*p, RANGE_MIN ), parallel_scalar_body<T,Allocator>( static_sums, allocator_name ) );
                REQUIRE( !static_sums.empty());
                ...
                typedef typename oneapi::tbb_kahypar::enumerable_thread_specific<T, Allocator<T>, oneapi::tbb_kahypar::ets_key_per_instance> cached_ets_type;

                cached_ets_type cconst(sums);
                oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>(0, N*p, RANGE_MIN), [&]( const oneapi::tbb_kahypar::blocked_range<int>& ) {
                    bool exists = false;
...

            {
...
#endif
                {
                    for (int t = -1; t < REPETITIONS; ++t) {
                        ...
                        REQUIRE(sums.empty());
                        oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<int>(0, N * p, RANGE_MIN),
...
class parallel_vector_for_body: utils::NoAssign {
    typedef std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> > container_type;
    typedef oneapi::tbb_kahypar::enumerable_thread_specific< container_type, Allocator<container_type> > ets_type;
    ets_type &locals;
    const char *allocator_name;
...
struct parallel_vector_reduce_body {
    ...
    parallel_vector_reduce_body ( parallel_vector_reduce_body<R, T> &, oneapi::tbb_kahypar::split ) : count(0) {  test_helper<T>::init(sum); }
...
void run_parallel_vector_tests(const char* /* test_name */, const char *allocator_name) {
    ...
    for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            ...
            oneapi::tbb_kahypar::parallel_reduce ( vs.range(1), pvrb );
            ...
            oneapi::tbb_kahypar::flattened2d<ets_type> fvs = flatten2d(vs);
            size_t ccount = fvs.size();
            REQUIRE( ccount == size_t(N*p) );
            size_t elem_cnt = 0;
            typename oneapi::tbb_kahypar::flattened2d<ets_type>::iterator it;
            auto it2(it);
            it = fvs.begin();
            REQUIRE(it != it2);
            typename oneapi::tbb_kahypar::flattened2d<ets_type>::iterator it3;
            typename oneapi::tbb_kahypar::flattened2d<ets_type>::const_iterator cit = fvs.begin();
            it3 = cit;
            REQUIRE(it3 == cit);
            REQUIRE(it3.operator->() == &(*it3));

            for(typename oneapi::tbb_kahypar::flattened2d<ets_type>::const_iterator i = fvs.begin(); i != fvs.end(); ++i) {
                ++elem_cnt;
            };
            REQUIRE( ccount == elem_cnt );

            elem_cnt = 0;
            for(typename oneapi::tbb_kahypar::flattened2d<ets_type>::iterator i = fvs.begin(); i != fvs.end(); i++) {
                ++elem_cnt;
            };
            REQUIRE( ccount == elem_cnt );
            ...
            ets_type vvs( typename container_type::size_type(25), minus_one, oneapi::tbb_kahypar::tbb_allocator<T>() );
            REQUIRE( vvs.empty() );
            oneapi::tbb_kahypar::parallel_for ( oneapi::tbb_kahypar::blocked_range<int> (0, N*p, RANGE_MIN), parallel_vector_for_body<T,Allocator>( vvs, allocator_name ) );
            REQUIRE( !vvs.empty() );

            parallel_vector_reduce_body< typename ets_type::const_range_type, T > pvrb2;
            oneapi::tbb_kahypar::parallel_reduce ( vvs.range(1), pvrb2 );
            REQUIRE( pvrb2.count == vvs.size() );
            REQUIRE( test_helper<T>::get(pvrb2.sum) == N*p-pvrb2.count*25 );

            oneapi::tbb_kahypar::flattened2d<ets_type> fvvs = flatten2d(vvs);
            ccount = fvvs.size();
...
void run_cross_type_vector_tests(const char* /* test_name */) {
    ...
    for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            typedef typename oneapi::tbb_kahypar::enumerable_thread_specific< container_type, Allocator<container_type>, oneapi::tbb_kahypar::ets_no_key > ets_nokey_type;
            typedef typename oneapi::tbb_kahypar::enumerable_thread_specific< container_type, Allocator<container_type>, oneapi::tbb_kahypar::ets_key_per_instance > ets_tlskey_type;
            ets_nokey_type vs;

            REQUIRE( vs.empty());
            oneapi::tbb_kahypar::parallel_for ( oneapi::tbb_kahypar::blocked_range<int> (0, N*p, RANGE_MIN), parallel_vector_for_body<T, Allocator>( vs, allocator_name ) );
            REQUIRE( !vs.empty());
            ...
            oneapi::tbb_kahypar::parallel_reduce ( vs3.range(1), pvrb );

            test_helper<T>::sum(sum, pvrb.sum);

            REQUIRE( vs3.size() == pvrb.count);

            oneapi::tbb_kahypar::flattened2d<ets_nokey_type> fvs = flatten2d(vs3);
            size_t ccount = fvs.size();
            size_t elem_cnt = 0;
            for(typename oneapi::tbb_kahypar::flattened2d<ets_nokey_type>::const_iterator i = fvs.begin(); i != fvs.end(); ++i) {
                ++elem_cnt;
            };
            REQUIRE(ccount == elem_cnt);

            elem_cnt = 0;
            for(typename oneapi::tbb_kahypar::flattened2d<ets_nokey_type>::iterator i = fvs.begin(); i != fvs.end(); ++i) {
                ++elem_cnt;
            };
            REQUIRE(ccount == elem_cnt);

            oneapi::tbb_kahypar::flattened2d<ets_nokey_type> fvs2 = flatten2d(vs3, vs3.begin(), std::next(vs3.begin()));
            REQUIRE(std::distance(fvs2.begin(), fvs2.end()) == vs3.begin()->size());
            const oneapi::tbb_kahypar::flattened2d<ets_nokey_type>& cfvs2(fvs2);
...
void run_serial_vector_tests(const char* /* test_name */) {
    ...
    for (int t = -1; t < REPETITIONS; ++t) {
        std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> > v;
        for (int i = 0; i < N; ++i) {
            v.push_back( one );
        }
        for (typename std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> >::const_iterator i = v.begin(); i != v.end(); ++i)
...
const size_t line_size = oneapi::tbb_kahypar::detail::max_nfs_size;
...
void run_reference_check() {
    ...
    run_serial_vector_tests<int>("std::vector<int, oneapi::tbb_kahypar::tbb_allocator<int> >");
    run_serial_vector_tests<double>("std::vector<double, oneapi::tbb_kahypar::tbb_allocator<double> >");
}
...
void run_parallel_tests(const char *allocator_name) {
    ...
    run_parallel_vector_tests<int, Allocator>("std::vector<int, oneapi::tbb_kahypar::tbb_allocator<int> >",allocator_name);
    run_parallel_vector_tests<double, Allocator>("std::vector<double, oneapi::tbb_kahypar::tbb_allocator<double> >",allocator_name);
}

void run_cross_type_tests() {
    // cross-type scalar tests are part of run_parallel_scalar_tests_nocombine
    run_cross_type_vector_tests<int, oneapi::tbb_kahypar::tbb_allocator>("std::vector<int, oneapi::tbb_kahypar::tbb_allocator<int> >");
    run_cross_type_vector_tests<double, oneapi::tbb_kahypar::tbb_allocator>("std::vector<double, oneapi::tbb_kahypar::tbb_allocator<double> >");
}

template<typename T, template<class> class Allocator, typename Init>
oneapi::tbb_kahypar::enumerable_thread_specific<T,Allocator<T> > MakeETS( Init init ) {
    return oneapi::tbb_kahypar::enumerable_thread_specific<T,Allocator<T> >(init);
}
...
oneapi::tbb_kahypar::enumerable_thread_specific<T,Allocator<T> > MakeETS( oneapi::tbb_kahypar::detail::stored_pack<P...> pack ) {
    typedef oneapi::tbb_kahypar::enumerable_thread_specific<T,Allocator<T> > result_type;
    return oneapi::tbb_kahypar::detail::call_and_return< result_type >(
...
void run_assign_and_copy_constructor_test(const char* /* test_name */, const char *allocator_name) {
    ...
    // The arguments are wrapped into oneapi::tbb_kahypar::internal::stored_pack to avoid variadic templates in ets_copy_assign_test.
    test_helper<T>::set(src_init,EXPECTED*17);
    ets_copy_assign_test< std::pair<int,T>, Allocator>(oneapi::tbb_kahypar::detail::save_pack(17,src_init), std::make_pair(-1,T()), Validator<T,EXPECTED>(), allocator_name);
    #undef EXPECTED
}
...
void TestInstantiation(const char* /* allocator_name */) {
    ...
    oneapi::tbb_kahypar::enumerable_thread_specific<utils::NoCopy, Allocator<utils::NoCopy> > ets1;
    ets1.local();
    ...
    oneapi::tbb_kahypar::enumerable_thread_specific<HasNoDefaultConstructor, Allocator<HasNoDefaultConstructor> > ets2(x);
    ets2.local();
    ...
    oneapi::tbb_kahypar::enumerable_thread_specific<HasNoDefaultConstructor, Allocator<HasNoDefaultConstructor> > ets3(f);
    ets3.local();
    ets3.combine(HasNoDefaultConstructorCombine());

    // Test instantiation with multiple arguments
    oneapi::tbb_kahypar::enumerable_thread_specific<HasSpecialAndMoveCtor, Allocator<HasSpecialAndMoveCtor> > ets4(SecretTag, 0x42, "meaningless");
    ets4.local();
    ...
    oneapi::tbb_kahypar::enumerable_thread_specific<HasSpecialAndMoveCtor, Allocator<HasSpecialAndMoveCtor> > ets5(SecretTag);
    ets5.local();
    ...
    oneapi::tbb_kahypar::enumerable_thread_specific<HasSpecialAndMoveCtor> ets6( std::move(ets4) );
    ets6.combine_each(EmptyCombineEach<HasSpecialAndMoveCtor>());
    ets6 = std::move(ets5);
}

void TestMemberTypes() {
    using default_container_type = oneapi::tbb_kahypar::enumerable_thread_specific<int>;
    static_assert(std::is_same<typename default_container_type::allocator_type, oneapi::tbb_kahypar::cache_aligned_allocator<int>>::value,
            "Incorrect default template allocator");

    using test_allocator_type = std::allocator<int>;
    using ets_container_type = oneapi::tbb_kahypar::enumerable_thread_specific<int, test_allocator_type>;
...
size_t init_tbb_alloc_mask() {
    ...
    if(oneapi::tbb_kahypar::tbb_allocator<int>::allocator_type() == oneapi::tbb_kahypar::tbb_allocator<int>::standard) {
...
static const size_t cache_allocator_mask = oneapi::tbb_kahypar::detail::r1::cache_line_size();
static const size_t tbb_allocator_mask = init_tbb_alloc_mask();

void TestETSIterator() {
    using ets_type = oneapi::tbb_kahypar::enumerable_thread_specific<int>;
    if (utils::get_platform_max_threads() == 1) {
        ...
        oneapi::tbb_kahypar::parallel_invoke(fill_ets_body, fill_ets_body);
...
TEST_CASE("Instantiation") {
    ...
    TestInstantiation<oneapi::tbb_kahypar::cache_aligned_allocator>("oneapi::tbb_kahypar::cache_aligned_allocator");
    AlignMask = tbb_allocator_mask;
    TestInstantiation<oneapi::tbb_kahypar::tbb_allocator>("oneapi::tbb_kahypar::tbb_allocator");
}
...
TEST_CASE("Assignment and copy constructor") {
    AlignMask = cache_allocator_mask;
    run_assignment_and_copy_constructor_tests<oneapi::tbb_kahypar::cache_aligned_allocator>("oneapi::tbb_kahypar::cache_aligned_allocator");
    AlignMask = tbb_allocator_mask;
    run_assignment_and_copy_constructor_tests<oneapi::tbb_kahypar::tbb_allocator>("oneapi::tbb_kahypar::tbb_allocator");
}
...
TEST_CASE("Parallel test") {
    run_reference_check();
    AlignMask = cache_allocator_mask;
    run_parallel_tests<oneapi::tbb_kahypar::cache_aligned_allocator>("oneapi::tbb_kahypar::cache_aligned_allocator");
    AlignMask = tbb_allocator_mask;
    run_parallel_tests<oneapi::tbb_kahypar::tbb_allocator>("oneapi::tbb_kahypar::tbb_allocator");
