...
TEST_CASE("collaborative_once_flag member functions match") {
    REQUIRE_MESSAGE(std::is_default_constructible<oneapi::tbb_kahypar::collaborative_once_flag>::value == true, 
        "collaborative_once_flag must be default constructible");
    REQUIRE_MESSAGE(std::is_copy_constructible<oneapi::tbb_kahypar::collaborative_once_flag>::value == false, 
        "collaborative_once_flag must not be copy constructible");
    REQUIRE_MESSAGE(std::is_copy_assignable<oneapi::tbb_kahypar::collaborative_once_flag>::value == false, 
        "collaborative_once_flag must not be copy assignable");
    REQUIRE_MESSAGE(std::is_move_constructible<oneapi::tbb_kahypar::collaborative_once_flag>::value == false, 
        "collaborative_once_flag must not be move constructible");
    REQUIRE_MESSAGE(std::is_move_assignable<oneapi::tbb_kahypar::collaborative_once_flag>::value == false, 
...
TEST_CASE("collaborative_call_once executes function exactly once") {
    oneapi::tbb_kahypar::collaborative_once_flag once_flag;

    for (int iter = 0; iter < 100; ++iter) {
        oneapi::tbb_kahypar::collaborative_call_once(once_flag, [](int number) {
    ...
    oneapi::tbb_kahypar::collaborative_once_flag once_flag_concurrent;
    utils::NativeParallelFor(num_threads, [&](std::size_t) {
        barrier.wait();
        oneapi::tbb_kahypar::collaborative_call_once(once_flag_concurrent, func);
    });
    REQUIRE(flag == 1);
}
...
TEST_CASE("Exception is received only by winner thread") {
    ...
    oneapi::tbb_kahypar::collaborative_once_flag flag;

    for (int i = 0; i < num_threads-1; ++i) {
        tg.run([&flag, &barrier] {
            barrier.wait();
            try {
                oneapi::tbb_kahypar::collaborative_call_once(flag, [] { });
            } catch(...) {
    ...
    try {
        oneapi::tbb_kahypar::collaborative_call_once(flag, [&barrier] {
            barrier.wait();
            throw std::exception{};
        });
    } catch (std::exception&) {
