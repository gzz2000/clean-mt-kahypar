...
#pragma GCC diagnostic ignored "-Wsequence-point"
    ...
    static range_t make_range(std::size_t shift, bool negative, val_t(*gen)(input_t), oneapi::tbb_kahypar::detail::index_sequence<Is...>) {
...
void SerialTest() {
    static_assert((oneapi::tbb_kahypar::blocked_nd_range<int, DimAmount>::dim_count() == oneapi::tbb_kahypar::blocked_nd_range<AbstractValueType, DimAmount>::dim_count()),
                         "different amount of dimensions");

    using range_t = oneapi::tbb_kahypar::blocked_nd_range<AbstractValueType, DimAmount>;
    using utils_t = range_utils<range_t, DimAmount>;

    // Generate empty range
    range_t r = utils_t::make_range(0, true, &MakeAbstractValue, oneapi::tbb_kahypar::detail::make_index_sequence<DimAmount>());
    ...
    r = utils_t::make_range(1, true, &MakeAbstractValue, oneapi::tbb_kahypar::detail::make_index_sequence<DimAmount>());
    REQUIRE((r.empty() == utils_t::is_empty(r) && !r.empty()));
    REQUIRE((r.is_divisible() == utils_t::is_divisible(r) && r.is_divisible()));

    range_t r_new(r, oneapi::tbb_kahypar::split());
...
template<unsigned int DimAmount>
void ParallelTest() {
    using range_t = oneapi::tbb_kahypar::blocked_nd_range<int, DimAmount>;
    using utils_t = range_utils<range_t, DimAmount>;
    ...
    range_t r = utils_t::make_range((1 << (20 / DimAmount - 1)) - DimAmount, false, &MakeInt, oneapi::tbb_kahypar::detail::make_index_sequence<DimAmount>());

    oneapi::tbb_kahypar::parallel_for(r, [&data](const range_t& range) {
...
TEST_CASE("Construction") {
    oneapi::tbb_kahypar::blocked_nd_range<int, 1>{ { 0,13,3 } };

    oneapi::tbb_kahypar::blocked_nd_range<int, 1>{ oneapi::tbb_kahypar::blocked_range<int>{ 0,13,3 } };

    oneapi::tbb_kahypar::blocked_nd_range<int, 2>(oneapi::tbb_kahypar::blocked_range<int>(-8923, 8884, 13), oneapi::tbb_kahypar::blocked_range<int>(-8923, 5, 13));

    oneapi::tbb_kahypar::blocked_nd_range<int, 2>({ -8923, 8884, 13 }, { -8923, 8884, 13 });

    oneapi::tbb_kahypar::blocked_range<int> r1(0, 13);

    oneapi::tbb_kahypar::blocked_range<int> r2(-12, 23);

    oneapi::tbb_kahypar::blocked_nd_range<int, 2>({ { -8923, 8884, 13 }, r1});

    oneapi::tbb_kahypar::blocked_nd_range<int, 2>({ r2, r1 });

    oneapi::tbb_kahypar::blocked_nd_range<int, 2>(r1, r2);

    int sizes[] = {174, 39, 2481, 93};
    oneapi::tbb_kahypar::blocked_nd_range<int, 4> rNd_1(sizes, /*grainsize*/7);

    oneapi::tbb_kahypar::blocked_nd_range<int, 4> rNd_2({174, 39, 2481, 93}, /*grainsize*/11);

    for (unsigned i = 0; i < rNd_1.dim_count(); ++i) {
        oneapi::tbb_kahypar::blocked_nd_range<int, 4>::dim_range_type dim1 = rNd_1.dim(i);
        oneapi::tbb_kahypar::blocked_nd_range<int, 4>::dim_range_type dim2 = rNd_2.dim(i);
    ...
    oneapi::tbb_kahypar::blocked_nd_range<AbstractValueType, 4>({ MakeAbstractValue(-3), MakeAbstractValue(13), 8 },
...
TEST_CASE("blocked_nd_range proportional splitting") {
    oneapi::tbb_kahypar::blocked_nd_range<int, 2> original{{0, 100}, {0, 100}};
    oneapi::tbb_kahypar::blocked_nd_range<int, 2> first(original);
    oneapi::tbb_kahypar::proportional_split ps(3, 1);
    oneapi::tbb_kahypar::blocked_nd_range<int, 2> second(first, ps);
