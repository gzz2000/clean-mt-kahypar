...
public:
    ...
    void operator()( const oneapi::tbb_kahypar::blocked_range<std::size_t>& r, Tag ) {
        T temp = sum;
    ...
    Body( Body& b, oneapi::tbb_kahypar::split ): identity(b.identity), sum(b.identity), y(b.y), z(b.z) {}
...
struct parallel_scan_wrapper{
    ...
    oneapi::tbb_kahypar::parallel_scan(std::forward<Args>(args)..., Partitioner());
    }
};
...
struct parallel_scan_wrapper<default_partitioner_tag>{
    template<typename... Args>
    void operator()(Args&&... args) {
    oneapi::tbb_kahypar::parallel_scan(std::forward<Args>(args)...);
    }
};
...
TEST_CASE("scan tags testing") {
    CHECK(oneapi::tbb_kahypar::pre_scan_tag::is_final_scan()==false);
    CHECK(oneapi::tbb_kahypar::final_scan_tag::is_final_scan()==true);
    CHECK((bool)oneapi::tbb_kahypar::pre_scan_tag()==false);
    CHECK((bool)oneapi::tbb_kahypar::final_scan_tag()==true);
}

//! Test parallel prefix sum calculation for body-based interface
//! \brief \ref requirement \ref interface
TEST_CASE_TEMPLATE("Test parallel scan with body", Partitioner, default_partitioner_tag, oneapi::tbb_kahypar::simple_partitioner, oneapi::tbb_kahypar::auto_partitioner) {
    ...
    parallel_scan_wrapper<Partitioner>()(oneapi::tbb_kahypar::blocked_range<std::size_t>(0U, size, 1U), body);
...
TEST_CASE_TEMPLATE("Test parallel scan with body", Partitioner, default_partitioner_tag, oneapi::tbb_kahypar::simple_partitioner, oneapi::tbb_kahypar::auto_partitioner) {
    ...
    parallel_scan_wrapper<Partitioner>()(oneapi::tbb_kahypar::blocked_range<std::size_t>(0U, size, 1U), std::size_t(0),
        [&](const oneapi::tbb_kahypar::blocked_range<std::size_t>& r, std::size_t sum, bool is_final) -> std::size_t
        {
            std::size_t temp = sum;
            for (std::size_t i = r.begin(); i<r.end(); ++i) {
                temp = temp + input[i];
...
void test_pscan_invoke(const std::vector<std::size_t>& desired_vector,
                       ...
                       Args&&... args) {
    auto result = oneapi::tbb_kahypar::parallel_scan(std::forward<Args>(args)...);
...
TEST_CASE("parallel_scan and std::invoke") {
    ...
    test_pscan_invoke(desired_vector, change_vector, range, identity, scan, combine, oneapi::tbb_kahypar::auto_partitioner());
    test_pscan_invoke(desired_vector, change_vector, range, identity, scan, combine, oneapi::tbb_kahypar::simple_partitioner());
}
