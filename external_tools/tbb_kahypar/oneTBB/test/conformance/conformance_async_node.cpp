...
TEST_CASE("async_node constructors"){
    ...
    async_node<int, int> fn2(g, unlimited, fun, oneapi::tbb_kahypar::flow::node_priority_t(1));

    async_node<int, int, lightweight> lw_node1(g, serial, fun, lightweight());
    async_node<int, int, lightweight> lw_node2(g, serial, fun, lightweight(), oneapi::tbb_kahypar::flow::node_priority_t(1));
}
...
TEST_CASE("async_node buffering") {
    conformance::dummy_functor<int> fun;
    conformance::test_buffering<oneapi::tbb_kahypar::flow::async_node<input_msg, int>, input_msg>(oneapi::tbb_kahypar::flow::unlimited, fun);
}

//! Test priorities work in single-threaded configuration
//! \brief \ref requirement
TEST_CASE("async_node priority support"){
    conformance::test_priority<oneapi::tbb_kahypar::flow::async_node<input_msg, int>, input_msg>(oneapi::tbb_kahypar::flow::unlimited);
}
...
TEST_CASE("async_node copy constructor"){
    conformance::test_copy_ctor<oneapi::tbb_kahypar::flow::async_node<int, int>>();
}

//! Test calling async body
//! \brief \ref interface \ref requirement
TEST_CASE("Test async_node body") {
    conformance::test_body_exec<oneapi::tbb_kahypar::flow::async_node<input_msg, output_msg>, input_msg, output_msg>(oneapi::tbb_kahypar::flow::unlimited);
}

//! Test async_node inheritance relations
//! \brief \ref interface
TEST_CASE("async_node superclasses"){
    conformance::test_inheritance<oneapi::tbb_kahypar::flow::async_node<int, int>, int, int>();
    conformance::test_inheritance<oneapi::tbb_kahypar::flow::async_node<void*, float>, void*, float>();
    conformance::test_inheritance<oneapi::tbb_kahypar::flow::async_node<input_msg, output_msg>, input_msg, output_msg>();
}
...
TEST_CASE("async_node broadcast"){
    ...
    conformance::test_forwarding<oneapi::tbb_kahypar::flow::async_node<input_msg, int>, input_msg, int>(1, oneapi::tbb_kahypar::flow::unlimited, fun);
}
...
TEST_CASE("concurrency follows set limits"){
    conformance::test_concurrency<oneapi::tbb_kahypar::flow::async_node<int, int>>();
}
...
TEST_CASE("async_node body copying"){
    conformance::test_copy_body_function<oneapi::tbb_kahypar::flow::async_node<int, int>, conformance::copy_counting_object<int>>(oneapi::tbb_kahypar::flow::unlimited);
}

//! Test node reject the incoming message if the concurrency limit achieved.
//! \brief \ref interface
TEST_CASE("async_node with rejecting policy"){
    conformance::test_rejecting<oneapi::tbb_kahypar::flow::async_node<int, int, oneapi::tbb_kahypar::flow::rejecting>>();
}
...
TEST_CASE("Test async_node Output and Input class") {
    ...
    conformance::test_output_input_class<oneapi::tbb_kahypar::flow::async_node<Body, Body>, Body>();
}

//! Test the body of assync_node typically submits the messages to an external activity for processing outside of the graph.
//! \brief \ref interface
TEST_CASE("async_node with rejecting policy"){
    using async_node_type = tbb_kahypar::flow::async_node<int, int>;
