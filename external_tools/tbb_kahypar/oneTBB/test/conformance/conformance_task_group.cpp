...
TEST_CASE("Task handle run"){
    oneapi::tbb_kahypar::task_handle h;
...
TEST_CASE("Task handle being non copyable"){
    static_assert(
              (!std::is_copy_constructible<oneapi::tbb_kahypar::task_handle>::value)
            &&(!std::is_copy_assignable<oneapi::tbb_kahypar::task_handle>::value),
            "oneapi::tbb_kahypar::task_handle should be non copyable");
}
...
TEST_CASE("Task handle blocks wait"){
    ...
    oneapi::tbb_kahypar::global_control s(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, 1);
    oneapi::tbb_kahypar::task_group tg;
...
TEST_CASE("Respect task_group_context passed from outside") {
    accept_task_group_context::test<tbb_kahypar::task_group>();
}
