...
struct ReduceBody {
    ...
    ReduceBody() : my_value() {}
    ReduceBody( ReduceBody &, oneapi::tbb_kahypar::split ) : my_value() {}

    void operator() ( const oneapi::tbb_kahypar::blocked_range<int>& r ) {
...
void TestDeterministicReductionFor() {
    const int N = 1000;
    const oneapi::tbb_kahypar::blocked_range<int> range(0, N);
    ...
    for ( int i=0; i<100; ++i ) {
        ...
        Type lambda_measurement_result = deterministic_reduce_invoker<Type>( range,
            [](const oneapi::tbb_kahypar::blocked_range<int>& br, Type value) -> Type {
                utils::ConcurrencyTracker ct;
...
TEST_CASE("Test deterministic reduce correctness") {
    for ( auto concurrency_level : utils::concurrency_range() ) {
        ...
        TestDeterministicReductionFor<oneapi::tbb_kahypar::simple_partitioner>();
        TestDeterministicReductionFor<oneapi::tbb_kahypar::static_partitioner>();
...
TEST_CASE("Test partitioners interaction with various ranges") {
    ...
    for ( auto concurrency_level : utils::concurrency_range() ) {
        ...
        oneapi::tbb_kahypar::affinity_partitioner ap;
        ...
        parallel_reduce(Range1(/*assert_in_split*/ true, /*assert_in_proportional_split*/ false), body, oneapi::tbb_kahypar::static_partitioner());
        parallel_reduce(Range6(false, true), body, oneapi::tbb_kahypar::static_partitioner());

        parallel_reduce(Range1(/*assert_in_split*/ false, /*assert_in_proportional_split*/ true), body, oneapi::tbb_kahypar::simple_partitioner());
        parallel_reduce(Range6(false, true), body, oneapi::tbb_kahypar::simple_partitioner());

        parallel_reduce(Range1(/*assert_in_split*/ false, /*assert_in_proportional_split*/ true), body, oneapi::tbb_kahypar::auto_partitioner());
        parallel_reduce(Range6(false, true), body, oneapi::tbb_kahypar::auto_partitioner());

        parallel_deterministic_reduce(Range1(/*assert_in_split*/true, /*assert_in_proportional_split*/ false), body, oneapi::tbb_kahypar::static_partitioner());
        parallel_deterministic_reduce(Range6(false, true), body, oneapi::tbb_kahypar::static_partitioner());

        parallel_deterministic_reduce(Range1(/*assert_in_split*/false, /*assert_in_proportional_split*/ true), body, oneapi::tbb_kahypar::simple_partitioner());
        parallel_deterministic_reduce(Range6(false, true), body, oneapi::tbb_kahypar::simple_partitioner());
    }
}
...
void test_preduce_invoke_basic(const Body& body, const Reduction& reduction) {
    ...
    CHECK(result == oneapi::tbb_kahypar::parallel_reduce(range, identity, body, reduction).get());
    CHECK(result == oneapi::tbb_kahypar::parallel_reduce(range, identity, body, reduction, oneapi::tbb_kahypar::simple_partitioner()).get());
    CHECK(result == oneapi::tbb_kahypar::parallel_reduce(range, identity, body, reduction, oneapi::tbb_kahypar::auto_partitioner()).get());
    CHECK(result == oneapi::tbb_kahypar::parallel_reduce(range, identity, body, reduction, oneapi::tbb_kahypar::static_partitioner()).get());
    oneapi::tbb_kahypar::affinity_partitioner aff;
    CHECK(result == oneapi::tbb_kahypar::parallel_reduce(range, identity, body, reduction, aff).get());

    CHECK(result == oneapi::tbb_kahypar::parallel_deterministic_reduce(range, identity, body, reduction).get());
    CHECK(result == oneapi::tbb_kahypar::parallel_deterministic_reduce(range, identity, body, reduction, oneapi::tbb_kahypar::simple_partitioner()).get());
    CHECK(result == oneapi::tbb_kahypar::parallel_deterministic_reduce(range, identity, body, reduction, oneapi::tbb_kahypar::static_partitioner()).get());
}
...
void test_vector_of_lists_rvalue_reduce_basic(const Runner& runner, PartitionerContext&&... args) {
    ...
    oneapi::tbb_kahypar::blocked_range<std::size_t> range(0, n_vectors, n_vectors * 2);
...
struct ReduceRunner {
    ...
    auto operator()(Args&&... args) const -> decltype(oneapi::tbb_kahypar::parallel_reduce(std::forward<Args>(args)...)) {
        return oneapi::tbb_kahypar::parallel_reduce(std::forward<Args>(args)...);
    }
};

struct DeterministicReduceRunner {
    template <typename... Args>
    auto operator()(Args&&... args) const -> decltype(oneapi::tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)...)) {
        return oneapi::tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)...);
    }
};

void test_vector_of_lists_rvalue_reduce() {
    ReduceRunner runner;
    oneapi::tbb_kahypar::affinity_partitioner af_partitioner;
    oneapi::tbb_kahypar::task_group_context context;

    test_vector_of_lists_rvalue_reduce_basic(runner);
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::auto_partitioner{});
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::simple_partitioner{});
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::static_partitioner{});
    test_vector_of_lists_rvalue_reduce_basic(runner, af_partitioner);

    test_vector_of_lists_rvalue_reduce_basic(runner, context);
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::auto_partitioner{}, context);
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::simple_partitioner{}, context);
    test_vector_of_lists_rvalue_reduce_basic(runner, oneapi::tbb_kahypar::static_partitioner{}, context);
