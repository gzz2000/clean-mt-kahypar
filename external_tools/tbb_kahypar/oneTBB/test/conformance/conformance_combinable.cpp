...
// INFO: #include "oneapi_kahypar/tbb_kahypar/tick_count.h"
...
public:
    typedef std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> > ContainerType;
...
class ParallelScalarBody: utils::NoAssign {
    ...
    oneapi::tbb_kahypar::combinable<T> &sums;

public:

    ParallelScalarBody ( oneapi::tbb_kahypar::combinable<T> &_sums ) : sums(_sums) { }
...
public:
    ...
    ParallelScalarBodyNoInit ( oneapi::tbb_kahypar::combinable<T> &_sums ) : sums(_sums) { }
...
void RunParallelScalarTests(const char* /* test_name */) {
    for (int p = MinThread; p <= MaxThread; ++p) {
        ...
        // INFO: oneapi::tbb_kahypar::tick_count t0;
        T combine_sum(0);
        T combine_ref_sum(0);
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            // INFO: if (Verbose && t == 0) t0 = oneapi::tbb_kahypar::tick_count::now();

            // test uninitialized parallel combinable
            oneapi::tbb_kahypar::combinable<T> sums;
            oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>( 0, N, 10000 ), ParallelScalarBody<T>( sums ) );
            combine_sum += sums.combine(my_combine<T>);
            combine_ref_sum += sums.combine(my_combine_ref<T>);

            // test combinable::clear()
            oneapi::tbb_kahypar::combinable<T> sums_to_clear;
            oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>(0, N, 10000), ParallelScalarBody<T>(sums_to_clear) );
            sums_to_clear.clear();
            ...
            oneapi::tbb_kahypar::combinable<T> finit_combinable(my_finit_decl);
            oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int>( 0, N, 10000 ), ParallelScalarBodyNoInit<T>( finit_combinable ) );
        ...
        // REMARK("  done parallel %s, %d, %g, %g\n", test_name, p, static_cast<double>(combine_sum), ( oneapi::tbb_kahypar::tick_count::now() - t0).seconds());
    }
}
...
class ParallelVectorForBody: utils::NoAssign {
    ...
    oneapi::tbb_kahypar::combinable< std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> > > &locals;

public:

    ParallelVectorForBody ( oneapi::tbb_kahypar::combinable< std::vector<T, oneapi::tbb_kahypar::tbb_allocator<T> > > &_locals ) : locals(_locals) { }
...
void RunParallelVectorTests(const char* /* test_name */) {
    ...
    for (int p = MinThread; p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            // if (Verbose && t == 0) t0 = oneapi::tbb_kahypar::tick_count::now();

            typedef typename oneapi::tbb_kahypar::combinable< ContainerType > CombinableType;

            // test uninitialized parallel combinable
            CombinableType vs;
            oneapi::tbb_kahypar::parallel_for( oneapi::tbb_kahypar::blocked_range<int> (0, N, 10000), ParallelVectorForBody<T>( vs ) );
        ...
        // REMARK("  done parallel %s, %d, %g, %g\n", test_name, p, ResultValue, ( oneapi::tbb_kahypar::tick_count::now() - t0).seconds());
    }
}
...
RunParallelTests() {
    ...
    RunParallelVectorTests<int>("std::vector<int, oneapi::tbb_kahypar::tbb_allocator<int> >");
    RunParallelVectorTests<double>("std::vector<double, oneapi::tbb_kahypar::tbb_allocator<double> >");
}
...
RunAssignmentAndCopyConstructorTest(const char* /* test_name */) {
    ...
    oneapi::tbb_kahypar::combinable<T> create1(my_finit7_decl);
    ...
    oneapi::tbb_kahypar::combinable<T> copy1(create1);
    ...
    FunctorAddFinit<T> my_finit_decl;
    oneapi::tbb_kahypar::combinable<T> assign1(my_finit_decl);
    assign1 = create1;
    ...
    oneapi::tbb_kahypar::combinable<T> move1(std::move(create1));
    ...
    oneapi::tbb_kahypar::combinable<T> move2;
...
void RunMoveSemanticsForStateTrackableObjectTest() {
    ...
    oneapi::tbb_kahypar::combinable< StateTrackable<true> > create1;
    ...
    oneapi::tbb_kahypar::combinable< StateTrackable<true> > copy1(create1);
    ...
    oneapi::tbb_kahypar::combinable< StateTrackable<true> > copy2;
    ...
    oneapi::tbb_kahypar::combinable< StateTrackable<true> > move1(std::move(create1));
...
struct Body : utils::NoAssign {
    oneapi::tbb_kahypar::combinable<int>* locals;
    const int nthread;
    const int nIters;
...
#define NITERATIONS 1000
    ...
    oneapi::tbb_kahypar::combinable<int> myCombinable;
