...
void TestSequentialFor() {
    using V = oneapi::tbb_kahypar::concurrent_vector<move_support_tests::FooWithAssign>;
    V v(N);
    ...
    {
        oneapi::tbb_kahypar::concurrent_vector<int> v1, v2(1ul, 100);
        v1.assign(1, 100);
        REQUIRE(v1 == v2);
...
void TestResizeAndCopy() {
    ...
    using vector_t = oneapi::tbb_kahypar::concurrent_vector<move_support_tests::Foo, allocator_t>;
...
void TestCopyAssignment() {
    ...
    using vector_t = oneapi::tbb_kahypar::concurrent_vector<move_support_tests::FooWithAssign, allocator_t>;
...
void TestCapacity() {
    using allocator_t = StaticCountingAllocator<std::allocator<move_support_tests::Foo> /*TODO: oneapi::tbb_kahypar::cache_aligned_allocator*/>;
    using vector_t = oneapi::tbb_kahypar::concurrent_vector<move_support_tests::Foo, allocator_t>;
    allocator_t::init_counters();
    for( std::size_t old_size=0; old_size<=11000; old_size=(old_size<5 ? old_size+1 : 3*old_size) ) {
        for( std::size_t new_size=0; new_size<=11000; new_size=(new_size<5 ? new_size+1 : 3*new_size) ) {
            std::size_t count = move_support_tests::foo_count;
            {
                ...
                if(oneapi::tbb_kahypar::detail::log2(/*reserved size*/old_size|1) > oneapi::tbb_kahypar::detail::log2(fill_size|1) ){
...
void verify_vector_partially_copied(
        oneapi::tbb_kahypar::concurrent_vector<move_support_tests::FooWithAssign, allocator_t> const& victim, size_t planned_victim_size,
        oneapi::tbb_kahypar::concurrent_vector<move_support_tests::FooWithAssign, allocator_t> const& src,  bool is_memory_allocation_failure)
{
...
void verify_assignment_operator_throws_bad_last_alloc(vector_t & victim){
    vector_t copy_of_victim(victim, victim.get_allocator());
    //CHECK_THROWS_AS(victim = copy_of_victim, oneapi::tbb_kahypar::bad_last_alloc); //TODO exceptions support
}
...
void TestInitList() {
    ...
    test_initializer_list_support<oneapi::tbb_kahypar::concurrent_vector<char>, test_grow_by>( { 1, 2, 3, 4, 5 } );
    test_initializer_list_support<oneapi::tbb_kahypar::concurrent_vector<int>, test_grow_by>( {} );
}
...
void TestSerialMoveInShrinkToFit(){
    ...
    using c_vector_t = oneapi::tbb_kahypar::concurrent_vector<dummy>;
...
struct c_vector_type : default_container_traits {
    ...
    using container_type = oneapi::tbb_kahypar::concurrent_vector<T, Allocator>;
    ...
    struct apply{
        using type = oneapi::tbb_kahypar::concurrent_vector<element_type,  allocator_type >;
    };
    ...
    static bool equal(oneapi::tbb_kahypar::concurrent_vector<element_type, allocator_type > const& c, iterator begin, iterator end){
...
namespace test_grow_to_at_least_helpers {
    ...
    public:
        void operator()( const oneapi::tbb_kahypar::blocked_range<std::size_t>& range ) const {
...
void TestConcurrentGrowToAtLeastImpl() {
    ...
    using MyVector = oneapi::tbb_kahypar::concurrent_vector<move_support_tests::Foo, MyAllocator>;
    ...
    for (std::size_t s=1; s<1000; s*=10) {
        oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(0, 10000*s, s), GrowToAtLeast<MyVector>(use_two_arg_form, v, copy_from), oneapi::tbb_kahypar::simple_partitioner());
    }
...
struct AssignElement {
    using iterator = oneapi::tbb_kahypar::concurrent_vector<int>::range_type::iterator;
    iterator base;
    void operator()( const oneapi::tbb_kahypar::concurrent_vector<int>::range_type& range ) const {
...
struct CheckElement {
    using iterator = oneapi::tbb_kahypar::concurrent_vector<int>::const_range_type::iterator;
    iterator base;
    void operator()( const oneapi::tbb_kahypar::concurrent_vector<int>::const_range_type& range ) const {
        for (iterator i = range.begin(); i != range.end(); ++i) {
            if (*i != int(i-base)) {
                REPORT("ERROR for v[%ld]\n", long(i-base));
            }
...
void TestParallelFor( std::size_t nthread ) {
    using vector_type = oneapi::tbb_kahypar::concurrent_vector<int>;
    vector_type v;
    v.resize(N);
    oneapi::tbb_kahypar::tick_count t0 = oneapi::tbb_kahypar::tick_count::now();
    INFO("Calling parallel_for with " << nthread << " threads");
    oneapi::tbb_kahypar::parallel_for(v.range(10000), AssignElement(v.begin()));
    oneapi::tbb_kahypar::tick_count t1 = oneapi::tbb_kahypar::tick_count::now();
    const vector_type& u = v;
    oneapi::tbb_kahypar::parallel_for(u.range(10000), CheckElement(u.begin()));
    oneapi::tbb_kahypar::tick_count t2 = oneapi::tbb_kahypar::tick_count::now();
...
void TestConcurrentGrowBy() {
    ...
    MyAllocator::init_counters();
    {
        ...
        MyAllocator a;
        MyVector v(a);
        oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(0, range_size, grain_size), GrowBy<MyVector>(v, m, part_weight), oneapi::tbb_kahypar::simple_partitioner());
...
void TestComparison() {
    ...
    oneapi::tbb_kahypar::concurrent_vector<char> var[3];
...
class test_grow_by_and_resize {
    oneapi::tbb_kahypar::concurrent_vector<Type, Allocator> &my_c;
public:
    test_grow_by_and_resize( oneapi::tbb_kahypar::concurrent_vector<Type, Allocator> &c ) : my_c(c) {}
    void operator()() const {
        const typename oneapi::tbb_kahypar::concurrent_vector<Type, Allocator>::size_type sz = my_c.size();
        my_c.grow_by( 5 );
...
namespace push_back_exception_safety_helpers {
    ...
    struct fixture {
        using vector_t = oneapi::tbb_kahypar::concurrent_vector<foo_t, std::allocator<foo_t> >;
        vector_t v;
...
void TestPushBackMoveOnlyContainer(){
    ...
    using vector_t = oneapi::tbb_kahypar::concurrent_vector<move_only_type >;
    vector_t v;
...
void TestDeductionGuides() {
    ...
    TVector v6(v5, oneapi::tbb_kahypar::cache_aligned_allocator<ComplexType>());
    static_assert(std::is_same<decltype(v6), TVector<ComplexType, oneapi::tbb_kahypar::cache_aligned_allocator<ComplexType>>>::value);
    ...
    // check TVector(TVector&&, Allocator)
    TVector v8(std::move(v5), oneapi::tbb_kahypar::cache_aligned_allocator<ComplexType>());
    static_assert(std::is_same<decltype(v8), TVector<ComplexType, oneapi::tbb_kahypar::cache_aligned_allocator<ComplexType>>>::value);
...
void test_member_types() {
    ...
    static_assert(std::is_same<typename default_container_type::allocator_type,
                               oneapi::tbb_kahypar::cache_aligned_allocator<int>>::value,
                  "Incorrect default template allocator");


    using test_allocator_type = oneapi::tbb_kahypar::cache_aligned_allocator<int>;
...
void TestVectorComparisons() {
    using integral_vector = oneapi::tbb_kahypar::concurrent_vector<int>;
    using two_way_comparable_vector = oneapi::tbb_kahypar::concurrent_vector<comparisons_testing::TwoWayComparable>;
...
#if __TBB_TEST_CPP20_COMPARISONS
    using two_way_less_only_vector = oneapi::tbb_kahypar::concurrent_vector<comparisons_testing::LessComparableOnly>;
    using three_way_only_vector = oneapi::tbb_kahypar::concurrent_vector<comparisons_testing::ThreeWayComparableOnly>;
    using three_way_comparable_vector = oneapi::tbb_kahypar::concurrent_vector<comparisons_testing::ThreeWayComparable>;
...
TEST_CASE("test type matching") {
    test_member_types<oneapi::tbb_kahypar::concurrent_vector>();
}
...
TEST_CASE("testing grow_to_at_least with source parameter"){
    TestGrowToAtLeastWithSourceParameter<oneapi::tbb_kahypar::concurrent_vector<int>>(12345);
}
...
TEST_CASE("testing std::iterator_traits for concurrent_vector::iterator"){
    TestIteratorTraits<oneapi::tbb_kahypar::concurrent_vector<move_support_tests::Foo>::iterator,move_support_tests::Foo>();
}
...
TEST_CASE("testing std::iterator_traits for concurrent_vector::const_iterator"){
    TestIteratorTraits<oneapi::tbb_kahypar::concurrent_vector<move_support_tests::Foo>::const_iterator,const move_support_tests::Foo>();
}
...
TEST_CASE("testing concurrency"){
    ...
    for (std::size_t p = 1; p <= 4; ++p) {
        oneapi::tbb_kahypar::global_control limit(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, p);
        TestParallelFor(p);
...
TEST_CASE("testing deduction guides"){
    TestDeductionGuides<oneapi::tbb_kahypar::concurrent_vector>();
}
#endif
