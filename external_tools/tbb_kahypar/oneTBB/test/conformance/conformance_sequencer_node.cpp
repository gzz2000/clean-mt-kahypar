...
TEST_CASE("sequencer_node single_push"){
    ...
    conformance::test_forwarding_single_push<oneapi::tbb_kahypar::flow::sequencer_node<int>>(sequencer);
}
...
TEST_CASE("sequencer_node buffering"){
    conformance::sequencer_functor<int> sequencer;
    conformance::test_buffering<oneapi::tbb_kahypar::flow::sequencer_node<int>, int>(sequencer);
}
...
TEST_CASE("sequencer_node copy constructor"){
    conformance::sequencer_functor<int> sequencer;
    conformance::test_copy_ctor_for_buffering_nodes<oneapi::tbb_kahypar::flow::sequencer_node<int>>(sequencer);
}

//! Test inheritance relations
//! \brief \ref interface
TEST_CASE("sequencer_node superclasses"){
    conformance::test_inheritance<oneapi::tbb_kahypar::flow::sequencer_node<int>, int, int>();
    conformance::test_inheritance<oneapi::tbb_kahypar::flow::sequencer_node<void*>, void*, void*>();
}
...
TEST_CASE("sequencer_node rejects duplicate"){
    ...
    oneapi::tbb_kahypar::flow::sequencer_node<int> node(g, sequencer);
...
TEST_CASE("sequencer_node ordering"){
    ...
    function_node<message> writer(g, tbb_kahypar::flow::serial, [&] (const message& msg) {
        CHECK_MESSAGE((msg.id == counter++), "The data is passed to the successor node in the exact same order it was read");
    });

    tbb_kahypar::flow::make_edge(process, ordering);
    tbb_kahypar::flow::make_edge(ordering, writer);
