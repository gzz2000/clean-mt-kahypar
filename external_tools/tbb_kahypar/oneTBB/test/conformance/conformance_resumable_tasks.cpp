...
TEST_CASE("Async test") {
    ...
    std::thread t;
    oneapi::tbb_kahypar::task::suspend([&t, &suspend, &resume](oneapi::tbb_kahypar::task::suspend_point sp) {
        suspend = true;
        t = std::thread([sp, &resume] {
            resume = true;
            oneapi::tbb_kahypar::task::resume(sp);
        });
    });
    CHECK(suspend);
    CHECK(resume);
...
TEST_CASE("Parallel test") {
    ...
    for (int i = 0; i < N; ++i) {
        tg.run([&tg, &suspend, &resume] {
            oneapi::tbb_kahypar::task::suspend([&tg, &suspend, &resume](oneapi::tbb_kahypar::task::suspend_point sp) {
                ++suspend;
                tg.run([sp, &resume] {
                    ++resume;
                    oneapi::tbb_kahypar::task::resume(sp);
