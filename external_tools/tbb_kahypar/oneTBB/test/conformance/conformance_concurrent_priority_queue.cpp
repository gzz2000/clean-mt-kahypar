...
void test_to_vector() {
    ...
    oneapi::tbb_kahypar::blocked_range<int*> range = utils::make_blocked_range(array);
    std::vector<int> source(range.begin(), range.end());

    oneapi::tbb_kahypar::concurrent_priority_queue<int> q(source.begin(), source.end());
...
void test_initializer_list() {
    ...
    test_initializer_list_support<oneapi::tbb_kahypar::concurrent_priority_queue<char>>({1, 2, 3, 4, 5});
    test_initializer_list_support<oneapi::tbb_kahypar::concurrent_priority_queue<int>>({});
}
...
struct CPQSrcFixture {
    ...
    using cpq_type = oneapi::tbb_kahypar::concurrent_priority_queue<MoveOperationTracker, cpq_compare_type, cpq_allocator_type>;
...
void test_move_support_in_push_pop() {
    ...
    oneapi::tbb_kahypar::concurrent_priority_queue<MoveOperationTracker> q1;
    ...
    oneapi::tbb_kahypar::concurrent_priority_queue<NoDefaultCtorType> q2;
    q2.emplace(15, 3);
    q2.emplace(2, 35);
    q2.emplace(8, 8);
    ...
    oneapi::tbb_kahypar::concurrent_priority_queue<ForwardInEmplaceTester> q3;
...
void test_ctors_dtor_accessors() {
    ...
    using cpq_type = oneapi::tbb_kahypar::concurrent_priority_queue<int, std::less<int>>;
    using cpq_with_compare_type = oneapi::tbb_kahypar::concurrent_priority_queue<int, LessA<int>>;
    using cpq_with_compare_and_allocator_type = oneapi::tbb_kahypar::concurrent_priority_queue<int, LessA<int>, std::allocator<int>>;
...
void test_serial_push_pop() {
    oneapi::tbb_kahypar::concurrent_priority_queue<int, std::less<int>> q(MAX_ITER);
    int e = 42;
    int prev = INT_MAX;
    std::size_t count = 0;
...
struct CPQTraits {
    ...
    using container_type = oneapi::tbb_kahypar::concurrent_priority_queue<T, std::less<T>, Allocator>;
}; // struct CPQTraits
...
void TestCPQComparisons() {
    using integral_container = oneapi::tbb_kahypar::concurrent_priority_queue<int>;
    using two_way_comparable_container = oneapi::tbb_kahypar::concurrent_priority_queue<comparisons_testing::TwoWayComparable>;
...
TEST_CASE("CTAD support in concurrent_priority_queue") {
    TestDeductionGuides<oneapi::tbb_kahypar::concurrent_priority_queue>();
}
#endif // __TBB_CPP17_DEDUCTION_GUIDES_PRESENT
