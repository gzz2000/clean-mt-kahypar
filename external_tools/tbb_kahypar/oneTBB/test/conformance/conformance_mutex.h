...
template <>
void TestTryAcquire<oneapi::tbb_kahypar::null_mutex>( const char* mutex_name ) {
    oneapi::tbb_kahypar::null_mutex tested_mutex;
    typename oneapi::tbb_kahypar::null_mutex::scoped_lock lock(tested_mutex);
...
template <>
void TestTryAcquireReader<oneapi::tbb_kahypar::null_rw_mutex>( const char* mutex_name ) {
    oneapi::tbb_kahypar::null_rw_mutex tested_mutex;
    typename oneapi::tbb_kahypar::null_rw_mutex::scoped_lock lock(tested_mutex, false);
...
struct NullRecursive: utils::NoAssign {
    ...
    void operator()(oneapi::tbb_kahypar::blocked_range<std::size_t>& range) const {
...
void TestNullMutex(const char* mutex_name) {
    ...
    oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(0, n, 10), NullRecursive<utils::AtomicCounter<M>>(counter));
    M m;
    m.lock();
    REQUIRE(m.try_lock());
    m.unlock();
...
void TestNullRWMutex(const char* mutex_name) {
    const std::size_t n = 100;
    M m;
    oneapi::tbb_kahypar::parallel_for(oneapi::tbb_kahypar::blocked_range<std::size_t>(0, n, 10), NullUpgradeDowngrade<M>(m, mutex_name));
    m.lock();
    REQUIRE(m.try_lock());
    m.lock_shared();
