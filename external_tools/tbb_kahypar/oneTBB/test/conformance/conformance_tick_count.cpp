...
#include "oneapi_kahypar/tbb_kahypar/detail/_template_helpers.h"
#include <type_traits>
...
TEST_CASE("Default construction") {
    oneapi::tbb_kahypar::tick_count t1;
...
TEST_CASE("Subtraction of equal tick_counts") {
    oneapi::tbb_kahypar::tick_count tick_f = oneapi::tbb_kahypar::tick_count::now();
    oneapi::tbb_kahypar::tick_count tick_s(tick_f);
...
TEST_CASE("Subtraction subsequent timestamp") {
    oneapi::tbb_kahypar::tick_count tick_f(oneapi::tbb_kahypar::tick_count::now());
    oneapi::tbb_kahypar::tick_count tick_s(oneapi::tbb_kahypar::tick_count::now());
    while ((tick_s - tick_f).seconds() == 0) {
        tick_s = oneapi::tbb_kahypar::tick_count::now();
    }
...
static void WaitForDuration( double duration ) {
    ...
    double sec = 0;
    do {
        sec = (oneapi::tbb_kahypar::tick_count::now() - start).seconds();
        CHECK_GE(sec, 0);
    } while (sec < duration);
}
...
TEST_CASE("Arithmetic operators") {
    ...
    oneapi::tbb_kahypar::tick_count t3 = oneapi::tbb_kahypar::tick_count::now();

    using interval_type = oneapi::tbb_kahypar::tick_count::interval_t;
    interval_type i = t2 - t1;
    interval_type j = t3 - t2;
    interval_type k = t3 - t1;
    CHECK_EQ(std::is_same<oneapi::tbb_kahypar::tick_count::interval_t, decltype(i - j)>::value, true);
    CHECK_EQ(std::is_same<oneapi::tbb_kahypar::tick_count::interval_t, decltype(i + j)>::value, true);
...
//! Test resolution of oneapi::tbb_kahypar::tick_count::interval_t
//! \brief \ref interface \ref requirement
TEST_CASE("oneapi::tbb_kahypar::tick_count::interval_t resolution") {
    ...
    for (int i = -range_value; i <= range_value; ++i) {
        ...
        oneapi::tbb_kahypar::tick_count::interval_t t0(my_time);
        double interval_time = t0.seconds();
        //! time always truncates
        CHECK_GE(interval_time, 0);
        CHECK_LT(my_time - interval_time, oneapi::tbb_kahypar::tick_count::resolution());
    }
}
