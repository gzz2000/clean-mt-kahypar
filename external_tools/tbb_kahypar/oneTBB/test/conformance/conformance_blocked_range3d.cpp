...
static void SerialTest() {
    ...
    typedef oneapi::tbb_kahypar::blocked_range3d<page_type,row_type,col_type> range_type;
                            ...
                            for( int col_x=-4; col_x<4; ++col_x ) {
                                for( int col_y=col_x; col_y<4; ++col_y ) {
                                    ...
                                    for( int col_grain=1; col_grain<4; ++col_grain ) {
                                        ...
                                        utils::AssertSameType( r.pages(), oneapi::tbb_kahypar::blocked_range<page_type>( page_i, page_j, 1 ));
                                        utils::AssertSameType( r.rows(), oneapi::tbb_kahypar::blocked_range<row_type>( row_i, row_j, 1 ));
                                        utils::AssertSameType( r.cols(), oneapi::tbb_kahypar::blocked_range<col_type>( col_i, col_j, 1 ));
                                        ...
                                        if( r.is_divisible() ) {
                                            range_type r2(r,oneapi::tbb_kahypar::split());
...
struct Striker {
   // Note: we use <int> here instead of <long> in order to test for problems similar to Quad 407676
    void operator()( const oneapi::tbb_kahypar::blocked_range3d<int>& r ) const {
        for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator i=r.pages().begin(); i!=r.pages().end(); ++i )
            for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator j=r.rows().begin(); j!=r.rows().end(); ++j )
                for( oneapi::tbb_kahypar::blocked_range<int>::const_iterator k=r.cols().begin(); k!=r.cols().end(); ++k )
                    ++Array[i][j][k];
    }
};
    ...
    for( int i=0; i<N; i=i<3 ? i+1 : i*3 ) {
        for( int j=0; j<N; j=j<3 ? j+1 : j*3 ) {
            for( int k=0; k<N; k=k<3 ? k+1 : k*3 ) {
                const oneapi::tbb_kahypar::blocked_range3d<int> r( 0, i, 5, 0, j, 3, 0, k, 1 );
                oneapi::tbb_kahypar::parallel_for( r, Striker() );
                for( int l=0; l<N; ++l ) {
                    for( int m=0; m<N; ++m ) {
                        for( int n=0; n<N; ++n ) {
...
TEST_CASE("blocked_range3d proportional splitting") {
    oneapi::tbb_kahypar::blocked_range3d<int> original(0, 100, 0, 100, 0, 100);
    oneapi::tbb_kahypar::blocked_range3d<int> first(original);
    oneapi::tbb_kahypar::proportional_split ps(3, 1);
    oneapi::tbb_kahypar::blocked_range3d<int> second(first, ps);
...
TEST_CASE("Deduction guides") {
    ...
    oneapi::tbb_kahypar::blocked_range3d r1(v.begin(), v.end(), 2, v2.begin(), v2.end(), 2, v3.begin(), v3.end(), 6);
    static_assert(std::is_same<decltype(r1),
        oneapi::tbb_kahypar::blocked_range3d<decltype(v)::iterator, decltype(v2)::iterator, decltype(v3)::iterator>>::value);

    // check blocked_range2d(blocked_range3d &)
    oneapi::tbb_kahypar::blocked_range3d r2(r1);
    static_assert(std::is_same<decltype(r2), decltype(r1)>::value);

    // check blocked_range2d(blocked_range3d &&)
    oneapi::tbb_kahypar::blocked_range3d r3(std::move(r1));
