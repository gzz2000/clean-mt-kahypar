...
TEST_CASE("Allocator concept") {
    // allocate/deallocate
    TestAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<void>>(Concept);
    TestAllocator<oneapi::tbb_kahypar::tbb_allocator<void>>(Concept);

    // max_size case for cache_aligned allocator
    using Allocator = oneapi::tbb_kahypar::cache_aligned_allocator<int>;
    Allocator allocator;
    ...
    // operator==
    TestAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<void>>(Comparison);
    TestAllocator<oneapi::tbb_kahypar::tbb_allocator<void>>(Comparison);
}
...
TEST_CASE("Exceptions") {
    TestAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<void>>(Exceptions);
    TestAllocator<oneapi::tbb_kahypar::tbb_allocator<void>>(Exceptions);
}
...
TEST_CASE("Thread safety") {
    TestAllocator<oneapi::tbb_kahypar::cache_aligned_allocator<void>>(ThreadSafety);
    TestAllocator<oneapi::tbb_kahypar::tbb_allocator<void>>(ThreadSafety);
    oneapi::tbb_kahypar::tbb_allocator<int> tbb_alloc;
...
TEST_CASE("tbb_allocator allocator type") {
    using Allocator = oneapi::tbb_kahypar::tbb_allocator<int>; Allocator tbb_alloc;
#if _MSC_VER && _MSC_VER <= 1900 && !__INTEL_COMPILER
    utils::suppress_unused_warning(tbb_alloc);
#endif
...
TEST_CASE("polymorphic_allocator test") {
    oneapi::tbb_kahypar::cache_aligned_resource aligned_resource;
