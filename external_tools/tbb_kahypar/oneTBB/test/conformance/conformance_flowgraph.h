...
template<typename V>
using test_push_receiver = oneapi::tbb_kahypar::flow::queue_node<V>;

template<typename Input, typename Output = Input>
using multifunc_ports_t =
      typename oneapi::tbb_kahypar::flow::multifunction_node<Input, std::tuple<Output>>::output_ports_type;

template<typename Input, typename Output = Input>
using async_ports_t =
      typename oneapi::tbb_kahypar::flow::async_node<Input, Output>::gateway_type;
...
struct counting_functor {
    ...
    OutputType operator()( oneapi::tbb_kahypar::flow_control& fc ) {
        ++execute_count;
...
struct passthru_body {
    OutputType operator()( const oneapi::tbb_kahypar::flow::continue_msg& ) {
        return OutputType(0);
    }
...
void test_forwarding_single_push(Args... node_args) {
    ...
    oneapi::tbb_kahypar::flow::make_edge(testing_node, suc_node1);
    oneapi::tbb_kahypar::flow::make_edge(testing_node, suc_node2);
...
void test_copy_ctor() {
    ...
    oneapi::tbb_kahypar::flow::make_edge(node1, suc_node1);
    ...
    oneapi::tbb_kahypar::flow::make_edge(node_copy, suc_node2);
...
void test_copy_ctor_for_buffering_nodes(Args... node_args) {
    ...
    Node testing_node(g, node_args...);
    oneapi::tbb_kahypar::flow::continue_node<int> pred_node(g, fun);
    test_push_receiver<int> suc_node1(g);
    test_push_receiver<int> suc_node2(g);

    oneapi::tbb_kahypar::flow::make_edge(pred_node, testing_node);
    oneapi::tbb_kahypar::flow::make_edge(testing_node, suc_node1);
    ...
    pred_node.try_put(oneapi::tbb_kahypar::flow::continue_msg());
    g.wait_for_all();
...
void test_priority(Args... node_args) {
    ...
    oneapi::tbb_kahypar::flow::graph g;
    oneapi::tbb_kahypar::flow::continue_node<InputType> source(g, dummy_functor<InputType>());
    ...
    Node high(g, node_args..., high_functor, oneapi::tbb_kahypar::flow::node_priority_t(1));
    Node low(g, node_args..., low_functor);

    tbb_kahypar::task_arena a(1, 1);
    a.execute([&] {
        g.reset(); // attach to this arena

        make_edge(source, low);
        make_edge(source, high);
        source.try_put(oneapi::tbb_kahypar::flow::continue_msg());
...
void test_concurrency() {
    auto max_num_threads = oneapi::tbb_kahypar::this_task_arena::max_concurrency();

    oneapi::tbb_kahypar::global_control c(oneapi::tbb_kahypar::global_control::max_allowed_parallelism,
                                  max_num_threads);

    std::vector<int> threads_count = {1, oneapi::tbb_kahypar::flow::serial, max_num_threads, oneapi::tbb_kahypar::flow::unlimited};
    ...
    for(auto num_threads : threads_count) {
        ...
        if(num_threads == oneapi::tbb_kahypar::flow::unlimited) {
            expected_threads = max_num_threads;
        }
        if(num_threads == oneapi::tbb_kahypar::flow::serial) {
            expected_threads = 1;
        }
        oneapi::tbb_kahypar::flow::graph g;
...
void test_rejecting() {
    ...
    wait_flag_body body;
    Node fnode(g, oneapi::tbb_kahypar::flow::serial, body);
