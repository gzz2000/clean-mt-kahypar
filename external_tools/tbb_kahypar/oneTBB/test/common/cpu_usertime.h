...
inline void TestCPUUserTime( std::size_t nthreads, std::size_t nactive = 1 ) {
    ...
    tbb_kahypar::tick_count stamp = tbb_kahypar::tick_count::now();
    ...
    while ( (usrtime_delta=GetCPUUserTime()-lastusrtime) < THRESHOLD ) {
        ...
        if ( (waittime_delta = (tbb_kahypar::tick_count::now()-stamp).seconds()) > maximal_waittime ) {
    ...
    stamp = tbb_kahypar::tick_count::now();
    while ( ((waittime_delta=(tbb_kahypar::tick_count::now()-stamp).seconds()) < minimal_waittime)
