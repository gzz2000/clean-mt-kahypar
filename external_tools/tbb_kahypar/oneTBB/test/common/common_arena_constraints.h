...
class system_info {
    ...
    using memory_handler_t = tbb_kahypar::concurrent_unordered_set<hwloc_bitmap_t>;
...
system_info::affinity_mask prepare_reference_affinity_mask(const tbb_kahypar::task_arena::constraints& c) {
    ...
    if (c.numa_id != tbb_kahypar::task_arena::automatic) {
    ...
    if (c.core_type != tbb_kahypar::task_arena::automatic) {
...
void test_constraints_affinity_and_concurrency(tbb_kahypar::task_arena::constraints constraints,
                                               system_info::affinity_mask arena_affinity) {
    int default_concurrency = tbb_kahypar::info::default_concurrency(constraints);
    ...
    if (constraints.max_threads_per_core == tbb_kahypar::task_arena::automatic || constraints.max_threads_per_core == max_threads_per_core) {
...
system_info::affinity_mask get_arena_affinity(tbb_kahypar::task_arena& ta) {
    system_info::affinity_mask arena_affinity;
    ta.execute([&]{
    ...
    tbb_kahypar::spin_mutex affinity_mutex{};
    for (int i = 0; i < ta.max_concurrency() - 1; ++i) {
        ta.enqueue([&] {
            {
                tbb_kahypar::spin_mutex::scoped_lock lock(affinity_mutex);
...
namespace system_info {
    ...
    std::vector<int> get_available_max_threads_values() { return {tbb_kahypar::task_arena::automatic, 1, 2}; }
}

#endif /*!__TBB_HWLOC_VALID_ENVIRONMENT*/

struct constraints_hash {
    std::size_t operator()(const tbb_kahypar::task_arena::constraints& c) const {
      return (std::hash<int>{}(c.numa_id) ^ std::hash<int>{}(c.core_type) ^ std::hash<int>{}(c.max_threads_per_core));
    }
};

struct constraints_equal {
    bool operator()(const tbb_kahypar::task_arena::constraints& c1, const tbb_kahypar::task_arena::constraints& c2) const {
...
using constraints_container = std::unordered_set<tbb_kahypar::task_arena::constraints, constraints_hash, constraints_equal>;
...
constraints_container generate_constraints_variety() {
    static constraints_container constraints_variety = [](){
        using constraints = tbb_kahypar::task_arena::constraints;
        constraints_container results{};

        std::vector<tbb_kahypar::numa_node_id> numa_nodes = tbb_kahypar::info::numa_nodes();
        numa_nodes.push_back((tbb_kahypar::numa_node_id)tbb_kahypar::task_arena::automatic);

#if __HYBRID_CPUS_TESTING
        std::vector<tbb_kahypar::core_type_id> core_types = tbb_kahypar::info::core_types();
        core_types.push_back((tbb_kahypar::core_type_id)tbb_kahypar::task_arena::automatic);
#endif
        ...
        if (get_processors_group_count() > 1) {
            for(auto it = results.begin(); it != results.end();) {
                if (it->max_threads_per_core != max_threads_per_core
                   && (it->numa_id == tbb_kahypar::task_arena::automatic || tbb_kahypar::info::numa_nodes().size() == 1)
#if __HYBRID_CPUS_TESTING
                   && (it->core_type == tbb_kahypar::task_arena::automatic || tbb_kahypar::info::core_types().size() == 1)
#endif /*__HYBRID_CPUS_TESTING*/
                ) {
                    it = results.erase(it);
                } else {
