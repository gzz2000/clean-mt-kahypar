...
std::pair<intptr_t, intptr_t> CheckRecursiveRange( Range range ) {
    ...
    if (range.is_divisible()) {
        Range range2(range, tbb_kahypar::split{});
...
void test_concurrent_common( bool asymptotic = false ) {
    ...
    if (!asymptotic) {
        ...
        tbb_kahypar::parallel_for(r, ParallelTraverseBody<Container, typename Container::range_type>(array, items));
        ...
        tbb_kahypar::parallel_for(cr, ParallelTraverseBody<Container, typename Container::const_range_type>(array, items));
        CheckRange(array, items, AllowMultimapping<Container>::value, (num_threads - 1) / 2);
        delete[] array;

        tbb_kahypar::parallel_for(0, items, CheckTable<Container>(table));
    }
...
void test_set_specific_types() {
    ...
    std::list<std::basic_string<char, std::char_traits<char>, tbb_kahypar::tbb_allocator<char>>> arr_strings;
    for (int i = 0; i != num; ++i) {
...
void CommonExamine( Table c, const std::list<typename Table::value_type>& lst ) {
    ...
    tbb_kahypar::parallel_for(c.range(), TestRange<value_type>(lst, marks));
    REQUIRE(std::find(marks.begin(), marks.end(), false) == marks.end());

    tbb_kahypar::parallel_for(constC.range(), TestRange<value_type>(lst, marks));
    REQUIRE(std::find(marks.begin(), marks.end(), false) == marks.end());
