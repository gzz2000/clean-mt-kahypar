...
struct invoke_tree {
    ...
    struct invoke_tree_node {
        ...
        void invoker(const std::tuple<Fs...>&) const {
            tbb_kahypar::parallel_invoke(Fs()...);
        }
...
class parallel_invoke_call {
    template<typename... Fs>
    static void invoke_tuple_args(tbb_kahypar::task_group_context* context_ptr, const std::tuple<Fs...>&) {
        if (context_ptr != nullptr) {
            tbb_kahypar::parallel_invoke(Fs()..., *context_ptr);
        } else {
            tbb_kahypar::parallel_invoke(Fs()...);
        }
    }
...
public:
    static void perform(tbb_kahypar::task_group_context* context_ptr = nullptr) {
