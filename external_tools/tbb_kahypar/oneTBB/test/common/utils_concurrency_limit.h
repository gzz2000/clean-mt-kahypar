...
inline thread_num_type get_platform_max_threads() {
    static thread_num_type platform_max_threads = tbb_kahypar::this_task_arena::max_concurrency();
...
public:
    thread_pinner() {
        tbb_kahypar::detail::suppress_unused_warning(thread_index);
#if OS_AFFINITY_SYSCALL_PRESENT
...
class pinning_observer : public tbb_kahypar::task_scheduler_observer {
    ...
    tbb_kahypar::enumerable_thread_specific<bool> register_threads;
public:
    pinning_observer(tbb_kahypar::task_arena& arena) : tbb_kahypar::task_scheduler_observer(arena), pinner() {
        observe(true);
    }
