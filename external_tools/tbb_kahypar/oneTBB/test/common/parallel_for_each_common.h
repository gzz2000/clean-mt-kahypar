...
struct FakeTaskGeneratorBody_VolatileRefVersion {
    void operator()(volatile value_t& depth, tbb_kahypar::feeder<value_t>&) const {
        g_tasks_observed += FindNumOfTasks(depth.value());
    }
};
...
void do_work(const value_t& depth, tbb_kahypar::feeder<value_t>& feeder) {
    ++g_tasks_observed;
...
struct TaskGeneratorBody {
    ...
    void operator()(value_t depth, tbb_kahypar::feeder<value_t>& feeder) const {
...
struct TaskGeneratorBody_ConstVersion {
    void operator()(const value_t depth, tbb_kahypar::feeder<value_t>& feeder) const {
        do_work(depth, feeder);
    }
};
...
struct TaskGeneratorBody_VolatileRefVersion {
    void operator()(volatile value_t& depth, tbb_kahypar::feeder<value_t>& feeder) const {
...
struct TaskGeneratorBody_ConstVolatileRefVersion {
    void operator()(const volatile value_t& depth, tbb_kahypar::feeder<value_t>& feeder) const {
        do_work(const_cast<value_t&>(depth), feeder);
    }
};
...
void TestBodyMove(size_t depth) {
    ...
    g_tasks_observed = 0;
    tbb_kahypar::parallel_for_each(mbegin, mend, body);
...
void TestBody(size_t depth) {
    ...
    g_tasks_observed = 0;
    tbb_kahypar::parallel_for_each(begin, end, body);
    REQUIRE (g_tasks_observed == g_tasks_expected);
}
...
void container_based_overload_test_case(std::size_t expected_value) {
    ...
    tbb_kahypar::parallel_for_each(container, Functor{});
...
template<typename T>
void RecordAndAdd(const T& in, tbb_kahypar::feeder<T>& feeder) {
...
struct TestMoveIteratorBody {
    void operator() (T&& in, tbb_kahypar::feeder<T>& feeder) const { RecordAndAdd(in, feeder); }
};
...
void TestMoveIterator() {
    ...
    g_tasks_observed = 0;
    value_type a[size];
    tbb_kahypar::parallel_for_each(std::make_move_iterator(Iterator(a)), std::make_move_iterator(Iterator(a+size)), body);
