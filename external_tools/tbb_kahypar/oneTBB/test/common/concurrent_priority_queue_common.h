...
std::vector<ElementType> toVector( const tbb_kahypar::concurrent_priority_queue<ElementType, Compare, Allocator>& source ) {
    auto cpq = source;
...
void type_tester( const std::vector<ValueType>& vec, Compare comp ) {
    using queue_type = tbb_kahypar::concurrent_priority_queue<ValueType, Compare>;
...
void type_tester_unique_ptr( const std::vector<T>& vec ) {
    ...
    using queue_type = tbb_kahypar::concurrent_priority_queue<value_type, LessForSmartPointers>;
...
class FillBody {
    std::size_t n_thread;
    T my_min, my_max;
    tbb_kahypar::concurrent_priority_queue<T, C>* q;
public:
    FillBody( const FillBody& ) = delete;
    FillBody& operator=( const FillBody& ) = delete;

    FillBody( std::size_t n, T max, T min, tbb_kahypar::concurrent_priority_queue<T, C>* cpq )
...
public:
    ...
    EmptyBody( T max, tbb_kahypar::concurrent_priority_queue<T, C>* cpq )
        : my_max(max), q(cpq) {}
...
public:
    ...
    FloggerBody( tbb_kahypar::concurrent_priority_queue<T, C>* cpq )
        : q(cpq) {}
...
void test_parallel_push_pop( std::size_t n, T t_max, T t_min ) {
    ...
    tbb_kahypar::concurrent_priority_queue<T, C> q(0);
