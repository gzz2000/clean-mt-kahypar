...
void InitializeAndTerminate( int maxthread ) {
    for( int i=0; i<200; ++i ) {
        switch( i&3 ) {
            ...
            default: {
                tbb_kahypar::task_arena arena( std::rand() % maxthread + 1 );
            ...
            case 1: {
                tbb_kahypar::task_arena arena( tbb_kahypar::task_arena::automatic );
                CHECK(!arena.is_active());
                arena.initialize();
                CHECK(arena.is_active());
                break;
...
typedef tbb_kahypar::blocked_range<int> Range;

// slot_id value: -1 is reserved by current_slot(), -2 is set in on_scheduler_exit() below
static tbb_kahypar::enumerable_thread_specific<int> local_id, old_id, slot_id(-3);
...
class ArenaObserver : public tbb_kahypar::task_scheduler_observer {
    ...
    void on_scheduler_entry( bool is_worker ) override {
        int current_index = tbb_kahypar::this_task_arena::current_thread_index();
    ...
    void on_scheduler_exit( bool /*is_worker*/ ) override {
        ...
        CHECK(slot_id.local() == tbb_kahypar::this_task_arena::current_thread_index());
        slot_id.local() = -2;
        local_id.local() = old_id.local();
        old_id.local() = 0;
    }
public:
    ArenaObserver(tbb_kahypar::task_arena &a, int maxConcurrency, int numReservedSlots, int id)
        : tbb_kahypar::task_scheduler_observer(a)
        , myId(id)
...
struct AsynchronousWork {
    ...
    void operator()() const {
        ...
        tbb_kahypar::parallel_for(Range(0,500), IndexTrackingBody(), tbb_kahypar::simple_partitioner());
...
void TestConcurrentArenasFunc(int idx) {
    ...
    struct LocalObserver : public tbb_kahypar::task_scheduler_observer {
        LocalObserver() : tbb_kahypar::task_scheduler_observer() { observe(true); }
        LocalObserver(tbb_kahypar::task_arena& a) : tbb_kahypar::task_scheduler_observer(a) {
            observe(true);
        }
        ~LocalObserver() {
            observe(false);
...
class MultipleMastersPart1 : utils::NoAssign {
    tbb_kahypar::task_arena &my_a;
    utils::SpinBarrier &my_b1, &my_b2;
public:
    MultipleMastersPart1( tbb_kahypar::task_arena &a, utils::SpinBarrier &b1, utils::SpinBarrier &b2)
...
public:
    MultipleMastersPart2( tbb_kahypar::task_arena &a, utils::SpinBarrier &b) : my_a(a), my_b(b) {}
    void operator()(int) const {
...
class MultipleMastersPart3 : utils::NoAssign {
    tbb_kahypar::task_arena &my_a;
    utils::SpinBarrier &my_b;
    using wait_context = tbb_kahypar::detail::d1::wait_context;
    ...
    struct Waiter : NoAssign {
        ...
        void operator()() const {
            tbb_kahypar::task_group_context ctx;
            tbb_kahypar::detail::d1::wait(myWait, ctx);
        }
    };

public:
    MultipleMastersPart3(tbb_kahypar::task_arena &a, utils::SpinBarrier &b)
        : my_a(a), my_b(b) {}
    void operator()(int) const {
        wait_context wait(0);
...
#include <sstream>
#include <stdexcept>
#include "oneapi_kahypar/tbb_kahypar/detail/_exception.h"
...
struct TestArenaEntryBody : FPModeContext {
    ...
    void operator()() { // inside task_arena::execute()
        ...
        my_id << (stage < 3 ? (tbb_kahypar::this_task_arena::current_thread_index()?
...
class ForEachArenaEntryBody : utils::NoAssign {
    tbb_kahypar::task_arena &my_a; // expected task_arena(2,1)
    std::atomic<int> &my_stage; // each execute increases it
    int my_idx;

public:
    ForEachArenaEntryBody(tbb_kahypar::task_arena &a, std::atomic<int> &c)
    ...
    void operator()() const {
        ...
        tbb_kahypar::parallel_for(4, 5, *this); // in different context
    }
...
public:
    TestArenaConcurrencyBody( tbb_kahypar::task_arena &a, int max_concurrency, int reserved_slots, utils::SpinBarrier *b = nullptr, utils::SpinBarrier *wb = nullptr )
    ...
    // Arena's functor
    void operator()() const {
        int idx = tbb_kahypar::this_task_arena::current_thread_index();
        REQUIRE( idx < (my_max_concurrency > 1 ? my_max_concurrency : 2) );
        REQUIRE( my_a.max_concurrency() == tbb_kahypar::this_task_arena::max_concurrency() );
        int max_arena_concurrency = tbb_kahypar::this_task_arena::max_concurrency();
...
void TestArenaConcurrency( int p, int reserved = 0, int step = 1) {
    for (; reserved <= p; reserved += step) {
        tbb_kahypar::task_arena a( p, reserved );
        if (p - reserved < tbb_kahypar::this_task_arena::max_concurrency()) {
...
struct TestMandatoryConcurrencyObserver : public tbb_kahypar::task_scheduler_observer {
    utils::SpinBarrier& m_barrier;

    TestMandatoryConcurrencyObserver(tbb_kahypar::task_arena& a, utils::SpinBarrier& barrier)
        : tbb_kahypar::task_scheduler_observer(a), m_barrier(barrier) {
        observe(true);
    }
        ...
        for (int j = 0; j < 5; ++j) {
            ...
            utils::NativeParallelFor(n_threads, [&](int) {
                for (int i = 0; i < 5; ++i) {
                    barrier.wait();
                    a.enqueue([&] {
                        CHECK(tbb_kahypar::this_task_arena::max_concurrency() == 2);
...
struct TaskArenaValidator {
    ...
    const tbb_kahypar::task_arena& my_arena;
    TaskArenaValidator( const tbb_kahypar::task_arena& other )
        : my_slot_at_construction(tbb_kahypar::this_task_arena::current_thread_index())
        , my_arena(other)
    {}
    ...
    void operator()() {
        CHECK_MESSAGE( tbb_kahypar::this_task_arena::current_thread_index()==my_slot_at_construction,
                "Current thread index has changed since the validator construction" );
    }
};

void ValidateAttachedArena( tbb_kahypar::task_arena& arena, bool expect_activated,
    ...
    if( arena.is_active() ) {
        ...
        if ( tbb_kahypar::this_task_arena::current_thread_index() != tbb_kahypar::task_arena::not_initialized ) {
            CHECK(tbb_kahypar::this_task_arena::current_thread_index() >= 0);
            ...
            CHECK(tbb_kahypar::this_task_arena::current_thread_index() == -1);
        }
...
struct TestAttachBody : utils::NoAssign {
    ...
    void operator()( int idx ) const {
        ...
        int default_threads = tbb_kahypar::this_task_arena::max_concurrency();

        tbb_kahypar::task_arena arena{tbb_kahypar::task_arena::attach()};
        ...
        tbb_kahypar::parallel_for(0, 1, [](int) {});

        tbb_kahypar::task_arena arena2{tbb_kahypar::task_arena::attach()};
        ValidateAttachedArena( arena2, true, default_threads, 1 );

        tbb_kahypar::task_arena arena3;
        arena3.initialize(tbb_kahypar::attach());
    ...
    // The functor body for tbb_kahypar::parallel_for
    void operator()( const Range& r ) const {
        for( int i = r.begin(); i<r.end(); ++i ) {
            tbb_kahypar::task_arena arena2{tbb_kahypar::task_arena::attach()};
            ValidateAttachedArena( arena2, true, tbb_kahypar::this_task_arena::max_concurrency(), 1 );
        }
    }
};
...
void TestAttach( int maxthread ) {
    ...
    tbb_kahypar::parallel_for( Range(0,10000*maxthread), TestAttachBody( maxthread ) );
}
...
namespace TestIsolatedExecuteNS {
    ...
    public:
        ...
        void operator()() const {
            NestedPartitioner p;
            tbb_kahypar::parallel_for( 0, 10, utils::DummyBody( 10 ), p );
        }
    };
    ...
    class ParForBody : utils::NoAssign {
        ...
        tbb_kahypar::enumerable_thread_specific<int> &myEts;
        std::atomic<bool> &myIsStolen;
    public:
        ParForBody( bool outer_isolation, tbb_kahypar::enumerable_thread_specific<int> &ets, std::atomic<bool> &is_stolen )
        ...
        void operator()( int ) const {
            ...
            if ( myOuterIsolation )
                ...
                tbb_kahypar::this_task_arena::isolate( NestedParFor<NestedPartitioner>() );
            --e;
        }
    };
    ...
    public:
        ...
        void operator()() const {
            tbb_kahypar::enumerable_thread_specific<int> ets( 0 );
            OuterPartitioner p;
            tbb_kahypar::parallel_for( 0, 1000, ParForBody<NestedPartitioner>( myOuterIsolation, ets, myIsStolen ), p );
        }
    };
    ...
    void TwoLoopsTest( bool outer_isolation ) {
        ...
        if ( outer_isolation ) {
            for ( int i = 0; i <= max_repeats; ++i ) {
                tbb_kahypar::this_task_arena::isolate( OuterParFor<OuterPartitioner, NestedPartitioner>( outer_isolation, is_stolen ) );
                if ( is_stolen ) break;
            }
    ...
    void TwoLoopsTest( bool outer_isolation ) {
        TwoLoopsTest<tbb_kahypar::simple_partitioner, tbb_kahypar::simple_partitioner>( outer_isolation );
        TwoLoopsTest<tbb_kahypar::simple_partitioner, tbb_kahypar::affinity_partitioner>( outer_isolation );
        TwoLoopsTest<tbb_kahypar::affinity_partitioner, tbb_kahypar::simple_partitioner>( outer_isolation );
        TwoLoopsTest<tbb_kahypar::affinity_partitioner, tbb_kahypar::affinity_partitioner>( outer_isolation );
    }
    ...
    class HeavyMixTestBody : utils::NoAssign {
        tbb_kahypar::enumerable_thread_specific<utils::FastRandom<>>& myRandom;
        tbb_kahypar::enumerable_thread_specific<int>& myIsolatedLevel;
        int myNestedLevel;

        template <typename Partitioner, typename Body>
        static void RunTwoBodies( utils::FastRandom<>& rnd, const Body &body, Partitioner& p, tbb_kahypar::task_group_context* ctx = nullptr ) {
            if ( rnd.get() % 2 ) {
                if  (ctx )
                    tbb_kahypar::parallel_for( 0, 2, body, p, *ctx );
                else
                    tbb_kahypar::parallel_for( 0, 2, body, p );
            } else {
                tbb_kahypar::parallel_invoke( body, body );
            }
        }
        ...
        void RunNextLevel( utils::FastRandom<>& rnd, int &isolated_level ) const {
            ...
            switch ( rnd.get() % 2 ) {
                ...
                case 1: {
                    ...
                    tbb_kahypar::this_task_arena::isolate( IsolatedBody<Partitioner>( *this, p ) );
    ...
    public:
        HeavyMixTestBody( tbb_kahypar::enumerable_thread_specific<utils::FastRandom<>>& random,
            tbb_kahypar::enumerable_thread_specific<int>& isolated_level, int nested_level )
        ...
        void operator()() const {
            ...
            if ( rnd.get() % 2 == 1 ) {
                RunNextLevel<tbb_kahypar::auto_partitioner>( rnd, isolated_level );
            } else {
                RunNextLevel<tbb_kahypar::affinity_partitioner>( rnd, isolated_level );
            }
        }
        void operator()(int) const {
            this->operator()();
    ...
    struct RandomInitializer {
        utils::FastRandom<> operator()() {
            return utils::FastRandom<>( tbb_kahypar::this_task_arena::current_thread_index() );
        }
    };

    void HeavyMixTest() {
        std::size_t num_threads = tbb_kahypar::this_task_arena::max_concurrency() < 3 ? 3 : tbb_kahypar::this_task_arena::max_concurrency();
        tbb_kahypar::global_control ctl(tbb_kahypar::global_control::max_allowed_parallelism, num_threads);

        RandomInitializer init_random;
        tbb_kahypar::enumerable_thread_specific<utils::FastRandom<>> random( init_random );
        tbb_kahypar::enumerable_thread_specific<int> isolated_level( 0 );
        for ( int i = 0; i < 5; ++i ) {
    ...
    struct ExceptionTestBody : utils::NoAssign {
        tbb_kahypar::enumerable_thread_specific<int>& myEts;
        std::atomic<bool>& myIsStolen;
        ExceptionTestBody( tbb_kahypar::enumerable_thread_specific<int>& ets, std::atomic<bool>& is_stolen )
            : myEts( ets ), myIsStolen( is_stolen ) {}
        void operator()( int i ) const {
            try {
                tbb_kahypar::this_task_arena::isolate( IsolatedBodyThrowsException() );
            ...
            tbb_kahypar::parallel_for( 0, 10+i, utils::DummyBody( 100 ) );
            --e;
        }
    };
    ...
    void ExceptionTest() {
#if TBB_USE_EXCEPTIONS
        tbb_kahypar::enumerable_thread_specific<int> ets;
        std::atomic<bool> is_stolen;
        is_stolen = false;
        for ( ;; ) {
            tbb_kahypar::parallel_for( 0, 1000, ExceptionTestBody( ets, is_stolen ) );
            if ( is_stolen ) break;
        }
    ...
    void TestNonConstBody() {
        ...
        tbb_kahypar::this_task_arena::isolate(body);
        REQUIRE_MESSAGE(body.state == 0x93682a12, "The wrong state");
    }

    // TODO: Consider tbb_kahypar::task_group instead of explicit task API.
    class TestEnqueueTask : public tbb_kahypar::detail::d1::task {
        using wait_context = tbb_kahypar::detail::d1::wait_context;

        tbb_kahypar::enumerable_thread_specific<bool>& executed;
    ...
    public:
        ...
        TestEnqueueTask(tbb_kahypar::enumerable_thread_specific<bool>& exe, std::atomic<int>& c, wait_context& w, tbb_kahypar::task_arena& a)
            : executed(exe), completed(c), waiter(w), arena(a) {}

        tbb_kahypar::detail::d1::task* execute(tbb_kahypar::detail::d1::execution_data&) override {
            for (int i = 0; i < N; ++i) {
                arena.enqueue([&]() {
                    executed.local() = true;
                    ++completed;
        ...
        tbb_kahypar::detail::d1::task* cancel(tbb_kahypar::detail::d1::execution_data&) override { return nullptr; }
    };
    ...
    public:
        ...
        TestEnqueueIsolateBody(tbb_kahypar::enumerable_thread_specific<bool>& exe, std::atomic<int>& c, tbb_kahypar::task_arena& a)
            : executed(exe), completed(c), arena(a) {}
        void operator()() {
            tbb_kahypar::task_group_context ctx;
            tbb_kahypar::detail::d1::wait_context waiter(N);

            TestEnqueueTask root(executed, completed, waiter, arena);
            tbb_kahypar::detail::d1::execute_and_wait(root, ctx, waiter, ctx);
        }
    };

    void TestEnqueue() {
        tbb_kahypar::enumerable_thread_specific<bool> executed(false);
        std::atomic<int> completed;
        tbb_kahypar::task_arena arena{tbb_kahypar::task_arena::attach()};
...
void TestIsolatedExecute() {
    ...
    int platform_max_thread = tbb_kahypar::this_task_arena::max_concurrency();
    int num_threads = utils::min( platform_max_thread, 3 );
    {
        ...
        tbb_kahypar::global_control ctl(tbb_kahypar::global_control::max_allowed_parallelism, utils::max(num_threads, 7));
...
class TestDelegatedSpawnWaitBody : utils::NoAssign {
    ...
    utils::SpinBarrier &my_b1, &my_b2;
public:
    TestDelegatedSpawnWaitBody( tbb_kahypar::task_arena &a, utils::SpinBarrier &b1, utils::SpinBarrier &b2)
        : my_a(a), my_b1(b1), my_b2(b2) {}
    // NativeParallelFor's functor
    void operator()(int idx) const {
...
void TestDelegatedSpawnWait() {
    if (tbb_kahypar::this_task_arena::max_concurrency() < 3) {
...
class TestMultipleWaitsArenaWait : utils::NoAssign {
    using wait_context = tbb_kahypar::detail::d1::wait_context;
public:
    TestMultipleWaitsArenaWait( int idx, int bunch_size, int num_tasks, std::vector<wait_context*>& waiters, std::atomic<int>& processed, tbb_kahypar::task_group_context& tgc )
    ...
    void operator()() const {
        ...
        if ( my_idx < my_num_tasks ) {
            tbb_kahypar::detail::d1::wait(*my_waiters[my_idx], my_context);
        }
...
private:
    ...
    tbb_kahypar::task_group_context& my_context;
};

class TestMultipleWaitsThreadBody : utils::NoAssign {
    using wait_context = tbb_kahypar::detail::d1::wait_context;
public:
    TestMultipleWaitsThreadBody( int bunch_size, int num_tasks, tbb_kahypar::task_arena& a, std::vector<wait_context*>& waiters, std::atomic<int>& processed, tbb_kahypar::task_group_context& tgc )
...
private:
    int my_bunch_size;
    int my_num_tasks;
    tbb_kahypar::task_arena& my_arena;
    std::vector<wait_context*>& my_waiters;
    std::atomic<int>& my_processed;
    tbb_kahypar::task_group_context& my_context;
};
...
void TestMultipleWaits( int num_threads, int num_bunches, int bunch_size ) {
    tbb_kahypar::task_arena a( num_threads );
    const int num_tasks = (num_bunches-1)*bunch_size;

    tbb_kahypar::task_group_context tgc;
    std::vector<tbb_kahypar::detail::d1::wait_context*> waiters(num_tasks);
    for (auto& w : waiters) w = new tbb_kahypar::detail::d1::wait_context(0);
...
#if TBB_TEST_LOW_WORKLOAD
    const int max_threads = std::min( 4, tbb_kahypar::this_task_arena::max_concurrency() );
#else
    const int max_threads = std::min( 16, tbb_kahypar::this_task_arena::max_concurrency() );
#endif
...
void TestSmallStackSize() {
    tbb_kahypar::global_control gc(tbb_kahypar::global_control::thread_stack_size,
            tbb_kahypar::global_control::active_value(tbb_kahypar::global_control::thread_stack_size) / 2 );
...
namespace TestReturnValueNS {
    ...
    tbb_kahypar::task_arena& arena() {
        static tbb_kahypar::task_arena a;
        return a;
    }
    ...
    void TestIsolate(F &f) {
        StateTrackableCounters::reset();
        ReturnType r{tbb_kahypar::this_task_arena::isolate(f)};
        r.check();
    }
    ...
    void Test() {
        ...
        tbb_kahypar::this_task_arena::isolate(Functor<void>());
        tbb_kahypar::this_task_arena::isolate(f2);
        tbb_kahypar::this_task_arena::isolate(function<void>);
    }
}
...
struct MyObserver: public tbb_kahypar::task_scheduler_observer {
    tbb_kahypar::enumerable_thread_specific<tbb_kahypar::task_arena*>& my_tls;
    tbb_kahypar::task_arena& my_arena;
    std::atomic<int>& my_failure_counter;
    std::atomic<int>& my_counter;
    utils::SpinBarrier& m_barrier;

    MyObserver(tbb_kahypar::task_arena& a,
        tbb_kahypar::enumerable_thread_specific<tbb_kahypar::task_arena*>& tls,
        std::atomic<int>& failure_counter,
        std::atomic<int>& counter,
        utils::SpinBarrier& barrier)
        : tbb_kahypar::task_scheduler_observer(a), my_tls(tls), my_arena(a),
    ...
    void on_scheduler_entry(bool worker) override {
        if (worker) {
            ++my_counter;
            tbb_kahypar::task_arena*& cur_arena = my_tls.local();
...
void TestArenaWorkersMigrationWithNumThreads(int n_threads = 0) {
    if (n_threads == 0) {
        n_threads = tbb_kahypar::this_task_arena::max_concurrency();
    }
    ...
    tbb_kahypar::global_control control(tbb_kahypar::global_control::max_allowed_parallelism, n_threads);
    ...
    std::vector<tbb_kahypar::task_arena> arenas(n_arenas);
    std::atomic<int> failure_counter;
    std::atomic<int> counter;
    tbb_kahypar::enumerable_thread_specific<tbb_kahypar::task_arena*> tls;
...
void TestArenaWorkersMigration() {
    ...
    if (tbb_kahypar::this_task_arena::max_concurrency() != 4) {
...
void TestDefaultCreatedWorkersAmount() {
    int threads = tbb_kahypar::this_task_arena::max_concurrency();
    utils::NativeParallelFor(1, [threads](int idx) {
        ...
        for (auto blocked : { false, true }) {
            for (int trail = 0; trail < (blocked ? 10 : 10000); ++trail) {
                tbb_kahypar::parallel_for(0, threads, [threads, blocked, &barrier](int) {
                    CHECK_FAST_MESSAGE(threads == tbb_kahypar::this_task_arena::max_concurrency(), "concurrency level is not equal specified threadnum");
                    CHECK_FAST_MESSAGE(tbb_kahypar::this_task_arena::current_thread_index() < tbb_kahypar::this_task_arena::max_concurrency(), "amount of created threads is more than specified by default");
                    local_id.local() = 1;
                    if (blocked) {
...
void TestAbilityToCreateWorkers(int thread_num) {
    tbb_kahypar::global_control thread_limit(tbb_kahypar::global_control::max_allowed_parallelism, thread_num);
...
void ExceptionInExecute() {
    ...
    tbb_kahypar::task_arena test_arena(arena_concurrency, arena_concurrency);
...
class simple_observer : public tbb_kahypar::task_scheduler_observer {
    ...
    {}
public:
    simple_observer(tbb_kahypar::task_arena &a, int maxConcurrency, int numReservedSlots)
        : tbb_kahypar::task_scheduler_observer(a), my_idx(idx_counter++)
        , myMaxConcurrency(maxConcurrency)
        , myNumReservedSlots(numReservedSlots) {
        observe(true);
    }
...
struct arena_handler {
    ...
    tbb_kahypar::spin_rw_mutex arena_in_use{};

    tbb_kahypar::concurrent_set<simple_observer> observers;

    arena_handler(tbb_kahypar::task_arena* ptr) : arena(ptr)
    {}
...
void StressTestMixFunctionality() {
    ...
    tbb_kahypar::spin_mutex random_operation_guard;

    auto get_random_operation = [&operation_rnd, &random_operation_guard, operations_number] () {
        tbb_kahypar::spin_mutex::scoped_lock lock(random_operation_guard);
        return static_cast<operation_type>(operation_rnd.get() % operations_number);
    };

    utils::FastRandom<> arena_rnd(42);
    tbb_kahypar::spin_mutex random_arena_guard;
    auto get_random_arena = [&arena_rnd, &random_arena_guard] () {
        tbb_kahypar::spin_mutex::scoped_lock lock(random_arena_guard);
        return arena_rnd.get();
    };

    tbb_kahypar::concurrent_set<arena_handler> arenas_pool;
    ...
    auto find_arena = [&arenas_pool](tbb_kahypar::spin_rw_mutex::scoped_lock& lock) -> decltype(arenas_pool.begin()) {
    ...
    auto thread_func = [&] () {
        arenas_pool.emplace(new tbb_kahypar::task_arena());
        thread_barrier.wait();
        while (curr_operation++ < max_operations) {
            switch (get_random_operation()) {
                ...
                case delete_arena :
                {
                    ...
                    tbb_kahypar::spin_rw_mutex::scoped_lock lock(curr_arena->arena_in_use, /*writer*/ true);
                ...
                {
                    tbb_kahypar::spin_rw_mutex::scoped_lock lock{};
                ...
                case arena_execute:
                {
                    ...
                    if (curr_arena != arenas_pool.end()) {
                        curr_arena->arena->execute([]() {
                            tbb_kahypar::affinity_partitioner aff;
                            tbb_kahypar::parallel_for(0, 10000, utils::DummyBody(10), tbb_kahypar::auto_partitioner{});
                            tbb_kahypar::parallel_for(0, 10000, utils::DummyBody(10), aff);
                        });
                    }
...
struct enqueue_test_helper {
    enqueue_test_helper(tbb_kahypar::task_arena& arena, tbb_kahypar::enumerable_thread_specific<bool>& ets , std::atomic<std::size_t>& task_counter)
...
TEST_CASE("task_arena initialize soft limit ignoring affinity mask") {
    ...
    tbb_kahypar::task_arena arena(int(utils::get_platform_max_threads() * 2));
    arena.execute([&ets] {
        tbb_kahypar::parallel_for(0, 10000000, [&ets](int){
            ets.local() = 1;
            utils::doDummyWork(100);
        });
    });
...
TEST_CASE("Isolated execute") {
    ...
    if (tbb_kahypar::this_task_arena::max_concurrency() > 2) {
        TestIsolatedExecute();
    }
}
#endif
...
//! \brief \ref error_guessing
TEST_CASE("Exception thrown during tbb_kahypar::task_arena::execute call") {
    struct throwing_obj {
        throwing_obj() {
...
TEST_CASE("Workers oversubscription") {
    ...
    tbb_kahypar::enumerable_thread_specific<bool> ets;
    tbb_kahypar::global_control gl(tbb_kahypar::global_control::max_allowed_parallelism, num_threads * 2);
    tbb_kahypar::task_arena arena(static_cast<int>(num_threads) * 2);

    utils::SpinBarrier barrier(num_threads * 2);

    arena.execute([&] {
        tbb_kahypar::parallel_for(std::size_t(0), num_threads * 2,
            [&] (const std::size_t&) {
                ets.local() = true;
                barrier.wait();
            }
...
TEST_CASE("Empty task_handle cannot be scheduled"
    ...
    CHECK_THROWS_WITH_AS(ta.enqueue(tbb_kahypar::task_handle{}),                    "Attempt to schedule empty task_handle", std::runtime_error);
    CHECK_THROWS_WITH_AS(tbb_kahypar::this_task_arena::enqueue(tbb_kahypar::task_handle{}), "Attempt to schedule empty task_handle", std::runtime_error);
}
#endif
...
TEST_CASE("is_inside_task in task_group"){
    CHECK( false == tbb_kahypar::is_inside_task());

    tbb_kahypar::task_group tg;
    tg.run_and_wait([&]{
        CHECK( true == tbb_kahypar::is_inside_task());
    });
}
...
TEST_CASE("worker threads occupy slots in correct range") {
    std::vector<tbb_kahypar::task_arena> arenas(42);
