...
public:
    ...
    {}
    InnerBody( InnerBody& x, tbb_kahypar::split ) :
        my_a(x.my_a), my_b(x.my_b), i(x.i), sum(0)
    {
    }
    void join( InnerBody& x ) { sum += x.sum; }
    void operator()( const tbb_kahypar::blocked_range<int>& range ) {
...
#pragma omp for
        for (int i = 0; i < m + n - 1; ++i) {
            ...
            tbb_kahypar::parallel_reduce(tbb_kahypar::blocked_range<int>(start, finish, 10), body);
            c[i] = body.sum;
        }
    }
}
...
public:
    ...
    {}
    void operator()( const tbb_kahypar::blocked_range<int>& range ) const {
...
void TBB_OpenMP_Convolve( data_type c[], const data_type a[], int m, const data_type b[], int n, int p ) {
    tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<int>(0, m + n - 1, 10), OuterBody(c, a, m, b, n, p));
}
...
void TestNumThreads() {
    ...
    #pragma omp parallel
    {}
    int tbb_nthreads = tbb_kahypar::this_task_arena::max_concurrency();
