...
struct decrement_wait : utils::NoAssign {
    ...
    tbb_kahypar::flow::graph * const my_graph;
    bool * const my_done_flag;

    decrement_wait( tbb_kahypar::flow::graph &h, bool *done_flag ) : my_graph(&h), my_done_flag(done_flag) {}
...
class my_int_buffer {
    tbb_kahypar::flow::buffer_node<int> *b;
    tbb_kahypar::flow::graph& my_graph;
public:
    my_int_buffer(tbb_kahypar::flow::graph &g) : my_graph(g) { b = new tbb_kahypar::flow::buffer_node<int>(my_graph); }
    my_int_buffer(const my_int_buffer& other) : my_graph(other.my_graph) {
        b = new tbb_kahypar::flow::buffer_node<int>(my_graph);
    }
...
void test_iterator() {
    ...
    int count = 0;
    for (tbb_kahypar::flow::graph::iterator it = g.begin(); it != g.end(); ++it) {
        count++;
    }
...
public:
    AddRemoveBody(int nthr, utils::SpinBarrier &barrier_, tbb_kahypar::flow::graph& _g) :
...
inline void check_arena(tbb_kahypar::task_arena* midway_arena) {
    CHECK_MESSAGE(midway_arena->max_concurrency() == 2, "");
    CHECK_MESSAGE(tbb_kahypar::this_task_arena::max_concurrency() == 1, "");
}

struct run_functor {
    tbb_kahypar::task_arena* midway_arena;
    int return_value;
    run_functor(tbb_kahypar::task_arena* a) : midway_arena(a), return_value(1) {}
    int operator()() {
...
struct function_body {
    tbb_kahypar::task_arena* midway_arena;
    function_body(tbb_kahypar::task_arena* a) : midway_arena(a) {}
    tbb_kahypar::flow::continue_msg operator()(const T& /*arg*/) {
        check_arena(midway_arena);
        return tbb_kahypar::flow::continue_msg();
    }
};

typedef tbb_kahypar::flow::multifunction_node< int, std::tuple< int > > mf_node;

struct multifunction_body {
    tbb_kahypar::task_arena* midway_arena;
    multifunction_body(tbb_kahypar::task_arena* a) : midway_arena(a) {}
...
struct input_body {
    tbb_kahypar::task_arena* midway_arena;
    int counter;
    input_body(tbb_kahypar::task_arena* a) : midway_arena(a), counter(0) {}
...
struct nodes_test_functor : utils::NoAssign {
    ...
    nodes_test_functor(tbb_kahypar::task_arena* a, tbb_kahypar::flow::graph& g) : midway_arena(a), my_graph(g) {}
    void operator()() const {

        // Define test nodes
        // Continue, function, source nodes
        tbb_kahypar::flow::continue_node< tbb_kahypar::flow::continue_msg > c_n(my_graph, function_body<tbb_kahypar::flow::continue_msg>(midway_arena));
        tbb_kahypar::flow::function_node< int > f_n(my_graph, tbb_kahypar::flow::unlimited, function_body<int>(midway_arena));
        tbb_kahypar::flow::input_node< int > s_n(my_graph, input_body(midway_arena));

        // Multifunction node
        mf_node m_n(my_graph, tbb_kahypar::flow::unlimited, multifunction_body(midway_arena));

        // Join node
        tbb_kahypar::flow::function_node< std::tuple< int, int > > join_f_n(
            my_graph, tbb_kahypar::flow::unlimited, function_body< std::tuple< int, int > >(midway_arena)
        );
        tbb_kahypar::flow::join_node< std::tuple< int, int > > j_n(my_graph);
        make_edge(j_n, join_f_n);

        // Split node
        tbb_kahypar::flow::function_node< int > split_f_n1 = f_n;
        tbb_kahypar::flow::function_node< int > split_f_n2 = f_n;
        tbb_kahypar::flow::split_node< std::tuple< int, int > > sp_n(my_graph);
        make_edge(tbb_kahypar::flow::output_port<0>(sp_n), split_f_n1);
        make_edge(tbb_kahypar::flow::output_port<1>(sp_n), split_f_n2);

        // Overwrite node
        tbb_kahypar::flow::function_node< int > ow_f_n = f_n;
        tbb_kahypar::flow::overwrite_node< int > ow_n(my_graph);
        make_edge(ow_n, ow_f_n);

        // Write once node
        tbb_kahypar::flow::function_node< int > w_f_n = f_n;
        tbb_kahypar::flow::write_once_node< int > w_n(my_graph);
        make_edge(w_n, w_f_n);

        // Buffer node
        tbb_kahypar::flow::function_node< int > buf_f_n = f_n;
        tbb_kahypar::flow::buffer_node< int > buf_n(my_graph);
        make_edge(w_n, buf_f_n);

        // Limiter node
        tbb_kahypar::flow::function_node< int > l_f_n = f_n;
        tbb_kahypar::flow::limiter_node< int > l_n(my_graph, 1);
        make_edge(l_n, l_f_n);

        // Execute nodes
        c_n.try_put( tbb_kahypar::flow::continue_msg() );
        f_n.try_put(1);
        m_n.try_put(1);
        s_n.activate();

        tbb_kahypar::flow::input_port<0>(j_n).try_put(1);
        tbb_kahypar::flow::input_port<1>(j_n).try_put(1);
...
TEST_CASE("Test wait_count"){
    for(unsigned int p=utils::MinThread; p<=utils::MaxThread; ++p ) {
        tbb_kahypar::task_arena arena(p);
        arena.execute(
            [&]() {
                test_wait_count();
            }
