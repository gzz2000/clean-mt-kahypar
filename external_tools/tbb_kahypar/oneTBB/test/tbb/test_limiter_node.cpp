...
using tbb_kahypar::detail::d2::graph_task;

template< typename T >
struct serial_receiver : public tbb_kahypar::flow::receiver<T>, utils::NoAssign {
   T next_value;
   tbb_kahypar::flow::graph& my_graph;

   serial_receiver(tbb_kahypar::flow::graph& g) : next_value(T(0)), my_graph(g) {}
   ...
   graph_task* try_put_task( const T &v ) override {
    ...
    graph_task * try_put_task( const T &v, const tbb_kahypar::detail::d2::message_metainfo& ) override {
...
template< typename T >
struct parallel_receiver : public tbb_kahypar::flow::receiver<T>, utils::NoAssign {

    std::atomic<int> my_count;
    tbb_kahypar::flow::graph& my_graph;

    parallel_receiver(tbb_kahypar::flow::graph& g) : my_graph(g) { my_count = 0; }
...
template< typename T >
struct empty_sender : public tbb_kahypar::flow::sender<T> {
...
struct put_body : utils::NoAssign {
    ...
    tbb_kahypar::flow::limiter_node<T> &my_lim;
    std::atomic<int> &my_accept_count;

    put_body( tbb_kahypar::flow::limiter_node<T> &lim, std::atomic<int> &accept_count ) :
...
struct put_dec_body : utils::NoAssign {
    ...
    put_dec_body( tbb_kahypar::flow::limiter_node<T> &lim, std::atomic<int> &accept_count ) :
        my_lim(lim), my_accept_count(accept_count) {}
    ...
    void operator()( int ) const {
        ...
        while ( local_accept_count < N ) {
            ...
            if ( msg == true ) {
                ++local_accept_count;
                ++my_accept_count;
                my_lim.decrementer().try_put( tbb_kahypar::flow::continue_msg() );
            }
        }
    }
...
template< typename T >
void test_puts_with_decrements( int num_threads, tbb_kahypar::flow::limiter_node< T >& lim , tbb_kahypar::flow::graph& g) {
    parallel_receiver<T> r(g);
    empty_sender< tbb_kahypar::flow::continue_msg > s;
    std::atomic<int> accept_count;
    accept_count = 0;
    tbb_kahypar::flow::make_edge( lim, r );
    tbb_kahypar::flow::make_edge(s, lim.decrementer());
...
int test_parallel(int num_threads) {
   ...
   for ( int i = 0; i < L; ++i ) {
       ...
       tbb_kahypar::flow::limiter_node< T > lim(g, i);
       parallel_receiver<T> r(g);
       std::atomic<int> accept_count;
       accept_count = 0;
       tbb_kahypar::flow::make_edge( lim, r );
   ...
   for ( int i = 1; i < L; ++i ) {
       ...
       tbb_kahypar::flow::limiter_node< T > lim_copy( lim );
...
int test_serial() {
   ...
   for ( int i = 1; i < L; ++i ) {
       ...
       for ( int j = 0; j < N; ++j ) {
           ...
           CHECK_MESSAGE( (( j < i && msg == true ) || ( j >= i && msg == false )), "" );
           if ( msg == false ) {
               lim.decrementer().try_put( tbb_kahypar::flow::continue_msg() );
               msg = lim.try_put( T(j) );
...
typedef tbb_kahypar::flow::multifunction_node<int, std::tuple<int,tbb_kahypar::flow::continue_msg> > mfnode_type;
...
struct mfnode_body {
    ...
    void operator()(const int &/*in*/, mfnode_type::output_ports_type &out) {
        ...
        if(!std::get<DECREMENT_OUTPUT>(out).try_put(tbb_kahypar::flow::continue_msg())) {
...
test_multifunction_to_limiter(int _max, int _nparallel) {
    ...
    local_cnt = 0;
    mfnode_type mf_node(g, tbb_kahypar::flow::unlimited, mfnode_body(_max, local_cnt));
    tbb_kahypar::flow::function_node<int, int> fn_node(g, tbb_kahypar::flow::unlimited, fn_body());
    tbb_kahypar::flow::limiter_node<int> lim_node(g, _nparallel);
    tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<LIMITER_OUTPUT>(mf_node), lim_node);
    tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<DECREMENT_OUTPUT>(mf_node), lim_node.decrementer());
    tbb_kahypar::flow::make_edge(lim_node, fn_node);
    tbb_kahypar::flow::make_edge(fn_node, mf_node);
...
test_continue_msg_reception() {
    tbb_kahypar::flow::graph g;
    tbb_kahypar::flow::limiter_node<int> ln(g,2);
    tbb_kahypar::flow::queue_node<int>   qn(g);
    tbb_kahypar::flow::make_edge(ln, qn);
    ln.decrementer().try_put(tbb_kahypar::flow::continue_msg());
    ln.try_put(42);
    g.wait_for_all();
    int outint;
...
void test_reserve_release_messages() {
    ...
    tbb_kahypar::flow::make_edge(limit, output_queue); //putting the successor back
...
void test_decrementer() {
    ...
    tbb_kahypar::flow::graph g;
    tbb_kahypar::flow::limiter_node<int, int> limit(g, threshold);
    tbb_kahypar::flow::queue_node<int> queue(g);
    ...
    tbb_kahypar::flow::limiter_node<int, long long> limit2(g, threshold2);
    ...
    tbb_kahypar::flow::limiter_node<int, long long> limit3(g, threshold3);
...
void test_try_put_without_successors() {
    tbb_kahypar::flow::graph g;
    int try_put_num{3};
    tbb_kahypar::flow::buffer_node<int> bn(g);
    tbb_kahypar::flow::limiter_node<int> ln(g, try_put_num);

    tbb_kahypar::flow::make_edge(bn, ln);
    ...
    tbb_kahypar::flow::function_node<int, int> fn(g, tbb_kahypar::flow::unlimited,
        [&](int input) {
            counter += input;
            return int{};
        }
    ...
    tbb_kahypar::flow::make_edge(ln, fn);

    g.wait_for_all();
    CHECK((counter == i * try_put_num / 2));

    // Check the lost message
    tbb_kahypar::flow::remove_edge(bn, ln);
    ln.decrementer().try_put(tbb_kahypar::flow::continue_msg());
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows
        <msg_t, tbb_kahypar::flow::limiter_node<msg_t, msg_t>>(messages_for_follows, 1000);
    follows_and_precedes_testing::test_precedes
        <msg_t, tbb_kahypar::flow::limiter_node<msg_t, msg_t>>(messages_for_precedes, 1000);
...
void test_decrement_while_try_put_task() {
    ...
    tbb_kahypar::flow::graph graph{};
    std::atomic<int> processed;
    tbb_kahypar::flow::input_node<int> input{ graph, [&](tbb_kahypar::flow_control & fc) -> int {
        static int i = {};
        if (i++ >= threshold) fc.stop();
        return i;
    }};
    tbb_kahypar::flow::limiter_node<int, int> blockingNode{ graph, 1 };
    tbb_kahypar::flow::multifunction_node<int, std::tuple<int>> processing{ graph, tbb_kahypar::flow::serial,
    ...
    tbb_kahypar::flow::make_edge(input, blockingNode);
    tbb_kahypar::flow::make_edge(blockingNode, processing);
    tbb_kahypar::flow::make_edge(processing, blockingNode.decrementer());
...
void test_try_put_and_wait() {
    ...
    arena.execute([] {
        ...
        tbb_kahypar::flow::limiter_node<int> limiter(g, threshold);
        tbb_kahypar::flow::function_node<int, tbb_kahypar::flow::continue_msg> function(g, tbb_kahypar::flow::serial,
            [&](int input) {
        ...
        tbb_kahypar::flow::make_edge(function, limiter.decrementer());
...
TEST_CASE("Serial and parallel tests") {
    for (unsigned i = 1; i <= 2 * utils::MaxThread; ++i) {
        tbb_kahypar::task_arena arena(i);
        arena.execute(
            [i]() {
                test_serial<int>();
                test_parallel<int>(i);
...
TEST_CASE("Test correct node deallocation while using small_object_pool") {
    tbb_kahypar::flow::graph graph;
    tbb_kahypar::flow::queue_node<TestLargeStruct> input_node( graph );
    tbb_kahypar::flow::function_node<TestLargeStruct> func( graph, tbb_kahypar::flow::serial,
        [](const TestLargeStruct& input) { return input; } );

    tbb_kahypar::flow::make_edge( input_node, func );
    CHECK( input_node.try_put( TestLargeStruct{} ) );
    graph.wait_for_all();

    tbb_kahypar::task_scheduler_handle handle{ tbb_kahypar::attach{} };
    tbb_kahypar::finalize( handle, std::nothrow );
}
