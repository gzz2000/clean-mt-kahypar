...
void buffered_levels( size_t concurrency, Body body ) {
    ...
    for ( size_t lc = 1; lc <= concurrency; ++lc ) {
        ...
        tbb_kahypar::flow::multifunction_node< InputType, OutputTuple > exe_node( g, lc, body );

        //Create a vector of identical exe_nodes
        std::vector< tbb_kahypar::flow::multifunction_node< InputType, OutputTuple > > exe_vec(2, exe_node);
        ...
        for (size_t node_idx=0; node_idx<exe_vec.size(); ++node_idx) {
            for (size_t num_receivers = 1; num_receivers <= MAX_NODES; ++num_receivers ) {
                ...
                for (size_t r = 0; r < num_receivers; ++r ) {
                    tbb_kahypar::flow::make_edge( tbb_kahypar::flow::output_port<0>(exe_vec[node_idx]), *receivers[r] );
                }
                ...
                for (size_t num_senders = 1; num_senders <= MAX_NODES; ++num_senders ) {
                    ...
                    for (size_t s = 0; s < num_senders; ++s ) {
                        ...
                        tbb_kahypar::flow::make_edge( *senders.back(), exe_vec[node_idx] );
                    }
                ...
                for (size_t r = 0; r < num_receivers; ++r ) {
                    tbb_kahypar::flow::remove_edge( tbb_kahypar::flow::output_port<0>(exe_vec[node_idx]), *receivers[r] );
                }
...
void buffered_levels_with_copy( size_t concurrency ) {
    ...
    for ( size_t lc = 1; lc <= concurrency; ++lc ) {
        ...
        tbb_kahypar::flow::multifunction_node< InputType, OutputTuple > exe_node( g, lc, cf );
        ...
        for (size_t num_receivers = 1; num_receivers <= MAX_NODES; ++num_receivers ) {
            ...
            for (size_t r = 0; r < num_receivers; ++r ) {
               tbb_kahypar::flow::make_edge( tbb_kahypar::flow::output_port<0>(exe_node), *receivers[r] );
            }
            ...
            for (size_t r = 0; r < num_receivers; ++r ) {
                tbb_kahypar::flow::remove_edge( tbb_kahypar::flow::output_port<0>(exe_node), *receivers[r] );
            }
...
void run_buffered_levels( int c ) {
    typedef typename tbb_kahypar::flow::multifunction_node<InputType,OutputTuple>::output_ports_type output_ports_type;
...
void concurrency_levels( size_t concurrency, Body body ) {
    ...
    for ( size_t lc = 1; lc <= concurrency; ++lc ) {
        ...
        tbb_kahypar::flow::multifunction_node< InputType, OutputTuple, tbb_kahypar::flow::rejecting > exe_node( g, lc, body );
            ...
            for (size_t num_senders = 1; num_senders <= MAX_NODES; ++num_senders ) {
                {
                    ...
                    tbb_kahypar::spin_rw_mutex::scoped_lock l(
                        harness_graph_multifunction_executor< InputType, OutputTuple>::template mutex_holder<tbb_kahypar::spin_rw_mutex>::mutex
                    );
...
void run_concurrency_levels( int c ) {
    typedef typename tbb_kahypar::flow::multifunction_node<InputType,OutputTuple>::output_ports_type output_ports_type;
    concurrency_levels<InputType,OutputTuple>( c, []( InputType i, output_ports_type &p ) { harness_graph_multifunction_executor<InputType, OutputTuple>::template tfunc<tbb_kahypar::spin_rw_mutex>(i,p); } );
    concurrency_levels<InputType,OutputTuple>( c, &harness_graph_multifunction_executor<InputType, OutputTuple>::template tfunc<tbb_kahypar::spin_rw_mutex> );
    concurrency_levels<InputType,OutputTuple>( c, typename harness_graph_multifunction_executor<InputType, OutputTuple>::template tfunctor<tbb_kahypar::spin_rw_mutex>() );
}
...
void unlimited_concurrency( Body body ) {
    ...
    for (unsigned int p = 1; p < 2*utils::MaxThread; ++p) {
        tbb_kahypar::flow::graph g;
        tbb_kahypar::flow::multifunction_node< InputType, OutputTuple, tbb_kahypar::flow::rejecting > exe_node( g, tbb_kahypar::flow::unlimited, body );
...
void run_multiport_test(int num_threads) {
    typedef typename tbb_kahypar::flow::multifunction_node<InputType, OutputTuple> mo_node_type;
    typedef typename std::tuple_element<0,OutputTuple>::type EvenType;
    typedef typename std::tuple_element<1,OutputTuple>::type OddType;
    ...
    arena.execute(
        [&] () {
            tbb_kahypar::flow::graph g;
            mo_node_type mo_node(g, tbb_kahypar::flow::unlimited, oddEvenBody<InputType, OutputTuple>() );

            tbb_kahypar::flow::queue_node<EvenType> q0(g);
            tbb_kahypar::flow::queue_node<OddType> q1(g);

            tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<0>(mo_node), q0);
            tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<1>(mo_node), q1);
...
void test_concurrency(int num_threads) {
    ...
    arena.execute(
        [&] () {
            ...
            run_concurrency_levels<int,std::tuple<tbb_kahypar::flow::continue_msg> >(num_threads);
            ...
            run_unlimited_concurrency<int,std::tuple<tbb_kahypar::flow::continue_msg> >();
            run_unlimited_concurrency<empty_no_assign,std::tuple<tbb_kahypar::flow::continue_msg> >();
...
void test_ports_return_references() {
    ...
    tbb_kahypar::flow::multifunction_node<InputType, OutputTuple, Policy> mf_node(
        g, tbb_kahypar::flow::unlimited,
...
void test_follows_and_precedes_api() {
    using multinode = tbb_kahypar::flow::multifunction_node<int, std::tuple<int, int, int>>;

    std::array<int, 3> messages_for_follows = { {0, 1, 2} };

    follows_and_precedes_testing::test_follows
        <int, tbb_kahypar::flow::multifunction_node<int, std::tuple<int, int, int>>>
        (messages_for_follows, tbb_kahypar::flow::unlimited, [](const int& i, multinode::output_ports_type& op) -> void {
            std::get<0>(op).try_put(i);
        });
...
TEST_CASE("Test ports retrurn references"){
    test_ports_return_references<tbb_kahypar::flow::queueing>();
    test_ports_return_references<tbb_kahypar::flow::rejecting>();
}

//! NativeParallelFor testing with various concurrency settings
//! \brief \ref error_guessing
TEST_CASE("Lightweight testing"){
    lightweight_testing::test<tbb_kahypar::flow::multifunction_node>(10);
}
...
TEST_CASE("constraints for multifunction_node input") {
    ...
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::multifunction_node, InputObject, int>);
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::multifunction_node, int, int>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::multifunction_node, test_concepts::NonCopyable, int>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::multifunction_node, test_concepts::NonDefaultInitializable, int>);
}

template <typename Input, typename Output, typename Body>
concept can_call_multifunction_node_ctor = requires( tbb_kahypar::flow::graph& graph, std::size_t concurrency, Body body,
                                                     tbb_kahypar::flow::node_priority_t priority, tbb_kahypar::flow::buffer_node<int>& f ) {
    tbb_kahypar::flow::multifunction_node<Input, Output>(graph, concurrency, body);
    tbb_kahypar::flow::multifunction_node<Input, Output>(graph, concurrency, body, priority);
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
    tbb_kahypar::flow::multifunction_node<Input, Output>(tbb_kahypar::flow::follows(f), concurrency, body);
    tbb_kahypar::flow::multifunction_node<Input, Output>(tbb_kahypar::flow::follows(f), concurrency, body, priority);
#endif // __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
};
