...
struct task_scheduler_handle_guard {
    tbb_kahypar::task_scheduler_handle m_handle{};

    task_scheduler_handle_guard() {
        m_handle = tbb_kahypar::task_scheduler_handle{tbb_kahypar::attach{}};
    }
    ...
    tbb_kahypar::task_scheduler_handle& get() {
        return m_handle;
    }
};
...
namespace TestBlockingTerminateNS {
    ...
    struct TestAutoInitBody {
        void operator()( int ) const {
            tbb_kahypar::parallel_for( 0, 100, utils::DummyBody() );
        }
    };
            ...
            case 5:
                {
                    ...
                    bool res = tbb_kahypar::finalize( init2.get(), std::nothrow );
                    REQUIRE( !res );
                }
                break;
            }
            if ( !myAutoInit && tbb_kahypar::finalize( init.get(), std::nothrow ) )
                ++gNumSuccesses;
        }
    };
    ...
    void TestMultpleWait() {
        ...
        gSeed = tbb_kahypar::this_task_arena::max_concurrency();
    ...
    void TestException( F &f ) {
        ...
        catch ( const tbb_kahypar::unsafe_wait& e) {
    ...
    public:
        ...
        void operator()() {
            ...
            tbb_kahypar::parallel_for(0, 2, utils::DummyBody()); // auto-init
            tbb_kahypar::finalize((myIndex == 0 ? tsi1.get() : tsi2.get()));
    ...
    struct ExceptionTest2 {
        ...
        public:
            ...
            void operator()( int ) const {
                myBarrier.wait();
                task_scheduler_handle_guard init;
                tbb_kahypar::finalize( init.get() );
                REQUIRE_MESSAGE( false, "Blocking terminate did not throw the exception inside the parallel region" );
            }
        };
        void operator()() {
            const int numThreads = 4;
            tbb_kahypar::global_control init(tbb_kahypar::global_control::max_allowed_parallelism, numThreads);
            tbb_kahypar::task_arena a(numThreads);
            a.execute([&] {
                utils::SpinBarrier barrier(numThreads);
                tbb_kahypar::parallel_for(0, numThreads, Body(barrier));
...
void TestTerminationAndAutoinit(bool autoinit) {
    ...
    bool res1 = tbb_kahypar::finalize(ctl1.get(), std::nothrow);
    if (autoinit) {
        REQUIRE(!res1);
    } else {
        REQUIRE(res1);
    }
    bool res2 = tbb_kahypar::finalize(ctl2.get(), std::nothrow);
    REQUIRE(res2);
}
...
TEST_CASE("test decrease reference") {
    ...
    std::thread thr([] { tbb_kahypar::parallel_for(0, 1, [](int) {}); } );
    thr.join();

    REQUIRE(tbb_kahypar::finalize(handle, std::nothrow));
}
...
TEST_CASE("test concurrent task_scheduler_handle destruction") {
    ...
    std::thread thr1([&] {
        while (!stop) {
            auto h = tbb_kahypar::task_scheduler_handle{ tbb_kahypar::attach{} };
            tbb_kahypar::finalize(h, std::nothrow_t{});
        }
    });
