...
class leaving_observer : public tbb_kahypar::task_scheduler_observer {
    std::atomic<int> my_thread_count{};
public:
    leaving_observer(tbb_kahypar::task_arena& a) : tbb_kahypar::task_scheduler_observer(a) {
        observe(true);
    }
...
template <typename PerBodyFunc> float test(PerBodyFunc&& body) {
    ...
    tbb_kahypar::global_control concurrency(tbb_kahypar::global_control::max_allowed_parallelism, num_threads);
    tbb_kahypar::task_arena big_arena(static_cast<int>(num_threads));
    ...
    for (std::size_t repeat = 0; repeat < repeats; ++repeat) {
        big_arena.execute([&] {
            tbb_kahypar::parallel_for(
                tbb_kahypar::blocked_range<std::size_t>(0, per_thread_iters * num_threads),
                [&](const tbb_kahypar::blocked_range<std::size_t>& r) {
                    int thread_id = tbb_kahypar::this_task_arena::current_thread_index();
...
void test_custom_range(int diff_mult) {
    ...
    oneapi::tbb_kahypar::mutex results_mutex;

    for (int i = 0; i < num_trials; ++i) {
        oneapi::tbb_kahypar::parallel_for(Range(0, int(100 * utils::get_platform_max_threads()), 1), [&] (const Range& r) {
            oneapi::tbb_kahypar::mutex::scoped_lock lock(results_mutex);
...
TEST_CASE("Test partitioned tasks count and size for static_partitioner") {
    class custom_range : public oneapi::tbb_kahypar::blocked_range<int> {
        using base_type = oneapi::tbb_kahypar::blocked_range<int>;
    public:
        custom_range(int l, int r, int g) : base_type(l, r, g) {}
        custom_range(const custom_range& r) : base_type(r) {}

        custom_range(custom_range& r, tbb_kahypar::split) : base_type(r, tbb_kahypar::split()) {}
    };

    test_custom_range<custom_range>(2);

    class custom_range_with_psplit : public oneapi::tbb_kahypar::blocked_range<int> {
        using base_type = oneapi::tbb_kahypar::blocked_range<int>;
    public:
        ...
        custom_range_with_psplit(custom_range_with_psplit& r, tbb_kahypar::split) : base_type(r, tbb_kahypar::split()) {}
        custom_range_with_psplit(custom_range_with_psplit& r, tbb_kahypar::proportional_split& p) : base_type(r, p) {}
    };
