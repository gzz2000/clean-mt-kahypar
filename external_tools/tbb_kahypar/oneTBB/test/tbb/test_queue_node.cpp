...
void spin_try_get( tbb_kahypar::flow::queue_node<T> &q, T &value ) {
    ...
    while ( q.try_get(value) != true ) {
        ...
        if (count == 1000000) {
            ...
            tbb_kahypar::task_arena a(tbb_kahypar::task_arena::attach{});
            a.enqueue([]{});
            ++count;
        }
    }
...
struct parallel_puts : utils::NoAssign {
    ...
    tbb_kahypar::flow::queue_node<T> &my_q;

    parallel_puts( tbb_kahypar::flow::queue_node<T> &q ) : my_q(q) {}
...
struct parallel_gets : utils::NoAssign {
    ...
    parallel_gets( tbb_kahypar::flow::queue_node<T> &q, touches<T> &t) : my_q(q), my_touches(t) {}
...
struct parallel_put_get : utils::NoAssign {
    ...
    parallel_put_get( tbb_kahypar::flow::queue_node<T> &q, touches<T> &t ) : my_q(q), my_touches(t) {}
...
int test_reservation() {
    ...
    // Simple tests
    tbb_kahypar::flow::queue_node<T> q(g);
...
int test_parallel(int num_threads) {
    tbb_kahypar::flow::graph g;
    tbb_kahypar::flow::queue_node<T> q(g);
    tbb_kahypar::flow::queue_node<T> q2(g);
    tbb_kahypar::flow::queue_node<T> q3(g);
    {
        Checker< T > my_check;
        T bogus_value(-1);
        T j = bogus_value;
        ...
        utils::NativeParallelFor( num_threads, parallel_puts<T>(q) );
        tbb_kahypar::flow::queue_node<T> q_copy(q);
        j = bogus_value;
        g.wait_for_all();
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows <int, tbb_kahypar::flow::queue_node<int>>(messages_for_follows);
    follows_and_precedes_testing::test_precedes <int, tbb_kahypar::flow::queue_node<int>>(messages_for_precedes);
}
#endif
...
void test_queue_node_try_put_and_wait() {
    ...
    // test_buffer_push returns the index from which the items processed during wait_for_all() starts
    {
        ...
        std::size_t after_start = test_buffer_push<tbb_kahypar::flow::queue_node<int>>(start_work_items, wait_message,
...

    {
        ...
        std::size_t after_start = test_buffer_pull<tbb_kahypar::flow::queue_node<int>>(start_work_items, wait_message, occupier,
                                                                               new_work_items, processed_items);
    ...
    // Test reserve
    {
        ...
        for (int threshold : thresholds) {
            ...
            std::size_t after_start = test_buffer_reserve<tbb_kahypar::flow::queue_node<int>>(threshold,
...
TEST_CASE("Resets test"){
    INFO("Testing resets\n");
    test_resets<int, tbb_kahypar::flow::queue_node<int> >();
    test_resets<float, tbb_kahypar::flow::queue_node<float> >();
}
