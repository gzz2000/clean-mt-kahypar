...
//! Testing clock type of tbb_kahypar::tick_count
//! Clock in tbb_kahypar::tick_count should be steady
//! \brief \ref requirement
TEST_CASE("Clock in tbb_kahypar::tick_count should be steady") {
    CHECK_EQ(tbb_kahypar::tick_count::clock_type::is_steady, true);
}
...
TEST_CASE("Subtraction of equal tick_counts should not throw") {
    tbb_kahypar::tick_count tick_f = tbb_kahypar::tick_count::now();
    tbb_kahypar::tick_count tick_s(tick_f);
...
TEST_CASE("Test for subtracting calls to tick_count from different threads") {
    ...
    tbb_kahypar::tick_count start_time;

    auto diff_func = [&thread_barrier, &start_time] (std::size_t ) {
        thread_barrier.wait([&start_time] { start_time = tbb_kahypar::tick_count::now(); });

        tbb_kahypar::tick_count end_time(tbb_kahypar::tick_count::now());
        while ((end_time - start_time).seconds() == 0) {
            end_time = tbb_kahypar::tick_count::now();
        }
