...
struct Accumulator {
    ValueType operator() ( const tbb_kahypar::blocked_range<ValueType*>& r, ValueType value ) const {
...
public:
    ...
    ParallelSumTester() : m_range(nullptr, nullptr) {
        ...
        for ( ValueType i = 0; i < count; ++i )
            m_array[i] = i + 1;
        m_range = tbb_kahypar::blocked_range<ValueType*>( m_array, m_array + count );
    }
    ...
    void CheckParallelReduce() {
        ...
        ValueType result2 = reduce_invoker<ValueType>( m_range,
            [](const tbb_kahypar::blocked_range<ValueType*>& r, ValueType value) -> ValueType {
...
private:
    ValueType* m_array;
    tbb_kahypar::blocked_range<ValueType*> m_range;
...
struct ReduceToCancel {
    std::size_t operator()( const tbb_kahypar::blocked_range<std::size_t>&, std::size_t ) const {
        ++g_CurExecuted;
...
struct ReduceFunctorToCancel {
    ...
    ReduceFunctorToCancel( ReduceFunctorToCancel&, tbb_kahypar::split ) : result(0) {}

    void operator()( const tbb_kahypar::blocked_range<std::size_t>& br ) {
...
class ParallelReduceRunner {
    ...
    void run_parallel_reduce( Args&&... args ) const {
        switch(Mode % 5) {
            case 0 : {
                tbb_kahypar::parallel_reduce(std::forward<Args>(args)..., my_ctx);
                break;
            }
            case 1 : {
                tbb_kahypar::parallel_reduce(std::forward<Args>(args)..., tbb_kahypar::simple_partitioner{}, my_ctx);
                break;
            }
            case 2 : {
                tbb_kahypar::parallel_reduce(std::forward<Args>(args)..., tbb_kahypar::auto_partitioner{}, my_ctx);
                break;
            }
            case 3 : {
                tbb_kahypar::parallel_reduce(std::forward<Args>(args)..., tbb_kahypar::static_partitioner{}, my_ctx);
                break;
            }
            case 4 : {
                tbb_kahypar::affinity_partitioner aff;
                tbb_kahypar::parallel_reduce(std::forward<Args>(args)..., aff, my_ctx);
                break;
            }
        }
    }
...
public:
    ParallelReduceRunner( tbb_kahypar::task_group_context& ctx )
        : my_ctx(ctx) {}

    void operator()() const {
        tbb_kahypar::blocked_range<std::size_t> br(0, buffer_test_size);
        if (Mode < 5) {
...
class ParallelDeterministicReduceRunner {
    ...
    void run_parallel_deterministic_reduce( Args&&... args ) const {
        switch(Mode % 3) {
            case 0 : {
                tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)..., my_ctx);
                break;
            }
            case 1 : {
                tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)..., tbb_kahypar::simple_partitioner{}, my_ctx);
                break;
            }
            case 2 : {
                tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)..., tbb_kahypar::static_partitioner{}, my_ctx);
                break;
            }
        }
    }
...
public:
    ParallelDeterministicReduceRunner( tbb_kahypar::task_group_context& ctx )
        : my_ctx(ctx) {}
...
void run_parallel_reduce_cancellation_test() {
    for ( auto concurrency_level : utils::concurrency_range() ) {
        ...
        tbb_kahypar::global_control gc(tbb_kahypar::global_control::max_allowed_parallelism, concurrency_level);
        ResetEhGlobals();
...
concept can_call_parallel_reduce_basic = requires( Args&&... args ) {
    tbb_kahypar::parallel_reduce(std::forward<Args>(args)...);
};

template <typename... Args>
concept can_call_parallel_deterministic_reduce_basic = requires ( Args&&... args ) {
    tbb_kahypar::parallel_deterministic_reduce(std::forward<Args>(args)...);
};

template <typename... Args>
concept can_call_preduce_helper = can_call_parallel_reduce_basic<Args...> &&
                                  can_call_parallel_reduce_basic<Args..., tbb_kahypar::task_group_context&>;

template <typename... Args>
concept can_call_pdet_reduce_helper = can_call_parallel_deterministic_reduce_basic<Args...> &&
                                      can_call_parallel_deterministic_reduce_basic<Args..., tbb_kahypar::task_group_context&>;

template <typename... Args>
concept can_call_preduce_with_partitioner = can_call_preduce_helper<Args...> &&
                                            can_call_preduce_helper<Args..., const tbb_kahypar::simple_partitioner&> &&
                                            can_call_preduce_helper<Args..., const tbb_kahypar::auto_partitioner&> &&
                                            can_call_preduce_helper<Args..., const tbb_kahypar::static_partitioner&> &&
                                            can_call_preduce_helper<Args..., tbb_kahypar::affinity_partitioner&>;

template <typename... Args>
concept can_call_pdet_reduce_with_partitioner = can_call_pdet_reduce_helper<Args...> &&
                                                can_call_pdet_reduce_helper<Args..., const tbb_kahypar::simple_partitioner&> &&
                                                can_call_pdet_reduce_helper<Args..., const tbb_kahypar::static_partitioner&>;
...
TEST_CASE("Test parallel summation correctness") {
    ...
    pst.CheckParallelReduce<tbb_kahypar::simple_partitioner>();
    pst.CheckParallelReduce<tbb_kahypar::auto_partitioner>();
    pst.CheckParallelReduce<tbb_kahypar::affinity_partitioner>();
    pst.CheckParallelReduce<tbb_kahypar::static_partitioner>();
}
...
public:
    MinimalRange( MinimalRange& r, tbb_kahypar::split ) : end(r.end) {
...
public:
    ...
    FooBody( FooBody& other, tbb_kahypar::split ) {
        ++FooBodyCount;
        ++ForkCount;
        sum = 0;
        parent = &other;
...
TEST_CASE("Test splitting range and body during reduction, test that all workers sleep when no work") {
    for ( auto concurrency_level : utils::concurrency_range() ) {
        tbb_kahypar::global_control control(tbb_kahypar::global_control::max_allowed_parallelism, concurrency_level);

        TestSplitting<tbb_kahypar::simple_partitioner>(concurrency_level);
        TestSplitting<tbb_kahypar::static_partitioner>(concurrency_level);
        TestSplitting<tbb_kahypar::auto_partitioner>(concurrency_level);
        TestSplitting<tbb_kahypar::affinity_partitioner>(concurrency_level);
...
namespace unsupported {
    template<typename Range, typename Body>
    void parallel_deterministic_reduce(const Range&, Body&, const tbb_kahypar::auto_partitioner&) { }
    template<typename Range, typename Body>
    void parallel_deterministic_reduce(const Range&, Body&, tbb_kahypar::affinity_partitioner&) { }

    template<typename Range, typename Value, typename RealBody, typename Reduction>
    Value parallel_deterministic_reduce(const Range& , const Value& identity, const RealBody& , const Reduction& , const tbb_kahypar::auto_partitioner&) {
        return identity;
    }
    template<typename Range, typename Value, typename RealBody, typename Reduction>
    Value parallel_deterministic_reduce(const Range& , const Value& identity, const RealBody& , const Reduction& , tbb_kahypar::affinity_partitioner&) {
        return identity;
    }
}
...
struct Body {
    float value;
    Body() : value(0) {}
    Body(Body&, tbb_kahypar::split) { value = 0; }
    void operator()(const tbb_kahypar::blocked_range<int>&) {}
    void join(Body&) {}
};
...
TEST_CASE("Test Unsupported Partitioners") {
    ...
    Body body;
    parallel_deterministic_reduce(blocked_range<int>(0, 10), body, tbb_kahypar::auto_partitioner());

    tbb_kahypar::affinity_partitioner ap;
...
//! Testing tbb_kahypar::parallel_reduce with tbb_kahypar::task_group_context
//! \brief \ref interface \ref error_guessing
TEST_CASE("cancellation test for tbb_kahypar::parallel_reduce") {
    test_cancellation::ParallelReduceTestRunner</*First mode = */0>::run();
}

//! Testing tbb_kahypar::parallel_deterministic_reduce with tbb_kahypar::task_group_context
//! \brief \ref interface \ref error_guessing
TEST_CASE("cancellation test for tbb_kahypar::parallel_deterministic_reduce") {
