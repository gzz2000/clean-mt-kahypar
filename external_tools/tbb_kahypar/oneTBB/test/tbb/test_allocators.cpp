...
#include <tbb_kahypar/version.h> // For __TBB_GLIBCXX_VERSION and __TBB_CPP20_PRESENT
...
TEST_CASE("Test cache_aligned_allocate throws") {
    ...
    using namespace tbb_kahypar::detail::r1;
...
TEST_CASE("Broken allocator concept") {
    TestAllocator<tbb_kahypar::cache_aligned_allocator<void>>(Broken);
    TestAllocator<tbb_kahypar::tbb_allocator<void>>(Broken);
}
#endif
...
TEST_CASE("Test allocators with STL containers") {
    TestAllocatorWithSTL<tbb_kahypar::cache_aligned_allocator<void>>();
    TestAllocatorWithSTL<tbb_kahypar::tbb_allocator<void>>();
}
...
TEST_CASE("polymorphic_allocator test") {
    tbb_kahypar::cache_aligned_resource aligned_resource;
    tbb_kahypar::cache_aligned_resource equal_aligned_resource(std::pmr::get_default_resource());
