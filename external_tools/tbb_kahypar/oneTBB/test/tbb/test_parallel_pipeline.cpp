...
public:
    U operator()( tbb_kahypar::flow_control& control ) const {
        CHECK(is_alive());
        if( --input_counter < 0 ) {
            control.stop();
        }
...
public:
    int
    operator()(tbb_kahypar::flow_control& control ) const {
        CHECK(is_alive());
...
static const tbb_kahypar::filter_mode filter_table[] = { tbb_kahypar::filter_mode::parallel, tbb_kahypar::filter_mode::serial_in_order, tbb_kahypar::filter_mode::serial_out_of_order};
const unsigned number_of_filter_types = sizeof(filter_table)/sizeof(filter_table[0]);

using filter_chain = tbb_kahypar::filter<void, void>;
using mode_array =tbb_kahypar::filter_mode;
...
void fill_chain( filter_chain &my_chain, mode_array *filter_type, input_filter<type1> i_filter,
    ...
    my_chain = tbb_kahypar::filter<void, type1>(filter_type[0], i_filter) &
        tbb_kahypar::filter<type1, type2>(filter_type[1], m_filter) &
        tbb_kahypar::filter<type2, void>(filter_type[2], o_filter);
}
...
void run_function_spec(Context&... context) {
    ...
    for( unsigned i = 0; i<number_of_filter_types; i++) {
        tbb_kahypar::filter<void, void> one_filter( filter_table[i], i_filter );
        CHECK_MESSAGE(filter_node_count==1, "some filter nodes left after previous iteration?");
        resetCounters();
        tbb_kahypar::parallel_pipeline( n_tokens, one_filter, context... );
        // no need to check counters
        std::atomic<int> counter;
        counter = max_counter;
        // Construct filter using lambda-syntax when parallel_pipeline() is being run;
        tbb_kahypar::parallel_pipeline( n_tokens,
            tbb_kahypar::filter<void, void>(filter_table[i], [&counter]( tbb_kahypar::flow_control& control ) {
                    if( counter-- == 0 )
                        control.stop();
                    }
            ),
...
void run_filter_set(
        ...
        Context&... context) {
    tbb_kahypar::filter<void, t1> filter1( filter_type[0], i_filter );
    tbb_kahypar::filter<t1, t2> filter2( filter_type[1], m_filter );
    tbb_kahypar::filter<t2, void> filter3( filter_type[2], o_filter );
    ...
    tbb_kahypar::parallel_pipeline( n_tokens, filter1, filter2, filter3, context...  );
    checkCounters(my_t);

    // Create filters sequence partially outside parallel_pipeline() and also when parallel_pipeline() is being run
    tbb_kahypar::filter<void, t2> filter12;
    filter12 = filter1 & filter2;
    for( int i = 0; i<3; i++)
    {
        filter12 &= tbb_kahypar::filter<t2,t2>(filter_type[i], [](t2 x) -> t2 { return x;});
    }
    resetCounters();
    tbb_kahypar::parallel_pipeline( n_tokens, filter12, filter3, context...  );
    checkCounters(my_t);

    tbb_kahypar::filter<void, void> filter123 = filter12 & filter3;
    // Run pipeline twice with the same filter sequence
    for( unsigned i = 0; i<2; i++ ) {
        resetCounters();
        tbb_kahypar::parallel_pipeline( n_tokens, filter123, context...  );
        checkCounters(my_t);
    }

    // Now copy-and-move-construct another filter instance, and use it to run pipeline
    {
        tbb_kahypar::filter<void, void> copy123( filter123 );
        resetCounters();
        tbb_kahypar::parallel_pipeline( n_tokens, copy123, context...  );
        checkCounters(my_t);
        tbb_kahypar::filter<void, void> move123( std::move(filter123) );
        resetCounters();
        tbb_kahypar::parallel_pipeline( n_tokens, move123, context...  );
        checkCounters(my_t);
    }
    ...
    resetCounters();
    tbb_kahypar::parallel_pipeline( n_tokens,
               tbb_kahypar::filter<void, t1>(filter_type[0], i_filter),
               tbb_kahypar::filter<t1, t2>(filter_type[1], m_filter),
               tbb_kahypar::filter<t2, void>(filter_type[2], o_filter),
               context...  );
    checkCounters(my_t);
    ...
    {
        tbb_kahypar::filter<void, void>* p123 = new tbb_kahypar::filter<void,void> (
               tbb_kahypar::filter<void, t1>(filter_type[0], i_filter)&
               tbb_kahypar::filter<t1, t2>(filter_type[1], m_filter)&
               tbb_kahypar::filter<t2, void>(filter_type[2], o_filter) );
        CHECK_MESSAGE(filter_node_count==cnt+5, "filter node accounting error?");
        tbb_kahypar::filter<void, void> copy123( *p123 );
        delete p123;
    ...
    {
        tbb_kahypar::filter<void, void> my_filter;
        fill_chain<t1,t2>( my_filter, filter_type, i_filter, m_filter, o_filter );
        resetCounters();
        tbb_kahypar::parallel_pipeline( n_tokens, my_filter, context...  );
        checkCounters(my_t);
    }
...
void run_lambdas_test( mode_array *filter_type, Context&... context ) {
    ...
    tbb_kahypar::parallel_pipeline( n_tokens,
        tbb_kahypar::make_filter<void, t1>(filter_type[0], [&counter]( tbb_kahypar::flow_control& control ) -> t1 {
                if( --counter < 0 )
                    control.stop();
                return t1(); }
        ),
        tbb_kahypar::make_filter<t1, t2>(filter_type[1], []( t1 /*my_storage*/ ) -> t2 {
                return t2(); }
        ),
        tbb_kahypar::make_filter<t2,void>(filter_type[2], [] ( t2 ) -> void {
                output_counter++; }
        ),
        context...
    );
    ...
    resetCounters();
    tbb_kahypar::parallel_pipeline( n_tokens,
        tbb_kahypar::filter<void,t1*>(filter_type[0], [&counter]( tbb_kahypar::flow_control& control ) -> t1* {
                if( --counter < 0 ) {
                    control.stop();
                    return nullptr;
                }
		...
		tbb_kahypar::filter<t1*, t2*>(filter_type[1], []( t1* my_storage ) -> t2* {
                my_storage->~t1();
                return new(my_storage) t2(); }
        ),
		tbb_kahypar::filter<t2*, void>(filter_type[2], [] ( t2* my_storage ) -> void {
                my_storage->~t2();
    ...
    resetCounters();
    tbb_kahypar::parallel_pipeline( n_tokens,
        tbb_kahypar::make_filter(filter_type[0], [&counter]( tbb_kahypar::flow_control& control ) -> t1* {
                if( --counter < 0 ) {
                    control.stop();
                    return nullptr;
                }
        ...
        tbb_kahypar::make_filter(filter_type[1], []( t1* my_storage ) -> t2 {
                my_storage->~t1();
                freeBuffer(my_storage);
                return t2(); }
        ),
        tbb_kahypar::make_filter(filter_type[2], [] ( t2 /*my_storage*/) -> void {
                output_counter++; }
        ),
        context...
    );
    ...
    tbb_kahypar::parallel_pipeline( n_tokens,
        ...
        tbb_kahypar::make_filter<t2*, void>(filter_type[2], [] ( t2* my_storage) -> void {
                my_storage->~t2();
                freeBuffer(my_storage);
                output_counter++; }
        ),
...
#define RUN_TYPED_TEST_CASE(type1, type2) TEST_CASE("Pipeline testing with "#type1" and "#type2) { \
                                                for ( std::size_t concurrency_level : {1, 2, 4, 5, 7, 8} ) { \
                                                    if ( concurrency_level > tbb_kahypar::global_control::active_value(tbb_kahypar::global_control::max_allowed_parallelism) ) \
                                                        break; \
