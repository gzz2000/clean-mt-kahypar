...
class makeSplit {
public:
    static SType *create(tbb_kahypar::flow::graph& g) {
...
public:
    ...
    typedef typename tbb_kahypar::flow::queue_node<IT> my_sink_node_type;
    ...
    static void add_sink_nodes(SType &my_split, tbb_kahypar::flow::graph &g) {
        my_sink_node_type *new_node = new my_sink_node_type(g);
        tbb_kahypar::flow::make_edge( tbb_kahypar::flow::output_port<ELEM-1>(my_split) , *new_node);
    ...
    static void remove_sink_nodes(SType& my_split) {
        my_sink_node_type *dp = reinterpret_cast<my_sink_node_type *>(all_sink_nodes[ELEM-1]);
        tbb_kahypar::flow::remove_edge( tbb_kahypar::flow::output_port<ELEM-1>(my_split) , *dp);
        delete dp;
...
public:
    ...
    static void add_sink_nodes(SType &my_split, tbb_kahypar::flow::graph &g) {
        my_sink_node_type *new_node = new my_sink_node_type(g);
        tbb_kahypar::flow::make_edge( tbb_kahypar::flow::output_port<0>(my_split) , *new_node);
    ...
    static void remove_sink_nodes(SType& my_split) {
        my_sink_node_type *dp = reinterpret_cast<my_sink_node_type *>(all_sink_nodes[0]);
        tbb_kahypar::flow::remove_edge( tbb_kahypar::flow::output_port<0>(my_split) , *dp);
        delete dp;
    }
};
...
public:
    ...
    typedef tbb_kahypar::flow::input_node<TType> input_type;
    ...
    static void test() {
        ...
        for(int nInputs = 1; nInputs <= MaxNInputs; ++nInputs) {
            ...
            for(int i = 0; i < nInputs; ++i) {
                ...
                tbb_kahypar::flow::make_edge(*s, *my_split);
                all_input_nodes[i] = s;
                s->activate();
            }
...
template<typename SType>
void test_one_serial( SType &my_split, tbb_kahypar::flow::graph &g) {
    typedef typename SType::input_type TType;
    ...
    tbb_kahypar::flow::make_edge( q3, my_split );
...
struct generate_test {
    typedef tbb_kahypar::flow::split_node<TupleType> split_node_type;
    static void do_test() {
...
void test_try_put_and_wait() {
    ...
    arena.execute([] {
        ...
        using tuple_type = std::tuple<int, int>;
        tbb_kahypar::flow::split_node<tuple_type> split(g);

        tbb_kahypar::flow::function_node<int, int> function1(g, tbb_kahypar::flow::unlimited,
            [&](int input) noexcept {
        ...
        tbb_kahypar::flow::function_node<int, int> function2(g, tbb_kahypar::flow::unlimited,
            [&](int input) noexcept {
                processed_items2.emplace_back(input);
                return 0;
            });

        tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<0>(split), function1);
        tbb_kahypar::flow::make_edge(tbb_kahypar::flow::output_port<1>(split), function2);
