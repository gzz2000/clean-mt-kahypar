...
std::size_t measure_median_start_time(tbb_kahypar::task_arena* ta, const F1& start = F1{}, const F2& end = F2{}) {
    std::size_t num_threads = ta ? ta->max_concurrency() : tbb_kahypar::this_task_arena::max_concurrency();
    ...
    auto measure_start_time = [&] {
        start_times[tbb_kahypar::this_task_arena::current_thread_index()] = std::chrono::steady_clock::now();
        barrier.wait();
    };
    ...
    auto work = [&] {
        ...
        for(std::size_t thr = 0; thr < num_threads-1; ++thr) {
            tbb_kahypar::this_task_arena::enqueue(measure_start_time);
        }
        start_times[tbb_kahypar::this_task_arena::current_thread_index()] = std::chrono::steady_clock::now();
        barrier.wait();
        end();
...
class start_time_collection_base {
    friend Impl;
public:
    start_time_collection_base(tbb_kahypar::task_arena& ta, std::size_t ntrials) :
    ...
    : public start_time_collection_base<start_time_collection_scoped_phase_wrapped>
{
    ...
    std::size_t measure_impl() {
        tbb_kahypar::task_arena::scoped_parallel_phase phase{*arena};
        auto median_start_time = measure_median_start_time(arena);
        return median_start_time;
    }
};
        ...
        if (arena) {
            ...
            median_start_time = measure_median_start_time(arena,
                [&] {
                    ...
                    arena->execute([&] {
                        for(std::size_t thr = 0; thr < num_threads-1; ++thr) {
                            tbb_kahypar::this_task_arena::enqueue(body);
                        }
                        barrier.wait();
                    });
            ...
            median_start_time = measure_median_start_time(arena,
                [&] {
                    std::size_t num_threads = tbb_kahypar::this_task_arena::max_concurrency();
                    tbb_kahypar::this_task_arena::start_parallel_phase();
                    for(std::size_t thr = 0; thr < num_threads-1; ++thr) {
                        tbb_kahypar::this_task_arena::enqueue(body);
                    }
                    barrier.wait();
                    tbb_kahypar::this_task_arena::end_parallel_phase(with_fast_leave); 
                }
            );
        }
        return median_start_time;
...
public:
    start_time_collection_sequenced_phases(tbb_kahypar::task_arena& ta, std::size_t ntrials, bool fast_leave = false) :
    ...
    std::size_t measure_impl() {
        ...
        auto median_start_time = measure_median_start_time(arena,
            [&] {
                std::size_t num_threads = arena->max_concurrency();
                {
                    tbb_kahypar::task_arena::scoped_parallel_phase phase{*arena, with_fast_leave};
                    arena->execute([&] {
                        for(std::size_t thr = 0; thr < num_threads-1; ++thr) {
                            tbb_kahypar::this_task_arena::enqueue(body);
                        }
...
public:
    start_time_collection_sequenced_scoped_phases(tbb_kahypar::task_arena& ta, std::size_t ntrials, bool fast_leave = false) :
        base(ta, ntrials), with_fast_leave(fast_leave)
    {}
...
TEST_CASE("Check that workers leave faster with leave_policy::fast") {
    ...
    tbb_kahypar::task_arena ta_automatic_leave {
        tbb_kahypar::task_arena::automatic, 1,
        tbb_kahypar::task_arena::priority::normal,
        tbb_kahypar::task_arena::leave_policy::automatic
    };
    tbb_kahypar::task_arena ta_fast_leave { 
        tbb_kahypar::task_arena::automatic, 1,
        tbb_kahypar::task_arena::priority::normal,
        tbb_kahypar::task_arena::leave_policy::fast
    };
...
TEST_CASE("Parallel Phase retains workers in task_arena") {
    ...
    tbb_kahypar::task_arena ta_fast1 {
        tbb_kahypar::task_arena::automatic, 1,
        tbb_kahypar::task_arena::priority::normal,
        tbb_kahypar::task_arena::leave_policy::fast
    };
    tbb_kahypar::task_arena ta_fast2 { 
        tbb_kahypar::task_arena::automatic, 1,
        tbb_kahypar::task_arena::priority::normal,
        tbb_kahypar::task_arena::leave_policy::fast
    };
...
TEST_CASE("Test one-time fast leave") {
    ...
    tbb_kahypar::task_arena ta1{};
    tbb_kahypar::task_arena ta2{};
