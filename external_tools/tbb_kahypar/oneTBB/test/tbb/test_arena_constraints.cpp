...
TEST_CASE("Test affinity and default_concurrency correctness for all available constraints.") {
    ...
    for (const auto& constraints: generate_constraints_variety()) {
        tbb_kahypar::task_arena ta{constraints};
        test_constraints_affinity_and_concurrency(constraints, get_arena_affinity(ta));
    }
}

bool is_observer_created(const tbb_kahypar::task_arena::constraints& c) {
    ...
    return
        (c.numa_id != tbb_kahypar::task_arena::automatic && numa_nodes.size() > 1) ||
        (c.core_type != tbb_kahypar::task_arena::automatic && core_types.size() > 1) ||
        c.max_threads_per_core != tbb_kahypar::task_arena::automatic;
}
...
void recursive_arena_binding(constraints_container::iterator current_pos, constraints_container::iterator end_pos) {
    ...
    if (current_pos != end_pos) {
        ...
        tbb_kahypar::task_arena current_level_arena{constraints};
...
TEST_CASE("Test constraints propagation during arenas copy construction") {
    system_info::initialize();
    for (const auto& constraints: generate_constraints_variety()) {
        tbb_kahypar::task_arena constructed{constraints};

        tbb_kahypar::task_arena copied(constructed);
...
TEST_CASE("Test memory leaks") {
    ...
    auto constraints = tbb_kahypar::task_arena::constraints{}
        .set_numa_id(tbb_kahypar::info::numa_nodes().front())
        .set_core_type(tbb_kahypar::info::core_types().front())
    ...
    for (size_t i = 0; i < num_trials; i++) {
        { /* All DTORs must be called before GetMemoryUsage() call*/
            tbb_kahypar::task_arena arena{constraints};
            arena.execute([]{
                utils::SpinBarrier barrier;
                barrier.initialize(tbb_kahypar::this_task_arena::max_concurrency());
                tbb_kahypar::parallel_for(
                    tbb_kahypar::blocked_range<size_t>(0, tbb_kahypar::this_task_arena::max_concurrency()),
                    [&barrier](const tbb_kahypar::blocked_range<size_t>&) {
                        barrier.wait();
                    }
                );
            });
...
TEST_CASE("Test arena constraints setters") {
    ...
    for(const auto& numa_index: tbb_kahypar::info::numa_nodes()) {
    ...
    for(const auto& core_type_index: tbb_kahypar::info::core_types()) {
...
void check_concurrency_level(const tbb_kahypar::task_arena::constraints& c) {
    REQUIRE_MESSAGE(tbb_kahypar::info::default_concurrency(c) == custom_concurrency_value,
        "Custom arena concurrency was passed to constraints, but was not respected by default_concurrency() call.");
    REQUIRE_MESSAGE(tbb_kahypar::task_arena{c}.max_concurrency() == custom_concurrency_value,
        "Custom arena concurrency was passed to constraints, but was not respected by default_concurrency() call.");
}
...
TEST_CASE("Test concurrency getters output for constraints with custom concurrency value") {
    tbb_kahypar::task_arena::constraints c{};
    c.set_max_concurrency(custom_concurrency_value);
    check_concurrency_level(c);

    c.set_numa_id(tbb_kahypar::info::numa_nodes().front());
    check_concurrency_level(c);

    c.set_core_type(tbb_kahypar::info::core_types().front());
    check_concurrency_level(c);
...
TEST_CASE("Testing constraints_threads_per_core() reserved entry point") {
    tbb_kahypar::task_arena::constraints c{};
    tbb_kahypar::detail::r1::constraints_threads_per_core(c);
}
