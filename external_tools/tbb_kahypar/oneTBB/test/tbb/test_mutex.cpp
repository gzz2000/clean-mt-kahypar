...
#include <oneapi_kahypar/tbb_kahypar/detail/_machine.h>
...
struct AddOne_CheckTransaction {
    ...
    void operator()(tbb_kahypar::blocked_range<size_t>& range) const {
        for (std::size_t i = range.begin(); i != range.end(); ++i) {
            ...
            tbb_kahypar::detail::machine_pause(static_cast<int32_t>(i));
        }
    }
...
void TestTransaction(const char* name)
{
    ...
    for (int i = 0; i < 5 && n_transactions_attempted.load(std::memory_order_relaxed) == 0; ++i) {
        counter.value = 0;
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(0, n, 2), AddOne_CheckTransaction<utils::Counter<M>>(counter));
...
TEST_CASE("Transaction test") {
    if (have_TSX()) {
        TestTransaction<tbb_kahypar::speculative_spin_mutex>("Speculative Spin Mutex");
        TestTransaction<tbb_kahypar::speculative_spin_rw_mutex>("Speculative Spin RW Mutex");
    }
}
...
TEST_CASE("test upgrade/downgrade with spin_rw_mutex") {
    test_rwm_upgrade_downgrade<tbb_kahypar::spin_rw_mutex>();
}

//! \brief \ref error_guessing
TEST_CASE("test upgrade/downgrade with queueing_rw_mutex") {
    test_rwm_upgrade_downgrade<tbb_kahypar::queuing_rw_mutex>();
}

//! \brief \ref error_guessing
TEST_CASE("test upgrade/downgrade with rw_mutex") {
    test_rwm_upgrade_downgrade<tbb_kahypar::rw_mutex>();
}

//! \brief \ref error_guessing
TEST_CASE("test upgrade/downgrade with speculative_spin_rw_mutex") {
    test_rwm_upgrade_downgrade<tbb_kahypar::speculative_spin_rw_mutex>();
}

//! \brief \ref error_guessing
TEST_CASE("test spin_mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::spin_mutex>();
}

#if !EMSCRIPTEN
//! \brief \ref error_guessing
TEST_CASE("test queuing_mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::queuing_mutex>();
}
#endif

//! \brief \ref error_guessing
TEST_CASE("test mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::mutex>();
}

//! \brief \ref error_guessing
TEST_CASE("test spin_rw_mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::spin_rw_mutex>();
    test_with_native_threads::test_rw<tbb_kahypar::spin_rw_mutex>();
}

#if !EMSCRIPTEN
//! \brief \ref error_guessing
TEST_CASE("test queuing_rw_mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::queuing_rw_mutex>();
    test_with_native_threads::test_rw<tbb_kahypar::queuing_rw_mutex>();
}
#endif

//! \brief \ref error_guessing
TEST_CASE("test rw_mutex with native threads") {
    test_with_native_threads::test<tbb_kahypar::rw_mutex>();
    test_with_native_threads::test_rw<tbb_kahypar::rw_mutex>();
}

//! Test scoped_lock::is_writer getter
//! \brief \ref error_guessing
TEST_CASE("scoped_lock::is_writer") {
    TestIsWriter<oneapi::tbb_kahypar::spin_rw_mutex>("spin_rw_mutex");
    TestIsWriter<oneapi::tbb_kahypar::queuing_rw_mutex>("queuing_rw_mutex");
    TestIsWriter<oneapi::tbb_kahypar::speculative_spin_rw_mutex>("speculative_spin_rw_mutex");
    TestIsWriter<oneapi::tbb_kahypar::null_rw_mutex>("null_rw_mutex");
    TestIsWriter<oneapi::tbb_kahypar::rw_mutex>("rw_mutex");
}

#if __TBB_CPP20_CONCEPTS_PRESENT
template <typename... Args>
concept mutexes = (... && tbb_kahypar::detail::scoped_lockable<Args>);

template <typename... Args>
concept rw_mutexes = (... && tbb_kahypar::detail::rw_scoped_lockable<Args>);

//! \brief \ref error_guessing
TEST_CASE("internal mutex concepts") {
    static_assert(mutexes<tbb_kahypar::spin_mutex, oneapi::tbb_kahypar::mutex, tbb_kahypar::speculative_spin_mutex, tbb_kahypar::null_mutex, tbb_kahypar::queuing_mutex,
                          tbb_kahypar::spin_rw_mutex, oneapi::tbb_kahypar::rw_mutex, tbb_kahypar::speculative_spin_rw_mutex, tbb_kahypar::null_rw_mutex, tbb_kahypar::queuing_rw_mutex>);
    static_assert(rw_mutexes<tbb_kahypar::spin_rw_mutex, oneapi::tbb_kahypar::rw_mutex, tbb_kahypar::speculative_spin_rw_mutex,
                             tbb_kahypar::null_rw_mutex, tbb_kahypar::queuing_rw_mutex>);
}
