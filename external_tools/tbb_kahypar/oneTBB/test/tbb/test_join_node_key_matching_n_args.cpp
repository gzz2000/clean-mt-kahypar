...
TEST_CASE_TEMPLATE_N_ARGS("Serial N tests on tuples") {
     generate_test<serial_test, T, tbb_kahypar::flow::key_matching<std::string&>>::do_test();
}

//! Parallel test with different tuple sizes
//! \brief \ref error_guessing
TEST_CASE_TEMPLATE_N_ARGS("Parallel N tests on tuples") {
    generate_test<parallel_test, T, tbb_kahypar::flow::key_matching<std::string&>>::do_test();
}
