...
template< typename T >
bool wait_try_get( tbb_kahypar::flow::graph &g, tbb_kahypar::flow::sequencer_node<T> &q, T &value ) {
    g.wait_for_all();
    return q.try_get(value);
}

template< typename T >
void spin_try_get( tbb_kahypar::flow::queue_node<T> &q, T &value ) {
...
struct parallel_puts : utils::NoAssign {
    ...
    tbb_kahypar::flow::sequencer_node<T> &my_q;
    int my_num_threads;

    parallel_puts( tbb_kahypar::flow::sequencer_node<T> &q, int num_threads ) : my_q(q), my_num_threads(num_threads) {}
...
struct parallel_gets : utils::NoAssign {
    ...
    parallel_gets( tbb_kahypar::flow::sequencer_node<T> &q, int num_threads, touches<T> &t ) : my_q(q), my_num_threads(num_threads), my_touches(t) {}
...
struct parallel_put_get : utils::NoAssign {
    ...
    tbb_kahypar::flow::sequencer_node<T> &my_s1;
    tbb_kahypar::flow::sequencer_node<T> &my_s2;
    int my_num_threads;
    std::atomic< int > &my_counter;
    touches<T> &my_touches;

    parallel_put_get( tbb_kahypar::flow::sequencer_node<T> &s1, tbb_kahypar::flow::sequencer_node<T> &s2, int num_threads,
...
int test_parallel(int num_threads) {
    ...
    tbb_kahypar::flow::sequencer_node<T> s(g, seq_inspector<T>());
    ...
    tbb_kahypar::flow::sequencer_node<T> s1(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s2(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s3(g, seq_inspector<T>());
    tbb_kahypar::flow::make_edge( s1, s2 );
    tbb_kahypar::flow::make_edge( s2, s3 );
    ...
    tbb_kahypar::flow::sequencer_node<T> s_copy(s);
...
int test_serial() {
    ...
    tbb_kahypar::flow::sequencer_node<T> s3(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s4(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s5(g, seq_inspector<T>());
    tbb_kahypar::flow::make_edge( s3, s4 );
    tbb_kahypar::flow::make_edge( s4, s5 );
    ...
    g.wait_for_all();
    tbb_kahypar::flow::remove_edge( s3, s4 );
    ...
    tbb_kahypar::flow::sequencer_node<T> s6(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s7(g, seq_inspector<T>());
    tbb_kahypar::flow::sequencer_node<T> s8(g, seq_inspector<T>());
    tbb_kahypar::flow::make_edge( s6, s7 );
    tbb_kahypar::flow::make_edge( s7, s8 );
    ...
    g.wait_for_all();
    tbb_kahypar::flow::remove_edge( s6, s7 );
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows
        <int, tbb_kahypar::flow::sequencer_node<int>>
...
void test_seq_node_try_put_and_wait() {
    ...
    // test_buffer_push returns the index from which the items processed during wait_for_all() starts
    {
        ...
        std::size_t after_start = test_buffer_push<tbb_kahypar::flow::sequencer_node<int>>(start_work_items, wait_message,
...

    {
        ...
        std::size_t after_start = test_buffer_pull<tbb_kahypar::flow::sequencer_node<int>>(start_work_items, wait_message, occupier,
                                                                                   new_work_items, processed_items,
                                                                                   simple_sequencer);
    ...
    // Test reserve
    {
        ...
        for (int threshold : thresholds) {
            ...
            std::size_t after_start = test_buffer_reserve<tbb_kahypar::flow::sequencer_node<int>>(threshold,
...
TEST_CASE("Serial and parallel test"){
    for (int p = 2; p <= 4; ++p) {
        tbb_kahypar::global_control thread_limit(tbb_kahypar::global_control::max_allowed_parallelism, p);
...
TEST_CASE("constraints for sequencer_node object") {
    ...
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::sequencer_node, Object>);
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::sequencer_node, int>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::sequencer_node, test_concepts::NonCopyable>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::sequencer_node, test_concepts::NonCopyAssignable>);
}

template <typename T, typename Sequencer>
concept can_call_sequencer_node_ctor = requires( tbb_kahypar::flow::graph& graph, Sequencer seq,
                                                 tbb_kahypar::flow::buffer_node<int>& f ) {
    tbb_kahypar::flow::sequencer_node<T>(graph, seq);
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
    tbb_kahypar::flow::sequencer_node<T>(tbb_kahypar::flow::follows(f), seq);
#endif
};
