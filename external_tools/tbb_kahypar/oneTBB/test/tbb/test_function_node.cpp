...
void buffered_levels( size_t concurrency, Body body ) {
   ...
   tbb_kahypar::flow::function_node< InputType, OutputType > exe_node( g, lc, body );
   tbb_kahypar::flow::function_node<InputType, InputType> pass_thru( g, tbb_kahypar::flow::unlimited, pass_through<InputType>());

   // Create a vector of identical exe_nodes and pass_thrus
   std::vector< tbb_kahypar::flow::function_node< InputType, OutputType > > exe_vec(2, exe_node);
   std::vector< tbb_kahypar::flow::function_node< InputType, InputType > > pass_thru_vec(2, pass_thru);
   // Attach each pass_thru to its corresponding exe_node
   for (size_t node_idx=0; node_idx<exe_vec.size(); ++node_idx) {
       tbb_kahypar::flow::make_edge(pass_thru_vec[node_idx], exe_vec[node_idx]);
   }
   ...
   // For num_receivers = 1 to MAX_NODES
       for (size_t num_receivers = 1; num_receivers <= MAX_NODES; ++num_receivers ) {
           ...
           for (size_t r = 0; r < num_receivers; ++r ) {
               tbb_kahypar::flow::make_edge( exe_vec[node_idx], *receivers[r] );
           }
           ...
           for (size_t r = 0; r < num_receivers; ++r ) {
               tbb_kahypar::flow::remove_edge( exe_vec[node_idx], *receivers[r] );
           }
...
void buffered_levels_with_copy( size_t concurrency ) {
    ...
    for ( size_t lc = 1; lc <= concurrency; ++lc ) {
        ...
        tbb_kahypar::flow::function_node< InputType, OutputType > exe_node( g, lc, cf );
        ...
        for (size_t num_receivers = 1; num_receivers <= MAX_NODES; ++num_receivers ) {
            ...
            for (size_t r = 0; r < num_receivers; ++r ) {
                tbb_kahypar::flow::make_edge( exe_node, *receivers[r] );
            }
            ...
            for (size_t num_senders = 1; num_senders <= MAX_NODES; ++num_senders ) {
                ...
                for (size_t s = 0; s < num_senders; ++s ) {
                    senders.push_back( std::make_shared<harness_counting_sender<InputType>>() );
                    senders.back()->my_limit = N;
                    tbb_kahypar::flow::make_edge( *senders.back(), exe_node );
                }
            ...
            for (size_t r = 0; r < num_receivers; ++r ) {
                tbb_kahypar::flow::remove_edge( exe_node, *receivers[r] );
            }
        ...
        inc_functor body_copy = tbb_kahypar::flow::copy_body<inc_functor>( exe_node );
        ...
        g.reset(tbb_kahypar::flow::rf_reset_bodies);
        body_copy = tbb_kahypar::flow::copy_body<inc_functor>( exe_node );
...
void concurrency_levels( size_t concurrency, Body body ) {
    ...
    for ( size_t lc = 1; lc <= concurrency; ++lc ) {
        ...
        typedef tbb_kahypar::flow::function_node< InputType, OutputType, tbb_kahypar::flow::rejecting > fnode_type;
        ...
        for (size_t num_receivers = 1; num_receivers <= MAX_NODES; ++num_receivers ) {
            ...
            for (size_t num_senders = 1; num_senders <= MAX_NODES; ++num_senders ) {
                senders.clear();
                {
                    ...
                    tbb_kahypar::spin_rw_mutex::scoped_lock l(
                        harness_graph_executor<InputType, OutputType>::template mutex_holder<tbb_kahypar::spin_rw_mutex>::mutex
                    );
...
void run_concurrency_levels( int c ) {
    concurrency_levels<InputType,OutputType>( c, []( InputType i ) -> OutputType { return harness_graph_executor<InputType, OutputType>::template tfunc<tbb_kahypar::spin_rw_mutex>(i); } );
    concurrency_levels<InputType,OutputType>( c, &harness_graph_executor<InputType, OutputType>::template tfunc<tbb_kahypar::spin_rw_mutex> );
    concurrency_levels<InputType,OutputType>( c, typename harness_graph_executor<InputType, OutputType>::template tfunctor<tbb_kahypar::spin_rw_mutex>() );
}
...
struct parallel_puts : private utils::NoAssign {
    ...
    tbb_kahypar::flow::receiver< InputType > * const my_exe_node;

    parallel_puts( tbb_kahypar::flow::receiver< InputType > &exe_node ) : my_exe_node(&exe_node) {}
...
void unlimited_concurrency( Body body ) {
    ...
    for (unsigned p = 1; p < 2*utils::MaxThread; ++p) {
        tbb_kahypar::flow::graph g;
        tbb_kahypar::flow::function_node< InputType, OutputType, tbb_kahypar::flow::rejecting > exe_node( g, tbb_kahypar::flow::unlimited, body );
...
struct continue_msg_to_int {
    ...
    int operator()(tbb_kahypar::flow::continue_msg) { return my_int; }
};
...
void test_function_node_with_continue_msg_as_input() {
    ...
    tbb_kahypar::flow::function_node<tbb_kahypar::flow::continue_msg, int, tbb_kahypar::flow::rejecting> FN1( g, tbb_kahypar::flow::serial, continue_msg_to_int(42));
    tbb_kahypar::flow::function_node<tbb_kahypar::flow::continue_msg, int, tbb_kahypar::flow::rejecting> FN2( g, tbb_kahypar::flow::serial, continue_msg_to_int(43));

    tbb_kahypar::flow::make_edge( Start, FN1 );
    tbb_kahypar::flow::make_edge( Start, FN2 );
...
void test_concurrency(int num_threads) {
    tbb_kahypar::global_control thread_limit(tbb_kahypar::global_control::max_allowed_parallelism, num_threads);
    run_concurrency_levels<int,int>(num_threads);
    run_concurrency_levels<int,tbb_kahypar::flow::continue_msg>(num_threads);
    ...
    run_unlimited_concurrency<int,tbb_kahypar::flow::continue_msg>();
    run_unlimited_concurrency<empty_no_assign,tbb_kahypar::flow::continue_msg>();
...
void test_follows_and_precedes_api() {
    using msg_t = tbb_kahypar::flow::continue_msg;
    ...
    follows_and_precedes_testing::test_follows
        <msg_t, tbb_kahypar::flow::function_node<msg_t, msg_t>>
        (messages_for_follows, tbb_kahypar::flow::unlimited, pass_msg);
    follows_and_precedes_testing::test_precedes
        <msg_t, tbb_kahypar::flow::function_node<msg_t, msg_t>>
        (messages_for_precedes, tbb_kahypar::flow::unlimited, pass_msg, tbb_kahypar::flow::node_priority_t(1));
}
#endif
...
void test_try_put_and_wait_lightweight(std::size_t concurrency_limit) {
    ...
    arena.execute([&]{
        ...
        using function_node_type = tbb_kahypar::flow::function_node<int, int, tbb_kahypar::flow::lightweight>;
        ...
        tbb_kahypar::flow::make_edge(function, writer);
        ...
        if (concurrency_limit == tbb_kahypar::flow::serial) {
...
void test_try_put_and_wait_queueing(std::size_t concurrency_limit) {
    ...
    arena.execute([&]{
        ...
        using function_node_type = tbb_kahypar::flow::function_node<int, int, tbb_kahypar::flow::queueing>;
        function_node_type* start_node = nullptr;
...
void test_try_put_and_wait() {
    test_try_put_and_wait_lightweight(tbb_kahypar::flow::serial);
    test_try_put_and_wait_lightweight(tbb_kahypar::flow::unlimited);

    test_try_put_and_wait_queueing(tbb_kahypar::flow::serial);
    test_try_put_and_wait_queueing(tbb_kahypar::flow::unlimited);

    test_try_put_and_wait_rejecting(tbb_kahypar::flow::serial);
}
...
TEST_CASE("Lightweight testing"){
   lightweight_testing::test<tbb_kahypar::flow::function_node>(10);
}
...
TEST_CASE("try_release try_consume"){
    ...
    tbb_kahypar::flow::function_node<int, int> fn(g, tbb_kahypar::flow::unlimited, [](const int&v){return v;});
...
TEST_CASE("constraints for function_node input and output") {
    ...
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::function_node, InputObject, OutputObject>);
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::function_node, int, int>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::function_node, test_concepts::NonCopyable, OutputObject>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::function_node, test_concepts::NonDefaultInitializable, OutputObject>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::function_node, InputObject, test_concepts::NonCopyable>);
}

template <typename Input, typename Output, typename Body>
concept can_call_function_node_ctor = requires( tbb_kahypar::flow::graph& graph, std::size_t concurrency, Body body,
                                                tbb_kahypar::flow::node_priority_t priority, tbb_kahypar::flow::buffer_node<int>& f ) {
    tbb_kahypar::flow::function_node<Input, Output>(graph, concurrency, body);
    tbb_kahypar::flow::function_node<Input, Output>(graph, concurrency, body, priority);
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
    tbb_kahypar::flow::function_node<Input, Output>(tbb_kahypar::flow::follows(f), concurrency, body);
    tbb_kahypar::flow::function_node<Input, Output>(tbb_kahypar::flow::follows(f), concurrency, body, priority);
#endif
};
...
TEST_CASE("test critical tasks memory pool correctness") {
    using node_type = tbb_kahypar::flow::function_node<int, tbb_kahypar::flow::continue_msg>;
    constexpr int num_iterations = 10000;
    int num_calls = 0;
    auto node_body = [&](int) { ++num_calls; };

    tbb_kahypar::flow::graph g;
    node_type node(g, tbb_kahypar::flow::serial, node_body, tbb_kahypar::flow::node_priority_t{1});
