...
template< typename T >
class test_push_receiver : public tbb_kahypar::flow::receiver<T>, utils::NoAssign {
...
public:
    ...
    test_push_receiver(tbb_kahypar::flow::graph& g) : my_graph(g) {
        for (int i = 0; i < N; ++i )
            my_counters[i] = 0;
    }
    ...
    typedef typename tbb_kahypar::flow::receiver<T>::predecessor_type predecessor_type;
...
public:
    ...
    T operator()( tbb_kahypar::flow_control& fc ) {
        T v = (T)my_count++;
...
void test_single_dest() {
    ...
    tbb_kahypar::flow::input_node<T> src(g, my_input_body<T>() );
    test_push_receiver<T> dest(g);
    tbb_kahypar::flow::make_edge( src, dest );
    src.activate();
    g.wait_for_all();
    for (int i = 0; i < N; ++i ) {
    ...
    tbb_kahypar::flow::input_node<T> src3(g, my_input_body<T>() );
    src3.activate();

    function_body<T> b3( counters3 );
    tbb_kahypar::flow::function_node<T,bool> dest3(g, tbb_kahypar::flow::unlimited, b3 );
    tbb_kahypar::flow::make_edge( src3, dest3 );
    g.wait_for_all();
    for (int i = 0; i < N; ++i ) {
        int v = counters3[i];
        CHECK_MESSAGE( v == 1, "" );
    ...
    // push & pull
    tbb_kahypar::flow::input_node<T> src2(g, my_input_body<T>() );
    src2.activate();
    std::atomic<int> counters2[N];

    function_body<T> b2( counters2 );
    tbb_kahypar::flow::function_node<T,bool,tbb_kahypar::flow::rejecting> dest2(g, tbb_kahypar::flow::serial, b2 );
    tbb_kahypar::flow::make_edge( src2, dest2 );
    g.wait_for_all();
    for (int i = 0; i < N; ++i ) {
        int v = counters2[i];
        CHECK_MESSAGE( v == 1, "" );
    ...
    tbb_kahypar::flow::input_node<T> src_copy(src);
    src_copy.activate();
...
void test_reset() {
    ...
    tbb_kahypar::flow::graph g;
    std::atomic<int> counters3[N];
    tbb_kahypar::flow::input_node<int> src3(g, my_input_body<int>());
    src3.activate();
    tbb_kahypar::flow::input_node<int> src_inactive(g, my_input_body<int>());
    function_body<int> b3( counters3 );
    tbb_kahypar::flow::function_node<int,bool> dest3(g, tbb_kahypar::flow::unlimited, b3);
    tbb_kahypar::flow::make_edge( src3, dest3 );
    ...
    g.reset(tbb_kahypar::flow::rf_reset_bodies);  // <-- re-initializes the counts.
    ...
    g.reset(tbb_kahypar::flow::rf_reset_bodies);  // <-- reinitializes the counts
    // src_inactive doesn't run
    g.wait_for_all();
    for (int i = 0; i < N; ++i ) {
        int v = counters3[i];
...
void test_follows_and_precedes_api() {
    ...
    input_node<bool> src(
        ...
        [&](tbb_kahypar::flow_control& fc) -> bool {
            if(!do_try_put)
                fc.stop();
...
TEST_CASE("try_get before activation"){
    tbb_kahypar::flow::graph g;
    tbb_kahypar::flow::input_node<int> in(g, [&](tbb_kahypar::flow_control& fc) { fc.stop(); return 0;});
...
TEST_CASE("constraints for input_node output") {
    ...
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::input_node, Object>);
    static_assert(utils::well_formed_instantiation<tbb_kahypar::flow::input_node, int>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::input_node, test_concepts::NonCopyable>);
    static_assert(!utils::well_formed_instantiation<tbb_kahypar::flow::input_node, test_concepts::NonCopyAssignable>);
}

template <typename Output, typename Body>
concept can_call_input_node_ctor = requires( tbb_kahypar::flow::graph& graph, Body body, tbb_kahypar::flow::buffer_node<int> f ) {
    tbb_kahypar::flow::input_node<Output>(graph, body);
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
    tbb_kahypar::flow::input_node<Output>(tbb_kahypar::flow::precedes(f), body);
#endif
};
