...
class Random {
    ...
    tbb_kahypar::concurrent_vector<State*> mStateList;
public:
    ~Random() {
...
private:
    ...
    tbb_kahypar::concurrent_vector<StatType*> mStatsList;
...
class ArenaTable {
    ...
    using ArenaPtrRWMutex = PtrRWMutex<tbb_kahypar::task_arena, arenaAligment>;
...
public:
    ...
    void create(Random& rnd) {
        ...
        if (guard.continue_execution()) {
            ...
            tbb_kahypar::task_arena::priority priorities[] = { tbb_kahypar::task_arena::priority::low , tbb_kahypar::task_arena::priority::normal, tbb_kahypar::task_arena::priority::high };
            tbb_kahypar::task_arena::priority priority = priorities[rnd.get() % 3];

            tbb_kahypar::task_arena* a = new (aligned_malloc(arenaAligment, arenaAligment)) tbb_kahypar::task_arena{ num_threads , num_reserved , priority };
    ...
    std::pair<tbb_kahypar::task_arena*, std::size_t> acquire(Random& rnd, ScopedLock& lock) {
        auto guard = mLifetimeTracker.makeGuard();

        tbb_kahypar::task_arena* a{nullptr};
        std::size_t resIdx{};
        if (guard.continue_execution()) {
            auto& ts = mThreadState;
            a = find_arena(rnd.get() % maxArenas,
                [&ts, &lock, &resIdx](ArenaPtrRWMutex& arena, std::size_t idx) -> tbb_kahypar::task_arena* {
                    if (!ts.lockedArenas[idx]) {
...
struct actor<arena_action> {
    static void do_it(Random& r, size_t arenaAfterStealing) {
        ...
        if (entry.first) {
            ...
            auto body = [process] {
                if (process) {
                    tbb_kahypar::detail::d1::wait_context wctx{ 1 };
                    tbb_kahypar::task_group_context ctx;
                    tbb_kahypar::this_task_arena::enqueue([&wctx] { wctx.release(); });
                    tbb_kahypar::detail::d1::wait(wctx, ctx);
                } else {
                    global_actor(0);
                }
            };
...
struct actor<parallel_algorithm> {
    static void do_it(Random& rnd) {
        ...
        case simpl_part:
            tbb_kahypar::parallel_for(0, sz, body, tbb_kahypar::simple_partitioner{}); break;
        case auto_part:
            tbb_kahypar::parallel_for(0, sz, body, tbb_kahypar::auto_partitioner{}); break;
        case aff_part:
        {
            tbb_kahypar::affinity_partitioner aff;
            tbb_kahypar::parallel_for(0, sz, body, aff); break;
        }
        case static_part:
            tbb_kahypar::parallel_for(0, sz, body, tbb_kahypar::static_partitioner{}); break;
        }
    }
};
