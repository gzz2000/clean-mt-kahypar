...
void TestSort() {
    for( int n=0; n<100; n=n*3+1 ) {
        tbb_kahypar::concurrent_vector<int> array(n);
        for( int i=0; i<n; ++i ){
            array.at(i) = (i*7)%n;
        }
...
void TestRangeBasedFor(){
    ...
    using c_vector = tbb_kahypar::concurrent_vector<int>;
    c_vector a_c_vector;
...
struct c_vector_type : default_container_traits {
    ...
    using container_type = tbb_kahypar::concurrent_vector<T, Allocator>;
    ...
    struct apply{
        using type = tbb_kahypar::concurrent_vector<element_type,  allocator_type >;
    };
    ...
    static bool equal(tbb_kahypar::concurrent_vector<element_type, allocator_type > const& c, iterator begin, iterator end){
...
void TestVectorTypes() {
    tbb_kahypar::concurrent_vector<ClassWithVectorType> v;
    for( int i = 0; i < 100; ++i ) {
...
static tbb_kahypar::concurrent_vector<std::size_t> Primes;
...
double TimeFindPrimes( std::size_t nthread ) {
    ...
    tbb_kahypar::tick_count t0 = tbb_kahypar::tick_count::now();
    std::size_t block_size = count / nthread;
    utils::NativeParallelFor(count, block_size, FindPrimes() );
    tbb_kahypar::tick_count t1 = tbb_kahypar::tick_count::now();
...
void TestFindPrimes() {
    ...
    // TimeFindPrimes( tbb_kahypar::task_scheduler_init::automatic );
...
class test_grow_by_and_resize {
    tbb_kahypar::concurrent_vector<Type, Allocator> &my_c;
public:
    test_grow_by_and_resize( tbb_kahypar::concurrent_vector<Type, Allocator> &c ) : my_c(c) {}
    void operator()() const {
        const typename tbb_kahypar::concurrent_vector<Type, Allocator>::size_type sz = my_c.size();
        my_c.grow_by( 5 );
...
void test_scoped_allocator() {
    ...
    using container_type = tbb_kahypar::concurrent_vector<allocator_data_type, allocator_type>;
...
template <typename Type, typename Allocator>
void CompareVectors( const tbb_kahypar::concurrent_vector<Type, Allocator> &c1, const tbb_kahypar::concurrent_vector<Type, Allocator> &c2 ) {
    REQUIRE( (!(c1 == c2) && c1 != c2) );
    REQUIRE( (c1 <= c2 && c1 < c2 && c2 >= c1 && c2 > c1) );
}

template <typename Type, typename Allocator>
void CompareVectors( const tbb_kahypar::concurrent_vector<std::weak_ptr<Type>, Allocator> &, const tbb_kahypar::concurrent_vector<std::weak_ptr<Type>, Allocator> & ) {
    /* do nothing for std::weak_ptr */
}

template <bool default_construction_present, typename Type, typename Allocator>
void Examine( tbb_kahypar::concurrent_vector<Type, Allocator> c, const std::vector<Type> &vec ) {
    using vector_t = tbb_kahypar::concurrent_vector<Type, Allocator>;
...
public:
    ...
    void operator()() const {
        ...
        tbb_kahypar::concurrent_vector<Type> c7( my_vec.size() );
        std::copy( my_vec.begin(), my_vec.end(), c7.begin() );
        Examine</*default_construction_present = */true>( c7, my_vec );
        tbb_kahypar::concurrent_vector< Type, std::allocator<Type> > c8( my_vec.size() );
...
void TypeTester( const std::vector<Type> &vec ) {
    ...
    tbb_kahypar::concurrent_vector< Type, std::allocator<Type> > c9(vec.size(), vec[1], allocator);
    Examine<default_construction_present>( c9, std::vector<Type>(vec.size(), vec[1]) );
    // Construction with copying iteration range and given allocator instance.
    tbb_kahypar::concurrent_vector< Type, std::allocator<Type> > c10(c1.begin(), c1.end(), allocator);
    Examine<default_construction_present>( c10, vec );
    tbb_kahypar::concurrent_vector<Type> c11(vec.begin(), vec.end());
...
void TestSerialGrowByRange( bool fragmented_vector ) {
    tbb_kahypar::concurrent_vector<int> v;
...
void TestConcurrentOperationsWithUnSafeOperations(std::size_t threads_number) {
    using vector_type = tbb_kahypar::concurrent_vector<move_support_tests::Foo, allocator_type>;
...
int reduce_vector(RangeType test_range) {
    return tbb_kahypar::parallel_reduce(test_range, 0,
...
TEST_CASE("Test exception in constructors") {
    ...
    using vector_type = tbb_kahypar::concurrent_vector<double, allocator_type>;
...
TEST_CASE("Reducing concurrent_vector") {
    ...
    tbb_kahypar::concurrent_vector<int> vec(final_sum, 1);
    const tbb_kahypar::concurrent_vector<int> cvec(vec);
...
TEST_CASE("swap with not always equal allocators"){
    ...
    using vector_type = tbb_kahypar::concurrent_vector<int, allocator_type>;
...
TEST_CASE("Testing vector in a highly concurrent environment") {
    ...
    for (int i = 0; i < num_repeats; ++i) {
        ...
        tbb_kahypar::concurrent_vector<int> test_vec;
        tbb_kahypar::parallel_for(0, num_inserts, [&] (int j) {
            tbb_kahypar::concurrent_vector<int>::iterator start_it = test_vec.grow_by(grow_by_vals[j]);
            tbb_kahypar::concurrent_vector<int>::iterator end_it = start_it + grow_by_vals[j];
            do {
...
TEST_CASE("container_range concept for concurrent_vector ranges") {
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_vector<int>::range_type>);
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_vector<int>::const_range_type>);
}
