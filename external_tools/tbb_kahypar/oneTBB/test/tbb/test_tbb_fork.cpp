...
typedef tbb_kahypar::concurrent_hash_map<DWORD, Data> TidTableType;
...
class AllocTask {
public:
    void operator() (const tbb_kahypar::blocked_range<int> &r) const {
#if _WIN32||_WIN64
        ...
        for (int y = r.begin(); y != r.end(); ++y) {
            void *p = tbb_kahypar::detail::r1::cache_aligned_allocate(7000);
            tbb_kahypar::detail::r1::cache_aligned_deallocate(p);
        }
    }
    AllocTask() {}
};
...
{
    tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<int>(0, 10000, 1), AllocTask(),
...
public:
    ...
    void operator()(const int /*threadID*/) const {
        tbb_kahypar::task_scheduler_handle tsi{tbb_kahypar::attach{}};
        CallParallelFor();
        if (wait_workers) {
            bool ok = tbb_kahypar::finalize(tsi, std::nothrow);
            ASSERT(ok, nullptr);
        } else {
            tsi.release();
        }
...
public:
    ...
    void operator()(const int /*threadID*/) const {
        tbb_kahypar::task_scheduler_handle tsi = tbb_kahypar::task_scheduler_handle{tbb_kahypar::attach{}};
        CallParallelFor();
        if (blocking) {
            bool ok = tbb_kahypar::finalize(tsi, std::nothrow);
...
{
    tbb_kahypar::task_scheduler_handle sch{tbb_kahypar::attach{}};
    CallParallelFor();
    utils::NativeParallelFor(2, RunInNativeThread(/*blocking=*/false));
    bool ok = tbb_kahypar::finalize(sch, std::nothrow);
    ASSERT(ok, nullptr);
}
...
#endif
        for (int i=0; i<100; i++) {
            tbb_kahypar::task_arena arena(1, 1); arena.initialize(); // right approach?
            // tbb_kahypar::task_scheduler_init sch(1);
            tbb_kahypar::task_scheduler_handle sch = tbb_kahypar::task_scheduler_handle::get();
            for (int k=0; k<10; k++) {
                // tbb_kahypar::empty_task *t = new( tbb_kahypar::task::allocate_root() ) tbb_kahypar::empty_task();
                // tbb_kahypar::task::enqueue(*t);
                arena.enqueue([&]{});
            }
            bool ok = tbb_kahypar::finalize(sch, std::nothrow);
            ASSERT(ok, nullptr);
...
void TestNestingTSI()
{
    ...
    for (int i=0; i<2; i++) {
        tbb_kahypar::task_scheduler_handle schBlock = tbb_kahypar::task_scheduler_handle{tbb_kahypar::attach{}};
        CallParallelFor();
        tbb_kahypar::task_scheduler_handle schBlock1;
        schBlock1 = tbb_kahypar::task_scheduler_handle{tbb_kahypar::attach{}};
        CallParallelFor();
        if (i) {
            schBlock1.release();
        } else {
            bool ok = tbb_kahypar::finalize(schBlock1, std::nothrow);
            ASSERT(!ok, "Nested blocking terminate must fail.");
        }
        bool ok = tbb_kahypar::finalize(schBlock, std::nothrow);
        ASSERT(ok, nullptr);
    }
    {
        tbb_kahypar::task_scheduler_handle schBlock{tbb_kahypar::attach{}};
...
int main()
{
    ...
    for (int threads=MinThread; threads<=MaxThread; threads+=MinThread) {
        for (int i=0; i<20; i++) {
            tbb_kahypar::global_control ctl(tbb_kahypar::global_control::max_allowed_parallelism, threads);
            {
                tbb_kahypar::task_scheduler_handle sch{tbb_kahypar::attach{}};
                bool ok = tbb_kahypar::finalize( sch, std::nothrow );
                ASSERT(ok, nullptr);
