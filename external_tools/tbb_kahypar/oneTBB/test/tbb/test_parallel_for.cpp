...
struct SSE_Functor {
    ...
    void operator()( tbb_kahypar::blocked_range<int>& r ) const {
...
void TestVectorTypes() {
    ...
    tbb_kahypar::parallel_for( tbb_kahypar::blocked_range<int>(0,aSize), SSE_Functor<ClassWithVectorType>(Array1, Array2) );
    for( int i=0; i<aSize; ++i ) {
        ClassWithVectorType foo(i);
        CHECK( Array2[i]==foo ) ;
    }
...
struct TestSimplePartitionerStabilityFunctor {
  ...
  void operator()(tbb_kahypar::blocked_range<size_t>& r)const{
      ranges.at(r.begin()) = 1;
  }
};
void TestSimplePartitionerStability(){
    ...
    for (std::size_t i=0 , grainsize=grainsizeStep; i<repeat_count;i++, grainsize+=grainsizeStep){
        ...
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<size_t>(0,rangeToSplitSize,grainsize),FunctorType(firstSeries),tbb_kahypar::simple_partitioner());
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<size_t>(0,rangeToSplitSize,grainsize),FunctorType(secondSeries),tbb_kahypar::simple_partitioner());

        CHECK_MESSAGE(
            firstSeries == secondSeries,
            "Splitting range with tbb_kahypar::simple_partitioner must be reproducible; i = " << i
        );
    }
}
...
public:
    ...
    Body(Body& b, tbb_kahypar::split) : m_sb(b.m_sb) { }
...
void test() {
    ...
    tbb_kahypar::parallel_for( range, SimpleBody(), ap );
}
...
void test() {
    ...
    tbb_kahypar::affinity_partitioner ap;
    tbb_kahypar::parallel_for( range, sync_body, ap );
    tbb_kahypar::parallel_for( range, sync_body, tbb_kahypar::static_partitioner() );
}
...
void test() {
    ...
    // tbb_kahypar::blocked_range implementation
...
struct FunctorToCancel {
    ...
    void operator()( const tbb_kahypar::blocked_range<std::size_t>& ) const {
        ++g_CurExecuted;
...
class ParallelForRunner {
    ...
    void run_parallel_for( Args&&... args ) const {
        Partitioner part;
        tbb_kahypar::parallel_for(std::forward<Args>(args)..., part, my_ctx);
    }
    ...
    void run_overload( Args&&... args ) const {
        ...
        switch(Mode % 5) {
            case 0 : {
                tbb_kahypar::parallel_for(std::forward<Args>(args)..., my_ctx);
                break;
            }
            case 1 : {
                run_parallel_for<tbb_kahypar::simple_partitioner>(std::forward<Args>(args)...);
                break;
            }
            case 2 : {
                run_parallel_for<tbb_kahypar::auto_partitioner>(std::forward<Args>(args)...);
                break;
            }
            case 3 : {
                run_parallel_for<tbb_kahypar::static_partitioner>(std::forward<Args>(args)...);
                break;
            }
            case 4 : {
                run_parallel_for<tbb_kahypar::affinity_partitioner>(std::forward<Args>(args)...);
                break;
            }
        }
    }
...
public:
    ParallelForRunner( tbb_kahypar::task_group_context& ctx )
        : my_ctx(ctx) {}
...
concept can_call_parallel_for_basic = requires( Args&&... args ) {
    tbb_kahypar::parallel_for(std::forward<Args>(args)...);
};

template <typename... Args>
concept can_call_parallel_for_helper = can_call_parallel_for_basic<Args...> &&
                                       can_call_parallel_for_basic<Args..., tbb_kahypar::task_group_context&>;

template <typename... Args>
concept can_call_parallel_for_with_partitioner = can_call_parallel_for_helper<Args...> &&
                                                 can_call_parallel_for_helper<Args..., const tbb_kahypar::simple_partitioner&> &&
                                                 can_call_parallel_for_helper<Args..., const tbb_kahypar::auto_partitioner&> &&
                                                 can_call_parallel_for_helper<Args..., const tbb_kahypar::static_partitioner> &&
                                                 can_call_parallel_for_helper<Args..., tbb_kahypar::affinity_partitioner&>;
...
TEST_CASE("Exceptions support") {
    for ( int p = MinThread; p <= MaxThread; ++p ) {
        if ( p > 0 ) {
            tbb_kahypar::global_control control(tbb_kahypar::global_control::max_allowed_parallelism, p);
...
TEST_CASE("That all workers sleep when no work") {
    ...
    tbb_kahypar::parallel_for(std::size_t(0), N, [&](std::size_t) {
...
TEST_CASE("Cancellation test for tbb_kahypar::parallel_for") {
