...
std::vector<tbb_kahypar::task_arena::priority> g_task_info;
...
using arena_info =
    std::tuple<std::unique_ptr<tbb_kahypar::task_arena>,
               concurrency_type,
               tbb_kahypar::task_arena::priority,
               std::unique_ptr<tbb_kahypar::task_group>>;

enum arena_info_keys {
    arena_pointer, arena_concurrency, arena_priority, associated_task_group
};

void prepare_logging_data(std::vector<tbb_kahypar::task_arena::priority>& task_log, unsigned overall_tasks_num) {
    task_log.clear();
    task_log.resize(overall_tasks_num);
    for( auto& record : task_log )
        record = tbb_kahypar::task_arena::priority::normal;
}

template<typename... ArenaArgs>
tbb_kahypar::task_arena* do_allocate_and_construct( const ArenaArgs&... arena_args )
{
    ...
    tbb_kahypar::task_arena* result_arena = nullptr;

    switch( initialization_method ) {

    case lazy:
        result_arena = new tbb_kahypar::task_arena( arena_args... );
        break;

    case explicit_initialize:
        result_arena = new tbb_kahypar::task_arena;
        result_arena->initialize( arena_args... );
        break;

    case explicit_initialize_with_different_constructor_parameters:
    {
        tbb_kahypar::task_arena tmp(dummy_max_concurrency, dummy_reserved_for_masters);
        result_arena = new tbb_kahypar::task_arena(tmp);
        result_arena->initialize(arena_args...);
        break;
    }
...
tbb_kahypar::task_arena* decide_on_arguments(
    const FirstArenaArg& first_arg, const int reserved_for_masters,
    tbb_kahypar::task_arena::priority a_priority )
{
    const tbb_kahypar::task_arena::priority default_priority = tbb_kahypar::task_arena::priority::normal;
...
tbb_kahypar::task_arena* allocate_and_construct_arena(
    int arena_max_concurrency, tbb_kahypar::task_arena::priority a_priority )
{
    ...
    if( use_constraints ) {
        tbb_kahypar::task_arena::constraints properties{tbb_kahypar::task_arena::automatic, arena_max_concurrency};
...
void submit_work( std::vector<arena_info>& arenas, unsigned repeats, utils::SpinBarrier& barrier ) {
    for( auto& item : arenas ) {
        tbb_kahypar::task_arena& arena = *std::get<arena_pointer>(item).get();
        concurrency_type concurrency = std::get<arena_concurrency>(item);
        tbb_kahypar::task_arena::priority priority_value = std::get<arena_priority>(item);
...
void test() {
    ...
    tbb_kahypar::global_control control(tbb_kahypar::global_control::max_allowed_parallelism, max_num_threads + 1);
    ...
    tbb_kahypar::task_arena::priority high   = tbb_kahypar::task_arena::priority::high;
    tbb_kahypar::task_arena::priority normal = tbb_kahypar::task_arena::priority::normal;
    tbb_kahypar::task_arena::priority low    = tbb_kahypar::task_arena::priority::low;
    ...
    tbb_kahypar::task_arena::priority priorities[] = {high, normal, low}; // keep it sorted
    ...
    for( const auto& progressing_arenas_num : progressing_arenas ) {
        ...
        for( unsigned arena_idx = 0; arena_idx < overall_arenas_num; ++arena_idx ) {
            tbb_kahypar::task_arena::priority a_priority = priorities[arena_idx];
            ...
            if( !actual_concurrency ) {
                concurrency = tbb_kahypar::task_arena::automatic;
            ...
            tbb_kahypar::task_arena* arena = allocate_and_construct_arena(concurrency, a_priority);
            arenas.push_back(
                std::make_tuple(
                    std::unique_ptr<tbb_kahypar::task_arena>(arena),
                    actual_concurrency,
                    a_priority,
                    std::unique_ptr<tbb_kahypar::task_group>(new tbb_kahypar::task_group)
                )
            );
        }
        ...
        std::map<tbb_kahypar::task_arena::priority, unsigned> wasted_tasks;

        tbb_kahypar::task_arena::priority* end_ptr = priorities + adjusted_progressing_arenas;

        {
            // First epoch - check progressing arenas only
            unsigned overall_progressing_arenas_tasks_num = 0;
            std::map<tbb_kahypar::task_arena::priority, unsigned> per_priority_tasks_num;
            ...
            for( unsigned i = 0; i < last_task_idx; ++i ) {
                tbb_kahypar::task_arena::priority p = g_task_info[i];
            ...
            for( unsigned i = 0; i < adjusted_progressing_arenas; ++i ) {
                tbb_kahypar::task_arena::priority p = priorities[i];
