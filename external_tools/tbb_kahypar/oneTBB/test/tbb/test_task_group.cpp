...
void LaunchChildrenWithFunctor () {
    ...
    tbb_kahypar::task_group_status status = tbb_kahypar::not_complete;
    ...
    if (g_Throw && !exceptionCaught && status != tbb_kahypar::canceled) {
...
void TestManualCancellationWithFunctor () {
    ...
    CHECK_MESSAGE ( !tbb_kahypar::is_current_task_group_canceling(), "Unexpected cancellation" );
    while ( g_MaxConcurrency > 1 && g_TaskCount == 0 )
        utils::yield();
    tg.cancel();
    g_ExecutedAtCancellation = int(g_TaskCount);
    tbb_kahypar::task_group_status status = tg.wait();
    CHECK_MESSAGE( status == tbb_kahypar::canceled, "Task group reported invalid status." );
...
void TestMissingWait () {
    ...
    catch ( const tbb_kahypar::missing_wait& e ) {
...
namespace TestIsolationNS {
    ...
    class ParForBody {
        ...
        tbb_kahypar::enumerable_thread_specific<int>& m_ets;
    public:
        ParForBody(
            task_group_type& tg,
    ...
    void CheckIsolation(bool isolation_is_expected) {
        ...
        tbb_kahypar::parallel_for(0, 100, ParForBody<task_group_type>(tg, isolation_is_preserved, ets));
    ...
    void TestIsolation() {
        CheckIsolation<tbb_kahypar::task_group>(false);
        CheckIsolation<tbb_kahypar::isolated_task_group>(true);
    }
}
#endif
...
TEST_CASE("Thread safety test for the task group") {
    ...
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::global_control limit(tbb_kahypar::global_control::max_allowed_parallelism, p);
        g_MaxConcurrency = p;
        TestThreadSafety<tbb_kahypar::task_group>();
    }
}
#endif
...
TEST_CASE("Fibonacci test for the task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        tbb_kahypar::global_control limit(tbb_kahypar::global_control::max_allowed_parallelism, p);
        tbb_kahypar::task_arena a(p);
        g_MaxConcurrency = p;
        a.execute([] {
            RunFibonacciTests<tbb_kahypar::task_group>();
        });
    }
}
...
TEST_CASE("Cancellation and exception test for the task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        a.execute([] {
            RunCancellationAndExceptionHandlingTests<tbb_kahypar::task_group>();
        });
    }
}
...
TEST_CASE("Constant functor test for the task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        tbb_kahypar::global_control limit(tbb_kahypar::global_control::max_allowed_parallelism, p);
        g_MaxConcurrency = p;
        TestConstantFunctorRequirement<tbb_kahypar::task_group>();
    }
}
...
TEST_CASE("Move semantics test for the task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        tbb_kahypar::global_control limit(tbb_kahypar::global_control::max_allowed_parallelism, p);
        g_MaxConcurrency = p;
        TestMoveSemantics<tbb_kahypar::task_group>();
    }
}
...
TEST_CASE("Thread safety test for the isolated task group") {
    ...
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::task_arena a(p);
        a.execute([] {
            TestThreadSafety<tbb_kahypar::isolated_task_group>();
        });
    }
}
#endif
...
TEST_CASE("Fibonacci test for the isolated task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::task_arena a(p);
        a.execute([] {
            RunFibonacciTests<tbb_kahypar::isolated_task_group>();
        });
    }
}
...
TEST_CASE("Cancellation and exception test for the isolated task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::task_arena a(p);
        a.execute([] {
            RunCancellationAndExceptionHandlingTests<tbb_kahypar::isolated_task_group>();
        });
    }
}
...
TEST_CASE("Constant functor test for the isolated task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::task_arena a(p);
        a.execute([] {
            TestConstantFunctorRequirement<tbb_kahypar::isolated_task_group>();
        });
    }
}
...
TEST_CASE("Move semantics test for the isolated task group") {
    for (unsigned p=MinThread; p <= MaxThread; ++p) {
        ...
        tbb_kahypar::task_arena a(p);
        a.execute([] {
            TestMoveSemantics<tbb_kahypar::isolated_task_group>();
        });
    }
}
...
void run_deep_stealing(tbb_kahypar::task_group& tg1, tbb_kahypar::task_group& tg2, int num_tasks, std::atomic<int>& tasks_executed) {
...
TEST_CASE("Test for stack overflow avoidance mechanism") {
    ...
    tbb_kahypar::global_control thread_limit(tbb_kahypar::global_control::max_allowed_parallelism, 2);
    tbb_kahypar::task_group tg1;
    tbb_kahypar::task_group tg2;
...
TEST_CASE("Async task group") {
    int num_threads = tbb_kahypar::this_task_arena::max_concurrency();
    if (num_threads < 3) {
        // The test requires at least 2 worker threads
        return;
    }
    tbb_kahypar::task_arena a(2*num_threads, num_threads);
    utils::SpinBarrier barrier(num_threads + 2);
    tbb_kahypar::task_group tg[2];
...
void run_cancellation_use_case(CancelF&& cancel, WaitF&& wait) {
    ...
    tbb_kahypar::task_group_context inner_ctx(tbb_kahypar::task_group_context::isolated);
    TaskGroup inner_tg(inner_ctx);

    tbb_kahypar::task_group outer_tg;
    auto outer_tg_task = [&] {
        inner_tg.run([&] {
    ...
    auto check = [&] {
        tbb_kahypar::task_group_status outer_status = tbb_kahypar::task_group_status::not_complete;
        outer_status = std::forward<WaitF>(wait)(outer_tg);
        CHECK_MESSAGE(
            outer_status == tbb_kahypar::task_group_status::canceled,
            "Outer task group should have been cancelled."
        );

        tbb_kahypar::task_group_status inner_status = inner_tg.wait();
        CHECK_MESSAGE(
            inner_status == tbb_kahypar::task_group_status::complete,
...
void test() {
    run_cancellation_use_case<TaskGroup>(
        [](tbb_kahypar::task_group& outer) { outer.cancel(); },
        [](tbb_kahypar::task_group& outer) { return outer.wait(); }
    );

#if TBB_USE_EXCEPTIONS
    run_cancellation_use_case<TaskGroup>(
        [](tbb_kahypar::task_group& /*outer*/) {
            volatile bool suppress_unreachable_code_warning = true;
            if (suppress_unreachable_code_warning) {
                throw int();
            }
        ...
        [](tbb_kahypar::task_group& outer) {
            try {
                outer.wait();
                return tbb_kahypar::task_group_status::complete;
            } catch(const int&) {
                return tbb_kahypar::task_group_status::canceled;
            }
        }
    );
#endif
...
#if TBB_PREVIEW_ISOLATED_TASK_GROUP
    accept_task_group_context::test<tbb_kahypar::isolated_task_group>();
#endif
}
...
TEST_CASE("Empty task_handle cannot be scheduled"
    ...
    CHECK_THROWS_WITH_AS(tg.run(tbb_kahypar::task_handle{}), "Attempt to schedule empty task_handle", std::runtime_error);
}
...
)
{
    ...
    tbb_kahypar::task_group tg(ctx);
    tbb_kahypar::task_group tg1(ctx);
