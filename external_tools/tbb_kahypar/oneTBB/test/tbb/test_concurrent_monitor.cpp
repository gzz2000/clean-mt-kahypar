...
TEST_CASE("Stress test") {
    ...
    tbb_kahypar::task_scheduler_handle handler{tbb_kahypar::attach{}};

    utils::SpinBarrier barrier(threads_number);

    tbb_kahypar::detail::r1::concurrent_monitor test_monitor;
    {
        tbb_kahypar::task_arena arena(static_cast<int>(threads_number - 1), 0);
        ...
        auto thread_func = [&, operation_number] {
            for (std::size_t i = 0; i < operation_number; ++i) {
                tbb_kahypar::detail::r1::concurrent_monitor::thread_context context{std::uintptr_t(1)};
