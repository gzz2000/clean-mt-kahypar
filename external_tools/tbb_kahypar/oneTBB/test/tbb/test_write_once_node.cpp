...
void simple_read_write_tests() {
    ...
    tbb_kahypar::flow::write_once_node<R> n(g);
...
class native_body : utils::NoAssign {
    tbb_kahypar::flow::write_once_node<R> &my_node;

public:

    native_body( tbb_kahypar::flow::write_once_node<R> &n ) : my_node(n) {}
...
void parallel_read_write_tests() {
    ...
    std::vector< tbb_kahypar::flow::write_once_node<R> > wo_vec(2, n);
    ...
    for (size_t node_idx=0; node_idx<wo_vec.size(); ++node_idx) {
        for ( int t = 0; t < T; ++t ) {
            ...
            for (int i = 0; i < M; ++i) {
                tbb_kahypar::flow::make_edge( wo_vec[node_idx], *r[i] );
            }
            R v0;
            ...
            for (int i = 0; i < M; ++i) {
                tbb_kahypar::flow::remove_edge( wo_vec[node_idx], *r[i] );
            }
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows<msg_t, tbb_kahypar::flow::write_once_node<msg_t>>(messages_for_follows);
    follows_and_precedes_testing::test_precedes<msg_t, tbb_kahypar::flow::write_once_node<msg_t>>(messages_for_precedes);
}
...
void test_try_put_and_wait() {
    ...
    // Test push
    {
        ...
        arena.execute([&] {
            ...
            tbb_kahypar::flow::write_once_node<int> wo_buffer(g);
            tbb_kahypar::flow::function_node<int, int> function(g, tbb_kahypar::flow::serial,
                [&](int input) {
            ...
            tbb_kahypar::flow::make_edge(wo_buffer, function);
    ...
    // Test reserve
    {
        ...
        arena.execute([&] {
            ...
            tbb_kahypar::flow::make_edge(wo_buffer, limiter);
    ...
    for( unsigned int p=utils::MinThread; p<=utils::MaxThread; ++p ) {
        ...
        arena.execute(
            [&]() {
                ...
                test_reserving_nodes<tbb_kahypar::flow::write_once_node, size_t>();
            }
        );
	}
}
...
TEST_CASE("Nested make_edge write_once_node to continue_node") {
    using msg_t = tbb_kahypar::flow::continue_msg;
    using buffer_t = tbb_kahypar::flow::write_once_node<msg_t>;
