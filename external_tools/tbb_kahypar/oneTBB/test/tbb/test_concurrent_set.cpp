...
struct AllowMultimapping<tbb_kahypar::concurrent_multiset<Args...>> : std::true_type {};
...
using set_type = tbb_kahypar::concurrent_set<int, std::less<int>, MyAllocator<int>>;
using multiset_type = tbb_kahypar::concurrent_multiset<int, std::less<int>, MyAllocator<int>>;
using checked_set_type = tbb_kahypar::concurrent_set<CheckType<int>, std::less<CheckType<int>>, MyAllocator<CheckType<int>>>;
using checked_multiset_type = tbb_kahypar::concurrent_multiset<CheckType<int>, std::less<CheckType<int>>, MyAllocator<CheckType<int>>>;
using greater_set_type = tbb_kahypar::concurrent_set<int, std::greater<int>, MyAllocator<int>>;
using greater_multiset_type = tbb_kahypar::concurrent_multiset<int, std::greater<int>, MyAllocator<int>>;
using checked_state_set_type = tbb_kahypar::concurrent_set<FooWithAssign, std::less<FooWithAssign>, MyAllocator<FooWithAssign>>;
using checked_state_multiset_type = tbb_kahypar::concurrent_multiset<FooWithAssign, std::less<FooWithAssign>, MyAllocator<FooWithAssign>>;

struct COSetTraits : OrderedMoveTraitsBase {
    template <typename T, typename Allocator>
    using container_type = tbb_kahypar::concurrent_set<T, std::less<T>, Allocator>;
...
struct COMultisetTraits : OrderedMoveTraitsBase {
    template <typename T, typename Allocator>
    using container_type = tbb_kahypar::concurrent_multiset<T, std::less<T>, Allocator>;
...
struct OrderedSetTypesTester {
    ...
    void check( const std::list<ValueType>& lst ) {
        TypeTester<DefCtorPresent, tbb_kahypar::concurrent_set<ValueType>>(lst);
        TypeTester<DefCtorPresent, tbb_kahypar::concurrent_multiset<ValueType>>(lst);
    }
...
void test_specific_types() {
    ...
    test_emplace_insert<tbb_kahypar::concurrent_set<test::unique_ptr<int>>, std::false_type>
                       (new int, new int);
    test_emplace_insert<tbb_kahypar::concurrent_multiset<test::unique_ptr<int>>, std::false_type>
                       (new int, new int);
}
...
void test_cycles_absense() {
    for (std::size_t execution = 0; execution != 10; ++execution) {
        tbb_kahypar::concurrent_multiset<int> mset;
        std::vector<int> v(2);
...
TEST_CASE("concurrent_set::swap with not always equal allocator") {
    using not_always_equal_alloc_set_type = tbb_kahypar::concurrent_set<int, std::less<int>, NotAlwaysEqualAllocator<int>>;
...
TEST_CASE("concurrent_multiset::swap with not always equal allocator") {
    using not_always_equal_alloc_mset_type = tbb_kahypar::concurrent_multiset<int, std::less<int>, NotAlwaysEqualAllocator<int>>;
...
TEST_CASE("concurrent_set throwing copy constructor") {
    using exception_set_type = tbb_kahypar::concurrent_set<ThrowOnCopy>;
    test_exception_on_copy_ctor<exception_set_type>();
}

//! \brief \ref error_guessing
TEST_CASE("concurrent_multiset throwing copy constructor") {
    using exception_mset_type = tbb_kahypar::concurrent_multiset<ThrowOnCopy>;
...
TEST_CASE("container_range concept for concurrent_set ranges") {
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_set<int>::range_type>);
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_set<int>::const_range_type>);
}

//! \brief \ref error_guessing
TEST_CASE("container range concept for concurrent_multiset ranges") {
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_multiset<int>::range_type>);
    static_assert(test_concepts::container_range<typename tbb_kahypar::concurrent_multiset<int>::const_range_type>);
}
