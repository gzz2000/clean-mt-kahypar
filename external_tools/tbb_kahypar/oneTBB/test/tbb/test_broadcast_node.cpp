...
using tbb_kahypar::TBB_INTERNAL_NAMESPACE::SUCCESSFULLY_ENQUEUED;
...
template< typename T >
class counting_array_receiver : public tbb_kahypar::flow::receiver<T> {
...
public:
    ...
    counting_array_receiver(tbb_kahypar::flow::graph& g) : my_graph(g) {
        for (int i = 0; i < N; ++i )
           my_counters[i] = 0;
    }
    ...
    tbb_kahypar::task * try_put_task( const T &v ) override {
        ++my_counters[(int)v];
        return const_cast<tbb_kahypar::task *>(SUCCESSFULLY_ENQUEUED);
    }

#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
    tbb_kahypar::task * try_put_task( const T &v, const tbb_kahypar::detail::d2::message_metainfo& ) override {
...
void test_serial_broadcasts() {
    ...
    tbb_kahypar::flow::broadcast_node<T> b(g);
    ...
    for ( int num_receivers = 1; num_receivers < R; ++num_receivers ) {
        ...
        for ( int r = 0; r < num_receivers; ++r ) {
            tbb_kahypar::flow::make_edge( b, *receivers[r] );
        }
        ...
        for ( int r = 0; r < num_receivers; ++r ) {
            ...
            tbb_kahypar::flow::remove_edge( b, *receivers[r] );
        }
...
class native_body : private utils::NoAssign {
    ...
    tbb_kahypar::flow::broadcast_node<T> &my_b;

public:

    native_body( tbb_kahypar::flow::broadcast_node<T> &b ) : my_b(b) {}
...
template< typename T >
void run_parallel_broadcasts(tbb_kahypar::flow::graph& g, int p, tbb_kahypar::flow::broadcast_node<T>& b) {
    for ( int num_receivers = 1; num_receivers < R; ++num_receivers ) {
        std::vector< std::shared_ptr<counting_array_receiver<T>> > receivers;
        for( int i = 0; i < num_receivers; ++i )
            receivers.push_back( std::make_shared< counting_array_receiver<T> >(g) );
...
void test_parallel_broadcasts(int p) {
    ...
    tbb_kahypar::flow::broadcast_node<T> b_copy(b);
...
void test_resets() {
    tbb_kahypar::flow::graph g;
    tbb_kahypar::flow::broadcast_node<T> b0(g);
    tbb_kahypar::flow::broadcast_node<T> b1(g);
    tbb_kahypar::flow::queue_node<T> q0(g);
    tbb_kahypar::flow::make_edge(b0,b1);
    tbb_kahypar::flow::make_edge(b1,q0);
    T j;
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows <msg_t, tbb_kahypar::flow::broadcast_node<msg_t>>(messages_for_follows);
    follows_and_precedes_testing::test_precedes <msg_t, tbb_kahypar::flow::broadcast_node<msg_t>>(messages_for_precedes);
}
#endif
...
void test_try_put_and_wait_spawning_and_serial_receiver() {
    ...
    arena.execute([&]{
        ...
        tbb_kahypar::flow::broadcast_node<int> broadcast(g);

        // Broadcast to 2 function_nodes, one with unlimited concurrency and the other serial
        tbb_kahypar::flow::function_node<int, int, tbb_kahypar::flow::queueing> unlimited(g, tbb_kahypar::flow::unlimited,
            [&](int input) noexcept {
        ...
        tbb_kahypar::flow::make_edge(broadcast, unlimited);

        tbb_kahypar::flow::function_node<int, int, tbb_kahypar::flow::queueing> serial(g, tbb_kahypar::flow::serial,
            [&](int input) noexcept {
                processed_items_serial.emplace_back(input);
                return 0;
            });
        tbb_kahypar::flow::make_edge(broadcast, serial);
...
void test_try_put_and_wait_spawning_receivers() {
    ...
    arena.execute([&]{
        ...
        std::vector<tbb_kahypar::flow::function_node<int, int, tbb_kahypar::flow::queueing>> successors;
        successors.reserve(num_successors);
        for (int i = 0; i < num_successors; ++i) {
            successors.emplace_back(g, tbb_kahypar::flow::unlimited,
            ...
            tbb_kahypar::flow::make_edge(broadcast, successors.back());
        }
