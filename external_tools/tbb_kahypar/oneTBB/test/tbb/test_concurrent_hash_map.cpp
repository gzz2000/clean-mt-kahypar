...
void TestRangeBasedFor(){
    ...
    using ch_map = tbb_kahypar::concurrent_hash_map<int,int>;
    ch_map a_ch_map;
...
void Examine( Table c, const std::list<typename Table::value_type> &lst) {
    ...
    tbb_kahypar::parallel_for( c.range(), test_range<Table,typename range_type::iterator,range_type>( c, lst, marks ) );
    CHECK(std::find( marks.begin(), marks.end(), false ) == marks.end());
...
template <typename T>
struct debug_hash_compare : public tbb_kahypar::detail::d1::tbb_hash_compare<T> {};
...
void TypeTester( const std::list<Value> &lst ) {
    ...
    using ch_map = tbb_kahypar::concurrent_hash_map<key_type, second_type>;
    ...
    using ch_map_debug_alloc = tbb_kahypar::concurrent_hash_map<key_type, second_type,
                                                        tbb_kahypar::detail::d1::tbb_hash_compare<key_type>,
    ...
    using ch_map_debug_hash = tbb_kahypar::concurrent_hash_map<key_type, second_type,
...
void TestSpecificTypes() {
    ...
    using tbb_string_key_type = std::basic_string<char, std::char_traits<char>, tbb_kahypar::tbb_allocator<char>>;
...
private:
    tbb_kahypar::tbb_hash_compare<int> my_hash_compare;
};

void TestScopedAllocator() {
    using allocator_data_type = AllocatorAwareData<std::scoped_allocator_adaptor<tbb_kahypar::tbb_allocator<int>>>;
    using allocator_type = std::scoped_allocator_adaptor<tbb_kahypar::tbb_allocator<std::pair<const allocator_data_type, allocator_data_type>>>;
    using hash_map_type = tbb_kahypar::concurrent_hash_map<allocator_data_type, allocator_data_type,
...
void TestInternalFastFind() {
    typedef tbb_kahypar::concurrent_hash_map<int, int> basic_chmap_type;
...
struct hash_map_traits : default_container_traits {
    ...
    using container_type = tbb_kahypar::concurrent_hash_map<T, T, hash_compare<T>, Allocator>;
    ...
    struct apply {
        using type = tbb_kahypar::concurrent_hash_map<element_type, element_type, hash_compare<element_type>, allocator_type>;
    };
...
void test_with_minimalistic_mutex() {
    ...
    using chmap_type = tbb_kahypar::concurrent_hash_map<int, int, tbb_kahypar::tbb_hash_compare<int>,
                                                tbb_kahypar::tbb_allocator<std::pair<const int, int>>,
                                                mutex_type>;
    ...
    auto thread_body = [&]( const tbb_kahypar::blocked_range<std::size_t>& range ) {
    ...
    tbb_kahypar::blocked_range<std::size_t> br(0, 1000, 8);

    tbb_kahypar::parallel_for(br, thread_body);
...
TEST_CASE("Test exception in constructors") {
    ...
    using map_type = tbb_kahypar::concurrent_hash_map<int, int, tbb_kahypar::tbb_hash_compare<int>, allocator_type>;
    ...
    tbb_kahypar::tbb_hash_compare<int> test_hash;
...
TEST_CASE("test concurrent_hash_map accessors issue") {
    using map_type = tbb_kahypar::concurrent_hash_map<int, int, SimpleTransparentHashCompare>;
...
template <bool ExpectSatisfies, typename Key, typename Mapped, typename... HCTypes>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::concurrent_hash_map, Key, Mapped, HCTypes> == ExpectSatisfies))
void test_chmap_hash_compare_constraints() {}

//! \brief \ref error_guessing
TEST_CASE("tbb_kahypar::concurrent_hash_map hash_compare constraints") {
    using key_type = int;
    ...
    test_chmap_hash_compare_constraints</*Expected = */true, /*key = */key_type, /*mapped = */mapped_type,
                                        Correct<key_type>, tbb_kahypar::tbb_hash_compare<key_type>>();
...
template <bool ExpectSatisfies, typename Key, typename Mapped, typename... RWMutexes>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::concurrent_hash_map, Key, Mapped,
                                                tbb_kahypar::tbb_hash_compare<Key>, tbb_kahypar::tbb_allocator<std::pair<const Key, Mapped>>, RWMutexes> == ExpectSatisfies))
void test_chmap_mutex_constraints() {}

//! \brief \ref error_guessing
TEST_CASE("tbb_kahypar::concurrent_hash_map rw_mutex constraints") {
    using key_type = int;
    using mapped_type = int;
...
//! \brief \ref error_guessing
TEST_CASE("container_range concept for tbb_kahypar::concurrent_hash_map ranges") {
    static_assert(test_concepts::container_range<tbb_kahypar::concurrent_hash_map<int, int>::range_type>);
    static_assert(test_concepts::container_range<tbb_kahypar::concurrent_hash_map<int, int>::const_range_type>);
}
