...
using Range = tbb_kahypar::blocked_range<long>;
...
public:
    ...
    Accumulator( Accumulator& a, tbb_kahypar::split ) :
...
T ParallelScanFunctionalInvoker(const Range& range, T idx, const Scan& scan, const ReverseJoin& reverse_join, int mode) {
    switch (mode%3) {
    case 0:
        return tbb_kahypar::parallel_scan(range, idx, scan, reverse_join);
        break;
    case 1:
        return tbb_kahypar::parallel_scan(range, idx, scan, reverse_join, tbb_kahypar::simple_partitioner());
        break;
    default:
        return tbb_kahypar::parallel_scan(range, idx, scan, reverse_join, tbb_kahypar::auto_partitioner());
    }
}
...
void TestAccumulator( int mode ) {
    ...
    for( int n=0; n<=MAXN; n = n <=128? n+1: n*3) {
        for( int gs : {1, 2, 100, 511, 12345, n/ 111, n/17, n-1, n}) {
            ...
            switch (mode) {
                case 0:
                    tbb_kahypar::parallel_scan( Range( 0, n,  gs ), acc );
                break;
                case 1:
                    tbb_kahypar::parallel_scan( Range( 0, n, gs ), acc, tbb_kahypar::simple_partitioner() );
                break;
                case 2:
                    tbb_kahypar::parallel_scan( Range( 0, n, gs ), acc, tbb_kahypar::auto_partitioner() );
                break;
            }
...
concept can_call_parallel_scan_basic = requires( Args&&... args ) {
    tbb_kahypar::parallel_scan(std::forward<Args>(args)...);
};

template <typename Range, typename Body>
concept can_call_imperative_pscan = can_call_parallel_scan_basic<const Range&, Body&> &&
                                    can_call_parallel_scan_basic<const Range&, Body&, const tbb_kahypar::simple_partitioner&> &&
                                    can_call_parallel_scan_basic<const Range&, Body&, const tbb_kahypar::auto_partitioner&>;
...
concept can_call_functional_pscan = can_call_parallel_scan_basic<const Range&, const Value&, const Func&, const Combine&> &&
                                    can_call_parallel_scan_basic<const Range&, const Value&, const Func&, const Combine&, const tbb_kahypar::simple_partitioner&> &&
                                    can_call_parallel_scan_basic<const Range&, const Value&, const Func&, const Combine&, const tbb_kahypar::auto_partitioner&>;
