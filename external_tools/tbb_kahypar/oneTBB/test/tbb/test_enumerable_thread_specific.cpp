...
template<size_t N = tbb_kahypar::detail::max_nfs_size>
...
T& check_alignment(T& t, const char *aname) {
    if( !tbb_kahypar::detail::is_aligned(&t, AlignMask)) {
...
const size_t line_size = tbb_kahypar::detail::max_nfs_size;

typedef tbb_kahypar::enumerable_thread_specific<minimalN<line_size> > flogged_ets;
...
void flog_segmented_interator() {
    ...
    tbb_kahypar::detail::d1::segmented_iterator<nested_vec, T> my_si(my_vec);
    ...
    tbb_kahypar::detail::d1::segmented_iterator<nested_vec, const T> my_csi(my_vec);
...
void flog_segmented_iterator_map() {
   ...
   tbb_kahypar::detail::d1::segmented_iterator<nested_vec, std::pair<const Key, Val> > my_si(my_vec);
   Key ii;
   for(my_si=my_vec.begin(), ii=0; my_si != my_vec.end(); ++my_si, ++ii) {
   ...
   tbb_kahypar::detail::d1::segmented_iterator<nested_vec, const std::pair<const Key, Val> > my_csi(my_vec);
   for(my_csi=my_vec.begin(), ii=0; my_csi != my_vec.end(); ++my_csi, ++ii) {
...
using tbb_kahypar::detail::d1::ets_element;
...
void allocate_ets_element_on_stack(const char* /* name */) {
    ...
    tbb_kahypar::detail::suppress_unused_warning(c1,c2);
    REQUIRE_MESSAGE(tbb_kahypar::detail::is_aligned(my_stack_element.value(), my_align), "Error in first stack alignment" );
    REQUIRE_MESSAGE(tbb_kahypar::detail::is_aligned(my_stack_element2.value(), my_align), "Error in second stack alignment" );
    REQUIRE_MESSAGE(tbb_kahypar::detail::is_aligned(mystruct1.my_struct_element.value(), my_align), "Error in struct element alignment" );
}
...
void TestConstructorWithBigType(const char* allocator_name) {
    typedef tbb_kahypar::enumerable_thread_specific<BigType, Allocator<BigType> > CounterBigType;
...
size_t init_tbb_alloc_mask() {
    ...
    if(tbb_kahypar::tbb_allocator<int>::allocator_type() == tbb_kahypar::tbb_allocator<int>::standard) {
        // scalable allocator is not available.
        // INFO("tbb_kahypar::tbb_allocator is not available\n");
        return 1;
    }
    else {
...
static const size_t cache_allocator_mask = tbb_kahypar::detail::r1::cache_line_size();
...
TEST_CASE("Constructor with big type") {
    ...
    TestConstructorWithBigType<tbb_kahypar::cache_aligned_allocator>("tbb_kahypar::cache_aligned_allocator");
    AlignMask = tbb_allocator_mask;
    TestConstructorWithBigType<tbb_kahypar::tbb_allocator>("tbb_kahypar::tbb_allocator");
}
