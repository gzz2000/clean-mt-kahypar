...
public:
    ...
    void submit(tbb_kahypar::task::suspend_point ctx) {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_tagQueue.push(ctx);
...
private:
    static void asyncLoop(AsyncActivity* async) {
        tbb_kahypar::task::suspend_point tag;
        for (;;) {
            {
    ...
    std::queue<tbb_kahypar::task::suspend_point> m_tagQueue;
...
struct SuspendBody {
    ...
    void operator()(tbb_kahypar::task::suspend_point tag) {
...
public:
    ...
    void operator()() {
        ...
        tbb_kahypar::parallel_for(0, N, inner_outer_body );
    }
...
private:
    struct InnermostInnerParFor {
        ...
        void operator()(int) const {
            tbb_kahypar::task::suspend(SuspendBody(m_asyncActivity, std::this_thread::get_id()));
        }
        AsyncActivity& m_asyncActivity;
    };
    struct InnermostOuterParFor {
        ...
        void operator()(int) const {
            ...
            tbb_kahypar::parallel_for(0, N, inner_inner_body);
        }
        AsyncActivity& m_asyncActivity;
    };
    AsyncActivity& m_asyncActivity;
...
public:
    OutermostArenaBody(AsyncActivity& a_, tbb_kahypar::task_arena& o_, tbb_kahypar::task_arena& i_
            , tbb_kahypar::task_arena& id_, tbb_kahypar::enumerable_thread_specific<int>& ets) :
        m_asyncActivity(a_), m_outermostArena(o_), m_innermostArena(i_), m_innermostArenaDefault(id_), m_local(ets) {}

    void operator()() {
        tbb_kahypar::parallel_for(0, 32, *this);
    }

    void operator()(int i) const {
        tbb_kahypar::task::suspend([&] (tbb_kahypar::task::suspend_point sp) { m_asyncActivity.submit(sp); });

        tbb_kahypar::task_arena& nested_arena = (i % 3 == 0) ?
...
private:
    AsyncActivity& m_asyncActivity;
    tbb_kahypar::task_arena& m_outermostArena;
    tbb_kahypar::task_arena& m_innermostArena;
    tbb_kahypar::task_arena& m_innermostArenaDefault;
    tbb_kahypar::enumerable_thread_specific<int>& m_local;
};

void TestNestedArena() {
    AsyncActivity asyncActivity(4);

    tbb_kahypar::task_arena outermost_arena;
    tbb_kahypar::task_arena innermost_arena(2,2);
    tbb_kahypar::task_arena innermost_arena_default;
...
struct EpochSuspendBody {
    ...
    void operator()(tbb_kahypar::task::suspend_point ctx) {
...
void TestSuspendResume() {
    ...
    tbb_kahypar::enumerable_thread_specific<int, tbb_kahypar::cache_aligned_allocator<int>, tbb_kahypar::ets_suspend_aware> ets_fiber;
    std::atomic<int> inner_par_iters, outer_par_iters;
    inner_par_iters = outer_par_iters = 0;

    tbb_kahypar::parallel_for(0, N, [&](int) {
        for (int i = 0; i < iter_size; ++i) {
            ets_fiber.local() = i;

            int local_epoch;
            tbb_kahypar::task::suspend(EpochSuspendBody(async, global_epoch, local_epoch));
            CHECK(local_epoch < global_epoch);
            CHECK(ets_fiber.local() == i);

            tbb_kahypar::parallel_for(0, N, [&](int) {
                int local_epoch2;
                tbb_kahypar::task::suspend(EpochSuspendBody(async, global_epoch, local_epoch2));
...
void TestCleanupMaster() {
    if (tbb_kahypar::this_task_arena::max_concurrency() == 1) {
        ...
        utils::NativeParallelFor(N, [&asyncActivity, &tg, &iter_spawned, &iter_executed](int j) {
            for (int k = 0; k < j*10 + 1; ++k) {
                tg.run([&asyncActivity, j, &iter_executed] {
                    ...
                    tbb_kahypar::task::suspend(SuspendBody(asyncActivity, std::this_thread::get_id()));
                    iter_executed++;
                });
                iter_spawned++;
            }
...
class ObserverTracker : public tbb_kahypar::task_scheduler_observer {
    static thread_local bool is_in_arena;
public:
    std::atomic<int> counter;

    ObserverTracker(tbb_kahypar::task_arena& a) : tbb_kahypar::task_scheduler_observer(a) {
        counter = 0;
        observe(true);
    }
...
void TestObservers() {
    ...
    do {
        arena.execute([] {
            tbb_kahypar::parallel_for(0, 10, [](int) {
                auto thread_id = std::this_thread::get_id();
                tbb_kahypar::task::suspend([thread_id](tbb_kahypar::task::suspend_point tag) {
                    CHECK(thread_id == std::this_thread::get_id());
                    tbb_kahypar::task::resume(tag);
                });
...
class TestCaseGuard {
    ...
    tbb_kahypar::global_control m_threadLimit;
    tbb_kahypar::global_control m_stackLimit;
public:
    TestCaseGuard()
        : m_threadLimit(tbb_kahypar::global_control::max_allowed_parallelism, std::max(tbb_kahypar::this_task_arena::max_concurrency(), 16))
        , m_stackLimit(tbb_kahypar::global_control::thread_stack_size, 128*1024)
    {
        CHECK(m_local == false);
        m_local = true;
    }
