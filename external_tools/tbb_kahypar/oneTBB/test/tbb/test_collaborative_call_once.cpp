...
#include "tbb_kahypar/collaborative_call_once.h"
...
void call_once_in_for_loop(std::size_t N, Fn&& body, Args&&... args) {
    tbb_kahypar::collaborative_once_flag flag;
    for (std::size_t i = 0; i < N; ++i) {
        tbb_kahypar::collaborative_call_once(flag, std::forward<Fn>(body), std::forward<Args>(args)...);
    }
}
...
void call_once_in_parallel_for(std::size_t N, Fn&& body, Args&&... args) {
    ...
    auto stored_pack = tbb_kahypar::detail::d0::save_pack(std::forward<Args>(args)...);
    auto func = [&] { tbb_kahypar::detail::d0::call(std::forward<Fn>(body), std::move(stored_pack)); };
#endif // __TBB_GCC_PARAMETER_PACK_IN_LAMBDAS_BROKEN

    tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<size_t>(0, N), [&](const tbb_kahypar::blocked_range<size_t>& range) {
        for (size_t i = range.begin(); i != range.end(); ++i) {
#if __TBB_GCC_PARAMETER_PACK_IN_LAMBDAS_BROKEN
            tbb_kahypar::collaborative_call_once(flag, func);
#else
            tbb_kahypar::collaborative_call_once(flag, std::forward<Fn>(body), std::forward<Args>(args)...);
#endif //__TBB_GCC_PARAMETER_PACK_IN_LAMBDAS_BROKEN
        }
...
TEST_CASE("only calls once - stress test") {
    ...
    constexpr std::size_t N = tbb_kahypar::detail::d0::max_nfs_size * 2;
#elif __TBB_USE_THREAD_SANITIZER
    // Reduce execution time under Thread Sanitizer
    constexpr std::size_t N = tbb_kahypar::detail::d0::max_nfs_size + 64;
#else 
    constexpr std::size_t N = tbb_kahypar::detail::d0::max_nfs_size * 4;
#endif
    {
        increment_functor f;
        ...
        utils::NativeParallelFor(N, [&](std::size_t) {
            for (int i = 0; i < 100; ++i) {
                ...
                barrier.wait([&] {
                    ...
                    new (&flag) tbb_kahypar::collaborative_once_flag{};
                });
                tbb_kahypar::collaborative_call_once(flag, f);
            }
        });
    }
}
...
TEST_CASE("handles exceptions - state reset") {
    ...
    tbb_kahypar::collaborative_once_flag flag;
    REQUIRE_THROWS_AS(tbb_kahypar::collaborative_call_once(flag, setBAndCancel), call_once_exception);
    REQUIRE(b);
    ...
    b = false;
    tbb_kahypar::collaborative_call_once(flag, setB);
    REQUIRE(b);

    b = false;
    tbb_kahypar::collaborative_call_once(flag, setB); // Now the call_once flag should be set.
    REQUIRE(!b);

    b = false;
    REQUIRE_NOTHROW(tbb_kahypar::collaborative_call_once(flag, setBAndCancel)); // Flag still set, so it shouldn't be called.
    REQUIRE(!b);
}
...
TEST_CASE("handles exceptions - stress test") {
    ...
    utils::NativeParallelFor(N, [&](std::size_t) {
        while(run_again) {
            try {
                tbb_kahypar::collaborative_call_once(flag, throwing_func);
...
TEST_CASE("multiple help") {
    ...
    tbb_kahypar::parallel_for<std::size_t>(0, num_threads, [&](std::size_t) {
        barrier.wait();
        tbb_kahypar::collaborative_call_once(flag, [&] {
            tbb_kahypar::parallel_for<std::size_t>(0, num_threads, [&](std::size_t) {
                barrier.wait();
            });
        });
...
TEST_CASE("multiple arenas") {
    ...
    utils::SpinBarrier barrier(num_threads);
    tbb_kahypar::task_arena a1(num_threads), a2(num_threads);
...
using FibBuffer = std::vector<std::pair<tbb_kahypar::collaborative_once_flag, std::uint64_t>>;
std::uint64_t collaborative_recursive_fib(int n, FibBuffer& buffer) {
    if (n <= 1) {
        return 1;
    }
    tbb_kahypar::collaborative_call_once(buffer[n].first, [&]() {
        std::uint64_t a, b;
        tbb_kahypar::parallel_invoke([&] { a = collaborative_recursive_fib(n - 2, buffer); },
