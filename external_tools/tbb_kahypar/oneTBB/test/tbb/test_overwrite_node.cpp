...
void simple_read_write_tests() {
    ...
    tbb_kahypar::flow::overwrite_node<R> n(g);
    ...
    for ( int t = 0; t < T; ++t ) {
        ...
        for (int i = 0; i < M; ++i) {
            tbb_kahypar::flow::make_edge( n, *r[i] );
        }
        ...
        for (int i = 0; i < M; ++i) {
            tbb_kahypar::flow::remove_edge( n, *r[i] );
        }
...
class native_body : utils::NoAssign {
    tbb_kahypar::flow::overwrite_node<R> &my_node;

public:

    native_body( tbb_kahypar::flow::overwrite_node<R> &n ) : my_node(n) {}
...
void parallel_read_write_tests() {
    ...
    std::vector< tbb_kahypar::flow::overwrite_node<R> > ow_vec(2, n);
    ...
    for (size_t node_idx=0; node_idx<ow_vec.size(); ++node_idx) {
        for ( int t = 0; t < T; ++t ) {
            ...
            for (int i = 0; i < M; ++i) {
                tbb_kahypar::flow::make_edge( ow_vec[node_idx], *r[i] );
            }
            R v0;
            ...
            for (int i = 0; i < M; ++i) {
                tbb_kahypar::flow::remove_edge( ow_vec[node_idx], *r[i] );
            }
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows<msg_t, tbb_kahypar::flow::overwrite_node<msg_t>>(messages_for_follows);
    follows_and_precedes_testing::test_precedes<msg_t, tbb_kahypar::flow::overwrite_node<msg_t>>(messages_for_precedes);
}
#endif
...
void test_overwrite_node_try_put_and_wait() {
    ...
    // Test push
    {
        ...
        std::size_t after_start = test_buffer_push<tbb_kahypar::flow::overwrite_node<int>>(start_work_items, wait_message,
    ...
    // Test pull
    {
        ...
        arena.execute([&] {
            ...
            tbb_kahypar::flow::graph g;
            tbb_kahypar::flow::overwrite_node<int> buffer(g);
            int start_message = 0;
            int new_message = 1;
    ...
    // Test reserve
    {
        ...
        arena.execute([&] {
            ...
            tbb_kahypar::flow::graph g;
            tbb_kahypar::flow::overwrite_node<int> buffer(g);
            tbb_kahypar::flow::limiter_node<int, int> limiter(g, 1);
            int start_message = 0;
            int new_message = 1;
    ...
    // Test explicit clear
    {
        ...
        tbb_kahypar::flow::function_node<int, int> f(g, tbb_kahypar::flow::serial,
            [&](int input) {
                processed_items.emplace_back(input);
                buffer.clear();
                return 0;
        ...
        tbb_kahypar::flow::make_edge(buffer, f);
    ...
    for( unsigned int p=utils::MinThread; p<=utils::MaxThread; ++p ) {
        ...
        arena.execute(
            [&]() {
                ...
                test_reserving_nodes<tbb_kahypar::flow::overwrite_node, size_t>();
            }
        );
	}
}
...
TEST_CASE("Cancel register_predecessor_task") {
    ...
    oneapi::tbb_kahypar::flow::overwrite_node<size_t> node(g);
    // Reserving join_node always fails during putting an item to it
    tbb_kahypar::flow::join_node<std::tuple<size_t>, tbb_kahypar::flow::reserving> j_node(g);
    ...
    // during execution
    tbb_kahypar::flow::make_edge(node, tbb_kahypar::flow::input_port<0>(j_node));
...
TEST_CASE("Nested make_edge to continue_node") {
    ...
    using msg_t = tbb_kahypar::flow::continue_msg;
    using buffer_t = tbb_kahypar::flow::overwrite_node<msg_t>;
