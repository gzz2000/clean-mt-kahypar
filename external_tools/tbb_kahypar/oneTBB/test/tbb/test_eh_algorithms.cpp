...
#include "tbb_kahypar/parallel_pipeline.h"
...
struct context_specific_counter {
    tbb_kahypar::concurrent_unordered_map<tbb_kahypar::task_group_context*, std::atomic<unsigned>> context_map{};

    void increment() {
        tbb_kahypar::task_group_context* ctx = tbb_kahypar::task::current_context();
        REQUIRE(ctx != nullptr);
        context_map[ctx]++;
    }
...
////////////////////////////////////////////////////////////////////////////////
// Tests for tbb_kahypar::parallel_for and tbb_kahypar::parallel_reduce
////////////////////////////////////////////////////////////////////////////////

typedef size_t count_type;
typedef tbb_kahypar::blocked_range<count_type> range_type;

inline intptr_t CountSubranges(range_type r) {
    if(!r.is_divisible()) return intptr_t(1);
    range_type r2(r,tbb_kahypar::split());
...
intptr_t TestNumSubrangesCalculation ( intptr_t length, intptr_t grain, intptr_t inner_length, intptr_t inner_grain ) {
    ...
    tbb_kahypar::parallel_for( range_type(0, length, grain), Body() );
...
public:
    void operator()( const range_type& r ) const {
        ...
        if( tbb_kahypar::is_current_task_group_canceling() ) g_TGCCancelled.increment();
...
void Test0 () {
    ResetGlobals();
    tbb_kahypar::simple_partitioner p;
    for( size_t i=0; i<10; ++i ) {
        tbb_kahypar::parallel_for( range_type(0, 0, 1), NoThrowParForBody() );
        tbb_kahypar::parallel_for( range_type(0, 0, 1), NoThrowParForBody(), p );
        tbb_kahypar::parallel_for( range_type(0, 128, 8), NoThrowParForBody() );
        tbb_kahypar::parallel_for( range_type(0, 128, 8), NoThrowParForBody(), p );
    }
} // void Test0 ()
...
public:
    ...
    SimpleParReduceBody( SimpleParReduceBody& left, tbb_kahypar::split ) : m_Body(left.m_Body) {}
...
void TestParallelLoopAux() {
    ...
    for( int i=0; i<2; ++i ) {
        ...
        TRY();
            if( i==0 )
                tbb_kahypar::parallel_for( range_type(0, FLAT_RANGE, FLAT_GRAIN), ParForBody(), partitioner );
            else {
                SimpleParReduceBody<ParForBody> rb;
                tbb_kahypar::parallel_reduce( range_type(0, FLAT_RANGE, FLAT_GRAIN), rb, partitioner );
            }
        CATCH_AND_ASSERT();
...
/** The Body only needs to be suitable for tbb_kahypar::parallel_for. */
template<typename Body>
void TestParallelLoop() {
    // The simple and auto partitioners should be const, but not the affinity partitioner.
    TestParallelLoopAux<Body, const tbb_kahypar::simple_partitioner  >();
    TestParallelLoopAux<Body, const tbb_kahypar::auto_partitioner    >();
#define __TBB_TEMPORARILY_DISABLED 1
    ...
    TestParallelLoopAux<Body, /***/ tbb_kahypar::affinity_partitioner>();
#endif
...
public:
    ...
    void operator()( const range_type& ) const {
        utils::ConcurrencyTracker ct;
        ++g_OuterParCalls;
        tbb_kahypar::parallel_for( tbb_kahypar::blocked_range<size_t>(0, INNER_RANGE, INNER_GRAIN), SimpleParForBody() );
    }
};
...
public:
    void operator()( const range_type& ) const {
        tbb_kahypar::task_group_context ctx(tbb_kahypar::task_group_context::isolated);
        ++g_OuterParCalls;
        tbb_kahypar::parallel_for( tbb_kahypar::blocked_range<size_t>(0, INNER_RANGE, INNER_GRAIN), SimpleParForBody(), tbb_kahypar::simple_partitioner(), ctx );
    }
};
...
void Test3 () {
    ...
    TRY();
        tbb_kahypar::parallel_for( range_type(0, OUTER_RANGE, OUTER_GRAIN), body_type() );
    CATCH_AND_ASSERT();
...
void Test4 () {
    ...
    TRY();
        tbb_kahypar::parallel_for( range_type(0, OUTER_RANGE, OUTER_GRAIN), OuterParForExceptionSafeBody() );
    CATCH();
...
TEST_CASE("parallel_for and parallel_reduce exception handling test #0") {
    for (auto concurrency_level: utils::concurrency_range()) {
        ...
        if (g_NumThreads > 1) {
            tbb_kahypar::task_arena a(g_NumThreads);
            a.execute([] {
...
public:
    void operator()() const {
        tbb_kahypar::parallel_for( range_type(0, FLAT_RANGE, FLAT_GRAIN), B(), tbb_kahypar::simple_partitioner(), my_ctx );
    }
    ParForLauncher ( tbb_kahypar::task_group_context& ctx ) : my_ctx(ctx) {}
};
...
class Cancellator2  {
    tbb_kahypar::task_group_context &m_GroupToCancel;
...
public:
    ...
    Cancellator2 ( tbb_kahypar::task_group_context& ctx, intptr_t ) : m_GroupToCancel(ctx) {}
};
...
public:
    void operator()( const range_type& ) const {
        ...
        while( !tbb_kahypar::is_current_task_group_canceling() )
            utils::yield();
    }
};

//! Test for cancelling an algorithm from outside (from a task running in parallel with the algorithm).
/** This version also tests tbb_kahypar::is_current_task_group_canceling() method. **/
...
class RecursiveParReduceBodyWithSharedWorker {
    ...
    int m_Result;
public:
    RecursiveParReduceBodyWithSharedWorker ( RecursiveParReduceBodyWithSharedWorker& src, tbb_kahypar::split )
    ...
    void operator() ( const tbb_kahypar::blocked_range<size_t>& r ) {
        if(g_Master == std::this_thread::get_id()) g_MasterExecuted = true;
        else g_NonMasterExecuted = true;
        if( tbb_kahypar::is_current_task_group_canceling() ) g_TGCCancelled.increment();
...
int Worker::DoWork ( int level ) {
    ...
    if ( level < max_nesting ) {
        ...
        tbb_kahypar::parallel_reduce (tbb_kahypar::blocked_range<size_t>(0, reduce_range, reduce_grain), rt);
        return rt.result();
    }
    else
        return 1;
...
public:
    ParReduceBody( StatsCounters& s_, tbb_kahypar::task_group_context& context, bool e_ ) :
    ...
    ParReduceBody( ParReduceBody& lhs, tbb_kahypar::split ) : tgc(lhs.tgc) {
        my_stats = lhs.my_stats;
        my_id = my_stats->my_total_created++;
    }
    ...
    void operator()( const tbb_kahypar::blocked_range<std::size_t>& /*range*/ ) const {
...
#endif
    {
        tbb_kahypar::task_group_context tgc1, tgc2;
        ParReduceBody body_for_cancellation(statsObj, tgc1, false), body_for_exception(statsObj, tgc2, true);
        tbb_kahypar::parallel_reduce( tbb_kahypar::blocked_range<std::size_t>(0,100000000,100), body_for_cancellation, tbb_kahypar::simple_partitioner(), tgc1 );
        tbb_kahypar::parallel_reduce( tbb_kahypar::blocked_range<std::size_t>(0,100000000,100), body_for_exception, tbb_kahypar::simple_partitioner(), tgc2 );
    }
#if TBB_USE_EXCEPTIONS
    catch(...) {}
#endif
...
////////////////////////////////////////////////////////////////////////////////
// Tests for tbb_kahypar::parallel_for_each
////////////////////////////////////////////////////////////////////////////////
...
void Feed ( tbb_kahypar::feeder<size_t> &feeder, size_t val ) {
...
public:
    void operator() ( size_t &value ) const {
        ...
        if( tbb_kahypar::is_current_task_group_canceling() ) {
...
public:
    void operator() ( size_t &value, tbb_kahypar::feeder<size_t> &feeder ) const {
        Feed(feeder, 0);
...
void Test1_parallel_for_each () {
    ...
    TRY();
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), simple_body() );
    CATCH_AND_ASSERT();
    REQUIRE_MESSAGE (g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads, "Too many tasks survived exception");
    g_TGCCancelled.validate(g_NumThreads, "Too many tasks survived cancellation");
    REQUIRE_MESSAGE (g_NumExceptionsCaught == 1, "No try_blocks in any body expected in this test");
...
public:
    void operator()( size_t& /*value*/ ) const {
        ++g_OuterParCalls;
        auto range = adaptive_range<Iterator>(get_iter_range_size());
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), SimpleParForEachBody());
    }
};
...
void Test2_parallel_for_each () {
    ...
    auto range = adaptive_range<Iterator>(get_iter_range_size());
    TRY();
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), outer_body() );
    CATCH_AND_ASSERT();
    REQUIRE_MESSAGE (g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads, "Too many tasks survived exception");
    g_TGCCancelled.validate(g_NumThreads, "Too many tasks survived cancellation");
    REQUIRE_MESSAGE (g_NumExceptionsCaught == 1, "No try_blocks in any body expected in this test");
...
public:
    void operator()( size_t& /*value*/ ) const {
        ...
        ++g_OuterParCalls;
        auto range = adaptive_range<Iterator>(get_iter_range_size());
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), SimpleParForEachBody(), ctx);
    }
};
...
void Test4_parallel_for_each () {
    ...
    g_Master = std::this_thread::get_id();
    TRY();
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), outer_body_with_eh());
    CATCH();
...
public:
    ...
    void operator() (const size_t &value, tbb_kahypar::feeder<size_t> &feeder ) const {
        ++g_CurExecuted;
        if(g_Master == std::this_thread::get_id()) g_MasterExecuted = true;
        else g_NonMasterExecuted = true;
        if( tbb_kahypar::is_current_task_group_canceling() ) g_TGCCancelled.increment();
...
void Test5_parallel_for_each () {
    ...
    g_Master = std::this_thread::get_id();
    TRY();
        tbb_kahypar::parallel_for_each(std::begin(range), std::end(range), ParForEachBodyWithThrowingFeederTasks());
    CATCH();
    if (g_SolitaryException) {
...
public:
    void operator()() const {
        auto range = adaptive_range<Iterator>(get_iter_range_size());
        tbb_kahypar::parallel_for_each( std::begin(range), std::end(range), B(), my_ctx );
    }

    ParForEachWorker ( tbb_kahypar::task_group_context& ctx ) : my_ctx(ctx) {}
};
...
////////////////////////////////////////////////////////////////////////////////
// Tests for tbb_kahypar::parallel_pipeline
////////////////////////////////////////////////////////////////////////////////
int g_NumTokens = 0;
...
public:
    ...
    void* operator()(tbb_kahypar::flow_control& control) const {
        size_t item = m_Item++;
        if(g_Master == std::this_thread::get_id()) g_MasterExecuted = true;
        else g_NonMasterExecuted = true;
        if( tbb_kahypar::is_current_task_group_canceling() ) g_TGCCancelled.increment();
...
struct FilterSet {
    tbb_kahypar::filter_mode mode1, mode2;
    bool throw1, throw2;

    FilterSet( tbb_kahypar::filter_mode m1, tbb_kahypar::filter_mode m2, bool t1, bool t2 )
        : mode1(m1), mode2(m2), throw1(t1), throw2(t2)
    {}
}; // struct FilterSet

FilterSet serial_parallel( tbb_kahypar::filter_mode::serial_in_order, tbb_kahypar::filter_mode::parallel, /*throw1*/false, /*throw2*/true );
...
public:
    CustomPipeline( const FilterSet& filters )
        : filter1(filters.throw1, filters.mode1 != tbb_kahypar::filter_mode::parallel),
          filter2(filters.throw2, filters.mode2 != tbb_kahypar::filter_mode::parallel),
          my_filters(filters)
    {}

    void run () {
        tbb_kahypar::parallel_pipeline(
            g_NumTokens,
            tbb_kahypar::make_filter<void, void*>(tbb_kahypar::filter_mode::parallel, inputFilter) &
            tbb_kahypar::make_filter<void*, void*>(my_filters.mode1, filter1) &
            tbb_kahypar::make_filter<void*, void>(my_filters.mode2, filter2)
        );
    }
    void run ( tbb_kahypar::task_group_context& ctx ) {
        tbb_kahypar::parallel_pipeline(
            g_NumTokens,
            tbb_kahypar::make_filter<void, void*>(tbb_kahypar::filter_mode::parallel, inputFilter) &
            tbb_kahypar::make_filter<void*, void*>(my_filters.mode1, filter1) &
    ...
    TRY();
        ...
        if ( g_CurExecuted == 2 * static_cast<int>(get_iter_range_size()) ) {
            ...
            if(!g_ExceptionInMaster && g_NonMasterExecutedThrow > 0) {
                ...
                REQUIRE_MESSAGE((filters.mode1 != tbb_kahypar::filter_mode::parallel && filters.mode2 != tbb_kahypar::filter_mode::parallel),
                    "Unusual count");
            }
...
void TestWithDifferentFiltersAndConcurrency() {
    ...
    for (auto concurrency_level: utils::concurrency_range()) {
        ...
        if (g_NumThreads > 1) {
            ...
            const tbb_kahypar::filter_mode modes[] = {
                tbb_kahypar::filter_mode::parallel,
                tbb_kahypar::filter_mode::serial_in_order,
                tbb_kahypar::filter_mode::serial_out_of_order
            };
...
class PipelineLauncher {
    tbb_kahypar::task_group_context &my_ctx;
public:
    PipelineLauncher ( tbb_kahypar::task_group_context& ctx ) : my_ctx(ctx) {}
    ...
    void operator()() const {
        ...
        tbb_kahypar::parallel_pipeline(
            g_NumTokens,
            tbb_kahypar::make_filter<void, void*>(tbb_kahypar::filter_mode::parallel, inputFilter) &
            tbb_kahypar::make_filter<void*, void>(tbb_kahypar::filter_mode::parallel, filterToCancel),
            my_ctx
        );
    }
};
