...
struct parallel_sort_test {
    static void run() {
        ...
        while (fill_ranges(std::begin(range), std::begin(sorted_range), Size, default_comp)) {
            tbb_kahypar::parallel_sort(range);
    ...
    static void run(Comparator& comp) {
        ...
        while (fill_ranges(std::begin(range), std::begin(sorted_range), Size, comp)) {
            tbb_kahypar::parallel_sort(range, comp);
            validate(range, sorted_range);
        }
    }
};
...
concept can_call_parallel_sort_with_cbs_and_compare = requires( CBS& cbs, const Compare& compare ) {
    tbb_kahypar::parallel_sort(cbs, compare);
};
...
//! tbb_kahypar::concurrent_vector<float> sorting test (less comparator)
//! \brief \ref error_guessing
TEST_CASE("tbb_kahypar::concurrent_vector<float> sorting test (less comparator)") {
    parallel_sort_test_suite<tbb_kahypar::concurrent_vector<float>, std::less<float>>();
}

//! tbb_kahypar::concurrent_vector<float> sorting test (default comparator)
//! \brief \ref error_guessing
TEST_CASE("tbb_kahypar::concurrent_vector<float> sorting test (default comparator)") {
    parallel_sort_test_suite<tbb_kahypar::concurrent_vector<float>>();
}
...
//! tbb_kahypar::concurrent_vector<Minimal> sorting test (less comparator)
//! \brief \ref error_guessing
TEST_CASE("tbb_kahypar::concurrent_vector<Minimal> sorting test (less comparator)") {
    parallel_sort_test_suite<tbb_kahypar::concurrent_vector<Minimal>, MinimalLessCompare>();
}
...
TEST_CASE("Array sorting test (default comparator)") {
    for ( auto concurrency_level : utils::concurrency_range() ) {
        ...
        sort_array_test([](int (&array)[array_size]) {
            tbb_kahypar::parallel_sort(array);
        });
    }
}
...
TEST_CASE("Test array sorting via rvalue span (default comparator)") {
    sort_array_test([](int (&array)[array_size]) {
        tbb_kahypar::parallel_sort(minimal_span<int>{array, array_size});
    });
}
...
TEST_CASE("Test array sorting via const span (default comparator)") {
    sort_array_test([](int (&array)[array_size]) {
        ...
        tbb_kahypar::parallel_sort(span);
    });
}
...
TEST_CASE("Test rvalue container with stateful comparator") {
    ...
    tbb_kahypar::parallel_sort(std::move(test_vector), [&](std::size_t lhs, std::size_t rhs) {
        ++count;
        return lhs < rhs;
    });
...
TEST_CASE("That all workers sleep when no work") {
    ...
    tbb_kahypar::parallel_sort(test_array);
