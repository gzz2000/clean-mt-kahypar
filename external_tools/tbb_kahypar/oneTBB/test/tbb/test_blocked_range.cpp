...
TEST_CASE("Range based for") {
    ...
    const tbb_kahypar::blocked_range<std::size_t*> r(int_array, int_array + sequence_length, 1);
...
TEST_CASE("Proportional split overflow") {
    using tbb_kahypar::blocked_range;
    using tbb_kahypar::proportional_split;
...
template <bool ExpectSatisfies, typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range, Types> == ExpectSatisfies))
void test_blocked_range_constraint() {}

template <bool ExpectSatisfies, typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range2d, Types, Types> == ExpectSatisfies))
void test_blocked_range2d_constraint() {}

template <typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range2d, Types, test_concepts::Dummy> == false))
void test_blocked_range2d_col_invalid_constraint() {}

template <typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range2d, test_concepts::Dummy, Types> == false))
void test_blocked_range2d_row_invalid_constraint() {}

template <bool ExpectSatisfies, typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range3d, Types, Types, Types> == ExpectSatisfies))
void test_blocked_range3d_constraint() {}

template <typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range3d, test_concepts::Dummy, Types, Types> == false))
void test_blocked_range3d_page_invalid_constraint() {}

template <typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range3d, Types, test_concepts::Dummy, Types> == false))
void test_blocked_range3d_row_invalid_constraint() {}

template <typename... Types>
    requires (... && (utils::well_formed_instantiation<tbb_kahypar::blocked_range3d, Types, Types, test_concepts::Dummy> == false))
void test_blocked_range3d_col_invalid_constraint() {}

template <typename T>
concept well_formed_blocked_nd_range_instantiation_basic = requires {
    typename tbb_kahypar::blocked_nd_range<T, 1>;
};
...
void test_deduction_guides() {
    using oneapi::tbb_kahypar::blocked_nd_range;
    ...
    oneapi::tbb_kahypar::blocked_range<T> dim_range(0, 100);
    ...
    {
        blocked_nd_range range(source_range, oneapi::tbb_kahypar::split{});
        static_assert(std::is_same_v<decltype(range), decltype(source_range)>);
    }
    {
        blocked_nd_range range(source_range, oneapi::tbb_kahypar::proportional_split{1, 3});
        static_assert(std::is_same_v<decltype(range), decltype(source_range)>);
    }
    {
