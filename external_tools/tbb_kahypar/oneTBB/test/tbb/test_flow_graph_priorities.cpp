...
void spin_for( double delta ) {
    tbb_kahypar::tick_count start = tbb_kahypar::tick_count::now();
    while( (tbb_kahypar::tick_count::now() - start).seconds() < delta ) ;
}
...
void body_func( int priority, utils::SpinBarrier& my_barrier ) {
    ...
    const bool all_threads_will_come =
        unsigned(current_task_index) < node_num - (node_num % tbb_kahypar::this_task_arena::max_concurrency());
...
void test_node( NodeTypeCreator node_creator ) {
    const int num_threads = tbb_kahypar::this_task_arena::max_concurrency();
...
void call_within_arena( tbb_kahypar::task_arena& arena ) {
    arena.execute(
        [&]() {
...
struct StartBody {
    bool has_run;
    data_type operator()(tbb_kahypar::flow_control& fc) {
        if (has_run){
            fc.stop();
            return data_type();
        }
...
struct CpuWorkBody {
    ...
    data_type operator()(const data_type& input) {
        tbb_kahypar::parallel_for(0, my_tasks_count, ParallelForBody(my_barrier, input), tbb_kahypar::simple_partitioner());
        return input;
    }
...
void test( unsigned num_threads ) {
    ...
    tbb_kahypar::task_arena arena(cpu_threads);
	arena.execute(
        [&]() {
...
void do_nested_work( const std::thread::id& tid, const tbb_kahypar::blocked_range<int>& subrange );
...
struct CommonBody {
    ...
    void operator()( const tbb_kahypar::blocked_range<int>& subrange ) const {
...
template<work_type_t work_type>
void do_node_work(int work_size) {
    tbb_kahypar::parallel_for( tbb_kahypar::blocked_range<int>(0, work_size), CommonBody<work_type>(),
...
struct IsolationFunctor {
    ...
    void operator()() const {
        tbb_kahypar::parallel_for( tbb_kahypar::blocked_range<int>(0, work_size), CommonBody<PRIORITIZED_WORK>(),
                           tbb_kahypar::simple_partitioner() );
    }
};
...
void do_node_work<PRIORITIZED_WORK>(int work_size) {
    ...
    tbb_kahypar::this_task_arena::isolate( IsolationFunctor(work_size) );
    exec_tracker.prioritized_work_finished = true;
}

template<>
void do_nested_work<PRIORITIZED_WORK>( const std::thread::id& tid,
                                       const tbb_kahypar::blocked_range<int>& /*subrange*/ ) {
    if( exec_tracker.prioritized_work_started && exec_tracker.prioritized_work_submitter == tid ) {
...
struct InputNodeBody {
    continue_msg operator()( tbb_kahypar::flow_control& fc ){
