...
TEST_CASE("That all workers sleep when no work") {
    ...
    tbb_kahypar::parallel_for_each(vec.begin(), vec.end(), [&](std::size_t& in) {
...
concept can_call_parallel_for_each_with_iterator = requires( Iterator it, const Body& body, tbb_kahypar::task_group_context ctx ) {
    tbb_kahypar::parallel_for_each(it, it, body);
    tbb_kahypar::parallel_for_each(it, it, body, ctx);
};
...
concept can_call_parallel_for_each_with_cbs = requires( ContainerBasedSequence cbs,
                                                        ...
                                                        const Body& body, tbb_kahypar::task_group_context ctx ) {
    tbb_kahypar::parallel_for_each(cbs, body);
    tbb_kahypar::parallel_for_each(cbs, body, ctx);
    tbb_kahypar::parallel_for_each(const_cbs, body);
    tbb_kahypar::parallel_for_each(const_cbs, body, ctx);
};
...
void test_with_cpp20_iterator() {
    ...
    oneapi::tbb_kahypar::parallel_for_each(begin, end, [](no_copy_move& element) {
        element.item = 42;
    });
