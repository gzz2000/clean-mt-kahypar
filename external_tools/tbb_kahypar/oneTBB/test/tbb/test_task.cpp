...
class CountingTask : public tbb_kahypar::detail::d1::task {
public:
    CountingTask( Body body, tbb_kahypar::detail::d1::wait_context& wait ) : my_body(body), my_wait(wait) {}

    CountingTask( tbb_kahypar::detail::d1::wait_context& wait ) : my_wait(wait) {}

    task* execute( tbb_kahypar::detail::d1::execution_data& ) override {
        ++my_execute_counter;
        my_body();
        my_wait.release();
        return nullptr;
    ...
    task* cancel( tbb_kahypar::detail::d1::execution_data& ) override {
        ++my_cancel_counter;
        my_wait.release();
        return nullptr;
    }
...
private:
    Body my_body;
    tbb_kahypar::detail::d1::wait_context& my_wait;
...
void test_cancellation_on_exception( bool reset_ctx ) {
    tbb_kahypar::detail::d1::wait_context wait(1);
    tbb_kahypar::task_group_context test_context;
    auto throw_body = [] {
        throw 1;
    };
    ...
    for (std::size_t i = 0; i < iter_counter; ++i) {
        try {
            tbb_kahypar::detail::d1::execute_and_wait(task, test_context, wait, test_context);
        } catch(int ex) {
            REQUIRE(ex == 1);
        }
        if (reset_ctx) {
...
TEST_CASE("External threads sleep") {
    ...
    tbb_kahypar::task_group test_gr;
...
TEST_CASE("Test that task was executed p times") {
    ...
    for (std::size_t i = 0; i < iter_counter; ++i) {
        tbb_kahypar::detail::d1::execute_and_wait(test_task, test_context, wait, test_context);
        wait.reserve(1);
    }
...
TEST_CASE("Simple test parallelism usage") {
    ...
    tbb_kahypar::detail::d1::wait_context wait(threads_num);
    tbb_kahypar::detail::d1::task_group_context test_context;
    using task_type = CountingTask<decltype(barrier_wait)>;

    std::vector<task_type, tbb_kahypar::cache_aligned_allocator<task_type>> vector_test_task(threads_num, task_type(barrier_wait, wait));

    constexpr std::size_t iter_counter = 100;
    for (std::size_t i = 0; i < iter_counter; ++i) {
        for (std::size_t j = 0; j < threads_num; ++j) {
            tbb_kahypar::detail::d1::spawn(vector_test_task[j], test_context);
        }
        tbb_kahypar::detail::d1::wait(wait, test_context);
...
TEST_CASE("Test parallelism usage with parallel_for") {
    ...
    auto parallel_for_func = [&pfor_counter, pfor_iter_count] {
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(0, pfor_iter_count),
                          [&pfor_counter] (tbb_kahypar::blocked_range<std::size_t>& range) {
    ...
    tbb_kahypar::detail::d1::wait_context wait(task_threads_num);
    tbb_kahypar::detail::d1::task_group_context test_context;
    using task_type = CountingTask<decltype(barrier_wait)>;
    std::vector<task_type, tbb_kahypar::cache_aligned_allocator<task_type>> vector_test_task(task_threads_num, task_type(barrier_wait, wait));
...
TEST_CASE("Test parallelism usage with spawn tasks in different threads") {
    ...
    auto thread_func = [&vector_test_task, &test_context] ( std::size_t idx ) {
        tbb_kahypar::detail::d1::spawn(vector_test_task[idx], test_context);
    };
    ...
    for (std::size_t i = 0; i < iter_count; ++i) {
        ...
        tbb_kahypar::detail::d1::execute_and_wait(vector_test_task[threads_num - 1], test_context, wait, test_context);
        wait.reserve(threads_num);
    }
    wait.release(threads_num);
...
class SpawningTaskBody {
public:
    using task_pool_type = std::vector<SpawningTask, tbb_kahypar::cache_aligned_allocator<SpawningTask>>;

    SpawningTaskBody( task_pool_type& task_pool, tbb_kahypar::task_group_context& test_ctx )
    ...
    void operator()() const {
        ...
        if (start_idx < my_task_pool.size()) {
            for (std::size_t idx = start_idx; idx != std::min(my_task_pool.size(), start_idx + delta); ++idx) {
                tbb_kahypar::detail::d1::spawn(my_task_pool[idx], my_test_ctx);
            }
        }
    }
private:
    task_pool_type& my_task_pool;
    tbb_kahypar::task_group_context& my_test_ctx;
...
TEST_CASE("Actively adding tasks") {
    ...
    tbb_kahypar::detail::d1::wait_context wait(task_number + 1);
    tbb_kahypar::task_group_context test_context;
    ...
    tbb_kahypar::detail::d1::execute_and_wait(first_task, test_context, wait, test_context);
...
struct suspended_task : public tbb_kahypar::detail::d1::task {

    suspended_task(tbb_kahypar::task::suspend_point tag, tbb_kahypar::detail::d1::wait_context& wait)
        : my_suspend_tag(tag), my_wait(wait)
    {}

    task* execute(tbb_kahypar::detail::d1::execution_data&) override {
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(0, 100000),
            [] (const tbb_kahypar::blocked_range<std::size_t>& range) {
        ...
        my_wait.release();
        tbb_kahypar::task::resume(my_suspend_tag);
        return nullptr;
    }
    ...
    tbb_kahypar::task::suspend_point my_suspend_tag;
    tbb_kahypar::detail::d1::wait_context& my_wait;
};
...
TEST_CASE("Isolation + resumable tasks") {
    ...
    tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(0, 100000),
        [&suspend_flag, &test_context, &suspend_count, &resume_count] (const tbb_kahypar::blocked_range<std::size_t>& range) {
            int ticket = 0;
            for (auto it = range.begin(); it != range.end(); ++it) {
                ticket = suspend_flag++;
            }
            ...
            if (ticket % 5 == 0) {
                std::vector<suspended_task, tbb_kahypar::cache_aligned_allocator<suspended_task>> test_task;
                tbb_kahypar::detail::d1::wait_context wait(1);
                ++suspend_count;
                tbb_kahypar::this_task_arena::isolate([&wait, &test_context, &test_task] {
                    auto thread_id = std::this_thread::get_id();
                    tbb_kahypar::task::suspend([&wait, &test_context, &test_task, thread_id] (tbb_kahypar::task::suspend_point tag) {
                        CHECK(thread_id == std::this_thread::get_id());
                        test_task.emplace_back(tag, wait);
                        tbb_kahypar::detail::d1::spawn(test_task[0], test_context);
                    });
                    }
                );
                tbb_kahypar::detail::d1::wait(wait, test_context);
...
struct bypass_task : public tbb_kahypar::detail::d1::task {
    using task_pool_type = std::vector<bypass_task, tbb_kahypar::cache_aligned_allocator<bypass_task>>;

    bypass_task(tbb_kahypar::detail::d1::wait_context& wait, task_pool_type& task_pool,
                std::atomic<int>& resume_flag, tbb_kahypar::task::suspend_point& suspend_tag)
...
TEST_CASE("Bypass suspended by resume") {
    ...
    tbb_kahypar::task::suspend_point test_suspend_tag;

    std::vector<bypass_task, tbb_kahypar::cache_aligned_allocator<bypass_task>> test_task_pool;
    ...
    for (std::size_t i = 0; i < utils::get_platform_max_threads(); ++i) {
        ...
        if (ticket < test_task_pool.size()) {
            tbb_kahypar::detail::d1::spawn(test_task_pool[ticket], test_context);
        }
    }

    auto suspend_func = [&resume_flag, &test_suspend_tag] {
        auto thread_id = std::this_thread::get_id();
        tbb_kahypar::task::suspend([&resume_flag, &test_suspend_tag, thread_id] (tbb_kahypar::task::suspend_point tag) {
            CHECK(thread_id == std::this_thread::get_id());
    ...
    tbb_kahypar::detail::d1::execute_and_wait(suspend_task, test_context, wait, test_context);
...
TEST_CASE("Critical tasks + resume") {
    ...
    tbb_kahypar::task_group_context test_context;
    tbb_kahypar::detail::d1::wait_context wait{ 0 };
    ...
    tbb_kahypar::global_control thread_limit(tbb_kahypar::global_control::max_allowed_parallelism, num_threads_in_test_arena);
    tbb_kahypar::task_arena test_arena(num_threads_in_test_arena);
    ...
    auto task_body = [&resume_flag, &resumed, &test_suspend_tag] {
        ...
        if (resume_flag.exchange(false)) {
            tbb_kahypar::task::resume(test_suspend_tag);
            resumed = true;
        }
    };

    using task_type = CountingTask<decltype(task_body)>;
    std::vector<task_type, tbb_kahypar::cache_aligned_allocator<task_type>> test_tasks;
    ...
    test_arena.execute([&wait, &test_tasks, &test_arena, &test_context, &resumed, task_number] {
        tbb_kahypar::this_task_arena::isolate([&wait, &test_tasks, &test_arena, &test_context, &resumed, task_number] {
            do {
                wait.reserve(task_number / 2);
                tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(task_number / 2, task_number),
                    [&test_tasks, &test_arena, &test_context] (tbb_kahypar::blocked_range<std::size_t>& range) {
...
TEST_CASE("Stress testing") {
    ...
    tbb_kahypar::task_group_context test_context;
    tbb_kahypar::detail::d1::wait_context wait(task_number);

    tbb_kahypar::task_arena test_arena;

    test_arena.initialize();

    auto task_body = [] {
        tbb_kahypar::parallel_for(tbb_kahypar::blocked_range<std::size_t>(0, 1000), [] (tbb_kahypar::blocked_range<std::size_t>&) {
    ...
    test_arena.execute([&test_tasks, &task_body, &wait, &test_context, &test_arena, iter_counter, task_number] {
        for (std::size_t i = 0; i < iter_counter; ++i) {
            ...
            for (std::size_t j = 0; j < task_number / 2; ++j) {
                tbb_kahypar::detail::d1::spawn(test_tasks[j], test_context);
            }
...
TEST_CASE("All workers sleep") {
    ...
    tbb_kahypar::concurrent_vector<tbb_kahypar::task::suspend_point> suspend_points;
    ...
    auto resumble_task = [&] {
        barrier.wait();
        auto thread_id = std::this_thread::get_id();
        tbb_kahypar::task::suspend([&] (tbb_kahypar::task::suspend_point sp) {
            CHECK(thread_id == std::this_thread::get_id());
...
TEST_CASE("Enqueue with exception") {
    ...
    tbb_kahypar::task_arena test_arena{int(std::thread::hardware_concurrency() + 1)};
    ...
    {
        tbb_kahypar::global_control gc(tbb_kahypar::global_control::max_allowed_parallelism, 1);
        test_arena.enqueue(task_body);
        // Initialize implicit arena
        tbb_kahypar::parallel_for(0, 1, [] (int) {});
        tbb_kahypar::task_arena test_arena2(tbb_kahypar::task_arena::attach{});
...
struct resubmitting_task : public tbb_kahypar::detail::d1::task {
    tbb_kahypar::task_arena& my_arena;
    tbb_kahypar::task_group_context& my_ctx;
    std::atomic<int> counter{100000};

    resubmitting_task(tbb_kahypar::task_arena& arena, tbb_kahypar::task_group_context& ctx) : my_arena(arena), my_ctx(ctx)
    {}
    ...
    tbb_kahypar::detail::d1::task* cancel( tbb_kahypar::detail::d1::execution_data& ) override {
        FAIL("The function should never be called.");
        return nullptr;
    }
};
...
TEST_CASE("Test with priority inversion") {
    ...
    std::uint32_t thread_number = static_cast<std::uint32_t>(utils::get_platform_max_threads());
    tbb_kahypar::global_control gc(tbb_kahypar::global_control::max_allowed_parallelism, thread_number + 1);

    tbb_kahypar::task_arena test_arena(2 * thread_number, thread_number);
    test_arena.initialize();
    ...
    tbb_kahypar::task_group_context test_context;
    tbb_kahypar::detail::d1::wait_context wait(critical_task_counter);
    ...
    test_arena.execute([&] {
        tbb_kahypar::parallel_for(std::uint32_t(0), thread_number + 1, [&] (std::uint32_t) {
            barrier.wait();
...
TEST_CASE("raii_guard move ctor") {
    ...
    tbb_kahypar::detail::d0::raii_guard<decltype(func)> guard1(func);
    tbb_kahypar::detail::d0::raii_guard<decltype(func)> guard2(std::move(guard1));
}

//! \brief \ref error_guessing
TEST_CASE("Check correct arena destruction with enqueue") {
    for (int i = 0; i < 100; ++i) {
        tbb_kahypar::task_scheduler_handle handle{ tbb_kahypar::attach{} };
        {
            tbb_kahypar::task_arena a(2, 0);

            a.enqueue([] {
                tbb_kahypar::parallel_for(0, 100, [] (int) { std::this_thread::sleep_for(std::chrono::nanoseconds(10)); });
            });
            std::this_thread::sleep_for(std::chrono::microseconds(1));
        }
        tbb_kahypar::finalize(handle, std::nothrow_t{});
    }
}
...
TEST_CASE("Try to force Leaked proxy observers warning") {
    ...
    tbb_kahypar::global_control gc(tbb_kahypar::global_control::max_allowed_parallelism, num_threads);
    tbb_kahypar::task_arena arena(num_threads, 0);
    std::deque<tbb_kahypar::task_scheduler_observer> observers;
    ...
    arena.enqueue([] {
        tbb_kahypar::parallel_for(0, 100000, [] (int) {
            utils::doDummyWork(1000);
        });
    });
}
...
TEST_CASE("Force thread limit on per-thread reference_vertex") {
    ...
    std::vector<tbb_kahypar::task_group> groups(num_groups);
    tbb_kahypar::parallel_for(0, num_threads, [&] (int) {
        std::vector<tbb_kahypar::task_group> local_groups(num_groups);
    ...
    std::deque<tbb_kahypar::task_handle> handles{};
    for (int i = 0; i < num_groups; ++i) {
