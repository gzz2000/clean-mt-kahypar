...
struct MyDataType {
    std::size_t priority;
    char padding[tbb_kahypar::detail::max_nfs_size - sizeof(int) % tbb_kahypar::detail::max_nfs_size];
...
using CPQExTestType = tbb_kahypar::concurrent_priority_queue<MyThrowingType, MyLess>;
...
void test_scoped_allocator() {
    ...
    using container_type = tbb_kahypar::concurrent_priority_queue<allocator_data_type, std::less<allocator_data_type>, allocator_type>;
