...
int TestNumaDistribution(std::vector<DWORD> &validateProcgrp, int additionalParallelism, bool allThreads){
    ...
    tbb_kahypar::global_control global_limit(oneapi::tbb_kahypar::global_control::max_allowed_parallelism, 1024);
    tbb_kahypar::enumerable_thread_specific< std::pair<int, int> > tls;
    tbb_kahypar::enumerable_thread_specific< double > tls_dummy;
    tbb_kahypar::static_partitioner s;
  
    utils::SpinBarrier sb(requested_parallelism);
    oneapi::tbb_kahypar::task_arena limited(requested_parallelism);
    limited.execute([&]() {

        tbb_kahypar::parallel_for(0, requested_parallelism, [&](int)
            {                    
                PROCESSOR_NUMBER proc;
