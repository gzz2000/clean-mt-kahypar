...
//    const std::list<T>::iterator prev = boost_kahypar::prior(p);
//    const std::list<T>::iterator next = boost_kahypar::next(prev, 2);
...
template< typename T, typename Distance, bool IsIterator = boost_kahypar::iterators::is_iterator< T >::value >
...
struct next_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        boost_kahypar::iterators::advance(x, n);
        return x;
    }
};
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        boost_kahypar::iterators::reverse_iterator< T > rx(x);
        boost_kahypar::iterators::advance(rx, n);
        return rx.base();
    }
};
