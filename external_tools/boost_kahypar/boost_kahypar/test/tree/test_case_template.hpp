...
#include <boost_kahypar/test/utils/algorithm.hpp>


// Boost
#include <boost_kahypar/shared_ptr.hpp>
#include <boost_kahypar/mpl/for_each.hpp>
...
public:
    void    operator()()    { TestCaseTemplate::run( (boost_kahypar::type<TestType>*)0 ); }
};
...
struct generate_test_case_4_type {
    ...
    void        operator()( mpl::identity<TestType> )
    {
        ...
        full_name += '<';
#if !defined(BOOST_NO_TYPEID) && !defined(BOOST_NO_RTTI)
        full_name += boost_kahypar::core::demangle(typeid(TestType).name()); // same as execution_monitor.ipp
#else
        ...
        typedef typename boost_kahypar::remove_reference<TestType>::type TestTypewoRef;
        if( boost_kahypar::is_const<TestTypewoRef>::value )
            full_name += "_const";
        if( boost_kahypar::is_volatile<TestTypewoRef>::value )
            full_name += "_volatile";
        if( boost_kahypar::is_rvalue_reference<TestType>::value )
            full_name += "_refref";
        else if( boost_kahypar::is_lvalue_reference<TestType>::value )
            full_name += "_ref";
...
class template_test_case_gen<
    TestCaseTemplate,
    C<parameter_pack...>,
    typename std::enable_if<!boost_kahypar::mpl::is_sequence<C<parameter_pack...>>::value>::type >
