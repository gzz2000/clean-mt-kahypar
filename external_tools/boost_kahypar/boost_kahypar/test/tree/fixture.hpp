...
namespace impl_fixture {
    ...
    struct has_setup {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(boost_kahypar::declval<U>().setup())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
    ...
    struct has_teardown {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(boost_kahypar::declval<U>().teardown())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
...
public:
    // Constructor
    function_based_fixture( boost_kahypar::function<void ()> const& setup_, boost_kahypar::function<void ()> const& teardown_ )
    : m_setup( setup_ )
...
private:
    ...
    // Data members
    boost_kahypar::function<void ()>    m_setup;
    boost_kahypar::function<void ()>    m_teardown;
};
