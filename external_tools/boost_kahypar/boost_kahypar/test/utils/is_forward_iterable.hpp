...
#include <boost_kahypar/utility/declval.hpp>
...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(boost_kahypar::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(boost_kahypar::declval<U&>())); // does not work with boost_kahypar::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(boost_kahypar::declval<U&>())); // does not work with boost_kahypar::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(boost_kahypar::declval<
        typename boost_kahypar::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename boost_kahypar::add_const<T>::type T_const;
    typedef typename boost_kahypar::add_pointer<T_const>::type const_iterator;
