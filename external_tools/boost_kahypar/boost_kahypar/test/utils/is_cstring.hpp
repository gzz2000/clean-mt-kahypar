...
#include <boost_kahypar/test/utils/basic_cstring/basic_cstring_fwd.hpp>
#include <string>
...
template <typename T, bool is_cstring = is_cstring_impl<typename boost_kahypar::decay<T>::type>::value >
...
struct deduce_cstring_transform_impl<T, true> {
    typedef typename boost_kahypar::add_const<
        typename boost_kahypar::remove_pointer<
            typename boost_kahypar::decay<T>::type
        >::type
    >::type U;
    typedef boost_kahypar::unit_test::basic_cstring<U> type;
};

template <typename T>
struct deduce_cstring_transform_impl< T, false > {
    typedef typename
        boost_kahypar::remove_const<
            typename boost_kahypar::remove_reference<T>::type
        >::type type;
};

template <typename T>
struct deduce_cstring_transform_impl< std::basic_string<T, std::char_traits<T> >, false > {
    typedef boost_kahypar::unit_test::basic_cstring<typename boost_kahypar::add_const<T>::type> type;
};
...
public:
    using type = stringview_cstring_helper<typename boost_kahypar::add_const<T>::type, sv_t>;
};
#endif
...
template<typename T, bool is_cstring = is_cstring<typename boost_kahypar::decay<T>::type>::value >
...
template<typename T>
struct is_cstring_comparable< boost_kahypar::unit_test::basic_cstring<T>, false > : public mpl::true_ {};
...
struct deduce_cstring_transform {
    typedef typename
        boost_kahypar::remove_const<
            typename boost_kahypar::remove_reference<T>::type
        >::type U;
    typedef typename ut_detail::deduce_cstring_transform_impl<typename boost_kahypar::decay<U>::type>::type type;
};
