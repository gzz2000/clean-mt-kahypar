...
// Boost.Test
#include <boost_kahypar/test/data/config.hpp>
#include <boost_kahypar/test/data/dataset.hpp>
#include <boost_kahypar/test/data/for_each_sample.hpp>
#include <boost_kahypar/test/tree/test_unit.hpp>

// Boost
#include <boost_kahypar/preprocessor/repetition/enum_params.hpp>
#include <boost_kahypar/preprocessor/repetition/enum_binary_params.hpp>
#include <boost_kahypar/preprocessor/repetition/repeat_from_to.hpp>

#include <boost_kahypar/preprocessor/variadic/to_seq.hpp>
#include <boost_kahypar/preprocessor/variadic/size.hpp>
...
#include <boost_kahypar/preprocessor/comparison/equal.hpp>

#include <boost_kahypar/bind/bind.hpp>
#include <boost_kahypar/type_traits/is_copy_constructible.hpp>

#include <boost_kahypar/test/tools/detail/print_helper.hpp>
#include <boost_kahypar/test/utils/string_cast.hpp>
...
boost_bind_rvalue_holder_helper_impl(T&& value, boost_kahypar::false_type /* is copy constructible */) {
    return parameter_holder<typename std::remove_reference<T>::type>(std::forward<T>(value));
}

template <class T>
T&& boost_bind_rvalue_holder_helper_impl(T&& value, boost_kahypar::true_type /* is copy constructible */) {
...
auto boost_bind_rvalue_holder_helper(T&& value)
  -> decltype(boost_bind_rvalue_holder_helper_impl(
                  std::forward<T>(value),
                  typename boost_kahypar::is_copy_constructible<typename std::remove_reference<T>::type>::type()))
{
    // need to use boost_kahypar::is_copy_constructible because std::is_copy_constructible is broken on MSVC12
...
public:
    ...
    // there is also a limit on boost_kahypar::bind
#define TC_MAKE(z,arity,_)                                                              \
    ...
    {                                                                                   \
        m_test_cases.push_back( new test_case( genTestCaseName(), m_tc_file, m_tc_line, \
           boost_kahypar::bind( &TestCase::template test_method<BOOST_PP_ENUM_PARAMS(arity,Arg)>,\
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
template<typename TestCase,typename DataSet>
boost_kahypar::shared_ptr<test_unit_generator> //test_case_gen<TestCase,DataSet>
make_test_case_gen( const_string tc_name, const_string tc_file, std::size_t tc_line, DataSet&& ds )
{
    return boost_kahypar::shared_ptr<test_unit_generator>(new test_case_gen<TestCase,DataSet>( tc_name, tc_file, tc_line, std::forward<DataSet>(ds) ));
}
#else
template<typename TestCase,typename DataSet>
...
#define BOOST_DATA_TEST_CONTEXT(r, _, param)  << BOOST_STRINGIZE(param) << " = " << boost_kahypar::test_tools::tt_detail::print_helper(param) << "; "
...
BOOST_AUTO_TEST_SUITE( test_name,                                       \
                       *boost_kahypar::unit_test::decorator::stack_decorator()) \
                                                                        \
BOOST_AUTO_TU_REGISTRAR( BOOST_PP_CAT(test_name, case) )(               \
    boost_kahypar::unit_test::data::ds_detail::make_test_case_gen<              \
                                      BOOST_PP_CAT(test_name, case)>(   \
          BOOST_STRINGIZE( test_name ),                                 \
          __FILE__, __LINE__,                                           \
          boost_kahypar::unit_test::data::ds_detail::seed{} ->* dataset ),      \
    boost_kahypar::unit_test::decorator::collector_t::instance() );             \
                                                                        \
