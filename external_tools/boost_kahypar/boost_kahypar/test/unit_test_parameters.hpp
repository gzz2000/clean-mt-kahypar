...
public:
    ...
    void            setup( const const_string& stream_name,
                           boost_kahypar::function<void ()> const &cleaner_callback = boost_kahypar::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = boost_kahypar::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(boost_kahypar::function<void ()> cleaner_callback)
        ...
        boost_kahypar::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    boost_kahypar::shared_ptr<callback_cleaner>   m_cleaner;
