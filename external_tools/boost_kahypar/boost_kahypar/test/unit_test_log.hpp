...
#include <boost_kahypar/test/detail/log_level.hpp>
#include <boost_kahypar/test/detail/fwd_decl.hpp>

#include <boost_kahypar/test/utils/wrap_stringstream.hpp>
#include <boost_kahypar/test/utils/lazy_ostream.hpp>
...
/// - exposing an interface to the test framework (as a boost_kahypar::unit_test::test_observer)
...
///   @c boost_kahypar::unit_test::unit_test_log.
///
/// Users/developers willing to implement their own formatter need to:
/// - implement a boost_kahypar::unit_test::unit_test_log_formatter that will output the desired format
/// - register the formatter during a eg. global fixture using the method @c set_formatter (though the framework singleton).
///
/// @warning this observer has a higher priority than the @ref boost_kahypar::unit_test::results_collector_t. This means
/// that the various @ref boost_kahypar::unit_test::test_results associated to each test unit may not be available at the time
/// the @c test_unit_start, @c test_unit_finish ... are called.
///
/// @see
/// - boost_kahypar::unit_test::test_observer
/// - boost_kahypar::unit_test::unit_test_log_formatter
...
#define BOOST_TEST_LOG_ENTRY( ll )                                                  \
    (::boost_kahypar::unit_test::unit_test_log                                              \
        << ::boost_kahypar::unit_test::log::begin( BOOST_TEST_L(__FILE__), __LINE__ ))(ll)  \
/**/
...
#define BOOST_TEST_FRAMEWORK_MESSAGE( M )                       \
   (::boost_kahypar::unit_test::unit_test_log                           \
        << ::boost_kahypar::unit_test::log::begin(                      \
                "boost.test framework",                         \
                0 ))                                     \
             ( ::boost_kahypar::unit_test::log_messages )               \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/


#define BOOST_TEST_MESSAGE( M )                                 \
    BOOST_TEST_LOG_ENTRY( ::boost_kahypar::unit_test::log_messages )    \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/
...
#define BOOST_TEST_PASSPOINT()                                  \
    ::boost_kahypar::unit_test::unit_test_log.set_checkpoint(           \
...
#define BOOST_TEST_CHECKPOINT( M )                              \
    ::boost_kahypar::unit_test::unit_test_log.set_checkpoint(           \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        (::boost_kahypar::wrap_stringstream().ref() << M).str() )       \
/**/
