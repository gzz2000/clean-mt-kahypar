...
// Boost.Test
#include <boost_kahypar/test/framework.hpp>
#include <boost_kahypar/test/execution_monitor.hpp>
#include <boost_kahypar/test/debug.hpp>
#include <boost_kahypar/test/unit_test_parameters.hpp>

#include <boost_kahypar/test/unit_test_log.hpp>
#include <boost_kahypar/test/unit_test_log_formatter.hpp>
#include <boost_kahypar/test/unit_test_monitor.hpp>
#include <boost_kahypar/test/results_collector.hpp>
#include <boost_kahypar/test/progress_monitor.hpp>
#include <boost_kahypar/test/results_reporter.hpp>
#include <boost_kahypar/test/test_framework_init_observer.hpp>

#include <boost_kahypar/test/tree/observer.hpp>
#include <boost_kahypar/test/tree/test_unit.hpp>
#include <boost_kahypar/test/tree/visitor.hpp>
#include <boost_kahypar/test/tree/traverse.hpp>
#include <boost_kahypar/test/tree/test_case_counter.hpp>
#include <boost_kahypar/test/tree/global_fixture.hpp>

#if BOOST_TEST_SUPPORT_TOKEN_ITERATOR
#include <boost_kahypar/test/utils/iterator/token_iterator.hpp>
#endif

#include <boost_kahypar/test/utils/foreach.hpp>
#include <boost_kahypar/test/utils/basic_cstring/io.hpp>
#include <boost_kahypar/test/utils/basic_cstring/compare.hpp>

#include <boost_kahypar/test/detail/global_typedef.hpp>
#include <boost_kahypar/test/detail/throw_exception.hpp>

// Boost
#include <boost_kahypar/test/utils/timer.hpp>
...
private:
    bool            filter_unit( test_unit const& tu )
    {
        ...
        return std::find_if( filters.begin(), filters.end(), bind( &component::pass, _1, boost_kahypar::ref(tu) ) ) != filters.end();
    }
...
public:
                                        ...
                                        random_generator_helper const * const p_random_generator = 0)
    {
        ...
        BOOST_TEST_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
            ...
            result = unit_test_monitor.execute_and_translate( boost_kahypar::bind( &test_unit_fixture::setup, F ) );
        ...
        if( result == unit_test_monitor_t::test_ok ) {
            ...
            boost_kahypar::unit_test::timer::timer tu_timer;
        ...
        if( !unit_test_monitor.is_critical_error( result ) ) {
            ...
            BOOST_TEST_REVERSE_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
                ut_detail::test_unit_id_restore restore_current_test_unit(m_curr_test_unit, tu.p_id);
                result = (std::min)( result, unit_test_monitor.execute_and_translate( boost_kahypar::bind( &test_unit_fixture::teardown, F ), 0 ) );
    ...
    boost_kahypar::execution_monitor m_aux_em;
...
setup_loggers()
{
    ...
    BOOST_TEST_I_TRY {
        ...
        if( !has_combined_logger ) {
            ...
            if( runtime_config::has( runtime_config::btrt_log_sink ) ) {
                ...
                boost_kahypar::function< void () > log_cleaner = boost_kahypar::bind( &unit_test_log_t::set_stream,
                                                                      &unit_test_log,
                                                                      boost_kahypar::ref(std::cout)
                                                                      );
            ...
            BOOST_TEST_FOREACH( const_string, current_multi_config, v_output_format ) {
                ...
                BOOST_TEST_FOREACH( std::string const&, current_config, v_processed_tokens ) {
                    ...
                    BOOST_TEST_I_ASSRT( format != OF_INVALID,
                                        boost_kahypar::runtime::access_to_missing_argument()
    ...
    BOOST_TEST_I_CATCH( boost_kahypar::runtime::init_error, ex ) {
        BOOST_TEST_SETUP_ASSERT( false, ex.msg );
    }
    BOOST_TEST_I_CATCH( boost_kahypar::runtime::input_error, ex ) {
        std::cerr << ex.msg << "\n\n";

        BOOST_TEST_I_THROW( framework::nothing_to_test( boost_kahypar::exit_exception_failure ) );
    }
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    if( runtime_config::has( runtime_config::btrt_report_sink ) ) {
        boost_kahypar::function< void () > report_cleaner = boost_kahypar::bind( &results_reporter::set_stream,
                                                                 boost_kahypar::ref(std::cerr)
                                                                );
    ...
    BOOST_TEST_I_TRY {
        s_frk_state().m_aux_em.vexecute( boost_kahypar::bind( &impl::invoke_init_func, init_func ) );
    }
...
int
add_context( ::boost_kahypar::unit_test::lazy_ostream const& context_descr, bool sticky )
{
    ...
    if( call_start_finish ) {
        ...
        BOOST_TEST_FOREACH( test_observer*, to, impl::s_frk_state().m_observers ) {
            BOOST_TEST_I_TRY {
                ...
                unit_test_monitor_t::error_level result = unit_test_monitor.execute_and_translate( boost_kahypar::bind( &test_observer::test_start, to, tcc.p_count, id ) );
                if( init_ok ) {
                    if( result != unit_test_monitor_t::test_ok ) {
                        init_ok = false;
                    }
