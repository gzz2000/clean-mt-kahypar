...
// Boost.Test
#include <boost_kahypar/test/output/junit_log_formatter.hpp>
...
#include <boost_kahypar/test/utils/xml_printer.hpp>
...
public:
    ...
    std::list<std::string> build_skipping_chain(test_unit const & tu) const
    {
        ...
        while( id != m_ts.p_id && id != INV_TEST_UNIT_ID) {
            test_unit const& tu_hierarchy = boost_kahypar::unit_test::framework::get( id, TUT_ANY );
    ...
    std::string get_class_name(test_unit const & tu_class) const {
        ...
        while( id != m_ts.p_id && id != INV_TEST_UNIT_ID ) {
            test_unit const& tu = boost_kahypar::unit_test::framework::get( id, TUT_ANY );
...
junit_log_formatter::log_finish( std::ostream& ostr )
{
    ...
    if(!map_tests.empty()) {
        test_unit* root = &boost_kahypar::unit_test::framework::get( map_tests.begin()->first, TUT_ANY );
        ...
        while(root->p_parent_id != INV_TEST_UNIT_ID && map_tests.count(root->p_parent_id) > 0) {
            root = &boost_kahypar::unit_test::framework::get( root->p_parent_id, TUT_ANY );
        }
...
junit_log_formatter::test_unit_finish( std::ostream& /*ostr*/, test_unit const& tu, unsigned long /*elapsed*/ )
{
    ...
    boost_kahypar::ignore_unused( tu );
