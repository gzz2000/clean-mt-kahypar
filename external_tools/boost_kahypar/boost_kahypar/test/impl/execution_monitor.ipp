...
// Boost.Test
#include <boost_kahypar/test/detail/config.hpp>
...
// Boost
#include <boost_kahypar/cstdlib.hpp>    // for exit codes
#include <boost_kahypar/config.hpp>     // for workarounds
#include <boost_kahypar/core/ignore_unused.hpp> // for ignore_unused
#ifndef BOOST_NO_EXCEPTIONS
#include <boost_kahypar/exception/get_error_info.hpp> // for get_error_info
#include <boost_kahypar/exception/current_exception_cast.hpp> // for current_exception_cast
#include <boost_kahypar/exception/diagnostic_information.hpp>
#endif
...
#  include <boost_kahypar/core/demangle.hpp>
#endif
...
extract( boost_kahypar::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = boost_kahypar::get_error_info<ErrorInfo>( *ex );
...
report_error( execution_exception::error_code ec, boost_kahypar::exception const* be, char const* format, va_list* args )
{
...
report_error( execution_exception::error_code ec, boost_kahypar::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return boost_kahypar::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    boost_kahypar::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         boost_kahypar::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        boost_kahypar::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( boost_kahypar::function<int ()> const& F )
{
...
int
execution_monitor::execute( boost_kahypar::function<int ()> const& F )
{
    ...
    // boost_kahypar::exception (before std::exception, with extended diagnostic)
    catch( boost_kahypar::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", boost_kahypar::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<boost_kahypar::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( boost_kahypar::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    boost_kahypar::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( boost_kahypar::function<void ()> const& F )
{
...
{
    boost_kahypar::ignore_unused(mask);
