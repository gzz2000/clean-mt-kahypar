...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_I_TRY {
        boost_kahypar::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ...
        if( result == 0 )
            result = ::boost_kahypar::exit_success;
        else if( result != ::boost_kahypar::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::boost_kahypar::exit_failure;
        }
    }
    BOOST_TEST_I_CATCH( ::boost_kahypar::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::boost_kahypar::exit_exception_failure;
    }
    BOOST_TEST_I_CATCH( ::boost_kahypar::system_error, ex ) {
    ...
    if( result != ::boost_kahypar::exit_success ) {
        ...
        ::boost_kahypar::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
...
{
    return ::boost_kahypar::prg_exec_monitor_main( &cpp_main, argc, argv );
}
