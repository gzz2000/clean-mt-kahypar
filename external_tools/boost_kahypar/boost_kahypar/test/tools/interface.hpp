...
// Boost.Test
#include <boost_kahypar/test/unit_test_log.hpp>
#ifdef BOOST_TEST_TOOLS_DEBUGGABLE
#include <boost_kahypar/test/debug.hpp>
#endif

#include <boost_kahypar/test/detail/pp_variadic.hpp>
...
#define BOOST_TEST_BUILD_ASSERTION( P )             \
    (::boost_kahypar::test_tools::assertion::seed()->*P)    \
/**/
...
do {                                                            \
    ::boost_kahypar::test_tools::assertion_result res = (P);            \
    report_assertion(                                           \
        res,                                                    \
        BOOST_TEST_LAZY_MSG( M ),                               \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        ::boost_kahypar::test_tools::tt_detail::level,                  \
        ::boost_kahypar::test_tools::tt_detail::CHECK_MSG,              \
        0 );                                                    \
...
do {                                                            \
    ...
    report_assertion(                                           \
      ...
      static_cast<std::size_t>(__LINE__),                       \
      ::boost_kahypar::test_tools::tt_detail::level,                    \
      ::boost_kahypar::test_tools::tt_detail::CHECK_BUILT_ASSERTION,    \
      0 );                                                      \
} while( ::boost_kahypar::test_tools::tt_detail::dummy_cond() )         \
/**/
...
do {                                                            \
    ...
    ::boost_kahypar::test_tools::tt_detail::                            \
    report_assertion(                                           \
      ::boost_kahypar::test_tools::tt_detail::assertion_evaluate(       \
        BOOST_TEST_BUILD_ASSERTION( P ) )                       \
          << arg,                                               \
      ::boost_kahypar::test_tools::tt_detail::assertion_text(           \
          BOOST_TEST_LAZY_MSG( BOOST_TEST_STRINGIZE(P) ),       \
          BOOST_TEST_LAZY_MSG( arg ) ),                         \
      BOOST_TEST_L(__FILE__),                                   \
      static_cast<std::size_t>(__LINE__),                       \
      ::boost_kahypar::test_tools::tt_detail::level,                    \
      ::boost_kahypar::test_tools::tt_detail::assertion_type()          \
          << arg,                                               \
      0 );                                                      \
} while( ::boost_kahypar::test_tools::tt_detail::dummy_cond() )         \
/**/
...
#define BOOST_TEST_TOOL_UNIV( level, P )                                    \
do {                                                                        \
    if( ::boost_kahypar::debug::under_debugger() )                                  \
        BOOST_TEST_TOOL_DIRECT_IMPL( P, level, BOOST_TEST_STRINGIZE( P ) ); \
    else                                                                    \
...
#define BOOST_TEST_IS_DEFINED( symb ) ::boost_kahypar::test_tools::tt_detail::is_defined_impl( symb, BOOST_STRINGIZE(= symb) )
...
#define BOOST_WARN_CLOSE( L, R, T )         BOOST_TEST_WARN( L == R, T % ::boost_kahypar::test_tools::tolerance() )
#define BOOST_CHECK_CLOSE( L, R, T )        BOOST_TEST_CHECK( L == R, T % ::boost_kahypar::test_tools::tolerance() )
#define BOOST_REQUIRE_CLOSE( L, R, T )      BOOST_TEST_REQUIRE( L == R, T % ::boost_kahypar::test_tools::tolerance() )

#define BOOST_WARN_CLOSE_FRACTION(L, R, T)  BOOST_TEST_WARN( L == R, ::boost_kahypar::test_tools::tolerance( T ) )
#define BOOST_CHECK_CLOSE_FRACTION(L, R, T) BOOST_TEST_CHECK( L == R, ::boost_kahypar::test_tools::tolerance( T ) )
#define BOOST_REQUIRE_CLOSE_FRACTION(L,R,T) BOOST_TEST_REQUIRE( L == R, ::boost_kahypar::test_tools::tolerance( T ) )

#define BOOST_WARN_SMALL( FPV, T )          BOOST_TEST_WARN( FPV == 0., ::boost_kahypar::test_tools::tolerance( T ) )
#define BOOST_CHECK_SMALL( FPV, T )         BOOST_TEST_CHECK( FPV == 0., ::boost_kahypar::test_tools::tolerance( T ) )
#define BOOST_REQUIRE_SMALL( FPV, T )       BOOST_TEST_REQUIRE( FPV == 0., ::boost_kahypar::test_tools::tolerance( T ) )

//____________________________________________________________________________//

#define BOOST_WARN_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_WARN( ::boost_kahypar::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                     ::boost_kahypar::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                     ::boost_kahypar::test_tools::per_element() )                           \
/**/

#define BOOST_CHECK_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_CHECK( ::boost_kahypar::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                      ::boost_kahypar::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                      ::boost_kahypar::test_tools::per_element() )                           \
/**/

#define BOOST_REQUIRE_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_REQUIRE( ::boost_kahypar::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                        ::boost_kahypar::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                        ::boost_kahypar::test_tools::per_element() )                           \
/**/
...
#define BOOST_WARN_BITWISE_EQUAL( L, R )    BOOST_TEST_WARN( L == R, ::boost_kahypar::test_tools::bitwise() )
#define BOOST_CHECK_BITWISE_EQUAL( L, R )   BOOST_TEST_CHECK( L == R, ::boost_kahypar::test_tools::bitwise() )
#define BOOST_REQUIRE_BITWISE_EQUAL( L, R ) BOOST_TEST_REQUIRE( L == R, ::boost_kahypar::test_tools::bitwise() )
