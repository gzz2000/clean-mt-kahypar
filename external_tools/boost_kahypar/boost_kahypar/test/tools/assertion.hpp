...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(boost_kahypar::declval<Lhs>() oper boost_kahypar::declval<Rhs>() ) optype; \
    typedef typename boost_kahypar::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _, _i )                     \
    template<typename T>                                        \
    binary_expr<ExprType,typename boost_kahypar::decay<T const>::type,  \
        op::name<ValType,typename boost_kahypar::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename boost_kahypar::decay<T const>::type Rhs;       \
