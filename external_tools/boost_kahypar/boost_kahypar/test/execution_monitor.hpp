...
#include <boost_kahypar/test/detail/fwd_decl.hpp>
#include <boost_kahypar/test/detail/throw_exception.hpp>

#include <boost_kahypar/test/utils/class_properties.hpp>

// Boost
#include <boost_kahypar/shared_ptr.hpp>
#include <boost_kahypar/scoped_array.hpp>
#include <boost_kahypar/type.hpp>
#include <boost_kahypar/cstdlib.hpp>
#include <boost_kahypar/function/function0.hpp>
...
#define MCW_EM _MCW_EM
  ...
  #include <boost_kahypar/detail/fenv.hpp>
#endif
...
///
/// All symbols in the Execution Monitor implementation are located in the namespace boost_kahypar. To use the Execution Monitor you need to:
...
typedef boost_kahypar::shared_ptr<translator_holder_base> translator_holder_base_ptr;

class BOOST_TEST_DECL translator_holder_base {
protected:
    typedef boost_kahypar::unit_test::const_string const_string;
public:
    // Constructor
    ...
    virtual int operator()( boost_kahypar::function<int ()> const& F ) = 0;
    ...
    translator_holder_base_ptr erase( translator_holder_base_ptr this_, boost_kahypar::type<ExceptionType>* = 0 )
    {
        if( m_next )
...
public:
    ...
    /// @see vexecute
    int         execute( boost_kahypar::function<int ()> const& F );
    ...
    /// @param[in] F  Function to monitor
    /// @see execute
    void         vexecute( boost_kahypar::function<void ()> const& F );
    // @}
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, const_string tag = const_string(), boost_kahypar::type<ExceptionType>* = 0 );
    ...
    template<typename ExceptionType>
    void        erase_exception_translator( boost_kahypar::type<ExceptionType>* = 0 )
    {
...
private:
    ...
    int         catch_signals( boost_kahypar::function<int ()> const& F );

    // Data members
    detail::translator_holder_base_ptr  m_custom_translators;
    boost_kahypar::scoped_array<char>           m_alt_stack;
}; // execution_monitor
...
public:
    ...
    // translator holder interface
    int operator()( boost_kahypar::function<int ()> const& F ) BOOST_OVERRIDE
    {
        BOOST_TEST_I_TRY {
        ...
        BOOST_TEST_I_CATCH( ExceptionType, e ) {
            m_translator( e );
            return boost_kahypar::exit_exception_failure;
        }
    }
#ifndef BOOST_NO_RTTI
...
template<typename ExceptionType, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, const_string tag, boost_kahypar::type<ExceptionType>* )
{
...
//!@internal
#define BOOST_TEST_SYS_ASSERT( cond ) BOOST_TEST_I_ASSRT( cond, ::boost_kahypar::system_error( BOOST_STRINGIZE( exp ) ) )
