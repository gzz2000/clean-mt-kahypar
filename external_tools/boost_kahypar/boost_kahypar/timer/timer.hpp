...
#include <boost_kahypar/timer/config.hpp>
...
namespace timer
{
  ...
  typedef boost_kahypar::int_least64_t nanosecond_type;
...
//  cpu_timer  -------------------------------------------------------------------------//
  ...
  public:
    ...
    std::string   format(short places, const std::string& format) const
                        { return ::boost_kahypar::timer::format(elapsed(), places, format); }
    std::string   format(short places = default_places) const
                        { return ::boost_kahypar::timer::format(elapsed(), places); }
    //  actions
