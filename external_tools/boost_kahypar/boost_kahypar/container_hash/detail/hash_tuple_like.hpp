...
#include <boost_kahypar/container_hash/is_tuple_like.hpp>
...
{
    using std::get;
    boost_kahypar::hash_combine( seed, get<I>( v ) );

    boost_kahypar::hash_detail::hash_combine_tuple_like<I + 1>( seed, v );
}
...
inline std::size_t hash_tuple_like( T const& v )
{
    ...
    boost_kahypar::hash_detail::hash_combine_tuple_like<0>( seed, v );
...
{
    return boost_kahypar::hash_detail::hash_tuple_like( v );
}
