...
#include <boost_kahypar/container_hash/hash_fwd.hpp>
#include <boost_kahypar/container_hash/is_range.hpp>
#include <boost_kahypar/container_hash/is_contiguous_range.hpp>
#include <boost_kahypar/container_hash/is_unordered_range.hpp>
#include <boost_kahypar/container_hash/is_described_class.hpp>
#include <boost_kahypar/container_hash/detail/hash_integral.hpp>
#include <boost_kahypar/container_hash/detail/hash_tuple_like.hpp>
#include <boost_kahypar/container_hash/detail/hash_mix.hpp>
#include <boost_kahypar/container_hash/detail/hash_range.hpp>
#include <boost_kahypar/describe/bases.hpp>
#include <boost_kahypar/describe/members.hpp>
#include <type_traits>
#include <cstdint>

#if defined(BOOST_DESCRIBE_CXX14)
# include <boost_kahypar/mp11/algorithm.hpp>
#endif
...
namespace boost_kahypar
{
        ...
        hash_value( T v )
    {
        ...
        //
        // boost_kahypar::hash_value( to_underlying(v) );
        //
    ...
    typename std::enable_if<std::is_floating_point<T>::value, std::size_t>::type
        hash_value( T v )
    {
        return boost_kahypar::hash_detail::hash_float_impl<T>::fn( v + 0 );
    }
    ...
    template <class T> std::size_t hash_value( T* const& v )
    {
        ...
        return boost_kahypar::hash_value( x + (x >> 3) );
    }
    ...
    {
        return boost_kahypar::hash_range( x, x + N );
    }
    ...
    {
        std::size_t re = boost_kahypar::hash<T>()( v.real() );
        std::size_t im = boost_kahypar::hash<T>()( v.imag() );
    ...
    std::size_t hash_value( std::pair<A, B> const& v )
    {
        ...
        boost_kahypar::hash_combine( seed, v.first );
        boost_kahypar::hash_combine( seed, v.second );
    ...
    {
        return boost_kahypar::hash_range( v.begin(), v.end() );
    }
    ...
    typename std::enable_if<container_hash::is_contiguous_range<T>::value, std::size_t>::type
        hash_value( T const& v )
    {
        return boost_kahypar::hash_range( v.data(), v.data() + v.size() );
    }
    ...
    typename std::enable_if<container_hash::is_unordered_range<T>::value, std::size_t>::type
        hash_value( T const& v )
    {
        return boost_kahypar::hash_unordered_range( v.begin(), v.end() );
    }
...
# pragma warning(disable: 4100) // unreferenced formal parameter
        ...
        hash_value( T const& v )
    {
        ...
        mp11::mp_for_each<Bd>([&](auto D){
            ...
            boost_kahypar::hash_combine( r, (B const&)v );
        ...
        mp11::mp_for_each<Md>([&](auto D){
            ...
            boost_kahypar::hash_combine( r, v.*D.pointer );
...
# pragma warning(pop)
    ...
    {
        return boost_kahypar::hash_value( x.get() );
    }
    ...
    inline std::size_t hash_value( std::error_code const& v )
    {
        ...
        boost_kahypar::hash_combine( seed, v.value() );
        boost_kahypar::hash_combine( seed, &v.category() );
    ...
    typename std::enable_if<std::is_same<T, std::nullptr_t>::value, std::size_t>::type
        hash_value( T const& /*v*/ )
    {
        return boost_kahypar::hash_value( static_cast<void*>( nullptr ) );
    }
    ...
    //
    // boost_kahypar::hash_combine
    //

    template <class T>
    inline void hash_combine( std::size_t& seed, T const& v )
    {
        seed = boost_kahypar::hash_detail::hash_mix( seed + 0x9e3779b9 + boost_kahypar::hash<T>()( v ) );
    }

    //
    // boost_kahypar::hash_range
    //
    ...
    //
    // boost_kahypar::hash_unordered_range
    //
    ...
    template<class E, class T, class A> struct hash< std::basic_string<E, T, A> >
    {
        ...
        {
            return boost_kahypar::hash_value( val );
        }
    };

#endif

    // boost_kahypar::unordered::hash_is_avalanching

    namespace unordered
    {
        template<class T> struct hash_is_avalanching;
        template<class Ch> struct hash_is_avalanching< boost_kahypar::hash< std::basic_string<Ch> > >: std::is_integral<Ch> {};

#if !defined(BOOST_NO_CXX17_HDR_STRING_VIEW)

        template<class Ch> struct hash_is_avalanching< boost_kahypar::hash< std::basic_string_view<Ch> > >: std::is_integral<Ch> {};
