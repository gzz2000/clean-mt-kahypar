...
#include <boost_kahypar/range/atl.hpp>
#include <boost_kahypar/range/begin.hpp>
#include <boost_kahypar/range/const_iterator.hpp>
#include <boost_kahypar/range/detail/microsoft.hpp>
...
namespace boost_kahypar { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit mfc_ptr_array_iterator(ArrayT& arr, INT_PTR index) :
            super_t(index), m_parr(boost_kahypar::addressof(arr))
        { }
    ...
    public:
        ...
        explicit mfc_map_iterator(MapT const& map, POSITION pos) :
            m_pmap(boost_kahypar::addressof(map)), m_posNext(pos)
        {
            increment();
        }
    ...
    public:
        ...
        explicit mfc_cpair_map_iterator(MapT& map, PairT *pp) :
            m_pmap(boost_kahypar::addressof(map)), m_pp(pp)
        { }
    ...
    struct customization< ::CTypedPtrArray<BaseClass, PtrType> >
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(boost_kahypar::begin<BaseClass>(x), fun<X>());
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(boost_kahypar::end<BaseClass>(x), fun<X>());
        }
    };
...
BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE(
    boost_kahypar::range_detail_microsoft::using_type_as_tag,
