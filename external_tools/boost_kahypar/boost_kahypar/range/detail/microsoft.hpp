...
#include <boost_kahypar/iterator/iterator_traits.hpp> // iterator_difference
#include <boost_kahypar/mpl/identity.hpp>
#include <boost_kahypar/mpl/if.hpp>
#include <boost_kahypar/preprocessor/cat.hpp>
#include <boost_kahypar/preprocessor/control/iif.hpp>
#include <boost_kahypar/preprocessor/comma_if.hpp>
#include <boost_kahypar/preprocessor/detail/is_unary.hpp>
#include <boost_kahypar/preprocessor/list/for_each.hpp>
#include <boost_kahypar/preprocessor/repetition/enum_params.hpp>
#include <boost_kahypar/preprocessor/repetition/repeat.hpp>
#include <boost_kahypar/preprocessor/seq/for_each_i.hpp>
#include <boost_kahypar/preprocessor/seq/size.hpp>
#include <boost_kahypar/preprocessor/tuple/eat.hpp>
#include <boost_kahypar/range/const_iterator.hpp>
...
#include <boost_kahypar/utility/enable_if.hpp> // disable_if
#include <boost_kahypar/next_prior.hpp>

#if !defined(BOOST_RANGE_DETAIL_MICROSOFT_RANGE_VERSION_1)
    #include <boost_kahypar/range/mutable_iterator.hpp>
#else
...
namespace boost_kahypar { namespace range_detail_microsoft {
    ...
    size_of(T const& x)
    {
        return std::distance(boost_kahypar::begin(x), boost_kahypar::end(x));
    }
...
} } // namespace boost_kahypar::range_detail_microsoft
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin(Fullname) \
        inline \
        boost_kahypar::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return boost_kahypar::range_detail_microsoft::begin_of(x); \
        } \
    /**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin_const(Fullname) \
        inline \
        boost_kahypar::range_detail_microsoft::const_iterator_of< Fullname >::type \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_end(Fullname) \
        ...
        { \
            return boost_kahypar::range_detail_microsoft::end_of(x); \
        } \
    /**/
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_size(Fullname) \
            inline \
            boost_kahypar::range_detail_microsoft::size_type_of< Fullname >::type \
            boost_range_size(Fullname const& x) \
            { \
                return boost_kahypar::range_detail_microsoft::size_of(x); \
            } \
        /**/
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin(Params, Fullname) \
        ...
        typename boost_kahypar::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return boost_kahypar::range_detail_microsoft::begin_of(x); \
        } \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin_const(Params, Fullname) \
        template< Params > inline \
        typename boost_kahypar::range_detail_microsoft::const_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname const& x) \
        { \
            return boost_kahypar::range_detail_microsoft::begin_of(x); \
        } \
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_size(Params, Fullname) \
            template< Params > inline \
            typename boost_kahypar::range_detail_microsoft::size_type_of< Fullname >::type \
            boost_range_size(Fullname const& x) \
            { \
                return boost_kahypar::range_detail_microsoft::size_of(x); \
            } \
...
namespace boost_kahypar { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit list_iterator(ListT& lst, POSITION pos) :
            m_plst(boost_kahypar::addressof(lst)), m_pos(pos)
        { }
...
#include <boost_kahypar/range/empty.hpp>
...
namespace boost_kahypar { namespace range_detail_microsoft {
    ...
    {
        return
            boost_kahypar::distance(rng1) == boost_kahypar::distance(rng2) &&
            std::equal(boost_kahypar::begin(rng1), boost_kahypar::end(rng1), boost_kahypar::begin(rng2))
        ;
    }
    ...
    {
        typedef typename boost_kahypar::range_const_iterator<AssocContainer>::type iter_t;
        for (iter_t it = boost_kahypar::const_begin(ac), last = boost_kahypar::const_end(ac); it != last; ++it) {
    ...
    bool test_emptiness(Range& )
    {
        ...
        Range emptyRng;
        result = result && boost_kahypar::empty(emptyRng);
    ...
    bool test_trivial(Range& rng)
    {
        ...
        citer_t cit = boost_kahypar::begin(rng);
        (void)cit; // unused

        // mutability check
        typedef typename range_value<Range>::type val_t;
        val_t v = *boost_kahypar::begin(rng);
        *boost_kahypar::begin(rng) = v;
        result = result && *boost_kahypar::begin(rng) == v;
    ...
    {
        boost_kahypar::function_requires< ForwardRangeConcept<Range> >();
        ...
        std::copy(boost_kahypar::begin(rng), boost_kahypar::end(rng), std::back_inserter(saved));
        std::rotate(boost_kahypar::begin(saved), boost_kahypar::next(boost_kahypar::begin(saved)), boost_kahypar::end(saved));

        std::rotate(boost_kahypar::begin(rng), boost_kahypar::next(boost_kahypar::begin(rng)), boost_kahypar::end(rng));
    ...
    {
        boost_kahypar::function_requires< BidirectionalRangeConcept<Range> >();
        ...
        result = result && (test_equals)(
            boost_kahypar::make_iterator_range(boost_kahypar::rbegin(saved), boost_kahypar::rend(saved)),
            boost_kahypar::make_iterator_range(boost_kahypar::rbegin(rng), boost_kahypar::rend(rng))
        );
    ...
    {
        boost_kahypar::function_requires< RandomAccessRangeConcept<Range> >();
        ...
        std::vector<val_t> saved;
        std::copy(boost_kahypar::begin(rng), boost_kahypar::end(rng), std::back_inserter(saved));
        std::sort(boost_kahypar::begin(saved), boost_kahypar::end(saved));

        std::random_shuffle(boost_kahypar::begin(rng), boost_kahypar::end(rng));
        std::sort(boost_kahypar::begin(rng), boost_kahypar::end(rng));
        result = result && (test_equals)(rng, saved);

        std::random_shuffle(boost_kahypar::begin(rng), boost_kahypar::end(rng));
        std::stable_sort(boost_kahypar::begin(rng), boost_kahypar::end(rng));
        result = result && (test_equals)(rng, saved);

        std::random_shuffle(boost_kahypar::begin(rng), boost_kahypar::end(rng));
        std::partial_sort(boost_kahypar::begin(rng), boost_kahypar::end(rng), boost_kahypar::end(rng));
        result = result && (test_equals)(rng, saved);
    ...
    bool test_init_array(ArrayT& arr, SampleRange const& sample)
    {
        ...
        for (iter_t it = boost_kahypar::const_begin(sample), last = boost_kahypar::const_end(sample); it != last; ++it) {
    ...
    bool test_init_map(MapT& map, SampleMap const& sample)
    {
        ...
        return boost_kahypar::distance(map) == boost_kahypar::distance(sample);
    }
    ...
    struct test_mutable_iter :
        boost_kahypar::is_same< typename boost_kahypar::BOOST_RANGE_DETAIL_MICROSOFT_range_mutable_iterator<Range>::type, Iter >
    { };


    template< class Range, class Iter >
    struct test_const_iter :
        boost_kahypar::is_same< typename boost_kahypar::range_const_iterator<Range>::type, Iter >
    { };
