...
unique_copy( const SinglePassRange& rng, OutputIterator out_it )
{
    ...
    return std::unique_copy(boost_kahypar::begin(rng), boost_kahypar::end(rng), out_it);
}
/// \overload
...
{
    BOOST_RANGE_CONCEPT_ASSERT(( SinglePassRangeConcept<const SinglePassRange> ));
    return std::unique_copy(boost_kahypar::begin(rng), boost_kahypar::end(rng), out_it, pred);
}
