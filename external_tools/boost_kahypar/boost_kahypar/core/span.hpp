...
struct span_capacity {
    static constexpr bool value = E == boost_kahypar::dynamic_extent || E == N;
};
...
struct span_is_span<boost_kahypar::span<T, E> > {
...
template<class T>
using span_ptr = decltype(boost_kahypar::data(std::declval<T&>()));
...
struct span_implicit {
    static constexpr bool value = E == boost_kahypar::dynamic_extent ||
        N != boost_kahypar::dynamic_extent;
};

template<class T, std::size_t E, class U, std::size_t N>
struct span_copyable {
    static constexpr bool value = (N == boost_kahypar::dynamic_extent ||
        span_capacity<E, N>::value) && span_convertible<U, T>::value;
};

template<std::size_t E, std::size_t O>
struct span_sub {
    static constexpr std::size_t value = E == boost_kahypar::dynamic_extent ?
        boost_kahypar::dynamic_extent : E - O;
};
...
template<class T>
struct span_store<T, boost_kahypar::dynamic_extent> {
...
template<class T>
struct span_bytes<T, boost_kahypar::dynamic_extent> {
    static constexpr std::size_t value = boost_kahypar::dynamic_extent;
};
...
public:
    ...
    constexpr span(R&& r) noexcept(noexcept(boost_kahypar::data(r)) &&
        noexcept(r.size()))
        : s_(boost_kahypar::data(r), r.size()) { }

    template<class R,
        typename std::enable_if<E != dynamic_extent &&
            detail::span_is_range<R, T>::value, int>::type = 0>
    explicit constexpr span(R&& r) noexcept(noexcept(boost_kahypar::data(r)) &&
        noexcept(r.size()))
        : s_(boost_kahypar::data(r), r.size()) { }
