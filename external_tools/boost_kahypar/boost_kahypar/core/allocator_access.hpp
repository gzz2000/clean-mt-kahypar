...
struct alloc_ptr {
    typedef typename boost_kahypar::allocator_value_type<A>::type* type;
};
...
struct alloc_const_ptr {
    typedef typename boost_kahypar::pointer_traits<typename
        boost_kahypar::allocator_pointer<A>::type>::template rebind_to<const typename
            boost_kahypar::allocator_value_type<A>::type>::type type;
};
...
struct alloc_void_ptr {
     typedef typename boost_kahypar::pointer_traits<typename
        boost_kahypar::allocator_pointer<A>::type>::template
...
struct alloc_diff_type {
    typedef typename boost_kahypar::pointer_traits<typename
        boost_kahypar::allocator_pointer<A>::type>::difference_type type;
};
...
struct alloc_size_type {
    typedef typename std::make_unsigned<typename
        boost_kahypar::allocator_difference_type<A>::type>::type type;
};
#endif
...
class alloc_has_allocate {
    ...
    -> alloc_no<decltype(std::declval<O&>().allocate(std::declval<typename
        boost_kahypar::allocator_size_type<A>::type>(), std::declval<typename
            boost_kahypar::allocator_const_void_pointer<A>::type>()))>;
...
class alloc_has_max_size {
    template<class O>
    static alloc_no<typename boost_kahypar::allocator_size_type<O>::type(O::*)(),
        &O::max_size> check(int);

    template<class O>
    static alloc_no<typename boost_kahypar::allocator_size_type<O>::type(O::*)() const,
        &O::max_size> check(int);

    template<class O>
    static alloc_no<typename boost_kahypar::allocator_size_type<O>::type(*)(),
        &O::max_size> check(int);
...
{
    while (n > 0) {
        boost_kahypar::allocator_destroy(a, p + --n);
    }
}
...
public:
    ...
    ~alloc_destroyer() {
        boost_kahypar::allocator_destroy_n(a_, p_, n_);
    }
...
allocator_construct_n(A& a, T* p, std::size_t n)
{
    ...
    for (std::size_t& i = d.size(); i < n; ++i) {
        boost_kahypar::allocator_construct(a, p + i);
    }
    d.size() = 0;
}
...
allocator_construct_n(A& a, T* p, std::size_t n, const T* l, std::size_t m)
{
    detail::alloc_destroyer<A, T> d(a, p);
    for (std::size_t& i = d.size(); i < n; ++i) {
        boost_kahypar::allocator_construct(a, p + i, l[i % m]);
    }
    d.size() = 0;
}
...
allocator_construct_n(A& a, T* p, std::size_t n, I b)
{
    ...
    for (std::size_t& i = d.size(); i < n; void(++i), void(++b)) {
        boost_kahypar::allocator_construct(a, p + i, *b);
    }
    d.size() = 0;
}
