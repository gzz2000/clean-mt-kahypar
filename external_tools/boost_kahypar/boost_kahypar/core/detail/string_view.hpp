...
// boost_kahypar::core::basic_string_view<Ch>
//
...
// forward declaration of boost_kahypar::hash_range from ContainerHash
...
public:
    ...
    template<class End> BOOST_CXX14_CONSTEXPR basic_string_view( Ch const* first, End last,
        typename boost_kahypar::enable_if<boost_kahypar::core::detail::is_same<End, Ch const*> >::type* = 0 ) BOOST_NOEXCEPT: p_( first ), n_( static_cast<size_type>( last - first ) )
    {
    ...
    template<class Ch2> basic_string_view( boost_kahypar::basic_string_view<Ch2, std::char_traits<Ch2> > const& str,
        typename boost_kahypar::enable_if<boost_kahypar::core::detail::is_same<Ch, Ch2> >::type* = 0 ) BOOST_NOEXCEPT: p_( str.data() ), n_( str.size() )
    {
    }
...
# endif
    ...
    template<class Ch2, class En = typename boost_kahypar::enable_if<boost_kahypar::core::detail::is_same<Ch2, Ch> >::type>
    ...
    template<class Ch2> operator boost_kahypar::basic_string_view<Ch2,
        typename boost_kahypar::enable_if<boost_kahypar::core::detail::is_same<Ch2, Ch>, std::char_traits<Ch> >::type> () const BOOST_NOEXCEPT
    {
        return boost_kahypar::basic_string_view< Ch, std::char_traits<Ch> >( data(), size() );
    }
    ...
    BOOST_CXX14_CONSTEXPR const_reference at( size_type pos ) const
    {
        if( pos >= size() )
        {
            boost_kahypar::throw_exception( std::out_of_range( "basic_string_view::at" ), BOOST_CURRENT_LOCATION );
        }
    ...
    BOOST_CXX14_CONSTEXPR size_type copy( Ch* s, size_type n, size_type pos = 0 ) const
    {
        if( pos > size() )
        {
            boost_kahypar::throw_exception( std::out_of_range( "basic_string_view::copy" ), BOOST_CURRENT_LOCATION );
        }
    ...
    BOOST_CXX14_CONSTEXPR basic_string_view substr( size_type pos = 0, size_type n = npos ) const
    {
        if( pos > size() )
        {
            boost_kahypar::throw_exception( std::out_of_range( "basic_string_view::substr" ), BOOST_CURRENT_LOCATION );
        }
    ...
    {
        return boost_kahypar::hash_range( sv.begin(), sv.end() );
    }
};
...
struct std::basic_common_reference<
    ...
    Q1, Q2>
{
    using type = boost_kahypar::core::basic_string_view<Ch>;
};
