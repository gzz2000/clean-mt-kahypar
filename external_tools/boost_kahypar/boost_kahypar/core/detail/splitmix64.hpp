...
public:
    ...
    explicit splitmix64( boost_kahypar::uint64_t seed ): x_( seed )
    {
    }

    boost_kahypar::uint64_t operator()()
    {
        x_ += ( boost_kahypar::uint64_t(0x9e3779b9u) << 32 ) + 0x7f4a7c15u;

        boost_kahypar::uint64_t z = x_;

        z ^= z >> 30;
        z *= ( boost_kahypar::uint64_t(0xbf58476du) << 32 ) + 0x1ce4e5b9u;
        z ^= z >> 27;
        z *= ( boost_kahypar::uint64_t(0x94d049bbu) << 32 ) + 0x133111ebu;
        z ^= z >> 31;
