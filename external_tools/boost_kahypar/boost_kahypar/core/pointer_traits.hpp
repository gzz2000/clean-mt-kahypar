...
struct ptr_to<T*, T, true> {
    static T* pointer_to(T& v) BOOST_NOEXCEPT {
        return boost_kahypar::addressof(v);
    }
};
...
-> decltype(boost_kahypar::pointer_traits<T>::to_address(v))
{
    return boost_kahypar::pointer_traits<T>::to_address(v);
}
...
{
    return boost_kahypar::detail::ptr_address(v.operator->(), 0);
}
...
{
    return boost_kahypar::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return boost_kahypar::to_address(v.operator->());
}
#endif
