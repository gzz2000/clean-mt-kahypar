...
/// pointer types.
///
/// boost_kahypar::typeindex::runtime_cast can be used to emulate dynamic_cast
...
T* runtime_cast_impl(U* u, std::integral_constant<bool, false>) noexcept {
    return const_cast<T*>(static_cast<T const*>(
        u->boost_type_index_find_instance_(boost_kahypar::typeindex::type_id<T>())
    ));
}

template<typename T, typename U>
T const* runtime_cast_impl(U const* u, std::integral_constant<bool, false>) noexcept {
    return static_cast<T const*>(u->boost_type_index_find_instance_(boost_kahypar::typeindex::type_id<T>()));
}
