...
inline type_index runtime_class_construct_type_id(T const*) {
    return boost_kahypar::typeindex::type_id<T>();
}

template <class Self>
constexpr const void* find_instance(boost_kahypar::typeindex::type_index const&, const Self*) noexcept {
    return nullptr;
}

template <class Base, class... OtherBases, class Self>
const void* find_instance(boost_kahypar::typeindex::type_index const& idx, const Self* self) noexcept {
    if (const void* ptr = self->Base::boost_type_index_find_instance_(idx)) {
        return ptr;
    }

    return boost_kahypar::typeindex::detail::find_instance<OtherBases...>(idx, self);
}
...
/// \brief Macro used to make a class compatible with boost_kahypar::typeindex::runtime_cast
///
...
/// boost_kahypar::typeindex::runtime_cast to accurately convert between dynamic types of instances of
/// the current class.
///
/// BOOST_TYPE_INDEX_REGISTER_RUNTIME_CLASS also adds support for boost_kahypar::typeindex::type_id_runtime
...
/// if(derived2* pb2 = boost_kahypar::typeindex::runtime_cast<derived2*>(pb1)) {
///     assert(boost_kahypar::typeindex::type_id_runtime(*pb1)) == boost_kahypar::typeindex::type_id<derived2>());
/// }
/// \endcode
///
...
/// \brief Macro used to make a class compatible with boost_kahypar::typeindex::runtime_cast without including
/// support for boost_kahypar::typeindex::type_id_runtime.
///
/// BOOST_TYPE_INDEX_IMPLEMENT_RUNTIME_CAST is provided as an alternative to BOOST_TYPE_INDEX_REGISTER_RUNTIME_CLASS
/// in the event that support for boost_kahypar::typeindex::type_id_runtime is undesirable.
///
/// \b Example:
/// \code
/// struct base1 {
...
/// base1* pb1 = get_object();
/// if(derived2* pb2 = boost_kahypar::typeindex::runtime_cast<derived2*>(pb1))
/// { /* can't call boost_kahypar::typeindex::type_id_runtime(*pb1) here */ }
/// \endcode
///
/// \param base_class_seq A Boost.Preprocessor sequence of the current class' direct bases, or
/// BOOST_TYPE_INDEX_NO_BASE_CLASS if this class has no direct base classes.
#define BOOST_TYPE_INDEX_IMPLEMENT_RUNTIME_CAST(...)                                                              \
    virtual void const* boost_type_index_find_instance_(boost_kahypar::typeindex::type_index const& idx) const noexcept { \
        if(idx == boost_kahypar::typeindex::detail::runtime_class_construct_type_id(this))                                \
            return this;                                                                                          \
        return boost_kahypar::typeindex::detail::find_instance<__VA_ARGS__>(idx, this);                                   \
    }
