...
/// \brief Contains boost_kahypar::typeindex::ctti_type_index class that is constexpr if C++14 constexpr is supported by compiler.
///
/// boost_kahypar::typeindex::ctti_type_index class can be used as a drop-in replacement
/// for std::type_index.
///
...
#include <boost_kahypar/type_index/type_index_facade.hpp>
#include <boost_kahypar/type_index/detail/compile_time_type_info.hpp>
...
inline const detail::ctti_data& ctti_construct() noexcept {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(boost_kahypar::detail::ctti<T>::n());
}
...
public:
    ...
    BOOST_CXX14_CONSTEXPR inline ctti_type_index() noexcept
        : data_(boost_kahypar::detail::ctti<void>::n())
    {}
...
BOOST_CXX14_CONSTEXPR inline bool ctti_type_index::equal(const ctti_type_index& rhs) const noexcept {
    ...
    return /*left == right ||*/ !boost_kahypar::typeindex::detail::constexpr_strcmp(left, right);
}

BOOST_CXX14_CONSTEXPR inline bool ctti_type_index::before(const ctti_type_index& rhs) const noexcept {
    const char* const left = raw_name();
    const char* const right = rhs.raw_name();
    return /*left != right &&*/ boost_kahypar::typeindex::detail::constexpr_strcmp(left, right) < 0;
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id() noexcept {
    ...
    return ctti_type_index(boost_kahypar::detail::ctti<no_cvr_t>::n());
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id_with_cvr() noexcept {
    return ctti_type_index(boost_kahypar::detail::ctti<T>::n());
}
...
inline std::size_t ctti_type_index::hash_code() const noexcept {
    return boost_kahypar::hash_range(raw_name(), raw_name() + get_raw_name_length());
}
