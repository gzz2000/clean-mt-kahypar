...
public:
    ...
    /// \note Derived class header \b must include <boost_kahypar/container_hash/hash.hpp>, \b unless this function is redefined in
    /// Derived class to not use boost_kahypar::hash_range().
    inline std::size_t hash_code() const noexcept {
        const char* const name_raw = derived().raw_name();
        return boost_kahypar::hash_range(name_raw, name_raw + std::strlen(name_raw));
    }
...
protected:
    ...
    /// boost_kahypar::typeindex::type_id() will call this method, if Derived has same type as boost_kahypar::typeindex::type_index.
    ///
    ...
    /// This is a factory method that is used to create instances of Derived classes.
    /// boost_kahypar::typeindex::type_id_with_cvr() will call this method, if Derived has same type as boost_kahypar::typeindex::type_index.
    ///
    /// \b Override: This function \b may be redefined and made public in Derived class. Overrides \b must not throw. 
    ...
    /// This is a factory method that is used to create instances of Derived classes.
    /// boost_kahypar::typeindex::type_id_runtime(const T&) will call this method, if Derived has same type as boost_kahypar::typeindex::type_index.
    ///
...
/// \note <boost_kahypar/container_hash/hash.hpp> has to be included if this function is used.
template <class Derived, class TypeInfo>
