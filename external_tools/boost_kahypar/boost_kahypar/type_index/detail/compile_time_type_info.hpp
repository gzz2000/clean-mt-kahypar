...
/// \brief Contains helper macros and implementation details of boost_kahypar::typeindex::ctti_type_index.
...
namespace boost_kahypar { namespace typeindex { namespace detail {
    ...
    /* Nothing to document. All the macro docs are moved to <boost_kahypar/type_index.hpp> */
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip BOOST_TYPE_INDEX_CTTI_USER_DEFINED_PARSING; }
    ...
    // sizeof("const char *__cdecl boost_kahypar::detail::ctti<") - 1, sizeof(">::n(void)") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(40, 10, ""); }
#elif defined(_MSC_VER) && !defined(__clang__) && !defined (BOOST_NO_CXX11_NOEXCEPT)
    // sizeof("const char *__cdecl boost_kahypar::detail::ctti<") - 1, sizeof(">::n(void) noexcept") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(40, 19, ""); }
    ...
    // sizeof("static const char *boost_kahypar::detail::ctti<") - 1, sizeof("]") - 1, true, "???????????>::n() [T = int"
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 1, "T = "); }
    ...
    // sizeof("static const char *boost_kahypar::detail::ctti<") - 1, sizeof(">::n()") - 1
    // note: checked on 3.0
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 6, ""); }
    ...
    // sizeof("static const char *boost_kahypar::detail::ctti<") - 1, sizeof("]") - 1, true, "int>::n() [T = int"
    // note: checked on 3.1, 3.4
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 1, "T = "); }
#elif defined(__EDG__) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static cha boost_kahypar::detail::ctti<T>::s() [with I = 40U, T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(58, 1, ""); }
#elif defined(__EDG__) && defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static const char *boost_kahypar::detail::ctti<T>::n() [with T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(57, 1, ""); }
    ...
    // sizeof("static constexpr char boost_kahypar::detail::ctti<T>::s() [with unsigned int I = 0u; } T = ") - 1, sizeof("]") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(81, 1, ""); }
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char boost_kahypar::detail::ctti<T>::s() [with unsigned int I = 0; } T = ") - 1, sizeof("]") - 1
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(0, 0, ""); }
    ...
    BOOST_CXX14_CONSTEXPR inline int constexpr_strcmp(const char *v1, const char *v2) noexcept {
        ...
        if (boost_kahypar::typeindex::detail::is_constant_string(v1) && boost_kahypar::typeindex::detail::is_constant_string(v2))
            return boost_kahypar::typeindex::detail::constexpr_strcmp_loop(v1, v2);
...
struct ctti {
    ...
    constexpr static const char* impl(::boost_kahypar::typeindex::detail::index_seq<Indexes...> ) noexcept {
        return ::boost_kahypar::typeindex::detail::cstring<s<Indexes>()...>::data_;
    }
    ...
    #else
        boost_kahypar::typeindex::detail::failed_to_get_function_name<T>();
    #endif

        boost_kahypar::typeindex::detail::assert_compile_time_legths<
            (size > boost_kahypar::typeindex::detail::skip().size_at_begin + boost_kahypar::typeindex::detail::skip().size_at_end + sizeof("const *") - 1)
        >();
        static_assert(!boost_kahypar::typeindex::detail::skip().until_runtime_length, "Skipping for GCC in C++14 mode is unsupported");

        typedef typename boost_kahypar::typeindex::detail::make_index_seq_impl<
            boost_kahypar::typeindex::detail::skip().size_at_begin,
            size - sizeof("const *") + 1 - boost_kahypar::typeindex::detail::skip().size_at_begin
        >::type idx_seq;
        return impl(idx_seq());
    }
#else
    ...
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
        return boost_kahypar::typeindex::detail::skip_begining< sizeof(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE) >(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE);
    #elif defined(__FUNCSIG__)
        return boost_kahypar::typeindex::detail::skip_begining< sizeof(__FUNCSIG__) >(__FUNCSIG__);
    #elif defined(__PRETTY_FUNCTION__) \
                || defined(__GNUC__) \
                || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) \
                || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) \
                ...
                || defined(__DMC__) \
                || defined(__clang__)
        return boost_kahypar::typeindex::detail::skip_begining< sizeof(__PRETTY_FUNCTION__) >(__PRETTY_FUNCTION__);
    #else
        boost_kahypar::typeindex::detail::failed_to_get_function_name<T>();
        return "";
    #endif
