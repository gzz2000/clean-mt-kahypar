...
// container/detail
#include <boost_kahypar/container/detail/algorithm.hpp> //algo_equal(), algo_lexicographical_compare
#include <boost_kahypar/container/detail/compare_functors.hpp>
...
#include <boost_kahypar/container/detail/node_alloc_holder.hpp>
#include <boost_kahypar/container/detail/type_traits.hpp>
#include <boost_kahypar/container/detail/value_functors.hpp>
// intrusive
#include <boost_kahypar/intrusive/pointer_traits.hpp>
#include <boost_kahypar/intrusive/slist.hpp>
// move
#include <boost_kahypar/move/iterator.hpp>
...
#endif
#include <boost_kahypar/move/detail/move_helpers.hpp>
...
{
   typedef boost_kahypar::container::allocator_traits<Allocator>      allocator_traits_type;
   typedef typename allocator_traits_type::value_type value_type;
   typedef typename boost_kahypar::intrusive::pointer_traits
      ...
      , typename dtl::intrusive_slist_type<typename real_allocator<T, Allocator>::type>::type>
{
   ...
   typedef boost_kahypar::container::
      allocator_traits<ValueAllocator>                           allocator_traits_type;
   typedef boost_kahypar::container::equal_to_value
   ...
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::pointer          pointer;
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::const_pointer    const_pointer;
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::reference        reference;
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::const_reference  const_reference;
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::size_type        size_type;
   typedef typename ::boost_kahypar::container::allocator_traits<ValueAllocator>::difference_type  difference_type;
      ...
      : AllocHolder(a)
   {
      ...
      if(this->node_alloc() == sr.node_alloc()){
         ...
         this->insert_after(this->cbefore_begin(), boost_kahypar::make_move_iterator(sr.begin()), boost_kahypar::make_move_iterator(sr.end()));
      }
   }
   ...
   {  return *this->emplace_after(this->cbefore_begin(), boost_kahypar::forward<Args>(args)...); }
   ...
   {
      NodePtr pnode(AllocHolder::create_node(boost_kahypar::forward<Args>(args)...));
      ...
      )
   {
      ...
      this->allocate_many_and_construct(first, boost_kahypar::container::iterator_udistance(first, last), func);
   ...
   {  return this->emplace_after(this->previous(p), boost_kahypar::forward<Args>(args)...);  }
   ...
   {  return x.size() == y.size() && ::boost_kahypar::container::algo_equal(x.begin(), x.end(), y.begin());  }
   ...
   BOOST_CONTAINER_ATTRIBUTE_NODISCARD inline
      friend bool operator<(const slist& x, const slist& y)
   {  return ::boost_kahypar::container::algo_lexicographical_compare(x.begin(), x.end(), y.begin(), y.end());  }
   ...
   void priv_move_assign(BOOST_RV_REF(slist) x, dtl::bool_<true> /*steal_resources*/)
   {
      ...
      //Obtain resources
      this->icont() = boost_kahypar::move(x.icont());
   }
   ...
   void priv_move_assign(BOOST_RV_REF(slist) x, dtl::bool_<false> /*steal_resources*/)
   {
      ...
      if (this->node_alloc() == x.node_alloc()) {
         ...
         this->assign(boost_kahypar::make_move_iterator(x.begin()), boost_kahypar::make_move_iterator(x.end()));
      }
   }

   template<class U>
   void priv_push_front(BOOST_FWD_REF(U) x)
   {  this->icont().push_front(*this->create_node(::boost_kahypar::forward<U>(x))); }
   ...
   {  return this->insert_after(previous(p), ::boost_kahypar::forward<U>(x)); }

   template<class U>
   iterator priv_insert_after(const_iterator prev_p, BOOST_FWD_REF(U) x)
   {  return iterator(this->icont().insert_after(prev_p.get(), *this->create_node(::boost_kahypar::forward<U>(x)))); }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<boost_kahypar::container::slist<T, Allocator> >
{
   typedef typename boost_kahypar::container::slist<T, Allocator>::allocator_type allocator_type;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::pointer pointer;
   BOOST_STATIC_CONSTEXPR bool value = ::boost_kahypar::has_trivial_destructor_after_move<allocator_type>::value &&
...
#include <boost_kahypar/move/detail/std_ns_begin.hpp>
...
class insert_iterator<boost_kahypar::container::slist<T, ValueAllocator> >
{
   private:
   typedef boost_kahypar::container::slist<T, ValueAllocator> Container;
   Container* container;
...
#include <boost_kahypar/move/detail/std_ns_end.hpp>
