...
{
   typedef boost_kahypar::container::allocator_traits<Allocator> AllocTraits;
...
struct scoped_node_destroy_deallocator
{
   ...
   {
      if(m_ptr){
         boost_kahypar::movelib::to_raw_pointer(m_ptr)->destructor(m_alloc);
...
struct scoped_destructor_n
{
   ...
   {
      if(m_n){
         value_type *raw_ptr = boost_kahypar::movelib::iterator_to_raw_pointer(m_p);
         do {
            --m_n;
...
struct scoped_destructor_range
{
   ...
   {
      while(m_p != m_e){
         value_type *raw_ptr = boost_kahypar::movelib::to_raw_pointer(m_p);
         AllocTraits::destroy(m_a, raw_ptr);
         ++m_p;
      }
   }
...
class allocator_node_destroyer
{
   ...
   {
      boost_kahypar::movelib::to_raw_pointer(p)->destructor(a_);
