...
#include <boost_kahypar/intrusive/detail/reverse_iterator.hpp>
      ...
      <std::random_access_iterator_tag, T, std::ptrdiff_t, const T*, const T &>
{
   ...
   //constructed until ::boost_kahypar::container::construct_in_place is called.
...
struct emplace_functor
{
   ...
   inline void inplace_impl(Allocator &a, T* ptr, const dtl::index_tuple<IdxPack...>&)
   {
      allocator_traits<Allocator>::construct
         (a, ptr, ::boost_kahypar::forward<Args>(dtl::get<IdxPack>(args_))...);
   }

   template<class DestIt, std::size_t ...IdxPack>
   inline void inplace_impl(DestIt dest, const dtl::index_tuple<IdxPack...>&)
   {
      typedef typename boost_kahypar::container::iterator_traits<DestIt>::value_type value_type;
      value_type && tmp= value_type(::boost_kahypar::forward<Args>(dtl::get<IdxPack>(args_))...);
      *dest = ::boost_kahypar::move(tmp);
   }
...
{\
   ...
   {\
      ...
      *dest = ::boost_kahypar::move(const_cast<value_type &>(BOOST_MOVE_IF(N, tmp, tmp.get())));\
   }\
   \
   BOOST_MOVE_MREF##N\
};\
...
struct iiterator_types
{
   ...
   typedef typename boost_kahypar::container::iterator_traits<IIterator>::pointer         it_pointer;
   typedef typename boost_kahypar::container::iterator_traits<IIterator>::difference_type difference_type;
   typedef typename ::boost_kahypar::intrusive::pointer_traits<it_pointer>::
...
{
   typedef typename ::boost_kahypar::container::iterator
...
class iterator_from_iiterator
{
   ...
   {  return ::boost_kahypar::intrusive::pointer_traits<pointer>::pointer_to(this->operator*());  }
...
using ::boost_kahypar::intrusive::reverse_iterator;
