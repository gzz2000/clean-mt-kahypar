...
#include <boost_kahypar/container/detail/construct_in_place.hpp>
#include <boost_kahypar/container/detail/destroyers.hpp>
...
#include <boost_kahypar/container/detail/pair.hpp>
// intrusive
#include <boost_kahypar/intrusive/detail/mpl.hpp>
#include <boost_kahypar/intrusive/options.hpp>
// move
   ...
   :  public HookDefiner::type
{
   ...
   explicit base_node(Alloc &a, Args &&...args)
      : hook_type()
   {
      ::boost_kahypar::container::allocator_traits<Alloc>::construct
         (a, &this->get_real_data(), ::boost_kahypar::forward<Args>(args)...);
   }
   ...
   explicit base_node(iterator_arg_t, Alloc &a, It it)
      : hook_type()
   {
      ::boost_kahypar::container::construct_in_place(a, &this->get_real_data(), it);
   }
   ...
   {
      typedef typename Pair::first_type first_type;
      const_cast<first_type&>(this->get_real_data().first) = ::boost_kahypar::move(p.first);
      this->get_real_data().second = ::boost_kahypar::move(p.second);
   }
   ...
   typename dtl::disable_if< dtl::is_pair<V>, void >::type
      do_move_assign(V &v)
   {  this->get_real_data() = ::boost_kahypar::move(v); }
            ...
            portable_rebind_alloc<typename ICont::value_type>::type   //NodeAlloc
{
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x)
      : NodeAlloc(boost_kahypar::move(BOOST_MOVE_TO_LV(x).node_alloc()))
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x, const intrusive_key_compare &c)
      : NodeAlloc(boost_kahypar::move(BOOST_MOVE_TO_LV(x).node_alloc())), m_icont(c)
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      ::new(boost_kahypar::movelib::iterator_to_raw_pointer(p), boost_container_new_t())
         Node(nalloc, boost_kahypar::forward<Args>(args)...);
   ...
   NodePtr create_node_from_key(BOOST_FWD_REF(KeyConvertible) key)
   {
      NodePtr p = this->allocate_one();
      BOOST_CONTAINER_TRY{
         ::new(boost_kahypar::movelib::iterator_to_raw_pointer(p), boost_container_new_t()) Node;
         NodeAlloc &na = this->node_alloc();
         node_allocator_traits_type::construct
            (na, dtl::addressof(p->get_real_data().first), boost_kahypar::forward<KeyConvertible>(key));
         BOOST_CONTAINER_TRY{
   ...
   {
      boost_kahypar::movelib::to_raw_pointer(nodep)->destructor(this->node_alloc());
      ...
      (FwdIterator beg, size_type n, Inserter inserter)
   {
      if(n){
         ...
         Node *p = 0;
            ...
            while(n){
               --n;
               p = boost_kahypar::movelib::iterator_to_raw_pointer(itbeg);
   ...
   void clear(version_2)
   {
      ...
      //BOOST_CONTAINER_STATIC_ASSERT((::boost_kahypar::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
   ...
   struct move_cloner
   {
      ...
      {  //Use get_real_data() instead of get_real_data to allow moving const key in [multi]map
         return m_holder.create_node(::boost_kahypar::move(other.get_real_data()));
      }
