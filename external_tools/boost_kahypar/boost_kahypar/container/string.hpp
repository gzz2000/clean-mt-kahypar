...
#include <boost_kahypar/container/new_allocator.hpp> //new_allocator
#include <boost_kahypar/container/throw_exception.hpp>
// container/detail
#include <boost_kahypar/container/detail/alloc_helpers.hpp>
#include <boost_kahypar/container/detail/allocator_version_traits.hpp>
#include <boost_kahypar/container/detail/allocation_type.hpp>
#include <boost_kahypar/container/detail/iterator.hpp>
#include <boost_kahypar/container/detail/iterators.hpp>
#include <boost_kahypar/container/detail/min_max.hpp>
#include <boost_kahypar/container/detail/mpl.hpp>
#include <boost_kahypar/container/detail/next_capacity.hpp>
#include <boost_kahypar/move/detail/to_raw_pointer.hpp>
#include <boost_kahypar/container/detail/version_type.hpp>
#include <boost_kahypar/container/detail/type_traits.hpp>
#include <boost_kahypar/container/detail/algorithm.hpp>
#include <boost_kahypar/container/detail/minimal_char_traits_header.hpp>  // for char_traits
//intrusive
#include <boost_kahypar/intrusive/pointer_traits.hpp>
#include <boost_kahypar/intrusive/detail/hash_combine.hpp>
#include <boost_kahypar/move/detail/launder.hpp>
//move
#include <boost_kahypar/move/utility_core.hpp>
#include <boost_kahypar/move/adl_move_swap.hpp>
...
class basic_string_base
{
 ...
 public:
   ...
   typedef ::boost_kahypar::intrusive::pointer_traits<pointer> pointer_traits;
      ...
      :  public allocator_type
   {
      ...
      inline explicit members_holder(BOOST_FWD_REF(AllocatorConvertible) a)
         :  allocator_type(boost_kahypar::forward<AllocatorConvertible>(a))
      { this->init(); }
   ...
   typedef dtl::integral_constant<unsigned,
      boost_kahypar::container::dtl::version<allocator_type>::value> alloc_version;
   ...
   {
      value_type *raw_p = boost_kahypar::movelib::to_raw_pointer(p);
      for(; n--; ++raw_p){
   ...
   inline void destroy(pointer p)
   {
      allocator_traits_type::destroy
         ( this->alloc()
         , boost_kahypar::movelib::to_raw_pointer(p)
         );
   }
   ...
   void swap_data(basic_string_base& other)
   {
      if(this->is_short()){
         ...
         if(other.is_short()){
            ...
            boost_kahypar::adl_move_swap(*this->members_.plong_repr(), *other.members_.plong_repr());
         }
      }
   }
};
   ...
   :  private dtl::basic_string_base<typename real_allocator<CharT, Allocator>::type>
{
   ...
   struct Not_within_traits
   {
      ...
      {
         return boost_kahypar::container::find_if(m_first, m_last,
                        boost_kahypar::container::bind1st(Eq_traits<Tr>(), x)) == m_last;
      }
   };
   #endif   //#ifndef BOOST_CONTAINER_DOXYGEN_INVOKED
   ...
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::pointer           pointer;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::const_pointer     const_pointer;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::reference         reference;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::const_reference   const_reference;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::size_type         size_type;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::difference_type   difference_type;
   typedef BOOST_CONTAINER_IMPDEF(allocator_type)                                      stored_allocator_type;
   typedef BOOST_CONTAINER_IMPDEF(pointer)                                             iterator;
   typedef BOOST_CONTAINER_IMPDEF(const_pointer)                                       const_iterator;
   typedef BOOST_CONTAINER_IMPDEF(boost_kahypar::container::reverse_iterator<iterator>)        reverse_iterator;
   typedef BOOST_CONTAINER_IMPDEF(boost_kahypar::container::reverse_iterator<const_iterator>)  const_reverse_iterator;
   ...
   basic_string(BOOST_RV_REF(basic_string) s) BOOST_NOEXCEPT_OR_NOTHROW
      : base_t(boost_kahypar::move(s.alloc()))
   {
                                  ...
                                  || allocator_traits_type::is_always_equal::value)
   {
      if (BOOST_LIKELY(this != &x)) {
         ...
         this->priv_move_assign(boost_kahypar::move(x), flag);
      }
      return *this;
   }
   ...
   void shrink_to_fit()
   {
      ...
      if(this->priv_storage() > InternalBufferChars){
         ...
         if(this->priv_size() < InternalBufferChars){
            ...
            Traits::copy( boost_kahypar::movelib::to_raw_pointer(this->priv_short_addr())
                        , boost_kahypar::movelib::to_raw_pointer(long_addr)
                        , long_size+1);
            BOOST_ASSERT(!this->is_short());
            this->destroy_long();
   ...
   //! <b>Effects</b>: Equivalent to assign(basic_string(first, last)).
    ...
    basic_string& assign(const CharT* first, const CharT* last)
    {
       ...
       this->reserve(n);
       CharT* ptr = boost_kahypar::movelib::to_raw_pointer(this->priv_addr());
      ...
      )
   {
      size_type cur = 0;
      const pointer addr = this->priv_addr();
      CharT *ptr = boost_kahypar::movelib::to_raw_pointer(addr);
      const size_type old_size = this->priv_size();
   ...
   basic_string& insert(size_type pos1, const basic_string& s, size_type pos2, size_type n = npos)
   {
      ...
      if (sz > this->max_size() - len)
         throw_length_error("basic_string::insert max_size() exceeded");
      const CharT *beg_ptr = boost_kahypar::movelib::to_raw_pointer(s.begin()) + pos2;
      ...
      )
   {
      ...
      if (first != last) {
         const size_type n = boost_kahypar::container::iterator_udistance(first, last);
         const size_type old_size = this->priv_size();
         ...
         if(enough_capacity){
            ...
            if (elems_after >= n) {
               ...
               Traits::move(const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(p + difference_type(n))),
                           boost_kahypar::movelib::to_raw_pointer(p),
                           (elems_after - n) + 1u);
               (priv_copy)(first, last, const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(p)));
               new_size = old_size + n;
            }
            else {
               ForwardIter mid = first;
               boost_kahypar::container::iterator_uadvance(mid, elems_after + 1u);
               ...
               (priv_copy)(first, mid, const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(p)));
            ...
            if(!hint){
               ...
               //than with vector<T>
               value_type * const oldbuf     = boost_kahypar::movelib::to_raw_pointer(old_start);
               value_type * const newbuf     = boost_kahypar::movelib::to_raw_pointer(new_start);
               const value_type *const pos   = boost_kahypar::movelib::to_raw_pointer(p);
   ...
   iterator erase(const_iterator p) BOOST_NOEXCEPT_OR_NOTHROW
   {
      ...
      CharT * const ptr = const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(p));
      const size_type old_size = this->priv_size();
      Traits::move(ptr,
                   boost_kahypar::movelib::to_raw_pointer(p + 1),
   ...
   {
      CharT * f = const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(first));
      if (first != last) { // The move includes the terminating null.
         const size_type num_erased = size_type(last - first);
         const size_type old_size = this->priv_size();
         Traits::move(f,
                      boost_kahypar::movelib::to_raw_pointer(last),
   ...
   basic_string& replace(const_iterator i1, const_iterator i2, size_type n, CharT c)
   {
      ...
      if (len >= n) {
         Traits::assign(const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(i1)), n, c);
         erase(i1 + difference_type(n), i2);
      }
      else {
         Traits::assign(const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(i1)), len, c);
         insert(i2, n - len, c);
      }
      return *this;
   }
      ...
      )
   {
      for ( ; i1 != i2 && j1 != j2; ++i1, ++j1){
         Traits::assign(*const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(i1)), *j1);
      }
   ...
   {
      difference_type n = boost_kahypar::container::iterator_distance(j1, j2);
      const difference_type len = i2 - i1;
      if (len >= n) {
         this->priv_copy(j1, j2, const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(i1)));
         this->erase(i1 + difference_type(n), i2);
      }
      else {
         ForwardIter m = j1;
         boost_kahypar::container::iterator_advance(m, len);
         this->priv_copy(j1, m, const_cast<CharT*>(boost_kahypar::movelib::to_raw_pointer(i1)));
         this->insert(i2, m, j2);
      }
      return *this;
   }
   ...
   size_type copy(CharT* s, size_type n, size_type pos = 0) const
   {
      ...
      Traits::copy(s, boost_kahypar::movelib::to_raw_pointer(this->priv_addr() + pos), len);
      return len;
   }
   ...
   {  return boost_kahypar::movelib::to_raw_pointer(this->priv_addr()); }
      ...
      size_type find(const CharT* s, size_type pos, size_type n) const
   {
      if (pos + n > this->size())
         ...
         const const_iterator result =
            boost_kahypar::container::search(boost_kahypar::movelib::to_raw_pointer(addr + difference_type(pos)),
                   boost_kahypar::movelib::to_raw_pointer(finish),
      ...
      size_type find(CharT c, size_type pos = 0) const
   {
      ...
      if (pos >= sz)
         ...
         const const_iterator result =
            boost_kahypar::container::find_if(addr + difference_type(pos), finish,
                  boost_kahypar::container::bind2nd(Eq_traits<Traits>(), c));
         return result != finish ? size_type(result - begin()) : npos;
      }
   }
      ...
      size_type rfind(const CharT* s, size_type pos, size_type n) const
   {
      ...
      if (n > len)
         ...
         const const_iterator result = boost_kahypar::container::find_end
      ...
      size_type rfind(CharT c, size_type pos = npos) const
   {
      ...
      if (len < 1)
         ...
         const_reverse_iterator rresult =
            boost_kahypar::container::find_if(const_reverse_iterator(last), rend(),
                  boost_kahypar::container::bind2nd(Eq_traits<Traits>(), c));
      ...
      size_type find_first_of(const CharT* s, size_type pos, size_type n) const
   {
      ...
      if (pos >= sz)
         ...
         const pointer addr    = this->priv_addr();
         pointer finish = addr + difference_type(sz);
         const_iterator result = boost_kahypar::container::find_first_of
      ...
      size_type find_last_of(const CharT* s, size_type pos, size_type n) const
   {
      ...
      if (len < 1)
         ...
         const const_reverse_iterator rresult =
            boost_kahypar::container::find_first_of(const_reverse_iterator(last), rend(),
                               s, s + difference_type(n), Eq_traits<Traits>());
      ...
      size_type find_first_not_of(const CharT* s, size_type pos, size_type n) const
   {
      if (pos > this->size())
         ...
         const const_iterator result = boost_kahypar::container::find_if
      ...
      size_type find_first_not_of(CharT c, size_type pos = 0) const
   {
      if (pos > this->size())
         ...
         const const_iterator result
            = boost_kahypar::container::find_if(addr + difference_type(pos), finish,
                     boost_kahypar::container::not1(boost_kahypar::container::bind2nd(Eq_traits<Traits>(), c)));
         return result != finish ? size_type(result - begin()) : npos;
      }
   }
   ...
   void priv_move_assign(BOOST_RV_REF(basic_string) x, dtl::bool_<false> /*steal_resources*/)
   {
      ...
      if (this->alloc() == x.alloc()) {
         this->priv_move_assign(boost_kahypar::move(x), dtl::true_());
      }
      else {
         this->assign(x.begin(), x.end());
      }
   ...
   static int s_compare(It1 f1, It1 l1, It2 f2, It2 l2)
   {
      ...
      const int cmp = Traits::compare(boost_kahypar::movelib::to_raw_pointer(f1),
                                      boost_kahypar::movelib::to_raw_pointer(f2),
                                      dtl::min_value(n1, n2));
      ...
      , typename dtl::enable_if<dtl::is_same<AllocVersion, version_1> >::type* = 0)
   {
      ...
      BOOST_CONTAINER_TRY{
         ...
         //Copy and update
         Traits::copy( boost_kahypar::movelib::to_raw_pointer(ret)
                     , boost_kahypar::movelib::to_raw_pointer(this->priv_long_addr())
                     , long_size+1);
   ...
   inline basic_string& priv_replace_dispatch(const_iterator first, const_iterator last,
                                       ...
                                       dtl::false_)
   {
      typedef typename boost_kahypar::container::iterator_traits<InputIter>::iterator_category Category;
...
inline std::size_t hash_value(basic_string<Ch, std::char_traits<Ch>, Allocator> const& v)
{
   ...
   {
      boost_kahypar::intrusive::detail::hash_combine_size_t(seed, static_cast<std::size_t>(*first));
      ++first;
   }
...
struct has_trivial_destructor_after_move<boost_kahypar::container::basic_string<C, T, Allocator> >
{
   typedef typename boost_kahypar::container::basic_string<C, T, Allocator>::allocator_type allocator_type;
   typedef typename ::boost_kahypar::container::allocator_traits<allocator_type>::pointer pointer;
   BOOST_STATIC_CONSTEXPR bool value =
      ::boost_kahypar::has_trivial_destructor_after_move<allocator_type>::value &&
      ::boost_kahypar::has_trivial_destructor_after_move<pointer>::value;
};
