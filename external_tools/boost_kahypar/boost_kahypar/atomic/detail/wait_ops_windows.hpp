...
#include <boost_kahypar/atomic/detail/wait_operations_fwd.hpp>
#include <boost_kahypar/atomic/detail/wait_capabilities.hpp>
#if defined(BOOST_ATOMIC_DETAIL_WINDOWS_HAS_WAIT_ON_ADDRESS)
#include <boost_kahypar/winapi/wait_on_address.hpp>
...
#include <boost_kahypar/atomic/detail/wait_ops_generic.hpp>
    ...
    public Base
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) BOOST_NOEXCEPT
    {
        ...
        {
            boost_kahypar::winapi::WaitOnAddress(const_cast< storage_type* >(&storage), &old_val, Size, boost_kahypar::winapi::infinite);
    ...
    {
        boost_kahypar::winapi::WakeByAddressSingle(const_cast< storage_type* >(&storage));
    }

    static BOOST_FORCEINLINE void notify_all(storage_type volatile& storage) BOOST_NOEXCEPT
    {
        boost_kahypar::winapi::WakeByAddressAll(const_cast< storage_type* >(&storage));
    }
};
    ...
    public atomics::detail::wait_operations_generic< Base, false >
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) BOOST_NOEXCEPT
    {
        ...
        if (BOOST_LIKELY(atomics::detail::wait_on_address != NULL))
        {
            ...
            while (new_val == old_val)
            {
                atomics::detail::wait_on_address(const_cast< storage_type* >(&storage), &old_val, Size, boost_kahypar::winapi::infinite);
                new_val = base_type::load(storage, order);
            }
