...
typedef boost_kahypar::winapi::BOOL_ BOOST_WINAPI_WINAPI_CC
wait_on_address_t(
...
typedef boost_kahypar::winapi::VOID_ BOOST_WINAPI_WINAPI_CC
wake_by_address_t(boost_kahypar::winapi::PVOID_ addr);
...
BOOST_FORCEINLINE void ensure_wait_functions_initialized() BOOST_NOEXCEPT
{
    ...
    if (BOOST_LIKELY(once_flag_operations::load(wait_functions_once_flag.m_flag, boost_kahypar::memory_order_acquire) == 0u))
        return;
