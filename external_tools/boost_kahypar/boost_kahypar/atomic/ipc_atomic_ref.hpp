...
#include <boost_kahypar/atomic/detail/atomic_ref_impl.hpp>
...
public:
    ...
    static_assert(sizeof(value_type) > 0u, "boost_kahypar::ipc_atomic_ref<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    static_assert(atomics::detail::is_trivially_copyable< value_type >::value, "boost_kahypar::ipc_atomic_ref<T> requires T to be a trivially copyable type");
#endif
