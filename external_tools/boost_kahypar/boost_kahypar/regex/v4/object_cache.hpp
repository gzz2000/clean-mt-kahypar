...
{
public:
   typedef std::pair< ::boost_kahypar::shared_ptr<Object const>, Key const*> value_type;
   typedef std::list<value_type> list_type;
   typedef typename list_type::iterator list_iterator;
   typedef std::map<Key, list_iterator> map_type;
   typedef typename map_type::iterator map_iterator;
   typedef typename list_type::size_type size_type;
   static boost_kahypar::shared_ptr<Object const> get(const Key& k, size_type l_max_cache_size);

private:
   static boost_kahypar::shared_ptr<Object const> do_get(const Key& k, size_type l_max_cache_size);
...
#endif
template <class Key, class Object>
boost_kahypar::shared_ptr<Object const> object_cache<Key, Object>::get(const Key& k, size_type l_max_cache_size)
{
#ifdef BOOST_HAS_THREADS
   static boost_kahypar::static_mutex mut = BOOST_STATIC_MUTEX_INIT;
   boost_kahypar::static_mutex::scoped_lock l(mut);
   if (l)
   {
   ...
   //
   ::boost_kahypar::throw_exception(std::runtime_error("Error in thread safety code: could not acquire a lock"));
#if defined(BOOST_NO_UNREACHABLE_RETURN_DETECTION) || defined(BOOST_NO_EXCEPTIONS)
   return boost_kahypar::shared_ptr<Object>();
#endif
#else
   return do_get(k, l_max_cache_size);
#endif
...
template <class Key, class Object>
boost_kahypar::shared_ptr<Object const> object_cache<Key, Object>::do_get(const Key& k, size_type l_max_cache_size)
{
   ...
   // so create it:
   //
   boost_kahypar::shared_ptr<Object const> result(new Object(k));
   //
