...
namespace boost_kahypar{ namespace BOOST_REGEX_DETAIL_NS{
...
inline const char* BOOST_REGEX_CALL get_default_error_string(regex_constants::error_type n)
{
   ...
   return (n > ::boost_kahypar::regex_constants::error_unknown) ? s_default_error_messages[::boost_kahypar::regex_constants::error_unknown] : s_default_error_messages[n];
}
...
inline bool BOOST_REGEX_CALL is_combining_implementation(boost_kahypar::uint_least16_t c)
{
   const boost_kahypar::uint_least16_t combining_ranges[] = { 0x0300, 0x0361,
                           0x0483, 0x0486,
                           0x0903, 0x0903,
                           0x093E, 0x0940,
                           0x0949, 0x094C,
   ...
   const boost_kahypar::uint_least16_t* p = combining_ranges + 1;
   while (*p < c) p += 2;
   --p;
   if ((c >= *p) && (c <= *(p + 1)))
      return true;
...
inline bool is_separator(charT c)
{
   return BOOST_REGEX_MAKE_BOOL(
      ...
      || (static_cast<boost_kahypar::uint16_t>(c) == 0x2028u)
      || (static_cast<boost_kahypar::uint16_t>(c) == 0x2029u)
      || (static_cast<boost_kahypar::uint16_t>(c) == 0x85u));
}
template <>
...
boost_kahypar::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   (void)t; // warning suppression
   boost_kahypar::intmax_t limit = (std::numeric_limits<boost_kahypar::intmax_t>::max)() / radix;
   boost_kahypar::intmax_t next_value = t.value(*p1, radix);
   if((p1 == p2) || (next_value < 0) || (next_value >= radix))
      return -1;
   boost_kahypar::intmax_t result = 0;
   while(p1 != p2)
   {
...
template <class charT>
inline typename boost_kahypar::enable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
...
template <class charT>
inline typename boost_kahypar::disable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
