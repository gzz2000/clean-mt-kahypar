...
#include <boost_kahypar/regex/v4/mem_block_cache.hpp>
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_all_states()
{
   static matcher_proc_type const s_match_vtable[34] = 
   {
      ...
      //(::boost_kahypar::is_random_access_iterator<BidiIterator>::value ? &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast : &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_slow),
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
{
   ...
   const re_repeat* rep = static_cast<const re_repeat*>(pstate);
   bool greedy = (rep->greedy) && (!(m_match_flags & regex_constants::match_any) || m_independent);   
   std::size_t count = static_cast<std::size_t>((std::min)(static_cast<std::size_t>(::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(position, last)), greedy ? rep->max : rep->min));
   if(rep->min > count)
   {
      position = last;
...
#pragma option push -w-8008 -w-8066 -w-8004
   ...
   if(::boost_kahypar::is_random_access_iterator<BidiIterator>::value)
   {
      ...
      std::size_t len = (desired == (std::numeric_limits<std::size_t>::max)()) ? 0u : ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(position, last);
      if(desired >= len)
         end = last;
      else
      ...
      count = (unsigned)::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(origin, position);
   }
   else
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_case(bool)
{
   saved_change_case* pmp = static_cast<saved_change_case*>(m_backup_state);
   icase = pmp->icase;
   boost_kahypar::BOOST_REGEX_DETAIL_NS::inplace_destroy(pmp++);
   m_backup_state = pmp;
   return true;
}
...
{
   boost_kahypar::BOOST_REGEX_DETAIL_NS::inplace_destroy(m_backup_state++);
   pstate = 0;   // nothing left to search
   return false; // end of stack nothing more to search
}
...
inline void perl_matcher<BidiIterator, Allocator, traits>::destroy_single_repeat()
{
   ...
   boost_kahypar::BOOST_REGEX_DETAIL_NS::inplace_destroy(p++);
   m_backup_state = p;
}
