...
public: 
   ...
   difference_type position(size_type sub = 0) const
   {
      ...
      if(sub < m_subs.size())
      {
         ...
         if(s.matched || (sub == 2))
         {
            return ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)(m_base), (BidiIterator)(s.first));
         }
      }
                         ...
                         const RegexT& re) const
   {
      if(m_is_singular)
         raise_logic_error();
      typedef ::boost_kahypar::regex_traits_wrapper<typename RegexT::traits_type> traits_type;
   ...
   void BOOST_REGEX_CALL set_named_subs(boost_kahypar::shared_ptr<named_sub_type> subs)
   {
      m_named_subs = subs;
   }
...
private:
   ...
   {
      std::logic_error e("Attempt to access an uninitialized boost_kahypar::match_results<> class.");
   ...
   boost_kahypar::shared_ptr<named_sub_type> m_named_subs;     // Shared copy of named subs in the regex object
...
void BOOST_REGEX_CALL match_results<BidiIterator, Allocator>::maybe_assign(const match_results<BidiIterator, Allocator>& m)
{
   ...
   for(i = 0; i < size(); ++i, ++p1, ++p2)
   {
      ...
      base1 = ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(l_base, p1->first);
      base2 = ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(l_base, p2->first);
      BOOST_REGEX_ASSERT(base1 >= 0);
      BOOST_REGEX_ASSERT(base2 >= 0);
      if(base1 < base2) return;
      if(base2 < base1) break;

      len1 = ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p1->first, (BidiIterator)p1->second);
      len2 = ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p2->first, (BidiIterator)p2->second);
