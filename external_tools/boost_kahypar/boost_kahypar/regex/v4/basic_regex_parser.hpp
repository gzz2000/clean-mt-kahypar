...
inline boost_kahypar::intmax_t umax(mpl::false_ const&)
{
   // Get out clause here, just in case numeric_limits is unspecialized:
   return std::numeric_limits<boost_kahypar::intmax_t>::is_specialized ? (std::numeric_limits<boost_kahypar::intmax_t>::max)() : INT_MAX;
}
inline boost_kahypar::intmax_t umax(mpl::true_ const&)
{
   return (std::numeric_limits<std::size_t>::max)();
}

inline boost_kahypar::intmax_t umax()
{
   return umax(mpl::bool_<std::numeric_limits<boost_kahypar::intmax_t>::digits >= std::numeric_limits<std::size_t>::digits>());
}
...
void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2, unsigned l_flags)
{
   ...
   if(!result)
   {
      fail(regex_constants::error_paren, ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Found a closing ) with no corresponding opening parenthesis.");
      return;
   }
...
void basic_regex_parser<charT, traits>::fail(regex_constants::error_type error_code, std::ptrdiff_t position, std::string message, std::ptrdiff_t start_pos)
{
   ...
   {
      boost_kahypar::regex_error e(message, error_code, position);
      e.raise();
   }
#else
...
bool basic_regex_parser<charT, traits>::parse_all()
{
   if (++m_recursion_count > 400)
   {
      ...
      fail(boost_kahypar::regex_constants::error_complexity, m_position - m_base, "Exceeded nested brace limit.");
   }
   bool result = true;
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
   ...
   if(m_position == m_end)
   {
      this->fail(regex_constants::error_paren, ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(m_base, m_end));
      return false;
   }
...
escape_type_class_jump:
   ...
   case regex_constants::escape_type_extended_backref:
      if(0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex)))
      {
         ...
         boost_kahypar::intmax_t i = this->m_traits.toi(pc, m_end, 10);
         if((i < 0) && syn_end)
         {
         ...
         if(negative)
            i = 1 + (static_cast<boost_kahypar::intmax_t>(m_mark_count) - i);
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
   ...
   {
      fail(regex_constants::error_badrepeat, ::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Nothing to repeat.");
      return false;
   }
...
bool valid_value(charT, boost_kahypar::intmax_t v, const mpl::true_&)
{
   return (v >> (sizeof(charT) * CHAR_BIT)) == 0;
}
template <class charT>
bool valid_value(charT, boost_kahypar::intmax_t, const mpl::false_&)
{
   return true; // v will alsways fit in a charT
}
template <class charT>
bool valid_value(charT c, boost_kahypar::intmax_t v)
{
   return valid_value(c, v, mpl::bool_<(sizeof(charT) < sizeof(boost_kahypar::intmax_t))>());
}
...
#pragma warning(disable:4127)
   ...
   case regex_constants::escape_type_hex:
      ...
      if(this->m_traits.syntax_type(*m_position) == regex_constants::syntax_open_brace)
      {
         ...
         boost_kahypar::intmax_t i = this->m_traits.toi(m_position, m_end, 16);
         if((m_position == m_end)
            || (i < 0)
            || ((std::numeric_limits<charT>::is_specialized) && (i > (boost_kahypar::intmax_t)(std::numeric_limits<charT>::max)()))
      ...
      else
      {
         ...
         boost_kahypar::intmax_t i = this->m_traits.toi(m_position, m_position + len, 16);
         if((i < 0)
   ...
   case regex_constants::syntax_digit:
      ...
      std::ptrdiff_t len = (std::min)(::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(m_position, m_end), static_cast<std::ptrdiff_t>(4));
      const charT* bp = m_position;
      boost_kahypar::intmax_t val = this->m_traits.toi(bp, bp + 1, 8);
      if(val != 0)
      {
         // Rewind to start of escape:
         --m_position;
      ...
      if((val < 0) || (val > (boost_kahypar::intmax_t)(std::numeric_limits<charT>::max)()))
      {
         // Rewind to start of escape:
         --m_position;
         while(this->m_traits.syntax_type(*m_position) != regex_constants::syntax_escape) --m_position;
...
bool basic_regex_parser<charT, traits>::parse_backref()
{
   ...
   const charT* pc = m_position;
   boost_kahypar::intmax_t i = this->m_traits.toi(pc, pc + 1, 10);
...
insert_recursion:
   ...
   case regex_constants::syntax_plus:
      ...
      if ((std::numeric_limits<boost_kahypar::intmax_t>::max)() - m_mark_count < v)
      {
         fail(regex_constants::error_perl_extension, m_position - m_base, "An invalid or unterminated recursive sub-expression.");
         return false;
      }
   ...
   case regex_constants::syntax_dash:
      ...
      v = static_cast<boost_kahypar::intmax_t>(m_mark_count) + 1 - v;
      if(v <= 0)
      {
         // Rewind to start of (? sequence:
         --m_position;
