...
#include <boost_kahypar/regex/v4/indexed_bit_flag.hpp>
...
protected:
   ...
   const ::boost_kahypar::regex_traits_wrapper<traits>&  
   ...
   boost_kahypar::uintmax_t              m_bad_repeats;        // bitmask of repeats we can't deduce a startmap for;
   ...
   m_has_backrefs(false), m_bad_repeats(0), m_has_recursions(false), m_word_mask(0), m_mask_space(0), m_lower_mask(0), m_upper_mask(0), m_alpha_mask(0)
{
   ...
   m_pdata->m_status = ::boost_kahypar::regex_constants::error_ok;
   ...
   const basic_char_set<charT, traits>& char_set, mpl::false_*)
{
   ...
   // fill in the basics:
   //
   result->csingles = static_cast<unsigned int>(::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(char_set.singles_begin(), char_set.singles_end()));
   result->cranges = static_cast<unsigned int>(::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(char_set.ranges_begin(), char_set.ranges_end())) / 2;
   result->cequivalents = static_cast<unsigned int>(::boost_kahypar::BOOST_REGEX_DETAIL_NS::distance(char_set.equivalents_begin(), char_set.equivalents_end()));
               ...
               if(idx >= hash_value_mask)
               {
                  ...
                  if(idx <= 0)
                  {
                     ...
                     if(0 == this->m_pdata->m_status) // update the error code if not already set
                        this->m_pdata->m_status = boost_kahypar::regex_constants::error_bad_pattern;
                     //
                     ...
                     if(0 == (this->flags() & regex_constants::no_except))
                     {
                        ...
                        boost_kahypar::regex_error e(message, boost_kahypar::regex_constants::error_bad_pattern, 0);
                        e.raise();
                     }
                  }
               }
   ...
   case syntax_element_long_set_rep:
      {
         ...
         static const boost_kahypar::uintmax_t one = 1uL;
