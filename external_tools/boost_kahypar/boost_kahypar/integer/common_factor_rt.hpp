...
#include <boost_kahypar/config.hpp>  // for BOOST_NESTED_TEMPLATE, etc.
...
#pragma intrinsic(_BitScanForward64)
      ...
      template <>
      struct gcd_traits<boost_kahypar::ulong_long_type> : public gcd_traits_defaults<boost_kahypar::ulong_long_type>
      {
         BOOST_FORCEINLINE static BOOST_CXX14_CONSTEXPR unsigned find_lsb(boost_kahypar::ulong_long_type mask)BOOST_NOEXCEPT
         {
            return __builtin_ctzll(mask);
         }
         BOOST_FORCEINLINE static BOOST_CXX14_CONSTEXPR unsigned make_odd(boost_kahypar::ulong_long_type& val)BOOST_NOEXCEPT
         {
            unsigned result = find_lsb(val);
            val >>= result;
            return result;
      ...
      // are called, all values are > 0.
      //
      template <> struct gcd_traits<boost_kahypar::long_long_type> : public gcd_traits_defaults<boost_kahypar::long_long_type>
      {
         BOOST_FORCEINLINE static BOOST_CXX14_CONSTEXPR unsigned make_odd(boost_kahypar::long_long_type& val)BOOST_NOEXCEPT { unsigned result = gcd_traits<boost_kahypar::ulong_long_type>::find_lsb(val); val >>= result; return result; }
      };
      template <> struct gcd_traits<long> : public gcd_traits_defaults<long>
      {
   ...
   BOOST_CXX14_CONSTEXPR T mixed_binary_gcd(T u, T v) BOOST_GCD_NOEXCEPT(T)
   {
    ...
    {
       T temp = boost_kahypar::integer::gcd_detail::optimal_gcd_select(a, b);
...
inline BOOST_CXX14_CONSTEXPR Integer gcd(Integer const &a, Integer const &b) BOOST_GCD_NOEXCEPT(Integer)
{
    if(a == (std::numeric_limits<Integer>::min)())
       return a == static_cast<Integer>(0) ? gcd_detail::gcd_traits<Integer>::abs(b) : boost_kahypar::integer::gcd(static_cast<Integer>(a % b), b);
    else if (b == (std::numeric_limits<Integer>::min)())
       return b == static_cast<Integer>(0) ? gcd_detail::gcd_traits<Integer>::abs(a) : boost_kahypar::integer::gcd(a, static_cast<Integer>(b % a));
...
//
template <typename Integer>
inline typename boost_kahypar::enable_if_c<std::numeric_limits<Integer>::is_specialized, boost_kahypar::rational<Integer> >::type gcd(boost_kahypar::rational<Integer> const &a, boost_kahypar::rational<Integer> const &b)
{
   return boost_kahypar::rational<Integer>(static_cast<Integer>(gcd(a.numerator(), b.numerator())), static_cast<Integer>(lcm(a.denominator(), b.denominator())));
}

template <typename Integer>
inline typename boost_kahypar::enable_if_c<std::numeric_limits<Integer>::is_specialized, boost_kahypar::rational<Integer> >::type lcm(boost_kahypar::rational<Integer> const &a, boost_kahypar::rational<Integer> const &b)
{
   return boost_kahypar::rational<Integer>(static_cast<Integer>(lcm(a.numerator(), b.numerator())), static_cast<Integer>(gcd(a.denominator(), b.denominator())));
}
/**
...
public:
   ...
   {
      return boost_kahypar::integer::gcd(a, b);
   }
};
...
public:
   ...
   IntegerType operator()(IntegerType const &a, IntegerType const &b)const
   {
      return boost_kahypar::integer::lcm(a, b);
   }
};
