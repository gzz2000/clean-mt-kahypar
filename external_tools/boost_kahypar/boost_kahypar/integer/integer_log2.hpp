...
template< typename T >
inline typename boost_kahypar::disable_if< boost_kahypar::is_integral< T >, int >::type integer_log2_impl(T x)
{
...
template< typename T >
inline typename boost_kahypar::enable_if< boost_kahypar::is_integral< T >, int >::type integer_log2_impl(T x)
{
    ...
    return static_cast< int >((sizeof(T) * CHAR_BIT - 1u) -
        boost_kahypar::core::countl_zero(static_cast< typename boost_kahypar::make_unsigned< T >::type >(x)));
}
...
inline int integer_log2_impl(boost_kahypar::uint128_type x)
{
    const boost_kahypar::uint64_t x_hi = static_cast< boost_kahypar::uint64_t >(x >> 64u);
    if (x_hi != 0u)
        return 127 - boost_kahypar::core::countl_zero(x_hi);
    else
        return 63 - boost_kahypar::core::countl_zero(static_cast< boost_kahypar::uint64_t >(x));
}

inline int integer_log2_impl(boost_kahypar::int128_type x)
{
    return detail::integer_log2_impl(static_cast< boost_kahypar::uint128_type >(x));
}
