...
#include <boost_kahypar/utility/string_ref_fwd.hpp>
...
// #endif
        ...
        const charT& at(size_type pos) const {
            if ( pos >= len_ )
                BOOST_THROW_EXCEPTION( std::out_of_range ( "boost_kahypar::string_ref::at" ) );
            return ptr_[pos];
            }
...
namespace std {
    // Hashing
    template<> struct hash<boost_kahypar::string_ref>;
    template<> struct hash<boost_kahypar::u16string_ref>;
    template<> struct hash<boost_kahypar::u32string_ref>;
    template<> struct hash<boost_kahypar::wstring_ref>;
}
#endif
