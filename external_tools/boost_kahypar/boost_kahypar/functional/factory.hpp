...
template<class A>
class fc_delete
    : boost_kahypar::empty_value<A> {
    typedef boost_kahypar::empty_value<A> base;

public:
    explicit fc_delete(const A& a) BOOST_NOEXCEPT
        : base(boost_kahypar::empty_init_t(), a) { }

    void operator()(typename fc_pointer<A>::type p) {
        boost_kahypar::detail::fc_destroy(base::get(), boost_kahypar::to_address(p));
...
public:
    ...
    typename A::value_type* get() const BOOST_NOEXCEPT {
        return boost_kahypar::to_address(p_);
    }
