...
#   include <boost_kahypar/preprocessor/facilities/intercept.hpp>
...
#   define BOOST_TMP_MACRO(f,fn,fc) \
        boost_kahypar::detail::forward_adapter_impl< \
    ...
    namespace detail
    {
        ...
        struct forward_adapter_result::apply< Self() >
            : boost_kahypar::result_of< BOOST_DEDUCED_TYPENAME c<Self>::t() >
        { };
        ...
        struct forward_adapter_impl_zero<MD, F, FC,
            typename forward_adapter_sfinae<typename boost_kahypar::result_of< FC() >::type>::type>
        {
            inline typename boost_kahypar::result_of< FC() >::type
            operator()() const
            {
                return static_cast<MD const*>(this)->target_function()();
            }

            inline typename boost_kahypar::result_of< F() >::type
            operator()()
            {
...
#       define BOOST_TMP_MACRO(tpl_params,arg_types,params,args)              \
            template< tpl_params >                                             \
            inline typename boost_kahypar::result_of< FC(arg_types) >::type            \
            operator()(params) const                                           \
            {                                                                  \
                return static_cast<MD const*>(this)->target_function()(args);  \
            }                                                                  \
            template< tpl_params >                                             \
            inline typename boost_kahypar::result_of< F(arg_types)>::type              \
            operator()(params)                                                 \
            {                                                                  \
...
#       define  BOOST_PP_FILENAME_1 <boost_kahypar/functional/forward_adapter.hpp>
...
#       undef BOOST_TMP_MACRO
    ...
    struct result_of<boost_kahypar::forward_adapter<F,A0,A1> const ()>
        : boost_kahypar::detail::forward_adapter_result::template apply<
            boost_kahypar::forward_adapter<F,A0,A1> const () >
    { };
    template<class F, int A0, int A1>
    struct result_of<boost_kahypar::forward_adapter<F,A0,A1>()>
        : boost_kahypar::detail::forward_adapter_result::template apply<
            boost_kahypar::forward_adapter<F,A0,A1>() >
    { };
    template<class F, int A0, int A1>
    struct result_of<boost_kahypar::forward_adapter<F,A0,A1> const& ()>
        : boost_kahypar::detail::forward_adapter_result::template apply<
...
#     define  BOOST_PP_FILENAME_2 <boost_kahypar/functional/forward_adapter.hpp>
#     define  BOOST_PP_ITERATION_LIMITS (0,limit2)
#     include BOOST_PP_ITERATE()
#   elif BOOST_PP_ITERATION_DEPTH() == 2 && limit3
#     define  BOOST_PP_FILENAME_3 <boost_kahypar/functional/forward_adapter.hpp>
...
#         endif
        ...
        struct forward_adapter_impl<MD,F,FC,BOOST_PP_DEC(N),N>
        {
            ...
            inline typename boost_kahypar::result_of< F(
...
#       if BOOST_WORKAROUND(BOOST_MSVC,BOOST_TESTED_AT(1400)) 
            template< BOOST_PP_ENUM_PARAMS(N,typename T) >
            inline typename boost_kahypar::result_of<  FC(BOOST_PP_ENUM_PARAMS(N,PT)) 
                >::type
            ...
            template< BOOST_PP_ENUM_PARAMS(N,typename T) >
            inline typename boost_kahypar::result_of<  F(BOOST_PP_ENUM_PARAMS(N,PT))
                >::type
