...
#include <boost_kahypar/winapi/error_handling.hpp>
#include <boost_kahypar/winapi/character_code_conversion.hpp>
#include <boost_kahypar/winapi/local_memory.hpp>
...
inline boost_kahypar::winapi::UINT_ message_cp_win32()
{
#if defined(BOOST_SYSTEM_USE_UTF8)

    return boost_kahypar::winapi::CP_UTF8_;

#else

    return boost_kahypar::winapi::CP_ACP_;
...
inline char const * system_category_message_win32( int ev, char * buffer, std::size_t len ) noexcept
{
    ...
    boost_kahypar::winapi::UINT_ const code_page = message_cp_win32();

    int r = 0;

#if !defined(BOOST_NO_ANSI_APIS)

    if( code_page == boost_kahypar::winapi::CP_ACP_ )
    {
        using namespace boost_kahypar::winapi;

        DWORD_ retval = boost_kahypar::winapi::FormatMessageA(
...

    {
        ...
        DWORD_ retval = boost_kahypar::winapi::FormatMessageW(
        ...
        if( retval != 0 )
        {
            r = boost_kahypar::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, buffer, static_cast<int>( len ), NULL, NULL );
            boost_kahypar::winapi::LocalFree( lpMsgBuf );
...
struct local_free
{
    ...
    ~local_free()
    {
        boost_kahypar::winapi::LocalFree( p_ );
    }
};
...
inline std::string system_category_message_win32( int ev )
{
    ...
    int r = boost_kahypar::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, 0, 0, NULL, NULL );
    ...
    r = boost_kahypar::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, &buffer[0], r, NULL, NULL );
