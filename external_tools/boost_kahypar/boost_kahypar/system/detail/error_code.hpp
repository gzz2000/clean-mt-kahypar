...
#include <boost_kahypar/system/detail/error_category.hpp>
#include <boost_kahypar/system/detail/error_condition.hpp>
#include <boost_kahypar/system/detail/system_category.hpp>
#include <boost_kahypar/system/detail/system_category_impl.hpp>
#include <boost_kahypar/system/detail/interop_category.hpp>
#include <boost_kahypar/system/detail/enable_if.hpp>
#include <boost_kahypar/system/detail/is_same.hpp>
#include <boost_kahypar/system/detail/append_int.hpp>
#include <boost_kahypar/system/detail/snprintf.hpp>
#include <boost_kahypar/system/detail/config.hpp>
#include <boost_kahypar/system/detail/std_category.hpp>
...
private:
    ...
    boost_kahypar::uintptr_t lc_flags_;
...
public:
    ...
    error_code( int val, const error_category & cat, source_location const * loc ) noexcept:
        d1_(), lc_flags_( ( loc? reinterpret_cast<boost_kahypar::uintptr_t>( loc ): 2 ) | +detail::failed_impl( val, cat ) )
    {
        d1_.val_ = val;
        d1_.cat_ = &cat;
    }
        ...
        d1_(), lc_flags_( 0 )
    {
        *this = ec;
        ...
        {
            lc_flags_ = ( loc? reinterpret_cast<boost_kahypar::uintptr_t>( loc ): 2 ) | ( ec.lc_flags_ & 1 );
        }
    }
        ...
        d1_(), lc_flags_( 0 )
    {
        ...
        {
            *this = boost_kahypar::system::error_code( ec.value(), pc2->original_category() );
        }
        else
    ...
    BOOST_SYSTEM_CONSTEXPR int value() const noexcept
    {
        ...
        else
        {
            ...
            unsigned ch = static_cast<unsigned>( reinterpret_cast<boost_kahypar::uintptr_t>( &ec.category() ) % 2097143 ); // 2^21-9, prime
    ...
    source_location const & location() const noexcept
    {
        ...
        return lc_flags_ >= 4? *reinterpret_cast<source_location const*>( lc_flags_ &~ static_cast<boost_kahypar::uintptr_t>( 1 ) ): loc;
    }
...
// This condition must be the same as the one in error_category_impl.hpp
            ...
            return std::error_code( 0, boost_kahypar::system::system_category() );
...
inline std::size_t hash_value( error_code const & ec )
{
    ...
    boost_kahypar::ulong_long_type id_ = cat.id_;

    if( id_ == 0 )
    {
        id_ = reinterpret_cast<boost_kahypar::uintptr_t>( &cat );
    }

    boost_kahypar::ulong_long_type hv = ( boost_kahypar::ulong_long_type( 0xCBF29CE4 ) << 32 ) + 0x84222325;
    boost_kahypar::ulong_long_type const prime = ( boost_kahypar::ulong_long_type( 0x00000100 ) << 32 ) + 0x000001B3;
