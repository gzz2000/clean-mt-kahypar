...
# include <boost_kahypar/concept_archetype.hpp> // for detail::dummy_constructor
...
void input_iterator_test(Iterator i, T v1, T v2)
{
  ...
  BOOST_TEST(*i++ == v1);
  boost_kahypar::ignore_unused(i1);
...
# endif
        BOOST_STATIC_ASSERT(boost_kahypar::is_reference<reference>::value);
        BOOST_STATIC_ASSERT((boost_kahypar::is_same<reference,value_type&>::value
                             || boost_kahypar::is_same<reference,const value_type&>::value
            ));
    }
};
...
# if !defined(BOOST_BORLANDC) || (BOOST_BORLANDC > 0x551)
  lvalue_test<(boost_kahypar::is_pointer<Iterator>::value)>::check(i);
#endif
}
