...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::boost_kahypar::is_function<typename boost_kahypar::remove_reference<T>::type>::value || (sizeof(boost_kahypar::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(boost_kahypar::declval< U >())) >
   ...
   template <class T>
   struct is_complete : boost_kahypar::integral_constant<bool, ::boost_kahypar::is_function<typename boost_kahypar::remove_reference<T>::type>::value || ::boost_kahypar::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : boost_kahypar::is_complete<T> {};
   
#else

      template <class T> struct is_complete
         : public boost_kahypar::integral_constant<bool, true> {};
