...
private:
    ...
    typedef typename boost_kahypar::remove_cv<T>::type T2;
    typedef typename boost_kahypar::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (boost_kahypar::is_same<T2, U2>::value)
        || boost_kahypar::is_void<T2>::value
        || boost_kahypar::is_void<U2>::value
        || (boost_kahypar::is_base_of<T2, U2>::value)
        || (boost_kahypar::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename boost_kahypar::conditional<
        ...
        boost_kahypar::is_same<T2, U2>::value || boost_kahypar::is_void<T2>::value || boost_kahypar::is_base_of<T2, U2>::value,
        typename boost_kahypar::copy_cv<T, U>::type,
        typename boost_kahypar::copy_cv<U, T>::type
...
public:
    ...
    typedef typename boost_kahypar::copy_cv<typename boost_kahypar::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using boost_kahypar::conditional in
