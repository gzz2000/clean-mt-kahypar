...
// https://connect.microsoft.com/VisualStudio/feedback/details/800328/std-is-copy-constructible-is-broken
    ...
    typedef boost_kahypar::type_traits::yes_type yes_type;
    typedef boost_kahypar::type_traits::no_type  no_type;

    template <class U>
    static decltype(::boost_kahypar::declval<U&>() = ::boost_kahypar::declval<const U&>(), yes_type() ) test(int);
...
#else
    static BOOST_DEDUCED_TYPENAME boost_kahypar::add_reference<T>::type produce();

    template <class T1>
    static boost_kahypar::type_traits::no_type test(T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
    ...
    BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(produce())) == sizeof(boost_kahypar::type_traits::yes_type)
    ));
   #endif
};
...
struct is_copy_assignable_impl {
    ...
    //types derived from boost_kahypar::noncopyable and types defined as BOOST_MOVEABLE_BUT_NOT_COPYABLE
    typedef BOOST_DEDUCED_TYPENAME boost_kahypar::remove_reference<T>::type unreferenced_t;
    BOOST_STATIC_CONSTANT(bool, value = (
        boost_kahypar::detail::is_copy_assignable_impl2<
            boost_kahypar::is_noncopyable<T>::value
            || boost_kahypar::is_const<unreferenced_t>::value || boost_kahypar::is_array<unreferenced_t>::value
            ,T
        >::value
    ));
    #else
    BOOST_STATIC_CONSTANT(bool, value = (
        boost_kahypar::detail::is_copy_assignable_impl2<
            boost_kahypar::is_noncopyable<T>::value,T
        >::value
    ));
    #endif
};
...
template <class T> struct is_copy_assignable : public integral_constant<bool, ::boost_kahypar::detail::is_copy_assignable_impl<T>::value>{};
