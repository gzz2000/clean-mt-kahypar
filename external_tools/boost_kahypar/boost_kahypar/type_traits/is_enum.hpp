...
{
   BOOST_STATIC_CONSTANT(bool, value = ::boost_kahypar::is_class<T>::value || ::boost_kahypar::is_union<T>::value);
};
...
# else
    template <class U> static ::boost_kahypar::type_traits::yes_type is_class_or_union_tester(void(U::*)(void));

#  if BOOST_WORKAROUND(__MWERKS__, <= 0x3000) // no SFINAE
    static ::boost_kahypar::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester(0)) == sizeof(::boost_kahypar::type_traits::yes_type));
#  else
    template <class U>
    static ::boost_kahypar::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester<T>(0)) == sizeof(::boost_kahypar::type_traits::yes_type));
#  endif
# endif
};
#endif
...
struct is_enum_helper<false>
{
    template <typename T> struct type
    {
       static const bool value = ::boost_kahypar::is_convertible<typename boost_kahypar::add_reference<T>::type, ::boost_kahypar::detail::int_convertible>::value;
    };
};

template <typename T> struct is_enum_impl
{
   //typedef ::boost_kahypar::add_reference<T> ar_t;
   ...
   BOOST_STATIC_CONSTANT(bool, selector =
           ::boost_kahypar::is_arithmetic<T>::value
   ...
   BOOST_STATIC_CONSTANT(bool, selector =
    ...
    typedef ::boost_kahypar::detail::is_enum_helper<
          ::boost_kahypar::detail::is_enum_impl<T>::selector
        > se_t;
#else
    typedef ::boost_kahypar::detail::is_enum_helper<selector> se_t;
#endif
...
template <class T> struct is_enum : public integral_constant<bool, ::boost_kahypar::detail::is_enum_impl<T>::value> {};
