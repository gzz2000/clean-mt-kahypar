...
// forward declaration
namespace boost_kahypar { template<class T> class optional; }
...
namespace boost_kahypar { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const boost_kahypar::any& value_store) const
    {
        const T* value = boost_kahypar::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                boost_kahypar::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                boost_kahypar::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const boost_kahypar::any& value);
    }
    ...
    */
    template<class T, class charT>
    void validate(boost_kahypar::any& v, 
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            boost_kahypar::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(boost_kahypar::any& v, 
    ...
    {
        if (v.empty()) {
            v = boost_kahypar::any(std::vector<T>());
        }
        std::vector<T>* tv = boost_kahypar::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(boost_kahypar::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                boost_kahypar::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
                  ...
                  int)
    {
        ...
        boost_kahypar::any a;
        validate(a, s, (T*)0, 0);
        v = boost_kahypar::any(boost_kahypar::optional<T>(boost_kahypar::any_cast<T>(a)));
    }

    template<class T, class charT>
    void 
    typed_value<T, charT>::
    xparse(boost_kahypar::any& value_store, 
    ...
    value()
    {
        ...
        return boost_kahypar::program_options::value<T>(0);
    }
