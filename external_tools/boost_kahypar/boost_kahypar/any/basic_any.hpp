...
/// \brief \copybrief boost_kahypar::anys::basic_any

#include <boost_kahypar/any/bad_any_cast.hpp>
#include <boost_kahypar/any/fwd.hpp>
...
namespace anys {
    ...
    /// \forcedlink{ValueType} requirements. Use boost_kahypar::any instead if not sure.
    ///
    /// boost_kahypar::anys::basic_any is the drop-in replacement for boost_kahypar::any
    /// that provides control over Small Object Optimization via
    /// `OptimizeForSize` and `OptimizeForAlignment` template parameters.
    ///
    /// There are certain applications that require boost_kahypar::any
    ...
    /// such applications may create a typedef for boost_kahypar::anys::basic_any
    ...
    ///
    /// Otherwise just use boost_kahypar::any.
    ...
    private:
        ...
        static void* small_manager(operation op, basic_any& left, const basic_any* right, const boost_kahypar::typeindex::type_info* info)
        {
            switch (op)
            {
                case Destroy:
                ...
                case Move: {
                    ...
                    BOOST_ASSERT(right->type() == boost_kahypar::typeindex::type_id<ValueType>());
                ...
                case AnyCast:
                    BOOST_ASSERT(info);
                    BOOST_ASSERT(!left.empty());
                    return boost_kahypar::typeindex::type_id<ValueType>() == *info ?
                ...
                case Typeinfo:
                    return const_cast<void*>(static_cast<const void*>(&boost_kahypar::typeindex::type_id<ValueType>().type_info()));
            }
        ...
        template <typename ValueType>
        static void* large_manager(operation op, basic_any& left, const basic_any* right, const boost_kahypar::typeindex::type_info* info)
        {
            switch (op)
            {
                case Destroy:
    ...
    public: // structors
        ...
        {
            static_assert(
                !std::is_same<ValueType, boost_kahypar::any>::value,
                "boost_kahypar::anys::basic_any shall not be constructed from boost_kahypar::any"
            );
            static_assert(
                !anys::detail::is_basic_any<ValueType>::value,
                "boost_kahypar::anys::basic_any<A, B> shall not be constructed from boost_kahypar::anys::basic_any<C, D>"
            );
          ...
          : man(0), content()
        {
            ...
            static_assert(
                !std::is_same<DecayedType, boost_kahypar::any>::value,
                "boost_kahypar::anys::basic_any shall not be constructed from boost_kahypar::any"
            );
            static_assert(
    ...
    public: // modifiers
        ...
        basic_any & operator=(ValueType&& rhs)
        {
            ...
            static_assert(
                !std::is_same<DecayedType, boost_kahypar::any>::value,
                "boost_kahypar::any shall not be assigned into boost_kahypar::anys::basic_any"
            );
            static_assert(
                !anys::detail::is_basic_any<DecayedType>::value || std::is_same<DecayedType, basic_any>::value,
                "boost_kahypar::anys::basic_any<A, B> shall not be assigned into boost_kahypar::anys::basic_any<C, D>"
            );
    ...
    public: // queries
        ...
        /// only at runtime.
        const boost_kahypar::typeindex::type_info& type() const BOOST_NOEXCEPT
        {
            return man
                    ? *static_cast<const boost_kahypar::typeindex::type_info*>(man(Typeinfo, const_cast<basic_any&>(*this), 0, 0))
                    : boost_kahypar::typeindex::type_id<void>().type_info();
        }
    ...
    private: // representation
        ...
        typedef void*(*manager)(operation op, basic_any& left, const basic_any* right, const boost_kahypar::typeindex::type_info* info);
    ...
    {
        return operand->man ?
                static_cast<typename std::remove_cv<ValueType>::type *>(operand->man(basic_any<Size, Alignment>::AnyCast, *operand, 0, &boost_kahypar::typeindex::type_id<ValueType>().type_info()))
                : 0;
    }
    ...
    {
        return boost_kahypar::anys::any_cast<ValueType>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> *>(operand));
    }

    /// \returns ValueType stored in `operand`
    /// \throws boost_kahypar::bad_any_cast if `operand` does not contain
    /// specified ValueType.
    template<typename ValueType, std::size_t OptimizeForSize, std::size_t OptimizeForAlignment>
    ValueType any_cast(basic_any<OptimizeForSize, OptimizeForAlignment> & operand)
    {
        ...
        nonref * result = boost_kahypar::anys::any_cast<nonref>(std::addressof(operand));
        if(!result)
            boost_kahypar::throw_exception(bad_any_cast());
...
#   pragma warning(pop)
    ...
    {
        typedef typename std::remove_reference<ValueType>::type nonref;
        return boost_kahypar::anys::any_cast<const nonref &>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> &>(operand));
    }
    ...
    inline ValueType any_cast(basic_any<OptimizeForSize, OptimizeForAlignment>&& operand)
    {
        static_assert(
            ...
            "boost_kahypar::any_cast shall not be used for getting nonconst references to temporary objects"
        );
        return boost_kahypar::anys::any_cast<ValueType>(operand);
    }
    ...
    {
        return boost_kahypar::anys::unsafe_any_cast<ValueType>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> *>(operand));
    }
    /// @endcond

} // namespace anys

using boost_kahypar::anys::any_cast;
using boost_kahypar::anys::unsafe_any_cast;
