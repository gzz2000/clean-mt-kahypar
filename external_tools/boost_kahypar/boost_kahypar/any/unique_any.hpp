...
/// \brief \copybrief boost_kahypar::anys::unique_any
...
#include <boost_kahypar/any/detail/placeholder.hpp>

#include <boost_kahypar/throw_exception.hpp>
#include <boost_kahypar/type_index.hpp>

namespace boost_kahypar { namespace anys {
...
public:
    ...
    template<typename T>
    unique_any(T&& value, typename std::enable_if<!std::is_same<T&&, boost_kahypar::any&&>::value>::type* = nullptr)
      : content(new holder< typename std::decay<T>::type >(std::forward<T>(value)))
    {
        static_assert(
            !boost_kahypar::anys::detail::is_basic_any< typename std::decay<T>::type >::value,
            "boost_kahypar::anys::unique_any could not be constructed from boost_kahypar::anys::basic_any."
        );

        static_assert(
            !std::is_same<unique_any, typename std::decay<T>::type >::value,
            "boost_kahypar::anys::unique_any could not be copied, only moved."
        );

        static_assert(
            !std::is_same<boost_kahypar::any, typename std::decay<T>::type >::value,
            "boost_kahypar::anys::unique_any could be constructed from an rvalue of boost_kahypar::any, "
            "not a lvalue."
        );
    }

    /// Moves the content of `boost_kahypar::any` into *this.
    ///
    /// \throws Nothing.
    /// \post `value.empty()` is true.
    template <class BoostAny>
    unique_any(BoostAny&& value, typename std::enable_if<std::is_same<BoostAny&&, boost_kahypar::any&&>::value>::type* = nullptr) noexcept
    {
    ...
    typename std::decay<T>::type& emplace(Args&&... args) {
        ...
        content = std::unique_ptr<boost_kahypar::anys::detail::placeholder>(raw_ptr);
        return raw_ptr->held;
    }
    ...
    /// only at runtime.
    const boost_kahypar::typeindex::type_info& type() const noexcept
    {
        return content ? content->type() : boost_kahypar::typeindex::type_id<void>().type_info();
    }

private: // types
    /// @cond
    template<typename T>
    class holder final: public boost_kahypar::anys::detail::placeholder
    {
    public:
        template <class... Args>
        holder(Args&&... args)
        ...
        const boost_kahypar::typeindex::type_info& type() const noexcept override
        {
            return boost_kahypar::typeindex::type_id<T>().type_info();
        }
...
private: // representation
    ...
    std::unique_ptr<boost_kahypar::anys::detail::placeholder> content;
    /// @endcond
};
...
{
    return operand && operand->type() == boost_kahypar::typeindex::type_id<T>()
...
/// \throws boost_kahypar::bad_any_cast if `operand` does not contain specified `T`.
template<typename T>
