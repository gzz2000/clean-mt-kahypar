...
protected:
    ...
    void save_override(const boost_kahypar::archive::version_type & t){
        ...
        if(boost_kahypar::serialization::library_version_type(6) < lvt){
            const boost_kahypar::uint_least16_t x = t;
            * this->This() << x;
        }
        else{
    ...
    void save_override(const boost_kahypar::serialization::item_version_type & t){
        library_version_type lvt = this->get_library_version();
        if(boost_kahypar::serialization::library_version_type(7) < lvt){
            this->detail_common_oarchive::save_override(t);
        }
    ...
    void save_override(class_id_type & t){
        ...
        if(boost_kahypar::serialization::library_version_type(6) < lvt){
            const boost_kahypar::int_least16_t x = t;
            * this->This() << x;
        }
        else{
            const int x = t;
    ...
    void save_override(const serialization::collection_size_type & t){
        if (get_library_version() < boost_kahypar::serialization::library_version_type(6)){
            unsigned int x=0;
            * this->This() >> x;
