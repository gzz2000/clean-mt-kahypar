...
class helper_collection
{
    ...
    typedef std::pair<
        const void *,
        boost_kahypar::shared_ptr<void>
    > helper_value_type;
    template<class T>
    boost_kahypar::shared_ptr<void> make_helper_ptr(){
        // use boost_kahypar::shared_ptr rather than std::shared_ptr to maintain
        // c++03 compatibility
        return boost_kahypar::make_shared<T>();
    }
