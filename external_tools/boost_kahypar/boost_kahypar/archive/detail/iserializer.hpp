...
#include <boost_kahypar/mpl/greater_equal.hpp>
#include <boost_kahypar/mpl/equal_to.hpp>
#include <boost_kahypar/core/no_exceptions_support.hpp>

#ifndef BOOST_SERIALIZATION_DEFAULT_TYPE_INFO
    #include <boost_kahypar/serialization/extended_type_info_typeid.hpp>
#endif
#include <boost_kahypar/serialization/throw_exception.hpp>
#include <boost_kahypar/serialization/smart_cast.hpp>
#include <boost_kahypar/serialization/static_warning.hpp>
...
#include <boost_kahypar/serialization/assume_abstract.hpp>
...
#include <boost_kahypar/serialization/void_cast.hpp>
#include <boost_kahypar/serialization/collection_size_type.hpp>
#include <boost_kahypar/serialization/singleton.hpp>
#include <boost_kahypar/serialization/wrapper.hpp>
#include <boost_kahypar/serialization/array_wrapper.hpp>

// the following is need only for dynamic cast of polymorphic pointers
#include <boost_kahypar/archive/archive_exception.hpp>
#include <boost_kahypar/archive/detail/basic_iarchive.hpp>
#include <boost_kahypar/archive/detail/basic_iserializer.hpp>
#include <boost_kahypar/archive/detail/basic_pointer_iserializer.hpp>
#include <boost_kahypar/archive/detail/archive_serializer_map.hpp>
#include <boost_kahypar/archive/detail/check.hpp>
...
private:
    void destroy(/*const*/ void *address) const BOOST_OVERRIDE {
        boost_kahypar::serialization::access::destroy(static_cast<T *>(address));
    }
public:
    explicit iserializer() :
        basic_iserializer(
            boost_kahypar::serialization::singleton<
                typename
                boost_kahypar::serialization::type_info_implementation< T >::type
            >::get_const_instance()
        )
    {}
    ...
    bool class_info() const BOOST_OVERRIDE {
        return boost_kahypar::serialization::implementation_level< T >::value
            >= boost_kahypar::serialization::object_class_info;
    }
    bool tracking(const unsigned int /* flags */) const BOOST_OVERRIDE {
        return boost_kahypar::serialization::tracking_level< T >::value
                == boost_kahypar::serialization::track_always
            || ( boost_kahypar::serialization::tracking_level< T >::value
                == boost_kahypar::serialization::track_selectively
                && serialized_as_pointer());
    }
    version_type version() const BOOST_OVERRIDE {
        return version_type(::boost_kahypar::serialization::version< T >::value);
    }
    bool is_polymorphic() const BOOST_OVERRIDE {
        return boost_kahypar::is_polymorphic< T >::value;
    }
...
BOOST_DLLEXPORT void iserializer<Archive, T>::load_object_data(
    ...
    if(file_version > static_cast<const unsigned int>(version()))
        boost_kahypar::serialization::throw_exception(
            archive::archive_exception(
                boost_kahypar::archive::archive_exception::unsupported_class_version,
                get_debug_info()
            )
        );
    #endif
    ...
    boost_kahypar::serialization::serialize_adl(
        boost_kahypar::serialization::smart_cast_reference<Archive &>(ar),
        * static_cast<T *>(x),
        file_version
    );
}
...
struct heap_allocation {
    // boost_kahypar::has_new_operator< T > doesn't work on these compilers
    #if BOOST_SERIALIZATION_DONT_USE_HAS_NEW_OPERATOR
        ...
        static T * invoke_new() {
            typedef typename
                mpl::eval_if<
                    boost_kahypar::has_new_operator< T >,
...
private:
    ...
    const basic_iserializer & get_basic_serializer() const BOOST_OVERRIDE {
        return boost_kahypar::serialization::singleton<
...
) const
{
    ...
    BOOST_TRY {
        ...
        boost_kahypar::serialization::load_construct_data_adl<Archive, T>(
            ar_impl,
            static_cast<T *>(t),
            file_version
        );
    ...
    ar_impl >> boost_kahypar::serialization::make_nvp(NULL, * static_cast<T *>(t));
}
...
struct load_non_pointer_type {
    ...
    struct load_only {
        ...
        static void invoke(Archive & ar, const T & t){
            ...
            boost_kahypar::serialization::serialize_adl(
                ar,
                const_cast<T &>(t),
                boost_kahypar::serialization::version< T >::value
            );
        }
    };
    ...
    struct load_standard {
        template<class T>
        static void invoke(Archive &ar, const T & t){
            void * x = boost_kahypar::addressof(const_cast<T &>(t));
            ar.load_object(
                x,
                boost_kahypar::serialization::singleton<
                    iserializer<Archive, T>
    ...
    static void invoke(Archive & ar, T &t){
        typedef typename mpl::eval_if<
                // if its primitive
                mpl::equal_to<
                    boost_kahypar::serialization::implementation_level< T >,
                    mpl::int_<boost_kahypar::serialization::primitive_type>
                >,
            ...
            mpl::greater_equal<
                        boost_kahypar::serialization::implementation_level< T >,
                        mpl::int_<boost_kahypar::serialization::object_class_info>
                    >,
            // do standard load
        ...
        typename mpl::eval_if<
            // no tracking
                    mpl::equal_to<
                        boost_kahypar::serialization::tracking_level< T >,
                        mpl::int_<boost_kahypar::serialization::track_never>
                >,
                // do a fast load
...
struct load_pointer_type {
    struct abstract
    {
        ...
        static const basic_pointer_iserializer * register_type(Archive & /* ar */){
            ...
            BOOST_STATIC_ASSERT(boost_kahypar::is_polymorphic< T >::value);
    ...
    static const basic_pointer_iserializer * register_type(Archive &ar, const T* const /*t*/){
        ...
        typedef typename
            mpl::eval_if<
                boost_kahypar::serialization::is_abstract<const T>,
                boost_kahypar::mpl::identity<abstract>,
                boost_kahypar::mpl::identity<non_abstract>
            >::type typex;
    ...
    static T * pointer_tweak(
        const boost_kahypar::serialization::extended_type_info & eti,
        void const * const t,
        const T &
    ) {
        // tweak the pointer back to the base class
        void * upcast = const_cast<void *>(
            boost_kahypar::serialization::void_upcast(
                eti,
                boost_kahypar::serialization::singleton<
                    typename
                    boost_kahypar::serialization::type_info_implementation< T >::type
    ...
    find(const boost_kahypar::serialization::extended_type_info & type){
...
struct load_enum_type {
    ...
    static void invoke(Archive &ar, T &t){
        ...
        int i;
        ar >> boost_kahypar::serialization::make_nvp(NULL, i);
        t = static_cast< T >(i);
    }
};
...
struct load_array_type {
    ...
    static void invoke(Archive &ar, T &t){
        ...
        boost_kahypar::serialization::collection_size_type count;
        ar >> BOOST_SERIALIZATION_NVP(count);
        if(static_cast<std::size_t>(count) > current_count)
            boost_kahypar::serialization::throw_exception(
                archive::archive_exception(
                    boost_kahypar::archive::archive_exception::array_size_too_short
                )
            );
        // explicit template arguments to pass intel C++ compiler
        ar >> serialization::make_array<
            value_type,
            boost_kahypar::serialization::collection_size_type
        >(
