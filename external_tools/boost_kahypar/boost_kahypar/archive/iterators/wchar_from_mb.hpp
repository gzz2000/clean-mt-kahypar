    ...
    >
{
    ...
    typedef typename boost_kahypar::iterator_adaptor<
        wchar_from_mb<Base>,
        Base,
        wchar_t,
        single_pass_traversal_tag,
    ...
    boost_kahypar::archive::detail::utf8_codecvt_facet m_codecvt_facet;
    std::mbstate_t m_mbs;

    template<typename T>
    struct sliding_buffer {
        boost_kahypar::array<T, 32> m_buffer;
        typename boost_kahypar::array<T, 32>::const_iterator m_next_available;
        typename boost_kahypar::array<T, 32>::iterator m_next;
        bool m_done;
        // default ctor
        sliding_buffer() :
...
void wchar_from_mb<Base>::drain(){
    ...
    for(;;){
        typename boost_kahypar::iterators::iterator_reference<Base>::type c = *(this->base_reference());
    ...
    const typename boost_kahypar::iterators::iterator_value<Base>::type * input_new_start;
