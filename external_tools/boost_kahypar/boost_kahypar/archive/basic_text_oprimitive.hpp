...
protected:
    ...
    boost_kahypar::archive::codecvt_null<typename OStream::char_type> codecvt_null_facet;
    ...
    template<class T>
    void save_impl(const T &t, boost_kahypar::mpl::bool_<false> &){
        if(os.fail())
            boost_kahypar::serialization::throw_exception(
                archive_exception(archive_exception::output_stream_error)
            );
    ...
    struct is_float {
        typedef typename mpl::bool_<
            boost_kahypar::is_floating_point<T>::value
    ...
    template<class T>
    void save_impl(const T &t, boost_kahypar::mpl::bool_<true> &){
