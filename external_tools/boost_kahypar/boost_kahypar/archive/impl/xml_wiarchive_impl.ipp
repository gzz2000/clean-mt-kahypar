...
xml_wiarchive_impl<Archive>::init(){
    ...
    this->set_library_version(
        boost_kahypar::serialization::library_version_type(gimpl->rv.version)
    );
}
    ...
    gimpl(new xml_wgrammar())
{
    if(0 == (flags & no_codecvt)){
        archive_locale = std::locale(
            is_.getloc(),
            new boost_kahypar::archive::detail::utf8_codecvt_facet
        );
...
BOOST_WARCHIVE_DECL
xml_wiarchive_impl<Archive>::~xml_wiarchive_impl(){
    if(boost_kahypar::core::uncaught_exceptions() > 0)
        return;
