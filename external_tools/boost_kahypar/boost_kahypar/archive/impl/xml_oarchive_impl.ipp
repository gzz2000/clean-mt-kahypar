...
#include <boost_kahypar/archive/wcslen.hpp>
...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef boost_kahypar::archive::iterators::mb_from_wchar<
        boost_kahypar::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(begin),
        translator(end),
        boost_kahypar::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef boost_kahypar::archive::iterators::xml_escape<
        const char * 
