...
#include <boost_kahypar/detail/lcast_precision.hpp>
...
#       error "boost_kahypar::lexical_cast to use only 'C' locale during conversions."
#   endif
#endif
...
#include <boost_kahypar/lexical_cast/detail/lcast_unsigned_converters.hpp>
#include <boost_kahypar/lexical_cast/detail/lcast_basic_unlockedbuf.hpp>
#include <boost_kahypar/lexical_cast/detail/inf_nan.hpp>
...
namespace boost_kahypar {
    ...
    // forward declaration of boost_kahypar::basic_string_view from Utility
    template<class Ch, class Tr> class basic_string_view;
}

namespace boost_kahypar { namespace detail { namespace lcast {

    template <typename T>
    struct exact {
        static_assert(!boost_kahypar::is_const<T>::value, "");
        static_assert(!boost_kahypar::is_reference<T>::value, "");
    ...
    private:
        ...
        bool shl_char(T ch) {
            static_assert(sizeof(T) <= sizeof(CharT),
                "boost_kahypar::lexical_cast does not support narrowing of char types."
                "Use boost_kahypar::locale instead" );
#ifndef BOOST_LEXICAL_CAST_ASSUME_C_LOCALE
            std::locale loc;
        ...
        inline bool shl_signed(const T n) {
            CharT* tmp_finish = buffer + CharacterBufferSize;
            typedef typename boost_kahypar::make_unsigned<T>::type utype;
        ...
        bool shl_real_type(float val, char* begin) {
            ...
            finish = start +
                boost_kahypar::core::snprintf(begin, CharacterBufferSize,
                "%.*g", static_cast<int>(boost_kahypar::detail::lcast_get_precision<float>()), val_as_double);
            return finish > start;
        }
        ...
        bool shl_real_type(double val, char* begin) {
            finish = start +
                boost_kahypar::core::snprintf(begin, CharacterBufferSize,
                "%.*g", static_cast<int>(boost_kahypar::detail::lcast_get_precision<double>()), val);
            return finish > start;
        }
        ...
        bool shl_real_type(long double val, char* begin) {
            finish = start +
                boost_kahypar::core::snprintf(begin, CharacterBufferSize,
                "%.*Lg", static_cast<int>(boost_kahypar::detail::lcast_get_precision<long double>()), val );
            return finish > start;
        }
#else
        bool shl_real_type(long double val, char* begin) {
        ...
        bool shl_real_type(float val, wchar_t* begin) {
            const double val_as_double = val;
            finish = start + boost_kahypar::core::swprintf(
                begin, CharacterBufferSize, L"%.*g",
                static_cast<int>(boost_kahypar::detail::lcast_get_precision<float >()),
                val_as_double
            );
            return finish > start;
        }
        ...
        bool shl_real_type(double val, wchar_t* begin) {
            finish = start + boost_kahypar::core::swprintf(
              begin, CharacterBufferSize, L"%.*g",
              static_cast<int>(boost_kahypar::detail::lcast_get_precision<double>()),
              val
            );
            return finish > start;
        }
        ...
        bool shl_real_type(long double val, wchar_t* begin) {
            finish = start + boost_kahypar::core::swprintf(
                ...
                static_cast<int>(boost_kahypar::detail::lcast_get_precision<long double>()),
                val
            );
            return finish > start;
        }
    ...
    public:
        template <class C>
        using enable_if_compatible_char_t = typename boost_kahypar::enable_if_c<
            boost_kahypar::is_same<const C, const CharT>::value || (
                boost_kahypar::is_same<const char, const CharT>::value && (
                    boost_kahypar::is_same<const C, const unsigned char>::value ||
                    boost_kahypar::is_same<const C, const signed char>::value
                )
            ), bool
        >::type;
        ...
        template<class CharTraits, class Alloc>
        bool stream_in(lcast::exact<boost_kahypar::container::basic_string<CharT,CharTraits,Alloc>> x) noexcept {
            start = x.payload.data();
            finish = start + x.payload.length();
            return true;
        }
        ...
        bool stream_in(lcast::exact<boost_kahypar::conversion::detail::buffer_view<CharT>> x) noexcept {
        ...
        stream_in(lcast::exact<boost_kahypar::iterator_range<C*>> x) noexcept {
            auto buf = boost_kahypar::conversion::detail::make_buffer_view(x.payload.begin(), x.payload.end());
        ...
        template <class C>
        typename boost_kahypar::enable_if_c<boost_kahypar::detail::is_character<C>::value, bool>::type
        ...
        template <class Type>
        typename boost_kahypar::enable_if_c<boost_kahypar::is_signed<Type>::value && !boost_kahypar::is_enum<Type>::value, bool>::type
                stream_in(lcast::exact<Type> x)                  { return shl_signed(x.payload); }

        template <class Type>
        typename boost_kahypar::enable_if_c<boost_kahypar::is_unsigned<Type>::value && !boost_kahypar::is_enum<Type>::value, bool>::type
        ...
        enable_if_compatible_char_t<C>
        stream_in(lcast::exact<boost_kahypar::array<C, N>> x) noexcept {
        ...
        template <class C, class CharTraits>
        enable_if_compatible_char_t<C>
        stream_in(lcast::exact<boost_kahypar::basic_string_view<C, CharTraits>> x) noexcept {
            start = reinterpret_cast<const CharT*>(x.payload.data());
            finish = start + x.payload.size();
            return true;
        }
    ...
    private:
        ...
        bool shl_input_streamable(InputStreamable& input) {
            ...
            static_assert(boost_kahypar::is_same<char, CharT>::value, "");
#endif
    ...
    public:
        template <class Type>
        typename boost_kahypar::enable_if_c<boost_kahypar::detail::is_character<Type>::value && sizeof(char) == sizeof(Type), bool>::type
    ...
    private:
        ...
        {
            static_assert(
                !boost_kahypar::is_pointer<InputStreamable>::value,
                "boost_kahypar::lexical_cast can not convert to pointers"
            );

#if defined(BOOST_NO_STRINGSTREAM) || defined(BOOST_NO_STD_LOCALE)
            static_assert(boost_kahypar::is_same<char, CharT>::value,
                "boost_kahypar::lexical_cast can not convert, because your STL library does not "
        ...
        inline bool shr_xchar(T& output) noexcept {
            static_assert(sizeof(CharT) == sizeof(T),
                "boost_kahypar::lexical_cast does not support narrowing of character types."
                "Use boost_kahypar::locale instead" );
    ...
    public:
        ...
        bool stream_out(boost_kahypar::ulong_long_type& output)     { return shr_unsigned(output); }
        bool stream_out(boost_kahypar::long_long_type& output)      { return shr_signed(output); }
        ...
        bool stream_out(boost_kahypar::uint128_type& output)        { return shr_unsigned(output); }
        bool stream_out(boost_kahypar::int128_type& output)         { return shr_signed(output); }
#endif
        ...
        template<class CharTraits, class Alloc>
        bool stream_out(boost_kahypar::container::basic_string<CharT,CharTraits,Alloc>& str) {
            str.assign(start, finish); return true;
        }
        ...
        template <class C, std::size_t N>
        bool stream_out(boost_kahypar::array<C, N>& output) noexcept {
            static_assert(sizeof(C) == sizeof(CharT), "");
            return shr_std_array<N>(output);
        }
...
}}} // namespace boost_kahypar::detail::lcast
