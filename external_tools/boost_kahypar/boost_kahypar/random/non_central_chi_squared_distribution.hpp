...
public:
    ...
    RealType operator()(URNG& eng) 
    {
        using std::sqrt;
        if (_param.k() > 1) {
            boost_kahypar::random::normal_distribution<RealType> n_dist;
            boost_kahypar::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            boost_kahypar::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            boost_kahypar::random::poisson_distribution<>::result_type _p = p_dist(eng);
            boost_kahypar::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
