 ...
 *
 * Much of the code here taken by boost_kahypar::math::hyperexponential_distribution.
...
#include <boost_kahypar/random/detail/operators.hpp>
#include <boost_kahypar/random/detail/vector_io.hpp>
...
namespace boost_kahypar { namespace random {
...
bool check_probabilities(std::vector<RealT> const& probabilities)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(boost_kahypar::core::isfinite)(probabilities[i]))
        {
            return false;
        }
        sum += probabilities[i];
...
bool check_rates(std::vector<RealT> const& rates)
{
    ...
    {
        if (rates[i] <= 0
            || !(boost_kahypar::core::isfinite)(rates[i]))
        {
            return false;
        }
    }
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename ProbRangeT, typename RateRangeT>
                param_type(ProbRangeT const& prob_range,
                           ...
                           typename boost_kahypar::disable_if_c<boost_kahypar::has_pre_increment<ProbRangeT>::value || boost_kahypar::has_pre_increment<RateRangeT>::value>::type* = 0)
        : probs_(boost_kahypar::begin(prob_range), boost_kahypar::end(prob_range)),
          rates_(boost_kahypar::begin(rate_range), boost_kahypar::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename boost_kahypar::enable_if_c<boost_kahypar::has_pre_increment<RateIterT>::value>::type* = 0)
        ...
        : probs_(boost_kahypar::size(rate_range), 1), // Will be normalized below
          rates_(boost_kahypar::begin(rate_range), boost_kahypar::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
    ...
    : dd_(std::vector<RealT>(boost_kahypar::size(rate_range), 1)),
      rates_(boost_kahypar::begin(rate_range), boost_kahypar::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return boost_kahypar::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    {
        dd_.param(typename boost_kahypar::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    private: boost_kahypar::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace boost_kahypar::random
