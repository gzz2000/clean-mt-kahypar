...
public:
    ...
    result_type operator()(Engine& eng) const
    {
        ...
        return generate(eng, boost_kahypar::random::traits::is_integral<base_result>());
    }
...
private:
    ...
    // \cond show_private
    template<class Engine>
    result_type generate(Engine& eng, boost_kahypar::true_type) const
    {
        // equivalent to (eng() - eng.min()) % (_max - _min + 1) + _min,
        // but guarantees no overflow.
        typedef typename Engine::result_type base_result;
        typedef typename boost_kahypar::random::traits::make_unsigned<base_result>::type base_unsigned;
        typedef typename boost_kahypar::random::traits::make_unsigned_or_unbounded<result_type>::type range_type;
#ifdef BOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
        ...
        if(range >= base_range) {
            return boost_kahypar::random::detail::add<range_type, result_type>()(
    ...
    template<class Engine>
    result_type generate(Engine& eng, boost_kahypar::false_type) const
    {
        typedef typename Engine::result_type base_result;
        typedef typename boost_kahypar::random::traits::make_unsigned<result_type>::type range_type;
        range_type range = random::detail::subtract<result_type>()(_max, _min);
        base_result val = boost_kahypar::uniform_01<base_result>()(eng);
        ...
        return boost_kahypar::random::detail::add<range_type, result_type>()(offset , _min);
    }
    // \endcond
