...
#include <boost_kahypar/random/detail/niederreiter_base2_table.hpp>
...
{
  if (v.none())
    boost_kahypar::throw_exception( std::invalid_argument("bitset_log2") );
...
public:
  ...
  {
    typedef boost_kahypar::dynamic_bitset<> bitset_type;
    ...
    for (std::size_t dim = 0; dim != dimension; ++dim)
    {
      ...
      if (poly > (std::numeric_limits<value_type>::max)()) {
        boost_kahypar::throw_exception( std::range_error("niederreiter_base2: polynomial value outside the given value type range") );
      }
 ...
 */
typedef niederreiter_base2_engine<boost_kahypar::uint_least64_t, 64u, default_niederreiter_base2_table> niederreiter_base2;
