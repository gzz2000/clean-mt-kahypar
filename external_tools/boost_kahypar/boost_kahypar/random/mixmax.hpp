...
#include <boost_kahypar/random/detail/seed.hpp>
#include <boost_kahypar/random/detail/seed_impl.hpp>
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> // MIXMAX TEMPLATE PARAMETERS
class mixmax_engine{
public:
    // Interfaces required by C++11 std::random and boost_kahypar::random
    typedef boost_kahypar::uint64_t result_type ;
    BOOST_STATIC_CONSTANT(boost_kahypar::uint64_t,mixmax_min=0);
    BOOST_STATIC_CONSTANT(boost_kahypar::uint64_t,mixmax_max=((1ULL<<61)-1));
    ...
    explicit mixmax_engine(boost_kahypar::uint64_t);          ///< Constructor, one 64-bit seed
    ...
    void seed(boost_kahypar::uint64_t seedval=default_seed){seed_uniquestream( &S, 0, 0, (uint32_t)(seedval>>32), (uint32_t)seedval );} ///< seed with one 64-bit seed
    
private: // DATATYPES
    struct rng_state_st{
        boost_kahypar::array<boost_kahypar::uint64_t, Ndim> V;
        boost_kahypar::uint64_t sumtot;
        int counter;
    };
...
public: // SEEDING FUNCTIONS
    ...
    boost_kahypar::uint64_t operator()(){
        if (S.counter<=(Ndim-1) ){
    ...
    void discard(boost_kahypar::uint64_t nsteps) { for(boost_kahypar::uint64_t j = 0; j < nsteps; ++j)  (*this)(); } ///< discard n steps, required in boost_kahypar::random
    ...
    operator<< (std::basic_ostream<CharT,Traits>& ost, const mixmax_engine& me){
        ...
        ost <<  (boost_kahypar::uint64_t)me.S.V[j] << " ";
        }
        ost << "\n";
        ost.flush();
        return ost;
    ...
    operator>> (std::basic_istream<CharT,Traits> &in, mixmax_engine& me){
        ...
        boost_kahypar::array<boost_kahypar::uint64_t, Ndim> vec;
        boost_kahypar::uint64_t sum=0, savedsum=0, counter=0;
        in >> counter >> std::ws;
...
BOOST_STATIC_CONSTANT(boost_kahypar::uint64_t, M61=2305843009213693951ULL);
BOOST_STATIC_CONSTANT(boost_kahypar::uint64_t, default_seed=1);
inline boost_kahypar::uint64_t MOD_MERSENNE(boost_kahypar::uint64_t k) {return ((((k)) & M61) + (((k)) >> BITS) );}
inline boost_kahypar::uint64_t MULWU(boost_kahypar::uint64_t k);
...
inline boost_kahypar::uint64_t iterate_raw_vec(boost_kahypar::uint64_t* Y, boost_kahypar::uint64_t sumtotOld);
inline boost_kahypar::uint64_t apply_bigskip(boost_kahypar::uint64_t* Vout, boost_kahypar::uint64_t* Vin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID );
inline boost_kahypar::uint64_t modadd(boost_kahypar::uint64_t foo, boost_kahypar::uint64_t bar);
inline boost_kahypar::uint64_t fmodmulM61(boost_kahypar::uint64_t cum, boost_kahypar::uint64_t s, boost_kahypar::uint64_t a);
};

template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine()
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine(boost_kahypar::uint64_t seedval){
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine(uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID){
    // constructor, four 32-bit seeds for 128-bit seeding flexibility
    seed_uniquestream( &S, clusterID,  machineID,  runID,  streamID );
}

template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::MULWU (uint64_t k){ return (( (k)<<(SPECIALMUL) & M61) ^ ( (k) >> (BITS-SPECIALMUL))  )  ;}

template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> boost_kahypar::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::iterate_raw_vec(boost_kahypar::uint64_t* Y, boost_kahypar::uint64_t sumtotOld){
    // operates with a raw vector, uses known sum of elements of Y
    boost_kahypar::uint64_t  tempP=0, tempV=sumtotOld;
    Y[0] = tempV;
    boost_kahypar::uint64_t sumtot = Y[0], ovflow = 0; // will keep a running sum of all new elements
    for (int i=1; i<Ndim; i++){
        boost_kahypar::uint64_t tempPO = MULWU(tempP);
        tempV = (tempV+tempPO);
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> void mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::seed_vielbein(rng_state_t* X, unsigned int index){
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> void mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::seed_uniquestream( rng_state_t* Xin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID ){
    seed_vielbein(Xin,0);
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> boost_kahypar::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::apply_bigskip( boost_kahypar::uint64_t* Vout, boost_kahypar::uint64_t* Vin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID ){
    /*
    ...
    const    boost_kahypar::uint64_t skipMat17[128][17] =
#include "boost_kahypar/random/detail/mixmax_skip_N17.ipp"
    ;
    
    const boost_kahypar::uint64_t* skipMat[128];
    BOOST_ASSERT(Ndim==17);
    for (int i=0; i<128; i++) { skipMat[i] = skipMat17[i];}
    
    uint32_t IDvec[4] = {streamID, runID, machineID, clusterID};
    boost_kahypar::uint64_t Y[Ndim], cum[Ndim];
    boost_kahypar::uint64_t sumtot=0;
    ...
    for (int IDindex=0; IDindex<4; IDindex++) { // go from lower order to higher order ID
        ...
        while (id){
            if (id & 1) {
                boost_kahypar::uint64_t* rowPtr = (boost_kahypar::uint64_t*)skipMat[r + IDindex*8*sizeof(uint32_t)];
                for (int i=0; i<Ndim; i++){ cum[i] = 0; }
                for (int j=0; j<Ndim; j++){              // j is lag, enumerates terms of the poly
                    // for zero lag Y is already given
                    boost_kahypar::uint64_t coeff = rowPtr[j]; // same coeff for all i
                    for (int i =0; i<Ndim; i++){
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> inline boost_kahypar::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::fmodmulM61(boost_kahypar::uint64_t cum, boost_kahypar::uint64_t s, boost_kahypar::uint64_t a){
    // works on all platforms, including 32-bit linux, PPC and PPC64, ARM and Windows
    const boost_kahypar::uint64_t MASK32=0xFFFFFFFFULL;
    boost_kahypar::uint64_t o,ph,pl,ah,al;
    o=(s)*a;
    ph = ((s)>>32);
    pl = (s) & MASK32;
    ah = a>>32;
...
template <int Ndim, unsigned int SPECIALMUL, boost_kahypar::int64_t SPECIAL> boost_kahypar::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::modadd(boost_kahypar::uint64_t foo, boost_kahypar::uint64_t bar){
    return MOD_MERSENNE(foo+bar);
}

/* @copydoc boost_kahypar::random::detail::mixmax_engine_doc */
