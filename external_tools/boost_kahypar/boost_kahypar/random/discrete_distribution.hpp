...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(boost_kahypar::begin(range), boost_kahypar::end(range))
        {
            normalize();
        }
    ...
    {
        init(boost_kahypar::begin(range), boost_kahypar::end(range));
    }
    /**
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::boost_kahypar::is_integral<WeightType>::value)
