...
#include <iterator>
#include <boost_kahypar/config/no_tr1/cmath.hpp>         // std::pow
...
#include <boost_kahypar/random/detail/generator_seed_seq.hpp>
...
public:
    ...
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_engine,
        UIntType, value)
    {
        minstd_rand0 intgen(static_cast<boost_kahypar::uint32_t>(value));
    ...
    {
        for(boost_kahypar::uintmax_t j = 0; j < z; ++j) {
            (*this)();
        }
    }
...
public:
    ...
    BOOST_STATIC_CONSTANT(boost_kahypar::uint32_t, default_seed = 331u);
     ...
     */
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_01_engine, boost_kahypar::uint32_t, value)
    {
...
template<class RealType, int w, unsigned int p, unsigned int q>
const boost_kahypar::uint32_t lagged_fibonacci_01_engine<RealType,w,p,q>::default_seed;
#endif
...
public:
    lagged_fibonacci_01() {}
    BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_01, boost_kahypar::uint32_t, val)
    { this->seed(val); }
...
/** @copydoc boost_kahypar::random::detail::lagged_fibonacci_doc */
