...
inline void check_seed_sign(const Integer& v, const boost_kahypar::true_type)
{
  if (v < 0)
  {
    boost_kahypar::throw_exception( std::range_error("seed must be a positive integer") );
  }
}
template <typename Integer>
inline void check_seed_sign(const Integer&, const boost_kahypar::false_type) {}
...
public:
  ...
  //!
  //!Throws: range_error.
  void discard(boost_kahypar::uintmax_t z)
  {
    ...
    boost_kahypar::uintmax_t vec_n = z / dimension_value;
    ...
    discard_vector(vec_n - static_cast<boost_kahypar::uintmax_t>(corr));
...
private:
  ...
  result_type next_state()
  {
    ...
    boost_kahypar::throw_exception( std::range_error("qrng_base: next_state") );
  }

  // Discards z consecutive s-dimensional vectors,
  // and preserves the position of the element-to-read
  void discard_vector(boost_kahypar::uintmax_t z)
  {
    const boost_kahypar::uintmax_t max_z = (std::numeric_limits<size_type>::max)() - seq_count;

    // Don't allow seq_count + z overflows here
    if (max_z < z)
      boost_kahypar::throw_exception( std::range_error("qrng_base: discard_vector") );
  ...
  {
    if (dimension == 0)
      boost_kahypar::throw_exception( std::invalid_argument("qrng_base: zero dimension") );
    return dimension;
  }
...
inline void dimension_assert(const char* generator, std::size_t dim, std::size_t maxdim)
{
  if (!dim || dim > maxdim)
  {
    ...
    boost_kahypar::throw_exception( std::invalid_argument(os.str()) );
  }
}
