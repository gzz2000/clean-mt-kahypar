...
#include <boost_kahypar/random/detail/large_arithmetic.hpp>
...
public:
  ...
  static IntType pow(IntType a, boost_kahypar::uintmax_t exponent)
  {
      IntType result = 1;
      while(exponent != 0) {
          if(exponent % 2 == 1) {
...
private:
  ...
  static IntType mult_general(IntType a, IntType b)
  {
    ...
    if(::boost_kahypar::uintmax_t(modulus) <=
        (::std::numeric_limits< ::boost_kahypar::uintmax_t>::max)() / modulus)
    {
      return static_cast<IntType>(boost_kahypar::uintmax_t(a) * b % modulus);
    } else {
