...
#include <boost_kahypar/core/bit.hpp> // lsb
...
template<class T> static int lsb( T x )
{
  ...
  return boost_kahypar::core::countr_zero( x );
}
...
template<class T> static int msb( T x )
{
  ...
  return std::numeric_limits<T>::digits - 1 - boost_kahypar::core::countl_zero( x );
}
...
private:
  ...
  struct check_bit_range {
    static void raise_bit_count() {
      boost_kahypar::throw_exception( std::range_error("gray_coded_qrng: bit_count") );
    }
  ...
  void seed(const size_type init)
  {
    if (init != this->curr_seq())
    {
      ...
      if (BOOST_UNLIKELY(!(init < seq_code)))
        boost_kahypar::throw_exception( std::range_error("gray_coded_qrng: seed") );
