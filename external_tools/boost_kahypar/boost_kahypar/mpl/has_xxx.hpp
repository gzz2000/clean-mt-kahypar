...
#include <boost_kahypar/mpl/aux_/type_wrapper.hpp>
#include <boost_kahypar/mpl/aux_/yes_no.hpp>
#include <boost_kahypar/mpl/aux_/config/gcc.hpp>
#include <boost_kahypar/mpl/aux_/config/has_xxx.hpp>
#include <boost_kahypar/mpl/aux_/config/msvc_typename.hpp>
...
#include <boost_kahypar/preprocessor/array/elem.hpp>
...
#include <boost_kahypar/preprocessor/repetition/enum_trailing_params.hpp>
...
template< typename T, typename name = ::boost_kahypar::mpl::aux::has_xxx_tag > \
struct BOOST_PP_CAT(trait,_impl) : T \
{ \
    static boost_kahypar::mpl::aux::no_tag \
    test(void(*)(::boost_kahypar::mpl::aux::has_xxx_tag)); \
    \
    static boost_kahypar::mpl::aux::yes_tag test(...); \
    \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(test(static_cast<void(*)(name)>(0))) \
            != sizeof(boost_kahypar::mpl::aux::no_tag) \
        ); \
    typedef boost_kahypar::mpl::bool_<value> type; \
}; \
\
template< typename T, typename fallback_ = boost_kahypar::mpl::bool_<default_> > \
struct trait \
    : boost_kahypar::mpl::if_c< \
          boost_kahypar::mpl::aux::msvc_is_incomplete<T>::value \
        , boost_kahypar::mpl::bool_<false> \
...
{ \
    BOOST_STATIC_CONSTANT(bool, value = false); \
    typedef boost_kahypar::mpl::bool_<false> type; \
}; \
/**/
...
{ \
    ...
    { \
        template < class U > \
        static int  resolve( boost_kahypar::mpl::aux::type_wrapper<U> const volatile * \
                           , boost_kahypar::mpl::aux::type_wrapper<typename U::name >* = 0 ); \
        static char resolve( ... ); \
    }; \
    typedef boost_kahypar::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT( bool, value = ( sizeof( trait_tester_impl::resolve( static_cast< t_ * >(0) ) ) == sizeof(int) ) ); \
}; \
template< typename T, typename fallback_ = boost_kahypar::mpl::bool_<default_> > \
struct trait           \
{                      \
    BOOST_STATIC_CONSTANT( bool, value = (trait_tester< T, boost_kahypar::is_class< T >::value >::value) );     \
    typedef boost_kahypar::mpl::bool_< trait< T, fallback_ >::value > type; \
};
...
{ \
    ...
    { \
        template< typename U > \
        static boost_kahypar::mpl::aux::yes_tag test( \
              boost_kahypar::mpl::aux::type_wrapper<U> const volatile* \
            , boost_kahypar::mpl::aux::type_wrapper<BOOST_MSVC_TYPENAME U::name>* = 0 \
            ); \
    \
        static boost_kahypar::mpl::aux::no_tag test(...); \
    }; \
    \
    typedef boost_kahypar::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(gcc_3_2_wknd::test(static_cast<t_*>(0))) \
            == sizeof(boost_kahypar::mpl::aux::yes_tag) \
        ); \
    typedef boost_kahypar::mpl::bool_<value> type; \
}; \
/**/
...
#     define BOOST_MPL_HAS_MEMBER_MULTI_ACCEPT(z, n, args) \
        template< typename V > \
        static boost_kahypar::mpl::aux::yes_tag \
        BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
            boost_kahypar::mpl::aux::type_wrapper< V > const volatile* \
...
#       define BOOST_MPL_HAS_MEMBER_TEST(args) \
          sizeof( \
              BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
                  static_cast< boost_kahypar::mpl::aux::type_wrapper< U >* >(0) \
              ) \
          ) == sizeof(boost_kahypar::mpl::aux::yes_tag) \
        /**/
#     else
...
#   define BOOST_MPL_HAS_MEMBER_IMPLEMENTATION( \
      ...
      template< \
          typename T \
        , typename fallback_ \
              = boost_kahypar::mpl::bool_< BOOST_PP_ARRAY_ELEM(3, args) > \
      > \
...
#   define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
      template< typename T \
              , typename fallback_ = boost_kahypar::mpl::bool_< default_ > > \
      struct trait { \
          BOOST_STATIC_CONSTANT(bool, value = fallback_::value); \
          typedef fallback_ type; \
      }; \
