...
private:
    ...
    boost_kahypar::uint_least32_t line_;
    boost_kahypar::uint_least32_t column_;
...
public:
    ...
    BOOST_CONSTEXPR source_location( char const * file, boost_kahypar::uint_least32_t ln, char const * function, boost_kahypar::uint_least32_t col = 0 ) BOOST_NOEXCEPT: file_( file ), function_( function ), line_( ln ), column_( col )
    {
    }
    ...
    BOOST_CONSTEXPR boost_kahypar::uint_least32_t line() const BOOST_NOEXCEPT
    {
        return line_;
    }

    BOOST_CONSTEXPR boost_kahypar::uint_least32_t column() const BOOST_NOEXCEPT
    {
        return column_;
    }
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location()

#elif defined(BOOST_MSVC) && BOOST_MSVC >= 1935

# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCSIG(), __builtin_COLUMN())
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCTION(), __builtin_COLUMN())
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__FILE__, BOOST_CURRENT_LOCATION_IMPL_1(__LINE__), "")
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(::std::source_location::current())
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCTION())
...
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__FILE__, __LINE__, __PRETTY_FUNCTION__)

#else

// __func__ macros aren't allowed outside functions, but BOOST_CURRENT_LOCATION is
# define BOOST_CURRENT_LOCATION ::boost_kahypar::source_location(__FILE__, __LINE__, "")
