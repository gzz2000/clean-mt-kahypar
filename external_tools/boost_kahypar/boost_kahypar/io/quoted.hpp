...
#include <boost_kahypar/io/detail/buffer_fill.hpp>
#include <boost_kahypar/io/detail/ostream_guard.hpp>
    ...
    Char delim)
{
    ...
    if (entry) {
        quoted_state<Char> state = boost_kahypar::io::detail::quoted_start(string,
            escape, delim);
        std::basic_streambuf<Char, Traits>& buf = *os.rdbuf();
        std::size_t width = static_cast<std::size_t>(os.width());
        if (width <= state.count) {
            if (!boost_kahypar::io::detail::quoted_put(buf, state.string, state.size,
            ...
            if (!boost_kahypar::io::detail::quoted_put(buf, state.string, state.size,
                    state.count, escape, delim) ||
                !boost_kahypar::io::detail::buffer_fill(buf, os.fill(),
                    width - state.count)) {
                return os;
            }
        } else if (!boost_kahypar::io::detail::buffer_fill(buf, os.fill(),
                width - state.count) ||
            !boost_kahypar::io::detail::quoted_put(buf, state.string, state.size,
                state.count, escape, delim)) {
            return os;
        }
        os.width(0);
...
{
    return boost_kahypar::io::detail::quoted_out(os, proxy.string, proxy.escape,
        proxy.delim);
}
    ...
    const quoted_proxy<std::basic_string<Char, Traits, Alloc>*, Char>& proxy)
{
    ...
    {
        boost_kahypar::io::ios_flags_saver ifs(is);
        std::noskipws(is);
        proxy.string->clear();
