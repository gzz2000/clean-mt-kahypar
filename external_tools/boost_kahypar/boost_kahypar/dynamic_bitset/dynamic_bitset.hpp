...
#include "boost_kahypar/dynamic_bitset/config.hpp"
...
#include "boost_kahypar/dynamic_bitset_fwd.hpp"
#include "boost_kahypar/dynamic_bitset/detail/dynamic_bitset.hpp"
#include "boost_kahypar/dynamic_bitset/detail/lowest_bit.hpp"
#include "boost_kahypar/move/move.hpp"
...
#include "boost_kahypar/functional/hash/hash.hpp"
...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using boost_kahypar::detail::dynamic_bitset_impl::value_to_type;
        using boost_kahypar::detail::dynamic_bitset_impl::is_numeric;
...
private:
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef boost_kahypar::detail::dynamic_bitset_impl
...
dynamic_bitset(dynamic_bitset<Block, Allocator>&& b)
  : m_bits(boost_kahypar::move(b.m_bits)), m_num_bits(boost_kahypar::move(b.m_num_bits))
{
...
{
    if (boost_kahypar::addressof(b) == this) { return *this; }

    m_bits = boost_kahypar::move(b.m_bits);
    m_num_bits = boost_kahypar::move(b.m_num_bits);
    // Required so that assert(m_check_invariants()); works.
    assert((b.m_bits = buffer_type()).empty());
    b.m_num_bits = 0;
    return *this;
...
{
    if (pos >= m_num_bits)
        BOOST_THROW_EXCEPTION(std::out_of_range("boost_kahypar::dynamic_bitset::at out_of_range"));
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    BOOST_THROW_EXCEPTION(std::overflow_error("boost_kahypar::dynamic_bitset::to_ulong overflow"));
...
inline std::size_t hash_value(const dynamic_bitset<Block, Allocator>& a)
{
    ...
    boost_kahypar::hash_combine(res, a.m_bits);
    return res;
}
...
{
    template<typename Block, typename Allocator>
    struct hash< boost_kahypar::dynamic_bitset<Block, Allocator> >
    {
        typedef boost_kahypar::dynamic_bitset<Block, Allocator> argument_type;
        typedef std::size_t result_type;
        result_type operator()(const argument_type& a) const BOOST_NOEXCEPT
        {
            boost_kahypar::hash<argument_type> hasher;
            return hasher(a);
        }
    };
}
