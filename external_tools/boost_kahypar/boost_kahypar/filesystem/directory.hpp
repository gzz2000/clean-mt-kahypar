...
#include <boost_kahypar/filesystem/detail/path_traits.hpp>
...
#include <boost_kahypar/detail/bitmask.hpp>
#include <boost_kahypar/system/error_code.hpp>
#include <boost_kahypar/smart_ptr/intrusive_ptr.hpp>
#include <boost_kahypar/smart_ptr/intrusive_ref_counter.hpp>
...
public:
    typedef boost_kahypar::filesystem::path::value_type value_type; // enables class path ctor taking directory_entry

    directory_entry() noexcept {}

    explicit directory_entry(boost_kahypar::filesystem::path const& p);

#if BOOST_FILESYSTEM_VERSION >= 4
    directory_entry(boost_kahypar::filesystem::path const& p, system::error_code& ec) :
        m_path(p)
    {
        refresh_impl(&ec);
        if (ec)
...
#else
    directory_entry(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st = file_status()) :
    ...
    directory_entry(directory_entry&& rhs) noexcept :
        m_path(static_cast< boost_kahypar::filesystem::path&& >(rhs.m_path)),
    ...
    void assign(boost_kahypar::filesystem::path&& p);

#if BOOST_FILESYSTEM_VERSION >= 4
    void assign(boost_kahypar::filesystem::path&& p, system::error_code& ec)
    {
        m_path = static_cast< boost_kahypar::filesystem::path&& >(p);
        refresh_impl(&ec);
    }
#else
    void assign(boost_kahypar::filesystem::path&& p, file_status st, file_status symlink_st = file_status())
    {
        assign_with_status(static_cast< boost_kahypar::filesystem::path&& >(p), st, symlink_st);
    }
#endif

    void assign(boost_kahypar::filesystem::path const& p);

#if BOOST_FILESYSTEM_VERSION >= 4
    void assign(boost_kahypar::filesystem::path const& p, system::error_code& ec)
    {
        m_path = p;
        refresh_impl(&ec);
    }
#else
    void assign(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st = file_status())
    {
        assign_with_status(p, st, symlink_st);
    }
#endif

    void replace_filename(boost_kahypar::filesystem::path const& p);

#if BOOST_FILESYSTEM_VERSION >= 4
    void replace_filename(boost_kahypar::filesystem::path const& p, system::error_code& ec)
    {
        m_path.replace_filename(p);
        refresh_impl(&ec);
    }
#else
    void replace_filename(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st = file_status())
    {
    ...
    void replace_leaf(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st)
    {
        replace_filename_with_status(p, st, symlink_st);
    }
#endif

    boost_kahypar::filesystem::path const& path() const noexcept { return m_path; }
    operator boost_kahypar::filesystem::path const&() const noexcept { return m_path; }
...
private:
    ...
    void assign_with_status(boost_kahypar::filesystem::path&& p, file_status st, file_status symlink_st)
    {
        m_path = static_cast< boost_kahypar::filesystem::path&& >(p);
    ...
    void assign_with_status(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st)
    {
        m_path = p;
        m_status = static_cast< file_status&& >(st);
        m_symlink_status = static_cast< file_status&& >(symlink_st);
    ...
    void replace_filename_with_status(boost_kahypar::filesystem::path const& p, file_status st, file_status symlink_st)
    {
        m_path.replace_filename(p);
        m_status = static_cast< file_status&& >(st);
        m_symlink_status = static_cast< file_status&& >(symlink_st);
...
private:
    boost_kahypar::filesystem::path m_path;
...
inline directory_entry::directory_entry(boost_kahypar::filesystem::path const& p) :
    m_path(p)
{
#if BOOST_FILESYSTEM_VERSION >= 4
    refresh_impl();
...
inline void directory_entry::assign(boost_kahypar::filesystem::path&& p)
{
    m_path = static_cast< boost_kahypar::filesystem::path&& >(p);
#if BOOST_FILESYSTEM_VERSION >= 4
    refresh_impl();
...
inline void directory_entry::assign(boost_kahypar::filesystem::path const& p)
{
    m_path = p;
#if BOOST_FILESYSTEM_VERSION >= 4
    refresh_impl();
...
inline void directory_entry::replace_filename(boost_kahypar::filesystem::path const& p)
{
    m_path.replace_filename(p);
#if BOOST_FILESYSTEM_VERSION >= 4
    refresh_impl();
...
{
    boost_kahypar::filesystem::path::string_type const& source = de.path().native();
...
//  - a conversion to 'path' using 'operator boost_kahypar::filesystem::path const&()',
...
struct dir_itr_imp :
    public boost_kahypar::intrusive_ref_counter< dir_itr_imp >
{
...
class directory_iterator :
    public boost_kahypar::iterator_facade<
        directory_iterator,
        directory_entry,
...
public:
    ...
    directory_iterator(directory_iterator&& that) noexcept :
        m_imp(static_cast< boost_kahypar::intrusive_ptr< detail::dir_itr_imp >&& >(that.m_imp))
    {
    }
...
private:
    ...
    boost_kahypar::intrusive_ptr< detail::dir_itr_imp > m_imp;
};
...
//  namespace boost_kahypar template specializations
template< typename C, typename Enabler >
struct range_mutable_iterator;

template<>
struct range_mutable_iterator< boost_kahypar::filesystem::directory_iterator, void >
{
    typedef boost_kahypar::filesystem::directory_iterator type;
};
...
template<>
struct range_const_iterator< boost_kahypar::filesystem::directory_iterator, void >
{
    typedef boost_kahypar::filesystem::directory_iterator type;
};
...
struct recur_dir_itr_imp :
    public boost_kahypar::intrusive_ref_counter< recur_dir_itr_imp >
{
...
public:
    ...
    recursive_directory_iterator(recursive_directory_iterator&& that) noexcept :
        m_imp(static_cast< boost_kahypar::intrusive_ptr< detail::recur_dir_itr_imp >&& >(that.m_imp))
    {
    }
...
private:
    // intrusive_ptr provides the shallow-copy semantics required for single pass iterators
    // (i.e. InputIterators). The end iterator is indicated by is_end().
    boost_kahypar::intrusive_ptr< detail::recur_dir_itr_imp > m_imp;
};
...
//  namespace boost_kahypar template specializations
template<>
struct range_mutable_iterator< boost_kahypar::filesystem::recursive_directory_iterator, void >
{
    typedef boost_kahypar::filesystem::recursive_directory_iterator type;
};

template<>
struct range_const_iterator< boost_kahypar::filesystem::recursive_directory_iterator, void >
{
    typedef boost_kahypar::filesystem::recursive_directory_iterator type;
};
