...
#include <boost_kahypar/mpl/size.hpp>
#include <boost_kahypar/mpl/empty.hpp>

#include <boost_kahypar/serialization/throw_exception.hpp>

// Boost Variant supports all C++ versions back to C++03
#include <boost_kahypar/variant/variant.hpp>
#include <boost_kahypar/variant/get.hpp>
...
// use visitor from boost_kahypar::variant
template<class Visitor, BOOST_VARIANT_ENUM_PARAMS(class T)>
typename Visitor::result_type visit(
    Visitor visitor,
    const boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)> & t
){
    return boost_kahypar::apply_visitor(visitor, t);
}
template<class Visitor, BOOST_VARIANT_ENUM_PARAMS(class T)>
typename Visitor::result_type visit(
    Visitor visitor,
    const boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)> & t,
    const boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)> & u
){
    return boost_kahypar::apply_visitor(visitor, t, u);
}
...
struct variant_save_visitor :
    boost_kahypar::static_visitor<void>
{
...
void save(
    Archive & ar,
    boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const & v,
...
void save(
    Archive & ar,
    boost_kahypar::variant2::variant<Types...> const & v,
    unsigned int /*version*/
){
    int which = v.index();
    ar << BOOST_SERIALIZATION_NVP(which);
...
void load(
    Archive & ar,
    boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& v,
    const unsigned int version
){
    int which;
    typedef typename boost_kahypar::variant<BOOST_VARIANT_ENUM_PARAMS(T)>::types types;
    ar >> BOOST_SERIALIZATION_NVP(which);
    if(which >=  mpl::size<types>::value){
        ...
        boost_kahypar::serialization::throw_exception(
            boost_kahypar::archive::archive_exception(
                boost_kahypar::archive::archive_exception::unsupported_version
            )
        );
    }
...
void load(
    Archive & ar,
    boost_kahypar::variant2::variant<Types...> & v,
    const unsigned int version
){
    int which;
    typedef typename boost_kahypar::variant<Types...>::types types;
    ar >> BOOST_SERIALIZATION_NVP(which);
...
inline void serialize(
    ...
    boost_kahypar::serialization::split_free(ar,v,file_version);
}
...
>{
    ...
    typedef mpl::int_< ::boost_kahypar::serialization::track_always> type;
