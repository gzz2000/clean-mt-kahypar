...
#include <boost_kahypar/aligned_storage.hpp>
...
private:
    typedef typename boost_kahypar::aligned_storage<
        sizeof(T),
        boost_kahypar::alignment_of<T>::value
    > type;
    type storage_;
};
...
struct stack_construct : public stack_allocate<T>
{
    stack_construct(Archive & ar, const unsigned int version){
        ...
        boost_kahypar::serialization::load_construct_data_adl(
            ar,
            this->address(),
            version
        );
