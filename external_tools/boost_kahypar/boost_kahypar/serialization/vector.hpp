...
inline void save(
    ...
    boost_kahypar::serialization::stl::save_collection<Archive, STD::vector<U, Allocator> >(
        ar, t
    );
}
...
inline void load(
    ...
    const boost_kahypar::serialization::library_version_type library_version(
...
inline void save(
    ...
    typedef typename
    boost_kahypar::serialization::use_array_optimization<Archive>::template apply<
...
inline void load(
    ...
    if (ar.get_library_version()==boost_kahypar::serialization::library_version_type(5))
    {
      load(ar,t,file_version, boost_kahypar::is_arithmetic<U>());
      return;
    }
#endif
    typedef typename
...
inline void save(
    ...
    while(count-- > 0){
        bool tb = *it++;
        ar << boost_kahypar::serialization::make_nvp("item", tb);
    }
}
...
inline void load(
    ...
    for(collection_size_type i = collection_size_type(0); i < count; ++i){
        bool b;
        ar >> boost_kahypar::serialization::make_nvp("item", b);
        t[i] = b;
    }
}
