...
#include <boost_kahypar/serialization/detail/shared_ptr_132.hpp>
...
inline void serialize(
    Archive & /* ar */,
    boost_kahypar_132::detail::sp_counted_base_impl<P, D> & /* t */,
    const unsigned int /*file_version*/
){
    // register the relationship between each derived class
    // its polymorphic base
    boost_kahypar::serialization::void_cast_register<
        boost_kahypar_132::detail::sp_counted_base_impl<P, D>,
        boost_kahypar_132::detail::sp_counted_base
    >(
        static_cast<boost_kahypar_132::detail::sp_counted_base_impl<P, D> *>(NULL),
        static_cast<boost_kahypar_132::detail::sp_counted_base *>(NULL)
    );
}
...
inline void save_construct_data(
    ...
    ar << boost_kahypar::serialization::make_nvp("ptr", t->ptr);
}
...
inline void load_construct_data(
    ...
    P ptr_;
    ar >> boost_kahypar::serialization::make_nvp("ptr", ptr_);
    // ::new(t)boost_kahypar_132::detail::sp_counted_base_impl<P, D>(ptr_,  D());
    // placement
    ...
    // new system as well.
    ::new(t)boost_kahypar_132::detail::sp_counted_base_impl<
        P,
        boost_kahypar_132::serialization::detail::null_deleter
    >(
        ptr_,  boost_kahypar_132::serialization::detail::null_deleter()
    ); // placement new
...
inline void save(
    Archive & ar,
    const boost_kahypar_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar << boost_kahypar::serialization::make_nvp("pi", t.pi_);
}

template<class Archive>
inline void load(
    Archive & ar,
    boost_kahypar_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar >> boost_kahypar::serialization::make_nvp("pi", t.pi_);
    if(NULL != t.pi_)
...
BOOST_SERIALIZATION_SPLIT_FREE(boost_kahypar_132::detail::shared_count)
...
inline void save(
    Archive & ar,
    const boost_kahypar_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
    ar.register_type(static_cast<
        boost_kahypar_132::detail::sp_counted_base_impl<T *, boost_kahypar::checked_deleter< T > > *
    >(NULL));
    ar << boost_kahypar::serialization::make_nvp("px", t.px);
    ar << boost_kahypar::serialization::make_nvp("pn", t.pn);
}

template<class Archive, class T>
inline void load(
    Archive & ar,
    boost_kahypar_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
...
#define BOOST_SHARED_POINTER_EXPORT_GUID(T, K)                     \
    typedef boost_kahypar_132::detail::sp_counted_base_impl<               \
        T *,                                                       \
        boost_kahypar::checked_deleter< T >                                \
    > __shared_ptr_ ## T;                                          \
