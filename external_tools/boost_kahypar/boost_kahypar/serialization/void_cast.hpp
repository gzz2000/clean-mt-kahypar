...
#include <boost_kahypar/serialization/force_include.hpp>
...
#include <boost_kahypar/serialization/void_cast_fwd.hpp>
    ...
    private boost_kahypar::noncopyable
{
    ...
    friend
    BOOST_SERIALIZATION_DECL void const *
    boost_kahypar::serialization::void_downcast(
        extended_type_info const & derived,
        extended_type_info const & base,
        void const * const
    );
    ...
    public void_caster
{
    void const * downcast(void const * const t) const BOOST_OVERRIDE {
        const Derived * d =
            boost_kahypar::serialization::smart_cast<const Derived *, const Base *>(
    ...
    void const * upcast(void const * const t) const BOOST_OVERRIDE {
        const Base * b =
            boost_kahypar::serialization::smart_cast<const Base *, const Derived *>(
...
private:
    static std::ptrdiff_t base_offset() {
        typename boost_kahypar::aligned_storage<sizeof(Derived)>::type data;
...
{
    typedef
        typename mpl::eval_if<boost_kahypar::is_virtual_base_of<Base,Derived>,
            mpl::identity<
