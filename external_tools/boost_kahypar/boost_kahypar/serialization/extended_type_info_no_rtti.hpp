...
#include <boost_kahypar/serialization/extended_type_info.hpp>
#include <boost_kahypar/serialization/factory.hpp>
#include <boost_kahypar/serialization/throw_exception.hpp>

#include <boost_kahypar/serialization/config.hpp>
// hijack serialization access
#include <boost_kahypar/serialization/access.hpp>

#include <boost_kahypar/config/abi_prefix.hpp> // must be the last header
#ifdef BOOST_MSVC
#  pragma warning(push)
...
public:
    ...
    is_less_than(const boost_kahypar::serialization::extended_type_info &rhs) const BOOST_OVERRIDE;
    BOOST_SERIALIZATION_DECL bool
    is_equal(const boost_kahypar::serialization::extended_type_info &rhs) const BOOST_OVERRIDE;
};
    ...
    public singleton<extended_type_info_no_rtti< T > >
{
    ...
    struct action {
        ...
        static const char * invoke(){
            typedef
                typename boost_kahypar::mpl::if_c<
                    tf,
                    defined,
                    undefined
                >::type type;
...
public:
    ...
    get_derived_extended_type_info(const T & t) const {
        ...
        BOOST_STATIC_WARNING(boost_kahypar::is_polymorphic< T >::value);
        const char * derived_key = t.get_key();
        BOOST_ASSERT(NULL != derived_key);
        return boost_kahypar::serialization::extended_type_info::find(derived_key);
    }
    ...
    void * construct(unsigned int count, ...) const BOOST_OVERRIDE {
        ...
        switch(count){
        case 0:
            return factory<typename boost_kahypar::remove_const< T >::type, 0>(ap);
        case 1:
            return factory<typename boost_kahypar::remove_const< T >::type, 1>(ap);
        case 2:
            return factory<typename boost_kahypar::remove_const< T >::type, 2>(ap);
        case 3:
            return factory<typename boost_kahypar::remove_const< T >::type, 3>(ap);
        case 4:
            return factory<typename boost_kahypar::remove_const< T >::type, 4>(ap);
        default:
    ...
    void destroy(void const * const p) const BOOST_OVERRIDE {
        boost_kahypar::serialization::access::destroy(
...
// before any of the other headers.
    ...
    struct extended_type_info_impl {
        typedef typename
            boost_kahypar::serialization::extended_type_info_no_rtti< T > type;
    };
    } // namespace serialization
    } // namespace boost_kahypar
#endif
...
#  pragma warning(pop)
#endif
#include <boost_kahypar/config/abi_suffix.hpp> // pops abi_suffix.hpp pragmas
