...
inline void save(
    Archive & ar,
    const boost_kahypar::weak_ptr< T > &t,
    const unsigned int /* file_version */
){
    const boost_kahypar::shared_ptr< T > sp = t.lock();
    ar << boost_kahypar::serialization::make_nvp("weak_ptr", sp);
}

template<class Archive, class T>
inline void load(
    Archive & ar,
    boost_kahypar::weak_ptr< T > &t,
    const unsigned int /* file_version */
){
    boost_kahypar::shared_ptr< T > sp;
    ar >> boost_kahypar::serialization::make_nvp("weak_ptr", sp);
    t = sp;
}
