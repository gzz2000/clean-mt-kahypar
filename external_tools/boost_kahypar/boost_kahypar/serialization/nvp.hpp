...
#define BOOST_SERIALIZATION_NVP(name)                       \
    boost_kahypar::serialization::make_nvp(BOOST_PP_STRINGIZE(name), name)
/**/

#define BOOST_SERIALIZATION_BASE_OBJECT_NVP(name)           \
    boost_kahypar::serialization::make_nvp(                         \
        BOOST_PP_STRINGIZE(name),                           \
        boost_kahypar::serialization::base_object<name >(*this)     \
    )
/**/
...
struct is_wrapper<const nvp<T> > {
    typedef boost_kahypar::mpl::true_ type;
};
template<class T>
