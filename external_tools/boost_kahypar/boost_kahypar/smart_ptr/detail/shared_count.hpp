...
#include <boost_kahypar/smart_ptr/bad_weak_ptr.hpp>
#include <boost_kahypar/smart_ptr/detail/sp_counted_base.hpp>
#include <boost_kahypar/smart_ptr/detail/sp_counted_impl.hpp>
...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( boost_kahypar::addressof( t ) )
    {
    }
...
{
    boost_kahypar::uintptr_t v = reinterpret_cast<boost_kahypar::uintptr_t>( p );

    // match boost_kahypar::hash<T*>
...
public:
...
#endif
    {
        ...
        {
            boost_kahypar::checked_delete( p );
            boost_kahypar::throw_exception( std::bad_alloc() );
        }
    ...
    template<class Y, class D>
    explicit shared_count( boost_kahypar::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        , id_(shared_count_id)
#endif
    {
        ...
        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename boost_kahypar::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
#endif
{
    ...
    {
        boost_kahypar::throw_exception( boost_kahypar::bad_weak_ptr() );
    }
}
