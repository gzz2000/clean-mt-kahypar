...
#include <boost_kahypar/smart_ptr/detail/sp_typeinfo_.hpp>
...
inline void atomic_increment( boost_kahypar::uint_least32_t * pw )
{
    __atomic_fetch_add( pw, 1, __ATOMIC_RELAXED );
}

inline boost_kahypar::uint_least32_t atomic_decrement( boost_kahypar::uint_least32_t * pw )
{
    return __atomic_fetch_sub( pw, 1, __ATOMIC_ACQ_REL );
}

inline boost_kahypar::uint_least32_t atomic_conditional_increment( boost_kahypar::uint_least32_t * pw )
{
    // long r = *pw;
    // if( r != 0 ) ++*pw;
    // return r;

    boost_kahypar::uint_least32_t r = __atomic_load_n( pw, __ATOMIC_RELAXED );
...
inline boost_kahypar::uint_least32_t atomic_load( boost_kahypar::uint_least32_t const * pw )
{
...
private:
    ...
    boost_kahypar::uint_least32_t use_count_;        // #shared
    boost_kahypar::uint_least32_t weak_count_;       // #weak + (#shared != 0)
