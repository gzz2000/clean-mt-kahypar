...
public:
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        boost_kahypar::sp_scalar_constructor_hook( px );
#endif
    }
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        boost_kahypar::sp_scalar_destructor_hook( px );
#endif
        boost_kahypar::checked_delete( px );
    }
...
template<class T> inline bool operator==( scoped_ptr<T> const & p, boost_kahypar::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( boost_kahypar::detail::sp_nullptr_t, scoped_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
