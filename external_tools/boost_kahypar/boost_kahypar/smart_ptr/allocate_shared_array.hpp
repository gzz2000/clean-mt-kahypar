...
#include <boost_kahypar/core/first_scalar.hpp>
...
#include <boost_kahypar/type_traits/is_bounded_array.hpp>
#include <boost_kahypar/type_traits/is_unbounded_array.hpp>
...
struct sp_array_element {
    typedef typename boost_kahypar::remove_cv<typename
        boost_kahypar::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<boost_kahypar::alignment_of<T>::value,
            boost_kahypar::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename boost_kahypar::type_with_alignment<sp_array_alignment<T,
...
private:
    typename boost_kahypar::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        boost_kahypar::alloc_construct_n(state_.allocator(),
            boost_kahypar::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        boost_kahypar::alloc_construct_n(state_.allocator(),
            boost_kahypar::first_scalar(start), state_.size() * count,
            boost_kahypar::first_scalar(&list), count);
    }
    ...
    void dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE {
        boost_kahypar::alloc_destroy_n(state_.allocator(),
            boost_kahypar::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return boost_kahypar::allocate_shared<T>(boost_kahypar::noinit_adapt(allocator), count);
}
...
{
    return boost_kahypar::allocate_shared<T>(boost_kahypar::noinit_adapt(allocator));
}
