...
#endif
        boost_kahypar::checked_array_delete( px );
    }
...
template<class T> inline bool operator==( scoped_array<T> const & p, boost_kahypar::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( boost_kahypar::detail::sp_nullptr_t, scoped_array<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
