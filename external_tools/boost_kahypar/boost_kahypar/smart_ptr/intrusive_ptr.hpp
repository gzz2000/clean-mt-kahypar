...
#include <boost_kahypar/config/no_tr1/functional.hpp>           // for std::less
...
public:
    ...
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename boost_kahypar::detail::sp_enable_if_convertible<U,T>::type = boost_kahypar::detail::sp_empty() )
...
// Move support
    ...
    intrusive_ptr(intrusive_ptr<U> && rhs, typename boost_kahypar::detail::sp_enable_if_convertible<U,T>::type = boost_kahypar::detail::sp_empty())
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, boost_kahypar::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( boost_kahypar::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( boost_kahypar::intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return boost_kahypar::hash< T* >()( p.get() );
}
...
template<class T> struct hash< ::boost_kahypar::intrusive_ptr<T> >
{
    std::size_t operator()( ::boost_kahypar::intrusive_ptr<T> const & p ) const BOOST_SP_NOEXCEPT
    {
