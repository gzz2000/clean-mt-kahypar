...
#include <boost_kahypar/move/detail/unique_ptr_meta_utils.hpp>
#include <boost_kahypar/move/default_delete.hpp>
...
struct deleter_types
{
   ...
   typedef ::boost_kahypar::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
struct unique_ptr_data
{
   ...
   inline unique_ptr_data(P p, BOOST_FWD_REF(U) d1) BOOST_NOEXCEPT
      : m_p(p), d(::boost_kahypar::forward<U>(d1))
   {}
   ...
   : private D
{
   ...
   inline unique_ptr_data(P p, BOOST_FWD_REF(U) d) BOOST_NOEXCEPT
      : D(::boost_kahypar::forward<U>(d)), m_p(p)
   {}
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::boost_kahypar::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
      , ::boost_kahypar::rv<T>&
...
class unique_ptr
{
   ...
   inline unique_ptr(Pointer p, BOOST_MOVE_SEEDOC(deleter_arg_type2) d2
      ...
      : m_data(p, ::boost_kahypar::move(d2))
   {
      //If T is not an array type, element_type_t<Pointer> derives from T
      //it uses the default deleter and T has no virtual destructor, then you have a problem
      BOOST_MOVE_STATIC_ASSERT(( !bmupd::missing_virtual_destructor
   ...
   inline unique_ptr(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type), BOOST_MOVE_SEEDOC(deleter_arg_type2) d2) BOOST_NOEXCEPT
      : m_data(pointer(), ::boost_kahypar::move(d2))
   {}
   ...
   inline unique_ptr(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
      : m_data(u.release(), ::boost_kahypar::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   inline unique_ptr( BOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> BOOST_RV_REF_END_IF_CXX11 u
      ...
      : m_data(u.release(), ::boost_kahypar::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   unique_ptr& operator=(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
   {
      ...
      m_data.deleter() = ::boost_kahypar::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   {
      this->reset(u.release());
      m_data.deleter() = ::boost_kahypar::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   {
      ::boost_kahypar::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::boost_kahypar::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
