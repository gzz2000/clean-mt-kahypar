...
#include <boost_kahypar/move/utility_core.hpp> //for boost_kahypar::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace boost_kahypar_move_member_swap {
...
namespace boost_kahypar_move_adl_swap{
...
template<class T>
inline void swap_proxy(T& x, T& y, typename boost_kahypar::move_detail::enable_if_c<!boost_kahypar::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
void swap_proxy(T& x, T& y
               , typename boost_kahypar::move_detail::enable_if< and_op_not_impl<boost_kahypar::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , boost_kahypar_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::boost_kahypar::move(x)); x = ::boost_kahypar::move(y); y = ::boost_kahypar::move(t);  }

template<class T>
inline void swap_proxy(T& x, T& y
               , typename boost_kahypar::move_detail::enable_if< and_op_impl< boost_kahypar::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , boost_kahypar_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::boost_kahypar_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::boost_kahypar::move(x)); x = ::boost_kahypar::move(y); y = ::boost_kahypar::move(t);</code>.
template<class T>
inline void adl_move_swap(T& x, T& y)
{
   ::boost_kahypar_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using boost_kahypar::adl_move_swap.
//! 
//! Parameters:
...
{
    while (first1 != last1) {
      ::boost_kahypar::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::boost_kahypar::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
...
{
   boost_kahypar::adl_move_swap(*a, *b); 
}
