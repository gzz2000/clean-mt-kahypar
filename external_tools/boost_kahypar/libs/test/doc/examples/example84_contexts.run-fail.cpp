...
BOOST_AUTO_TEST_CASE(quadratic_estimation)
{
  ...
  for(int i = 0; i < 50; i++) {
    ...
    BOOST_TEST(estimated.first == double(root1), 10. % boost_kahypar::test_tools::tolerance());
    BOOST_TEST(estimated.second == double(root2), 10. % boost_kahypar::test_tools::tolerance());
  }
}
//]
