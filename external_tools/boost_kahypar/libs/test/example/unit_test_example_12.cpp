...
#include <boost_kahypar/test/parameterized_test.hpp>
...
public:
    ...
    unsigned long   operator()( std::string const& arg )
    {
        ...
        std::string::const_iterator it = std::find_if( arg.begin(), arg.end(),
            BOOST_TEST_BIND1ST( boost_kahypar::mem_fun( &hash_function::helper_ ), this ) );
...
struct massive_hash_function_test : test_suite {
    massive_hash_function_test() : test_suite( "massive_hash_function_test" ) {
        ...
        boost_kahypar::shared_ptr<hash_function_tester> instance( new hash_function_tester( alphabet ) );
