...
#include <boost_kahypar/test/tools/output_test_stream.hpp>
#include <boost_kahypar/test/execution_monitor.hpp>
#include <boost_kahypar/test/unit_test_parameters.hpp>
#include <boost_kahypar/test/output/compiler_log_formatter.hpp>
#include <boost_kahypar/test/framework.hpp>
#include <boost_kahypar/core/noncopyable.hpp>
...
namespace boost_kahypar{ namespace test_tools{ namespace tt_detail{
template<>
...
{                                                   \
    ...
    try {                                           \
        tool_usage;                                 \
    } catch( boost_kahypar::execution_aborted const& ) {    \
        throw_ = true;                              \
    }                                               \
                                                    \
...
struct shorten_lf : public boost_kahypar::unit_test::output::compiler_log_formatter
{
    void    print_prefix( std::ostream& output, boost_kahypar::unit_test::const_string, std::size_t line )
    {
    }
};
...
ots()
{
    static boost_kahypar::shared_ptr<tt::output_test_stream> inst;
...
boost_kahypar::test_tools::predicate_result
test_pred1()
{
    boost_kahypar::test_tools::predicate_result res( false );
...
{
    boost_kahypar::test_tools::predicate_result res( c1.m_i == c2.m_i && c1.m_id == c2.m_id );
...
boost_kahypar::test_tools::assertion_result
...
TEST_CASE( test_BOOST_CHECK_PREDICATE )
{
    ...
    int j=15;
    BOOST_CHECK_PREDICATE( boost_kahypar::bind( is_even, boost_kahypar::bind( &foo, _1, _2 ) ), (i)(j) );
...
TEST_CASE( test_BOOST_TEST_fpv_comp )
{
    ...
    boost_kahypar::ignore_unused( norm );
...
{
    using namespace boost_kahypar::test_tools;
...
struct copy_counter : boost_kahypar::noncopyable {
    static int s_value;
