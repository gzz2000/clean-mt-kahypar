...
#include <limits>
#include <iostream>
#include <boost_kahypar/multiprecision/cpp_dec_float.hpp>

#include <boost_kahypar/test/tools/floating_point_comparison.hpp> // Extra test tool for FP comparison.
#include <boost_kahypar/test/unit_test.hpp>


typedef boost_kahypar::multiprecision::number<
   boost_kahypar::multiprecision::cpp_dec_float<50>,
   boost_kahypar::multiprecision::et_off> cpp_dec_float_50_noet;

typedef boost_kahypar::multiprecision::number<
   boost_kahypar::multiprecision::cpp_dec_float<50>,
   boost_kahypar::multiprecision::et_on> cpp_dec_float_50_et;
...
// comparison.
#if 0
namespace boost_kahypar { namespace math { namespace fpc {

  template <class A, boost_kahypar::multiprecision::expression_template_option B>
  struct tolerance_based< boost_kahypar::multiprecision::number<
   A,
   B > > : boost_kahypar::mpl::true_ {};

  template <class tag, class Arg1, class Arg2, class Arg3, class Arg4>
  struct tolerance_based<
    boost_kahypar::multiprecision::detail::expression<
      tag, Arg1, Arg2, Arg3, Arg4> > : boost_kahypar::mpl::true_ {};
...
you must pass two template parameters to `boost_kahypar::multiprecision::number`.

It may be more legible to use a two-staged type definition such as this:

``
typedef boost_kahypar::multiprecision::cpp_dec_float<50> mp_backend;
typedef boost_kahypar::multiprecision::number<mp_backend, boost_kahypar::multiprecision::et_off> cpp_dec_float_50_noet;
``

Here, we first define `mp_backend` as `cpp_dec_float` with 50 digits.
The second step passes this backend to `boost_kahypar::multiprecision::number`
with `boost_kahypar::multiprecision::et_off`, an enumerated type.

  typedef boost_kahypar::multiprecision::number<boost_kahypar::multiprecision::cpp_dec_float<50>, boost_kahypar::multiprecision::et_off>
  cpp_dec_float_50_noet;

You can reduce typing with a `using` directive `using namespace boost_kahypar::multiprecision;`
if desired, as shown below.
*/

using namespace boost_kahypar::multiprecision;
...
{ // No expression templates/
  ...
  namespace tt = boost_kahypar::test_tools;
...
{ // Using expression templates.
  ...
  */
  ...
  // cannot convert from 'int' to 'boost_kahypar::multiprecision::detail::expression<tag,Arg1,Arg2,Arg3,Arg4>'
  //] [/expression_template_1]
