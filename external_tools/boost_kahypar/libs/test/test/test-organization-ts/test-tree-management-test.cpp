...
#else
BOOST_TEST_DECORATOR(*boost_kahypar::unit_test::expected_failures(1)) BOOST_AUTO_TEST_CASE( tc1 ) /* on the same line, otherwise some test below should be updated */
#endif
{ BOOST_ERROR(""); }
...
BOOST_AUTO_TEST_CASE( tc1, * expected_failures(1) )
#else
BOOST_TEST_DECORATOR(*boost_kahypar::unit_test::expected_failures(1)) BOOST_AUTO_TEST_CASE( tc1 ) /* on the same line, otherwise some test below should be updated (the test is sensitive to line numbers) */
#endif
{ BOOST_ERROR( "" ); }
...
typedef boost_kahypar::mpl::vector<int,float,char> test_types;
...
typedef boost_kahypar::mpl::vector<int,float,char,double,short int> test_types;
...
{
    boost_kahypar::shared_ptr<A> instA( new A );
    ...
    boost_kahypar::shared_ptr<B> instB( new B );
    ...
    boost_kahypar::shared_ptr<C> instC1( new D );
    ...
    boost_kahypar::shared_ptr<C> instC2( new E );
