...
BOOST_AUTO_TEST_CASE( test_clashing_suites )
{
    ...
    BOOST_CHECK_THROW(framework::finalize_setup_phase( master_ts->p_id ),
                      boost_kahypar::unit_test::framework::setup_error );
}
...
BOOST_AUTO_TEST_CASE( test_clashing_cases_template_test_case )
{
    ...
    typedef boost_kahypar::mpl::list<int, long, unsigned char> test_types1;
    typedef boost_kahypar::mpl::list<int, long, unsigned char, int> test_types2;
...
BOOST_AUTO_TEST_CASE( test_clashing_cases_with_name )
{
    ...
    master_ts->add(t_suite1);
    t_suite1->add( BOOST_TEST_CASE( suite1_test1 ) );
    t_suite1->add( BOOST_TEST_CASE( boost_kahypar::bind( test2, 0 ) ) );
    BOOST_CHECK_NO_THROW( t_suite1->add( BOOST_TEST_CASE( boost_kahypar::bind( test2, 0 ) ) ) );
    BOOST_CHECK_THROW(framework::finalize_setup_phase( master_ts->p_id ),
                      boost_kahypar::unit_test::framework::setup_error );

    for(int i = 0; i < 10; i++) {
        t_suite1->add( BOOST_TEST_CASE_NAME( boost_kahypar::bind( test2, i ), "test-X-" + boost_kahypar::unit_test::utils::string_cast(i) ) );
    }
}
