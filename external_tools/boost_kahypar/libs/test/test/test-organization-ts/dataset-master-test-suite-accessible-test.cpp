...
#include <boost_kahypar/test/data/test_case.hpp>
#include <iostream>
...
public:
    ...
    boost_kahypar::unit_test::data::size_t size() const {
...
dataset_loader::dataset_loader()
{
    ...
    int argc = boost_kahypar::unit_test::framework::master_test_suite().argc;
    char** argv = boost_kahypar::unit_test::framework::master_test_suite().argv;
...
namespace boost_kahypar { namespace unit_test { namespace data {

namespace monomorphic {
  template <>
  struct is_dataset<dataset_loader> : boost_kahypar::mpl::true_ {};
}
}}}
...
BOOST_DATA_TEST_CASE(master_access_zips,
    boost_kahypar::unit_test::data::make_delayed<dataset_loader>( ) ^ boost_kahypar::unit_test::data::make(expected),
    input, expect)
{
    // ...
...
BOOST_DATA_TEST_CASE(master_access_zips_flip,
    boost_kahypar::unit_test::data::make_delayed<dataset_loader>( ) ^ dataset_loader({"--param1=1", "--param2=2"}),
    input, expect)
{
    BOOST_TEST(input == expect);
}
...
BOOST_DATA_TEST_CASE(master_access_zips_args_forward,
    boost_kahypar::unit_test::data::make_delayed<dataset_loader>( "--param1=1", "--param2=2" ) ^ boost_kahypar::unit_test::data::make_delayed<dataset_loader>( ),
    input, expect)
{
    BOOST_TEST(input == expect);
}
...
BOOST_DATA_TEST_CASE(master_access_grid_flip,
    dataset_loader({"--param1=1", "--param2=2"}) * boost_kahypar::unit_test::data::make_delayed<dataset_loader>( ),
    input, expect)
{
    BOOST_TEST((input == expect || input == "--param1=1" || expect == "--param1=1" ));
}
...
BOOST_DATA_TEST_CASE(master_access_joins_flip,
    dataset_loader({"--param2=2", "--param1=1"}) + boost_kahypar::unit_test::data::make_delayed<dataset_loader>( ),
    input)
{
...
namespace monomorphic {
  template <>
  struct is_dataset<dataset_loader_arity3> : boost_kahypar::mpl::true_ {};
}
}}}

BOOST_DATA_TEST_CASE(master_access_make_ds_with_arity,
    boost_kahypar::unit_test::data::make_delayed<dataset_loader_arity3>( "something-in-the-middle"),
