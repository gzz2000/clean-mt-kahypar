...
//
//  Checks boost_kahypar::exception, see https://github.com/boostorg/test/issues/147
// ***************************************************************************
...
#include <boost_kahypar/exception/info.hpp>
#include <boost_kahypar/cstdint.hpp>

#include <iostream>
#include <ios>

using boost_kahypar::test_tools::output_test_stream;
using namespace boost_kahypar::unit_test;

typedef boost_kahypar::error_info<struct tag_error_code, boost_kahypar::uint32_t> error_code;
typedef boost_kahypar::error_info<struct tag_error_string, std::string> error_string;
struct myexception : std::exception, virtual boost_kahypar::exception
{};
...
BOOST_AUTO_TEST_CASE( test_logs )
{
    ...
    boost_kahypar::unit_test::unit_test_log.set_stream(test_output);
    boost_kahypar::unit_test::unit_test_log.set_threshold_level( log_successful_tests );
    ...
    boost_kahypar::unit_test::unit_test_log.set_stream(std::cout);

    std::string error_string(test_output.get_stream_string_representation());
    // the message is "Dynamic exception type: boost_kahypar::exception_detail::clone_impl<myexception>" on Unix
    // and "Dynamic exception type: boost_kahypar::exception_detail::clone_impl<struct myexception>" on Windows.
    ...
    BOOST_TEST(error_string.find("boost_kahypar::wrapexcept<") != std::string::npos);
