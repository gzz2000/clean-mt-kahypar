...
public:
    explicit output_test_stream_for_loggers(
        boost_kahypar::unit_test::const_string    pattern_file_name = boost_kahypar::unit_test::const_string(),
...
struct log_setup_teardown {
  ...
  {
    boost_kahypar::unit_test::unit_test_log.set_format(log_format);
    if(additional_log_format != OF_INVALID) {
      boost_kahypar::unit_test::unit_test_log.add_format(additional_log_format);
    }
    boost_kahypar::unit_test::unit_test_log.set_stream(output);
    if(level_to_restore == invalid_log_level) {
      m_previous_ll = boost_kahypar::unit_test::unit_test_log.set_threshold_level(ll);
    }
  }

  ~log_setup_teardown() {
    boost_kahypar::unit_test::unit_test_log.set_format(OF_CLF);
    boost_kahypar::unit_test::unit_test_log.set_stream(std::cout);
    boost_kahypar::unit_test::unit_test_log.set_threshold_level( m_previous_ll );
    boost_kahypar::unit_test::unit_test_log.configure(); // forces reconfiguration
  }
