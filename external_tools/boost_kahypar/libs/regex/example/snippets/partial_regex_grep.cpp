...
bool grep_callback(const boost_kahypar::match_results<const char*>& m)
{
...
void search(std::istream& is)
{
   ...
   while(have_more)
   {
      ...
      next_pos = buf + sizeof(buf);
      // and then grep:
      boost_kahypar::regex_grep<bool(*)(const boost_kahypar::cmatch&), const char*>(grep_callback,
                        static_cast<const char*>(buf),
                        static_cast<const char*>(buf + read + leftover),
                        e,
                        boost_kahypar::match_default | boost_kahypar::match_partial);
   }
}
