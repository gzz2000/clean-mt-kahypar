...
{
   return boost_kahypar::u32regex_match(password, boost_kahypar::make_u32regex(requirements));
}
...
{
   boost_kahypar::u32regex r = boost_kahypar::make_u32regex("(?:\\A|.*\\\\)([^\\\\]+)");
   boost_kahypar::smatch what;
   if(boost_kahypar::u32regex_match(path, what, r))
   {
...
U_NAMESPACE_QUALIFIER UnicodeString extract_greek(const U_NAMESPACE_QUALIFIER UnicodeString& text)
{
   ...
   boost_kahypar::u32regex r = boost_kahypar::make_u32regex(L"[\\x{370}-\\x{3FF}](?:[^[:L*:]]|[\\x{370}-\\x{3FF}])*");
   boost_kahypar::u16match what;
   if(boost_kahypar::u32regex_search(text, what, r))
   {
...
void enumerate_currencies(const std::string& text)
{
   ...
   const char* re = 
      ...
      "(?(1)"
         ...
         "[[:Sc:]]"
      ")";
   boost_kahypar::u32regex r = boost_kahypar::make_u32regex(re);
   boost_kahypar::u32regex_iterator<std::string::const_iterator> i(boost_kahypar::make_u32regex_iterator(text, r)), j;
   while(i != j)
   {
...
void enumerate_currencies2(const std::string& text)
{
   ...
   const char* re = 
      ...
      ")";
   boost_kahypar::u32regex r = boost_kahypar::make_u32regex(re);
   boost_kahypar::u32regex_token_iterator<std::string::const_iterator> 
      i(boost_kahypar::make_u32regex_token_iterator(text, r, 1)), j;
   while(i != j)
   {
      std::cout << *i << std::endl;
      ++i;
...
// just works:
//
const boost_kahypar::u32regex e = boost_kahypar::make_u32regex("\\A(\\d{3,4})[- ]?(\\d{4})[- ]?(\\d{4})[- ]?(\\d{4})\\z");
...
{
   return boost_kahypar::u32regex_replace(s, e, human_format);
}
