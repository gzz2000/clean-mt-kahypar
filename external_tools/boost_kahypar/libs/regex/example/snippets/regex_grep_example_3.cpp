...
class class_index
{
   boost_kahypar::regex expression;
   map_type index;
   std::string::const_iterator base;

   bool grep_callback(boost_kahypar::match_results<std::string::const_iterator> what);
public:
...
bool class_index::grep_callback(boost_kahypar::match_results<std::string::const_iterator> what)
{
...
void class_index::IndexClasses(const std::string& file)
{
   ...
   boost_kahypar::regex_grep(std::bind1st(std::mem_fun1(&class_index::grep_callback), this),
            start,
            end,
            expression);
#elif defined(BOOST_NO_CXX98_BINDERS)
   boost_kahypar::regex_grep(std::bind(&class_index::grep_callback, this, std::placeholders::_1),
            start,
            end,
            expression);
#else
   boost_kahypar::regex_grep(std::bind1st(std::mem_fun(&class_index::grep_callback), this),
            start,
            end,
            expression);
#endif
