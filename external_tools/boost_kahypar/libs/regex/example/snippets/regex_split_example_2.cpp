...
boost_kahypar::regex e("<\\s*A\\s+[^>]*href\\s*=\\s*\"([^\"]*)\"",
               boost_kahypar::regex::normal | boost_kahypar::regbase::icase);
...
int main(int argc, char** argv)
{
   ...
   for(i = 1; i < argc; ++i)
   {
      ...
      load_file(s, is);
      is.close();
      boost_kahypar::regex_split(std::back_inserter(l), s, e);
      while(l.size())
      {
         s = *(l.begin());
         l.pop_front();
   ...
   for(i = 1; i < argc; ++i)
   {
      ...
      load_file(s, is);
      is.close();
      while(boost_kahypar::regex_split(std::ostream_iterator<std::string>(std::cout), s, e, boost_kahypar::match_default, 1)) std::cout << std::endl;
   }
