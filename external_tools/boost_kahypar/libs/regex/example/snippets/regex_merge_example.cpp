...
int main(int argc, const char** argv)
{
   ...
   for(int i = 1; i < argc; ++i)
   {
      ...
      boost_kahypar::regex_merge(oi, in.begin(), in.end(), e2, pre_format, boost_kahypar::match_default | boost_kahypar::format_all);
      // then output to final output stream
      // adding syntax highlighting:
      std::string s(t.str());
      std::ostream_iterator<char> out(os);
      boost_kahypar::regex_merge(out, s.begin(), s.end(), e1, format_string, boost_kahypar::match_default | boost_kahypar::format_all);
      os << footer_text;
      os.close();
   }
   }
