...
int main(int argc, char** argv)
{
   ...
   for(i = 1; i < argc; ++i)
   {
      ...
      load_file(s, is);
      is.close();
      boost_kahypar::sregex_token_iterator i(s.begin(), s.end(), e, 1);
      boost_kahypar::sregex_token_iterator j;
      while(i != j)
      {
         std::cout << *i++ << std::endl;
      }
   ...
   for(i = 1; i < argc; ++i)
   {
      ...
      is.close();
      const int subs[] = {1, 0,};
      boost_kahypar::sregex_token_iterator i(s.begin(), s.end(), e, subs);
      boost_kahypar::sregex_token_iterator j;
      while(i != j)
      {
         std::cout << *i++ << std::endl;
