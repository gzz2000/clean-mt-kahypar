...
{
   return boost_kahypar::regex_match(password, boost_kahypar::make_regex(requirements));
}
...
{
   boost_kahypar::tregex r(__T("(?:\\A|.*\\\\)([^\\\\]+)"));
   boost_kahypar::tmatch what;
   if(boost_kahypar::regex_match(path, what, r))
   {
...
CString extract_postcode(const CString& address)
{
   ...
   boost_kahypar::tregex r(__T("^(([A-Z]{1,2}[0-9]{1,2})|([A-Z]{1,2}[0-9][A-Z]))\\s?([0-9][A-Z]{2})$"));
   boost_kahypar::tmatch what;
   if(boost_kahypar::regex_search(address, what, r))
   {
...
void enumerate_links(const CString& html)
{
   ...
   boost_kahypar::tregex r(__T("href=[\"\']((http:\\/\\/|\\.\\/|\\/)?\\w+(\\.\\w+)*(\\/\\w+(\\.\\w+)?)*(\\/|\\?\\w*=\\w*(&\\w*=\\w*)*)?)[\"\']"));
   boost_kahypar::tregex_iterator i(boost_kahypar::make_regex_iterator(html, r)), j;
   while(i != j)
   {
...
void enumerate_links2(const CString& html)
{
   ...
   // the expression used is by Andew Lee on www.regxlib.com:
   boost_kahypar::tregex r(__T("href=[\"\']((http:\\/\\/|\\.\\/|\\/)?\\w+(\\.\\w+)*(\\/\\w+(\\.\\w+)?)*(\\/|\\?\\w*=\\w*(&\\w*=\\w*)*)?)[\"\']"));
   boost_kahypar::tregex_token_iterator i(boost_kahypar::make_regex_token_iterator(html, r, 1)), j;
   while(i != j)
   {
      std::cout << *i << std::endl;
      ++i;
...
//
const boost_kahypar::tregex e(__T("\\A(\\d{3,4})[- ]?(\\d{4})[- ]?(\\d{4})[- ]?(\\d{4})\\z"));
const CString human_format = __T("$1-$2-$3-$4");

CString human_readable_card_number(const CString& s)
{
   return boost_kahypar::regex_replace(s, e, human_format);
}
