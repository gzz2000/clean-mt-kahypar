...
#include <boost_kahypar/smart_ptr.hpp>
...
int main(int argc, char**argv)
{
   ...
   boost_kahypar::regex ex;
   boost_kahypar::match_results<std::string::const_iterator> sm;
#ifndef BOOST_NO_WREGEX
   std::wstring ws1, ws2;
   boost_kahypar::wregex wex;
   boost_kahypar::match_results<std::wstring::const_iterator> wsm;
#endif
   boost_kahypar::match_results<std::deque<char>::iterator> dm;
   std::string s1, s2, ts;
   std::deque<char> ds;
   boost_kahypar::regex_tA r;
   boost_kahypar::scoped_array<boost_kahypar::regmatch_t> matches;
   std::size_t nsubs;
   ...
   while(true)
   {
      ...
      int code = regcompA(&r, s1.c_str(), boost_kahypar::REG_PERL);
      if(code != 0)
      {
         char buf[256];
      ...
      nsubs = r.re_nsub + 1;
      matches.reset(new boost_kahypar::regmatch_t[nsubs]);

      while(true)
      {
         cout << "Enter string to search (or \"quit\" to exit): ";
         boost_kahypar::getline(*p_in, s2);
         if(argc == 2)
            cout << endl << s2 << endl;
         if(s2 == "quit")
            break;
...
#ifndef BOOST_NO_WREGEX
         // measure time interval for boost_kahypar::wregex
         iters = 10;
         tim = 1.1;
         // cache load:
