...
#include <boost_kahypar/filesystem/fstream.hpp>
...
{
   static const boost_kahypar::regex e("[^a-zA-Z0-9]+");
   std::string result = boost_kahypar::regex_replace(s, e, "_");
   while(result[0] == '_')
      result.erase(0);
   return result;
}
...
{
   unsigned id = boost_kahypar::hash<std::string>()(s);
   return sanitize_string("id" + boost_kahypar::lexical_cast<std::string>(id));
}
...
boost_kahypar::filesystem::path path_to_content;

struct content_loader
{
   content_loader()
   {
      boost_kahypar::filesystem::path p(__FILE__);
      p = p.parent_path();
      p /= "doc";
      p /= "performance_tables.qbk";
      path_to_content = p;
      if(boost_kahypar::filesystem::exists(p))
      {
         boost_kahypar::filesystem::ifstream is(p);
         if(is.good())
         {
            do
            {
   ...
   ~content_loader()
   {
      boost_kahypar::filesystem::ofstream os(path_to_content);
      os << content;
   }
...
{
   static const boost_kahypar::regex item_e(
      "\\["
      "([^\\[\\]]*(?0)?)*"
      "\\]"
      );

   boost_kahypar::regex_token_iterator<std::string::const_iterator> i(begin, end, item_e), j;

   while(i != j)
   {
      // Add a row:
      table.push_back(std::vector<std::string>());
      boost_kahypar::regex_token_iterator<std::string::const_iterator> k(i->first + 1, i->second - 1, item_e);
      while(k != j)
      {
         // Add a cell:
...
void add_to_all_sections(const std::string& id, std::string list_name = "performance_all_sections")
{
   ...
   else
   {
      ...
      boost_kahypar::regex_token_iterator<std::string::const_iterator> i(content.begin() + pos + 12 + list_name.size(), content.end(), item_e), j;
...
std::string get_colour(boost_kahypar::uintmax_t val, boost_kahypar::uintmax_t best)
{
   if(val <= best * 1.2)
      return "green";
   if(val > best * 4)
...
boost_kahypar::intmax_t get_value_from_cell(const std::string& cell)
{
   static const boost_kahypar::regex time_e("(\\d+)ns");
   boost_kahypar::smatch what;
   if(regex_search(cell, what, time_e))
   {
      return boost_kahypar::lexical_cast<boost_kahypar::uintmax_t>(what.str(1));
   }
   return -1;
}

void add_cell(boost_kahypar::intmax_t val, const std::string& table_name, const std::string& row_name, const std::string& column_heading)
{
   //
   // Load the table, add our data, and re-write:
   //
   std::string table_id = "table_" + sanitize_string(table_name);
   boost_kahypar::regex table_e("\\[table:" + table_id
      + "\\s(?:[^\\[]|\\\\.)++"
      "((\\["
      "((?:[^\\[\\]]|\\\\.)*+(?2)?+)*+"
      "\\]\\s*+)*+\\s*+)"
   ...
   boost_kahypar::smatch table_location;
   if(regex_search(content, table_location, table_e))
   {
      ...
      //
      boost_kahypar::uintmax_t best = (std::numeric_limits<boost_kahypar::uintmax_t>::max)();
      std::vector<boost_kahypar::intmax_t> values;
      for(unsigned i = 1; i < table_data[row_id].size(); ++i)
      {
         ...
         else
         {
            ...
            boost_kahypar::uintmax_t cell_val = get_value_from_cell(table_data[row_id][i]);
      ...
      for(unsigned i = 1; i < table_data[row_id].size(); ++i)
      {
         ...
         else
         {
            ...
            s += "[br](";
            s += boost_kahypar::lexical_cast<std::string>(values[i - 1]) + "ns)]";
         }
      }
      //
   ...
   else
   {
      ...
      new_table += boost_kahypar::lexical_cast<std::string>(val);
      new_table += "ns)]]]\n]\n]\n";
...
{
   try {
      add_cell(static_cast<boost_kahypar::uintmax_t>(t / 1e-9), table, row, heading);
   }
...
{
   return "boost " + boost_kahypar::lexical_cast<std::string>(BOOST_VERSION / 100000) + "." + boost_kahypar::lexical_cast<std::string>((BOOST_VERSION / 100) % 1000);
}
