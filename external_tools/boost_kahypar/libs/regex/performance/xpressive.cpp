...
#include <boost_kahypar/xpressive/xpressive.hpp>

using namespace boost_kahypar::xpressive;
...
public:
   ...
   struct initializer
   {
      initializer()
      {
         xpressive_regex::register_instance(boost_kahypar::shared_ptr<abstract_regex>(new xpressive_regex));
      }
...
{
   init.do_nothing();
   return "boost_kahypar::xpressive::cregex";
}
