...
public:
   ...
   struct initializer
   {
      initializer()
      {
         pcre_regex::register_instance(boost_kahypar::shared_ptr<abstract_regex>(new pcre_regex));
      }
...
{
   init.do_nothing();
   return std::string("PCRE-") + boost_kahypar::lexical_cast<std::string>(PCRE2_MAJOR) + "." + boost_kahypar::lexical_cast<std::string>(PCRE2_MINOR);
}
