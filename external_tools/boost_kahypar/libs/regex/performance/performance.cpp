...
typedef std::list<boost_kahypar::shared_ptr<abstract_regex> > list_type;
...
void abstract_regex::register_instance(boost_kahypar::shared_ptr<abstract_regex> item)
{
...
double exec_timed_test(Func f)
{
   ...
   unsigned repeats = 1;
   do {
      stopwatch<boost_kahypar::chrono::high_resolution_clock> w;
      ...
      t = boost_kahypar::chrono::duration_cast<boost_kahypar::chrono::duration<double>>(w.elapsed()).count();
      if(t < 0.5)
         repeats *= 2;
   } while(t < 0.5);
   return t / repeats;
...
{
   static const boost_kahypar::regex e("[`/_*=$^@#&%\\\\]");
   static const boost_kahypar::regex open_b("\\[");
   static const boost_kahypar::regex close_b("\\]");
...
{
   boost_kahypar::filesystem::path here(__FILE__);
   here = here.parent_path().parent_path().parent_path().parent_path();

   boost_kahypar::filesystem::path cpp_file = here / "boost";
   cpp_file /= "crc.hpp";
   ...
   const char* boost_include_expression = "^[ \t]*#[ \t]*include[ \t]+(\"boost_kahypar/[^\"]+\"|<boost_kahypar/[^>]+>)";
   ...
   test_search(function_with_body_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(brace_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(call_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(highlight_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(class_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(include_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
   test_search(boost_include_expression, file_contents.c_str(), true, "boost_kahypar/multiprecision/number.hpp");
