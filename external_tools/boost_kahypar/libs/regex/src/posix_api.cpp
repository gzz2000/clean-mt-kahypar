...
#else
namespace boost_kahypar { namespace core { using std::snprintf; } }
#endif
...
typedef boost_kahypar::basic_regex<char, c_regex_traits<char> > c_regex_type;
...
BOOST_REGEX_DECL int BOOST_REGEX_CCALL regcompA(regex_tA* expression, const char* ptr, int f)
{
   ...
   catch(const boost_kahypar::regex_error& be)
   {
      result = be.code();
   }
   catch(...)
...
BOOST_REGEX_DECL regsize_t BOOST_REGEX_CCALL regerrorA(int code, const regex_tA* e, char* buf, regsize_t buf_size)
{
   ...
   if(code == REG_ATOI)
   {
      ...
      for(int i = 0; i <= (int)REG_E_UNKNOWN; ++i)
      {
         if(std::strcmp(e->re_endp, names[i]) == 0)
         {
            ...
            //
            int r = (boost_kahypar::core::snprintf)(localbuf, 5, "%d", i);
            if(r < 0)
      ...
      int r = (boost_kahypar::core::snprintf)(localbuf, 5, "%d", 0);
      if(r < 0)
         return 0; // sprintf failed
      if(std::strlen(localbuf) < buf_size)
         BOOST_REGEX_DETAIL_NS::strcpy_s(buf, buf_size, localbuf);
   ...
   if(code <= (int)REG_E_UNKNOWN)
   {
      ...
      if((e) && (e->re_magic == magic_value))
         p = static_cast<c_regex_type*>(e->guts)->get_traits().error_string(static_cast< ::boost_kahypar::regex_constants::error_type>(code));
      else
      {
         p = BOOST_REGEX_DETAIL_NS::get_default_error_string(static_cast< ::boost_kahypar::regex_constants::error_type>(code));
      }
