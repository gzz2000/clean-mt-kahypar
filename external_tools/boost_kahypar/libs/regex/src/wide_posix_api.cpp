...
#else
namespace boost_kahypar { namespace core { using std::swprintf; } }
#endif
...
typedef boost_kahypar::basic_regex<wchar_t, c_regex_traits<wchar_t> > wc_regex_type;
...
BOOST_REGEX_DECL regsize_t BOOST_REGEX_CCALL regerrorW(int code, const regex_tW* e, wchar_t* buf, regsize_t buf_size)
{
...
#else
            (boost_kahypar::core::swprintf)(localbuf, 5, L"%d", i);
#endif
...
#else
      (boost_kahypar::core::swprintf)(localbuf, 5, L"%d", 0);
#endif
      if(std::wcslen(localbuf) < buf_size)
#if BOOST_WORKAROUND(BOOST_MSVC, >= 1400) && !defined(_WIN32_WCE) && !defined(UNDER_CE)
         ::wcscpy_s(buf, buf_size, localbuf);
   ...
   if(code <= (int)REG_E_UNKNOWN)
   {
      ...
      if((e) && (e->re_magic == wmagic_value))
         p = static_cast<wc_regex_type*>(e->guts)->get_traits().error_string(static_cast< ::boost_kahypar::regex_constants::error_type>(code));
      else
      {
