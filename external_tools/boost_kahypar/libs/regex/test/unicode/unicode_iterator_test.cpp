...
#include <boost_kahypar/regex/pending/unicode_iterator.hpp>
...
void spot_checks()
{
   ...
   // self-consistent:
   ::boost_kahypar::uint32_t spot16[] = { 0x10302u, };
   typedef boost_kahypar::u32_to_u16_iterator<const ::boost_kahypar::uint32_t*> u32to16type;
   ...
   ::boost_kahypar::uint32_t spot8[] = { 0x004Du, 0x0430u, 0x4E8Cu, 0x10302u, };
   typedef boost_kahypar::u32_to_u8_iterator<const ::boost_kahypar::uint32_t*> u32to8type;
   ...
   //
   boost_kahypar::uint8_t bad_seq[10] = { 0x4Du, 0xD0u, 0xB0u, 0xE4u, 0xBAu, 0x8Cu, 0xF0u, 0x90u, 0x8Cu, 0x82u };
   BOOST_CHECK_EQUAL(
      iterate_over(
         boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq, bad_seq, bad_seq + 10),
         boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq+10, bad_seq, bad_seq + 10)),
      0x000149f3u);
   BOOST_CHECK_THROW(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq, bad_seq, bad_seq + 9), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq, bad_seq, bad_seq + 8), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq, bad_seq, bad_seq + 7), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq + 2, bad_seq, bad_seq + 10), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq + 2, bad_seq + 2, bad_seq + 10), std::out_of_range);

   boost_kahypar::uint16_t bad_seq2[6] =  { 0xD800, 0xDF02, 0xD800, 0xDF02, 0xD800, 0xDF02 };
   BOOST_CHECK_EQUAL(
      iterate_over(
         boost_kahypar::u16_to_u32_iterator<const boost_kahypar::uint16_t*>(bad_seq2, bad_seq2, bad_seq2 + 6),
         boost_kahypar::u16_to_u32_iterator<const boost_kahypar::uint16_t*>(bad_seq2+6, bad_seq2, bad_seq2 + 6)),
      66306u);
   BOOST_CHECK_THROW(boost_kahypar::u16_to_u32_iterator<const boost_kahypar::uint16_t*>(bad_seq2, bad_seq2, bad_seq2 + 5), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u16_to_u32_iterator<const boost_kahypar::uint16_t*>(bad_seq2 + 1, bad_seq2 + 1, bad_seq2 + 6), std::out_of_range);
   BOOST_CHECK_THROW(boost_kahypar::u16_to_u32_iterator<const boost_kahypar::uint16_t*>(bad_seq2 + 1, bad_seq2, bad_seq2 + 6), std::out_of_range);

   boost_kahypar::uint8_t bad_seq3[5] = { '.', '*', 0xe4, '.', '*' };
   BOOST_CHECK_THROW(iterate_over(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq3, bad_seq3, bad_seq3 + 5), boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq3 + 5, bad_seq3, bad_seq3 + 5)), std::out_of_range);
   boost_kahypar::uint8_t bad_seq4[5] = { '.', '*', 0xf6, '.', '*' };
   BOOST_CHECK_THROW(iterate_over(boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq4, bad_seq4, bad_seq4 + 5), boost_kahypar::u8_to_u32_iterator<const boost_kahypar::uint8_t*>(bad_seq4 + 5, bad_seq4, bad_seq4 + 5)), std::out_of_range);

   // Invalid sequences containing surrogate pairs:
   const char* invalid_pseq = "\xed\xa0\x80"; // single lowest lead surrogate U+D800
   BOOST_CHECK_THROW(iterate_over(boost_kahypar::u8_to_u32_iterator<const char*>(invalid_pseq, invalid_pseq, invalid_pseq + std::strlen(invalid_pseq)), boost_kahypar::u8_to_u32_iterator<const char*>(invalid_pseq + std::strlen(invalid_pseq), invalid_pseq, invalid_pseq + std::strlen(invalid_pseq))), std::out_of_range);
...
void test(const std::vector< ::boost_kahypar::uint32_t>& v)
{
   typedef std::vector< ::boost_kahypar::uint32_t> vector32_type;
#ifdef TEST_UTF16
   typedef std::vector< ::boost_kahypar::uint16_t> vector16_type;
#endif
   typedef std::vector< ::boost_kahypar::uint8_t>  vector8_type;
#ifdef TEST_UTF16
   typedef boost_kahypar::u32_to_u16_iterator<vector32_type::const_iterator, ::boost_kahypar::uint16_t> u32to16type;
   typedef boost_kahypar::u16_to_u32_iterator<vector16_type::const_iterator, ::boost_kahypar::uint32_t> u16to32type;
...
#endif // TEST_UTF16
#ifdef TEST_UTF8
   typedef boost_kahypar::u32_to_u8_iterator<vector32_type::const_iterator, ::boost_kahypar::uint8_t> u32to8type;
   typedef boost_kahypar::u8_to_u32_iterator<vector8_type::const_iterator, ::boost_kahypar::uint32_t> u8to32type;
#if !defined(BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION) && !defined(BOOST_NO_STD_ITERATOR) && !defined(_RWSTD_NO_CLASS_PARTIAL_SPEC)
...
int cpp_main( int, char* [] ) 
{
   ...
   std::vector< ::boost_kahypar::uint32_t> v;
