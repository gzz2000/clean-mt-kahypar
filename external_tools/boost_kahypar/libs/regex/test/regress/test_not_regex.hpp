...
void test_empty(boost_kahypar::basic_regex<charT, traits>& r)
{
   if(!r.empty())
   {
...
template<class charT, class traits>
void test(boost_kahypar::basic_regex<charT, traits>& r, const test_invalid_regex_tag&)
{
...
#endif
   {
      if(0 == r.assign(expression, syntax_options | boost_kahypar::regex_constants::no_except).status())
      {
