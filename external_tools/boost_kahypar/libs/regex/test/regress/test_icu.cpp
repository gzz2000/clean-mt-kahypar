...
#include <boost_kahypar/mpl/int.hpp>
#else
#include <type_traits>
...
void compare_result(const MR1& w1, const MR2& w2, boost_kahypar::mpl::int_<2> const*)
{
   typedef typename MR2::value_type MR2_value_type;
   typedef typename MR2_value_type::const_iterator MR2_iterator_type;
   typedef boost_kahypar::u16_to_u32_iterator<MR2_iterator_type> iterator_type;
...
template <class MR1, class MR2>
void compare_result(const MR1& w1, const MR2& w2, boost_kahypar::mpl::int_<1> const*)
{
   typedef typename MR2::value_type MR2_value_type;
   typedef typename MR2_value_type::const_iterator MR2_iterator_type;
   typedef boost_kahypar::u8_to_u32_iterator<MR2_iterator_type> iterator_type;
   //typedef typename MR1::size_type size_type;
   if(w1.size() != w2.size())
   {
      BOOST_REGEX_TEST_ERROR("Size mismatch in match_results class", UChar32);
...
void test_icu_grep(const boost_kahypar::u32regex& r, const std::vector< ::UChar32>& search_text)
{
   typedef std::vector< ::UChar32>::const_iterator const_iterator;
   typedef boost_kahypar::u32regex_iterator<const_iterator> test_iterator;
   boost_kahypar::regex_constants::match_flag_type opts = test_info<wchar_t>::match_options();
...
void test_icu(const wchar_t&, const test_regex_search_tag& )
{
...
#endif
   boost_kahypar::regex_constants::syntax_option_type syntax_options = test_info<UChar32>::syntax_options();
...
#endif
   {
      ...
      boost_kahypar::regex_constants::match_flag_type opts = test_info<wchar_t>::match_options();
      const int* answer_table = test_info<wchar_t>::answer_table();
      boost_kahypar::match_results<std::vector< ::UChar32>::const_iterator> what;
      if(boost_kahypar::u32regex_search(
      ...
      if(0 == *test_locale::c_str())
      {
         ...
         //
         typedef boost_kahypar::u32_to_u16_iterator<std::vector<UChar32>::const_iterator> u16_conv;
         std::vector<UChar> expression16, text16;
         boost_kahypar::match_results<std::vector<UChar>::const_iterator> what16;
         boost_kahypar::match_results<const UChar*> what16c;
#ifndef BOOST_NO_TEMPLATED_ITERATOR_CONSTRUCTORS
...
#endif
         r = boost_kahypar::make_u32regex(expression16.begin(), expression16.end(), syntax_options);
         if(boost_kahypar::u32regex_search(const_cast<const std::vector<UChar>&>(text16).begin(), const_cast<const std::vector<UChar>&>(text16).end(), what16, r, opts))
         {
            compare_result(what, what16, static_cast<boost_kahypar::mpl::int_<2> const*>(0));
         }
         else if(answer_table[0] >= 0)
         {
            // we should have had a match but didn't:
         ...
         if(std::find(expression16.begin(), expression16.end(), 0) == expression16.end())
         {
            ...
            r = boost_kahypar::make_u32regex(&*expression16.begin(), syntax_options);
            if(std::find(text16.begin(), text16.end(), 0) == text16.end())
            {
               text16.push_back(0);
               if(boost_kahypar::u32regex_search((const UChar*)&*text16.begin(), what16c, r, opts))
               {
                  compare_result(what, what16c, static_cast<boost_kahypar::mpl::int_<2> const*>(0));
               }
               else if(answer_table[0] >= 0)
               {
                  // we should have had a match but didn't:
         ...
         //
         typedef boost_kahypar::u32_to_u8_iterator<std::vector<UChar32>::const_iterator, unsigned char> u8_conv;
         std::vector<unsigned char> expression8, text8;
         boost_kahypar::match_results<std::vector<unsigned char>::const_iterator> what8;
         boost_kahypar::match_results<const unsigned char*> what8c;
#ifndef BOOST_NO_TEMPLATED_ITERATOR_CONSTRUCTORS
...
#endif
         r = boost_kahypar::make_u32regex(expression8.begin(), expression8.end(), syntax_options);
         if(boost_kahypar::u32regex_search(const_cast<const std::vector<unsigned char>&>(text8).begin(), const_cast<const std::vector<unsigned char>&>(text8).end(), what8, r, opts))
         {
            compare_result(what, what8, static_cast<boost_kahypar::mpl::int_<1> const*>(0));
         }
         else if(answer_table[0] >= 0)
         {
            // we should have had a match but didn't:
         ...
         if(std::find(expression8.begin(), expression8.end(), 0) == expression8.end())
         {
            ...
            r = boost_kahypar::make_u32regex(&*expression8.begin(), syntax_options);
            if(std::find(text8.begin(), text8.end(), 0) == text8.end())
            {
               text8.push_back(0);
               if(boost_kahypar::u32regex_search((const unsigned char*)&*text8.begin(), what8c, r, opts))
               {
                  compare_result(what, what8c, static_cast<boost_kahypar::mpl::int_<1> const*>(0));
               }
               else if(answer_table[0] >= 0)
               {
                  // we should have had a match but didn't:
...
#ifndef BOOST_NO_EXCEPTIONS
   catch(const boost_kahypar::bad_expression& e)
   {
...
{
   //typedef boost_kahypar::u16_to_u32_iterator<std::wstring::const_iterator, ::UChar32> conv_iterator;
   std::vector< ::UChar32> expression;
#ifndef BOOST_NO_TEMPLATED_ITERATOR_CONSTRUCTORS
   expression.assign(test_info<wchar_t>::expression().begin(), test_info<wchar_t>::expression().end());
#else
   std::copy(test_info<wchar_t>::expression().begin(), test_info<wchar_t>::expression().end(), std::back_inserter(expression));
#endif
   boost_kahypar::regex_constants::syntax_option_type syntax_options = test_info<wchar_t>::syntax_options();
   boost_kahypar::u32regex r;
   if(*test_locale::c_str())
   {
      U_NAMESPACE_QUALIFIER Locale l(test_locale::c_str());
   ...
   {
#if !defined(BOOST_NO_MEMBER_TEMPLATES) && !defined(__IBMCPP__)
      if(0 == r.assign(expression.begin(), expression.end(), syntax_options | boost_kahypar::regex_constants::no_except).status())
#else
      if(expression.size())
         r.assign(&*expression.begin(), expression.size(), syntax_options | boost_kahypar::regex_constants::no_except);
      else
         r.assign(static_cast<UChar32 const*>(0), static_cast<boost_kahypar::u32regex::size_type>(0), syntax_options | boost_kahypar::regex_constants::no_except);
      if(0 == r.status())
#endif
      {
...
#endif
   {
      ...
      if(expression.size())
         ...
         r.assign(static_cast<UChar32 const*>(0), static_cast<boost_kahypar::u32regex::size_type>(0), syntax_options);
#endif
...
#ifndef BOOST_NO_EXCEPTIONS
   catch(const boost_kahypar::bad_expression&)
   {
      have_catch = true;
   }
   catch(const std::runtime_error& e)
   ...
   if(0 == *test_locale::c_str())
   {
      ...
      std::copy(u16_conv(expression.begin()), u16_conv(expression.end()), std::back_inserter(expression16));
#endif
      if(0 == boost_kahypar::make_u32regex(expression16.begin(), expression16.end(), syntax_options | boost_kahypar::regex_constants::no_except).status())
      {
         BOOST_REGEX_TEST_ERROR("Expression compiled when it should not have done so.", wchar_t);
      }
      if(std::find(expression16.begin(), expression16.end(), 0) == expression16.end())
      {
         expression16.push_back(0);
         if(0 == boost_kahypar::make_u32regex(&*expression16.begin(), syntax_options | boost_kahypar::regex_constants::no_except).status())
         {
            BOOST_REGEX_TEST_ERROR("Expression compiled when it should not have done so.", wchar_t);
         }
      }
      ...
      // Now try UTF-8 construction:
      //
      typedef boost_kahypar::u32_to_u8_iterator<std::vector<UChar32>::const_iterator> u8_conv;
      ...
      std::copy(u8_conv(expression.begin()), u8_conv(expression.end()), std::back_inserter(expression8));
#endif
      if(0 == boost_kahypar::make_u32regex(expression8.begin(), expression8.end(), syntax_options | boost_kahypar::regex_constants::no_except).status())
      {
         BOOST_REGEX_TEST_ERROR("Expression compiled when it should not have done so.", wchar_t);
      }
      if(std::find(expression8.begin(), expression8.end(), 0) == expression8.end())
      {
         expression8.push_back(0);
         if(0 == boost_kahypar::make_u32regex(&*expression8.begin(), syntax_options | boost_kahypar::regex_constants::no_except).status())
         {
            BOOST_REGEX_TEST_ERROR("Expression compiled when it should not have done so.", wchar_t);
         }
      }
...
void test_icu(const wchar_t&, const test_regex_replace_tag&)
{
...
#endif
   {
      ...
      boost_kahypar::regex_constants::match_flag_type opts = test_info<UChar32>::match_options();
      ...
      boost_kahypar::u32regex_replace(unnecessary_fix::back_inserter(result), search_text.begin(), search_text.end(), r, &*format_string.begin(), opts);
      ...
      if(0 == *test_locale::c_str())
      {
...
#endif
         r = boost_kahypar::make_u32regex(expression16.begin(), expression16.end(), syntax_options);
         boost_kahypar::u32regex_replace(unnecessary_fix::back_inserter(found16), text16.begin(), text16.end(), r, &*format16.begin(), opts);
         if(result16 != found16)
         {
         ...
         r = boost_kahypar::make_u32regex(expression16.begin(), expression16.end(), syntax_options);
         found16u = boost_kahypar::u32regex_replace(text16u, r, format16u, opts);
...
#endif
         r = boost_kahypar::make_u32regex(expression8.begin(), expression8.end(), syntax_options);
         boost_kahypar::u32regex_replace(unnecessary_fix::back_inserter(found8), text8.begin(), text8.end(), r, &*format8.begin(), opts);
         if(result8 != found8)
         {
         ...
         r = boost_kahypar::make_u32regex(expression8.begin(), expression8.end(), syntax_options);
         found8s = boost_kahypar::u32regex_replace(text8s, r, format8s, opts);
         if(result8s != found8s)
         {
            BOOST_REGEX_TEST_ERROR("u32regex_replace with UTF-8 string returned incorrect result", UChar32);
         }
