...
int get_posix_compile_options(boost_kahypar::regex_constants::syntax_option_type opts)
{
...
int get_posix_match_flags(boost_kahypar::regex_constants::match_flag_type f)
{
   int result = 0;
   if(f & boost_kahypar::regex_constants::match_not_bol)
      result |= boost_kahypar::REG_NOTBOL;
   if(f & boost_kahypar::regex_constants::match_not_eol)
      result |= boost_kahypar::REG_NOTEOL;
   if(f & ~(boost_kahypar::regex_constants::match_not_bol|boost_kahypar::regex_constants::match_not_eol))
      return -1;
   return result;
}
...
void test_deprecated(const char&, const test_regex_search_tag&)
{
   ...
   boost_kahypar::regex_tA re;
   if(boost_kahypar::regcompA(&re, expression.c_str(), posix_options) != 0)
   {
   ...
   boost_kahypar::regmatch_t matches[max_subs];
   if(boost_kahypar::regexecA(&re, search_text.c_str(), max_subs, matches, posix_match_options) == 0)
   {
      int i = 0;
      while(results[2*i] != -2)
      {
...
void test_deprecated(const wchar_t&, const test_regex_search_tag&)
{
   ...
   // OK try and compile the expression:
   boost_kahypar::regex_tW re;
   if(boost_kahypar::regcompW(&re, expression.c_str(), posix_options) != 0)
   {
   ...
   static const unsigned max_subs = 100;
   boost_kahypar::regmatch_t matches[max_subs];
   if(boost_kahypar::regexecW(&re, search_text.c_str(), max_subs, matches, posix_match_options) == 0)
   {
      int i = 0;
      while(results[2*i] != -2)
      {
...
void test_deprecated(const char&, const test_invalid_regex_tag&)
{
   ...
   // OK try and compile the expression:
   boost_kahypar::regex_tA re;
   int code = boost_kahypar::regcompA(&re, expression.c_str(), posix_options);
   if(code == 0)
   {
      boost_kahypar::regfreeA(&re);
   ...
   {
      char buf[100];
      int s = boost_kahypar::regerrorA(code, &re, 0, 0);
      if(s < 100)
         s = boost_kahypar::regerrorA(code, &re, buf, 100);
      s = boost_kahypar::regerrorA(code | boost_kahypar::REG_ITOA, &re, 0, 0);
      if(s < 100)
      {
         s = boost_kahypar::regerrorA(code | boost_kahypar::REG_ITOA, &re, buf, 100);
         re.re_endp = buf;
         s = boost_kahypar::regerrorA(code | boost_kahypar::REG_ATOI, &re, buf, 100);
         if(s)
         {
...
void test_deprecated(const wchar_t&, const test_invalid_regex_tag&)
{
   ...
   // OK try and compile the expression:
   boost_kahypar::regex_tW re;
   int code = boost_kahypar::regcompW(&re, expression.c_str(), posix_options);
   if(code == 0)
   {
      boost_kahypar::regfreeW(&re);
   ...
   {
      wchar_t buf[100];
      int s = boost_kahypar::regerrorW(code, &re, 0, 0);
      if(s < 100)
         s = boost_kahypar::regerrorW(code, &re, buf, 100);
      s = boost_kahypar::regerrorW(code | boost_kahypar::REG_ITOA, &re, 0, 0);
      if(s < 100)
      {
         s = boost_kahypar::regerrorW(code | boost_kahypar::REG_ITOA, &re, buf, 100);
         re.re_endp = buf;
         s = boost_kahypar::regerrorW(code | boost_kahypar::REG_ATOI, &re, buf, 100);
         if(s)
         {
