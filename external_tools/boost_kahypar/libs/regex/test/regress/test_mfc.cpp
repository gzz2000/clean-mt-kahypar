...
#include <boost_kahypar/regex/mfc.hpp>
#include "test.hpp"
#include "atlstr.h"
...
void test_mfc(const char&, const test_regex_search_tag&)
{
   ...
   boost_kahypar::regex_constants::match_flag_type opts = test_info<char>::match_options();
   const int* answer_table = test_info<char>::answer_table();
   boost_kahypar::regex r = boost_kahypar::make_regex(s2, test_info<char>::syntax_options());
   ...
   if(answer_table[0] < 0)
   {
      if(boost_kahypar::regex_match(s, r, opts))
      {
         BOOST_REGEX_TEST_ERROR("boost_kahypar::regex_match found a match when it should not have done so.", char);
      }
   }
   else
   {
      if((answer_table[0] > 0) && boost_kahypar::regex_match(s, r, opts))
      {
         BOOST_REGEX_TEST_ERROR("boost_kahypar::regex_match found a match when it should not have done so.", char);
      }
   ...
   // test regex_iterator:
   //
   boost_kahypar::cregex_iterator start(boost_kahypar::make_regex_iterator(s, r, opts)), end;
   boost_kahypar::cregex_iterator copy(start);
   while(start != end)
   {
      if(start != copy)
      {
   ...
   //
   typedef boost_kahypar::regex_token_iterator<const char*> token_iterator;
   answer_table = test_info<char>::answer_table();
   //
   // we start by testing sub-expression 0:
   //
   token_iterator tstart(boost_kahypar::make_regex_token_iterator(s, r, 0, opts)), tend;
   ...
   //
   token_iterator tstart2(boost_kahypar::make_regex_token_iterator(s, r, -1, opts)), tend2;
...
void test_mfc(const wchar_t&, const test_regex_search_tag&)
{
   ...
   boost_kahypar::wregex r = boost_kahypar::make_regex(s2, test_info<wchar_t>::syntax_options());
   boost_kahypar::wcmatch what;
   if(boost_kahypar::regex_search(
      s,
      what,
      r,
   ...
   if(answer_table[0] < 0)
   {
      if(boost_kahypar::regex_match(s, r, opts))
      {
         BOOST_REGEX_TEST_ERROR("boost_kahypar::regex_match found a match when it should not have done so.", wchar_t);
      }
   }
   else
   {
   ...
   // test regex_iterator:
   //
   boost_kahypar::wcregex_iterator start(boost_kahypar::make_regex_iterator(s, r, opts)), end;
   boost_kahypar::wcregex_iterator copy(start);
   while(start != end)
   {
      if(start != copy)
      {
   ...
   // test regex_token_iterator:
   //
   typedef boost_kahypar::regex_token_iterator<const wchar_t*> token_iterator;
...
void test_mfc(const char&, const test_invalid_regex_tag&)
{
   ...
   try{
      boost_kahypar::regex e = boost_kahypar::make_regex(s, test_info<char>::syntax_options());
      if(e.error_code())
         have_catch = true;
   }
...
void test_mfc(const wchar_t&, const test_invalid_regex_tag&)
{
   ...
   bool have_catch = false;
   try{
      boost_kahypar::wregex e = boost_kahypar::make_regex(s, test_info<wchar_t>::syntax_options());
      if(e.error_code())
         have_catch = true;
   }
   catch(const boost_kahypar::bad_expression&)
...
void test_mfc(const char&, const test_regex_replace_tag&)
{
   ...
   boost_kahypar::regex_constants::syntax_option_type syntax_options = test_info<char>::syntax_options();
   try{
      boost_kahypar::regex r = boost_kahypar::make_regex(expression, syntax_options);
      if(r.status())
      {
      ...
      CStringA result = boost_kahypar::regex_replace(search_text, r, format_string, opts);
...
void test_mfc(const wchar_t&, const test_regex_replace_tag&)
{
   ...
   try{
      boost_kahypar::wregex r = boost_kahypar::make_regex(expression, syntax_options);
      if(r.status())
      {
      ...
      CStringW result = boost_kahypar::regex_replace(search_text, r, format_string, opts);
      if(result != result_string)
      {
