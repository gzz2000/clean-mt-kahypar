...
void test_sub_match(const boost_kahypar::sub_match<BidirectionalIterator>& sub, BidirectionalIterator base, const int* answer_table, int i, bool recurse = true)
{
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable:4244)
   ...
   if(recurse)
   {
      boost_kahypar::sub_match<BidirectionalIterator> copy(sub);
      test_sub_match(copy, base, answer_table, i, false);
   }
   typedef typename boost_kahypar::sub_match<BidirectionalIterator>::value_type charT;
   if((sub.matched == 0) 
      && 
         !((i == 0)
          && (test_info<charT>::match_options() & boost_kahypar::match_partial)) )
   {
...
void test_result(const boost_kahypar::match_results<BidirectionalIterator, Allocator>& what, BidirectionalIterator base, const int* answer_table, bool recurse = true)
{
   if(recurse)
   {
      boost_kahypar::match_results<BidirectionalIterator, Allocator> copy(what);
      test_result(copy, base, answer_table, false);
      boost_kahypar::match_results<BidirectionalIterator, Allocator> s;
      s.swap(copy);
      test_result(s, base, answer_table, false);
      boost_kahypar::match_results<BidirectionalIterator, Allocator> s2;
      s2 = what;
...
void test_simple_search(boost_kahypar::basic_regex<charT, traits>& r)
{
   ...
   boost_kahypar::regex_constants::match_flag_type opts = test_info<charT>::match_options();
   const int* answer_table = test_info<charT>::answer_table();
   boost_kahypar::match_results<const_iterator> what;
   if(boost_kahypar::regex_search(
      search_text.begin(),
      search_text.end(),
      what,
      r,
      opts))
   {
      ...
      if(!boost_kahypar::regex_search(
         ...
         search_text.end(),
         r,
         opts|boost_kahypar::regex_constants::match_any))
      {
   ...
   else
   {
      ...
      // setting match_any should have no effect on the result returned:
      else if(boost_kahypar::regex_search(
         search_text.begin(),
         search_text.end(),
         r,
         opts|boost_kahypar::regex_constants::match_any))
   ...
   boost_kahypar::match_results<std::rope<charT>::const_iterator> rwhat;
   if(boost_kahypar::regex_search(
      rsearch_text.begin(),
      rsearch_text.end(),
      rwhat,
...
template<class charT, class traits>
void test_regex_iterator(boost_kahypar::basic_regex<charT, traits>& r)
{
   typedef typename std::basic_string<charT>::const_iterator const_iterator;
   typedef boost_kahypar::regex_iterator<const_iterator, charT, traits> test_iterator;
   const std::basic_string<charT>& search_text = test_info<charT>::search_text();
   boost_kahypar::regex_constants::match_flag_type opts = test_info<charT>::match_options();
   const int* answer_table = test_info<charT>::answer_table();
...
template<class charT, class traits>
void test_regex_token_iterator(boost_kahypar::basic_regex<charT, traits>& r)
{
   typedef typename std::basic_string<charT>::const_iterator const_iterator;
   typedef boost_kahypar::regex_token_iterator<const_iterator, charT, traits> test_iterator;
   const std::basic_string<charT>& search_text = test_info<charT>::search_text();
   boost_kahypar::regex_constants::match_flag_type opts = test_info<charT>::match_options();
   const int* answer_table = test_info<charT>::answer_table();
   //
...
struct grep_test_predicate
{
   ...
   grep_test_predicate(test_iter b, const int* a)
      : m_base(b), m_table(a)
   {}
   bool operator()(const boost_kahypar::match_results<test_iter>& what)
   {
...
template<class charT, class traits>
void test_regex_grep(boost_kahypar::basic_regex<charT, traits>& r)
{
   //typedef typename std::basic_string<charT>::const_iterator const_iterator;
   const std::basic_string<charT>& search_text = test_info<charT>::search_text();
   boost_kahypar::regex_constants::match_flag_type opts = test_info<charT>::match_options();
   ...
   boost_kahypar::regex_grep(pred, search_text.begin(), search_text.end(), r, opts);
}

template<class charT, class traits>
void test_regex_match(boost_kahypar::basic_regex<charT, traits>& r)
{
   typedef typename std::basic_string<charT>::const_iterator const_iterator;
   const std::basic_string<charT>& search_text = test_info<charT>::search_text();
   boost_kahypar::regex_constants::match_flag_type opts = test_info<charT>::match_options();
   ...
   if(answer_table[0] < 0)
   {
      if(boost_kahypar::regex_match(search_text, r, opts))
      {
         BOOST_REGEX_TEST_ERROR("boost_kahypar::regex_match found a match when it should not have done so.", charT);
      }
   }
   else
   {
      if((answer_table[0] > 0) && boost_kahypar::regex_match(search_text, r, opts))
      {
         BOOST_REGEX_TEST_ERROR("boost_kahypar::regex_match found a match when it should not have done so.", charT);
      }
      else if((answer_table[0] == 0) && (answer_table[1] == static_cast<int>(search_text.size())))
      {
         if(boost_kahypar::regex_match(
            search_text.begin(),
            search_text.end(),
            what,
            r,
...
template<class charT, class traits>
void test(boost_kahypar::basic_regex<charT, traits>& r, const test_regex_search_tag&)
{
   const std::basic_string<charT>& expression = test_info<charT>::expression();
   boost_kahypar::regex_constants::syntax_option_type syntax_options = test_info<charT>::syntax_options();
...
#endif
   {
      ...
      //
#ifndef BOOST_NO_EXCEPTIONS
      if((syntax_options & boost_kahypar::regbase::save_subexpression_location) == 0)
      {
...
#endif
      r.assign(expression, syntax_options | boost_kahypar::regbase::save_subexpression_location);
