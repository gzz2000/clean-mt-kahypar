 ...
 */
  ...
  *   DESCRIPTION: test program for boost_kahypar::static_mutex.
  */

#include <boost_kahypar/regex/pending/static_mutex.hpp>
#include <boost_kahypar/thread/thread.hpp>
#include <boost_kahypar/timer.hpp>
#include <iostream>
#include <iomanip>
...
void failed_test(const char* file, int line, const char* pred)
{
   ...
   boost_kahypar::static_mutex::scoped_lock guard(mut);
   ++total_failures;
...
bool t2()
{
   ...
   boost_kahypar::static_mutex::scoped_lock guard(mut, false);
...
int main()
{
   ...
   {
      try{
         threads.push_back(boost_kahypar::shared_ptr<boost_kahypar::thread>(new boost_kahypar::thread(&thread1_proc)));
      }
   ...
   {
      try{
         threads.push_back(boost_kahypar::shared_ptr<boost_kahypar::thread>(new boost_kahypar::thread(&thread2_proc)));
      }
      catch(const std::exception& e)
      {
         std::cerr << "<note>Thread creation failed with message: " << e.what() << "</note>" << std::endl;
   ...
   {
      try{
         threads.push_back(boost_kahypar::shared_ptr<boost_kahypar::thread>(new boost_kahypar::thread(&thread3_proc)));
      }
      catch(const std::exception& e)
      {
         std::cerr << "<note>Thread creation failed with message: " << e.what() << "</note>" << std::endl;
