...
#endif
{
   boost_kahypar::regex e(regx);
   boost_kahypar::smatch what;
   if(boost_kahypar::regex_match(text, what, e, boost_kahypar::match_extra))
   {
      unsigned i, j;
#ifndef __sgi
   ...
   boost_kahypar::wregex we(wre);
   boost_kahypar::wsmatch wwhat;
   if(boost_kahypar::regex_match(wtext, wwhat, we, boost_kahypar::match_extra))
   {
      unsigned i, j;
#ifndef __sgi
      // strange type deduction causes this test to fail on SGI:
...
#ifdef BOOST_HAS_ICU
   boost_kahypar::u32regex ure = boost_kahypar::make_u32regex(regx);
   what = boost_kahypar::smatch();
   if(boost_kahypar::u32regex_match(text, what, ure, boost_kahypar::match_extra))
   {
      unsigned i, j;
#ifndef __sgi
      // strange type deduction causes this test to fail on SGI:
