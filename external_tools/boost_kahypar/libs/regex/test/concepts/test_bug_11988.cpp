...
struct char32_traits
{
   ...
   {
      std::string s(p1, p2);
      return boost_kahypar::c_regex_traits<char>::lookup_classname(s.c_str(), s.c_str() + s.length());
      return 0;
   }
   ...
   {
      if(c < 0xff)
         return boost_kahypar::c_regex_traits<char>::isctype(c, t);
      return false;
   }
   static boost_kahypar::intmax_t value(char_type c, int radix)
   {
      switch(radix)
      {
      case 8:
...
int main()
{
   ...
   boost_kahypar::basic_regex<char32_t, char32_traits> e(U"\\x{F}\\x{FF}\\x{FFF}\\x{FFFF}\\x{FFFFF}\\x{FFFFFF}\\x{FFFFFFF}\\x{FFFFFFFF}");
