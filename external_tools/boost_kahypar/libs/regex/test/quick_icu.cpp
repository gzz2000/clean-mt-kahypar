...
{
    static const boost_kahypar::u32regex e = boost_kahypar::make_u32regex("(\\d{4}[- ]){3}\\d{4}");
    return boost_kahypar::u32regex_match(s, e);
}

const boost_kahypar::u32regex card_rx = boost_kahypar::make_u32regex("\\A(\\d{3,4})[- ]?(\\d{4})[- ]?(\\d{4})[- ]?(\\d{4})\\z");
const std::string machine_format("\\1\\2\\3\\4");
const std::string human_format("\\1-\\2-\\3-\\4");

std::string machine_readable_card_number(const std::string& s)
{
    return boost_kahypar::u32regex_replace(s, card_rx, machine_format, boost_kahypar::match_default | boost_kahypar::format_sed);
}

std::string human_readable_card_number(const std::string& s)
{
    return boost_kahypar::u32regex_replace(s, card_rx, human_format, boost_kahypar::match_default | boost_kahypar::format_sed);
}
