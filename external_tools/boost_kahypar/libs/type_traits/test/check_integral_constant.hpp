...
#define BOOST_CHECK_INTEGRAL_CONSTANT(expression, expected_value)\
   if(!::boost_kahypar::detail::tt_compare(::boost_kahypar::detail::integral_constant<(int)(expression)>::value(), (int)expression)){\
   ...
   if(!::boost_kahypar::detail::tt_compare((int)expression, expected_value))\
      BOOST_CHECK_MESSAGE(false, "The expression: \"" << BOOST_STRINGIZE(expression) << "\" had an invalid value (found " << ::boost_kahypar::detail::integral_constant<(int)(expression)>::value() << ", expected " << expected_value << ")" )
...
#define BOOST_CHECK_SOFT_INTEGRAL_CONSTANT(expression, expected_value, alternate_value)\
   ...
   if(!::boost_kahypar::detail::tt_compare((int)expression, expected_value))\
   {\
      if(!::boost_kahypar::detail::tt_compare((int)expression, alternate_value))\
      {\
         BOOST_CHECK_MESSAGE(false, "The expression: \"" << BOOST_STRINGIZE(expression) << "\" had an invalid value (found " << ::boost_kahypar::detail::integral_constant<(int)(expression)>::value() << ", expected " << expected_value << ")" );\
      }\
      else\
      {\
         BOOST_WARN_MESSAGE(false, "<note>The expression: \"" << BOOST_STRINGIZE(expression) << "\" did not have the value we wish it to have (found " << ::boost_kahypar::detail::integral_constant<(int)(expression)>::value() << ", expected " << expected_value << ")</note>" );\
      }\
    }
