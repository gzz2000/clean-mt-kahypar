...
template <class T>
void do_destroy_array(T* first, T* last, const boost_kahypar::false_type&)
{
   while(first != last)
   {
      first->~T();
...
template <class T>
inline void do_destroy_array(T* first, T* last, const boost_kahypar::true_type&)
{
}
...
{
   detail::do_destroy_array(p1, p2, ::boost_kahypar::has_trivial_destructor<T>());
}
