...
#include <boost_kahypar/test/included/prg_exec_monitor.hpp>
...
void do_fill(I first, I last, const T& val, const boost_kahypar::integral_constant<bool, b>&)
{
   while(first != last)
   {
      *first = val;
...
template <typename T>
void do_fill(T* first, T* last, const T& val, const boost_kahypar::true_type&)
{
...
inline void fill(I first, I last, const T& val)
{
   ...
   //
   typedef boost_kahypar::integral_constant<bool, 
      ::boost_kahypar::has_trivial_assign<T>::value && (sizeof(T) == 1)> truth_type;
...
inline void fill(I first, I last, const T& val)
{
   opt::detail::do_fill(first, last, val, boost_kahypar::false_type());
}
