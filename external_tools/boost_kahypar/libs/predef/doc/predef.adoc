...
* All possible definitions are included with the single `#include <boost_kahypar/predef.h>`
  so that it's friendly to pre-compiled header usage.
* Specific definitions can be included, ex. `#include <boost_kahypar/predef/os/windows.h>`
  for single checks.
...
[source]
----
#include <boost_kahypar/predef/CATEGORY_TAG/DEPENDENCY.h>
----
...
#if BOOST_category_tag
#   define BOOST_category_tag_AVAILABLE
#   include <boost_kahypar/predef/detail/CATEGORY_detected.h>
#endif
----
