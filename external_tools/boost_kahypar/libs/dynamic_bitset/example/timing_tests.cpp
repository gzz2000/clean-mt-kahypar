...
//  boost_kahypar::dynamic_bitset timing tests
//
//  NOTE:
//  ~~~~~
...
//   Also, if you use boost_kahypar::dynamic_bitset on a platform where
...
void timing_test(T* = 0) // dummy parameter to workaround VC6
{
    ...
    typename boost_kahypar::dynamic_bitset<T>::size_type dummy = 0;
    ...
    {
        boost_kahypar::timer::auto_cpu_timer time;

        const typename boost_kahypar::dynamic_bitset<T>::size_type sz = 5000;
        for (unsigned long i = 0; i < num; ++i) {
            boost_kahypar::dynamic_bitset<T> bs(sz, i);
            dummy += bs.count();
        }
    }
...
# ifdef BOOST_HAS_LONG_LONG
    timing_test< ::boost_kahypar::ulong_long_type>();
# endif
