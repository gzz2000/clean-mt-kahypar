...
public:
  scoped_temp_file()
    : m_path(boost_kahypar::filesystem::unique_path())
  {
  }

  ~scoped_temp_file()
  {
    boost_kahypar::filesystem::remove(m_path);
  }

  const boost_kahypar::filesystem::path& path() const
  {
    return m_path;
  }
...
struct bitset_test {
  ...
  static void to_block_range(const Bitset & b /*, BlockOutputIterator result*/)
  {
    ...
    boost_kahypar::to_block_range(b, v.begin() + offset);
  ...
  static void from_block_range(const std::vector<Block>& blocks)
  {
    ...
    { // test boost_kahypar::from_block_range
      const typename Bitset::size_type n = blocks.size();
      Bitset bset(n * bits_per_block);
      boost_kahypar::from_block_range(blocks.begin(), blocks.end(), bset);
      for (std::size_t b = 0; b < n; ++b) {
        typename Bitset::block_width_type i = 0;
        for (; i < bits_per_block; ++i) {
          std::size_t bit = b * bits_per_block + i;
  ...
  {
    Bitset copy(boost_kahypar::move(b));
    BOOST_TEST(b == copy);
  }
  ...
  static void move_assignment_operator(const Bitset& lhs, const Bitset& rhs)
  {
    Bitset b(lhs);
    Bitset c(rhs);
    b = boost_kahypar::move(c);
    b = boost_kahypar::move(b); // self assignment check
    BOOST_TEST(b == rhs);
  }
...
//                               I/O TESTS
                              ...
                              )
  {
    ...
    if (!stream_was_good) {
      ...
      boost_kahypar::to_string(b, expected);
