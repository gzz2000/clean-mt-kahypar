...
{
    using boost_kahypar::range::unit_test::mock_range;
    using boost_kahypar::adaptors::uniqued;

    // This next line should fail when Boost.Range concept checking is
    // enabled since the adaptor takes at least a ForwardRange.
    return (mock_range<boost_kahypar::single_pass_traversal_tag>() | uniqued).front();
}
