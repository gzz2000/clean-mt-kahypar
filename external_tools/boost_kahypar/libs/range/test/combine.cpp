...
//
#include <boost_kahypar/range/combine.hpp>
...
void test_combine2()
{
    ...
    std::vector<boost_kahypar::tuple<int,int> > output;
    boost_kahypar::push_back(output, boost_kahypar::combine(in1, in2));

    int index = 0;
    int i1, i2;
    BOOST_FOREACH(boost_kahypar::tie(i1,i2), output)
    {
...
void test_combine3()
{
    ...
    std::vector<boost_kahypar::tuple<int,int,int> > output;
    boost_kahypar::push_back(output, boost_kahypar::combine(in1, in2, in3));

    int index = 0;
    int i1, i2, i3;

    BOOST_FOREACH(boost_kahypar::tie(i1,i2,i3), output)
    {
        BOOST_CHECK_EQUAL(i1, index);
        BOOST_CHECK_EQUAL(i2, index * 2);
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite(int, char*[] )
{
    ...
    test->add(BOOST_TEST_CASE((
            &boost_kahypar_range_test::test_combine2<
    ...
    test->add(BOOST_TEST_CASE((
            &boost_kahypar_range_test::test_combine3<
                        std::vector<int>,
                        std::vector<int>,
                        std::vector<int> >)));
