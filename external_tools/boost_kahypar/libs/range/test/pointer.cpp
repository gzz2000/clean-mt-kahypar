...
#include <boost_kahypar/range/pointer.hpp>
...
void test_pointer()
{
    ...
    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            cont::pointer,
            boost_kahypar::range_pointer<cont>::type
        >::value));

    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            cont::const_pointer,
            boost_kahypar::range_pointer<const cont>::type
        >::value));
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] )
{
    ...
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_pointer));
