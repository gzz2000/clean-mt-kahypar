...
namespace
{
    ...
    {
        MockBinaryFn fn = boost_kahypar::range::for_each(rng1, rng2, MockBinaryFn());

        BOOST_CHECK_EQUAL_COLLECTIONS( ::boost_kahypar::begin(rng1), ::boost_kahypar::end(rng1),
            fn.xs.begin(), fn.xs.end() );

        BOOST_CHECK_EQUAL_COLLECTIONS( ::boost_kahypar::begin(rng2), ::boost_kahypar::end(rng2),
