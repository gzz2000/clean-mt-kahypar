...
//
#include <boost_kahypar/range/algorithm_ext/is_sorted.hpp>
...
namespace
{
    ...
    void test_is_sorted_impl()
    {
        ...
        BOOST_CHECK( boost_kahypar::is_sorted(ascending) );
        BOOST_CHECK( boost_kahypar::is_sorted(ascending, std::less<std::size_t>()) );
        BOOST_CHECK( boost_kahypar::is_sorted(ascending, std::greater<std::size_t>()) );
        ...
        BOOST_CHECK( boost_kahypar::is_sorted(ascending) );
        BOOST_CHECK( !boost_kahypar::is_sorted(descending) );
        BOOST_CHECK( !boost_kahypar::is_sorted(ascending, std::greater<std::size_t>()) );
        BOOST_CHECK( boost_kahypar::is_sorted(descending, std::greater<std::size_t>()) );
    }
