...
std::string reference_result(const std::vector<boost_kahypar::int32_t>& v,
                             ...
                             const std::string& postfix)
{
    ...
    {
        out << v.at(0);
        std::vector<boost_kahypar::int32_t>::const_iterator it = v.begin();
        for (++it; it != v.end(); ++it)
        {
            out << separator << *it;
        }
...
void test_formatted_0args_impl(const std::vector<boost_kahypar::int32_t>& v)
{
    std::ostringstream out1;
    out1 << '[' << (v | boost_kahypar::adaptors::formatted()) << ']';
    BOOST_CHECK_EQUAL(out1.str(), reference_result(v, ",", "[{", "}]"));

    std::ostringstream out2;
    out2 << '[' << boost_kahypar::adaptors::format(v) << ']';
    BOOST_CHECK_EQUAL(out2.str(), reference_result(v, ",", "[{", "}]"));

    std::ostringstream out3;
    out3 << (v | boost_kahypar::adaptors::formatted());
    BOOST_CHECK_EQUAL(out3.str(), reference_result(v, ",", "{", "}"));

    std::ostringstream out4;
    out4 << boost_kahypar::adaptors::format(v);
    BOOST_CHECK_EQUAL(out4.str(), reference_result(v, ",", "{", "}"));
}

template<typename Sep>
void test_formatted_1arg_impl(
        const std::vector<boost_kahypar::int32_t>& v,
        const Sep& sep)
{
    const std::string ref_sep = make_string(sep);
    std::ostringstream out1;
    out1 << '[' << (v | boost_kahypar::adaptors::formatted(sep)) << ']';

    BOOST_CHECK_EQUAL(out1.str(), reference_result(v, ref_sep, "[{", "}]"));

    std::ostringstream out2;
    out2 << '[' << boost_kahypar::adaptors::format(v, sep) << ']';
    BOOST_CHECK_EQUAL(out2.str(), reference_result(v, ref_sep, "[{", "}]"));

    std::ostringstream out3;
    out3 << (v | boost_kahypar::adaptors::formatted(sep));
    BOOST_CHECK_EQUAL(out3.str(), reference_result(v, ref_sep, "{", "}"));

    std::ostringstream out4;
    out4 << boost_kahypar::adaptors::format(v, sep);
    BOOST_CHECK_EQUAL(out4.str(), reference_result(v, ref_sep, "{", "}"));
}

void test_formatted_1arg_impl(const std::vector<boost_kahypar::int32_t>& v)
{
...
)
{
    ...
    std::ostringstream out1;
    out1 << '[' << (v | boost_kahypar::adaptors::formatted(sep, prefix)) << ']';
    BOOST_CHECK_EQUAL(
        out1.str(),
        reference_result(v, ref_sep, make_string('[', prefix), "}]"));

    std::ostringstream out2;
    out2 << '[' << boost_kahypar::adaptors::format(v, sep, prefix) << ']';
    BOOST_CHECK_EQUAL(
        out2.str(),
        reference_result(v, ref_sep, make_string('[', prefix), "}]"));

    std::ostringstream out3;
    out3 << (v | boost_kahypar::adaptors::formatted(sep, prefix));
    BOOST_CHECK_EQUAL(
        out3.str(),
        reference_result(v, ref_sep, make_string(prefix), "}"));

    std::ostringstream out4;
    out4 << boost_kahypar::adaptors::format(v, sep, prefix);
    BOOST_CHECK_EQUAL(
        out4.str(),
        reference_result(v, ref_sep, make_string(prefix), "}"));
}

void test_formatted_2args_impl(const std::vector<boost_kahypar::int32_t>& v)
{
...
)
{
    ...
    std::ostringstream out1;
    out1 << '[' << (v | boost_kahypar::adaptors::formatted(sep, prefix, postfix))
         << ']';
    BOOST_CHECK_EQUAL(
        out1.str(),
        reference_result(v, ref_sep, make_string('[', prefix),
...
void test_formatted_3args_impl(const std::vector<boost_kahypar::int32_t>& v)
{
...
void test_formatted_impl(const std::vector<boost_kahypar::int32_t>& v)
{
...
{
    std::vector<boost_kahypar::int32_t> v;
    for (boost_kahypar::int32_t i = 0; i < 5; ++i)
        v.push_back(i);
...
void test_formatted5()
{
    ...
    std::ostringstream out1;
    out1 << (v | boost_kahypar::adaptors::formatted(udt_separator()));
    BOOST_CHECK_EQUAL(out1.str(), "{0[sep]1[sep]2[sep]3[sep]4}");

    std::ostringstream out2;
    out2 << boost_kahypar::adaptors::format(v, udt_separator());
...
void test_formatted_empty()
{
    ...
    out5 << (v | boost_kahypar::adaptors::formatted("#$"));
    BOOST_CHECK_EQUAL(out5.str(), "{}");

    std::ostringstream out6;
    out6 << boost_kahypar::adaptors::format(v, "#$");
    BOOST_CHECK_EQUAL(out6.str(), "{}");

    std::ostringstream out7;
    out7 << (v | boost_kahypar::adaptors::formatted("", "12", "34"));
    BOOST_CHECK_EQUAL(out7.str(), "1234");

    std::ostringstream out8;
    out8 << boost_kahypar::adaptors::format(v, "", "12", "34");
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite(int, char*[] )
{
    ...
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted1));
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted2));
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted3));
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted4));
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted5));

    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_formatted_empty));
