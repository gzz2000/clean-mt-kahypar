...
//
#include <boost_kahypar/range/adaptor/replaced.hpp>
...
namespace boost_kahypar
{
    namespace
    {
        ...
        void replaced_test_impl( Container& c )
        {
            ...
            boost_kahypar::push_back(test_result1, c | replaced(value_to_replace, replacement_value));

            std::vector< int > test_result2;
            boost_kahypar::push_back(test_result2, adaptors::replace(c, value_to_replace, replacement_value));
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::replaced_test ) );
