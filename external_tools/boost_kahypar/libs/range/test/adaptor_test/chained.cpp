...
#include <boost_kahypar/range/adaptor/filtered.hpp>
...
private:
    boost_kahypar::uint64_t m_value;
    bool m_valid;
};
...
void chained_adaptors_test()
{
    ...
    boost_kahypar::copy(sep
        | boost_kahypar::adaptors::transformed(boost_kahypar::bind(&foo::from_string, _1))
        | boost_kahypar::adaptors::filtered(boost_kahypar::bind(&foo::is_valid, _1)),
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add(BOOST_TEST_CASE( boost_kahypar_range_test::chained_adaptors_test));
