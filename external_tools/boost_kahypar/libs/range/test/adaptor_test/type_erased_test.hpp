...
public:
    ...
    MockType(boost_kahypar::int32_t x)
        : m_x(x)
    {
    }

    boost_kahypar::int32_t get() const { return m_x; }
...
public:
    MockType2() {}
    MockType2(boost_kahypar::int32_t x) : MockType(x) { }
...
void test_type_erased_impl(Container& c)
{
    ...
    typedef typename boost_kahypar::adaptors::type_erased<> type_erased_t;


    std::vector<value_type> output;

    boost_kahypar::push_back(output, boost_kahypar::adaptors::type_erase(c, type_erased_t()));

    BOOST_CHECK_EQUAL_COLLECTIONS( output.begin(), output.end(),
                                   c.begin(), c.end() );

    output.clear();
    boost_kahypar::push_back(output, c | type_erased_t());
...
template<
  ...
  , class Container
>
void test_writeable(Container&, boost_kahypar::single_pass_traversal_tag)
{}
...
template<
  ...
  , class Container
>
void test_writeable(Container& source, boost_kahypar::forward_traversal_tag)
{
    using namespace boost_kahypar::adaptors;

    typedef typename boost_kahypar::range_value<Container>::type value_type;
    typedef typename boost_kahypar::range_difference<Container>::type difference_type;
    typedef typename boost_kahypar::range_reference<Container>::type mutable_reference_type;
    ...
    mutable_any_range r = source | boost_kahypar::adaptors::type_erased<>();
    std::vector<value_type> output_test;
    boost_kahypar::fill(r, value_type(1));
    BOOST_CHECK_EQUAL( boost_kahypar::distance(r), boost_kahypar::distance(source) );
...
void test_type_erased_impl()
{
    ...
    typedef typename boost_kahypar::any_range_type_generator<
        Container
    ...
    typedef boost_kahypar::adaptors::type_erased<
                boost_kahypar::use_default
              , Traversal
              , boost_kahypar::use_default
              , boost_kahypar::use_default
    ...
    r = boost_kahypar::adaptors::type_erase(source, type_erased_t());
    BOOST_CHECK_EQUAL_COLLECTIONS( source.begin(), source.end(),
                                   r.begin(), r.end() );
    r = mutable_any_range();
    ...
    // source range
    cr = const_any_range();
    cr = boost_kahypar::adaptors::type_erase(const_source, type_erased_t());
    BOOST_CHECK_EQUAL_COLLECTIONS( const_source.begin(), const_source.end(),
                                   cr.begin(), cr.end() );
...
{
    using boost_kahypar::any_iterator_default_buffer;
    using boost_kahypar::any_iterator_buffer;
    using boost_kahypar::any_iterator_heap_only_buffer;
    using boost_kahypar::any_iterator_stack_only_buffer;
