...
//
#include <boost_kahypar/range/adaptor/map.hpp>
...
namespace boost_kahypar
{
    namespace
    {
        ...
        void map_test_keys( Container& c )
        {
            ...
            std::vector<int> keys;
            boost_kahypar::push_back(keys, c | map_keys);

            std::vector<int> keys2;
            boost_kahypar::push_back(keys2, adaptors::keys(c));
        ...
        void map_test_values( Container& c )
        {
            ...
            boost_kahypar::push_back(values, c | map_values);

            std::vector<int> values2;
            boost_kahypar::push_back(values2, adaptors::values(c));
        ...
        void test_trac_item_4388()
        {
            ...
            const boost_kahypar::array<pair_t,3> ar = {{
                pair_t(3, 'a'),
                pair_t(1, 'b'),
                pair_t(4, 'c')
            }};

            const boost_kahypar::array<int, 3> expected_keys = {{ 3, 1, 4 }};
            const boost_kahypar::array<char, 3> expected_values = {{ 'a', 'b', 'c' }};

            {
                std::vector<int> test;
                boost_kahypar::push_back(test, ar | boost_kahypar::adaptors::map_keys);
...

            {
                ...
                boost_kahypar::push_back(test, ar | boost_kahypar::adaptors::map_values);
                BOOST_CHECK_EQUAL_COLLECTIONS(
            ...
            {
                std::vector<char> test;
                boost_kahypar::array<std::pair<int, char>, 3> src(ar);
                boost_kahypar::push_back(test, src | boost_kahypar::adaptors::map_values);
                BOOST_CHECK_EQUAL_COLLECTIONS(
                    expected_values.begin(), expected_values.end(),
                    test.begin(), test.end()
                );
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::map_test ) );
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_trac_item_4388 ) );
