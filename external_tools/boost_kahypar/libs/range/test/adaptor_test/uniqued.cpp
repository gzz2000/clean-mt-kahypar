...
//
#include <boost_kahypar/range/adaptor/uniqued.hpp>
...
#include <boost_kahypar/algorithm/string/predicate.hpp>
...
namespace boost_kahypar
{
    namespace
    {
        ...
        void uniqued_test_impl( Container& c )
        {
            ...
            boost_kahypar::push_back(test_result1, c | uniqued);

            std::vector< int > test_result2;
            boost_kahypar::push_back(test_result2, adaptors::unique(c));
...
public:
    ...
    {
        return boost_kahypar::iequals(m_value, r.m_value);
    }
...
void uniqued_return_first()
{
    ...
    boost_kahypar::unique_copy(strs, std::back_inserter(output1));

    std::vector<istring> output2;
    boost_kahypar::push_back(output2, strs | uniqued);

    std::vector<std::string> test1;
    boost_kahypar::push_back(test1, output1 | transformed(istring_to_string()));

    std::vector<std::string> test2;
    boost_kahypar::push_back(test2, output2 | transformed(istring_to_string()));
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::uniqued_test ) );

    test->add(BOOST_TEST_CASE(&boost_kahypar::uniqued_return_first));
