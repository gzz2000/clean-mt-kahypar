...
void check_direct()
{
    ...
    Rng rng = boost_kahypar::assign::list_of(1)(2)(3)(4)(5).to_container( rng );
    Rng out;
    ...
    BOOST_CHECK_EQUAL( *boost_kahypar::next(out.begin()), 4 );
    BOOST_CHECK_EQUAL( *boost_kahypar::next(out.begin(),2), 6 );

    rng = boost_kahypar::assign::list_of(1)(1)(2)(2)(3)(3)(4)(5).to_container( rng );
    out.clear();
    /*
...
void check_indirect()
{
    ...
    std::vector< boost_kahypar::shared_ptr< int > > holder;
    
    for( unsigned i = 0u; i != 20u; ++i )
    {
        boost_kahypar::shared_ptr<int> v(new int(i));
        rng.push_back( v.get() );
    }
...
void check_map()
{
    ...
    std::vector<int> keys 
        = boost_kahypar::copy_range< std::vector<int> >( m | map_keys );
    std::vector<int> values 
       = boost_kahypar::copy_range< std::vector<int> >( m | map_values );
}
...
void check_regex()
{
    ...
    std::vector<std::string> tokens =
        boost_kahypar::copy_range< std::vector<std::string> >( s | tokenized( "\\s+", -1 ) );
}
