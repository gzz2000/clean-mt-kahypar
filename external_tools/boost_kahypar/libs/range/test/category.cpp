...
void test_category()
{
    ...
    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            boost_kahypar::iterator_category<cont::const_iterator>::type,
            boost_kahypar::range_category<cont>::type
        >::value));

    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            boost_kahypar::iterator_category<cont::const_iterator>::type,
            boost_kahypar::range_category<const cont>::type
        >::value));
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] )
{
    ...
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_category));
