...
class single_pass_iterator
        : public boost_kahypar::iterator_facade<
            ...
            const boost_kahypar::int32_t&
        >
{
    friend class boost_kahypar::iterator_core_access;

    typedef std::vector<boost_kahypar::int32_t>::const_iterator iterator_t;
...
boost_kahypar::iterator_range<single_pass_iterator>
single_pass_range(const SinglePassRange& rng)
{
    return boost_kahypar::iterator_range<single_pass_iterator>(
                single_pass_iterator(boost_kahypar::begin(rng)),
                single_pass_iterator(boost_kahypar::end(rng)));
}

template<typename BidirectionalRange>
boost_kahypar::iterator_range<bidirectional_iterator>
bidirectional_range(const BidirectionalRange& rng)
{
    return boost_kahypar::iterator_range<bidirectional_iterator>(
                bidirectional_iterator(boost_kahypar::begin(rng)),
                bidirectional_iterator(boost_kahypar::end(rng)));
}

void test_drop_front()
{
    std::vector<boost_kahypar::int32_t> v;
    std::vector<boost_kahypar::int32_t> ref_output;
    ...
    boost_kahypar::iterator_range<single_pass_iterator> rng = single_pass_range(v);
...
void test_drop_back()
{
    ...
    boost_kahypar::iterator_range<bidirectional_iterator> rng = bidirectional_range(v);
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] )
{
    ...
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_drop_front));
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_drop_back));
