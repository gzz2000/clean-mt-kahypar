...
#define BOOST_LIB_NAME boost_kahypar_test_exec_monitor
#include <boost_kahypar/config/auto_link.hpp>

#define BOOST_RANGE_DETAIL_MICROSOFT_TEST
#include <boost_kahypar/range/atl.hpp> // can be placed first
...
namespace brdm = boost_kahypar::range_detail_microsoft;
...
{
    typedef typename boost_kahypar::range_iterator<SampleRange>::type iter_t;

    for (iter_t it = boost_kahypar::begin(sample), last = boost_kahypar::end(sample); it != last; ++it) {
        arr.Add(*it); // moves ownership
    }

    return boost_kahypar::distance(arr) == boost_kahypar::distance(sample);
}
...
{
    typedef typename boost_kahypar::range_iterator<SampleRange>::type iter_t;
    typedef typename boost_kahypar::range_value<SampleRange>::type val_t;

    for (iter_t it = boost_kahypar::begin(sample), last = boost_kahypar::end(sample); it != last; ++it) {
        lst.AddTail(*it); // moves ownership
    }

    return boost_kahypar::distance(lst) == boost_kahypar::distance(sample);
}
...
{
    typedef typename boost_kahypar::range_iterator<AtlMapT>::type iter_t;

    for (iter_t it = boost_kahypar::begin(map), last = boost_kahypar::end(map); it != last; ++it) {
...
{
    typedef typename boost_kahypar::range_iterator<AtlMapT>::type iter_t;
    typedef typename boost_kahypar::range_const_iterator<MapT>::type siter_t;

    bool result = true;

    result = result && (boost_kahypar::distance(map) == boost_kahypar::distance(sample));
    if (!result)
        return false;
    ...
    {
        for (siter_t it = boost_kahypar::begin(sample), last = boost_kahypar::end(sample); it != last; ++it) {
...
{
    typedef typename boost_kahypar::range_const_iterator<SampleMap>::type iter_t;
...
{
    typedef typename boost_kahypar::range_value<Range>::type val_t;
...
void test_CAutoPtrArray(Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter<rng_t, boost_kahypar::indirect_iterator< ATL::CAutoPtr<val_t> *> >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  <rng_t, boost_kahypar::indirect_iterator< ATL::CAutoPtr<val_t> const*> >::value ));
...
void test_CAutoPtrList(Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter< rng_t, boost_kahypar::indirect_iterator< brdm::list_iterator<rng_t,       ATL::CAutoPtr<val_t> > > >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  < rng_t, boost_kahypar::indirect_iterator< brdm::list_iterator<rng_t const, ATL::CAutoPtr<val_t> const> > >::value ));
...
void test_CHeapPtrList(const Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter< rng_t, boost_kahypar::indirect_iterator< brdm::list_iterator<rng_t,       ATL::CHeapPtr<val_t> > > >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  < rng_t, boost_kahypar::indirect_iterator< brdm::list_iterator<rng_t const, ATL::CHeapPtr<val_t> const> > >::value ));
...
void test_CSimpleStringT(const Range& sample)
{
    ...
    typedef typename boost_kahypar::mpl::if_< boost_kahypar::is_same<val_t, char>,
        ATL::CAtlStringA,
        ATL::CAtlStringW
    >::type derived_t;
...
void test_CStaticString(const Range& sample)
{
#if !defined(BOOST_RANGE_ATL_NO_TEST_UNDOCUMENTED_RANGE)
    {
        ...
        BOOST_CHECK( *(boost_kahypar::begin(rng)+4) == 'o' );
        BOOST_CHECK( *(boost_kahypar::end(rng)-3) == 'i' );
    }

    {
        ...
        BOOST_CHECK( *(boost_kahypar::begin(rng)+4) == L'o' );
        BOOST_CHECK( *(boost_kahypar::end(rng)-3) == L'i' );
    }
#endif
...
void test_CSimpleMap(const Range& sample)
{
    ...
    BOOST_CHECK( boost_kahypar::begin(rng)->get<0>() == 3.0 );
    BOOST_CHECK( (boost_kahypar::end(rng)-1)->get<1>() == 2.0 );
...
void test_CAtlMap(const MapT& sample)
{
    ...
    rng_t rng;
    boost_kahypar::function_requires< boost_kahypar::ForwardRangeConcept<rng_t> >();
...
void test_CRBTree(const MapT& sample)
{
    ...
    boost_kahypar::function_requires< boost_kahypar::BidirectionalRangeConcept<rng_t> >();
...
void test_atl()
{
...
#if !(_ATL_VER < 0x0700)
    {
        ...
        boost_kahypar::iterator_range< ptr_t * > workaround(ptrs, ptrs+8);
...

    {
        ...
        boost_kahypar::iterator_range< ptr_t * > workaround(ptrs, ptrs+5);
