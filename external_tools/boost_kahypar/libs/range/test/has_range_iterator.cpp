...
namespace
{
    ...
    {
        BOOST_CHECK_EQUAL( boost_kahypar::has_range_iterator<Container>::value, expected_value );
    }

    template<typename Container>
    void test_has_range_const_iterator_impl(const bool expected_value)
    {
        BOOST_CHECK_EQUAL( boost_kahypar::has_range_const_iterator<Container>::value, expected_value );
    }
