...
namespace
{
    ...
    inline typename boost_kahypar::range_iterator<Range>::type
    find( Range& c, const T& value )
    {
       return std::find( boost_kahypar::begin( c ), boost_kahypar::end( c ), value );
    }
    ...
    inline typename boost_kahypar::range_difference<Range>::type
    my_generic_replace( Range& c, const T& value, const T& replacement )
    {
       typename boost_kahypar::range_iterator<Range>::type found = find( c, value );
       
       if( found != boost_kahypar::end( c ) )
           *found = replacement;
       return std::distance( boost_kahypar::begin( c ), found );
    }                  
}
...
void check_algorithm()
{
    ...
    std::pair<iterator,iterator>       my_view( boost_kahypar::begin( my_vector ), 
                                                boost_kahypar::begin( my_vector ) + N );
