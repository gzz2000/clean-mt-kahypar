...
#include <boost_kahypar/range/sub_range.hpp>
...
void check_sub_range()
{
    ...
    typedef boost_kahypar::sub_range<std::string>       srange;
    typedef boost_kahypar::sub_range<const std::string> csrange;
    srange s     = r;
    BOOST_CHECK( r == r );
    BOOST_CHECK( s == r );
    s            = boost_kahypar::make_iterator_range( str );
    csrange s2   = r;
    s2           = r2;
    s2           = boost_kahypar::make_iterator_range( cstr );
    BOOST_CHECK( r2 == r2 );
    BOOST_CHECK( s2 != r2 );
    s2           = boost_kahypar::make_iterator_range( str );
    BOOST_CHECK( !(s != s) );
...
//#endif
    ...
    srange rr = boost_kahypar::make_iterator_range( str );
    ...
    rrr.back()  = 'D';
    rrr[1]      = 'E';
    BOOST_CHECK( rrr == boost_kahypar::as_literal("HEllo worlD") );
}

template<class T>
void check_mutable_type(T&)
{
    BOOST_STATIC_ASSERT(!boost_kahypar::is_const<T>::value);
}
...
void check_is_const_iterator(Iterator it)
{
    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<
                BOOST_DEDUCED_TYPENAME boost_kahypar::add_const<Range>::type
            >::type,
            Iterator
        >::value));
}
...
void check_is_iterator(Iterator it)
{
    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<
                BOOST_DEDUCED_TYPENAME boost_kahypar::remove_const<Range>::type
            >::type,
            Iterator
        >::value));
}
...
void const_propagation_mutable_collection(void)
{
    ...
    typedef boost_kahypar::sub_range<coll_t> sub_range_t;
...
{
    typedef std::vector<int> coll_t;
    typedef boost_kahypar::sub_range<const coll_t> sub_range_t;
...
inline void test_advance()
{
    ...
    l.push_back(1);
    l.push_back(2);
    typedef boost_kahypar::sub_range<std::vector<int> > rng_t;
...
void ticket_10514()
{
    ...
    typedef boost_kahypar::sub_range<vec_t> range_t;
    vec_t v(10);
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite(int, char*[])
{
    boost_kahypar::unit_test::test_suite* test =
            BOOST_TEST_SUITE( "Boost.Range sub_range test suite" );

    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::check_sub_range));

    test->add(BOOST_TEST_CASE(
                  &boost_kahypar_range_test::const_propagation_const_collection));

    test->add(BOOST_TEST_CASE(
                  &boost_kahypar_range_test::const_propagation_mutable_collection));

    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_advance));

    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::ticket_10514));
