...
//#include <boost_kahypar/range/as_array.hpp>
...
void check_iterator_range()
{
    ...
    typedef boost_kahypar::iterator_range<iterator>       irange;
    typedef boost_kahypar::iterator_range<const_iterator> cirange;
    std::string       str  = "hello world";
    const std::string cstr = "const world";
    irange r    = boost_kahypar::make_iterator_range( str );
    r           = boost_kahypar::make_iterator_range( str.begin(), str.end() );
    cirange r2  = boost_kahypar::make_iterator_range( cstr );
    r2          = boost_kahypar::make_iterator_range( cstr.begin(), cstr.end() );
    r2          = boost_kahypar::make_iterator_range( str );
...
//#endif
    ...
    BOOST_CHECK_EQUAL( r.size(), boost_kahypar::size( r ) );
    BOOST_CHECK_EQUAL( r2.size(), boost_kahypar::size( r2 ) );

    BOOST_CHECK_EQUAL( std::distance( r.begin(), r.end() ),
                       std::distance( boost_kahypar::begin( r2 ), boost_kahypar::end( r2 ) ) );
    std::cout << r << r2;


#ifndef BOOST_NO_STD_WSTRING
    std::wcout << boost_kahypar::make_iterator_range( std::wstring( L"a wide string" ) )
               << boost_kahypar::make_iterator_range( L"another wide string" );
#endif

    std::string res  = boost_kahypar::copy_range<std::string>( r );
    BOOST_CHECK_EQUAL_COLLECTIONS( res.begin(), res.end(), r.begin(), r.end() );

    irange rr = boost_kahypar::make_iterator_range( str );
    BOOST_CHECK( rr.equal( r ) );

    rr  = boost_kahypar::make_iterator_range( str.begin(), str.begin() + 5 );
    BOOST_CHECK( rr == boost_kahypar::as_literal("hello") );
    BOOST_CHECK( rr != boost_kahypar::as_literal("hell") );
    BOOST_CHECK( rr < boost_kahypar::as_literal("hello dude") );
    BOOST_CHECK( boost_kahypar::as_literal("hello") == rr );
    BOOST_CHECK( boost_kahypar::as_literal("hell")  != rr );
    BOOST_CHECK( ! (boost_kahypar::as_literal("hello dude") < rr ) );
    irange rrr = rr;
    BOOST_CHECK( rrr == rr );
    BOOST_CHECK( !( rrr != rr ) );
    BOOST_CHECK( !( rrr < rr ) );

    const irange cr = boost_kahypar::make_iterator_range( str );
    BOOST_CHECK_EQUAL( cr.front(), 'h' );
    BOOST_CHECK_EQUAL( cr.back(), 'd' );
    BOOST_CHECK_EQUAL( cr[1], 'e' );
    BOOST_CHECK_EQUAL( cr(1), 'e' );

    rrr = boost_kahypar::make_iterator_range( str, 1, -1 );
    BOOST_CHECK( rrr == boost_kahypar::as_literal("ello worl") );
    rrr = boost_kahypar::make_iterator_range( rrr, -1, 1 );
    BOOST_CHECK( rrr == str );
    ...
    int arr[2][2];
    boost_kahypar::make_iterator_range(arr, arr + 2);
}
...
namespace iterator_range_test_detail
{
    ...
    void check_iterator_range_operators_impl(Pred pred)
    {
        ...
        typedef boost_kahypar::iterator_range<citer> iter_range;
        ...
        for (value_const_iterator left_it = first_val; left_it != last_val; ++left_it)
        {
            ...
            for (value_const_iterator right_it = first_val; right_it != last_val; ++right_it)
            {
                ...
                iter_range left = boost_kahypar::make_iterator_range(leftValue);
                iter_range right = boost_kahypar::make_iterator_range(rightValue);
    ...
    void check_iterator_range_from_array()
    {
        ...
        boost_kahypar::iterator_range<double*> rng = boost_kahypar::make_iterator_range(source);
    ...
    {
        using boost_kahypar::uint32_t;

        std::vector<uint32_t> input;
        for (uint32_t i = 0; i < 10u; ++i)
            input.push_back(i);

        boost_kahypar::iterator_range<std::vector<uint32_t>::iterator> rng =
                boost_kahypar::make_iterator_range_n(boost_kahypar::begin(input), 8u);
        ...
        boost_kahypar::iterator_range<std::vector<uint32_t>::const_iterator> crng =
                boost_kahypar::make_iterator_range_n(boost_kahypar::begin(cinput), 8u);
...
inline void test_advance()
{
    ...
    l.push_back(1);
    l.push_back(2);
    typedef boost_kahypar::iterator_range<std::vector<int>::iterator> rng_t;
...
struct ptr_iterator
  : boost_kahypar::iterator_adaptor<ptr_iterator, int *>
{
    ptr_iterator() {}
    ptr_iterator(int *p) : boost_kahypar::iterator_adaptor<ptr_iterator, int *>(p) {}
private:
...
void test_sfinae()
{
    boost_kahypar::iterator_range<ptr_iterator> r(ptr_iterator(0), ptr_iterator(0));
}
...
{
    typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<Container>::type citer_type;
    typedef boost_kahypar::iterator_range<citer_type> riter_type;
    riter_type a_riter( boost_kahypar::make_iterator_range( a_cont ) );
    a_riter.front();
    a_riter.back();
    int i = a_riter[0];
    return i;
