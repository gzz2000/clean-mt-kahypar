...
void test_mutable_iterator()
{
    ...
    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            cont::iterator,
            boost_kahypar::range_mutable_iterator<cont>::type
        >::value));

    BOOST_STATIC_ASSERT((
        boost_kahypar::is_same<
            cont::iterator,
            boost_kahypar::range_mutable_iterator<const cont>::type
        >::value));
...
boost_kahypar::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] )
{
    ...
    test->add(BOOST_TEST_CASE(&boost_kahypar_range_test::test_mutable_iterator));
