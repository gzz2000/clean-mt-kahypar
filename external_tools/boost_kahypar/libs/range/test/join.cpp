...
namespace boost_kahypar
{
    namespace
    {
        ...
        void fill_with_ints(Range rng)
        {
            ...
            iterator target = boost_kahypar::begin(rng);
            const int count = boost_kahypar::distance(rng);
        ...
        void test_join_traversal(Range1& rng1, Range2& rng2, boost_kahypar::bidirectional_traversal_tag)
        {
            typedef typename range_value<Range1>::type value_type;
            std::vector<value_type> reference(boost_kahypar::begin(rng1), boost_kahypar::end(rng1));
            boost_kahypar::push_back(reference, rng2);
        ...
        {
            BOOST_CHECK_EQUAL( boost_kahypar::end(joined) - boost_kahypar::begin(joined), boost_kahypar::distance(joined) );
            BOOST_CHECK( boost_kahypar::end(joined) <= boost_kahypar::begin(joined) );
            BOOST_CHECK( boost_kahypar::begin(joined) >= boost_kahypar::end(joined) );
            if (boost_kahypar::empty(joined))
            {
                BOOST_CHECK(!(boost_kahypar::begin(joined) < boost_kahypar::end(joined)));
                BOOST_CHECK(!(boost_kahypar::end(joined) > boost_kahypar::begin(joined)));
            }
            ...
            typedef typename boost_kahypar::range_difference<JoinedRange>::type difference_t;
            const difference_t count = boost_kahypar::distance(joined);
            BOOST_CHECK( boost_kahypar::begin(joined) + count == boost_kahypar::end(joined) );
            BOOST_CHECK( boost_kahypar::end(joined) - count == boost_kahypar::begin(joined) );

            typedef typename boost_kahypar::range_iterator<JoinedRange>::type iterator_t;
            iterator_t it = boost_kahypar::begin(joined);
            it += count;
            BOOST_CHECK( it == boost_kahypar::end(joined) );

            it = boost_kahypar::end(joined);
            it -= count;
            BOOST_CHECK( it == boost_kahypar::begin(joined) );
        }

        // test_join_traversal function for random access traversal joined
        // ranges.
        template< typename Range1, typename Range2 >
        void test_join_traversal(Range1& rng1, Range2& rng2, boost_kahypar::random_access_traversal_tag)
        {
            test_join_traversal(rng1, rng2, boost_kahypar::bidirectional_traversal_tag());
        ...
        void test_write_to_joined_range(const Collection1& rng1, const Collection2& rng2)
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_value<
                Collection1
            >::type value_t BOOST_RANGE_UNUSED;
            
            fill_with_ints(boost_kahypar::join(c1,c2));

            // Ensure that the size of the written range has not been
            // altered.
            BOOST_CHECK_EQUAL( boost_kahypar::distance(c1), boost_kahypar::distance(rng1) );
            BOOST_CHECK_EQUAL( boost_kahypar::distance(c2), boost_kahypar::distance(rng2) );
            ...
            iterator1 it1 = boost_kahypar::begin(c1);
            for (; it1 != boost_kahypar::end(c1); ++it1)
            {
            ...
            iterator2 it2 = boost_kahypar::begin(c2);
            for (; it2 != boost_kahypar::end(c2); ++it2)
            {
        ...
        void test_join_impl(Collection1& rng1, Collection2& rng2)
        {
            ...
            std::vector<value_type> test_result;
            boost_kahypar::push_back(test_result, join(rng1, rng2));

            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           test_result.begin(), test_result.end() );

            typedef boost_kahypar::range_detail::join_iterator<
                typename boost_kahypar::range_iterator<Collection1>::type,
                typename boost_kahypar::range_iterator<Collection2>::type
                > join_iterator_t;

            typedef boost_kahypar::iterator_traversal< join_iterator_t > tag_t;
        ...
        boost_kahypar::shared_ptr<Collection> makeCollection(const Range& source)
        {
            boost_kahypar::shared_ptr<Collection> c(new Collection);
            c->insert(c->end(), boost_kahypar::begin(source), boost_kahypar::end(source));
            return c;
        }
        ...
        void test_join_impl()
        {
            typedef boost_kahypar::shared_ptr<Collection1> collection1_ptr;
            typedef boost_kahypar::shared_ptr<Collection2> collection2_ptr;
            typedef boost_kahypar::shared_ptr<const Collection1> collection1_cptr;
            typedef boost_kahypar::shared_ptr<const Collection2> collection2_cptr;
        ...
        void test_join_iterator_reference_type_constness_ticket8483()
        {
            ...
            boost_kahypar::push_back(joined, join(v1, v2));
            boost_kahypar::push_back(joined, join(v2, v1));
        }
        ...
        namespace trac7376
        {
            struct base_type
            {
                explicit base_type(boost_kahypar::int32_t value)
                    : value(value)
                {
                }

                virtual boost_kahypar::int32_t get() const = 0;

                boost_kahypar::int32_t value;
            };

            struct derived_type1
                : base_type
            {
                derived_type1(boost_kahypar::int32_t value)
                    : base_type(value)
                {
                }

                virtual boost_kahypar::int32_t get() const
                {
                    return value * 2;
                }
            };
            ...
            struct derived_type2
                : base_type
            {
                derived_type2(boost_kahypar::int32_t value)
                    : base_type(value)
                {
                }
            ...
            void test_reference_types()
            {
                ...
                std::vector<boost_kahypar::int32_t> reference_output;
                ...
                join_iterator_t it(
                    x,
                    y,
                    boost_kahypar::range_detail::join_iterator_begin_tag());

                std::vector<boost_kahypar::int32_t> output;
                boost_kahypar::push_back(
                    output,
                    boost_kahypar::make_iterator_range(
                        join_iterator_t(
                            x, y,
                            boost_kahypar::range_detail::join_iterator_begin_tag()),
                        join_iterator_t(
                            x, y,
                            boost_kahypar::range_detail::join_iterator_end_tag()))
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::join_test ) );
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_join_iterator_reference_type_constness_ticket8483 ) );
    test->add( BOOST_TEST_CASE( &boost_kahypar::trac7376::test_reference_types ) );
