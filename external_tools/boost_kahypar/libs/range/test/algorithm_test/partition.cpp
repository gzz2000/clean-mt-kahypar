...
namespace boost_kahypar_range_test_algorithm_partition
{
    struct equal_to_5
    {
    ...
    public:
        ...
        test_iter(Container& cont)
        {
            ...
            iter_t result = boost_kahypar::partition(cont, UnaryPredicate());

            boost_kahypar::partition(cont2, UnaryPredicate());
            cont2 = old_cont;
            boost_kahypar::partition(
        ...
        struct test_range
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                const Container old_cont(cont);
                Container cont2(old_cont);
                result_t result = boost_kahypar::partition<return_type>(cont, policy.pred());

                // Test that operation a temporary created by using
                // make_iterator_range.
                boost_kahypar::partition<return_type>(
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_partition::test_partition ) );
