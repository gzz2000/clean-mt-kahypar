...
namespace boost_kahypar
{
    namespace
    {
        ...
        void test_transform_impl1(Container& cont)
        {
            ...
            boost_kahypar::ignore_unused_variable_warning(reference_it);

            iterator_t test_it
                = boost_kahypar::transform(cont, target.begin(), fn);

            BOOST_CHECK( test_it == target.end() );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                
            BOOST_CHECK( test_it == boost_kahypar::transform(boost_kahypar::make_iterator_range(cont), target.begin(), fn) );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
            ...
            test_it = boost_kahypar::transform(ccont, target.begin(), fn);
            
            BOOST_CHECK( test_it == target.end() );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
            BOOST_CHECK( test_it == boost_kahypar::transform(boost_kahypar::make_iterator_range(ccont), target.begin(), fn) );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
        }
        ...
        void test_transform_impl2(Container1& cont1, Container2& cont2)
        {
            ...
            iterator_t test_it
                = boost_kahypar::transform(cont1, cont2, target.begin(), fn);
                
            BOOST_CHECK( test_it == target.end() );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                                           
            BOOST_CHECK( test_it == boost_kahypar::transform(boost_kahypar::make_iterator_range(cont1), cont2, target.begin(), fn) );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                                           
            BOOST_CHECK( test_it == boost_kahypar::transform(cont1, boost_kahypar::make_iterator_range(cont2), target.begin(), fn) );
            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                                           
            BOOST_CHECK( test_it == boost_kahypar::transform(boost_kahypar::make_iterator_range(cont1),
            ...
            test_it = boost_kahypar::transform(ccont1, ccont2, target.begin(), fn);
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_transform ) );
