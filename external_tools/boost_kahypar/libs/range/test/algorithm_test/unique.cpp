...
namespace boost_kahypar_range_test_algorithm_unique
{
    ...
    public:
        ...
        {
            // There isn't an iterator return version of boost_kahypar::unique, so just
        ...
        struct test_range
        {
            ...
            operator()(Policy&, Container& cont)
            {
                ...
                result_t result = boost_kahypar::unique<return_type>(cont);

                boost_kahypar::unique<return_type>(boost_kahypar::make_iterator_range(cont2));
        ...
        struct test_range_overload
        {
            BOOST_STATIC_CONSTANT(
                ::boost_kahypar::range_return_value,
                result_type = ::boost_kahypar::return_begin_found);
                
            template<typename Policy>
            BOOST_DEDUCED_TYPENAME boost_kahypar::range_return<
                Container, result_type
            >::type
            operator()(Policy& policy, Container& cont)
            {
                typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_return<
                                Container,result_type>::type result_t;
                
                Container cont2(cont);
                
                result_t result = boost_kahypar::unique(cont);
                
                boost_kahypar::unique(boost_kahypar::make_iterator_range(cont2));
    ...
    public:
        ...
        struct test_range
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                result_t result = boost_kahypar::unique<return_type>(cont, policy.pred());

                boost_kahypar::unique<return_type>(boost_kahypar::make_iterator_range(cont2), policy.pred());
        ...
        struct test_range_overload
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                result_t result = boost_kahypar::unique(cont, policy.pred());
                
                boost_kahypar::unique(boost_kahypar::make_iterator_range(cont2), policy.pred());
    ...
    void test_unique_impl(TestPolicy policy, Pred pred)
    {
        ...
        boost_kahypar::range_test::range_overload_test_driver test_driver;
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_unique::test_unique ) );
