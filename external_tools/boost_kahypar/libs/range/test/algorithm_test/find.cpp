...
namespace boost_kahypar_range_test_algorithm_find
{
    ...
    public:
        ...
        test_iter(Container& cont)
        {
            ...
            iter_t result = boost_kahypar::find(cont, 3);
            iter_t result2 = boost_kahypar::find(boost_kahypar::make_iterator_range(cont), 3);
        ...
        struct test_range
        {
            ...
            operator()(Policy&, Container& cont)
            {
                ...
                result_t result = boost_kahypar::find<return_type>(cont, 3);
                result_t result2 = boost_kahypar::find<return_type>(boost_kahypar::make_iterator_range(cont), 3);
                BOOST_CHECK( result == result2 );
                return result;
            }
        };
    ...
    void test_find()
    {
        ...
        std::vector<int>::const_iterator it = boost_kahypar::find(vi, 0);
        std::vector<int>::const_iterator it2 = boost_kahypar::find(cvi, 0);
        BOOST_CHECK( it == it2 );
    }
    ...
    // use the syntax boost_kahypar::find<boost_kahypar::return_found_end>(rng, x)
    ...
    void test_find_as_contains()
    {
        ...
        BOOST_CHECK(boost_kahypar::find<boost_kahypar::return_found_end>(l, 3));
        BOOST_CHECK(!boost_kahypar::find<boost_kahypar::return_found_end>(l, 10));
    }
}
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_find::test_find ) );
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_find::test_find_as_contains ) );
