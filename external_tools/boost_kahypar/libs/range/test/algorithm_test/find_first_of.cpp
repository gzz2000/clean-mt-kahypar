...
namespace boost_kahypar_range_test_algorithm_find_first_of
{
    ...
    public:
        ...
        test_iter(Container& cont)
        {
            ...
            iter_t result = boost_kahypar::find_first_of(cont, m_cont);
            BOOST_CHECK( result == boost_kahypar::find_first_of(boost_kahypar::make_iterator_range(cont), m_cont) );
            BOOST_CHECK( result == boost_kahypar::find_first_of(cont, boost_kahypar::make_iterator_range(m_cont)) );
            BOOST_CHECK( result == boost_kahypar::find_first_of(boost_kahypar::make_iterator_range(cont), boost_kahypar::make_iterator_range(m_cont)) );
            return result;
        }
        ...
        struct test_range
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                result_t result = boost_kahypar::find_first_of<return_type>(cont, policy.cont());
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(boost_kahypar::make_iterator_range(cont), policy.cont()) );
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(cont, boost_kahypar::make_iterator_range(policy.cont())) );
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(boost_kahypar::make_iterator_range(cont), boost_kahypar::make_iterator_range(policy.cont())) );
                return result;
            }
        };
    ...
    public:
        ...
        {
            typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<Container>::type iter_t;
            iter_t result = boost_kahypar::find_first_of(cont, m_cont, m_pred);
            BOOST_CHECK( result == boost_kahypar::find_first_of(boost_kahypar::make_iterator_range(cont), m_cont, m_pred) );
            BOOST_CHECK( result == boost_kahypar::find_first_of(cont, boost_kahypar::make_iterator_range(m_cont), m_pred) );
            BOOST_CHECK( result == boost_kahypar::find_first_of(boost_kahypar::make_iterator_range(cont), boost_kahypar::make_iterator_range(m_cont), m_pred) );
            return result;
        }
        ...
        struct test_range
        {
            ...
            {
                typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_return<Container,return_type>::type result_t;
                result_t result = boost_kahypar::find_first_of<return_type>(cont, policy.cont(), policy.pred());
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(boost_kahypar::make_iterator_range(cont), policy.cont(), policy.pred()) );
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(cont, boost_kahypar::make_iterator_range(policy.cont()), policy.pred()) );
                BOOST_CHECK( result == boost_kahypar::find_first_of<return_type>(boost_kahypar::make_iterator_range(cont), boost_kahypar::make_iterator_range(policy.cont()), policy.pred()) );
                return result;
            }
        };
    ...
    void test_find_first_of_impl()
    {
        ...
        typedef BOOST_DEDUCED_TYPENAME boost_kahypar::remove_const<Container1>::type container1_t;
        typedef BOOST_DEDUCED_TYPENAME boost_kahypar::remove_const<Container2>::type container2_t;
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_find_first_of::test_find_first_of ) );
