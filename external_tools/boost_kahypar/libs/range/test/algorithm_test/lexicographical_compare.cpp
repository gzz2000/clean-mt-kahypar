...
namespace boost_kahypar
{
    namespace
    {
                                                      ...
                                                      ForwardRange2& rng2)
        {
            ...
            const bool test = boost_kahypar::lexicographical_compare(rng1, rng2);

            BOOST_CHECK( reference == test );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(boost_kahypar::make_iterator_range(rng1), rng2) );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(rng1, boost_kahypar::make_iterator_range(rng2)) );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(boost_kahypar::make_iterator_range(rng1), boost_kahypar::make_iterator_range(rng2)) );
        }
                                                    ...
                                                    BinaryPredicate pred)
        {
            ...
            const bool test = boost_kahypar::lexicographical_compare(rng1, rng2, pred);

            BOOST_CHECK( reference == test );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(boost_kahypar::make_iterator_range(rng1), rng2, pred) );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(rng1, boost_kahypar::make_iterator_range(rng2), pred) );
            BOOST_CHECK( test == boost_kahypar::lexicographical_compare(boost_kahypar::make_iterator_range(rng1), boost_kahypar::make_iterator_range(rng2), pred) );
        }
        ...
        void test_lexicographical_compare_impl(Container1& cont1,
                                               Container2& cont2)
        {
            typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_value<Container1>::type value_t;
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_lexicographical_compare ) );
