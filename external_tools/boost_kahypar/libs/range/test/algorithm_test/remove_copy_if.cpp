...
namespace
{
    ...
    void test_remove_copy_if_impl( const Container& c, UnaryPredicate pred )
    {
        ...
        test_append(
            boost_kahypar::remove_copy_if(c, std::back_inserter(test), pred),
            value_type()
            );
        ...
        test_append(
            boost_kahypar::remove_copy_if(boost_kahypar::make_iterator_range(c),
    ...
    void test_remove_copy_if_( const Container& c, int to_remove )
    {
        ...
        test_remove_copy_if_impl(c, boost_kahypar::bind(std::equal_to<int>(), _1, to_remove));
        test_remove_copy_if_impl(c, boost_kahypar::bind(std::not_equal_to<int>(), _1, to_remove));
    }
