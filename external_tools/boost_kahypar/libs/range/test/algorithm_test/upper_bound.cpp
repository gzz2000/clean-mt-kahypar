...
namespace boost_kahypar_range_test_algorithm_upper_bound
{
    ...
    public:
        ...
        test_iter(Container& cont)
        {
            ...
            iter_t result = boost_kahypar::upper_bound(cont, 5);
            BOOST_CHECK( result == boost_kahypar::upper_bound(boost_kahypar::make_iterator_range(cont), 5) );
            return result;
        }

        template<boost_kahypar::range_return_value result_type>
        struct test_range
        {
            template<class Container, class Policy>
            BOOST_DEDUCED_TYPENAME boost_kahypar::range_return<Container,result_type>::type
            operator()(Policy&, Container& cont)
            {
                typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_return<Container,result_type>::type result_t;
                result_t result = boost_kahypar::upper_bound<result_type>(cont, 5);
                BOOST_CHECK( result == boost_kahypar::upper_bound<result_type>(boost_kahypar::make_iterator_range(cont), 5) );
                return result;
            }
        };
    ...
    struct upper_bound_pred_policy
    {
        ...
        {
            typedef BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<Container>::type iter_t;
            iter_t result = boost_kahypar::upper_bound(cont, 5, BinaryPredicate());
            BOOST_CHECK( result == boost_kahypar::upper_bound(boost_kahypar::make_iterator_range(cont), 5, BinaryPredicate()) );
            return result;
        }
        ...
        struct test_range
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                result_t result = boost_kahypar::upper_bound<result_type>(cont, 5, policy.pred());

                BOOST_CHECK( result == boost_kahypar::upper_bound<result_type>(
                    boost_kahypar::make_iterator_range(cont), 5, policy.pred()) );
    ...
    void test_upper_bound_impl(TestPolicy policy, BinaryPredicate pred)
    {
        ...
        typedef BOOST_DEDUCED_TYPENAME boost_kahypar::remove_const<Container>::type container_t;
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_upper_bound::test_upper_bound ) );
