...
namespace boost_kahypar
{
    namespace
    {
        ...
        void test_adjacent_find_impl()
        {
            ...
            BOOST_CHECK( boost_kahypar::adjacent_find(cont) == cont.end() );
            BOOST_CHECK( boost_kahypar::adjacent_find(cref_cont) == cref_cont.end() );
            BOOST_CHECK( boost_kahypar::adjacent_find(boost_kahypar::make_iterator_range(cont)) == cont.end() );
            BOOST_CHECK( boost_kahypar::adjacent_find(cont, pred) == cont.end() );
            BOOST_CHECK( boost_kahypar::adjacent_find(cref_cont, pred) == cref_cont.end() );
            BOOST_CHECK( boost_kahypar::adjacent_find(boost_kahypar::make_iterator_range(cont), pred) == cont.end() );
            ...
            iterator_t it = boost_kahypar::adjacent_find(cont);
            iterator_t it_pred = boost_kahypar::adjacent_find(cont, pred);
            ...
            BOOST_CHECK( it == boost_kahypar::adjacent_find(boost_kahypar::make_iterator_range(cont)) );
            BOOST_CHECK( it_pred == boost_kahypar::adjacent_find(boost_kahypar::make_iterator_range(cont), pred) );
            const_iterator_t cit = boost_kahypar::adjacent_find(cref_cont);
            const_iterator_t cit_pred = boost_kahypar::adjacent_find(cref_cont, pred);
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_adjacent_find ) );
