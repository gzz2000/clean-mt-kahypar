...
namespace boost_kahypar_range_test_algorithm_stable_partition
{
    struct equal_to_5
    {
    ...
    public:
        ...
        test_iter(Container& cont)
        {
            ...
            iter_t result = boost_kahypar::stable_partition(cont, UnaryPredicate());

            iter_t temp_result = boost_kahypar::stable_partition(
                boost_kahypar::make_iterator_range(cont2), UnaryPredicate());
        ...
        struct test_range
        {
            ...
            operator()(Policy& policy, Container& cont)
            {
                ...
                Container cont2(cont);
                result_t result = boost_kahypar::stable_partition<return_type>(cont, policy.pred());

                result_t result2 = boost_kahypar::stable_partition<return_type>(
                                    boost_kahypar::make_iterator_range(cont2), policy.pred());
                                    
                boost_kahypar::ignore_unused_variable_warning(result2);
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar_range_test_algorithm_stable_partition::test_stable_partition ) );
