...
namespace boost_kahypar
{
    namespace
    {
        ...
        void test_replace_impl(Container& cont)
        {
            ...
            boost_kahypar::replace(target, what, with_what);

            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                
            std::vector<int> target2(cont.begin(), cont.end());
            boost_kahypar::replace(boost_kahypar::make_iterator_range(target2), what,
                           with_what);
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &boost_kahypar::test_replace ) );
