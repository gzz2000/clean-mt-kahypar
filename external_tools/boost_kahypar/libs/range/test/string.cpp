...
namespace
{
    ...
    private:
        typedef boost_kahypar::array<value_type, Length + 1> buffer_t;
        buffer_t m_buffer;
    };
}

template< class T >
inline BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<T>::type
str_begin( T& r )
{
    return boost_kahypar::begin( boost_kahypar::as_literal(r) );
}
...
str_end( T& r )
{
    return boost_kahypar::end( boost_kahypar::as_literal(r) );
}

template< class T >
inline BOOST_DEDUCED_TYPENAME boost_kahypar::range_difference<T>::type
str_size( const T& r )
{
    return boost_kahypar::size( boost_kahypar::as_literal(r) );
}
...
str_empty( T& r )
{
    return boost_kahypar::empty( boost_kahypar::as_literal(r) );
}
...
template< typename Container, typename T >
BOOST_DEDUCED_TYPENAME boost_kahypar::range_iterator<const Container>::type
...
void check_char()
{
    typedef boost_kahypar::range_difference<std::string>::type diff_t;
...
void check_string()
{
    ...
    typedef boost_kahypar::range_difference<std::wstring>::type diff_t;
    ...
    boost_kahypar::ignore_unused_variable_warning( my_wstring );
