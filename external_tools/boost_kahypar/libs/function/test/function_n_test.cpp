...
test_member_functions()
{
  ...
  boost_kahypar::function1<int, X*> f1(&X::twice);
  ...
  boost_kahypar::function1<int, X*> f1_2;
  f1_2 = &X::twice;

  BOOST_CHECK(f1_2(&one) == 2);
  BOOST_CHECK(f1_2(&five) == 10);

  boost_kahypar::function2<int, X&, int> f2(&X::plus);
...
test_ref()
{
  ...
  try {
    boost_kahypar::function2<int, int, int> f(ref(atc));
    BOOST_CHECK(f(1, 3) == 4);
  }
