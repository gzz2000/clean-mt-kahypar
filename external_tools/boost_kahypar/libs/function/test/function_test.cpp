...
{
  boost_kahypar::function<int (X*)> f1(&X::twice);
  ...
  boost_kahypar::function<int (X*)> f1_2;
  f1_2 = &X::twice;

  BOOST_CHECK(f1_2(&one) == 2);
  BOOST_CHECK(f1_2(&five) == 10);

  boost_kahypar::function<int (X&, int)> f2(&X::plus);
...
test_ref()
{
  ...
  try {
    boost_kahypar::function<int (int, int)> f(boost_kahypar::ref(atc));
    BOOST_CHECK(f(1, 3) == 4);
  }
...
static void test_empty_ref()
{
  boost_kahypar::function<void()> f1;
  boost_kahypar::function<void()> f2(boost_kahypar::ref(f1));
...
{
  boost_kahypar::function<int (int, int)> f;
  try {
    f(5, 4);
    BOOST_CHECK(false);
  }
  catch(boost_kahypar::bad_function_call const&) {
    // okay
  }
}

typedef boost_kahypar::function< void * (void * reader) > reader_type;
...
static void test_call_obj(boost_kahypar::function<int (int, int)> f)
{
  BOOST_CHECK(!f.empty());
}

static void test_call_cref(const boost_kahypar::function<int (int, int)>& f)
{
  BOOST_CHECK(!f.empty());
}
...
int main()
{
  ...
  test_move_semantics<boost_kahypar::function0<void> >();
