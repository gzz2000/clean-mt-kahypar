...
only_movable two(BOOST_RV_REF(only_movable) t) {
    only_movable t1 = boost_kahypar::move(t);
...
std::string&& four(std::string&& s) { return boost_kahypar::move(s); }
...
int main()
{
    ...
    only_movable om1(1);
    BOOST_CHECK(f1(boost_kahypar::move(om1)) == 1);

    function <only_movable(BOOST_RV_REF(only_movable))>  f2 = two;

    only_movable om2(2);
    only_movable om2_2 = f2(boost_kahypar::move(om2));
...

    {
        ...
        only_movable om2_sum_2 = f2_sum(boost_kahypar::move(om1_sum), boost_kahypar::move(om2_sum));
