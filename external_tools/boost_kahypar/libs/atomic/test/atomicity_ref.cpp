...
bool test_arithmetic(value_type& shared_value, std::size_t instance)
{
    ...
    boost_kahypar::atomic_ref<value_type> shared_value_ref(shared_value);

    for (std::size_t n = 0; n < 255; ++n)
    {
        value_type tmp = shared_value_ref.fetch_add(increment, boost_kahypar::memory_order_relaxed);
    ...
    for (std::size_t n = 0; n < 255; ++n)
    {
        value_type tmp = shared_value_ref.fetch_sub(increment, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift) )
            return false;
        --expected;
    }
...
bool test_bitops(value_type& shared_value, std::size_t instance)
{
    ...
    {
        value_type mod = 1u << k;
        value_type tmp = shared_value_ref.fetch_or(mod << shift, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
    ...
    for (std::size_t k = 0; k < 8; ++k)
    {
        value_type tmp = shared_value_ref.fetch_and(~(1u << (shift + k)), boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
    ...
    for (std::size_t k = 0; k < 8; ++k)
    {
        ...
        value_type tmp = shared_value_ref.fetch_xor(mod << shift, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
        expected = expected ^ mod;
    }

    value_type tmp = shared_value_ref.fetch_and(~mask, boost_kahypar::memory_order_relaxed);
    if ( (tmp & mask) != (expected << shift) )
        return false;
