...
boost_kahypar::atomic< unsigned int > g_atomic(0u);
...
void thread_func(test_barrier* barrier)
{
    ...
    while (true)
    {
        unsigned int new_count = g_atomic.wait(old_count, boost_kahypar::memory_order_relaxed);
...
int main()
{
    ...
    while (true)
    {
        for (unsigned int i = 0u; i < thread_count; ++i)
        {
            g_atomic.opaque_add(1u, boost_kahypar::memory_order_relaxed);
            g_atomic.notify_one();
        }

        unsigned int old_count = g_atomic.fetch_add(1u, boost_kahypar::memory_order_relaxed);
        g_atomic.notify_all();
