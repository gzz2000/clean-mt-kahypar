...
public:
    ...
    void wait_finish(steady_clock::duration& timeout)
    {
        ...
        finished_.store(true, boost_kahypar::memory_order_relaxed);
    ...
    {
        return finished_.load(boost_kahypar::memory_order_relaxed);
    }
...
private:
    ...
    boost_kahypar::atomic<bool> finished_;
    bool failure_;
...
bool test_arithmetic(boost_kahypar::atomic< value_type >& shared_value, std::size_t instance)
{
    std::size_t shift = instance * 8;
    ...
    for (std::size_t n = 0; n < 255; ++n)
    {
        value_type tmp = shared_value.fetch_add(increment, boost_kahypar::memory_order_relaxed);
    ...
    for (std::size_t n = 0; n < 255; ++n)
    {
        value_type tmp = shared_value.fetch_sub(increment, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift) )
            return false;
        --expected;
    }
...
template<typename value_type, std::size_t shift_>
bool test_bitops(boost_kahypar::atomic< value_type >& shared_value, std::size_t instance)
{
    std::size_t shift = instance * 8;
    value_type mask = 0xff << shift;
    ...
    {
        value_type mod = 1u << k;
        value_type tmp = shared_value.fetch_or(mod << shift, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
    ...
    for (std::size_t k = 0; k < 8; ++k)
    {
        value_type tmp = shared_value.fetch_and(~(1u << (shift + k)), boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
    ...
    for (std::size_t k = 0; k < 8; ++k)
    {
        ...
        value_type tmp = shared_value.fetch_xor(mod << shift, boost_kahypar::memory_order_relaxed);
        if ( (tmp & mask) != (expected << shift))
            return false;
        expected = expected ^ mod;
    }

    value_type tmp = shared_value.fetch_and(~mask, boost_kahypar::memory_order_relaxed);
    if ( (tmp & mask) != (expected << shift) )
        return false;
...
int main(int, char *[])
{
    ...
    {
        boost_kahypar::atomic<unsigned int> value(0);
