...
void verify_lock_free(const char* type_name, int lock_free_macro_val, int lock_free_expect)
{
    ...
    BOOST_TEST_EQ(boost_kahypar::atomic<T>::is_always_lock_free, (lock_free_expect == 2));
    ...
    if (sizeof(boost_kahypar::atomic<T>) == sizeof(T))
    {
        aligned_object<T, boost_kahypar::atomic_ref<T>::required_alignment> object;
        boost_kahypar::atomic_ref<T> ref(object.get());

        BOOST_TEST_EQ(ref.is_lock_free(), value.is_lock_free());
        BOOST_TEST_EQ(boost_kahypar::atomic_ref<T>::is_always_lock_free, boost_kahypar::atomic<T>::is_always_lock_free);
    }
}
...
int main(int, char *[])
{
    ...
    verify_lock_free<boost_kahypar::int128_type>("int128", BOOST_ATOMIC_INT128_LOCK_FREE, EXPECT_INT128_LOCK_FREE);
#endif
    ...
    verify_lock_free<boost_kahypar::float128_type>("float128", BOOST_ATOMIC_INT128_LOCK_FREE, EXPECT_INT128_LOCK_FREE);
#endif
