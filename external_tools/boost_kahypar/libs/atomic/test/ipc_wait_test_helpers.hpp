...
public:
    ...
    bool run()
    {
        ...
        m_wrapper.a.store(m_value2, boost_kahypar::memory_order_release);
        m_wrapper.a.notify_one();

        std::this_thread::sleep_for(chrono::milliseconds(200));

        m_wrapper.a.store(m_value3, boost_kahypar::memory_order_release);
        m_wrapper.a.notify_one();
...
template< template< typename > class Wrapper, typename T >
void test_wait_notify_api_impl(T value1, T value2, T value3, boost_kahypar::true_type)
{
...
template< template< typename > class Wrapper, typename T >
inline void test_wait_notify_api_impl(T, T, T, boost_kahypar::false_type)
{
}
...
{
    test_wait_notify_api_impl< Wrapper >(value1, value2, value3, boost_kahypar::integral_constant< bool, Wrapper< T >::atomic_type::is_always_lock_free >());
}
...
inline void test_flag_wait_notify_api()
{
    ...
    boost_kahypar::ipc_atomic_flag f = BOOST_ATOMIC_FLAG_INIT;
#else
    boost_kahypar::ipc_atomic_flag f;
#endif
