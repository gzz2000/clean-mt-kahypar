...
#include <boost_kahypar/atomic/atomic.hpp>
...
template<boost_kahypar::memory_order store_order, boost_kahypar::memory_order load_order>
...
private:
    ...
    boost_kahypar::atomic<bool> terminate_threads_;
    boost_kahypar::atomic<int> termination_consensus_;
...
void total_store_order_test<store_order, load_order>::run(steady_clock::duration& timeout)
{
    ...
    terminate_threads_.store(true, boost_kahypar::memory_order_relaxed);
...
void total_store_order_test<store_order, load_order>::thread1fn(void)
{
    while (true)
    {
        ...
        if (terminate_threads_.load(boost_kahypar::memory_order_relaxed))
        {
            while (true)
            {
                int tmp = termination_consensus_.fetch_or(1, boost_kahypar::memory_order_relaxed);
        ...
        termination_consensus_.fetch_xor(4, boost_kahypar::memory_order_relaxed);

        unsigned int delay = std::rand() % 10000;
        a_.store(0, boost_kahypar::memory_order_relaxed);
...
void total_store_order_test<store_order, load_order>::thread2fn(void)
{
    while (true)
    {
        ...
        if (terminate_threads_.load(boost_kahypar::memory_order_relaxed))
        {
            while (true)
            {
                int tmp = termination_consensus_.fetch_or(2, boost_kahypar::memory_order_relaxed);
        ...
        unsigned int delay = std::rand() % 10000;
        b_.store(0, boost_kahypar::memory_order_relaxed);
...
void test_seq_cst(void)
{
    ...
    for (std::size_t n = 0; n < 10; n++)
    {
        ...
        total_store_order_test<boost_kahypar::memory_order_relaxed, boost_kahypar::memory_order_relaxed> test;
        test.run(timeout);
    ...
    total_store_order_test<boost_kahypar::memory_order_seq_cst, boost_kahypar::memory_order_seq_cst> test;
    test.run(timeout);
