...
TMP_ASSERT_PAREN((boost_kahypar::is_const<std::map<int, char> const>));

// Specify both the extra parenthesis `((...))` and `BOOST_IDENTITY_TYPE` macro.
TMP_ASSERT(BOOST_IDENTITY_TYPE((boost_kahypar::is_const<std::map<int, char> const>)));
//]

//[paren_always
TMP_ASSERT_PAREN((boost_kahypar::is_const<int const>)); // Always extra `((...))`.

TMP_ASSERT(boost_kahypar::is_const<int const>); // No extra `((...))` and no macro.
//]
