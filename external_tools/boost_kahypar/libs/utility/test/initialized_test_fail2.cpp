...
//
// Test program for boost_kahypar::initialized<T>. Must fail to compile.
//
// Initial: 2 May 2010
...
namespace
{
  ...
  {
    boost_kahypar::value_initialized<int> value_initialized_int;

    // Okay: initialized<T> can be initialized by value_initialized<T>.
    boost_kahypar::initialized<int> initialized_int(value_initialized_int);
  }

  void from_initialized_to_value_initialized()
  {
    boost_kahypar::initialized<int> initialized_int(13);

    // The following line should not compile, because initialized<T>
    // should not be convertible to value_initialized<T>.
    boost_kahypar::value_initialized<int> value_initialized_int(initialized_int);
  }
}
