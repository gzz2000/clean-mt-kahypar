...
{
  T initializedValue = boost_kahypar::initialized_value ;
...
void check_initialized_value( NonPOD const& )
{
  ...
  //   NonPOD initializedValue = boost_kahypar::initialized_value() ;
  //
...
//
// This test function tests boost_kahypar::value_initialized<T> for a specific type T.
...
{
  const int errors_before_test = boost_kahypar::detail::test_errors();

  check_initialized_value(y);

  boost_kahypar::value_initialized<T> x ;
  BOOST_TEST ( y == x ) ;
  BOOST_TEST ( y == boost_kahypar::get(x) ) ;

  static_cast<T&>(x) = z ;
  boost_kahypar::get(x) = z ;
  BOOST_TEST ( x == z ) ;

  boost_kahypar::value_initialized<T> const x_c ;
  BOOST_TEST ( y == x_c ) ;
  BOOST_TEST ( y == boost_kahypar::get(x_c) ) ;
  T& x_c_ref = const_cast<T&>( boost_kahypar::get(x_c) ) ;
  x_c_ref = z ;
  BOOST_TEST ( x_c == z ) ;

  boost_kahypar::value_initialized<T> const copy1 = x;
  BOOST_TEST ( boost_kahypar::get(copy1) == boost_kahypar::get(x) ) ;

  boost_kahypar::value_initialized<T> copy2;
  copy2 = x;
  BOOST_TEST ( boost_kahypar::get(copy2) == boost_kahypar::get(x) ) ;

  {
    boost_kahypar::value_initialized<T> * ptr = new boost_kahypar::value_initialized<T>;
    BOOST_TEST ( y == *ptr ) ;
    delete ptr;
  }

#if !BOOST_WORKAROUND(BOOST_MSVC, < 1300)
  boost_kahypar::value_initialized<T const> cx ;
  BOOST_TEST ( y == cx ) ;
  BOOST_TEST ( y == boost_kahypar::get(cx) ) ;

  boost_kahypar::value_initialized<T const> const cx_c ;
  BOOST_TEST ( y == cx_c ) ;
  BOOST_TEST ( y == boost_kahypar::get(cx_c) ) ;
#endif

  return boost_kahypar::detail::test_errors() == errors_before_test ;
}
...
int main()
{
  ...
  boost_kahypar::value_initialized<ArrayOfBytes> valueInitializedArrayOfBytes;
  BOOST_TEST (std::memcmp(get(valueInitializedArrayOfBytes), zeroInitializedArrayOfBytes, sizeof(ArrayOfBytes)) == 0);

  boost_kahypar::value_initialized<ArrayOfBytes> valueInitializedArrayOfBytes2;
  ...
  boost_kahypar::value_initialized<CopyFunctionCallTester> copyFunctionCallTester1;
  BOOST_TEST ( ! get(copyFunctionCallTester1).is_copy_constructed);
  BOOST_TEST ( ! get(copyFunctionCallTester1).is_assignment_called);

  boost_kahypar::value_initialized<CopyFunctionCallTester> copyFunctionCallTester2 = boost_kahypar::value_initialized<CopyFunctionCallTester>(copyFunctionCallTester1);
  BOOST_TEST ( get(copyFunctionCallTester2).is_copy_constructed);
  BOOST_TEST ( ! get(copyFunctionCallTester2).is_assignment_called);

  boost_kahypar::value_initialized<CopyFunctionCallTester> copyFunctionCallTester3;
  copyFunctionCallTester3 = boost_kahypar::value_initialized<CopyFunctionCallTester>(copyFunctionCallTester1);
  BOOST_TEST ( ! get(copyFunctionCallTester3).is_copy_constructed);
  BOOST_TEST ( get(copyFunctionCallTester3).is_assignment_called);

  boost_kahypar::value_initialized<SwapFunctionCallTester> swapFunctionCallTester1;
  boost_kahypar::value_initialized<SwapFunctionCallTester> swapFunctionCallTester2;
  get(swapFunctionCallTester1).data = 1;
  get(swapFunctionCallTester2).data = 2;
  boost_kahypar::core::invoke_swap(swapFunctionCallTester1, swapFunctionCallTester2);
