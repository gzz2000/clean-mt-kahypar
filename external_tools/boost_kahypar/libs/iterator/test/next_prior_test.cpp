...
bool plus_n_test(RandomAccessIterator first, RandomAccessIterator last, ForwardIterator first2)
{
    ...
    for(int n = 0; i != last; ++n)
        i = first + n, j = boost_kahypar::next(first2, n);
    return std::distance(first, i) == std::distance(first2, j);
}
...
bool minus_n_test(RandomAccessIterator first, RandomAccessIterator last, BidirectionalIterator last2)
{
    ...
    for(int n = 0; i != first; ++n)
        i = last - n, j = boost_kahypar::prior(last2, n);
    return std::distance(i, last) == std::distance(j, last2);
}
...
{
    Iterator i = boost_kahypar::prior(last, size);
    return i == first;
}
...
int main(int, char*[])
{
    ...
    // Tests with integers
    BOOST_TEST(boost_kahypar::next(5) == 6);
    BOOST_TEST(boost_kahypar::next(5, 7) == 12);
    BOOST_TEST(boost_kahypar::prior(5) == 4);
    BOOST_TEST(boost_kahypar::prior(5, 7) == -2);
    BOOST_TEST(boost_kahypar::prior(5, 7u) == -2);
