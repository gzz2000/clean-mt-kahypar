...
template <class V>
struct ptr_iterator
  : boost_kahypar::iterator_adaptor<
        ptr_iterator<V>
      , V*
      , V
...
public:
  ...
  ptr_iterator(
      ...
      , typename boost_kahypar::enable_if_convertible<V2*, V*>::type* = 0
  )
    : super_t(x.base())
  {}
};
...
struct fwd_iterator
  : boost_kahypar::iterator_adaptor<
        fwd_iterator<T>
      , boost_kahypar::forward_iterator_archetype<T>
    >
{
private:
  typedef boost_kahypar::iterator_adaptor<
...
public:
  fwd_iterator() { }
  fwd_iterator(boost_kahypar::forward_iterator_archetype<T> d) : super_t(d) { }
};
...
struct in_iterator
  : boost_kahypar::iterator_adaptor<
        in_iterator<T>
      , boost_kahypar::input_iterator_archetype_no_proxy<T>
    >
{
private:
  typedef boost_kahypar::iterator_adaptor<
...
public:
  in_iterator() { }
  in_iterator(boost_kahypar::input_iterator_archetype_no_proxy<T> d) : super_t(d) { }
};
...
char (& traversal2(boost_kahypar::incrementable_traversal_tag) )[1];
char (& traversal2(boost_kahypar::single_pass_traversal_tag  ) )[2];
char (& traversal2(boost_kahypar::forward_traversal_tag      ) )[3];
char (& traversal2(boost_kahypar::bidirectional_traversal_tag) )[4];
char (& traversal2(boost_kahypar::random_access_traversal_tag) )[5];

template <class Cat>
struct traversal3
{
    static typename boost_kahypar::iterator_category_to_traversal<Cat>::type x;
...
{
    typedef typename boost_kahypar::iterator_category_to_traversal<
...
main()
{
  ...
  boost_kahypar::random_access_iterator_test(array, N, array);
...
#if !BOOST_WORKAROUND(__MWERKS__, <= 0x2407)
    BOOST_STATIC_ASSERT((boost_kahypar::is_convertible<Iter1::iterator_category, std::random_access_iterator_tag>::value));
#endif
  }
...
#if !BOOST_WORKAROUND(__MWERKS__, <= 0x2407)
    BOOST_STATIC_ASSERT(boost_kahypar::is_readable_iterator<Iter1>::value);
# ifndef BOOST_NO_LVALUE_RETURN_DETECTION
    BOOST_STATIC_ASSERT(boost_kahypar::is_lvalue_iterator<Iter1>::value);
# endif
#endif
...
#ifndef BOOST_NO_LVALUE_RETURN_DETECTION
    BOOST_STATIC_ASSERT(boost_kahypar::is_non_const_lvalue_iterator<BaseIter>::value);
    BOOST_STATIC_ASSERT(boost_kahypar::is_lvalue_iterator<Iter>::value);
#endif

    typedef modify_traversal<BaseIter, boost_kahypar::incrementable_traversal_tag> IncrementableIter;

    static_assert_traversal<BaseIter,boost_kahypar::random_access_traversal_tag>();
    static_assert_traversal<IncrementableIter,boost_kahypar::incrementable_traversal_tag>();
  }
  ...
  {
    boost_kahypar::forward_iterator_archetype<dummyT> forward_iter;
  ...
  {
    boost_kahypar::input_iterator_archetype_no_proxy<dummyT> input_iter;
