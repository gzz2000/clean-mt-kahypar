...
{
    typedef boost_kahypar::indirect_iterator<typename Container::iterator> iterator;

    typedef boost_kahypar::indirect_iterator<
...
void more_indirect_iterator_tests()
{
    ...
    boost_kahypar::random_access_iterator_test(db + 1, store.size() - 1, boost_kahypar::next(store.begin()));

    *db = 999;
    BOOST_TEST(store.front() == 999);

    // Borland C++ is getting very confused about the typedefs here
    typedef boost_kahypar::indirect_iterator<iterator_set::iterator> indirect_set_iterator;
    typedef boost_kahypar::indirect_iterator<
...
# endif
    ...
    *boost_kahypar::prior(se) = 888;
    BOOST_TEST(store.back() == 888);
    BOOST_TEST(std::equal(sb, se, store.begin()));

    boost_kahypar::bidirectional_iterator_test(boost_kahypar::next(sb), store[1], store[2]);
...
# if BOOST_WORKAROUND(BOOST_MSVC, == 1200)
  boost_kahypar::shared_ptr<dummyT> zz((dummyT*)0);  // Why? I don't know, but it suppresses a bad instantiation.
# endif

  typedef std::vector<boost_kahypar::shared_ptr<dummyT> > shared_t;
  shared_t shared;

  // Concept checks
  {
    typedef boost_kahypar::indirect_iterator<shared_t::iterator> iter_t;
    ...
    typedef boost_kahypar::indirect_iterator<
        shared_t::iterator
      , boost_kahypar::iterator_value<shared_t::iterator>::type const
    > c_iter_t;
...
# endif
  ...
  // Test indirect_iterator_generator
  {
      for (int jj = 0; jj < N; ++jj)
          shared.push_back(boost_kahypar::shared_ptr<dummyT>(new dummyT(jj)));

      dummyT* ptr[N];
      for (int k = 0; k < N; ++k)
          ptr[k] = array + k;

      typedef boost_kahypar::indirect_iterator<dummyT**> indirect_iterator;

      typedef boost_kahypar::indirect_iterator<dummyT**, dummyT const>
          const_indirect_iterator;

      indirect_iterator i(ptr);
      boost_kahypar::random_access_iterator_test(i, N, array);

      boost_kahypar::random_access_iterator_test(
          boost_kahypar::indirect_iterator<shared_t::iterator>(shared.begin())
          , N, array);

      boost_kahypar::random_access_iterator_test(boost_kahypar::make_indirect_iterator(ptr), N, array);
      ...
      boost_kahypar::random_access_iterator_test(boost_kahypar::make_indirect_iterator(const_ptr), N, array);
