...
struct Xiter : boost_kahypar::iterator_adaptor<Xiter,X*>
{
    Xiter();
    Xiter(X* p) : boost_kahypar::iterator_adaptor<Xiter, X*>(p) {}
};
...
struct static_assert_min_cat
  : static_assert_same<
       typename boost_kahypar::iterators::minimum_category<T,U>::type, Min
    >
{};

void category_test()
{
    using namespace boost_kahypar::iterators;
    using namespace boost_kahypar::iterators::detail;
