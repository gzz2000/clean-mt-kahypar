...
#include <boost_kahypar/iterator/zip_iterator.hpp> // 2nd #include tests #include guard.
#include <iostream>
#include <vector>
#include <list>
#include <set>
...
void category_test()
{
    ...
    boost_kahypar::make_zip_iterator(
        ZI_MAKE_TUPLE(
            boost_kahypar::make_transform_iterator(rng1.begin(), &to_value), // BidirectionalInput
...
int main( void )
{
  ...
  std::cout << "\n"
            << "***********************************************\n"
            << "*                                             *\n"
            << "* Test driver for boost_kahypar::zip_iterator         *\n"
  ...
  typedef boost_kahypar::tuples::cons<
    std::set<int>::iterator,
    ZI_TUPLE<
  ...
  typedef boost_kahypar::zip_iterator<cons_12_its_type> zip_it_12_type;
...
//  zip_it_non_const = ++zip_it_const;  // Error: can't convert from const to non-const
  ...
  bool bBigItIsBidirectionalIterator = boost_kahypar::is_convertible<
    boost_kahypar::iterator_traversal<zip_it_12_type>::type
        , boost_kahypar::bidirectional_traversal_tag
        >::value;

  bool bBigItIsRandomAccessIterator = boost_kahypar::is_convertible<
    boost_kahypar::iterator_traversal<zip_it_12_type>::type
  ...
  // traversal.
  //
  typedef boost_kahypar::zip_iterator<
    ZI_TUPLE<
      std::vector<double>::const_iterator,
      std::vector<double>::const_iterator
      >
  ...
  bool bAllVectsIsRandomAccessIterator = boost_kahypar::is_convertible<
    boost_kahypar::iterator_traversal<all_vects_type>::type
        , boost_kahypar::random_access_traversal_tag
    >::value;
