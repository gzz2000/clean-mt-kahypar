...
{
    T m_x;
    explicit wrapper(typename boost_kahypar::call_traits<T>::param_type x)
        : m_x(x)
    { }
    template <class U>
    wrapper(const wrapper<U>& other,
        typename boost_kahypar::enable_if< boost_kahypar::is_convertible<U,T> >::type* = 0)
        : m_x(other.m_x)
    { }
};
...
{ BOOST_STATIC_ASSERT((boost_kahypar::is_same<T,U>::value)); }
...
int main()
{
    {
        int state = 0;
        boost_kahypar::readable_iterator_test(counter_iterator<int const&>(&state), 0);
        state = 3;
        boost_kahypar::readable_iterator_test(counter_iterator<proxy>(&state), 3);
        boost_kahypar::writable_iterator_test(counter_iterator<proxy>(&state), 9, 7);
        BOOST_TEST(state == 8);
    }
    ...
    {
        derived d(1);
        boost_kahypar::readable_iterator_test(abstract_iterator<derived *, base>(&d), derived(1));
    }
