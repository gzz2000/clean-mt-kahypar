    ...
    std::random_access_iterator_tag)
{
    ...
    BOOST_TEST(!std::binary_search(start, boost_kahypar::prior(finish), *finish));
    ...
    boost_kahypar::random_access_iterator_test(start, v.size(), v.begin());
}
...
void category_test(CountingIterator start, Value v1, std::bidirectional_iterator_tag)
{
    ...
    // Note that this test requires a that the first argument is
    // dereferenceable /and/ a valid iterator prior to the first argument
    boost_kahypar::bidirectional_iterator_test(start, v1, v2);
}
...
void category_test(CountingIterator start, CountingIterator finish, Value v1, std::forward_iterator_tag)
{
    Value v2 = v1;
    ++v2;
    if (finish != start && finish != boost_kahypar::next(start))
        boost_kahypar::forward_readable_iterator_test(start, finish, v1, v2);
}
...
void test_aux(CountingIterator start, CountingIterator finish, Value v1)
{
    ...
    for (CountingIterator p = start
             ; p != finish && boost_kahypar::next(p) != finish
             ; ++p)
    {
        BOOST_TEST(boost_kahypar::next(*p) == *boost_kahypar::next(p));
    }
...
{
    test_aux(boost_kahypar::make_counting_iterator(start), boost_kahypar::make_counting_iterator(finish), start);
}
...
void test_integer3(Integer* = 0, Category* = 0, Difference* = 0) // default arg works around MSVC bug
{
    Integer start = 0;
    Integer finish = 120;
    typedef boost_kahypar::counting_iterator<Integer,Category,Difference> iterator;
...
int main()
{
    ...
    test_integer<boost_kahypar::long_long_type>();
    test_integer<boost_kahypar::ulong_long_type>();
#endif
#if defined(BOOST_HAS_INT128)
    test_integer<boost_kahypar::int128_type>();
    test_integer<boost_kahypar::uint128_type>();
#endif
...
# endif
    ...
    int array[2000];
    test(boost_kahypar::make_counting_iterator(array), boost_kahypar::make_counting_iterator(array+2000-1));
