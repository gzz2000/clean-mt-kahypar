...
// This will eventually stop working when <boost_kahypar/progress.hpp> is removed
...
#include <boost_kahypar/program_options/detail/utf8_codecvt_facet.hpp>
...
{
    boost_kahypar::program_options::detail::utf8_codecvt_facet facet;
    
    std::wstring output;
    { 
        boost_kahypar::progress_timer t;
        for (int i = 0; i < 10000; ++i)
            output = boost_kahypar::from_8_bit(
                input,
                facet);
    }
    ...
    string ref = boost_kahypar::to_8_bit(output, facet);
...
{       
    ...
    if (ac > 1) {
        ...
        std::wstring w = boost_kahypar::from_local_8_bit(av[1]);
