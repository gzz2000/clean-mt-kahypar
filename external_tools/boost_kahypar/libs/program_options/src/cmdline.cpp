...
#include <boost_kahypar/program_options/detail/cmdline.hpp>
#include <boost_kahypar/program_options/errors.hpp>
#include <boost_kahypar/program_options/value_semantic.hpp>
#include <boost_kahypar/program_options/options_description.hpp>
#include <boost_kahypar/program_options/positional_options.hpp>
...
namespace boost_kahypar { namespace program_options {
    ...
    using namespace std;
    using namespace boost_kahypar::program_options::command_line_style;
...
namespace boost_kahypar { namespace program_options { namespace detail {
    ...
    cmdline::check_style(int style) const
    {
        ...
        if (allow_some_long && 
            ...
            error = "boost_kahypar::program_options misconfiguration: "
        ...
        if (error)
            boost_kahypar::throw_exception(invalid_command_line_style(error));
    ...
    cmdline::run()
    {
        ...
        if (m_additional_parser)
            style_parsers.push_back(
                boost_kahypar::bind(&cmdline::handle_additional_parser, this, _1));

        if (m_style & allow_long)
            style_parsers.push_back(
                boost_kahypar::bind(&cmdline::parse_long_option, this, _1));

        if ((m_style & allow_long_disguise))
            style_parsers.push_back(
                boost_kahypar::bind(&cmdline::parse_disguised_long_option, this, _1));

        if ((m_style & allow_short) && (m_style & allow_dash_for_short))
            style_parsers.push_back(
                boost_kahypar::bind(&cmdline::parse_short_option, this, _1));

        if ((m_style & allow_short) && (m_style & allow_slash_for_short))
            style_parsers.push_back(boost_kahypar::bind(&cmdline::parse_dos_option, this, _1));

        style_parsers.push_back(boost_kahypar::bind(&cmdline::parse_terminator, this, _1));
        ...
        if (m_positional)
        {
            ...
            for (unsigned i = 0; i < result.size(); ++i) {
                ...
                if (opt.position_key != -1) {
                    ...
                    {
                        boost_kahypar::throw_exception(too_many_positional_options_error());
                    }
    ...
    cmdline::parse_long_option(vector<string>& args)
    {
        ...
        {   
            ...
            if (p != tok.npos)
            {
                ...
                if (adjacent.empty())
                    boost_kahypar::throw_exception( invalid_command_line_syntax(
