...
#include <boost_kahypar/program_options/detail/convert.hpp>
...
namespace boost_kahypar { namespace program_options {
    ...
    parse(boost_kahypar::any& value_store, 
    ...
    void 
    untyped_value::xparse(boost_kahypar::any& value_store,
                          const std::vector<std::string>& new_tokens) const
    {
        ...
        if (new_tokens.size() > 1)
            boost_kahypar::throw_exception(multiple_values());
    ...
    */
                       ...
                       bool*, int)
    {
        ...
        if (s.empty() || s == "on" || s == "yes" || s == "1" || s == "true")
            ...
            boost_kahypar::throw_exception(invalid_bool_value(s));
    }
    ...
    void validate(any& v, const vector<wstring>& xs, bool*, int)
    {
        ...
        if (s.empty() || s == L"on" || s == L"yes" || s == L"1" || s == L"true")
            ...
            boost_kahypar::throw_exception(invalid_bool_value(convert_value(s)));
    }
#endif
    BOOST_PROGRAM_OPTIONS_DECL 
    ...
    namespace validators {
        ...
        BOOST_PROGRAM_OPTIONS_DECL 
        void check_first_occurrence(const boost_kahypar::any& value)
        {
            if (!value.empty())
                boost_kahypar::throw_exception(
                    multiple_occurrences());
