...
#include <boost_kahypar/program_options/detail/config_file.hpp>
...
namespace boost_kahypar { namespace program_options { namespace detail {
    ...
    common_config_file_iterator::add_option(const char* name)
    {
        ...
        if (*s.rbegin() == '*') {
            ...
            if (bad_prefixes)
                boost_kahypar::throw_exception(error("options '" + string(name) + "' and '" +
            ...
            if (!s.empty()) {
                ...
                if (*s.begin() == '[' && *s.rbegin() == ']') {
                    ...
                    if (!registered && !m_allow_unregistered)
                        boost_kahypar::throw_exception(unknown_option(name));
                    ...
                    boost_kahypar::throw_exception(invalid_config_file_syntax(s, invalid_syntax::unrecognized_line));
                }
            }
        }
        if (!found)
...
#if 0
using boost_kahypar::program_options::config_file;
