...
#include <boost_kahypar/unordered_set.hpp>
...
#if SIZE_MAX > UINT32_MAX
        boost_kahypar::uint64_t h = 0xCBF29CE484222325ull;
#else
        boost_kahypar::uint64_t h = 0x811C9DC5u;
#endif
...
template<class H, class V> void test_hash_speed( int N, V const& v )
{
    ...
    std::string hash = boost_kahypar::core::type_name<H>();
...
{
    boost_kahypar::unordered_set<std::size_t> s;
    H const h;
...
{
    boost_kahypar::unordered_set<K, H> s( 0 );
    test4( N, v, boost_kahypar::core::type_name<H>().c_str(), s );
}
...
int main()
{
...

    {
        ...
        boost_kahypar::detail::splitmix64 rnd;
    ...
    test_hash_speed<boost_kahypar::hash<std::string> >( N * 16, v );
...

    {
        ...
        {
            boost_kahypar::unordered_set<std::string> s;
        ...
        test_hash_collision<boost_kahypar::hash<std::string> >( N * 16, v, n );
    ...
    test_container_speed<K, boost_kahypar::hash<std::string> >( N, v );
