...
#include <cmath>
#include <boost_kahypar/container_hash/detail/limits.hpp>
...
void float_tests(char const* name, T* = 0)
{
    std::cerr
        ...
        <<  ">\n"
        <<  "\n"
        <<  "boost_kahypar::hash_detail::limits<T>::digits = "
        <<  boost_kahypar::hash_detail::limits<T>::digits<< "\n"
        <<  "boost_kahypar::hash_detail::limits<int>::digits = "
        <<  boost_kahypar::hash_detail::limits<int>::digits<< "\n"
        <<  "boost_kahypar::hash_detail::limits<std::size_t>::digits = "
        <<  boost_kahypar::hash_detail::limits<std::size_t>::digits
        <<  "\n"
        <<  "\n"
        ;
...
// Doing anything with infinity causes borland to crash.
    ...
    std::cerr
        <<  "Not running infinity checks on Borland, as it causes it to crash."
            "\n";
#else
    if(boost_kahypar::hash_detail::limits<T>::has_infinity) {
        T infinity = -log(zero);
        T infinity2 = (T) 1. / zero;
        T infinity3 = (T) -1. / minus_zero;
        T infinity4 = boost_kahypar::hash_detail::limits<T>::infinity();
        ...
        if(boost_kahypar::hash_detail::limits<T>::has_denorm) {
            if(x1(boost_kahypar::hash_detail::limits<T>::denorm_min()) == x1(infinity))
            {
                std::cerr
            ...
            if(x1(boost_kahypar::hash_detail::limits<T>::denorm_min()) ==
                x1(minus_infinity))
            {
                std::cerr
                    <<  "x1(denorm_min) == x1(-infinity) == "
        ...
        if(boost_kahypar::hash_detail::limits<T>::has_quiet_NaN) {
            if(x1(boost_kahypar::hash_detail::limits<T>::quiet_NaN()) == x1(infinity))
            {
                std::cerr
            ...
            if(x1(boost_kahypar::hash_detail::limits<T>::quiet_NaN()) ==
                x1(minus_infinity))
            {
                std::cerr
                    <<  "x1(quiet_NaN) == x1(-infinity) == "
    ...
    T max = (boost_kahypar::hash_detail::limits<T>::max)();
    T half_max = max / 2;
    T quarter_max = max / 4;
...
// Intel with gcc stdlib sometimes segfaults on calls to asin and acos.
...
#if defined(BOOST_HASH_TEST_EXTENSIONS)
    BOOST_TEST(x1(boost_kahypar::hash_detail::limits<T>::epsilon()) ==
            BOOST_HASH_TEST_NAMESPACE::hash_value(
                boost_kahypar::hash_detail::limits<T>::epsilon()));
#endif

    BOOST_TEST(boost_kahypar::hash_detail::limits<T>::epsilon() != (T) 0);
    if(x1(boost_kahypar::hash_detail::limits<T>::epsilon()) == x1((T) 0))
    ...
    BOOST_TEST((T) 1 + boost_kahypar::hash_detail::limits<T>::epsilon() != (T) 1);
    if(x1((T) 1 + boost_kahypar::hash_detail::limits<T>::epsilon()) == x1((T) 1))
    ...
    // As before.
    if(boost_kahypar::hash_detail::limits<T>::has_denorm) {
        if(x1(boost_kahypar::hash_detail::limits<T>::denorm_min()) == x1(zero)) {
        ...
        // specialization of boost_kahypar::hash_detail::limits::denorm_min() for long
        // doubles which causes this test to fail.
        if(x1(boost_kahypar::hash_detail::limits<T>::denorm_min()) !=
            BOOST_HASH_TEST_NAMESPACE::hash_value(
                boost_kahypar::hash_detail::limits<T>::denorm_min()))
        {
            std::cerr
                <<  "x1(boost_kahypar::hash_detail::limits<T>::denorm_min()) = "
                <<  x1(boost_kahypar::hash_detail::limits<T>::denorm_min())
                <<  "\nhash_value(boost_kahypar::hash_detail::limits<T>::denorm_min())"
                    " = "
                <<  BOOST_HASH_TEST_NAMESPACE::hash_value(
                        boost_kahypar::hash_detail::limits<T>::denorm_min())
                <<  "\nx1(0) = "
...
// NaN also causes borland to crash.
    ...
    if(boost_kahypar::hash_detail::limits<T>::has_quiet_NaN) {
        if(x1(boost_kahypar::hash_detail::limits<T>::quiet_NaN()) == x1(1.0)) {
            std::cerr<<"x1(quiet_NaN) == x1(1.0) == "<<x1(1.0)<<"\n";
        }
        BOOST_TEST(x1(boost_kahypar::hash_detail::limits<T>::quiet_NaN()) ==
            BOOST_HASH_TEST_NAMESPACE::hash_value(
                boost_kahypar::hash_detail::limits<T>::quiet_NaN()));
    }
#endif
}
