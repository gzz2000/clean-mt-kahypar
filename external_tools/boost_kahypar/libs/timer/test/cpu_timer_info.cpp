...
using boost_kahypar::timer::cpu_timer;
using boost_kahypar::timer::auto_cpu_timer;
...
int cpp_main( int argc, char * argv[] )
{
  ...
  {
    cpu_timer cpu;
    cout << "measure boost_kahypar::timer::cpu_timer resolution for user time..."
              << endl;
    for (int i = 0; i < 3; ++i)
    {
      cpu.start();
  ...
  {
    cpu_timer cpu;
    cout << "measure boost_kahypar::timer::cpu_timer resolution for wall-clock time..."
              << endl;
    for (int i = 0; i < 100; ++i)
    {
      cpu.start();
