...
// define BOOST_TIMER_SOURCE so that <boost_kahypar/timer/config.hpp> knows
...
#include <boost_kahypar/io/ios_state.hpp>
#include <boost_kahypar/predef.h>
...
using boost_kahypar::timer::nanosecond_type;
using boost_kahypar::timer::cpu_times;
...
namespace
{
  ...
  //  be as low as 10, although will be 15 for many common platforms.
  {
    if (places > 9)
      ...
      places = boost_kahypar::timer::default_places;
 
    boost_kahypar::io::ios_flags_saver ifs(os);
    boost_kahypar::io::ios_precision_saver ips(os);
...
boost_kahypar::long_long_type query_performance_frequency()
{
    LARGE_INTEGER li;
...
void get_cpu_times( boost_kahypar::timer::cpu_times& current )
{
    static const boost_kahypar::long_long_type freq = query_performance_frequency();

    LARGE_INTEGER li;
    ::QueryPerformanceCounter( &li ); // never fails

    boost_kahypar::long_long_type ctr = li.QuadPart;

    boost_kahypar::long_long_type const nano = INT64_C( 1000000000 ); // ns

    // ctr * nano / freq, but with less overflow

    boost_kahypar::long_long_type whole = (ctr / freq) * nano;
    boost_kahypar::long_long_type part  = (ctr % freq) * nano / freq;

    current.wall = whole + part;
    current.user = boost_kahypar::timer::nanosecond_type( -1 );
    current.system = boost_kahypar::timer::nanosecond_type( -1 );
    ...
    current.system = ( ( boost_kahypar::timer::nanosecond_type( kernel.dwHighDateTime ) << 32 ) + kernel.dwLowDateTime ) * 100;
    current.user = ( ( boost_kahypar::timer::nanosecond_type( user.dwHighDateTime ) << 32 ) + user.dwLowDateTime ) * 100;
...
boost_kahypar::int_least64_t tick_factor()
{
    boost_kahypar::int_least64_t tf = ::sysconf( _SC_CLK_TCK );
    if( tf <= 0 ) return -1;
...
void get_cpu_times( boost_kahypar::timer::cpu_times& current )
{
    current.wall = boost_kahypar::timer::nanosecond_type( -1 );
    current.user = boost_kahypar::timer::nanosecond_type( -1 );
    current.system = boost_kahypar::timer::nanosecond_type( -1 );

    static boost_kahypar::int_least64_t tf = tick_factor();
    ...
    current.wall = boost_kahypar::timer::nanosecond_type( c ) * tf;
    current.system = boost_kahypar::timer::nanosecond_type( tm.tms_stime + tm.tms_cstime ) * tf;
    current.user = boost_kahypar::timer::nanosecond_type( tm.tms_utime + tm.tms_cutime ) * tf;
}
