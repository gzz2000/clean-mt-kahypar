...
using namespace boost_kahypar::timer;
...
namespace {
boost_kahypar::int64_t max_cycles;

template< class STD_STRING >
nanosecond_type time_ctor(const STD_STRING& s)
{
    boost_kahypar::timer::auto_cpu_timer tmr;
    boost_kahypar::int64_t count = 0;
    do
    {
        fs::path p(s);
        ++count;
    ...
    boost_kahypar::timer::cpu_times elapsed = tmr.elapsed();
