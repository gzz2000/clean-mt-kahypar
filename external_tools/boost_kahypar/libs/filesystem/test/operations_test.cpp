...
#include <boost_kahypar/filesystem/fstream.hpp> // for BOOST_FILESYSTEM_C_STR
...
using boost_kahypar::system::system_error;
...
struct poison_category_impl : public boost_kahypar::system::error_category
{
    char const* name() const noexcept { return "poison"; }
    std::string message(int) const { return "poison_category::message"; }
};

boost_kahypar::system::error_category& poison_category()
{
...
void directory_iterator_tests()
{
    ...
    try
    {
        ...
        BOOST_TEST(ecx);
        BOOST_TEST(ecx == boost_kahypar::system::errc::no_such_file_or_directory);
    }
    catch (const fs::filesystem_error&)
    {
        dir_itr_exception = true;
...
void remove_all_tests(const fs::path& dirx)
{
    ...
    // remove_all() directory tree
    {
        ...
        boost_kahypar::uintmax_t removed_count = fs::remove_all(d1x);
...
void remove_all_symlink_tests(const fs::path& dirx)
{
    ...
    boost_kahypar::uintmax_t removed_count = fs::remove_all(d2x);
    BOOST_TEST_EQ(removed_count, created_count);
...
void creation_time_tests(const fs::path& dirx)
{
    ...
    {
        if (e.code() == make_error_condition(boost_kahypar::system::errc::function_not_supported))
        {
...
void symlink_file_size_tests()
{
    ...
    boost_kahypar::uintmax_t size = fs::file_size(sym_f1);
...
#error neither BOOST_POSIX_API nor BOOST_WINDOWS_API is defined. See boost/system/api_config.hpp
    ...
    cout << "sizeof(boost_kahypar::uintmax_t) = " << sizeof(boost_kahypar::uintmax_t) << '\n';
    BOOST_TEST(sizeof(boost_kahypar::uintmax_t) >= 8);
