...
#include <boost_kahypar/filesystem.hpp> // make sure filesystem.hpp works
...
using namespace boost_kahypar::filesystem;
...
void query_test()
{
    ...
    CHECK(file_size("no-such-file", ec) == static_cast< boost_kahypar::uintmax_t >(-1));
    ...
    CHECK(!boost_kahypar::filesystem::is_empty("/"));
    CHECK(!is_other("/"));
}
...
void operations_test()
{
    ...
    CHECK(!boost_kahypar::filesystem::remove("no-such-file-or-directory"));
...
void directory_entry_test(path const& temp_dir)
{
    ...
    boost_kahypar::filesystem::remove(temp_dir / "bar.foo");
    boost_kahypar::filesystem::remove(temp_dir / "goo.bar");
    boost_kahypar::filesystem::remove(temp_dir / "foo.bar");
}
...
void error_handling_test()
{
    ...
    catch (const boost_kahypar::filesystem::filesystem_error& ex)
    {
        threw = true;
