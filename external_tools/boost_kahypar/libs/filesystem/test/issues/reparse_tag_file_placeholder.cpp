...
// Test correct boost_kahypar::filesystem::status when reparse point ReparseTag set to IO_REPARSE_TAG_FILE_PLACEHOLDER
...
int main()
{
    boost_kahypar::filesystem::path rpt = boost_kahypar::filesystem::temp_directory_path() / boost_kahypar::filesystem::unique_path(L"bfs_reparse_point_test-%%%%-%%%%.txt");
    ...
    BOOST_TEST_NO_THROW(BOOST_TEST(boost_kahypar::filesystem::status(rpt).type() == boost_kahypar::filesystem::reparse_file));
    BOOST_TEST_NO_THROW(BOOST_TEST(boost_kahypar::filesystem::remove(rpt)));
