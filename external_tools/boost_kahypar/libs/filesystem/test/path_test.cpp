...
using boost_kahypar::next;
using boost_kahypar::prior;
...
void check(const fs::path& source, const std::string& expected, const char* file, int line)
{
    ...
    ++::boost_kahypar::detail::test_errors();
}
...
void exception_tests()
{
    ...
    boost_kahypar::system::error_code ec(12345, boost_kahypar::system::system_category());
    try
    {
...
void iterator_tests()
{
    ...
    BOOST_TEST(boost_kahypar::next(itr_ck.begin()) == itr_ck.end());
    BOOST_TEST(*boost_kahypar::prior(itr_ck.end()) == std::string("foo"));
    BOOST_TEST(boost_kahypar::prior(itr_ck.end()) == itr_ck.begin());

    itr_ck = path("/foo");
    BOOST_TEST((itr_ck.begin())->string() == "/");
    BOOST_TEST(*boost_kahypar::next(itr_ck.begin()) == std::string("foo"));
    BOOST_TEST(boost_kahypar::next(boost_kahypar::next(itr_ck.begin())) == itr_ck.end());
    BOOST_TEST(boost_kahypar::next(itr_ck.begin()) == boost_kahypar::prior(itr_ck.end()));
    BOOST_TEST(*boost_kahypar::prior(itr_ck.end()) == std::string("foo"));
    BOOST_TEST(*boost_kahypar::prior(boost_kahypar::prior(itr_ck.end())) == std::string("/"));
    BOOST_TEST(boost_kahypar::prior(boost_kahypar::prior(itr_ck.end())) == itr_ck.begin());
...
void construction_tests()
{
    ...
    PATH_TEST_EQ(boost_kahypar::string_view("foo"), "foo");
#if !defined(BOOST_NO_CXX17_HDR_STRING_VIEW)
...
#define APPEND_TEST(pth, appnd, expected)\
    {\
        ...
        path p5(p);\
        p5 /= boost_kahypar::string_view(appnd);\
...
#define CONCAT_TEST(pth, appnd, expected)\
    {\
        ...
        path p8(p);\
        p8 += boost_kahypar::string_view(appnd);\
...
#pragma clang diagnostic push
// explicitly assigning value of variable of type 'boost_kahypar::filesystem::path' to itself
...
void compare_tests()
{
    ...
    COMPARE_TEST(boost_kahypar::string_view("foo"), boost_kahypar::string_view("zoo"))
#if !defined(BOOST_NO_CXX17_HDR_STRING_VIEW)
...
static boost_kahypar::filesystem::path ticket_6737 = "FilePath"; // #6737 reported this crashed
                                                         // on VC++ debug mode build
const boost_kahypar::filesystem::path ticket_6690("test");       // #6690 another V++ static init crash
