...
mbpath_traits::to_external(const mbpath& ph, const internal_string_type& src)
{
    ...
    boost_kahypar::scoped_array< char > work(new char[work_size]);
    std::mbstate_t state;
    ...
    if (cvt->out(
        ...
        boost_kahypar::throw_exception< fs::basic_filesystem_error< mbpath > >(
            fs::basic_filesystem_error< mbpath >(
                "user::mbpath::to_external conversion error",
                ph, boost_kahypar::system::error_code(EINVAL, boost_kahypar::system::errno_ecat)));
    *to_next = '\0';
...
mbpath_traits::to_internal(const external_string_type& src)
{
    ...
    boost_kahypar::scoped_array< wchar_t > work(new wchar_t[work_size]);
    std::mbstate_t state;
    ...
    if (cvt->in(
        ...
        boost_kahypar::throw_exception< fs::basic_filesystem_error< mbpath > >(
            fs::basic_filesystem_error< mbpath >(
                ...
                boost_kahypar::system::error_code(EINVAL, boost_kahypar::system::errno_ecat)));
    *to_next = L'\0';
