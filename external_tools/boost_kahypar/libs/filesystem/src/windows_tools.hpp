...
using unique_handle = boost_kahypar::scope::unique_resource< HANDLE, handle_deleter, handle_resource_traits >;
...
inline boost_kahypar::filesystem::perms make_permissions(boost_kahypar::filesystem::path const& p, DWORD attr)
{
    boost_kahypar::filesystem::perms prms = boost_kahypar::filesystem::owner_read | boost_kahypar::filesystem::group_read | boost_kahypar::filesystem::others_read;
    if ((attr & FILE_ATTRIBUTE_READONLY) == 0u)
        prms |= boost_kahypar::filesystem::owner_write | boost_kahypar::filesystem::group_write | boost_kahypar::filesystem::others_write;
    boost_kahypar::filesystem::path ext = detail::path_algorithms::extension_v4(p);
    ...
    if (equal_extension(q, L".exe", L".EXE") || equal_extension(q, L".com", L".COM") || equal_extension(q, L".bat", L".BAT") || equal_extension(q, L".cmd", L".CMD"))
        prms |= boost_kahypar::filesystem::owner_exe | boost_kahypar::filesystem::group_exe | boost_kahypar::filesystem::others_exe;
    return prms;
}

ULONG get_reparse_point_tag_ioctl(HANDLE h, boost_kahypar::filesystem::path const& p, boost_kahypar::system::error_code* ec);
...
struct io_status_block
{
    union
    {
        boost_kahypar::winapi::NTSTATUS_ Status;
        PVOID Pointer;
    };
...
typedef boost_kahypar::winapi::NTSTATUS_ (NTAPI NtCreateFile_t)(
...
typedef boost_kahypar::winapi::NTSTATUS_ (NTAPI NtQueryDirectoryFile_t)(
