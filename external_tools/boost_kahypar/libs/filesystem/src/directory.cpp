...
#include <cwchar>
#include <windows.h>
#include <boost_kahypar/winapi/basic_types.hpp> // NTSTATUS_
...
boost_kahypar::scope::unique_fd open_directory(path const& p, directory_options opts, system::error_code& ec)
{
    ec.clear();
    ...
    while (true)
    {
        ...
        if (BOOST_UNLIKELY(res < 0))
        {
            ...
            return boost_kahypar::scope::unique_fd();
        }
    ...
    boost_kahypar::scope::unique_fd fd(res);
    ...
    return fd;
#else
    return boost_kahypar::scope::unique_fd(res);
#endif
}
...
//! Opens a directory file and returns a file descriptor. Returns a negative value in case of error.
boost_kahypar::scope::unique_fd openat_directory(int basedir_fd, path const& p, directory_options opts, system::error_code& ec)
{
    ec.clear();
...
system::error_code dir_itr_create(boost_kahypar::intrusive_ptr< detail::dir_itr_imp >& imp, fs::path const& dir, directory_options opts, directory_iterator_params* params, fs::path& first_filename, fs::file_status&, fs::file_status&)
{
    ...
    boost_kahypar::intrusive_ptr< detail::dir_itr_imp > pimpl(new (extra_size) detail::dir_itr_imp());
    ...
    default:
        {
            ...
            if (data->NextEntryOffset == 0u)
            {
                ...
                boost_kahypar::winapi::NTSTATUS_ status = filesystem::detail::atomic_load_relaxed(nt_query_directory_file_api)
                (
                    imp.handle,
                    nullptr, // Event
                    nullptr, // ApcRoutine
...
system::error_code dir_itr_create(boost_kahypar::intrusive_ptr< detail::dir_itr_imp >& imp, fs::path const& dir, directory_options opts, directory_iterator_params* params, fs::path& first_filename, fs::file_status& sf, fs::file_status& symlink_sf)
{
    boost_kahypar::intrusive_ptr< detail::dir_itr_imp > pimpl(new (dir_itr_extra_size) detail::dir_itr_imp());
    if (BOOST_UNLIKELY(!pimpl))
        return make_error_code(system::errc::not_enough_memory);
    ...
    fallback_to_file_directory_information_format:
        {
            NtQueryDirectoryFile_t* nt_query_directory_file = filesystem::detail::atomic_load_relaxed(boost_kahypar::filesystem::detail::nt_query_directory_file_api);
            if (BOOST_UNLIKELY(!nt_query_directory_file))
                return error_code(ERROR_NOT_SUPPORTED, system_category());

            io_status_block iosb;
            boost_kahypar::winapi::NTSTATUS_ status = nt_query_directory_file
            (
                iterator_handle,
                nullptr, // Event
                nullptr, // ApcRoutine
...
bool is_empty_directory(boost_kahypar::scope::unique_fd&& fd, path const& p, error_code* ec)
{
#if !defined(BOOST_FILESYSTEM_USE_READDIR_R)
...
void directory_iterator_construct(directory_iterator& it, path const& p, directory_options opts, directory_iterator_params* params, system::error_code* ec)
{
    ...
    {
        emit_error(not_found_error_code, p, ec, "boost_kahypar::filesystem::directory_iterator::construct");
        return;
    }
    ...
    try
    {
        boost_kahypar::intrusive_ptr< detail::dir_itr_imp > imp;
        path filename;
        ...
        while (true)
        {
            if (result)
            {
                ...
                {
                    if (!ec)
                        BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::directory_iterator::construct", p, result));
                    *ec = result;
                }
    ...
    try
    {
        ...
        while (true)
        {
            ...
            if (BOOST_UNLIKELY(!!increment_ec)) // happens if filesystem is corrupt, such as on a damaged optical disc
            {
                ...
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::directory_iterator::operator++", error_path, increment_ec));
...
void recursive_directory_iterator_construct(recursive_directory_iterator& it, path const& dir_path, directory_options opts, system::error_code* ec)
{
    ...
    boost_kahypar::intrusive_ptr< detail::recur_dir_itr_imp > imp;
    if (!ec)
    {
...
void recursive_directory_iterator_pop(recursive_directory_iterator& it, system::error_code* ec)
{
    ...
    while (true)
    {
        ...
        if (BOOST_UNLIKELY(!!increment_ec))
        {
            ...
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::recursive_directory_iterator::pop", increment_ec));
