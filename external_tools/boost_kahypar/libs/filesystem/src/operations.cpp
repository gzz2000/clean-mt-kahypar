...
#include <boost_kahypar/predef/os/bsd/open.h>
#include <boost_kahypar/filesystem/config.hpp>
#include <boost_kahypar/filesystem/operations.hpp>
#include <boost_kahypar/filesystem/file_status.hpp>
#include <boost_kahypar/filesystem/exception.hpp>
#include <boost_kahypar/filesystem/directory.hpp>
...
#include <boost_kahypar/scope/unique_fd.hpp>
...
#include <boost_kahypar/winapi/dll.hpp> // get_proc_address, GetModuleHandleW
#include <cwchar>
#include <io.h>
#include <windows.h>
#include <winnt.h>
...
#include <boost_kahypar/filesystem/detail/header.hpp> // must be the last #include

namespace fs = boost_kahypar::filesystem;
using boost_kahypar::filesystem::path;
using boost_kahypar::filesystem::filesystem_error;
using boost_kahypar::filesystem::perms;
using boost_kahypar::system::error_code;
using boost_kahypar::system::system_category;
...
)
{
    ...
    if (err != 0)
    {
        ...
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::status", p, system::error_code(err, system::system_category())));
    ...
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::status");
        return fs::file_status(fs::status_error);
    }
#endif
...
)
{
    ...
    if (err != 0)
    {
        ...
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::symlink_status", p, system::error_code(err, system::system_category())));
    ...
    if (BOOST_UNLIKELY((path_stat.stx_mask & (STATX_TYPE | STATX_MODE)) != (STATX_TYPE | STATX_MODE)))
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::symlink_status");
        return fs::file_status(fs::status_error);
    }
#endif
...
{
    {
        ...
        const std::size_t buf_size = static_cast< std::size_t >(boost_kahypar::core::bit_ceil(static_cast< uint_least32_t >(buf_sz)));
...
)
{
    ...
    if (res != 0)
    {
        ...
        if (BOOST_UNLIKELY(!not_found_error(err)))
            emit_error(err, p, ec, "boost_kahypar::filesystem::remove");
...
inline bool remove_impl(path const& p, error_code* ec)
{
    ...
    {
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::remove", p, local_ec));
...
)
{
    ...
    for (unsigned int attempt = 0u; attempt < remove_all_directory_replaced_retry_count; ++attempt)
    {
        fs::file_type type;
        {
            ...
            {
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::remove_all", p, local_ec));
        ...
        if (type == fs::directory_file) // but not a directory symlink
        {
            ...
            if (BOOST_UNLIKELY(!!dit_create_ec))
            {
                ...
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::remove_all", p, dit_create_ec));
    ...
    if (!ec)
        BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::remove_all: path cannot be opened as a directory", p, dit_create_ec));
...
{
    boost_kahypar::winapi::HMODULE_ h = boost_kahypar::winapi::GetModuleHandleW(L"kernel32.dll");
    if (BOOST_LIKELY(!!h))
    {
        GetFileInformationByHandleEx_t* get_file_information_by_handle_ex = (GetFileInformationByHandleEx_t*)boost_kahypar::winapi::get_proc_address(h, "GetFileInformationByHandleEx");
        filesystem::detail::atomic_store_relaxed(get_file_information_by_handle_ex_api, get_file_information_by_handle_ex);
        SetFileInformationByHandle_t* set_file_information_by_handle = (SetFileInformationByHandle_t*)boost_kahypar::winapi::get_proc_address(h, "SetFileInformationByHandle");
        filesystem::detail::atomic_store_relaxed(set_file_information_by_handle_api, set_file_information_by_handle);
        filesystem::detail::atomic_store_relaxed(create_hard_link_api, (CreateHardLinkW_t*)boost_kahypar::winapi::get_proc_address(h, "CreateHardLinkW"));
        filesystem::detail::atomic_store_relaxed(create_symbolic_link_api, (CreateSymbolicLinkW_t*)boost_kahypar::winapi::get_proc_address(h, "CreateSymbolicLinkW"));
    ...
    h = boost_kahypar::winapi::GetModuleHandleW(L"ntdll.dll");
    if (BOOST_LIKELY(!!h))
    {
        filesystem::detail::atomic_store_relaxed(nt_create_file_api, (NtCreateFile_t*)boost_kahypar::winapi::get_proc_address(h, "NtCreateFile"));
        filesystem::detail::atomic_store_relaxed(nt_query_directory_file_api, (NtQueryDirectoryFile_t*)boost_kahypar::winapi::get_proc_address(h, "NtQueryDirectoryFile"));
    }
...
boost_kahypar::winapi::NTSTATUS_ nt_create_file_handle_at
(
    unique_handle& out,
    HANDLE basedir_handle,
    boost_kahypar::filesystem::path const& p,
...
)
{
    ...
    boost_kahypar::winapi::NTSTATUS_ status = nt_create_file
    (
        &out_handle,
        DesiredAccess,
        &obj_attrs,
...
inline fs::file_status process_status_failure(DWORD errval, path const& p, error_code* ec)
{
    ...
    if (!ec)
        BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::status", p, error_code(errval, system_category())));
...
uintmax_t remove_all_nt6_by_handle(HANDLE h, path const& p, error_code* ec)
{
    ...
    if (st.type() == fs::directory_file)
    {
        ...
        while (itr != end_dit)
        {
            ...
            if (BOOST_LIKELY(nt_create_file != nullptr))
            {
                ...
                boost_kahypar::winapi::NTSTATUS_ status = nt_create_file_handle_at
                (
                    hh,
                    h,
                ...
                if (!NT_SUCCESS(status))
                {
                    ...
                    emit_error(err, nested_path, ec, "boost_kahypar::filesystem::remove_all");
                    return static_cast< uintmax_t >(-1);
                }
            }
            else
    ...
    if (BOOST_UNLIKELY(err != 0u))
    {
        emit_error(err, p, ec, "boost_kahypar::filesystem::remove_all");
        return static_cast< uintmax_t >(-1);
    }
...
path absolute_v3(path const& p, path const& base, system::error_code* ec)
{
    ...
    if (!base.is_absolute())
    {
        ...
        if (BOOST_UNLIKELY(!cur_path.is_absolute()))
        {
            ...
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::absolute", p, base, local_ec));
...
path canonical_v3(path const& p, path const& base, system::error_code* ec)
{
    ...
    fail_local_ec:
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::canonical", source, local_ec));
...
void copy(path const& from, path const& to, copy_options options, system::error_code* ec)
{
    ...
    if (!exists(from_stat))
    {
        emit_error(BOOST_ERROR_FILE_NOT_FOUND, from, to, ec, "boost_kahypar::filesystem::copy");
        return;
    }
    ...
    else if (is_regular_file(from_stat))
    {
        ...
        {
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::copy", from, to, local_ec));
            *ec = local_ec;
            return;
        }
    ...
    {
    fail:
        emit_error(BOOST_ERROR_NOT_SUPPORTED, from, to, ec, "boost_kahypar::filesystem::copy");
    }
}
...
bool copy_file(path const& from, path const& to, copy_options options, error_code* ec)
{
    ...
    boost_kahypar::scope::unique_fd infile, outfile;
    ...
    while (true)
    {
        ...
        fail:
            emit_error(err, from, to, ec, "boost_kahypar::filesystem::copy_file");
            return false;
        }
...
bool create_directories(path const& p, system::error_code* ec)
{
    if (p.empty())
    {
        if (!ec)
        {
            BOOST_FILESYSTEM_THROW(filesystem_error(
                "boost_kahypar::filesystem::create_directories", p,
    ...
    for (path fname = path_algorithms::filename_v4(parent); parent.has_relative_path(); fname = path_algorithms::filename_v4(parent))
    {
        if (!fname.empty() && path_algorithms::compare_v4(fname, dot_p) != 0 && path_algorithms::compare_v4(fname, dot_dot_p) != 0)
        {
            ...
            {
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::create_directories", p, parent, local_ec));
                *ec = local_ec;
                return false;
            }
        }
...
bool create_directory(path const& p, const path* existing, error_code* ec)
{
    ...
    if (existing)
    {
        ...
        {
            emit_error(errno, p, *existing, ec, "boost_kahypar::filesystem::create_directory");
            return false;
        }

        if (BOOST_UNLIKELY((existing_stat.stx_mask & (STATX_TYPE | STATX_MODE)) != (STATX_TYPE | STATX_MODE)))
        {
            emit_error(BOOST_ERROR_NOT_SUPPORTED, p, *existing, ec, "boost_kahypar::filesystem::create_directory");
            return false;
        }
#else
        ...
        {
            emit_error(ENOTDIR, p, *existing, ec, "boost_kahypar::filesystem::create_directory");
            return false;
        }
    ...
    emit_error(errval, p, ec, "boost_kahypar::filesystem::create_directory");
    return false;
}
...
void create_directory_symlink(path const& to, path const& from, system::error_code* ec)
{
    ...
    {
        err = errno;
        emit_error(err, to, from, ec, "boost_kahypar::filesystem::create_directory_symlink");
    }
#else
    // see if actually supported by Windows runtime dll
    if (!create_symbolic_link_api)
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec, "boost_kahypar::filesystem::create_directory_symlink");
        return;
    }
    ...
    {
        emit_error(BOOST_ERRNO, to, from, ec, "boost_kahypar::filesystem::create_directory_symlink");
    }
#endif
}
...
void create_hard_link(path const& to, path const& from, error_code* ec)
{
    ...
    {
        err = errno;
        emit_error(err, to, from, ec, "boost_kahypar::filesystem::create_hard_link");
    }
#else
    // see if actually supported by Windows runtime dll
    ...
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec, "boost_kahypar::filesystem::create_hard_link");
        return;
    }

    if (BOOST_UNLIKELY(!chl_api(from.c_str(), to.c_str(), nullptr)))
    {
        emit_error(BOOST_ERRNO, to, from, ec, "boost_kahypar::filesystem::create_hard_link");
    }
#endif
}
...
void create_symlink(path const& to, path const& from, error_code* ec)
{
    ...
    {
        err = errno;
        emit_error(err, to, from, ec, "boost_kahypar::filesystem::create_symlink");
    }
#else
    // see if actually supported by Windows runtime dll
    ...
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec, "boost_kahypar::filesystem::create_symlink");
        return;
    }

    if (BOOST_UNLIKELY(!csl_api(from.c_str(), to.c_str(), SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE)))
    {
        emit_error(BOOST_ERRNO, to, from, ec, "boost_kahypar::filesystem::create_symlink");
    }
#endif
}
...
path current_path(error_code* ec)
{
    ...
    emit_error(BOOST_ERROR_NOT_SUPPORTED, ec, "boost_kahypar::filesystem::current_path");
    return path();
    ...
    struct local
    {
        static bool getcwd_error(error_code* ec)
        {
            ...
            return error((err != ERANGE
                         ...
                         ec, "boost_kahypar::filesystem::current_path");
        }
    };
    ...
    else if (BOOST_LIKELY(!local::getcwd_error(ec)))
    {
        for (std::size_t path_max = sizeof(small_buf) * 2u;; path_max *= 2u) // loop 'til buffer large enough
        {
            ...
            {
                emit_error(ENAMETOOLONG, ec, "boost_kahypar::filesystem::current_path");
                break;
            }
    ...
    error(::GetCurrentDirectoryW(sz, buf.get()) == 0 ? BOOST_ERRNO : 0, ec, "boost_kahypar::filesystem::current_path");
    return path(buf.get());
#endif
}
...
{
#if defined(BOOST_FILESYSTEM_USE_WASI)
    emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::current_path");
#else
    error(!BOOST_SET_CURRENT_DIRECTORY(p.c_str()) ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::current_path");
#endif
}
...
bool equivalent_v3(path const& p1, path const& p2, system::error_code* ec)
{
    ...
    if (BOOST_LIKELY(e2 == 0))
    {
        ...
        {
        fail_unsupported:
            emit_error(BOOST_ERROR_NOT_SUPPORTED, p1, p2, ec, "boost_kahypar::filesystem::equivalent");
            return false;
        }
    }
    ...
    if (BOOST_UNLIKELY(e1 != 0 || e2 != 0))
    {
        ...
        {
            int err = errno;
            emit_error(err, p1, p2, ec, "boost_kahypar::filesystem::equivalent");
        }
        return false;
    }
    ...
    if (BOOST_UNLIKELY(!h1 || !h2))
    {
        ...
        // but if both are invalid then it is an error
        if (!h1 && !h2)
            error(BOOST_ERRNO, p1, p2, ec, "boost_kahypar::filesystem::equivalent");
        return false;
    }
    ...
    if (error(!::GetFileInformationByHandle(h1.get(), &info1) ? BOOST_ERRNO : 0, p1, p2, ec, "boost_kahypar::filesystem::equivalent"))
        return false;

    if (error(!::GetFileInformationByHandle(h2.get(), &info2) ? BOOST_ERRNO : 0, p1, p2, ec, "boost_kahypar::filesystem::equivalent"))
        return false;
...
uintmax_t file_size(path const& p, error_code* ec)
{
    ...
    if (BOOST_UNLIKELY(invoke_statx(AT_FDCWD, p.c_str(), AT_NO_AUTOMOUNT, STATX_TYPE | STATX_SIZE, &path_stat) < 0))
    {
        err = errno;
    fail:
        emit_error(err, p, ec, "boost_kahypar::filesystem::file_size");
        return static_cast< uintmax_t >(-1);
    }
...
uintmax_t hard_link_count(path const& p, system::error_code* ec)
{
    ...
    {
        emit_error(errno, p, ec, "boost_kahypar::filesystem::hard_link_count");
        return static_cast< uintmax_t >(-1);
    }

    if (BOOST_UNLIKELY((path_stat.stx_mask & STATX_NLINK) != STATX_NLINK))
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::hard_link_count");
        return static_cast< uintmax_t >(-1);
    }
    ...
    {
    fail_errno:
        emit_error(BOOST_ERRNO, p, ec, "boost_kahypar::filesystem::hard_link_count");
        return static_cast< uintmax_t >(-1);
    }
...
(
#if defined(BOOST_POSIX_API)
    boost_kahypar::scope::unique_fd&& fd,
#else
    unique_handle&& h,
#endif
    path const& p,
...
bool is_empty(path const& p, system::error_code* ec)
{
    ...
    boost_kahypar::scope::unique_fd file;
    int err = 0;
    while (true)
    {
        ...
        fail:
            emit_error(err, p, ec, "boost_kahypar::filesystem::is_empty");
            return false;
        }
...
std::time_t creation_time(path const& p, system::error_code* ec)
{
    ...
    {
        emit_error(BOOST_ERRNO, p, ec, "boost_kahypar::filesystem::creation_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
    if (BOOST_UNLIKELY((stx.stx_mask & STATX_BTIME) != STATX_BTIME))
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::creation_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
    ...
    fail_errno:
        err = BOOST_ERRNO;
    fail:
        emit_error(err, p, ec, "boost_kahypar::filesystem::creation_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
...
std::time_t last_write_time(path const& p, system::error_code* ec)
{
    ...
    {
        emit_error(BOOST_ERRNO, p, ec, "boost_kahypar::filesystem::last_write_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
    if (BOOST_UNLIKELY((stx.stx_mask & STATX_MTIME) != STATX_MTIME))
    {
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::last_write_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
    ...
    fail_errno:
        err = BOOST_ERRNO;
    fail:
        emit_error(err, p, ec, "boost_kahypar::filesystem::last_write_time");
        return (std::numeric_limits< std::time_t >::min)();
    }
...
void permissions(path const& p, perms prms, system::error_code* ec)
{
...
#if defined(BOOST_FILESYSTEM_USE_WASI)
    emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::permissions");
#elif defined(BOOST_POSIX_API)
    error_code local_ec;
    ...
    {
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::permissions", p, local_ec));
...
#endif
    {
        ...
        {
            BOOST_FILESYSTEM_THROW(filesystem_error(
                "boost_kahypar::filesystem::permissions", p, error_code(err, system::generic_category())));
        }
    ...
    if (error(attr == 0 ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::permissions"))
        return;
    ...
    error(::SetFileAttributesW(p.c_str(), attr) == 0 ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::permissions");
#endif
}
...
path read_symlink(path const& p, system::error_code* ec)
{
    ...
    fail:
        const int err = errno;
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::read_symlink", p, error_code(err, system_category())));
    ...
    else
    {
        for (std::size_t path_max = sizeof(small_buf) * 2u;; path_max *= 2u) // loop 'til buffer large enough
        {
            ...
            {
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::read_symlink", p, error_code(ENAMETOOLONG, system_category())));
    ...
    return_last_error:
        error = ::GetLastError();
        emit_error(error, p, ec, "boost_kahypar::filesystem::read_symlink");
        return symlink_path;
    }
    ...
    default:
        emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "Unknown ReparseTag in boost_kahypar::filesystem::read_symlink");
        return symlink_path;
    }
...
path relative(path const& p, path const& base, error_code* ec)
{
    ...
    if (!p.is_absolute() || !base.is_absolute())
    {
        ...
        fail_local_ec:
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::relative", p, base, local_ec));
...
{
    error(!BOOST_MOVE_FILE(old_p.c_str(), new_p.c_str()) ? BOOST_ERRNO : 0, old_p, new_p, ec, "boost_kahypar::filesystem::rename");
}
...
void resize_file(path const& p, uintmax_t size, system::error_code* ec)
{
    ...
    {
        emit_error(system::errc::file_too_large, p, ec, "boost_kahypar::filesystem::resize_file");
        return;
    }
#endif
    error(!BOOST_RESIZE_FILE(p.c_str(), size) ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::resize_file");
}
...
space_info space(path const& p, error_code* ec)
{
    ...
    emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec, "boost_kahypar::filesystem::space");

#elif defined(BOOST_POSIX_API)

    struct BOOST_STATVFS vfs;
    if (!error(::BOOST_STATVFS(p.c_str(), &vfs) ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::space"))
    {
    ...
    fail_local_ec:
        if (!ec)
            BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::space", p, local_ec));
        *ec = local_ec;
        return info;
    }
    ...
    if (!error(::GetDiskFreeSpaceExW(str.c_str(), &avail, &total, &free) == 0 ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::space"))
    {
...
path temp_directory_path(system::error_code* ec)
{
    ...
    {
    fail_not_dir:
        error(ENOTDIR, p, ec, "boost_kahypar::filesystem::temp_directory_path");
        return p;
    }
    ...
    if (p.empty())
    {
        ...
        getwindir_error:
            ...
            error(errval, ec, "boost_kahypar::filesystem::temp_directory_path");
            return path();
        }
...
path system_complete(path const& p, system::error_code* ec)
{
    ...
    if (error(len == 0 ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::system_complete"))
        return path();
    ...
    return error(get_full_path_name(p, len, big_buf.get(), &pfn) == 0 ? BOOST_ERRNO : 0, p, ec, "boost_kahypar::filesystem::system_complete") ? path() : path(big_buf.get());
...
path weakly_canonical_v3(path const& p, path const& base, system::error_code* ec)
{
    ...
    for (; !head.empty(); path_algorithms::decrement_v4(itr))
    {
        ...
        {
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("boost_kahypar::filesystem::weakly_canonical", head, local_ec));
...
#include <boost_kahypar/filesystem/detail/footer.hpp>
