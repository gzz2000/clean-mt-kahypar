...
#include <boost_kahypar/filesystem/detail/path_traits.hpp> // codecvt_error_category()
#include <boost_kahypar/system/error_category.hpp> // for BOOST_SYSTEM_HAS_CONSTEXPR
...
#include <boost_kahypar/filesystem/detail/utf8_codecvt_facet.hpp>
#endif
...
using boost_kahypar::filesystem::detail::colon;
using boost_kahypar::filesystem::detail::questionmark;

inline bool is_alnum(wchar_t c)
{
    return boost_kahypar::filesystem::detail::is_letter(c) || (c >= L'0' && c <= L'9');
}
...
inline size_type find_separator(const wchar_t* p, size_type size) noexcept
{
    ...
    for (; pos < size; ++pos)
    {
        ...
        if (boost_kahypar::filesystem::detail::is_directory_separator(c))
            break;
    }
    return pos;
}
...
std::locale default_locale()
{
    ...
    return std::locale(global_loc, new boost_kahypar::filesystem::detail::windows_file_codecvt());
#elif defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__HAIKU__)
    ...
    std::locale global_loc = std::locale();
    return std::locale(global_loc, new boost_kahypar::filesystem::detail::utf8_codecvt_facet());
#else // Other POSIX
...
const boost_kahypar::filesystem::path* g_dot_path = nullptr;
const boost_kahypar::filesystem::path* g_dot_dot_path = nullptr;

inline void schedule_path_locale_cleanup() noexcept
{
}

inline boost_kahypar::filesystem::path const& get_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const boost_kahypar::filesystem::path* > a(g_dot_path);
    const boost_kahypar::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const boost_kahypar::filesystem::path* new_p = new boost_kahypar::filesystem::path(dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const boost_kahypar::filesystem::path* p = g_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_path = p = new boost_kahypar::filesystem::path(dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}

inline boost_kahypar::filesystem::path const& get_dot_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const boost_kahypar::filesystem::path* > a(g_dot_dot_path);
    const boost_kahypar::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const boost_kahypar::filesystem::path* new_p = new boost_kahypar::filesystem::path(dot_dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const boost_kahypar::filesystem::path* p = g_dot_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_dot_path = p = new boost_kahypar::filesystem::path(dot_dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}
...
BOOST_FILESYSTEM_INIT_FUNC init_path_globals()
{
    ...
    boost_kahypar::filesystem::codecvt_error_category();
#endif
...
const boost_kahypar::filesystem::path g_dot_path(dot_path_literal);
BOOST_FILESYSTEM_INIT_PRIORITY(BOOST_FILESYSTEM_PATH_GLOBALS_INIT_PRIORITY)
const boost_kahypar::filesystem::path g_dot_dot_path(dot_dot_path_literal);
...
inline boost_kahypar::filesystem::path const& get_dot_path()
{
    static const boost_kahypar::filesystem::path g_dot_path(dot_path_literal);
    return g_dot_path;
}

inline boost_kahypar::filesystem::path const& get_dot_dot_path()
{
    static const boost_kahypar::filesystem::path g_dot_dot_path(dot_dot_path_literal);
    return g_dot_dot_path;
}
