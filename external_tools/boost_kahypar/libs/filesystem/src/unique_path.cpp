...
#include <boost_kahypar/winapi/crypt.hpp>
...
void system_crypt_random(void* buf, std::size_t len, boost_kahypar::system::error_code* ec)
{
#if defined(BOOST_POSIX_API)
    ...
    if (BOOST_UNLIKELY(err != 0))
        emit_error(err, ec, "boost_kahypar::filesystem::unique_path");
    ...
    boost_kahypar::winapi::NTSTATUS_ status = boost_kahypar::winapi::BCryptOpenAlgorithmProvider(&handle, boost_kahypar::winapi::BCRYPT_RNG_ALGORITHM_, nullptr, 0);
    if (BOOST_UNLIKELY(status != 0))
    {
    fail:
        emit_error(translate_ntstatus(status), ec, "boost_kahypar::filesystem::unique_path");
        return;
    }

    status = boost_kahypar::winapi::BCryptGenRandom(handle, static_cast< boost_kahypar::winapi::PUCHAR_ >(buf), static_cast< boost_kahypar::winapi::ULONG_ >(len), 0);
    ...
    boost_kahypar::winapi::HCRYPTPROV_ handle;
    boost_kahypar::winapi::DWORD_ err = 0u;
    if (BOOST_UNLIKELY(!boost_kahypar::winapi::CryptAcquireContextW(&handle, nullptr, nullptr, boost_kahypar::winapi::PROV_RSA_FULL_, boost_kahypar::winapi::CRYPT_VERIFYCONTEXT_ | boost_kahypar::winapi::CRYPT_SILENT_)))
    {
        err = boost_kahypar::winapi::GetLastError();
    ...
    boost_kahypar::winapi::BOOL_ gen_ok = boost_kahypar::winapi::CryptGenRandom(handle, static_cast< boost_kahypar::winapi::DWORD_ >(len), static_cast< boost_kahypar::winapi::BYTE_* >(buf));

    if (BOOST_UNLIKELY(!gen_ok))
        err = boost_kahypar::winapi::GetLastError();

    boost_kahypar::winapi::CryptReleaseContext(handle, 0);
