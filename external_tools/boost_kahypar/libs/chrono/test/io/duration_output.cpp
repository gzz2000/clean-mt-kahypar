...
void test_good_symbol(const char* str, D d)
{
  ...
  out << boost_kahypar::chrono::duration_fmt(boost_kahypar::chrono::duration_style::symbol) << d;
#else
  out << boost_kahypar::chrono::duration_short  << d;
#endif
  BOOST_TEST(out.good());
...
void test_good(const char* str, D d, boost_kahypar::chrono::duration_style style)
{
  ...
  out << boost_kahypar::chrono::duration_fmt(style) << d;
  BOOST_TEST(out.good());
  BOOST_TEST(out.str() == str);
}

template<typename D>
void test_state_saver(const char* str, const char* str2, D d, boost_kahypar::chrono::duration_style style)
{
  std::ostringstream out;
  {
    boost_kahypar::chrono::duration_style_io_saver ios(out);
    out << boost_kahypar::chrono::duration_fmt(style) << d;
    BOOST_TEST(out.good());
    BOOST_TEST(out.str() == str);
  }
...
template<typename D>
void test_state_saver2(const char* str, const char* str2, D d, boost_kahypar::chrono::duration_style style)
{
  std::ostringstream out;
  {
    boost_kahypar::chrono::duration_style_io_saver ios(out, style);
    out << d;
    BOOST_TEST(out.good());
    BOOST_TEST(out.str() == str);
  }
...
int main()
{
  ...
  test_good_prefix("5000 deciseconds", duration<boost_kahypar::int_least64_t, deci> (5000));
  test_good_prefix("5000 [1/30]seconds", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (5000));
  ...
  test_good_symbol("5000 ds", duration<boost_kahypar::int_least64_t, deci> (5000));
  test_good_symbol("5000 [1/30]s", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (5000));
