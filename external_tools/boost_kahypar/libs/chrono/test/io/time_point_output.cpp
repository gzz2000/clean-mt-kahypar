...
#include <boost_kahypar/chrono/system_clocks.hpp>
...
void test_good_prefix(const char* str, D d)
{
  ...
  boost_kahypar::chrono::time_point<Clock, D> tp(d);
  out << tp;
  BOOST_TEST(out.good());
  //std::cout << "Expected= " << std::string(str) + boost_kahypar::chrono::clock_string<Clock, char>::since() << std::endl;
  //std::cout << "Obtained= " << out.str() << std::endl;
  BOOST_TEST( (out.str() == std::string(str) + boost_kahypar::chrono::clock_string<Clock, char>::since()));
}
...
void test_good_symbol(const char* str, D d)
{
  ...
  out << boost_kahypar::chrono::duration_fmt(boost_kahypar::chrono::duration_style::symbol) << tp;
#else
  out << boost_kahypar::chrono::duration_short << tp;
#endif
  BOOST_TEST(out.good());
  BOOST_TEST( (out.str() == std::string(str) + boost_kahypar::chrono::clock_string<Clock, char>::since()));
}
...
void test_good_utc_fmt_system_clock(const char* str, const char* fmt, D d)
{
  ...
  std::ostringstream out;
  boost_kahypar::chrono::time_point<Clock, D> tp(d);
  boost_kahypar::chrono::time_fmt_io_saver<> fmts(out);
  boost_kahypar::chrono::timezone_io_saver tzs(out);
  out << time_fmt(boost_kahypar::chrono::timezone::utc, fmt)  << tp;
  BOOST_TEST(out.good());
  std::cout << "Expected= " << str << std::endl;
  std::cout << "Obtained= " << out.str() << std::endl;
...
void test_good_utc_fmt_system_clock2(const char* str, const char* fmt, D d)
{
  ...
  std::ostringstream out;
  boost_kahypar::chrono::time_point<Clock, D> tp(d);
  boost_kahypar::chrono::time_fmt_io_saver<> fmts(out, fmt);
  boost_kahypar::chrono::timezone_io_saver tzs(out, boost_kahypar::chrono::timezone::utc);
  out << tp;
  BOOST_TEST(out.good());
  std::cout << "Expected= " << str << std::endl;
  std::cout << "Obtained= " << out.str() << std::endl;
...
template<typename Clock, typename D>
void test_good(const char* str, D d, boost_kahypar::chrono::duration_style style)
{
  std::ostringstream out;
  boost_kahypar::chrono::time_point<Clock,D> tp(d);
  out << boost_kahypar::chrono::duration_fmt(style) << tp;
  BOOST_TEST(out.good());
  BOOST_TEST((out.str() == std::string(str)+boost_kahypar::chrono::clock_string<Clock,char>::since()));
}

template<typename D>
void test_good_system_clock(const char* str, D d, boost_kahypar::chrono::duration_style style)
{
  typedef boost_kahypar::chrono::system_clock Clock;
...
void check_all()
{
  ...
  test_good_prefix<Clock> ("2 deciseconds", duration<boost_kahypar::int_least64_t, deci> (2));
  test_good_prefix<Clock> ("2 [1/30]seconds", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (2));
  ...
  test_good_symbol<Clock> ("2 ds", duration<boost_kahypar::int_least64_t, deci> (2));
  test_good_symbol<Clock> ("2 [1/30]s", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (2));
}
...
void check_all_system_clock()
{
  ...
  test_good_prefix_system_clock("1970-01-01 00:00:00.200000000 +0000", duration<boost_kahypar::int_least64_t, deci> (2));
  test_good_prefix_system_clock("1970-01-01 00:00:00.066666667 +0000", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (2));
  ...
  test_good_symbol_system_clock("1970-01-01 00:00:00.200000000 +0000", duration<boost_kahypar::int_least64_t, deci> (2));
  test_good_symbol_system_clock("1970-01-01 00:00:00.066666667 +0000", duration<boost_kahypar::int_least64_t, ratio<1, 30> > (2));
...
void test_gmtime(std::time_t t)
{
  ...
  if (boost_kahypar::chrono::detail::internal_gmtime(&t, &tm))
  {
    tm.tm_isdst = -1;
    (void)mktime(&tm);
    std::tm tm2;
...
int main()
{
  ...
  check_all<boost_kahypar::chrono::high_resolution_clock> ();
#ifdef BOOST_CHRONO_HAS_CLOCK_STEADY
  std::cout << "steady_clock=" << std::endl;
  check_all<boost_kahypar::chrono::steady_clock> ();
#endif
  std::cout << "system_clock=" << std::endl;
#if BOOST_CHRONO_VERSION >= 2  && defined BOOST_CHRONO_PROVIDES_DATE_IO_FOR_SYSTEM_CLOCK_TIME_POINT
  check_all_system_clock();
#else
  check_all<boost_kahypar::chrono::system_clock> ();
#endif

#if defined(BOOST_CHRONO_HAS_THREAD_CLOCK)
  std::cout << "thread_clock="<< std::endl;
  check_all<boost_kahypar::chrono::thread_clock>();
#endif

#if defined(BOOST_CHRONO_HAS_PROCESS_CLOCKS)
  std::cout << "process_real_cpu_clock=" << std::endl;
  check_all<boost_kahypar::chrono::process_real_cpu_clock> ();
#if ! BOOST_OS_WINDOWS || BOOST_PLAT_WINDOWS_DESKTOP
  std::cout << "process_user_cpu_clock=" << std::endl;
  check_all<boost_kahypar::chrono::process_user_cpu_clock> ();
  std::cout << "process_system_cpu_clock=" << std::endl;
  check_all<boost_kahypar::chrono::process_system_cpu_clock> ();
  std::cout << "process_cpu_clock=" << std::endl;
  check_all<boost_kahypar::chrono::process_cpu_clock> ();
#endif
#endif

#if defined BOOST_CHRONO_INTERNAL_GMTIME
#elif BOOST_CHRONO_VERSION == 2
  boost_kahypar::chrono::system_clock::time_point tp = boost_kahypar::chrono::system_clock::now();
  std::cout << tp << std::endl;
  time_t t = boost_kahypar::chrono::system_clock::to_time_t(tp);
  test_gmtime(  t );
#endif
