...
test(const FromDuration& df, const ToDuration& d)
{
    ...
    typedef boost_kahypar::chrono::time_point<Clock, FromDuration> FromTimePoint;
    typedef boost_kahypar::chrono::time_point<Clock, ToDuration> ToTimePoint;
    FromTimePoint f(df);
    ToTimePoint t(d);
//~ #if defined(BOOST_NO_CXX11_DECLTYPE)
    //~ typedef BOOST_TYPEOF_TPL(boost_kahypar::chrono::time_point_cast<ToDuration>(f)) R;
//~ #else
    //~ typedef decltype(boost_kahypar::chrono::time_point_cast<ToDuration>(f)) R;
//~ #endif
    //~ BOOST_CHRONO_STATIC_ASSERT((boost_kahypar::is_same<R, ToTimePoint>::value), NOTHING, ());
    BOOST_TEST(boost_kahypar::chrono::time_point_cast<ToDuration>(f) == t);
}
...
int main()
{
         ...
         boost_kahypar::chrono::duration<int, boost_kahypar::ratio<3, 5> >(10));
    {
      ...
      typedef boost_kahypar::chrono::time_point<Clock, boost_kahypar::chrono::hours> ToTimePoint;
      BOOST_CONSTEXPR FromTimePoint f(boost_kahypar::chrono::milliseconds(7265000));
      BOOST_CONSTEXPR ToTimePoint tph = boost_kahypar::chrono::time_point_cast<boost_kahypar::chrono::hours>(f);
