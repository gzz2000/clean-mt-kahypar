...
void __print_time(boost_kahypar::chrono::system_clock::time_point t)
{
...
void sleep_for(const boost_kahypar::chrono::duration<Rep, Period>& d)
{
    boost_kahypar::chrono::microseconds t = boost_kahypar::chrono::duration_cast<boost_kahypar::chrono::microseconds>(d);
    if (t < d)
        ++t;
    if (t > boost_kahypar::chrono::microseconds(0))
        std::cout << "sleep_for " << t.count() << " microseconds\n";
}

template <class Clock, class Duration>
void sleep_until(const boost_kahypar::chrono::time_point<Clock, Duration>& t)
{
    using namespace boost_kahypar::chrono;
    ...
    if (t > Clock::now())
    {
        typedef typename boost_kahypar::common_type<typename Time::duration,
...
struct timed_mutex
{
    ...
    template <class Rep, class Period>
        bool try_lock_for(const boost_kahypar::chrono::duration<Rep, Period>& d)
        {
            boost_kahypar::chrono::microseconds t = boost_kahypar::chrono::duration_cast<boost_kahypar::chrono::microseconds>(d);
            if (t <= boost_kahypar::chrono::microseconds(0))
                return try_lock();
    ...
    template <class Clock, class Duration>
    bool try_lock_until(const boost_kahypar::chrono::time_point<Clock, Duration>& t)
    {
        using namespace boost_kahypar::chrono;
        typedef time_point<Clock, Duration> Time;
        typedef system_clock::time_point SysTime;
...
{
    template <class Rep, class Period>
        bool wait_for(mutex&, const boost_kahypar::chrono::duration<Rep, Period>& d)
        {
            boost_kahypar::chrono::microseconds t = boost_kahypar::chrono::duration_cast<boost_kahypar::chrono::microseconds>(d);
    ...
    template <class Clock, class Duration>
    bool wait_until(mutex&, const boost_kahypar::chrono::time_point<Clock, Duration>& t)
    {
        using namespace boost_kahypar::chrono;
        typedef time_point<Clock, Duration> Time;
        typedef system_clock::time_point SysTime;
...
namespace User1
{
// Example type-safe "physics" code interoperating with boost_kahypar::chrono::duration types
//  and taking advantage of the boost_kahypar::ratio infrastructure and design philosophy.

// length - mimics boost_kahypar::chrono::duration except restricts representation to double.
//    Uses boost_kahypar::ratio facilities for length units conversions.
...
public:
    ...
    length(const length<R>& d)
            : len_(d.count() * boost_kahypar::ratio_divide<Ratio, R>::type::den /
                               boost_kahypar::ratio_divide<Ratio, R>::type::num) {}
...
typedef length<boost_kahypar::ratio<1> >          meter;        // set meter as "unity"
typedef length<boost_kahypar::centi>              centimeter;   // 1/100 meter
typedef length<boost_kahypar::kilo>               kilometer;    // 1000  meters
typedef length<boost_kahypar::ratio<254, 10000> > inch;         // 254/10000 meters
// length takes ratio instead of two integral types so that definitions can be made like so:
typedef length<boost_kahypar::ratio_multiply<boost_kahypar::ratio<12>, inch::ratio>::type>   foot;  // 12 inchs
typedef length<boost_kahypar::ratio_multiply<boost_kahypar::ratio<5280>, foot::ratio>::type> mile;  // 5280 feet

// Need a floating point definition of seconds
typedef boost_kahypar::chrono::duration<double> seconds;                         // unity
// Demo of (scientific) support for sub-nanosecond resolutions
typedef boost_kahypar::chrono::duration<double,  boost_kahypar::pico> picosecond;  // 10^-12 seconds
typedef boost_kahypar::chrono::duration<double, boost_kahypar::femto> femtosecond; // 10^-15 seconds
typedef boost_kahypar::chrono::duration<double,  boost_kahypar::atto> attosecond;  // 10^-18 seconds
...
template <>
class quantity<boost_kahypar::ratio<1>, boost_kahypar::ratio<0> >
{
    double q_;
public:
    quantity() : q_(1) {}
...
template <>
class quantity<boost_kahypar::ratio<0>, boost_kahypar::ratio<1> >
{
    double q_;
public:
    quantity() : q_(1) {}
...
template <>
class quantity<boost_kahypar::ratio<0>, boost_kahypar::ratio<0> >
{
    double q_;
public:
    quantity() : q_(1) {}
...
// Example SI-Units
typedef quantity<boost_kahypar::ratio<0>, boost_kahypar::ratio<0> >  Scalar;
typedef quantity<boost_kahypar::ratio<1>, boost_kahypar::ratio<0> >  Time;         // second
typedef quantity<boost_kahypar::ratio<0>, boost_kahypar::ratio<1> >  Distance;     // meter
typedef quantity<boost_kahypar::ratio<-1>, boost_kahypar::ratio<1> > Speed;        // meter/second
typedef quantity<boost_kahypar::ratio<-2>, boost_kahypar::ratio<1> > Acceleration; // meter/second^2

template <class R1, class R2, class R3, class R4>
quantity<typename boost_kahypar::ratio_subtract<R1, R3>::type, typename boost_kahypar::ratio_subtract<R2, R4>::type>
operator/(const quantity<R1, R2>& x, const quantity<R3, R4>& y)
{
    typedef quantity<typename boost_kahypar::ratio_subtract<R1, R3>::type, typename boost_kahypar::ratio_subtract<R2, R4>::type> R;
    R r;
    r.set(x.get() / y.get());
    return r;
}
...
template <class R1, class R2, class R3, class R4>
quantity<typename boost_kahypar::ratio_add<R1, R3>::type, typename boost_kahypar::ratio_add<R2, R4>::type>
operator*(const quantity<R1, R2>& x, const quantity<R3, R4>& y)
{
    typedef quantity<typename boost_kahypar::ratio_add<R1, R3>::type, typename boost_kahypar::ratio_add<R2, R4>::type> R;
    R r;
    r.set(x.get() * y.get());
    return r;
}
...
void testUser1()
{
    ...
    User1::Time t( boost_kahypar::chrono::hours(2) );
    User1::Speed s = d / t;
...
// This class is subsequently used as the rep in boost_kahypar::chrono::duration to demonstrate a
...
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::pico                 > picoseconds;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::nano                 > nanoseconds;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::micro                > microseconds;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::milli                > milliseconds;
typedef boost_kahypar::chrono::duration<saturate<long long>                            > seconds;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::ratio<         60LL> > minutes;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::ratio<       3600LL> > hours;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::ratio<      86400LL> > days;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::ratio<   31556952LL> > years;
typedef boost_kahypar::chrono::duration<saturate<long long>, boost_kahypar::ratio<31556952000LL> > millennium;
...
template <class T1, class T2, bool = boost_kahypar::is_integral<T1>::value>
struct promote_helper;

template <class T1, class T2>
struct promote_helper<T1, saturate<T2>, true>  // integral
{
    typedef typename boost_kahypar::common_type<T1, T2>::type rep;
...
//    picoseconds ps1 = yr;  // does not compile, compile-time overflow in ratio arithmetic
    ...
    days d = boost_kahypar::chrono::duration_cast<days>(yr);
    std::cout << "200 million years ago encoded in days: " << d.count() << '\n';
    millennium c = boost_kahypar::chrono::duration_cast<millennium>(yr);
...
public:
    typedef xtime                                  rep;
    typedef boost_kahypar::micro                           period;
    typedef boost_kahypar::chrono::duration<rep, period>   duration;
    typedef boost_kahypar::chrono::time_point<xtime_clock> time_point;
...
public:
    typedef boost_kahypar::chrono::duration<double, boost_kahypar::nano> tonanosec;
...
void test()
{
    ...
    clock::duration delay(boost_kahypar::chrono::milliseconds(5));
...
template <class Rep, class Period>
void inspect_duration(boost_kahypar::chrono::duration<Rep, Period> d, const std::string& name)
{
    typedef boost_kahypar::chrono::duration<Rep, Period> Duration;
    std::cout << "********* " << name << " *********\n";
    ...
    if (boost_kahypar::is_floating_point<Rep>::value)
    {
        std::cout << "floating point\n";
        std::cout << "The precision is the most significant ";
        std::cout << std::numeric_limits<Rep>::digits10 << " decimal digits.\n";
    }
    else if (boost_kahypar::is_integral<Rep>::value)
    {
        std::cout << "integral\n";
        d = Duration(Rep(1));
        boost_kahypar::chrono::duration<double> dsec = d;
    ...
    using namespace boost_kahypar::chrono;
    using namespace std;
    typedef duration<double, boost_kahypar::ratio_multiply<boost_kahypar::ratio<24*3652425,10000>, hours::period>::type> Years;
    Years years = d;
...
typedef boost_kahypar::chrono::duration<zero_default<long long>, boost_kahypar::nano        > nanoseconds;
typedef boost_kahypar::chrono::duration<zero_default<long long>, boost_kahypar::micro       > microseconds;
typedef boost_kahypar::chrono::duration<zero_default<long long>, boost_kahypar::milli       > milliseconds;
typedef boost_kahypar::chrono::duration<zero_default<long long>                   > seconds;
typedef boost_kahypar::chrono::duration<zero_default<long long>, boost_kahypar::ratio<60>   > minutes;
typedef boost_kahypar::chrono::duration<zero_default<long long>, boost_kahypar::ratio<3600> > hours;
...
inline
typename boost_kahypar::common_type<time_point<Clock, Duration1>,
...
void test_min()
{
    typedef time_point<system_clock,
      boost_kahypar::common_type<system_clock::duration, seconds>::type> T1;
    typedef time_point<system_clock,
      boost_kahypar::common_type<system_clock::duration, nanoseconds>::type> T2;
    typedef boost_kahypar::common_type<T1, T2>::type T3;
...
void explore_limits()
{
    typedef duration<long long, boost_kahypar::ratio_multiply<boost_kahypar::ratio<24*3652425,10000>,
...
struct cycle_count
{
    typedef typename boost_kahypar::ratio_multiply<boost_kahypar::ratio<speed>, boost_kahypar::mega>::type frequency;  // Mhz
    typedef typename boost_kahypar::ratio_divide<boost_kahypar::ratio<1>, frequency>::type period;
    typedef long long rep;
    typedef boost_kahypar::chrono::duration<rep, period> duration;
    typedef boost_kahypar::chrono::time_point<cycle_count> time_point;
...
struct approx_cycle_count
{
    ...
    typedef boost_kahypar::chrono::time_point<approx_cycle_count> time_point;
...
void cycle_count_delay()
{
    ...
    cout << "\nSimulated " << clock::frequency::num / boost_kahypar::mega::num << "MHz clock which has a tick period of "
         << duration<double, boost_kahypar::nano>(clock::duration(1)).count() << " nanoseconds\n";
...
int main()
{
    ...
    duration<double, boost_kahypar::milli> d = milliseconds(3) * 2.5;
