...
#include <boost_kahypar/cstdlib.hpp>               // for boost_kahypar::exit_success
#include <boost_kahypar/integer/integer_mask.hpp>  // for boost_kahypar::high_bit_mask_t, etc.
...
#define MASK_TYPE ::boost_kahypar::ulong_long_type
...
#define PRIVATE_HIGH_BIT_SLOW_TEST(v)  BOOST_TEST( ::boost_kahypar::high_bit_mask_t< \
 (v) >::high_bit == (ONE << (v)) );
#define PRIVATE_HIGH_BIT_FAST_TEST(v)  BOOST_TEST( ::boost_kahypar::high_bit_mask_t< \
...
#define PRIVATE_LOW_BITS_SLOW_TEST(v)  \
   do{ \
      ...
      BOOST_TEST( ::boost_kahypar::low_bits_mask_t< (v) >::sig_bits ==  mask); \
   }while(false);
#define PRIVATE_LOW_BITS_FAST_TEST(v)  \
   do{ \
      ...
      if(v > 0)\
         { mask = ((ONE << (v-1)) - 1); mask <<= 1; mask |= 1; }\
      BOOST_TEST( ::boost_kahypar::low_bits_mask_t< (v) >::sig_bits_fast == mask);\
   }while(false);
