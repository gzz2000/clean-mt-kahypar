...
#include <boost_kahypar/config.hpp> // msvc 6.0 needs this to suppress warnings
...
#include <boost_kahypar/serialization/state_saver.hpp>
...
#include <boost_kahypar/archive/detail/decl.hpp>
...
using namespace boost_kahypar::serialization;
...
//public:
    ...
    const basic_pointer_iserializer * load_pointer(
        ...
        const basic_pointer_iserializer * (*finder)(
            const boost_kahypar::serialization::extended_type_info & type
        )
    );
};
...
basic_iarchive_impl::load_object(
    ...
    boost_kahypar::serialization::state_saver<object_id_type> ss_start(m_moveable_objects.start);
...
basic_iarchive_impl::load_pointer(
    ...
    const basic_pointer_iserializer * bpis_ptr,
    const basic_pointer_iserializer * (*finder)(
        const boost_kahypar::serialization::extended_type_info & type_
    )
){
