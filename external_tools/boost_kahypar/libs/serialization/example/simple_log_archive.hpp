...
#include <boost_kahypar/serialization/array.hpp>
...
class simple_log_archive {
    ...
    void save(const T &t){
        typedef
            BOOST_DEDUCED_TYPENAME boost_kahypar::mpl::eval_if<boost_kahypar::is_enum< T >,
                boost_kahypar::mpl::identity<save_enum_type<simple_log_archive> >,
            //else
            BOOST_DEDUCED_TYPENAME boost_kahypar::mpl::eval_if<
                // if its primitive
                    boost_kahypar::mpl::equal_to<
                        boost_kahypar::serialization::implementation_level< T >,
                        boost_kahypar::mpl::int_<boost_kahypar::serialization::primitive_type>
                    >,
                    boost_kahypar::mpl::identity<save_primitive<simple_log_archive> >,
            // else
                boost_kahypar::mpl::identity<save_only<simple_log_archive> >
            > >::type typex;
        typex::invoke(*this, t);
    }
...
public:
    ...
    typedef boost_kahypar::mpl::bool_<false> is_loading;
    typedef boost_kahypar::mpl::bool_<true> is_saving;
    ...
    simple_log_archive & operator<<(const T (&t)[N]){
        return *this << boost_kahypar::serialization::make_array(
            static_cast<const T *>(&t[0]),
            N
        );
    }
    template<class T>
    simple_log_archive & operator<<(const boost_kahypar::serialization::nvp< T > & t){
