...
private:
    ...
    template<class Archive>
    void serialize(Archive & ar, const unsigned int /* file_version */){
        ar & boost_kahypar::serialization::base_object<A>(*this);
    }
public:
    static int count;
    B() : A() {};
...
void display(boost_kahypar::shared_ptr<A> &spa, boost_kahypar::shared_ptr<A> &spa1)
{
...
{
    std::string filename(boost_kahypar::archive::tmpdir());
    filename += "/testfile";

    // create  a new shared pointer to ta new object of type A
    boost_kahypar::shared_ptr<A> spa(new A);
    boost_kahypar::shared_ptr<A> spa1;
    spa1 = spa;
    display(spa, spa1);
    // serialize it
    {
        std::ofstream ofs(filename.c_str());
        boost_kahypar::archive::text_oarchive oa(ofs);
        oa << spa;
        oa << spa1;
    }
    ...
    // creating a new type A object
    {
        ...
        boost_kahypar::archive::text_iarchive ia(ifs);
    ...
    // ADDITION BY DT
    // create  a new shared pointer to ta new object of type A
    spa = boost_kahypar::shared_ptr<A>(new B);
    spa1 = spa;
    display(spa, spa1);
    // serialize it
    {
