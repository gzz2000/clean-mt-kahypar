...
struct D {
    ...
    D(){
        ...
        const C & c = boost_kahypar::serialization::singleton<C>::get_const_instance();
        const C & c1 = boost_kahypar::serialization::singleton<C>::get_const_instance();
        BOOST_CHECK_EQUAL(&c, &c1);

        // verify that objects are created in sequence of definition
        BOOST_CHECK_EQUAL(c.m_id, 1);
        const B & b = boost_kahypar::serialization::singleton<B>::get_const_instance();
        BOOST_CHECK_EQUAL(b.m_id, 2);
        const A & a = boost_kahypar::serialization::singleton<A>::get_const_instance();
