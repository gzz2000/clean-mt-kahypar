...
#include <boost_kahypar/archive/dinkumware.hpp>
#endif

#ifndef BOOST_NO_CWCHAR
#include <boost_kahypar/archive/iterators/mb_from_wchar.hpp>
#include <boost_kahypar/archive/iterators/wchar_from_mb.hpp>
#endif
#include <boost_kahypar/archive/iterators/xml_escape.hpp>
#include <boost_kahypar/archive/iterators/xml_unescape.hpp>
#include <boost_kahypar/archive/iterators/transform_width.hpp>
#include <boost_kahypar/archive/iterators/istream_iterator.hpp>
#include <boost_kahypar/archive/iterators/ostream_iterator.hpp>
...
void test_wchar_from_mb(const wchar_t *la, const char * a, const unsigned int size){
    typedef boost_kahypar::archive::iterators::wchar_from_mb<const char *> translator;
    BOOST_CHECK((
        std::equal(
            translator(a),
            translator(a + size),
...
void test_mb_from_wchar(const char * a, const wchar_t *la, const unsigned int size){
    typedef boost_kahypar::archive::iterators::mb_from_wchar<const wchar_t *> translator;
    BOOST_CHECK(
        std::equal(
            translator(la),
            translator(la + size),
...
void test_roundtrip(const wchar_t * la){
    ...
    {
        typedef boost_kahypar::archive::iterators::wchar_from_mb<std::vector<char>::const_iterator> translator;
        std::copy(
            translator(a.begin()),
            translator(),
...
void test_xml_escape(
    ...
    typedef boost_kahypar::archive::iterators::xml_escape<const CharType *> translator;
...
void test_xml_unescape(
    ...
    // test xml_unescape
    typedef boost_kahypar::archive::iterators::xml_unescape<const CharType *> translator;
...
void test_transform_width(unsigned int size){
    ...
    typedef boost_kahypar::archive::iterators::transform_width<
        char *, BitsOut, BitsIn
    > translator1;
...
void test_stream_iterators(
    ...
    boost_kahypar::archive::iterators::ostream_iterator<CharType> osi =
        boost_kahypar::archive::iterators::ostream_iterator<CharType>(ss);
    std::copy(test_data, test_data + size, osi);
    ss.flush();

    BOOST_CHECK(size == ss.str().size());

    boost_kahypar::archive::iterators::istream_iterator<CharType> isi =
        boost_kahypar::archive::iterators::istream_iterator<CharType>(ss);
