...
int test3(){
    ...
    K k;
    boost_kahypar::archive::archive_exception exception(
        boost_kahypar::archive::archive_exception::no_exception
    );
    {
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        ...
        BOOST_CATCH_END
        BOOST_CHECK(
            exception.code == boost_kahypar::archive::archive_exception::pointer_conflict
        );
    }
    // if exception wasn't invoked
    if(exception.code == boost_kahypar::archive::archive_exception::no_exception){
        // try to read the archive
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        exception = boost_kahypar::archive::archive_exception(
            boost_kahypar::archive::archive_exception::no_exception
        );
        BOOST_TRY {
