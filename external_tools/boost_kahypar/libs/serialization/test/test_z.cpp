...
#include <boost_kahypar/serialization/optional.hpp>
...
int main()
{
    boost_kahypar::optional<Foo> oldFoo = Foo(10);
    std::ostringstream outStream;
    boost_kahypar::archive::text_oarchive outArchive(outStream);
    outArchive & oldFoo;

    boost_kahypar::optional<Foo> newFoo;
    std::istringstream inStream(outStream.str());
    boost_kahypar::archive::text_iarchive inArchive(inStream);
    inArchive & newFoo;
...
#include <boost_kahypar/archive/archive_exception.hpp>
...
{
    const char * testfile = boost_kahypar::archive::tmpnam(NULL);
    BOOST_REQUIRE(NULL != testfile);

    const boost_kahypar::optional<int> aoptional1;
    const boost_kahypar::optional<int> aoptional2(123);
    {
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        //oa << boost_kahypar::serialization::make_nvp("aoptional1",aoptional1);
        //oa << boost_kahypar::serialization::make_nvp("aoptional2",aoptional2);
    }
    /*
    boost_kahypar::optional<int> aoptional1a(999);
    boost_kahypar::optional<int> aoptional2a;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("aoptional1",aoptional1a);
        ia >> boost_kahypar::serialization::make_nvp("aoptional2",aoptional2a);
    }
...
#include <boost_kahypar/archive/xml_woarchive.hpp>
#include <boost_kahypar/archive/xml_wiarchive.hpp>
#include <boost_kahypar/serialization/string.hpp>
...
int test_main(int, char *argv[])
{
	...
	std::wstring w2 = L"апр"; // some non-latin (for example russians) letters
    {
        ...
        {
            boost_kahypar::archive::xml_woarchive oa(ofs);
            oa << boost_kahypar::serialization::make_nvp("key1", s1);
            //oa << boost_kahypar::serialization::make_nvp("key2", w1);
            //oa << boost_kahypar::serialization::make_nvp("key3", w2); // here exception is thrown
        }
    }
    std::string new_s1;
    //std::wstring new_w1;
    //std::wstring new_w2;
    {
        ...
        {
            boost_kahypar::archive::xml_wiarchive ia(ifs);
            ia >> boost_kahypar::serialization::make_nvp("key1", new_s1);
            //ia >> boost_kahypar::serialization::make_nvp("key2", new_w1);
            //ia >> boost_kahypar::serialization::make_nvp("key3", new_w2); // here exception is thrown
        }
    }
...
int main() {
        boost_kahypar::archive::xml_oarchive oa( std::cerr );
        int bob = 3;
        oa << boost_kahypar::serialization::make_nvp( "bob", bob );
}
...
#include <boost_kahypar/archive/binary_iarchive.hpp>
#include <boost_kahypar/serialization/nvp.hpp>
#include <boost_kahypar/serialization/shared_ptr.hpp>
#include <boost_kahypar/serialization/vector.hpp>
...
typedef boost_kahypar::shared_ptr<boxed_int> pi;

void go(std::istream &is) {
    std::vector<pi> vv;
    try
    {
        boost_kahypar::archive::binary_iarchive ia(is);
        ia & vv;
    }
...
#include <boost_kahypar/archive/iterators/base64_from_binary.hpp>
#include <boost_kahypar/archive/iterators/transform_width.hpp>

namespace bai = boost_kahypar::archive::iterators;
int main()
{
    std::string input =
...
int main()
{
	...
	{
		boost_kahypar::archive::text_oarchive oa(ss);
		oa << s0;
	}

	std::shared_ptr<S> s1;
	{
		boost_kahypar::archive::text_iarchive ia(ss);
		ia >> s1;
	}
...
void f()
{
  ...
  boost_kahypar::archive::binary_iarchive ar(iss);
  std::vector<int> out;
  ar >> out;
}
...
#include <boost_kahypar/archive/detail/archive_serializer_map.hpp>
#include <boost_kahypar/archive/impl/archive_serializer_map.ipp>
#include <boost_kahypar/archive/impl/basic_xml_iarchive.ipp>
#include <boost_kahypar/archive/impl/xml_iarchive_impl.ipp>
#include <boost_kahypar/serialization/nvp.hpp>
#include <limits>
#include <fstream>

class xml_iarchive_nan :
public boost_kahypar::archive::xml_iarchive_impl<xml_iarchive_nan>
{
protected:
    friend class boost_kahypar::archive::detail::interface_iarchive<xml_iarchive_nan>;
    friend class boost_kahypar::archive::detail::common_iarchive<xml_iarchive_nan>;
    friend class boost_kahypar::archive::basic_xml_iarchive<xml_iarchive_nan>;
    friend class boost_kahypar::archive::load_access;

    using boost_kahypar::archive::xml_iarchive_impl<xml_iarchive_nan>::load;
    void load(double & t)
    {
        char c='0';
        if(!is.fail())
        {
            ...
            if(c!='n')
            {
                boost_kahypar::archive::xml_iarchive_impl<xml_iarchive_nan>::load(t);
            }
            else
            {
                char c1='0',c2='0';
                ...
                if(is.fail()||(!((c=='n')&&(c1=='a')&&(c2=='n'))))
                    boost_kahypar::serialization::throw_exception(
                                                          boost_kahypar::archive::archive_exception(
                                                                                            boost_kahypar::archive::archive_exception::input_stream_error));
                else
            ...
            boost_kahypar::serialization::throw_exception(
                                                  boost_kahypar::archive::archive_exception(boost_kahypar::archive::archive_exception::input_stream_error));
    }

public:
    xml_iarchive_nan(std::istream & is, unsigned int flags = 0) :
    boost_kahypar::archive::xml_iarchive_impl<xml_iarchive_nan>(is, flags)
    {}
    ~xml_iarchive_nan(){};
};
...
// boost_kahypar::variant not correctly serialized when reusing the memory location #203
#include <boost_kahypar/archive/text_iarchive.hpp>
#include <boost_kahypar/archive/text_oarchive.hpp>
#include <boost_kahypar/serialization/serialization.hpp>
#include <boost_kahypar/serialization/variant.hpp>

int main() {
  boost_kahypar::variant<int> variant = 1;
  ...
  // read it two times
  boost_kahypar::variant<int> v1;
  boost_kahypar::variant<int> v2;
  boost_kahypar::archive::text_iarchive ia(ss);
  ia >> boost_kahypar::serialization::make_nvp("var1", v1);
  ia >> boost_kahypar::serialization::make_nvp("var2", v2);

  assert(boost_kahypar::get<int>(v1) == 1);
  assert(boost_kahypar::get<int>(v2) == 1);  // Fails: v2 is actually zero
}

#else

#include <boost_kahypar/serialization/set.hpp>
