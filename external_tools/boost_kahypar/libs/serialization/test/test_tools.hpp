...
#include <boost_kahypar/filesystem/operations.hpp> // unique_path
...
#include <direct.h>
#include <boost_kahypar/archive/tmpdir.hpp>
...
namespace archive {
    const char * test_filename(const char * dir = NULL, char *fname = NULL){
        ...
        ibuffer[0] = '\0';
        if(NULL == dir){
            dir = boost_kahypar::archive::tmpdir();
        }
        STRCPY(ibuffer, dir);
        std::strcat(ibuffer, "/");
        if(NULL == fname){
            ...
            boost_kahypar::filesystem::path tmp_path =
                boost_kahypar::filesystem::unique_path("%%%%%");
...
namespace archive {
    char const * tmpnam(char * buffer) {
        ...
        if (name[0] == 0) {
            boost_kahypar::filesystem::path tempdir(tmpdir());
            boost_kahypar::filesystem::path tempfilename =
                boost_kahypar::filesystem::unique_path("serialization-%%%%");
            boost_kahypar::filesystem::path temp = tempdir / tempfilename;
...
#define BOOST_CHECK_MESSAGE( P, M )  \
    ((P)? (void)0 : ::boost_kahypar::detail::error_impl( (M) , __FILE__, __LINE__, BOOST_CURRENT_FUNCTION))
...
} } // boost_kahypar::detail

#define BOOST_WARN_MESSAGE( P, M )  \
    ((P)? (void)0 : ::boost_kahypar::detail::msg_impl( (M) , __FILE__, __LINE__, BOOST_CURRENT_FUNCTION))
...
main(int argc, char * argv[]){
    boost_kahypar::serialization::get_singleton_module().lock();
    ...
    boost_kahypar::serialization::get_singleton_module().unlock();

    int error_count = boost_kahypar::report_errors();
    if(error_count > 0)
        retval = error_count;
    return retval;
}
