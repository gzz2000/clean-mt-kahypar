...
bool A::operator==(const A &rhs) const
{
    return
        ...
        && v == rhs.v
        #if BOOST_CXX_VERSION > 199711L // only include floating point if C++ version >= C++11
            && std::abs( boost_kahypar::math::float_distance(w, rhs.w)) < 2
            && std::abs( boost_kahypar::math::float_distance(x, rhs.x)) < 2
        #endif
    ;
}
...
bool A::operator<(const A &rhs) const
{
    ...
    if(! (v == rhs.v) )
        return t < rhs.v;
    #if BOOST_CXX_VERSION > 199711L // only include floating point if C++ version >= C++11
        if(std::abs( boost_kahypar::math::float_distance(w, rhs.w)) > 1)
            return false;
        if(std::abs( boost_kahypar::math::float_distance(x, rhs.x)) > 1)
            return false;
    #endif
    return false;
}
...
void serialize(
    ...
    ar & boost_kahypar::serialization::make_nvp("s", a.s);
    ar & boost_kahypar::serialization::make_nvp("t", a.t);
    ar & boost_kahypar::serialization::make_nvp("u", a.u);
    ar & boost_kahypar::serialization::make_nvp("v", a.v);
    ar & boost_kahypar::serialization::make_nvp("w", a.w);
    ar & boost_kahypar::serialization::make_nvp("x", a.x);
}
