...
struct Foo: boost_kahypar::serialization::singleton<Foo>{
    Foo(): i(42) {
...
controller::~controller() {
    ...
    THROW_ON_FALSE(boost_kahypar::serialization::singleton<Foo>::is_destroyed());
...
test_main( int /* argc */, char* /* argv */[] )
{
    ...
    BOOST_CHECK(!boost_kahypar::serialization::singleton<Foo>::is_destroyed());
    BOOST_CHECK(!Foo::is_destroyed());

    BOOST_CHECK(boost_kahypar::serialization::singleton<Foo>::get_const_instance().i == 42);
