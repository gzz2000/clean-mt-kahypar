...
#include <boost_kahypar/archive/polymorphic_oarchive.hpp>
#include <boost_kahypar/archive/polymorphic_iarchive.hpp>
...
int test_main(int /* argc */, char * /* argv */ [])
{
    ...
    for(int i=0; i<1000; ++i){
        v1.push_back(my_string(boost_kahypar::lexical_cast<std::string>(i % 100)));
    }
    ...
    // test using using polymorphic implementation.
    {
        ...
        oa_implementation << boost_kahypar::serialization::make_nvp("vector", v1);
    }
    {
        std::vector<my_string> v2;
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive  ia_implementation(is, TEST_ARCHIVE_FLAGS);
        ia_implementation >> boost_kahypar::serialization::make_nvp("vector", v2);
        BOOST_CHECK(v1 == v2);
    }
    std::remove(testfile);
    ...
    // test using using polymorphic interface.
    {
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa_implementation(os, TEST_ARCHIVE_FLAGS);
        boost_kahypar::archive::polymorphic_oarchive & oa_interface = oa_implementation;
        oa_interface << boost_kahypar::serialization::make_nvp("vector", v1);
    }
    {
        std::vector<my_string> v2;
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive  ia_implementation(is, TEST_ARCHIVE_FLAGS);
        boost_kahypar::archive::polymorphic_iarchive & ia_interface = ia_implementation;
        ia_interface >> boost_kahypar::serialization::make_nvp("vector", v2);
        BOOST_CHECK(v1 == v2);
    }
    std::remove(testfile);
    std::remove(testfile);
