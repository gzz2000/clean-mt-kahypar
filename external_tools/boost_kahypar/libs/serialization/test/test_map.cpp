...
test_map(){
    ...
    amap.insert(std::make_pair(random_key(), A()));
    {
        ...
        oa << boost_kahypar::serialization::make_nvp("amap", amap);
    }
    std::map<random_key, A> amap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("amap", amap1);
    }
...
test_multimap(){
    ...
    amultimap.insert(std::make_pair(random_key(), A()));
    {
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        oa << boost_kahypar::serialization::make_nvp("amultimap", amultimap);
    }
    std::multimap<random_key, A> amultimap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("amultimap", amultimap1);
    }
