...
#include <boost_kahypar/serialization/hash_map.hpp>
...
struct random_key {
    ...
    void serialize(
        ...
        ar & boost_kahypar::serialization::make_nvp("random_key", m_i);
    }
    int m_i;
...
test_hash_map(){
    ...
    ahash_map.insert(std::make_pair(random_key(), A()));
    {
        ...
        oa << boost_kahypar::serialization::make_nvp("ahashmap",ahash_map);
    }
    BOOST_STD_EXTENSION_NAMESPACE::hash_map<random_key, A> ahash_map1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("ahashmap",ahash_map1);
    }
...
test_hash_multimap(){
    ...
    ahash_multimap.insert(std::make_pair(random_key(), A()));
    {
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        oa << boost_kahypar::serialization::make_nvp("ahash_multimap", ahash_multimap);
    }
    BOOST_STD_EXTENSION_NAMESPACE::hash_multimap<random_key, A> ahash_multimap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("ahash_multimap", ahash_multimap1);
    }
    std::vector< std::pair<random_key, A> > tvec, tvec1;
    tvec.clear();
    tvec1.clear();
