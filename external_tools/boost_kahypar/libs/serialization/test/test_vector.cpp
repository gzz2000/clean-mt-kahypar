...
int test_vector_detail(const std::vector<T> & avector)
{
    ...
    // test array of objects
    {
        ...
        oa << boost_kahypar::serialization::make_nvp("avector", avector);
    }
    std::vector< T > avector1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> boost_kahypar::serialization::make_nvp("avector", avector1);
    }
...
inline void save_construct_data(
    ...
    ar << boost_kahypar::serialization::make_nvp("i", x->m_i);
}
