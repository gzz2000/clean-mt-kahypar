    ...
    public:
        ...
        high_resolution_timer(double t) 
        {
            ...
            if (!QueryPerformanceFrequency(&frequency))
                boost_kahypar::throw_exception(std::runtime_error("Couldn't acquire frequency"));
        ...
        { 
            if (!QueryPerformanceCounter(&start_time))
                boost_kahypar::throw_exception(std::runtime_error("Couldn't initialize start_time"));
        } 
        double elapsed() const                  // return elapsed time in seconds
        { 
            LARGE_INTEGER now;
            if (!QueryPerformanceCounter(&now))
                boost_kahypar::throw_exception(std::runtime_error("Couldn't get current time"));
    ...
    public:
        ...
        { 
            ...
            if (-1 == clock_getres(CLOCK_REALTIME, &resolution))
                boost_kahypar::throw_exception(std::runtime_error("Couldn't get resolution"));
...
namespace xml {
    ...
    private:
        bool use_backup;
        timespec start_time;
        boost_kahypar::timer start_time_backup;
    }; 
...
//  simply fall back to boost_kahypar::timer
#include <boost_kahypar/timer.hpp>
