...
#include <boost_kahypar/archive/add_facet.hpp>
...
test_main(int /* argc */, char * /* argv */[]) {
    ...
    std::locale * utf8_locale
        = boost_kahypar::archive::add_facet(
            old_loc, 
    ...
    // Send our test UTF-8 data to file
    {
        ...
        std::copy(
            td::utf8_encoding,
                ...
                td::utf8_encoding + 12,
            #endif
            boost_kahypar::archive::iterators::ostream_iterator<utf8_t>(ofs)
        );
    }
    ...
    // Send the UCS4_data back out, converting to UTF-8
    {
        ...
        std::copy(
            from_file.begin(),
            from_file.end(),
            boost_kahypar::archive::iterators::ostream_iterator<wchar_t>(ofs)
        );
    }

    // Make sure that both files are the same
    {
        typedef boost_kahypar::archive::iterators::istream_iterator<utf8_t> is_iter;
        is_iter end_iter;
    ...
    // Make sure that both files are the same
    {
        ...
        BOOST_CHECK(
            std::equal(
                test3_data,
                test3_data + l,
                boost_kahypar::archive::iterators::istream_iterator<wchar_t>(ifs)
            )
        );
    }
