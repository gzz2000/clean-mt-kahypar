...
class my_category: public boost_kahypar::system::error_category
{
public:
    ...
    boost_kahypar::system::error_condition default_error_condition( int ev ) const noexcept
    {
        switch( ev )
        {
        case my_enoent:

            return boost_kahypar::system::error_condition( ENOENT, boost_kahypar::system::generic_category() );
...
template<> struct boost_kahypar::system::is_error_code_enum<my_errc>: std::true_type {};
...
boost_kahypar::system::error_code make_error_code( my_errc e )
{
    ...
    return boost_kahypar::system::error_code( e, cat );
}

int main()
{
    {
        boost_kahypar::system::error_code e1 = my_enoent;

        BOOST_TEST( e1 == my_enoent );
        BOOST_TEST_NOT( e1 != my_enoent );

        BOOST_TEST( e1 == boost_kahypar::system::errc::no_such_file_or_directory );
