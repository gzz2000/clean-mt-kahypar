...
int main()
{
  ...
  catch (const boost_kahypar::system::system_error& ex)
  {
    std::cout << "  caught boost_kahypar::system::system_error as expected\n";
    std::cout << "  what() reports " << ex.what() << '\n';
    return 0;
  }
  catch (const std::runtime_error& ex)
  {
    std::cout << "  error: caught std::runtime_error instead of boost_kahypar::system::system_error\n";
    std::cout << "  what() reports " << ex.what() << '\n';
    return 1;
  }

  std::cout << "  error: failed to catch boost_kahypar::system::system_error\n";
  return 1;
}
