...
//  with a boost_kahypar::system using directive increases use scenario coverage.
...
namespace
{
  ...
  int divide(int dividend, int divisor, boost_kahypar::system::error_code& ec = boost_kahypar::throws())
  {
    if (divisor == 0)                            // is there an error?
    {
      if (&ec == &boost_kahypar::throws())               // throw on error
        throw "oops!";                           // whatever exception you prefer
      ec = error_code(EDOM, generic_category()); // report error via error_code
      return 0;
    }
    
    if (&ec != &boost_kahypar::throws())                 // error reporting via error_code
      ec.clear();
  ...
  void test_throws_usage()
  {
    ...
    //error_code should_fail(boost_kahypar::throws());     // should fail at runtime
    //boost_kahypar::throws() = ec;                        // should fail at runtime
  }
}
