...
#include <boost_kahypar/math/special_functions/gamma.hpp>

#include <boost_kahypar/math/distributions/uniform.hpp>
#include <boost_kahypar/math/distributions/chi_squared.hpp>
#include <boost_kahypar/math/distributions/normal.hpp>
#include <boost_kahypar/math/distributions/triangular.hpp>
#include <boost_kahypar/math/distributions/cauchy.hpp>
#include <boost_kahypar/math/distributions/gamma.hpp>
#include <boost_kahypar/math/distributions/exponential.hpp>
#include <boost_kahypar/math/distributions/lognormal.hpp>
...
public:
  equidistribution_test(test_environment & env, unsigned int classes, 
    ...
    : test_base(env), classes(classes),
      test_distrib_chi_square(boost_kahypar::math::chi_squared(classes-1), high_classes)
  { }
...
public:
  ...
  void run(RNG & rng, int n1, int n2)
  {
    boost_kahypar::math::uniform ud(static_cast<double>((rng.min)()), static_cast<double>((rng.max)()));
    run(rng, ud, n1, n2);
  }
...
public:
  permutation_test(test_environment & env, unsigned int classes,
                   unsigned int high_classes)
    : test_base(env), classes(classes),
      test_distrib_chi_square(boost_kahypar::math::chi_squared(fac<int>(classes)-1), 
                              high_classes)
  { }
...
public:
  birthday_test(test_environment & env, unsigned int high_classes)
    : test_base(env),
      test_distrib_chi_square(boost_kahypar::math::chi_squared(4-1), high_classes)
  { }
  ...
  void run(RNG & rng, int n1, int n2)
  {
    ...
    boost_kahypar::variate_generator<RNG&, boost_kahypar::uniform_int<> > uni(rng, boost_kahypar::uniform_int<>(0, (1<<25)-1));
...
public:
  ...
  explicit test_environment(double confid) 
    : confidence(confid),
    confidence_chi_square_quantil(quantile(boost_kahypar::math::chi_squared(classes-1), confidence)),
      test_distrib_chi_square6(boost_kahypar::math::chi_squared(7-1), classes),
      ...
      bday_test(*this, classes)
  {
    std::cout << "Confidence level: " << confid 
              ...
              << ") = " 
              << cdf(boost_kahypar::math::chi_squared(classes-1), confidence_chi_square_quantil)
              << std::endl;
  }
  ...
  bool check_confidence(double val, double chi_square_conf) const
  {
    ...
    if(!result) {
      ...
      double prob = (val > 10*chi_square_conf ? 1 :
                     cdf(boost_kahypar::math::chi_squared(classes-1), val));
      std::cout << (1-prob) << "]";
    }
    std::cout << " " << std::flush;
    return result;
...
#define TEST(name)                      \
  if(args.check_(#name))                 \
    env.run_test<boost_kahypar::name>(#name)
  ...
  if(args.check_("normal"))
    env.run_test<boost_kahypar::mt19937>("normal", boost_kahypar::normal_distribution<>(), boost_kahypar::math::normal());
  if(args.check_("triangle"))
    env.run_test<boost_kahypar::mt19937>("triangle", boost_kahypar::triangle_distribution<>(0, 1, 3), boost_kahypar::math::triangular(0, 1, 3));
  if(args.check_("cauchy"))
    env.run_test<boost_kahypar::mt19937>("cauchy", boost_kahypar::cauchy_distribution<>(), boost_kahypar::math::cauchy());
  if(args.check_("gamma"))
    env.run_test<boost_kahypar::mt19937>("gamma", boost_kahypar::gamma_distribution<>(1), boost_kahypar::math::gamma_distribution<>(1));
  if(args.check_("exponential"))
    env.run_test<boost_kahypar::mt19937>("exponential", boost_kahypar::exponential_distribution<>(), boost_kahypar::math::exponential());
  if(args.check_("lognormal"))
    env.run_test<boost_kahypar::mt19937>("lognormal", boost_kahypar::lognormal_distribution<>(1, 1),
      boost_kahypar::math::lognormal(std::log(1.0/std::sqrt(2.0)), std::sqrt(std::log(2.0))));
}
