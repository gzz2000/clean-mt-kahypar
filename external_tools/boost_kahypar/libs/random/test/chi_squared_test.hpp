...
#include <boost_kahypar/math/special_functions/pow.hpp>
...
struct chi_squared_collector {
    ...
    void update(IntType actual, double expected) {
        chi_squared += boost_kahypar::math::pow<2>(actual - expected) / expected;
        ++variables;
    }
    ...
    double cdf() {
        ...
        if(variables <= 1) {
            ...
            return boost_kahypar::math::cdf(boost_kahypar::math::chi_squared(static_cast<double>(variables - 1)), chi_squared);
        }
    }
