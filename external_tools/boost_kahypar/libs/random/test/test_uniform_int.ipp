...
BOOST_AUTO_TEST_CASE(test_uniform_int)
{
    ...
    typedef boost_kahypar::random::variate_generator<boost_kahypar::random::mt19937&, int_gen> level_one;
    ...
    // identity map
    typedef boost_kahypar::random::variate_generator<level_one&, int_gen> level_two;
    ...
    typedef boost_kahypar::random::variate_generator<level_two&, int_gen> level_three;
    level_three uint1_4(uint05, int_gen(1, 4));
    check_uniform_int(uint1_4, 100000);

    typedef BOOST_RANDOM_UNIFORM_INT<boost_kahypar::uint8_t> int8_gen;
    typedef boost_kahypar::random::variate_generator<boost_kahypar::random::mt19937&, int8_gen> gen8_t;

    gen8_t gen8_03(gen, int8_gen(0, 3));

    // use the full range of the type, where the destination
    // range is a power of the source range
    typedef boost_kahypar::random::variate_generator<gen8_t, int8_gen> uniform_uint8;
    ...
    typedef boost_kahypar::random::variate_generator<gen8_t, int_gen> uniform_uint_from8;
...
BOOST_AUTO_TEST_CASE(test_misc)
{
    ...
    boost_kahypar::minstd_rand mr;
    boost_kahypar::variate_generator<boost_kahypar::minstd_rand, uint_dist> r2(mr,
    ...
    boost_kahypar::variate_generator<boost_kahypar::minstd_rand, BOOST_RANDOM_DISTRIBUTION > x(mr, BOOST_RANDOM_DISTRIBUTION(0, 8361));
    x();

    // bug report from Alan Stokes and others: this throws an assertion
    boost_kahypar::variate_generator<boost_kahypar::minstd_rand, BOOST_RANDOM_DISTRIBUTION > y(mr, BOOST_RANDOM_DISTRIBUTION(1,1));
    y();
    y();
    y();
}
