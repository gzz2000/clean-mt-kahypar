...
bool do_test(BOOST_RANDOM_ARG1_TYPE BOOST_RANDOM_ARG1_NAME,
...
#endif
             long long max, boost_kahypar::mt19937& gen) {
    ...
    std::vector<double> expected_pdf(max_value+1);
    {
        ...
        expected_pdf.back() += 1 - boost_kahypar::accumulate(expected_pdf, 0.0);
    }
    ...
    long long sum = boost_kahypar::accumulate(results, 0ll);
    if(sum != max) {
    ...
    boost_kahypar::variate_generator<boost_kahypar::mt19937&, BOOST_RANDOM_DISTRIBUTION > vgen(gen, dist);
