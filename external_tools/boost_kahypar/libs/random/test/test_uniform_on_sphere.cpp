...
public:
    ...
    result_type operator()(Engine& rng) {
        const boost_kahypar::random::uniform_on_sphere<>::result_type& tmp = impl(rng);
        // This should be uniformly distributed in [-pi,pi)
        return std::atan2(tmp[idx1], tmp[idx2]);
    }
private:
    boost_kahypar::random::uniform_on_sphere<> impl;
    int idx1, idx2;
};
...
#define BOOST_MATH_DISTRIBUTION boost_kahypar::math::uniform
#define BOOST_RANDOM_ARG1_TYPE double
#define BOOST_RANDOM_ARG1_NAME n
#define BOOST_RANDOM_ARG1_DEFAULT 6
#define BOOST_RANDOM_ARG1_DISTRIBUTION(n) boost_kahypar::uniform_int<>(2, n)
