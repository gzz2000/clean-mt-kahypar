...
#include <boost_kahypar/math/distributions/hyperexponential.hpp>
...
bool do_test(std::vector<double> const& probabilities,
             ...
             long long max,
             boost_kahypar::mt19937& gen)
{
    std::cout << "running hyperexponential(probabilities,rates) " << max << " times: " << std::flush;

    boost_kahypar::math::hyperexponential_distribution<> expected(probabilities, rates);

    boost_kahypar::random::hyperexponential_distribution<> dist(probabilities, rates);

    kolmogorov_experiment test(boost_kahypar::numeric_cast<int>(max));
    boost_kahypar::variate_generator<boost_kahypar::mt19937&, boost_kahypar::random::hyperexponential_distribution<> > vgen(gen, dist);
...
{
    boost_kahypar::mt19937 gen;
    boost_kahypar::random::uniform_int_distribution<> num_phases_dist(BOOST_RANDOM_HYPEREXP_NUM_PHASES_MIN, max_num_phases);

    int errors = 0;
    for (int i = 0; i < repeat; ++i)
    {
        const int num_phases = num_phases_dist(gen);
        boost_kahypar::random::uniform_real_distribution<> prob_dist(BOOST_RANDOM_HYPEREXP_PROBABILITY_MIN, BOOST_RANDOM_HYPEREXP_PROBABILITY_MAX);
        boost_kahypar::random::uniform_real_distribution<> rate_dist(BOOST_RANDOM_HYPEREXP_RATE_MIN, max_rate);
...
bool handle_option(int& argc, char**& argv, const char* opt, T& value)
{
    if (std::strcmp(argv[0], opt) == 0 && argc > 1)
    {
        --argc;
        ++argv;
        value = boost_kahypar::lexical_cast<T>(argv[0]);
        return true;
    }
    else
    {
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr << boost_kahypar::current_exception_diagnostic_information() << std::endl;
        return EXIT_FAILURE;
    }
}
