...
#define BOOST_RANDOM_DISTRIBUTION boost_kahypar::random::uniform_on_sphere<>
#define BOOST_RANDOM_ARG1 dim
#define BOOST_RANDOM_ARG1_DEFAULT 2
#define BOOST_RANDOM_ARG1_VALUE 3

std::vector<double> min0 = boost_kahypar::assign::list_of(-1.0)(0.0);
std::vector<double> max0 = boost_kahypar::assign::list_of(1.0)(0.0);
std::vector<double> min1 = boost_kahypar::assign::list_of(-1.0)(0.0)(0.0);
std::vector<double> max1 = boost_kahypar::assign::list_of(1.0)(0.0)(0.0);
...
#include <boost_kahypar/math/special_functions/fpclassify.hpp>
...
public:
    ...
    static unsigned (max)() { return boost_kahypar::random::minstd_rand0::max(); }
    unsigned operator()() {
...
private:
    int i;
    boost_kahypar::random::minstd_rand0 gen;
};

BOOST_AUTO_TEST_CASE(test_zeros) {
    generate_zeros gen;
    boost_kahypar::random::uniform_on_sphere<> dist(2);
    std::vector<double> val = dist(gen);
    BOOST_CHECK(!(boost_kahypar::math::isnan)(val[0]));
}

BOOST_AUTO_TEST_CASE(test_valid_output) {
    boost_kahypar::random::minstd_rand0 gen;
    for(int n = 0; n < 10; ++n) {
        boost_kahypar::random::uniform_on_sphere<> dist(n);
