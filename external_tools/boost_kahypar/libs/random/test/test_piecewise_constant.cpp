...
public:
    piecewise_constant(const std::vector<double>& intervals, const std::vector<double>& weights)
      : intervals(intervals),
        cumulative(1, 0.0)
    {
        boost_kahypar::partial_sum(weights, std::back_inserter(cumulative));
    ...
    {
        std::size_t index = boost_kahypar::lower_bound(intervals, x) - intervals.begin();
        if(index == 0) return 0;
...
bool do_test(int n, int max) {
    ...
    boost_kahypar::variate_generator<boost_kahypar::mt19937&, boost_kahypar::random::piecewise_constant_distribution<> > vgen(gen, dist);
...
bool do_tests(int repeat, int max_n, int trials) {
    boost_kahypar::mt19937 gen;
    boost_kahypar::uniform_int<> idist(1, max_n);
    int errors = 0;
