...
#define BOOST_RANDOM_URNG boost_kahypar::random::mixmax
...
struct seed_seq_0 {
    ...
    void generate(It begin, It end) const {
        std::fill(begin, end, boost_kahypar::uint32_t(0xFFFFFFFF));
    }
};
...
BOOST_AUTO_TEST_CASE(test_special_seed) {
    {
        seed_seq_0 seed;
        std::vector<boost_kahypar::uint32_t> vec(17);
        seed.generate(vec.begin(), vec.end()); // fill vec with ones
        std::vector<boost_kahypar::uint32_t>::iterator it = vec.begin();
        boost_kahypar::random::mixmax gen1(it, vec.end()); // init gen1 with vec iterator
        BOOST_CHECK_EQUAL(gen1(), 775778250716139533ULL);
        BOOST_CHECK_EQUAL(gen1(), 846264592759195742ULL);

        boost_kahypar::random::mixmax gen2(seed); // init gen2 with seeq_seq, should be the same as gen1!
