...
using boost_kahypar::random::test::RandomNumberEngine;
BOOST_CONCEPT_ASSERT((RandomNumberEngine< BOOST_RANDOM_URNG >));

typedef BOOST_RANDOM_URNG::result_type result_type;
typedef boost_kahypar::random::detail::seed_type<result_type>::type seed_type;
...
void test_seed(seed_type value)
{
    ...
    test_seed_conversion<boost_kahypar::int64_t>(urng, value);
    test_seed_conversion<boost_kahypar::uint64_t>(urng, value);
#endif
...
{
    boost_kahypar::random::seed_seq q;
...
{
    boost_kahypar::uintmax_t val = (std::numeric_limits<boost_kahypar::uintmax_t>::max)();
    boost_kahypar::uintmax_t half = val / 2;
    BOOST_RANDOM_URNG urng;
    BOOST_RANDOM_URNG urng2;
    urng.discard(half);
    urng.discard(half);
...
{
    boost_kahypar::random::seed_seq seed;
...
{
    BOOST_RANDOM_URNG urng;
    boost_kahypar::uint32_t expected[] = BOOST_RANDOM_GENERATE_VALUES;
    static const std::size_t N = sizeof(expected)/sizeof(expected[0]); 
    boost_kahypar::uint32_t actual[N];
