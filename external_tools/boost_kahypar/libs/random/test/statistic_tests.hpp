...
{
    return invert_monotone_inc(boost_kahypar::bind(&cdf, dist, _1), val, 0.0, 1000.0);
}
...
private:
  struct generator {
    generator(base_type & f, int t) : f(f, boost_kahypar::uniform_01<>()), t(t) { }
    double operator()()
    {
      double mx = f();
      for(int i = 1; i < t; ++i)
  ...
  private:
    boost_kahypar::variate_generator<base_type&, boost_kahypar::uniform_01<> > f;
    int t;
  };
  base_type & f;
...
double run_experiment(const Experiment & experiment, Generator & gen, int n)
{
  ...
  return chi_square_value(v.begin(), v.end(),
                          boost_kahypar::bind(&Experiment::probability, 
                                       experiment, boost_kahypar::placeholders::_1));
}
