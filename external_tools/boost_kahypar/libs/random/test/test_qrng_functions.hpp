...
#include <boost_kahypar/test/floating_point_comparison.hpp>
...
inline void trivial_discard(Engine& eng, boost_kahypar::uintmax_t n)
{
  for( ; n != 0; --n ) eng();
}
...
inline void match_vector(Engine& eng, T (&pt)[Dimension])
{
  ...
  boost_kahypar::uniform_real<T> dist;
...
inline void discard_function(T (&pt)[N][Dimension], std::size_t skip)
{
  ...
  const T* pt_array = reinterpret_cast<T *>(boost_kahypar::addressof(pt));
...
void test_max_discard(std::size_t dim)
{
  ...
  static const boost_kahypar::uintmax_t maxdiscard = dim * engine_type::max();
...
\
typedef boost_kahypar::random::QRNG engine_t; \
\
...
struct gen_engine \
{ \
  typedef boost_kahypar::random::QRNG##_engine<IntType, w> type; \
}; \
\
...
{ \
  static const std::size_t dim = 2;\
   \
   /* test full 8 bits */ \
  test::test_max_discard<gen_engine<boost_kahypar::uint8_t, 8u> >(dim); \
  \
  /* test 7 bits */ \
  test::test_max_discard<gen_engine<boost_kahypar::uint8_t, 7u> >(dim); \
  \
  /* test 6 bits for a good measure */ \
  test::test_max_discard<gen_engine<boost_kahypar::uint8_t, 6u> >(dim); \
} \
/**/
