...
#include <boost_kahypar/math/distributions/binomial.hpp>
...
bool do_test(double p, long long max) {
    ...
    boost_kahypar::math::binomial expected(static_cast<double>(max), p);
    
    boost_kahypar::random::bernoulli_distribution<> dist(p);
...
bool do_tests(int repeat, long long trials) {
    boost_kahypar::mt19937 gen;
    boost_kahypar::uniform_01<> rdist;
    int errors = 0;
