...
#include <boost_kahypar/format.hpp>
#include <boost_kahypar/multi_index_container.hpp>
#include <boost_kahypar/multi_index/hashed_index.hpp>
#include <boost_kahypar/multi_index/sequenced_index.hpp>

boost_kahypar::regex generator_regex("(?:fixed-range )?([^:]+): (\\d+(?:\\.\\d+)?) nsec/loop = \\d+(?:\\.\\d+)? CPU cycles");
boost_kahypar::regex distribution_regex("([^\\s]+)( virtual function)? ([^:]+): (\\d+(?:\\.\\d+)?) nsec/loop = \\d+(?:\\.\\d+)? CPU cycles");

std::string template_name(std::string arg) {
    return boost_kahypar::regex_replace(arg, boost_kahypar::regex("[^\\w]"), "_");
}
...
typedef boost_kahypar::multi_index_container<
    std::string,
    boost_kahypar::mpl::vector<
        boost_kahypar::multi_index::sequenced<>,
        boost_kahypar::multi_index::hashed_unique<boost_kahypar::multi_index::identity<std::string> >
    >
> unique_list;
...
int main(int argc, char** argv) {
    ...
    while(std::getline(std::cin, line)) {
        boost_kahypar::smatch match;
        if(std::strncmp(line.c_str(), "counting ", 9) == 0) break;
        if(boost_kahypar::regex_match(line, match, generator_regex)) {
            std::string generator(match[1]);
            double time = boost_kahypar::lexical_cast<double>(match[2]);
    ...
    BOOST_FOREACH(const pair_type& pair, generator_info) {
        generator_defs << boost_kahypar::format("[template %s_speed[] %d%%]\n")
            % template_name(pair.first) % static_cast<int>(100*min/pair.second);
        generator_performance << boost_kahypar::format("  [[%s][%g][%g][%d%%]]\n")
    ...
    do {
        boost_kahypar::smatch match;
        if(boost_kahypar::regex_match(line, match, distribution_regex)) {
            if(!match[2].matched && match[1] != "counting") {
                std::string generator(match[1]);
                std::string distribution(match[3]);
                double time = boost_kahypar::lexical_cast<double>(match[4]);
    ...
    BOOST_FOREACH(const std::string& generator, generator_names) {
        distribution_performance << boost_kahypar::format("[%s]") % generator;
    }
    distribution_performance << "]\n";
    BOOST_FOREACH(const std::string& distribution, distribution_names) {
        distribution_performance << boost_kahypar::format("  [[%s]") % distribution;
        BOOST_FOREACH(const std::string& generator, generator_names) {
            ...
            if(pos != distribution_info.end()) {
                distribution_performance << boost_kahypar::format("[%g]") % (1000/pos->second);
            } else {
                distribution_performance << "[-]";
            }
        }
