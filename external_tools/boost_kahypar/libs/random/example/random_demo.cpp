...
// Try boost_kahypar::mt19937 or boost_kahypar::ecuyer1988 instead of boost_kahypar::minstd_rand
typedef boost_kahypar::minstd_rand base_generator_type;
...
void experiment(base_generator_type & generator)
{
  ...
  // 1 and 6 inclusive.
  typedef boost_kahypar::uniform_int<> distribution_type;
  typedef boost_kahypar::variate_generator<base_generator_type&, distribution_type> gen_type;
  gen_type die_gen(generator, distribution_type(1, 6));

  // If you want to use an STL iterator interface, use iterator_adaptors.hpp.
  boost_kahypar::generator_iterator<gen_type> die(&die_gen);
  for(int i = 0; i < 10; i++)
    std::cout << *die++ << " ";
  std::cout << '\n';
}
...
int main()
{
  ...
  boost_kahypar::uniform_real<> uni_dist(0,1);
  boost_kahypar::variate_generator<base_generator_type&, boost_kahypar::uniform_real<> > uni(generator, uni_dist);
  ...
  boost_kahypar::uniform_int<> degen_dist(4,4);
  boost_kahypar::variate_generator<base_generator_type&, boost_kahypar::uniform_int<> > deg(generator, degen_dist);
