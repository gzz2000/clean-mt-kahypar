...
{
    int a[4];
    boost_kahypar::span s(&a[0], 4);
    BOOST_TEST_EQ(s.extent, boost_kahypar::dynamic_extent);
...
{
    int a[4];
    boost_kahypar::span s(boost_kahypar::span<int>(&a[0], 4));
    BOOST_TEST_EQ(s.extent, boost_kahypar::dynamic_extent);
    BOOST_TEST_EQ(s.data(), &a[0]);
    BOOST_TEST_EQ(s.size(), 4);
}
...
{
    int a[4];
    boost_kahypar::span s(boost_kahypar::span<int, 4>(&a[0], 4));
    BOOST_TEST_EQ(s.extent, 4);
    BOOST_TEST_EQ(s.data(), &a[0]);
    BOOST_TEST_EQ(s.size(), 4);
}
