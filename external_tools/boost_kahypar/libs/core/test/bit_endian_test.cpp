...
int main()
{
    boost_kahypar::uint64_t v = ( static_cast<boost_kahypar::uint64_t>( 0x01020304u ) << 32 ) + 0x05060708u;

    if( boost_kahypar::core::endian::native == boost_kahypar::core::endian::little )
    {
        unsigned char w[] = { 8, 7, 6, 5, 4, 3, 2, 1 };
        BOOST_TEST( std::memcmp( &v, w, 8 ) == 0 );
    }
    else if( boost_kahypar::core::endian::native == boost_kahypar::core::endian::big )
    {
