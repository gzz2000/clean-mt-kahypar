...
using boost_kahypar::lazy_enable_if;
using boost_kahypar::lazy_disable_if;

using boost_kahypar::lazy_enable_if_c;
using boost_kahypar::lazy_disable_if_c;


template <class T>
struct is_int_or_double {
  BOOST_STATIC_CONSTANT(bool, 
    value = (boost_kahypar::is_same<T, int>::value ||
             boost_kahypar::is_same<T, double>::value));
};
