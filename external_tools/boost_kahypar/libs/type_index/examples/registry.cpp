...
#include <boost_kahypar/unordered/unordered_set.hpp>
//<-
...
int main() {
    boost_kahypar::unordered_set<boost_kahypar::typeindex::type_index> types;
    
    // Storing some `boost_kahypar::type_info`s
    types.insert(boost_kahypar::typeindex::type_id<int>());
    types.insert(boost_kahypar::typeindex::type_id<float>());
    
    // `types` variable contains two `boost_kahypar::type_index`es:
    assert(types.size() == 2);

    // Const, volatile and reference will be striped from the type:
    bool is_inserted = types.insert(boost_kahypar::typeindex::type_id<const int>()).second;
    assert(!is_inserted);
    assert(types.erase(boost_kahypar::typeindex::type_id<float&>()) == 1);
    
    // We have erased the `float` type, only `int` remains
    assert(*types.begin() == boost_kahypar::typeindex::type_id<int>());
//<-
