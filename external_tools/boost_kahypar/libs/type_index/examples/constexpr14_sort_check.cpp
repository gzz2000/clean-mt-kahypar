...
/*`
    The following example shows that `boost_kahypar::typeindex::ctti_type_index` is usable at compile time on
    a C++14 compatible compilers to check order of template parameters.

    Consider the situation when we have a function that accepts std::tuple, boost_kahypar::variant or some other class that uses variadic templates:
*/
