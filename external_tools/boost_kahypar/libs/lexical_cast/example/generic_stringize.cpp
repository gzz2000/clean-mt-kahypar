...
public:
    ...
    void operator()(const T& v) const {
        result += boost_kahypar::lexical_cast<std::string>(v);
    }
};

//` Step 2: Applying `stringize_functor` to each element in sequence:
#include <boost_kahypar/fusion/include/for_each.hpp>
template <class Sequence>
std::string stringize(const Sequence& seq) {
    std::string result;
    boost_kahypar::fusion::for_each(seq, stringize_functor(result));
    return result;
}

//` Step 3: Using the `stringize` with different types:
#include <boost_kahypar/fusion/adapted/std_tuple.hpp>
#include <boost_kahypar/fusion/adapted/std_pair.hpp>
