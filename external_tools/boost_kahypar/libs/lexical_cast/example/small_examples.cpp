...
{
    log_message("Error " + boost_kahypar::lexical_cast<std::string>(yoko) + ": " + strerror(yoko));
}
...
void number_to_file(int number, std::FILE* file)
{
    ...
    buf_t buffer = boost_kahypar::lexical_cast<buf_t>(number); // No dynamic memory allocation
...
{
    return boost_kahypar::lexical_cast<int>(s.data() + pos, n);
}
