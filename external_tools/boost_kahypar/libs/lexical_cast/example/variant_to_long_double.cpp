...
struct to_long_double_functor: boost_kahypar::static_visitor<long double> {
    template <class T>
    long double operator()(const T& v) const {
        ...
        return boost_kahypar::lexical_cast<long double>(v);
    }
};

// Throws `boost_kahypar::bad_lexical_cast` if value of the variant is not convertible to `long double`
template <class Variant>
long double to_long_double(const Variant& v) {
    return boost_kahypar::apply_visitor(to_long_double_functor(), v);
}

int main() {
    boost_kahypar::variant<char, int, std::string> v1('0'), v2("10.0001"), v3(1);
