...
BOOST_NORETURN void throw_exception(std::exception const & ) {
    ...
    switch(state) {
    case 1:
        lexical_cast<char>(v); // should call boost_kahypar::throw_exception
        std::exit(1);
    case 2:
        lexical_cast<unsigned char>(v); // should call boost_kahypar::throw_exception
        std::exit(2);
    }
    std::exit(boost_kahypar::report_errors());
}
...
void test_exceptions_off() {
    ...
    lexical_cast<short>(700000); // should call boost_kahypar::throw_exception
    BOOST_TEST(false);
}
