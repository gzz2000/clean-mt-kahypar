...
void do_test_iterator_range_impl(const RngT& rng)
{
    ...
    BOOST_TEST_EQ(lexical_cast<boost_kahypar::ulong_long_type>(rng), 1u);
    BOOST_TEST_EQ(lexical_cast<boost_kahypar::ulong_long_type>(rng.begin(), rng.size()), 1u);
    BOOST_TEST_EQ(lexical_cast<boost_kahypar::long_long_type>(rng), 1);
    BOOST_TEST_EQ(lexical_cast<boost_kahypar::long_long_type>(rng.begin(), rng.size()), 1);
