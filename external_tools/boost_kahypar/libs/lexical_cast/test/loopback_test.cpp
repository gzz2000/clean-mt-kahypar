...
void test_round_conversion()
{
    ...
    std::string const epsilon_s = boost_kahypar::lexical_cast<std::string>(epsilon);
    BOOST_TEST(epsilon == boost_kahypar::lexical_cast<T>(epsilon_s));

    T max_ = (std::numeric_limits<T>::max)();
    std::string const max_s = boost_kahypar::lexical_cast<std::string>(max_);
    BOOST_TEST(max_ == boost_kahypar::lexical_cast<T>(max_s));

    T min_ = (std::numeric_limits<T>::min)();
    std::string const min_s = boost_kahypar::lexical_cast<std::string>(min_);
    BOOST_TEST(min_ == boost_kahypar::lexical_cast<T>(min_s));

    T max_div137 = max_ / 137;
    std::string max_div137_s = boost_kahypar::lexical_cast<std::string>(max_div137);
    BOOST_TEST(max_div137 == boost_kahypar::lexical_cast<T>(max_div137_s));

    T epsilon_mult137 = epsilon * 137;
    std::string epsilon_mult137_s(boost_kahypar::lexical_cast<std::string>(epsilon_mult137));
    BOOST_TEST(epsilon_mult137 == boost_kahypar::lexical_cast<T>(epsilon_mult137_s));
...
void test_msvc_magic_values()
{
    ...
    std::string magic_msvc_s = boost_kahypar::lexical_cast<std::string>(magic_msvc);
    BOOST_TEST(magic_msvc == boost_kahypar::lexical_cast<T>(magic_msvc_s));
}
