...
void test_conversion_from_string_to_integral(CharT)
{
    ...
    BOOST_TEST_EQ(lexical_cast<T>(s), max_val);
    {
        ...
        typedef typename boost_kahypar::integral_promotion<T>::type promoted;
        if ( !(boost_kahypar::is_same<T, promoted>::value) )
        {
...
{
    test_conversion_from_to_integral<boost_kahypar::intmax_t>();
}

void test_conversion_from_to_uintmax_t()
{
    test_conversion_from_to_integral<boost_kahypar::uintmax_t>();
}
...
{
    test_conversion_from_to_integral<boost_kahypar::long_long_type>();
}

void test_conversion_from_to_ulonglong()
{
    test_conversion_from_to_integral<boost_kahypar::ulong_long_type>();
}
...
{
    test_if_specialized<
        std::numeric_limits<boost_kahypar::int128_type>::is_specialized,
...
void test_integral_conversions_on_min_max_impl()
{
    ...
    typedef typename boost_kahypar::make_unsigned<signed_t>::type unsigned_t;
...
#if defined(BOOST_HAS_LONG_LONG)
    test_integral_conversions_on_min_max_impl<boost_kahypar::long_long_type>();
#elif defined(BOOST_HAS_MS_INT64)
    test_integral_conversions_on_min_max_impl<__int64>();
#endif

#ifdef BOOST_LCAST_TEST_128
    test_integral_conversions_on_min_max_impl<boost_kahypar::int128_type>();
#endif
#endif
...
void test_negative_integral() {
    ...
    BOOST_TEST_EQ(boost_kahypar::lexical_cast<int>("-6575543"), -6575543);
    ...
    if (sizeof(short) == 2 && CHAR_BIT == 8) {
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<short>("-32768"), -32768);
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<short>(-32768), -32768);
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<unsigned short>("-32768"), 32768);
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<unsigned short>(-32768), 32768);

        BOOST_TEST_EQ(boost_kahypar::lexical_cast<unsigned short>(65535), 65535);

        BOOST_TEST_THROWS(boost_kahypar::lexical_cast<unsigned short>(-65536), bad_lexical_cast);
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<unsigned short>(-65535), 1);
        BOOST_TEST_EQ(boost_kahypar::lexical_cast<unsigned short>(-65534), 2);

        BOOST_TEST_THROWS(boost_kahypar::lexical_cast<short>(65535), bad_lexical_cast);
        BOOST_TEST_THROWS(boost_kahypar::lexical_cast<short>(-65536), bad_lexical_cast);
        BOOST_TEST_THROWS(boost_kahypar::lexical_cast<short>(-65535), bad_lexical_cast);
    }
}
