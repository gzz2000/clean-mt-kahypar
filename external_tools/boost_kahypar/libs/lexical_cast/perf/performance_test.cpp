...
inline std::istream& operator>> (std::istream& in, boost_kahypar::array<char,50>& res) {
    in >> res.begin();
    return in;
}
}
...
static inline void test_lexical(const InT& in_val) {
    OutT out_val = boost_kahypar::lexical_cast<OutT>(in_val);
    (void)out_val;
}
...
static inline void test_ss_constr(const boost_kahypar::array<CharT, N>& in_val) {
    OutT out_val;
    std::stringstream ss;
    ss << in_val.begin();
    if (ss.fail()) throw std::logic_error("descr");
...
static inline void test_ss_noconstr(StringStreamT& ss, const boost_kahypar::array<CharT, N>& in_val) {
    OutT out_val;
...
struct structure_sscanf {
    ...
    static inline void test(BufferT* /*buffer*/, const boost_kahypar::array<CharT, N>& in_val, const char* const conv) {
        OutT out_val;
    ...
    template <class OutT, class BufferT>
    static inline void test(BufferT* /*buffer*/, const boost_kahypar::iterator_range<const char*>& in_val, const char* const conv) {
        OutT out_val;
...
static inline void perf_test_impl(const FromT& in_val, const char* const conv) {
    ...
    typedef boost_kahypar::chrono::steady_clock test_clock;
    test_clock::time_point start;
    typedef boost_kahypar::chrono::milliseconds duration_t;
    ...
    lexical_cast_time = boost_kahypar::chrono::duration_cast<duration_t>(test_clock::now() - start);
    ...
    ss_constr_time = boost_kahypar::chrono::duration_cast<duration_t>(test_clock::now() - start);
    ...
    ss_noconstr_time = boost_kahypar::chrono::duration_cast<duration_t>(test_clock::now() - start);
    ...
    printf_time = boost_kahypar::chrono::duration_cast<duration_t>(test_clock::now() - start);

    min_fancy_output(
                lexical_cast_time.count(),
                ss_constr_time.count(),
                ss_noconstr_time.count(),
                boost_kahypar::is_same<SprintfT, structure_fake>::value ? fake_test_value : printf_time.count()
    );
}
...
void string_like_test_set(const std::string& from) {
    ...
    perf_test<boost_kahypar::long_long_type, ssc_t>(from + "->long long", conv("100"), "%lld");
    ...
    perf_test<boost_kahypar::ulong_long_type, ssc_t>(from + "->unsigned long long", conv("100"), "%llu");
    ...
    perf_test<boost_kahypar::array<char, 50>, ssc_t>(from + "->array<char, 50>", conv("1.123"), "%s");

    perf_test<std::string, structure_fake>(from + "->string", conv("string"), "%Lf");
    perf_test<boost_kahypar::container::string, structure_fake>(from + "->container::string"
                                                        , conv("string"), "%Lf");
...
struct to_iterator_range {
    boost_kahypar::iterator_range<const char*>  operator()(const char* const c) const {
        return boost_kahypar::make_iterator_range(c, c + std::strlen(c));
    }
};

struct to_array_50 {
    boost_kahypar::array<char, 50> operator()(const char* const c) const {
        boost_kahypar::array<char, 50> ret;
...
int main(int argc, char** argv) {
    ...
    perf_test<std::string, structure_sprintf>("unsigned long long->string",  static_cast<boost_kahypar::ulong_long_type>(100), "%llu");
